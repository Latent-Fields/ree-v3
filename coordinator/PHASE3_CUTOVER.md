...
| `PHASE3_DISABLE_RUNNER_HEARTBEAT_WRITE` | runner + `runner_remote_control.py` | When `1`, ALSO suppress the LOCAL `write_heartbeat` + `write_commands_file` writes (not just the git push). Strict-superset of `PHASE3_DISABLE_RUNNER_HEARTBEAT_PUSH` -- setting `WRITE=1` makes `PUSH=1` effective regardless of its own value. **Hub-only flag**: needed only on the hub VM (`ree-cloud-1`) because it shares its `REE_assembly` checkout with `sync_daemon.phase3_heartbeat_writer`; on other workers a local write is harmless (different filesystem) and you should leave this unset so the file is still around for diagnostic poking. Setting this on the hub is what re-enables cloud-1 as a worker after the 2026-05-28 cutover-day `systemctl disable ree-runner` workaround. Per-experiment `git_push_status` (runner_status writes) is NOT gated by this flag because the runner reads its own status file for cross-restart continuity; the writer's clean-tree check tolerates the occasional once-per-experiment dirty state. |
