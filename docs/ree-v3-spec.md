...
| SD-091 / MECH-481: Coalition/Topology Control Substrate (PARTIAL, steps 1-3 of 5) | control_plane.coalition_topology_control -- STEPS 1-3 of the design doc's five-step implementation path IMPLEMENTED 2026-08-02 (chip `chip-20260802-sd091-coalition-control`; new self-contained `ree_core/control_plane/coalition.py`; module inert -- nothing in `ree_core` consumes it yet). Landed: (1) `CoalitionRequest` primitive + `SENSORY_RESAMPLE` / `PROVENANCE_CHECK` templates (the two MECH-481 taxonomy entries with clear reads on `operating_mode`); (2) the `CoalitionController` dispatch primitive; (3) request-lifetime bookkeeping + telemetry hooks. NOT DONE (blocks MECH-481 4-arm falsifier): step 4 (agent-loop mount-point for the two templated request types), step 5 (agent-loop consumer that reads `operating_mode` and gates on it). SD-091 + MECH-481 stay candidate / v3_pending / substrate_conditional; `substrate_queue.json` `sd_091_coalition_topology_control` entry recorded PARTIAL landing. | Substrate landed 2026-08-02 (ree-v3 main `71182f3`; REE_assembly master `01b054d5d7` records the partial). MECH-481 4-arm falsifier NOT queued -- gated on steps 4-5. PROMOTES NOTHING. |
