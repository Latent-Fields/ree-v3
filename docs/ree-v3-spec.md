...
| SD-024 / MECH-232 | DA-modulated RBF density (hippocampal_module.da_modulated_rbf_density) -- DA at reward encounters allocates a local jittered CLUSTER of RBF centers + optional per-center bandwidth narrowing on the BENEFIT terrain (representational expansion), with intensity SPLIT across the cluster so summed benefit VALUE is conserved while representational DENSITY rises; `compute_local_density` reads that density WEIGHT-INDEPENDENTLY (the MECH-232 discriminator against a valence tag). Harm/safety fields never DA-modulated (MECH-233 asymmetry preserved). All new config no-op default -> bit-identical OFF (full pytest tests/ 1475 passed; 13 SD-024 contracts); no phased training (no encoder head); MECH-094 hypothesis_tag gate inherited via `accumulate_benefit`. | Implemented 2026-07-16 (ree-v3 main `402285a`; V3-EXQ-766 DIAGNOSTIC validation queued -- leg-1 representational expansion + leg-2 CRUX approach-without-explicit-gradient via weight-zeroing persistence; cloud PASS `766a` on `ree-cloud-2` 2026-07-16 adjudicated `verified` and PROMOTED MECH-232 candidate->provisional in the 2026-07-16d governance cycle -- REE_assembly master `46db726a00`; SD-024 itself carries an implementation_note). |
| SD-025 / ARC-057 (component 2) | Hippocampal curiosity drive (hippocampal_module.curiosity_drive) -- 2nd component of ARC-057, on the SD-024 density substrate; `_score_trajectory` subtracts `curiosity_weight * mean(density * (1 - familiarity))` from the CEM terrain score; density = `compute_representational_density` (SD-024 weight-independent -> follows representational QUALITY, not a valence gradient); familiarity = new `FamiliarityTracker` (proximity-weighted visit-count EMA raised on WAKING visits only via MECH-094 hypothesis_tag gate, anti-perseveration discount). New `HippocampalConfig` fields all no-op default -> bit-identical OFF (full pytest tests/ 1488 passed; 7 SD-025 contracts); no phased training. SUBSTRATE-CONSTRAINT: buildable now, but the full ARC-057 ecological approach-emergence claim (SD-024 x SD-025 interaction, neither alone sufficient) is ENV-CONSTRAINED (CausalGridWorld cannot test it faithfully); validation is scoped to the DRIVE MECHANISM (propagation into CEM selection + familiarity anti-perseveration), NOT the interaction claim. | Implemented 2026-07-16 (ree-v3 main `c886d7c`; REE_assembly master `dfe49a940c` -- claims.yaml SD-025 implementation_note + sd_024 doc updated); V3-EXQ-767 DIAGNOSTIC PASS 2026-07-16 was flagged `vacuous_pass` (saturating binary `pref_dense` argmin gate pinned at 1.0/8 seeds zero-variance) and adjudicated `non_contributory / measurement_degeneracy` in governance cycle 2026-07-16d (REE_assembly master `e40362ebb7`); RE-OPERATIONALIZED as V3-EXQ-767a (supersedes 767; continuous CEM score-margin `sparse - dense`; cloud-pinned `ree-cloud-3`), queued on ree-v3 main `521ad16` 2026-07-16. SD-025 stays candidate. |
...
| SD-068: sleep.consolidation_pipeline_lesion_harness | Experiment-layer harness (no ree_core change) instrumenting the MECH-120 (SWS denoising) -> MECH-121 (NREM slot-filling) -> MECH-123 (REM precision) offline-consolidation pipeline with per-phase damageable + per-phase functional readouts, so the MECH-168 / INV-047 / MECH-169 staged-decline-under-uniform-damage falsifier is buildable. Injects known clean content onto each phase's operative substrate (context_memory slots / consolidation params / E3 precision reference; V3-EXQ-702 injected-content precedent, sidesteps the failure_autopsy_V3-EXQ-538a encoding-starvation ceiling) -> one UNIFORM RMS-scaled Gaussian sigma -> per-phase denoising-SNR / transfer-fidelity / precision-calibration-error against the injection. Non-vacuity carried by staging-order + a REM passthrough-nudge-vs-generative-pass contrast, NOT a synthetic cross-phase content-propagation pipe (the three phases operate on disjoint state). MECH-121 hold RESPECTED (EXPERIMENT_PURPOSE=diagnostic; NREM leg is plumbing-fidelity only, does not tag MECH-121 as promotion evidence). Backward-compatible; pure instrumentation. | Implemented 2026-07-17 (ree-v3 main `2472aef` harness + `da873a1` REM generative-fidelity readout replacing terrain-variance proxy; REE_assembly master `27c7cae00b` + `c8696cce97` + `c95b2f4a0f`; V3-EXQ-778 staged-damage diagnostic PASS 2026-07-17T160320Z + V3-EXQ-778a 8-seed statistical staging power PASS 2026-07-17T163507Z; observed tolerance order (nrem, rem, sws) stable across seeds; MECH-168 / INV-047 REM generative-gain non-vacuity banked). Design doc: `REE_assembly/docs/architecture/sd_068_consolidation_lesion_harness.md` |
| SD-069: control_plane.phasic_surprise_burst | LC-NE PHASIC complement to MECH-313 noise_floor (tonic) on the SAME E3 softmax temperature channel -- the substrate that makes MECH-063 sub-claim (ii) (tonic/phasic split) behaviourally testable. Pure-arithmetic regulator (`ree_core/regulators/phasic_surprise_burst.py`; sibling to `noise_floor.py` MECH-313 / `broadcast_override.py` SD-037; no nn.Module, no learned params). Per-tick surprise source -> event iff surprise >= trigger_ratio x EMA baseline -> geometrically-decaying burst envelope -> temperature_delta -> `e3.select(candidates, combined_T)` at `select_action`; kept SEPARATE from the MECH-313 tonic lift in the control-vector + score-bias-decomp diagnostics. Reuses the MECH-104 volatility-surprise lit basis (Aston-Jones & Cohen 2005 phasic mode) but routes to the SELECTION softmax, NOT the ARC-016 commit gate -- so it does NOT implement MECH-104's `control_plane.volatility_interrupt` claim (left untouched). Sharp-surprise source (2026-07-17): `phasic_burst_signal_source="instantaneous_pe"` reads a new no-op `e3.last_instantaneous_pe` = raw per-tick PE-MSE captured BEFORE the running-variance EMA smoothing folds it in, so genuine spikes survive (default `"running_variance"` keeps the pre-change smoothed source; invalid values raise `ValueError` at construction). Bit-identical OFF (contract C8, n_events==0). MECH-094: simulation_mode returns cached burst without state advance. | Implemented 2026-07-17 (ree-v3 main `ab6ff33` + `f386179` sharp source + `50b0e69` V3-EXQ-779 queue; REE_assembly master `d0a858ad96` architecture doc + landing record + `d590cd47d5` sharp-source note; 17 SD-069 contracts pass; full `tests/contracts` 1462 pass with master OFF; injected-surprise smoke: pre-commit softmax entropy drops on the event 0.80->0.06 and decays over the tail). V3-EXQ-779 MECH-063 sub-claim (ii) tonic-vs-phasic dissociation validation (777-harness pattern; phasic_burst_signal_source="instantaneous_pe" on PHASIC-ON arms; P0 readiness gate on real-surprise event count) user-routed `blocked_substrate` at queue time -- no cloud run in the 2026-07-17T18:21Z window. Design doc: `REE_assembly/docs/architecture/sd_069_phasic_surprise_burst.md` |
...
| MECH-448 / ARC-107: rank-preserving F->eligibility demotion (LEAD lever of the basal-ganglia E3-selector constitution) | ethics_engine_3.rank_preserving_f_to_eligibility_demotion -- F decides who is ELIGIBLE, not who wins. New `_f_eligibility_envelope` helper computes a graded divisive-normalisation share `elig[i] = merit[i] / (sigma + merit.sum())`; eligibility is the absolute-share floor; the eligible set is an F-RANK PREFIX (rank-preserving). New `f_demotion` mode of the shortlist-then-modulate block reuses the EXISTING within-eligible `_modulatory_accum` arbitration with F REMOVED from the final argmin. 3 E3Config flags `use_f_eligibility_demotion` / `f_eligibility_envelope_floor` (0.30 default) / `f_eligibility_dn_sigma` no-op default, bit-identical OFF. SAFETY: clearly-harmful candidate has near-zero merit -> below floor -> excluded; contract-verified an overwhelming modulatory pull toward an excluded harmful candidate never selects it. THE FIRST major worked application of ARC-106 (brain-like construction; grounding ladder L2 + divergence ledger anchoring the rank-ALTERING-at-commit departure from canonical DN; psychiatric-failure-mode column: envelope-too-wide -> impulsivity/mania, envelope-too-tight -> bradykinesia/avolition). 10/10 new contracts + 8/8 preflight. | Substrate landed 2026-06-20 (ree-v3 main 4c9b3c9; REE_assembly master 5bdf97c105; design doc `docs/architecture/mech_448_f_eligibility_demotion.md` + design note `evidence/planning/arc_107_selector_constitution_design_2026-06-20.md`). **V3-EXQ-689d PASSed 2026-06-21** on the GAP-A-ready foraging substrate (BG DN-envelope arm 0.938 vs F-route top-k control 0.371, ~2x); MECH-448 PROMOTED candidate -> provisional. F-dominance selection-readout route (569i top-k / commit-T / near-tie family) EXHAUSTED -- downstream retests all run with the demotion as the new constant. |
| MECH-448 AMEND: channel-adaptive (mean-relative) eligibility floor (collapses per-channel hand-floor dances into one knob) | ethics_engine_3.rank_preserving_f_to_eligibility_demotion.channel_adaptive_floor_amend -- the fixed absolute 0.30 floor was tuned to PASS on the GAP-A foraging bank (V3-EXQ-689d) but each downstream channel has a different F-merit distribution, so the same fixed floor mis-fires (V3-EXQ-654h all-admit no-op on the arc_062 rule-apprehension bank; 485i/485j needed bespoke per-seed envelope-floor recalibration on the OFC bank). New `use_f_eligibility_adaptive_floor` flag + `f_eligibility_adaptive_mean_factor` (default 1.0) replaces the fixed floor with `mean_factor * elig.mean()` -- scale-invariant per channel, conflict-graded preserved, excluded_count > 0 by construction on any non-uniform field, RANK-PRESERVING. Collapses ~5 per-channel hand-floor dances (654h + 485i/485j + pending 625/445/687) into ONE global knob. No-op default (double-guarded: `use_f_eligibility_demotion` also OFF by default) -> bit-identical OFF. 16/16 MECH-448 contracts (8 new) + 8/8 preflight + 48/48 E3-cluster pass. | Amend landed 2026-06-21 (ree-v3 main 7b44131; REE_assembly master 9a5ddc0388; design doc `docs/architecture/mech_448_f_eligibility_demotion.md` channel-adaptive amend section). **V3-EXQ-689e** channel-adaptive envelope readiness diagnostic queued + END-TO-END VALIDATED at toy 8-ep probe: ADAPTIVE excl>0_frac=1.0 env~17/32 rankpres=1.0 on BOTH channels vs FIXED-0.30 excl>0_frac~0.01-0.02 env~31/32 (the 654h all-admit no-op reproduced). PROMOTES NOTHING (MECH-448 stays at the validation stage). |
| MECH-449 / ARC-107: Go/No-Go eligibility constitution (the OPPONENCY leg of the basal-ganglia E3-selector constitution; generalises MECH-260) | selection.go_nogo_eligibility_constitution -- component 1 of the ARC-107 BG constitution; the missing OPPONENCY leg complementing MECH-448 (component 3, pallidal permission gate). MECH-448's rank-preserving demotion is order-preserving over F, so it structurally CANNOT exclude an F-eligible-but-undesirable candidate on a non-F axis (safety / staleness / perseveration / low-viability) -- only an active No-Go can. Bounded Go/No-Go pressure SET as methods on `E3TrajectorySelector._go_nogo_eligibility_gate` (NO parallel module per ARC-106 G2). No-Go drops candidates orthogonal to F-rank; bounded Go re-admits a demoted channel; fail-open guard (safety never overridden). Perseveration axis REUSES MECH-260 (dACC suppression routed in) per reuse-before-duplicate. SECOND major worked application of ARC-106 (Kravitz 2010 D1/D2 opponency; Mink 1996 focal-go + surround-no-go; Maia/Frank 2011). 8 E3Config flags `use_go_nogo_constitution` + per-axis floors + Go threshold + max-promote + protect-min-eligible, all no-op default + bit-identical OFF. 6/6 new contracts + 8/8 preflight + 148 E3-cluster pass. | Substrate landed 2026-06-21 (ree-v3 main 829e51e; REE_assembly master 7deb4a2aff; design doc `docs/architecture/mech_449_go_nogo_constitution.md` + design note `evidence/planning/arc_107_selector_constitution_design_2026-06-20.md` s3.2 + s6b completeness ledger). Build gate cleared on BOTH faces: selection-face V3-EXQ-689f (No-Go-necessity falsifier; admit 0.866 / No-Go 0.0 / rank-preserving 1.0; `demotion_insufficient_nogo_necessary`) + behavioural-face the 485e->485k six-autopsy lineage. **V3-EXQ-689g** (selection-face conversion ablation falsifier; claim_ids=[MECH-449, ARC-107]; priority 430) queued as the validation falsifier. MECH-449 stays candidate / substrate_conditional (the ablation falsifier gates promotion). |
| Commit/release-DURATION lever: graded natural-commit-occupancy release (rung-6 of `f_dominance_conversion_ceiling`; PARALLEL to MECH-448) | control_plane.natural_commit_occupancy_release -- the commit/release-DURATION face of the F-dominance front, PARALLEL to the selection-face MECH-439/MECH-448 levers. NOT blocked on GAP-I / V3-EXQ-689c (a dead-end selection-face parametric retest); the duration face is its own lever. New `ree_core/policy/natural_commit_urgency.py` regulator (reuses `BetaGate.committed_run_length` -- NO parallel latch per ARC-106 G2 reuse-before-duplicate); two D1-faithful release modes both togglable so V3-EXQ-460i discriminates which lifts: (1) URGENCY (Thura/Cisek): per-tick `decisiveness_scale = 1 + gap_entry_sensitivity * gap_norm_at_entry`; `urgency += urgency_rate * decisiveness_scale`; fire at >= release_bound (gap-scaling LOAD-BEARING -- F-decisive commits accrue urgency faster); (2) ACTION-EXTENT (Jin): release when the committed trajectory's action sequence completes (renders maintenance co-extensive with the executing action). Sibling LATCH-HOLD amend (V3-EXQ-460i gate amend; landed 2026-06-21) establishes a sustained natural-commit baseline for the release to act on -- yields to closure-refractory + MECH-091 urgency interrupts + the rung-6 release itself. Bit-identical OFF; 10 new release + 7 new hold contracts + 7/7 preflight + 1169 full PASS. | Substrate landed 2026-06-20 (ree-v3 main ab2c1a9; REE_assembly master fd98756f87; design doc `docs/architecture/natural_commit_occupancy_release.md`); LATCH-HOLD amend landed 2026-06-21 (ree-v3 main + LATCH-HOLD amend section in the same design doc). V3-EXQ-460i drained with the prior 6-item queue; the gate is now squarely the selection-face BG-constitution. PROMOTES NOTHING; MECH-446/445/SD-034 stay candidate / v3_pending / pending_retest_after_substrate. |
| DR-12 (self_model_v4:SELF-4): E2 forward-PE -> E3 trajectory-scoring confidence down-weight (FIRST V4 SUBSTRATE BUILD) | ethics_engine_3.pe_conditioned_confidence_weighting -- THE FIRST-EVER V4 substrate build (generation:v4, off the V3 critical path; PROMOTES NOTHING in V3). E3 currently scores purely from the E2-rolled-out world_states, trusting them as if E2 were reliable everywhere. DR-12 adds an E2-forward-PE confidence DOWN-WEIGHT alongside the existing ARC-016 (world-forward PE) + MECH-111 (E1 PE) signals. New `_pe_confidence_penalty` helper threaded per-candidate via `select(e2_forward_pe_per_candidate=[K])` -- a UNIFORM scalar is argmin-invariant (contract-pinned), only varying per-candidate PE can change the committed argmin. CALLER-SUPPLIED per-candidate PE source in v1 (the DR-12 pilot is a CONTROLLED probe assigning known per-candidate PE); ecological region-PE auto-source documented as a follow-on. 4 E3Config flags `use_pe_confidence_weighting` + `pe_confidence_weight` + `pe_confidence_mode` (linear/saturating) + `pe_confidence_scale`, all no-op default + bit-identical OFF. PRECEDENT: V4-EXQ-001 is the first V4 experiment (sets a NEW V4 architecture_epoch + V4 run_id suffix). | Substrate landed 2026-06-17 (ree-v3 main; design doc `docs/architecture/dr12_pe_conditioned_e3_confidence.md`; plan node self_model_v4:SELF-4). V4-EXQ-001 (DR-12 pilot) queued; PROMOTES NOTHING (MECH-215 stays candidate / implementation_phase=v4 / unchanged). |
...
| ARC-110 + ARC-109 + MECH-452: parallel segregated cortico-BG-thalamic loops (motor / associative / limbic) + S2 in-layer null + D1/D2 split + loop-local traces (the 4th basal-ganglia missing piece; cluster autopsy 704b+706b verdict = single-arena collapse) | selection.parallel_segregated_loops -- IMPLEMENTED 2026-06-27. The full BG-thalamo-cortical loop substrate the conversion-ceiling lineage converged on from FOUR angles (V3-EXQ-700b/700c learned-gating+settling, V3-EXQ-704b finer-channel, V3-EXQ-706b double-gated curiosity); all four showed the V3 single E3 foraging arena structurally denies (a) committed-action-class conversion from non-motor channels and (b) a valid same-layer null. Gate CLEARED by V3-EXQ-704b FAIL-to-convert (the pre-registered positive-evidence-FOR-ARC-110 outcome: compression is NOT the binding constraint). Strict additive extension to `e3_selector.py` behind no-op-default flags; ARC-108 w_chan / MECH-451 finer / MECH-450 settling / single-arena argmin are BYTE-IDENTICAL OFF. (1) `_segregated_loop_arbitrate` REPLACES the single-arena within-eligible argmin (gated on `use_loop_segregation`): MOTOR loop = F (raw_scores); ASSOCIATIVE = dACC + lPFC; LIMBIC = OFC + liking + vigour (via the active finer/base channel registry). Each non-motor loop accumulates its own subset, optionally settles (MECH-450 W_lat), splits into D1/D2 (ARC-109); cross-loop arbitration runs AFTER via Haber's ascending dopamine spiral (limbic -> assoc -> motor: weighted sum with `loop_segregation_spiral_gain_assoc / _limbic / _motor_authority`). Each loop's preference is NORMALISED (`loop_segregation_normalize`, zscore default) BEFORE arbitration -- strips F's raw-magnitude advantage, the conversion mechanism (F dominates only the motor loop). Runs STRICTLY within the F + MECH-449 Go/No-Go eligible set -> a non-motor loop can FLIP the within-eligible winner but can NEVER re-admit a suppressed candidate (safety inherited from the envelope, orthogonal-to-F guarantee). Meaningful only WITH finer-channel gating on (the base registry has one score_bias lump). (2) `_loop_inlayer_null` (gated on `loop_segregation_noise_on`): replaces each NON-motor loop accumulator with a magnitude-matched random-structure (gaussian) perturbation -- range = alpha * the real loop range -- injected at the SAME eligibility/settling field the loops settle on (NOT policy softmax temperature, the decoupled 700-lineage null). `noise_verified_lifting` becomes a MEANINGFUL non-vacuity precondition (the substrate-level fix for the measurement ROOT the 704b/706b autopsies named). Motor (F) is NEVER nulled. Selection-only (no memory write; MECH-094 not engaged). (3) ARC-109 D1/D2 population split `_d1_d2_split` (gated on `use_d1_d2_population_split`): each loop's accumulator (COST) decomposes into Go/D1 = relu(-accum) potentiated by DA (LTP) + No-Go/D2 = relu(+accum) depressed by DA (LTD); net cost = D2 - D1; da = the shared ARC-108 value baseline V-hat_t (tanh-squashed). At da==0 net == accum EXACTLY (bit-identical; dissociation is earned only once da != 0). The two populations make approach-avoidance CONFLICT (both high) dissociable from indifference (both low) -- the representational distinction the additive scalar destroys (OCD/Parkinson/dyskinesia CSTC axis substrate, the ARC-106 EARNS). (4) MECH-452 loop-local eligibility traces (gated on `use_loop_local_eligibility_traces`): credit a channel for the shared signed-RPE delta_t ONLY if its loop's within-loop winner matched the committed action (the loop "voted for" the outcome), so credit stays loop-local under one broadcast dopamine signal. Non-degeneracy diagnostics: `loop_committed_neq_motor_winner`, `loop_cross_loop_winner_disagreement`, `loop_assoc_pref_range / loop_limbic_pref_range`, `loop_d1_d2_conflict_signal`, `loop_local_credited_channels` (a loop pinned to the motor winner is a vacuous split -> substrate_not_ready_requeue). THIRD major worked application of ARC-106 (grounding ladder L1 functional translation of Alexander/DeLong/Strick + Haber spiral; divergence ledger rows per loop; per-loop ablation falsifier; psychiatric failure modes named). | Substrate landed 2026-06-27 (ree-v3 main c969621; REE_assembly master 72b69bc2ba; design doc `docs/architecture/sd_v4_loop_segregation.md`). **V3-EXQ-707** (ARC-110 parallel segregated loop-segregation substrate-readiness validation; 4 arms x 6 seeds A0_SINGLE_ARENA / A1_LOOPS / ARM_NOISE / ARM_DROP_LIMBIC; PRE-REGISTERED decisive: A1 committed-class entropy strict-above A0 AND valid in-layer null with live cross-loop variance => supports ARC-110; valid null verify-lifts but loops vary without conversion => weakens (ceiling intrinsic); non-degeneracy self-routes substrate_not_ready_requeue; priority 400; queued 2026-06-27; claimed ree-cloud-2 11:10Z; PROMOTES NOTHING) is the falsifier. ARC-110 + ARC-109 + MECH-452 stay candidate / substrate_conditional / v3. |
| MECH-440 + MECH-441: state-conditioned exploration -- propagating selection-head weight noise (NoisyNet) + model-disagreement directed curiosity (RND/Plan2Explore) (the mechanistic refinement of MECH-313's NON-PROPAGATING temperature floor + MECH-314's broadcast EMA curiosity) | selection.state_conditioned_self_annealing_noise_floor + selection.model_disagreement_directed_curiosity -- both IMPLEMENTED 2026-06-27. Built on top of the same-day-landed ARC-110 substrate per the user-confirmed CPKT-TONIC-EXPLORATION-NOISE BUILD decision (adjudicator recommended defer-pending-ARC-110; user overrode to BUILD-BOTH; biology-before-formal gate PASSED). (1) MECH-440 NoisyNet propagating selection-head weight noise (`ree_core/policy/noisy_selection_head.py`): factorised-Gaussian WEIGHT NOISE built per-candidate from each candidate's first-action vector and added into `_modulatory_accum` BEFORE the within-eligible argmin (and into the ARC-110 segregated-loop `final`), so it PROPAGATES into the committed action -- unlike MECH-313's NON-PROPAGATING softmax-temperature floor that V3-EXQ-687 ruled `r1a_entropy_only_artefact`. mu FROZEN at 0 (pure noise injector -- isolates the falsifier). State-conditioned via the action activation; SELF-ANNEALS via a LOCAL confidence EMA on (1 - F-gap_norm). Lazy-built in e3_selector (action_dim from the candidate width). Config (E3Config): `use_noisy_selection_head` (False, bit-identical OFF), `noisy_selection_sigma_init` (0.0 -> exactly-zero output -> bit-identical even ON), `noisy_selection_weight`, `noisy_selection_anneal{,_floor,_ema_alpha}`. MECH-094: zero perturbation on simulation ticks. (2) MECH-441 model-disagreement directed curiosity (`ree_core/policy/model_disagreement.py`): a standalone SMALL K-head ensemble of ResidualHarmForward delta-predictors over (z_world, action); per-candidate cross-head VARIANCE feeds E3 selection as a propagating per-candidate curiosity BONUS (a cost reduction, lower=preferred) added into `_modulatory_accum` -- unlike the V3-EXQ-590a broadcast EMA, this is per-candidate so it can change the committed argmin (WITHIN the F-eligible set; safety inherited). Self-annealing is INTRINSIC (variance -> 0 as the heads train). Built at the agent level only when `use_model_disagreement_curiosity` AND `n_disagreement_heads>=2`; per-candidate disagreement computed in select_action and passed via the version-layering-guarded `model_disagreement_per_candidate` kwarg. Config: E3Config `use_model_disagreement_curiosity`, `model_disagreement_weight` (0.0), LatentStackConfig `n_disagreement_heads` (0 = not built). REQUIRES PHASED TRAINING: each head trains on the FROZEN z_world target via `disagreement_ensemble.train_step()` (P1 stop-gradient). MECH-094: waking-only no_grad read. ARC-106 divergences logged: (i) per-parameter sigma is one level below biology's systems-level tonic/phasic LC-NE mode gate; (ii) sigma self-anneals via REE's LOCAL confidence EMA, not NoisyNet's RL gradient (REE does not backprop through E3 selection). | Substrate landed 2026-06-27 (ree-v3 main a65609b; REE_assembly master 6e096b8448; design doc `docs/architecture/state_conditioned_exploration_noise_floor.md` #mech-440 / #mech-441; decision-of-record `evidence/decisions/cpkt_tonic_exploration_noise_build_decision_2026-06-27.md`). **V3-EXQ-708** (MECH-440 noisy-selection-head propagation falsifier on the 569i top-k + MECH-448 demotion SOTA stack; 4 arms A0_OFF / ARM_TEMP=687 matched-pre-commit-variance temperature control / ARM_NOISE_SINGLE / ARM_NOISE_LOOPSEG; only swept factor = exploration-injection site; PRIMARY DV committed-action-class entropy; SECONDARY pre-commit sampling-class entropy thrash-vs-carve; loop-seg OFF vs ON disambiguates injection-site fix vs single-arena/ARC-110 axis; priority 380; queued 2026-06-27; claimed ree-cloud-3 12:05Z; PROMOTES NOTHING) is the falsifier. MECH-441 falsifier HELD (`blocked_substrate`, claims.yaml note + carry-forward chip) gated on ARC-110 validation V3-EXQ-707 returning contributory -- the 706b autopsy already proved the curiosity channel works on a sufficient substrate; running MECH-441 before ARC-110 is validated would re-derive the arena ceiling (vacuous FAIL). MECH-440 + MECH-441 stay candidate / substrate_ceiling / v3_pending. |
...
| ARC-110 C2 RELEASE: per-named-channel range-preserving routing (the loop-segregation amend that finally makes the LIMBIC loop carry per-candidate range, so ARM_DROP_LIMBIC can differ from A1_LOOPS -> C2 limbic load-bearing testable) | selection.parallel_segregated_loops.named_channel_routing -- IMPLEMENTED 2026-06-28 via /implement-substrate. The 707/707a runs left C2 (limbic load-bearing) SUBSTRATE-BLOCKED on the same root MECH-191 names: the named cortical bias HEADS (OFC / dACC / lPFC / vigour / liking) emit per-candidate-FLAT output (OFC input range ~0.028 -> output range EXACTLY 0.0; the phasic-externalisation gap), so under per-loop zscore the LIMBIC loop carried no per-candidate competition and ARM_DROP_LIMBIC came up byte-identical to A1_LOOPS on every seed -- the 707 limb manifest's C2 was untestable, the 707a c2_drop_differs_from_a1 diagnostic flagged exactly this. Fix substitutes the segregated-loop arbitration's view of each NAMED channel with that channel's per-candidate REPRESENTATION routed through `project_channel_range` (range-preserving, the same GAP-A path that keeps `route` phasic); the legacy flat bias-head scalar is replaced ONLY in the segregated-loop accumulation -- the `_lcg_terms` eligibility traces, the authority/shortlist `_modulatory_accum` recompose, and the F-score commit path are UNCHANGED. So the limbic loop now carries REAL per-candidate range while every existing pathway runs byte-identical OFF. New no-op-default flag `E3Config.use_named_channel_routing`. Implementation: `e3_selector.score_bias_channel_routed` kwarg -> `loop_term_override` in `_segregated_loop_arbitrate` + 2 new diagnostics `loop_named_channel_routed_ranges` + `loop_limbic_routed_max_range` (the C2 non-degeneracy gate signal); `agent.py` captures each named channel's per-candidate REPRESENTATION (ofc/lpfc summaries; liking goal-proximity; vigour first-action one-hots; dacc payoff/effort; gated_policy summaries) -> `project_channel_range` -> routed kwarg. Selection-only (MECH-094 not engaged). Verified end-to-end: arbitrate-level test proves flat-terms reproduce 707 byte-identity (limbic range 0, DROP==A1) AND routed terms give limbic range 2.08 + DROP!=A1; smoke 4/4 arms exit 0; 9/9 new contracts `TestNamedChannelRoutingC2Release` + `TestRoutedRepsReachSelectorThroughAgent` PASS; MECH-451 contracts 12/12 unaffected. | Substrate landed 2026-06-28 (ree-v3 main 64f20b6; REE_assembly master 728061a5e5; design doc `docs/architecture/sd_v4_loop_segregation.md` C2-RELEASE section + status). **V3-EXQ-707b** (priority 420; `claim_ids=[ARC-110]`; supersedes V3-EXQ-707a + V3-EXQ-707; enables `use_named_channel_routing` + the limbic input modules + a per-NAMED-channel `named_channel_routing_live` non-degeneracy precondition evaluated BEFORE scoring C2 -- so vacuous DROP==A1 self-routes substrate_not_ready_requeue, NEVER a false weakens; C1 (A1 strict-above max(A0, valid in-layer null ARM_NOISE) + margin) + the 707a in-layer-null LIVENESS gate UNCHANGED; same 4-arm matched envelope; queued 2026-06-28; claimed ree-cloud-2 18:15Z; PROMOTES NOTHING) is the falsifier. ARC-110 stays candidate / substrate_conditional / v3. |
| MECH-451 finer-channel-gating substrate defect FIX (Defect 1: the named decomposition was DEAD substrate-wide because the agent.py builder gate read the wrong config attr) | selection.intermediate_channel_granularity.builder_gate_fix -- DEFECT-FIXED 2026-06-28. ROOT (V3-EXQ-707 autopsy): MECH-451 _fcg_channels builder at `agent.py:5238` gated on top-level `self.config.use_finer_channel_gating` (NEVER set anywhere in ree_core; always False), while the consumer gate at the e3.select() call site reads `config.e3.use_finer_channel_gating` (correct). Net: `_fcg_channels` was ALWAYS None -> the named channels (ofc/dacc/lpfc/vigour/liking/gated_policy) NEVER reached the selector; only the lumped residual/mech341/route did. EVERY finer-channel experiment (704/704b/706*/707/708) ran with MECH-451's NAMED decomposition DEAD -- ARM_A2_FINER_CHANNELS was only a residual/mech341/route 3-way split, NOT the named decomposition the claim asserts. MECH-451 is effectively UNTESTED (pending_retest_after_substrate). NB a genuine retest is itself gated on the named bias heads carrying per-candidate range -- they currently emit per-candidate-FLAT output (MECH-191 phasic gap), the same root the ARC-110 C2 RELEASE above closes for the segregated-loop arbitration. Fix: builder reads `config.e3.use_finer_channel_gating` matching the consumer. 4 regression tests added (limbic input modules; finer channels reach selector; finer-vs-base channel reuse). MECH-451 contracts 12/12 + 8/8 preflight + the 9 new ARC-110 C2 RELEASE contracts all PASS. | Defect fixed 2026-06-28 (ree-v3 main 81f2898; REE_assembly master cfcb81b5c7 annotates claims.yaml ARC-110.validation_note + MECH-451.defect_note + sd_v4_loop_segregation.md "VALIDATION + DEFECT 2026-06-28" + substrate_dependencies.json + 707 manifest). The full MECH-451 retest is a separate `/queue-experiment` session GATED on the same MECH-191 phasic-externalisation gap the ARC-110 C2 RELEASE closes -- run the MECH-451 retest only AFTER the named bias heads' per-candidate REPRESENTATIONS reach the finer-channel decomposition (the same routing the C2 RELEASE installs for the loop arbitration). MECH-451 stays candidate / substrate_ceiling / pending_retest_after_substrate. PROMOTES NOTHING. |
...
| SD-094: causal_grid_world waypoint-arrival + hazard-free contamination fix | environment.causal_grid_world_bug_fixes -- IMPLEMENTED 2026-08-03 (ree-v3 main `a95e103`; REE_assembly master `a906cc0824` substrate_queue status pending_implementation -> implemented_pending_validation). Two real bugs in the shared grid-world env, both starving MECH-428/EXQ-884-class experiments of the data they needed: (a) the agent's own grid-position marker overwrote the "arrived at waypoint" signal on first transit, so `transition_type` stayed `none` and the sequence stalled forever (reproduced live before fixing); (b) a background self-contamination effect killed the agent early even in hazard-free test setups (EXQ-884 config: dead at 74/49/52 of 400 steps, health=0.0, num_hazards=0 -- entirely self-inflicted). Substrate landing only. | Substrate landed 2026-08-03 (recovered from a crashed worker; status flip landed separately in `a906cc0824` after the coordinator missed the flip). Unblocks the V3-EXQ-884 MECH-428 subgoal-bootstrap successor re-run flagged by `failure_autopsy_V3-EXQ-884_2026-08-03` as a substrate bug (not claim evidence). PROMOTES NOTHING. |
| SD-MECH267-HORIZON-DEPTH: Mode-Conditioned Horizon-Depth Modulation | hippocampal.mode_conditioned_horizon_depth -- IMPLEMENTED 2026-08-02 via `/implement-substrate`. Extends the MECH-267 mode-conditioned hippocampal proposals substrate with a per-`operating_mode` horizon-depth multiplier so committed_run-mode candidates carry systematically deeper rollouts than exploit_run-mode candidates (the missing lever the V3-EXQ-869 MECH-267 content-persistence failure autopsy named as blocking). No-op-default (multiplier=1.0 -> bit-identical OFF); consumers wired at the CEM candidate generation site. Substrate landing only. | Substrate landed 2026-08-02 (ree-v3 main `e0117ee`; REE_assembly master `37d1da187d` implement-substrate + `94a8c7f292` status corrected `implemented_pending_validation`). Validation queued as **V3-EXQ-869a** (MECH-267 content-persistence retest with both noise-scale + horizon-depth mechanisms, ree-v3 main `bbb6348`, PENDING). MECH-267 stays `pending_retest_after_substrate=true` per the 2026-08-02 governance cycle. PROMOTES NOTHING. |
| SD-093 / MECH-426: Progress-Velocity Effort/Persistence Modulation | goal.progress_velocity_maintenance -- IMPLEMENTED 2026-08-02 (chip `chip-20260802-mech426-velocity-substrate`; ree-v3 main `b0bde65`; REE_assembly master `da90714ddd`). New `progress_velocity_maintenance` primitive on `GoalState` tracking per-goal progress-derivative; modulates E3 effort/persistence gates so an agent maintains effort on a goal it is progressing on and disengages from a stalled one. No-op-default (gain=0.0 -> bit-identical OFF); regression contracts pin flag-off equivalence. Substrate landing only. | Substrate landed 2026-08-02 (ree-v3 main `b0bde65`; REE_assembly master `da90714ddd` + design doc `sd_093_progress_velocity_maintenance.md`). No `substrate_queue.json` entry added -- direct-consumer wiring already present. SD-093 + MECH-426 stay candidate / v3_pending. Validation experiment V3-EXQ-885 authored as blocked (documented in ree-v3 `7cb37e1`) -- gated on the MECH-426 progress-derivative readout being live for consumers. PROMOTES NOTHING. |
| SD-092 / MECH-427 / MECH-428: Cross-Level Subgoal Credit | goal.cross_level_subgoal_credit -- primitive IMPLEMENTED 2026-08-02; agent-loop call site wired 2026-08-02 (ree-v3 main `500fd67` primitive + `d9e0586` call site; REE_assembly master `ebe9bc6ec6` + `0799489886`). Registers a parent<->subgoal credit-propagation channel `GoalState` never had (`parent_goal` field + `credit_subgoal_attainment()`); on subgoal attainment, MECH-427 maintenance-reinforces an already-seeded parent and MECH-428 bootstrap-seeds a near-zero parent. Backward-compatible `state_dict`/`load_state_dict` (pre-SD-092 checkpoints load cleanly). No-op-default (`use_cross_level_subgoal_credit=False`); regression + shape contracts pin flag-off equivalence. Substrate landing only. | Substrate landed 2026-08-02 (ree-v3 main `500fd67` + `d9e0586`; REE_assembly master `ebe9bc6ec6`). Validation experiments: **V3-EXQ-883** (MECH-427 cross-level subgoal credit, ATTAINED vs NO_ATTAINMENT contrast, smoke PASS -- ree-v3 main `4e4b603`; EXP-0385 executed by REE_assembly `13947774e8`) + **V3-EXQ-884** (MECH-428 subgoal-bootstrapped goal seeding, 3-arm NO_SUBGOAL / SUBGOAL_BOOTSTRAP / FORCED_SEED, smoke PASS -- ree-v3 main `e8127e1`; EXP-0390 executed by REE_assembly `dfc9a14ffb`). SD-092 stays candidate / `partially_implemented_pending_consumer_wiring`. PROMOTES NOTHING until the two experiments score. |
| SD-hazard-aware-policy-decomposition (MECH-321 harm-aware selection) | policy.decomposition_via_event_segmenter.harm_aware_selection -- IMPLEMENTED 2026-08-01. Adds a two-stage threat-modulated selection rule to MECH-321's `_apply_policy_decomposition`: Stage 1 always-active per-candidate graded harm bias `w(h) * harm_penalty` (gain-scaled + clamped, tagged to `Trajectory.metadata["decomposition_harm_bias"]` and composed additively into `dacc_score_bias` alongside SD-058/059 blocks), Stage 2 categorical override at `w(h) >= override_threshold` that restricts a withheld chunk's own leaf tiles to the single lowest-harm-penalty one. Reuses the SD-058/059 `threat_scale` ramp verbatim; reads `harm_penalty` per leaf tile from `VALENCE_HARM_DISCRIMINATIVE` averaged over the leaf's own predicted `world_states`. ARC-007 value-flatness preserved (PolicyDecomposition never scores a trajectory itself). No-op default (`use_harm_aware_selection=False`); 11 new contracts pin config wiring, threshold behaviour, and live-agent pool-admission below/at threshold. Routed by `failure_autopsy_V3-EXQ-844_2026-08-01` (mid-execution abort engages but reduces no task harm because decomposition candidates are recombined unweighted). | Substrate landed 2026-08-01 (ree-v3 main; REE_assembly master + design doc `sd_hazard_aware_policy_decomposition.md`). Validation experiment: none queued this session (substrate build only, scope discipline). MECH-321 / ARC-070 status unchanged; PROMOTES NOTHING. |
| MECH-307 `from_dims()` Reachability Repair | affect.anticipatory_conjunction_architecture -- REPAIRED 2026-08-07. The MECH-307 substrate (landed 2026-05-11, contract-verified since) had all TWELVE parameters as `REEConfig` dataclass fields with NO entry in the `from_dims()` signature, so `REEConfig.from_dims(..., use_mech307_conjunction=True)` was silently discarded and returned a config with all four gaps OFF. `[memory] reference-reeconfig-from-dims-silent-kwargs` failure mode with all three sites needed and only site 1 present. This pass adds all twelve to the `from_dims()` signature + a body assignment block + the resolver re-apply (site 3 is independently required to force sub-flags True post-`cls()`, mirroring the MECH-090 / GAP-3 sleep-cluster re-applies). NOT bit-identical -- 84 experiment drivers that pass `use_mech307_conjunction=True` into `from_dims` go from MECH-307 entirely OFF to ON, so prior results from those drivers (including V3-EXQ-603q / 866a / 866b, the scaffolded lineage ARC-030's retest was built on) MUST be read as measured with MECH-307 OFF. Bit-identical for every driver that does not mention MECH-307 (12 signature defaults match dataclass defaults exactly). 19 new contracts pin the factory route specifically, each against a direct-construction parity case. | Repair landed 2026-08-07 (contract file `test_mech307_from_dims_wiring.py`; verified differentially in a throwaway worktree -- 16 of 19 fail against pre-fix HEAD). ARC-030's Go/NoGo retest was blocked on this -- after this repair it still needs a readiness step (re-run 603q/866b with MECH-307 genuinely ON, confirm scaffolded lineage clears G0 non-degeneracy) before the COMBINED-vs-NOGO_ONLY pair is worth queuing. Out of scope: `from_dims`'s `**kwargs` still silently swallows unknown flags -- that reject-or-warn guard is the general mechanism, tracked as a separate triage. |
| SD-099 / MECH-489: Defensive-Orienting Response | pag.defensive_orienting_response -- IMPLEMENTED 2026-08-09. New `ree_core/pag/defensive_orienting.py` (`DefensiveOrientingGate` / `DefensiveOrientingConfig` / `DefensiveOrientingOutput`) -- non-trainable pure-scalar-arithmetic gate matching the `PAGFreezeGate` (MECH-279) pattern; a SEPARATE gate deliberately, not a modification of `freeze_gate.py` (MECH-279 chronic/accumulated-suffering vs MECH-489 phasic/unidentified-onset -- OR-composed at the action-constraint site so MECH-279 is unchanged bit-for-bit). Trigger: positive-derivative onset over `residue_surprise` (MECH-205 unsigned `VALENCE_SURPRISE`) OR `z_harm_s` norm (SD-010 sensory-discriminative harm), each channel's rolling EMA baseline FROZEN while orienting is active (verified necessary via inline smoke). Identification-confidence dynamics scale with baseline-decay of the triggering channel (not a clock -- a decaying spike resolves in a handful of ticks; a sustained elevation never overrides). Action decision (approach/withdraw/resume) resolved by the AGENT via `residue_field.evaluate_benefit()` vs `z_harm.norm()` at trigger `z_world`, expressed as a per-candidate `score_bias` toward/away from `_orienting_trigger_z_world` composed additively into `dacc_score_bias`. No-op default (`use_defensive_orienting=False`); 11 new tunables all present at all three from_dims sites. Reuses `pag_freeze_noop_action_class` so both freeze causes constrain to the same "hold still" action. | Substrate landed 2026-08-09 (ree-v3 main; REE_assembly master + design doc `sd_094_defensive_orienting_response.md` [file kept its early name]). Validation experiment queued via `/queue-experiment` -- ablation ON/OFF against Section 12h ground-truth injected events (`limb_damage_injected` / `external_hazard_injected` / `world_rule_shift_occurred`) and Section 4/12b coupling baselines. SD-099 + MECH-489 stay candidate / v3_pending; PROMOTES NOTHING. Distinct from MECH-279 (unchanged), SD-037, MECH-395/482/483 (adjacent orienting-territory claims, cross-referenced not duplicated). |
| SD-ORIENTING-DECISION-SCALE / MECH-489: Defensive-Orienting Decision Normalization | pag.defensive_orienting_response.decision_normalization -- IMPLEMENTED 2026-08-10. Fixes a bug INSIDE SD-099's Component 4/5 block (SD-099 itself, `defensive_orienting.py`, is UNCHANGED): `agent.py select_action()` compared `_do_harm_val` (SD-010 z_harm_s L2 norm, structurally non-negative persistent nonzero floor) directly against `_do_benefit` (ARC-030 RBF value, near-zero except near a benefit center) as if the two raw magnitudes were commensurable, structurally biasing every override toward `withdraw` (V3-EXQ-910 logged 206 overrides, 0 approach / 0 resume / 206 withdraw). Fix: both channels z-scored against their own running distribution (`(value - EMA_mean) / (EMA_mean_absolute_deviation + scale_floor)`) before comparison; baselines live at the agent level and FREEZE while orienting is active (mirrors DefensiveOrientingGate's own onset-baseline freeze -- prevents the triggering elevation from pulling its own baseline toward itself). `evaluate_benefit()` now called every waking tick while the gate is enabled. Two new config knobs + one changed default (`orienting_decision_epsilon` 0.01 -> 0.25 -- it is now a z-score margin, not a raw-magnitude margin). Routed by confirmed `failure_autopsy_V3-EXQ-910_2026-08-10`. | Substrate landed 2026-08-10 (ree-v3 main; REE_assembly master + design doc `sd_orienting_decision_scale.md` + substrate_queue entry `SD-ORIENTING-DECISION-SCALE` severity=corrupting). Validation experiment: V3-EXQ-910a queued (script copied to `v3_exq_910a_mech489_defensive_orienting_decision_retest.py`, 910's own script untouched; adds a NEW pre-registered criterion (c) `C_decision_alignment_non_degenerate` -- the actual acceptance test for this fix). Superseded again 2026-08-20 by the V3-EXQ-910b substrate half + driver-side sentinel tap (see 2026-08-22 Experiment Status entry above). |
| CausalGridWorldV2: `max_episode_steps` constructor kwarg | environment.max_episode_steps -- IMPLEMENTED 2026-08-10 (chip `chip-20260810-fishtank-max-episode-steps`). Not a numbered SD -- pure parameterization of an existing termination condition. `env.step()` computed `_step_cap_reached = self.steps >= 500` -- a bare literal, no constructor parameter -- so no driver could make a single continuous segment exceed 500 steps regardless of its own `steps_per_episode` argument. Fix: `max_episode_steps: int = 500` constructor kwarg, stored as `self.max_episode_steps`; `body_state[9]` "episode_progress" observable also generalised from `self.steps / 500.0` to `self.steps / float(self.max_episode_steps)` (would have silently saturated to 1.0 at step 500 of a longer-capped episode otherwise). Follows the SD-022/023/029/047/048/049 precedent of a flat env-only constructor kwarg not surfaced through `REEConfig.from_dims`. Origin: `organism_lifespan_development_review_906_lineage_2026-08-10.md` Section 10 item 1 (uncensored survival-to-death Fishtank successor design assumed the per-segment step cap was already driver-configurable; verified false against source before V3-EXQ-912 was designed around a segment-count workaround instead). | Landed 2026-08-10 (ree-v3 main; substrate_queue entry SD-FISHTANK-MAX-EPISODE-STEPS). Backward compatible (default 500 bit-identical to the pre-existing hardcoded literal). No validation experiment queued -- writing the actual uncensored-single-life survival experiment that consumes this kwarg is separate `/queue-experiment` follow-on. Full contract suite (3516 pass, 11 skip, 43 subtests) green under this change. |
| SD-100 / ARC-032 / MECH-089: Phase-Aware ThetaBuffer Summary | hippocampal.theta_buffer_phase_aware_summary -- IMPLEMENTED 2026-08-10. `ThetaBuffer.summary()` (E1 z_world -> E3, the exact site the failure record implicates) previously computed a flat unweighted mean over the buffered window -- permutation-invariant, so two theta cycles holding the same z_world values in a different order summarized identically. Dragoi & Buzsaki 2006 (theta-sequence compression) / Colgin et al. 2016 (theta-gamma phase nesting) attribute theta's functional payload specifically to within-cycle ordinal/phase structure a flat mean cannot represent. Fix: a fixed (non-trained) forward-sweep phase-weighted sum with a von-Mises-style kernel `w = softmax(-kappa * cos(phase_t))` centred on the read-out phase (cycle completion / "now"); forward HALF cycle only (single sweep per cycle per Dragoi's compression, not a full wraparound). Degrades gracefully: at `kappa=0`, softmax returns uniform `1/T` == flat mean (the OFF path still runs the original `.mean(dim=0)` directly, so disabled behaviour stays bit-identical rather than merely equivalent up to softmax rounding). Two new config knobs on `HeartbeatConfig` (both at all three from_dims sites). Scope: `summary()` only (E1->E3); `self_summary()` (E1->E2) and `consolidation_summary()` (MECH-122, already order-sensitive via its own weighting) are untouched. Routed by `failure_autopsy_V3-EXQ-228c_2026-08-10` (theta ACTIVE noisier than bypassed, `noise_delta_mean=-0.0671`; no persistence benefit; converged with MECH-089's confirmed EXQ-066/EXQ-122 defects). | Substrate landed 2026-08-10 (ree-v3 main; design doc `sd_100_theta_buffer_phase_aware_summary.md`). Validation recommended (not queued by that session): `/queue-experiment` a V3-EXQ-228d re-test of ARC-032 with `use_theta_phase_weighted_summary=True`. 7 new contracts (OFF bit-identical; same z_world multiset in different order produces different phase-weighted summary but same flat mean; `kappa=0` matches flat mean up to softmax rounding; single-entry T=1 does not divide by zero; weights strictly increase with recency). |
| SD-RESIDUE-VALENCE-BOUND: RBFLayer.update_valence.accumulator_bound | residue.field.RBFLayer.update_valence.accumulator_bound -- IMPLEMENTED 2026-08-11. Fixes `RBFLayer.update_valence()` (the single write path behind all 6 SD-014/ARC-036 valence components -- wanting/liking/harm_discriminative/surprise/positive_surprise/negative_surprise) which was a raw unclamped `+=` with no decay. Only 32 RBF centers exist by default, so a long-lived agent revisiting the same regions drove the same centers' components unboundedly (`z_world_norm` ~150-320 vs ~0.5-0.7 at smoke scale in 906a; `liking` mean=19.88 vs a 0.39 smoke ceiling; `dread` rising 40-110x across episodes in the 906b/906c cluster). Fix: leaky-integrator decay + hard clamp (`v <- v*(1-decay_rate) + value`, then `clamp(v, -clamp_abs, clamp_abs)`), gated behind a NEW master switch rather than an unconditional default change (`valence_enabled` defaults True and this write path is exercised by dozens of historical experiments with real claims.yaml evidence, so an unconditional fix would require the /implement-substrate Step-8.5 evidence-staleness audit across all of them -- disproportionate to a plumbing bound-fix). Three new `ResidueConfig` fields (not `REEConfig` top-level -- matches the sibling-flag convention). Routed by `failure_autopsy_V3-EXQ-906a_894b` + `failure_autopsy_906b-906c-911-cluster`. | Substrate landed 2026-08-11 (ree-v3 main; substrate_queue entry `SD-RESIDUE-VALENCE-BOUND` severity=degrading). Validation: V3-EXQ-918 queued (unit-level ON/OFF ablation directly against `ResidueField`/`RBFLayer`; smoke PASS -- OFF reproduces unbounded growth 200.0/395.5 after 200 writes, ON plateaus exactly at `clamp_abs=5.0`). No claims.yaml claim gates on this SD (all three motivating runs are `claim_ids=[]` diagnostics); no v3_pending flip. Registry-drift side-fix: `test_flag_registry_is_current` widened to scan every nested config dataclass (was scanning only REEConfig's own top-level fields, silently missing 85 nested flags). |
...
| SD-e1-rollout-consistency-training ABSOLUTE-VS-RESIDUAL branch | e1.rollout.output_proj_residual -- IMPLEMENTED 2026-09-01 (ree-v3 `b40139b3ed`). Design-doc pre-registered branch (the very sentence in the ITEM 1 row above naming this as "the next thing to test" once ITEM 1 was validated). `E1Config.output_proj_residual` (default False, bit-identical off; A/B against pre-change verified identical params + max abs diff 0.0 across three shapes) reparameterises the per-step read-out in BOTH rollout branches from `predicted = output_proj(...)` (absolute) to `predicted = state_i + output_proj(...)` (residual, matching `e2_fast.py::self_forward`/`world_forward`). Deliberately independent of `action_conditioned_transition` -- the discrimination it enables is an A/B ON the ITEM 1 ON arm, so both knobs must be separately settable. NO new module, NO new parameter (unlike ITEM 1's `action_encoder`); zero RNG consumption change. `predict_long_horizon`'s residual/absolute variants are pinned by contract, incl. the h=1 algebraic identity `ON == seed + OFF`. | Substrate landed 2026-09-01 (ree-v3 `b40139b3ed`; design doc `sd_e1_rollout_consistency_training.md`). **V3-EXQ-968 PASS 2026-09-01** (diagnostic, `residual_no_material_difference`; neither `residual_materially_exceeds` nor `residual_materially_below` on either seed; `cr_ratio(h=1)` seed42 2.673e-03 -> 5.909e-03 and seed123 2.717e-03 -> 9.227e-04 -- seeds disagree in SIGN; readiness cleared; the `~675x` LSTM+output_proj crush is therefore NOT a mere parameterisation artefact). Knob stays default-off as a CHARACTERISED NULL. Does NOT unblock INV-088 / MECH-135 -- both keep `pending_retest_after_substrate=true`; bars still missed by 25-37x (`cr_ratio`) and 5-7 orders (`e1coe_score_var`). ITEM 2 candidate 1 landed 2026-09-01 same day (below). |
| SD-e1-rollout-consistency-training ITEM 2: multi-step rollout-consistency objective (candidate 1) | e1.transition.rollout_consistency -- IMPLEMENTED 2026-09-01. `E1Config.e1_rollout_consistency_enabled` (default False; master switch), `_weight` (1.0), `_horizon` (5), `_horizon_weights_decay` (1.0 = uniform; <1.0 = TD-MPC discounting). Helper `E1DeepPredictor.rollout_consistency_loss(initial_state, actions, observed_states)` returns per-step-MSE horizon-mean via `L = sum_t (decay**t * L_t) / sum_t decay**t`. Flag gates an OBJECTIVE not the forward path -- turning it on does not perturb prediction at all; adds NO module and NO parameter; construction-time RNG identical. What this ADDS over `REEAgent.compute_prediction_loss` (which nearly stopped this build): that method already rolls E1 out autoregressively and MSEs the whole trajectory, but weights every horizon step EQUALLY -- under autoregressive rollout deep-step error is larger by construction, so a flat mean lets the deepest steps dominate the gradient; `decay < 1.0` is TD-MPC's actual form. At `decay=1.0` the helper reduces to the flat form within float32 reduction-order error (`rtol=1e-6`, NOT bit-exactly). `compute_prediction_loss` deliberately NOT rewired -- several hundred experiments depend on it and there is no consumer yet; agent-loop wiring held until validation reports. Candidate 1 chosen over a rollout-endpoint contrastive (deliberately NOT built): the doc's ranked-strongest AND the untried one (E2's sibling `e2_action_contrastive_multistep_enabled` has 0 runs in the whole evidence corpus -- built, never validated). 24 contracts (`eval()`-mode discipline for the same `dropout=0.1` reason as ITEM 1; pins flat-form identity at `decay=1.0` at float32-eps tolerance, the discount's SIGN, gradient reaches BOTH `output_proj` AND `transition_rnn` -- the ~675x crush location, deep-step-only error still produces gradient, MECH-094 gate). MECH-094 gate: `simulation_mode=True` returns zero (SD-056 helper convention). | Substrate landed 2026-09-01 (ree-v3; design doc `sd_e1_rollout_consistency_training.md`). Validation experiment OWED, chipped as follow-on -- must actually TRAIN with `rollout_consistency_loss` (enabling the flag changes nothing on its own, BY DESIGN) and should carry `decay=1.0` as the flat-form control. Does NOT unblock INV-088 / MECH-135. Both keep `pending_retest_after_substrate=true`; as of V3-EXQ-965 the bars are still missed by 25-37x (`cr_ratio`) and 5-7 orders (`e1coe_score_var`). Do NOT queue a retest off this build. |
| SD-018 AMEND: encoder.resource_field_supervision (directional resource-field head) | encoder.resource_field_supervision -- IMPLEMENTED 2026-09-02. Extends SD-018's original scalar `resource_prox_head` from 1 to 25 dims to preserve the DIRECTIONAL gradient the field carries. Routed by confirmed `failure_autopsy_V3-EXQ-948_2026-08-25` (H-observation-interface CONFIRMED; the one named un-owned build on the v3 critical path per `cross_plan_root_cause_synthesis_20260902.md` section 0 / GFLAG-0114). V3-EXQ-948 found z_world DISCARDS the directional gradient carried by `world_obs[225:250]` (`resource_field_view`) even when it sits inside z_world's own input -- a scalar `max(field)` target supervises magnitude only. New `resource_field_head = Linear(world_dim, 25) + Sigmoid` on `SplitEncoder`; MSE loss backprops INTO `world_encoder`. Chose shape (a) DIRECTIONAL head, NOT (b) side-channel routing past z_world (large blast radius on every world_dim consumer, bypasses the interface synthesis says must carry direction through E1/E2 rollouts). (b) stays fallback if (a) nulls. All three flags (`use_resource_field_head`, `resource_field_weight`, `resource_field_dim`) at all three from_dims sites; P0 leg `ZWorldP0Config.resource_field_weight` (default 0.0). Backward compatible: bit-identical OFF (verified via same-seed sense() hash + 51 state-dict keys identical). | Substrate landed 2026-09-02 (ree-v3 main; REE_assembly master + design doc `sd_018_resource_proximity_supervision.md`). Contracts C1-C5 (5 tests: OFF inert + state-dict/param-order identity + P0 leg skipped at weight 0; ON shape/range + grad reaches world_encoder; from_dims wiring; slice constants agree with env layout + P0 leg learns decodable field with r2 > 0; width mismatch is loud). Validation experiment NOT queued in landing (per routing chip); design reported for `/queue-experiment` -- 948-shape diagnostic, arms = field head OFF vs ON, PPO reader of z_world alone, DV res/ep vs 1.0 D3 floor + held-out linear decode as mechanism check; claims INV-088 + MECH-457 read-across; if it nulls, (b) is the next build. |
...
- **2026-09-13T09:07Z nightly attestation (scheduled `/update-docs`, bot
  ...
  manifests under `evidence/experiments/*/runs/`: **2946** (+1 vs 2945).
  **Currently queued (`experiment_queue.json` items[]): 0 items** (drained
  from 1 at the 2026-09-12T01:15Z read -- V3-EXQ-1023 completed in-window).
  **Pending review (`pending_review.md`, regenerated 2026-09-11T16:59:22Z):
  0 items** -- unchanged; the file has not been regenerated since the
  V3-EXQ-1023 landing, so the FAIL and its diagnostic self-route are not yet
  reflected in it and will surface at the next `generate_pending_review.py`
  run. Coordinator-DB 30-day rolling (per `experiment_error_rate.py` against
  the hub, span 2026-08-14T01:24Z .. 2026-09-12T04:53Z): **64 PASS /
  55 FAIL / 2 ERROR, ERROR rate 1.7% (2 / 121 classified runs)** -- down
  from 64/57/3/2.4% at 2026-09-12T01:15Z as the rolling window shifted
  (older PASS/FAIL/ERROR aged out) and V3-EXQ-1023 landed as the only new
  outcome in the window. (a) **NO new substrate landings in the window** --
  SD-106 (2026-09-11) remains the most recent; no substrate landings, no
  ratified amends, no substrate_queue additions between 2026-09-12T01:15Z
  and this snapshot. (b) **ONE new completion in the window, and it is
  substantive: V3-EXQ-1023 FAIL 2026-09-12T04:53Z** on `ree-cloud-2`, label
  `sd106_below_pca32_parity`. All 5 preconditions met (raw-field control
  0.97 vs 0.60 floor; PCA-32 anchor cleared parity bar on the seed
  majority; SD-106 bypass norm 4.13 >> 0 so the linear skip trained;
  encoder weight-delta n_changed 4 >= 1 so the P0 path ran; z_world
  participation ratio 8.13 >> 2.0 floor so latent did not collapse), so the
  null is a verdict rather than an instrument failure: **SD-106's
  scale-normalised preservation term + world-encoder skip DID NOT reach
  PCA-32 parity at the x734.PPOPolicyNet consumer rung on a seed majority
  under `preservation_weight=200.0`**. `experiment_purpose=diagnostic`;
  needs confirmed `/failure-autopsy` target before governance marks
  reviewed. The routing outlined in the 2026-09-11T20:15Z snapshot
  ("V3-EXQ-1023 will either meet PCA-32 parity or NULL, in which case
  shape-(b) side-channel routing past z_world becomes the next build") is
  now the operative path -- adjudication routes SD-106 disposition + the
  successor's shape (side-channel routing past z_world, or a
  `preservation_weight` sweep beyond 200.0) at the next autopsy. (c) **NO
  new governance walks, no `/failure-autopsy` sessions, no
  `/claim-synthesis` cycles in the window.** One AM lit-pull did land:
  **ARC-069 policy regranularisation (3 entries) + ARC-074 Phase 0
  babbling (4 entries)** in `REE_assembly` `ef267d1839` at
  2026-09-12T06:15Z (targeted_review_arc_069 all-supports lit_conf 0.80
  with a Jin/Tecuapetla/Costa 2014 supports on parsing-vs-concatenation
  BG-subcircuit separability, plus Lai/Huang/Gershman 2025 chunking as
  conditional policy compression and Ostlund/Winterbauer/Balleine 2009
  dmPFC-lesion biological ARC-069-OFF control; targeted_review_arc_074 1
  supports + 3 mixed lit_conf 0.809 posterior 0.58 with Aronov/Andalman/Fee
  2008 supports on subsong needing LMAN not HVC, and three mixed entries
  from Garst-Orozco/Babadi/Olveczky 2014 / Dhawale/Smith/Olveczky 2017 /
  Jin/Krishnamurthy/Simchowitz/Yu 2020 that undercut ARC-074's
  thresholded-exit and precede-reward-in-time framings). Both claims
  dropped `literature` from `evidence_needed`; owed `/governance` action
  the AM run did not perform: two of ARC-074's own three cited anchors
  ("Griffin et al. 2026", "Garcia-Guzman 2026") do not resolve on PubMed
  or web search and are flagged for reconciliation. Also in the window:
  hold-lane hygiene (45 -> 26 entries) `REE_assembly` `4ba253e677`; curated
  pause-window agenda g6 (proposal only, halt NOT executed) `REE_assembly`
  `9785847013`; IGW-routine backlog drained after PHASE-4 writer flag
  reversion `REE_assembly` `7756ff04fb` (82 stalled log lines 437-518 from
  the hollow-ack window); V3-EXQ-1026 MECH-005 nu-path-authority
  RECOVERY-QUEUE REFUSED at red-team (`env.step` never called; measurement
  re-senses a single frozen observation) recorded in `REE_assembly`
  `b0d0a1a887`; V3-EXQ-1003 EXT-004 RECOVERY-QUEUE already-refused re-noted
  (2026-09-09 refusal held); metaworker orchestration cycle STOP verdict
  across all 3 dispatchers, 17 paced proposals adjudicated read-only
  0 GREEN / 4 AMBER / 13 RED, no lease granted, generation-9 queue-floor
  alarm fired mechanically per the 2026-09-11 respec. (d) **NO
  coordination-plane / infra work landed in the window** -- no commits to
  `ree_commit.py`, `task_claim.py`, `chip_ledger.py`, coordinator hub,
  `sync_daemon`, or fleet wrappers between the two snapshots. This
  attestation is itself one of the two content changes: `docs/ree-v3-spec.md`
  §0 date bump + Experiment Status entry + SD-106 row update, and the
  `docs/roadmap.md` snapshot. **Bottleneck: SHIFTED in shape though not in
  location -- convergence root remains the H-observation-interface axis,
  but the direct SD-106 substrate lever has FAILED its own acceptance
  validation.** V3-EXQ-1023's FAIL under `preservation_weight=200.0` +
  world-encoder skip is the first negative datum on the observation->z_world
  H-F reading since the failure autopsy that routed the build; SD-106 is
  IMPLEMENTED and available but its recipe is not yet a validated parity
  path. Next scientific move (owed to `/failure-autopsy`): (i) shape-(b)
  side-channel routing past z_world into E1/E2/E3 consumers (the routing
  autopsy's alternate branch), or (ii) a `preservation_weight` sweep beyond
  200.0 with the acceptance criterion re-derived from what SD-070's
  collapse floor actually admits, or (iii) both as a portfolio. Green-board
  target 2026-07-19 is now **56 days overdue**. **ETHICS-PERIMETER Phase 0
  datum** stays on the record (Phases 1-3 deferred; NON-BLOCKING).
  Public-information-architecture impact: reviewed against
  `docs/design/public_information_architecture.md` -- no `/api/*` surface,
  generated visualization, or public export changed; nightly snapshot +
  spec date bump + one SD table row content update (SD-106 status: queued
  in-flight -> completed FAIL diagnostic-adjudication-pending; no rows
  added, moved, or restatused; same category as the 2026-09-12
  attestation).
...
- **2026-09-05T01:10Z nightly attestation (scheduled `/update-docs`, bot
  ...
  `v3_exq_*` manifests on disk: **983** (+7 vs 976 at 2026-09-04); nested
  per-run manifests under `evidence/experiments/*/runs/`: **2918** (+7 vs
  2911). **Currently queued (`experiment_queue.json` items[]): 2 items**
  (drained from 6 yesterday) -- both CLAIMED: V3-EXQ-822e (`SD-082 raw
  pre-tanh candidate-discrimination index DV`, no `claim_ids`, prio 45,
  CLAIMED `ree-cloud-2`; note "same scientific question as V3-EXQ-822d") +
  V3-EXQ-983a (`EXT-002 residue error-persistence redesign -- headroom-derived
  C1 bar (0.15 -> 0.04 from 983's measured control range) + runtime
  dv_headroom precondition + training-completion gate before seed pooling`,
  claims `EXT-002` + `ARC-013`, prio 45, CLAIMED `ree-cloud-3`; supersedes
  V3-EXQ-983 per confirmed `failure_autopsy_ext-claim-probe-cluster`).
  **Pending review (`pending_review.md`, regenerated 2026-09-04T18:57:45Z):
  0 items** -- "All experiments reviewed. Nothing pending" (unchanged from
  yesterday's 0 close-out; the 2026-09-03T20:17Z /failure-autopsy walk
  cleared the queue and no new PASS/FAIL has surfaced above the review bar).
  (a) **NEW SUBSTRATE LANDINGS in the window (three, all 2026-09-04):**
  (i) **SD-104 + SD-105 co-landed** (ree-v3 `ba95c43`, "phasic burst
  refractory duty bound + selection-entropy headroom floor") -- BOTH legs of
  `sd_phasic_burst_decay_and_warmup_headroom` from confirmed
  `failure_autopsy_V3-EXQ-963a_2026-09-02`. SD-104 adds a duty-cycle bound
  (extinction level + lifetime refractory that carries across reset()) so a
  warmed-agent phasic burst does not occupy 88% of E3 selections. SD-105 is a
  ONE-SIDED integral controller on selection-entropy in log-temperature that
  lifts tonic-side temperature enough to leave headroom in the readout that
  963a measured pinned at 0.02. BOTH legs must be shown fixed before
  **V3-EXQ-963b** (the MECH-063 (ii) retest, queued this window) is
  admissible -- neither knob alone suffices. Unblocks MECH-063 + SD-069
  (both `pending_retest_after_substrate`). (ii) **SD-WAYPOINT-FIELD** (ree-v3
  `7719385`) -- CausalGridWorldV2 gains the env-only
  `waypoint_proximity_field_enabled` / `waypoint_field_decay` kwargs; when the
  master switch is on the env appends an additional 25-dim waypoint proximity
  field patch (appended LAST after SD-065) so a waypoint > 2 cells away is
  observable at all in `subgoal_mode`;
  fixes V3-EXQ-977's `blocked_substrate` finding that the agent visited 0/0/1
  waypoints across seeds 42/43/44 because the field was outside the 5x5
  local-view. Validation drafted in `c3_deferred_writes_20260904.md`; queue
  entry NOT written yet (`/governance` held an exact-file pause on
  `experiment_queue.json` / `substrate_queue.json` / `claims.yaml` during
  landing). Unblocks INV-086 / MECH-428 (EXP-0390). (iii) **DV-headroom
  class** (ree-v3 `8e133d2`, `harness: dv-dynamic-range-precondition-class
  -- criterion_exceeds_achievable_range lint + dv_headroom precondition
  kind`) -- substrate entry `dv-dynamic-range-precondition-class` (priority
  1, severity DEGRADING) from confirmed cluster autopsy
  `failure_autopsy_ext-claim-probe-cluster_2026-09-03`; adds a runtime
  `dv_headroom` precondition kind + a static `validate_experiments.py
  --checks criterion_exceeds_achievable_range` lint so a new driver whose
  criterion reads a difference/spread/elevation statistic must declare and
  clear a runtime headroom check (or exempt with reason). Fires on 112/1448
  drivers at landing; V3-EXQ-983a is the first consumer downstream (its own
  queue-entry note flags `runtime dv_headroom precondition`). (b) **New
  completions in the window (7 flat + 7 nested):** V3-EXQ-1002
  (`zworld_actor_adequacy_locus discriminator -- supervised oracle-adapter
  on the frozen 978 latent`), V3-EXQ-1004 (`SD-WAYPOINT-FIELD validation`,
  deferred landing), V3-EXQ-993a (`ARC-021/MECH-069 merged-channel ablation,
  action-conditioned uncompressed harm readout`, supersedes 993), V3-EXQ-642c
  (`MECH-353 blocked-agency floor validation on headroom DV z_block_mean`,
  supersedes 642b), V3-EXQ-1000 (`SD-e1 ITEM 3 rollout-endpoint contrastive
  validation`), V3-EXQ-996 (`ISEF-005 phase-gate live-channels`), V3-EXQ-1001
  (`EXT-005/SD-031 attribution causal-shift`), V3-EXQ-997 (`MECH-162 zresource
  zworld planning reconvergence`), V3-EXQ-999 (`MECH-161 vigilance
  inverted-U heartbeat`), plus late writebacks of V3-EXQ-983 EXT-002 /
  V3-EXQ-978 SD-018 fishtank / V3-EXQ-995 EXT-005 / V3-EXQ-991 EXT-004 /
  V3-EXQ-981 MECH-027 / V3-EXQ-993 EXT-003 / V3-EXQ-994 EXT-007 /
  V3-EXQ-982 EXT-001 / V3-EXQ-591h ISEF-005 (the pending-review cohort the
  2026-09-03 autopsy walk adjudicated). (c) **Governance apply cycle in the
  window:** no broad `claims.yaml` walk since 2026-08-28 (`62edbf45`); the
  window's `REE_assembly` commits (fresh log) are dominated by IGW-ledger
  materialisation, GOV-EDGE-1 (`97ce76a811` -- claim-graph edge type split,
  `depends_on` prerequisite DAG vs `coupled_with` undirected reciprocal;
  `58b9218c6c` -- negative supports-layer probe record, 0 promotions rest
  on another claim's status), an ARC-021 stale-note governance flag
  (`9b667b5c17`), an ARC-019 EVB-1189 blocked_substrate marking
  (`dcfa8ddc24` via IGW-20260904-214), and IMPL-008 lit-pull work
  (`c7a69704bd`, EVB-1297 skipped as not-V3-testable via IGW-20260905-234).
  (d) **Coordination-plane / infra work in the window:** live IGW workset
  247 items / 31 ready / 0 in flight throughout; V3-EXQ-963b driver
  authored with a BLOCKING red-team verdict and explicitly NOT queued
  (`d2104f8` -- pinned as PENDING against the SD-104/105 substrate
  landing above); WORKSPACE_STATE.md ownership boundary moved
  2026-09-03 to coordinator-owned (per [memory]
  `reference-phase2b-materializer-ownership-boundary`). **Bottleneck:
  unchanged from 2026-09-04** -- convergence root remains the
  H-observation-interface axis (SD-018 AMEND + SD-e1 ITEM 2 the two named
  un-owned builds on the v3 critical path per
  `cross_plan_root_cause_synthesis_20260902.md`; SD-018 AMEND landed
  2026-09-02, SD-e1 ITEM 2 candidate-1 landed 2026-09-01 and now owed a
  validation experiment that actually TRAINS with
  `rollout_consistency_loss`; V3-EXQ-978 SD-018 AMEND validation was
  CLAIMED yesterday on `ree-cloud-3` and has completed to a late-writeback
  manifest, adjudicated non_contributory in the 2026-09-03 autopsy walk).
  Green-board target 2026-07-19 is now **48 days overdue**.
  **ETHICS-PERIMETER Phase 0 datum** stays on the record (Phases 1-3
  deferred; NON-BLOCKING). Public-information-architecture impact:
  reviewed against `docs/design/public_information_architecture.md` -- no
  `/api/*` surface, generated visualization, or public export changed;
  nightly snapshot + spec date bump + three new SD table rows (SD-104 /
  SD-105 / SD-WAYPOINT-FIELD, plus the SD-e1 ABSOLUTE-VS-RESIDUAL branch
  row and SD-e1 ITEM 2 candidate-1 row + SD-018 AMEND row from the
  2026-09-01/2026-09-02 landings not yet in the table). This snapshot is
  the same public-information category as the 2026-09-04 attestation.
...
- **2026-08-22T01:14Z nightly read (scheduled `/update-docs`, bot identity).**
  ...
  refresh this section). Nested per-run manifests under
  `evidence/experiments/*/runs/`: **2884** (+40 vs the 2026-08-17 nightly's 2844).
  Corpus status counts (from the regenerated `claim_evidence.v1.json` tree): PASS
  1056, FAIL 1996, UNKNOWN 76. **Currently queued (`experiment_queue.json`
  items[]): 1 item** -- V3-EXQ-910b (`MECH-489 (SD-099) valence-gating retest with
  decision counted at the override tick`, `claim_id=MECH-489`, prio 62, ~340min,
  CLAIMED `ree-cloud-1` 2026-08-21T19:30:28Z; supersedes V3-EXQ-910a). The DRIVER
  half of the MECH-489 valence-gating retest -- the substrate half already landed
  2026-08-20 (post-override decision window now expires unconditionally); this
  entry lands the corrected driver with a non-destructive sentinel tap on
  `REEAgent.select_action` that counts the decision AT the override tick only,
  fixing a 910-lineage denominator inflation (per-env-step read of latched
  DefensiveOrientingOutput fields against an E3-tick-denominated persistence
  window). **Pending review (`pending_review.md`, regenerated 2026-08-21T17:11:51Z):
  0 items** -- "All experiments reviewed. Nothing pending." Cleared from the
  2026-08-17 1-item board (V3-EXQ-874b) plus every intervening completion.
  **Substrate landings in the window:** (a) 2026-08-19 --
  `contextmemory-write-path-addressing-degeneracy` closed with two ORTHOGONAL,
  co-landed mechanisms behind the same substrate_queue entry: the
  `contextmemory_write_usage_balancing` conscience bias (frequency-sensitive
  usage-EMA penalty on the write-address argmin) and the
  `contextmemory_write_selection="refractory"` mode (k-most-recent-slots
  ineligibility mask). Both no-op-default; both individually clear the >=2
  occupied slots on >=3/5 seeds acceptance floor (the write-side sibling of
  SD-016's cue-indexed retrieval, closing the same defect V3-EXQ-436e/f
  established). SD-017/ARC-045/MECH-166 unblocked for their own validation now
  that the write-path precondition is repaired. Validation ablations chipped via
  /queue-experiment; substrate_queue status stays
  `implemented_pending_validation`. (b) 2026-08-20 -- MECH-489 SD-099 substrate
  amend: `defensive_orienting.py` post-override decision window now expires
  unconditionally (per `chip-20260820-mech489-retest-driver-decision-at-override-tick`),
  paired with V3-EXQ-910b's driver-side sentinel tap on select_action.
  **Non-substrate work in the window:** chip ledger churn heavy (many recorded
  chips including the 2026-08-19 write-path landings, the 2026-08-20 orienting
  amend, and the 2026-08-21 recommendation log). **Bottleneck: shifting toward
  the MECH-489 valence-gating driver retest (V3-EXQ-910b, CLAIMED on
  `ree-cloud-1`) + the write-path validation ablations** the 2026-08-19 landings
  unblock. Green-board target 2026-07-19 is now **34 days overdue**, but the
  window shipped two structural substrate closes (contextmemory write-path
  degeneracy + the orienting decision window) without a governance apply cycle
  in the same window (the last one was 2026-08-16). **ETHICS-PERIMETER Phase 0
  datum** stays on the record (Phases 1-3 deferred; NON-BLOCKING).
...
- **2026-08-04T01:10Z nightly read (scheduled `/update-docs`, bot identity).**
  ...
  **Substrate + governance landings in the window:** (a) **`/governance` cycle 2026-08-03 APPLIED** (REE_assembly master `04068b925b` + follow-up `d6699d550e` marking V3-EXQ-870 ERROR discussed after 870a re-queue): 19 confirmed-autopsy FAIL dispositions + 5 clean PASS dispositions applied to claims.yaml; pending_review drained 19 -> 1. Confirmed autopsies landed: V3-EXQ-436c (SD-017/ARC-045/MECH-166 ContextMemory write-path collapse convergence), V3-EXQ-671b + V3-EXQ-876 MECH-025 cluster, V3-EXQ-848a ARC-005 precision-only ladder routed to /queue-experiment ladder-granularity redesign, V3-EXQ-862a Q-040c + V3-EXQ-870a MECH-480 dACC cluster, V3-EXQ-866a INV-034/Q-021 goal-maintenance-vs-avoidance, V3-EXQ-871a MECH-090/ARC-071 commit-latch AT-tick persistence, V3-EXQ-873 MECH-322 sleep-replay carve-out + V3-EXQ-882 MECH-472 held-out context worst-of-N-seed veto cluster, V3-EXQ-874 MECH-467 leg-3 zero-event denominator, V3-EXQ-875 MECH-471 competence provenance (SD-070 zworld_p0 omission), V3-EXQ-877 MECH-072, V3-EXQ-878 MECH-332 efference-copy vs AIC dissociation. Substrate-queue status flips: **ARC-058** candidate_v3_pending -> implemented (`6ce1cf2969`, shared HarmForwardTrunk landed 2026-04-19 in ree-v3 `2ae52be4`; entry was never flipped); **SD-094** pending_implementation -> implemented_pending_validation (`a906cc0824`, code+tests landed in `a95e103` but the crashed worker never flipped status); **MECH-122** write_gate audit result + landed fix (V3-EXQ-436c target discharged, `19bbfe8292`). (b) **TWO substrate landings this window:** (i) **SD-094 causal_grid_world waypoint-arrival + hazard-free contamination fix** (ree-v3 main `a95e103`; REE_assembly master `a906cc0824`) -- two real bugs in the shared grid-world env starving MECH-428/EXQ-884-class experiments: agent's grid-position marker overwrote the "arrived at waypoint" signal (transition_type stayed `none`, sequences stalled) + a background self-contamination effect killed the agent early even in hazard-free setups. (ii) **SD-091 / MECH-481 STEPS 4-6** (chip `chip-20260802-sd091-live-wiring`; ree-v3 main `87a7e21` + REE_assembly master `88375d0126`) -- agent-loop mount-point + `REEAgent.select_action` consumer wiring across 8 named target sites (`e1_sensory_encoder` / `e2_fast_forward_model` / `e3_candidate_count` / `hippocampal_anchor_set` / `hippocampal_persistence_appraisal` / `e3_commitment_monitor` + `motor_commitment` / `hippocampal_write_consolidation`) + live-tick smoke test. Step 7 remains: `/queue-experiment` the MECH-481 4-arm falsifier (V3-EXQ-886 authored as INSTRUMENT strict-validated + smoke PASS in `e7dfa8e` but NOT queued, blocked at Step 2.5a on the competence/adaptation harness prerequisite premise probe). (c) **Governance-tooling repair batch**: IGW workset generator FM8/FM9 fix (REE_assembly `5aa0d3267a` + `b8870edcf9` regen 245 items) fixing EXPERIMENT-lane staleness (stale ree-v3 queue snapshot) + confirmer-lane singular claim_id blindness; FM10 confirmer lane adjudicated-proposal suppressor live (`5c57ce6252` + `8fea1faf8d`); FM11 verdict writeback for SD-039/MECH-203/MECH-074 confirmer items (`20af336501` MECH-203 + `784435d51a` SD-039 + `24d39148df` regen suppresses those confirmer items); `e3_score_decomp_enabled` gate silently no-ops modulatory channel routing audit -- corpus blast-radius bounded to V3-EXQ-851/859/863, 858 cleared (`3b0f21b685`); modulatory-channel route decomp-gate fix (`3db7d23`); `precommit_contracts` Block 1c experiment-script corpus-lint trigger (verification-harness Gap 1); `tests/contracts/LINT_INDEX.md` added (verification-harness Gap 2); zworld_p0_warmup lint (SD-070, confirmed twice, `c81813b`). (d) **Substrate stability P1c**: record enabled default-off REEConfig flags via existing agent= threading (`bdc37ec`). (e) **Failure-autopsy activity high**: dACC substrate cluster (V3-EXQ-862a Q-040c + V3-EXQ-870a MECH-480, `5decc227d5` + `56a356bbb8` confirmed), MECH-025 cluster (V3-EXQ-876 + V3-EXQ-671b, `4f45477483`), MECH-322/MECH-472 worst-of-N-seed veto cluster (V3-EXQ-873 + V3-EXQ-882, `408696da90` + `63d8792b94` post-governance addendum untrained z_world confound + `6ab9bcef37` MECH-472 evidence_quality_note append), V3-EXQ-436c/874 ContextMemory write-path collapse convergence (`b00ada9dcd`), V3-EXQ-878 MECH-332 (`71adf4611b`), V3-EXQ-877 + V3-EXQ-866a (`a043a7f404`), V3-EXQ-871a MECH-090/ARC-071 commit-latch (`de5002dcd6`), V3-EXQ-884 MECH-428 substrate bug not claim evidence (`53e14b66fa`), V3-EXQ-875 MECH-471 SD-070 zworld_p0 omission (`90059edaf3`). Autopsy-driven successors queued (in the 9-item queue): V3-EXQ-875a (corrected requeue, `f491b2e`), V3-EXQ-848b (ARC-005 finer 7-level precision ladder, `90a17bf`), V3-EXQ-867b (MECH-321 screened matched-abort seed pool, `f4dc18a`), V3-EXQ-436d (ContextMemory gated_content_write=True, `1df184c`), V3-EXQ-873a (MECH-322 fraction-gate retest, `fd5f053`), V3-EXQ-866b (603q ARM_BASE_IA_ONLY substrate-regression diagnostic, `ab381e2`), V3-EXQ-882a (MECH-472 retest, pending). (f) **IGW workset activity**: five automated chip spawns fed by the FM10+FM11-fixed workset generator -- MECH-074 GOV-CONFIRM-1 (IGW-20260803-230 -> chip-20260803-igw-confirm-mech074, `09aac065cd`); SD-039 GOV-CONFIRM-1 (IGW-20260803-231 -> chip-20260803-igw-confirm-sd039, `3399c1a19f`; released `cab39e28a1` verdict recorded FM11); MECH-269 confirmer (IGW-20260804-231 -> chip-20260804-igw-confirm-mech269, `83569cb22b`). (g) **New MECH claim registered**: MECH-472 held-out context vs memorization -- lit-pull 4 entries on the topic (`d87a766f4e`, IGW-20260803-226, EVB-0578). (h) **Hypothesis-space integrity work**: Mode C (discovery growth) sanctioned path added (GOV-FROZEN-1, `796ffcfb38`); H-dynamics-collapse re-filed as discovery growth (Mode C) not fan-out (`ba5b005ae5`); traceability exempt ARC-071 + MECH-323 for incidental 'narrow-repertoire' keyword match (`63e177b761`); hypothesis-space integrity bare-string synthesis fix on inv088_evaluator_degeneracy_cause (`5819381b43`). (i) **Lit-pull**: `targeted_review_e1_forward_model_rollout_consistency` (7 entries + synthesis, `fb581f136e`). (j) **ARC-005 plan reconciliation**: GAP-A-precision-diagnostic reconciled with V3-EXQ-848a's confirmed autopsy (measurement-ceiling, not substrate, `0c810b14d0`).
  **Bottleneck (shifting toward the F-dominance conversion-ceiling attack + the SD-091 coalition-controller step-7 falsifier):** the load-bearing near-term items are (1) the ARC-005 precision monotonicity finer 7-level ladder redesign (V3-EXQ-848b, CLAIMED on `ree-cloud-3`) closing the measurement-ceiling face named by V3-EXQ-848a's confirmed autopsy; (2) the MECH-471 corrected requeue (V3-EXQ-875a, CLAIMED on `ree-cloud-4`) on the SD-070 zworld_p0-fixed substrate; (3) the MECH-321 harm-aware selection screened matched-abort seed pool (V3-EXQ-867b, CLAIMED on `ree-cloud-1`); (4) the SD-091 / MECH-481 step-7 4-arm falsifier (V3-EXQ-886 authored + strict-validated but NOT queued, gated on the competence/adaptation harness premise probe). Pending review dropped 19 -> 1 in a single governance cycle; queue is 9 items with 4 CLAUMED and 5 PENDING autopsy-successors + confirmer chips. Green-board target 2026-07-19 is now **16 days overdue**, but governance throughput was HIGH in the window (19 FAIL + 5 PASS applied in one cycle; the substrate-side coalition-controller wiring completed steps 4-6 of 7). **ETHICS-PERIMETER Phase 0 datum** stays on the record (Phases 1-3 deferred; NON-BLOCKING).

---

- **2026-08-03T01:10Z nightly read (scheduled `/update-docs`, bot identity).**
  1-day window since the 2026-08-02 nightly. Manifest count on disk: **780 `v3_exq_*` + 3 `v4_exq_*` flat** (+11 v3 vs the 2026-08-02 nightly's 769; per-outcome tally on the v3 flats: 248 PASS / 505 FAIL / 1 ERROR / ~5 PARTIAL/INCONCLUSIVE / 20 unclassified). **Currently queued (`experiment_queue.json` items[]): 8 items** (down from 9 on 2026-08-02) -- 5 CLAIMED (V3-EXQ-875 on `ree-cloud-3`, V3-EXQ-671b MECH-025b on `ree-cloud-1`, V3-EXQ-149b Q-004 on `ree-cloud-2`, V3-EXQ-882 MECH-472 on `ree-cloud-4`, V3-EXQ-878) + 3 PENDING (V3-EXQ-883 MECH-427 cross-level subgoal credit, V3-EXQ-884 MECH-428 subgoal-bootstrapped goal seeding, V3-EXQ-866a INV-034/Q-021 goal-maintenance re-run on scaffolded_sd054_onboarding). **Pending review (`pending_review.md`, regenerated 2026-08-02T22:57:01Z): 19 items** -- 3 PASS + 7 FAIL + 8 unclaimed manifest(s) + 1 ERROR + 1 diagnostic self-route flagged (V3-EXQ-863 `vacuous_pass`) + 1 dead z_goal stream. **Sharp rebuild of the pending_review backlog** on top of yesterday's cleared board -- the 2026-08-02 governance cycle drained a big wave of autopsy-driven successor FAILs (857a / 858 / 867 / 862 / 671a / 436b / 869 / 108a / 828b / 866 / 871 all confirmed-autopsy FAILs) alongside 2 clean PASSes (865 / 868), and a further batch has landed since.
  **Substrate + governance landings in the window:** (a) **`/governance` cycle 2026-08-02 APPLIED** (REE_assembly master `e9de7d3635`, session `infallible-villani-f280fa`): 11 confirmed-autopsy FAILs applied to claims.yaml (857a / 858 / 867 / 862 / 671a / 436b / 869 / 108a / 828b / 866 / 871) + 2 clean PASSes (865 / 868); **MECH-135 `epistemic_category` -> `substrate_ceiling` then parked (`ceiling_decision:deferred`)** with the disambiguating experiment routed per 108a's fanout_recommendation; MECH-267 flipped `pending_retest_after_substrate=true`; `substrate_queue.json` closed `INV091-NULL-VALIDATION-RUN-LENGTH` as **aleatoric** (3rd refused null-validation attempt), added `SD-MECH267-HORIZON-DEPTH`; 13 run_ids marked reviewed; 4 pending_user promotion/demotion holds confirmed (MECH-322 / MECH-481 / SD-091 / SD-087). sleep_substrate_plan.md GAP-2 stale-since-review acknowledged. (b) **FOUR substrate landings in the window:** (i) **SD-091 + MECH-481 coalition/topology control substrate PARTIAL** (steps 1-3 of 5; ree-v3 `71182f3`, REE_assembly `01b054d5d7`) -- new self-contained `ree_core/control_plane/coalition.py`; step 4 mount-point + step 5 consumer NOT DONE, MECH-481 4-arm falsifier NOT queued. (ii) **SD-MECH267-HORIZON-DEPTH IMPLEMENTED** (ree-v3 `e0117ee`, REE_assembly `37d1da187d` + `94a8c7f292`) -- mode-conditioned horizon-depth multiplier extending MECH-267 (the lever V3-EXQ-869's autopsy named as blocking); validation queued as **V3-EXQ-869a** (`bbb6348`, retest with BOTH noise-scale + horizon-depth mechanisms). (iii) **SD-093 / MECH-426 progress-velocity effort/persistence modulation IMPLEMENTED** (ree-v3 `b0bde65`, REE_assembly `da90714ddd` + `sd_093_progress_velocity_maintenance.md`) -- goal.progress_velocity_maintenance primitive; V3-EXQ-885 authored as BLOCKED pending consumer wiring (documented in ree-v3 `7cb37e1`). (iv) **SD-092 cross-level subgoal credit IMPLEMENTED** (primitive `500fd67` + agent-loop call site `d9e0586`; REE_assembly `ebe9bc6ec6` + `0799489886`) -- new parent<->subgoal credit-propagation channel `GoalState` never had (MECH-427 maintenance-reinforcement + MECH-428 bootstrap-seeding); validation queued as **V3-EXQ-883** (MECH-427) + **V3-EXQ-884** (MECH-428), both smoke PASS. (c) **13 failure autopsies APPLIED** (REE_assembly `d29cb07838` + `b9cac03c83` + `4f16bf92f4` + `eae58fcbef`): V3-EXQ-867 MECH-321 env-adequacy, 862 Q-040c dacc_weight=0, 436b SD-017/ARC-045/MECH-166 recording gap, 108a MECH-135 substrate_ceiling + INV-088 cross-ref, 866 INV-034/Q-021 substrate_not_ready, 671a MECH-025b measurement gap, 869 MECH-267 implementation gap, 828b INV-091 3rd null-val infra failure, 871 MECH-090/ARC-071 commit-latch post-fix confirmed, 871 follow-on sd084_midexec_reachability.py cleared (no defect). Autopsy-driven successors queued: V3-EXQ-867a (harm-aware selection redesign, ree-v3 `7a4e818`), 862a (dacc_weight fix, `a01b8ee`), 671b (MECH-025b positive-control gate + 4-seed pooling, `c4c1e4e`), 436c (recording fix, `91cc5f1`), 866a (`823f85a`), 869a (both mechanisms, `bbb6348`), 871a (probe-phase scope, `2899549`), 847a (P-D corrected denominator, `e1d473c`). (d) **MECH-203 SR-2 harm-symmetric harm-salience write path IMPLEMENTED** (ree-v3 `adfd8c0`; REE_assembly `801ceba495` design doc amend) -- closes a documented gap in the serotonergic cross-state substrate. Companion `b4d324a` MECH-203 opt-in magnitude tiebreak for proximity-approach classification. (e) **Explorer UI improvement pass** (audit + implementation, session `determined-ritchie-55a3a6`, worktree): REE_assembly `d4fc2b7f13` Docs view phone-viewport overflow fix + `90bfe64e24` Docs picker C4 redesign + `2aacbbbe32` metaworker-box /machines role visibility + `b978a0ae67` distinct Phase H orchestrator card + `166f3b773e` stale-claim + orphaned-stash corner dock + `ce7ac6096b` sortable/paginated List + `01b054d5d7` queue priority + stale-claim age + `50aba391de` chip claim badges on workset. (f) **Governance-tooling repair batch** (REE_assembly `2b99eeaa8b` + `ac3733327f` + `8346f43229` + `2fefba11f9` + `02aaa00d32` + `838b24e796` + `950e4bf361`): fixes to `build_experiment_indexes.py` (duplicate-row bug + backlog_id-null carry-forward instability + resolved status write-back into `manual_proposals.v1.json`); EXP-* proposal-eligibility gate for substrate_conditional / v3_pending+v4 / unregistered claims; 3 pipeline residuals cleaned up; `mandatory_decision_checkpoint` since-decision freshness + dormant/chronic watchlist + human-readable markdown surface; governance agenda folds `dormant_high_conflict` watchlist. (g) **Proposal backlog reconciliation**: 34 + 12 + 3 + 8 + 14 + 35 medium/high-priority `experiment_proposals.v1.json` items resolved (REE_assembly `a3f4236c25`, `a1c00ea168`, `117881e7af`, `65909f6fcb`, `0106236035`, `e7be2c32d4`) -- confirmed already-answered / substrate-blocked / duplicate / superseded-by-realization, plus `MECH-428` blocked_substrate (`1e00faaf8e`), `MECH-472` executed by V3-EXQ-882 (`fb2967eab9`), `ARC-063` blocked_substrate (`3499ff3d9f`), `Q-019/Q-088` blocked_substrate (`335d647b65`), `MECH-317` gated superseded-by-realization (`2c818d9e79`), and `EXP-0384 MECH-426 progress_velocity_maintenance` route blocked_substrate (`ec26557d12`). (h) **Hypothesis-space cleanup** (REE_assembly `ccc067123e`): 2 stale-alive `competence_floor` legs resolved for the withdrawn MECH-476. (i) **New MECH claim registered**: MECH-472 (held-out context vs memorization) with V3-EXQ-882 as its executor (`0062ba7`). (j) **Lit-pull Q-088**: 5 entries on develop-without vs mature-lesion dissociation (REE_assembly `3b0226db7e`, IGW-20260802-225, EVB-0483). (k) **IGW workset**: retest lane now respects `experiment_proposals.v1.json` blocked_substrate status (REE_assembly `a7983a7e05`).
  **Bottleneck (shifting toward MECH-457 F-dominance conversion-ceiling AND cross-level subgoal credit):** the load-bearing near-term item is now a **two-front attack**: (1) the F-dominance conversion-ceiling MECH-457 stack, whose outward face has been substantially attacked over the last 3-day window (MECH-180 / INV-050 graduated 2026-08-01; SD-hazard-aware-policy-decomposition + MECH-122 content-packaging + SD-057 L7 amend landed in the 08-01 window; SD-MECH267-HORIZON-DEPTH landed 08-02 to close the MECH-267 mode-conditioned proposal gap); and (2) the newly-live **cross-level subgoal credit + progress-velocity effort/persistence** front (SD-092 + SD-093 both landed 08-02; V3-EXQ-883 + V3-EXQ-884 pending scores; SD-091 + MECH-481 coalition/topology substrate at 3/5 steps of implementation, MECH-481 falsifier gated on steps 4-5). Pending_review rebuild to 19 items reflects a HIGH-throughput autopsy-and-successor day rather than a stall -- 13 autopsies applied + 8 new successor experiments queued. Green-board target 2026-07-19 is now **15 days overdue**; the queue is 8 items but structurally in-motion (5 CLAIMED right now, 3 pending are the fresh SD-092/094 validators). **ETHICS-PERIMETER Phase 0 datum** stays on the record (Phases 1-3 deferred; NON-BLOCKING).

---

- **2026-08-02T01:10Z nightly read (scheduled `/update-docs`, bot identity).**
  1-day window since the 2026-08-01 nightly. Manifest count on disk: **769 `v3_exq_*` + 3 `v4_exq_*` flat** (+24 v3 vs the 2026-08-01 nightly's 745). **Currently queued (`experiment_queue.json` items[]): 9 items** (down sharply from the 16-item peak on 2026-08-01) -- 4 CLAIMED (V3-EXQ-862 Q-040c dACC PE-weight prio 45, V3-EXQ-857a Q-086 gentler-env confound control prio 40, V3-EXQ-858 ARC-062 GOV-FANOUT-1 P-B prio 40, V3-EXQ-863 ARC-062/MECH-309 lateral_pfc route ablation replication prio 15) + 5 PENDING (V3-EXQ-867 MECH-321 harm-aware selection prio 45, V3-EXQ-828b INV-091 dynamic-budget re-run prio 40, V3-EXQ-436b MECH-166 wall-independent representation confirmer prio 32, V3-EXQ-108a MECH-135 discriminative pair prio 25, V3-EXQ-866 INV-034/Q-021 goal-maintenance-vs-avoidance ablation prio 20). **Pending review (`pending_review.md`, regenerated 2026-08-01T22:12:19Z): 0 items -- fully cleared** (down from 1 on 2026-08-01).
  **Substrate + governance landings in the window:** (a) **`/governance` cycle 2026-08-01 (partial->resumed->second-resume) APPLIED** (REE_assembly `a82c5ed16e` + `7f90dae525` + `5c7e5e1134` + `7cd432683a`): 13 confirmed autopsies applied (V3-EXQ-853 H2 / 475b / 828a / 851 / 840b / 861 / 864 / 847 / 843 / 836e / 856 / 857 / 848 / 860 / 859), MECH-476 RETIRED (baseline-informativeness reversal falsifier withdrawn per V3-EXQ-836e autopsy), 4 clean PASS evidence items landed, and **MECH-180 / INV-050 `substrate_ceiling` -> `standard` GRADUATED** (attacks the outward face of the F-dominance conversion ceiling MECH-457 stack). Also: `mech294_coherence_magnitude_strengthening` + `mech122_content_packaging_offline_protection` substrate_queue entries minted. (b) **THREE substrate landings in the window:** (i) **SD-hazard-aware-policy-decomposition IMPLEMENTED** (ree-v3 `a4a1b1d`, REE_assembly `b88880f068` design doc) -- V3-EXQ-844 autopsy successor, wires z_harm_a-scaled harm-penalty selection into MECH-321 policy_decomposition (harm_bias gain + threshold-gated leaf-tile override). (ii) **MECH-122 content-packaging IMPLEMENTED** (ree-v3 `a7d3642`, REE_assembly `bcd71ff226`, IGW-20260801-197) -- wires the pre-existing ThetaBuffer consolidation-mode helpers (dead code since EXQ-246) into `run_sws_schema_pass` as per-prototype cosine-novelty content SELECTION, closing the V3-EXQ-861 gap where the diversity DV was structurally scale-invariant to the pre-existing MECH-272 anchor_weight lever. (iii) **SD-057 L7 amend: dACC goal_readout calibration fix** (ree-v3 `4b79d18`, REE_assembly `c44928a475`, IGW-20260801-199 `arc005_dacc_adapter_goal_proximity_training`) -- fixes wiring gap + adds per-candidate-set min-max normalise so the goal-readout channel actually reaches the committed argmin under `dacc_goal_readout_weight>0`. (c) **ARC-071 / MECH-090 E3-tick reselection short-circuit** (ree-v3 `31ed78e` + `278599a`) -- committed arc071_chunk trajectories skip re-selection between E3 ticks for their unexpired horizon, preventing the commit-latch persistence bug V3-EXQ-810a/834 exposed. (d) **Corpus-wide agent-seed-order lint** (ree-v3 `ccb9624` + `3a654c1` precision fix + `b3b35cb` docstring backlog record) -- 18-file agent-constructed-before-torch-seeded triage completed (all 18 immaterial). (e) **GOV-SKILL-1 standing skill-improvement recurrence audit BUILT** (REE_assembly `c40f241e95` + `fd79dd36c3`) -- planned closure node for the standing skill-audit scope. (f) **New MECH/SD claims registered:** MECH-480 (ACC-LOFC execution-gain dissociation), SD-091 + MECH-481 (selective cognitive coalition instantiation) via `a016fa7991`. (g) **New evidence intake:** mouse OFC information-value / GCML cognitive-map sampling / working-memory replanning / DLS procedural replay / foundational AI paper catalogue / Nova machinery / schizophrenia synaptic vulnerability / free-recall strategy / siibra multilevel atlas / metacognitive coalition control (11+ thought-digestion + lit-pull commits).
  **Bottleneck (shape shifting sharply, toward the conversion-ceiling attack maturing):** the load-bearing near-term item has NARROWED -- pending_review CLEARED (0), the peak 16-item queue is now 9, and the day's `/governance` cycle graduated MECH-180 / INV-050 out of substrate_ceiling. The remaining queued portfolio is a mix of near-tie/confounder confirmers (V3-EXQ-436b MECH-166 wall-independent, V3-EXQ-108a MECH-135 discriminative pair, V3-EXQ-866 INV-034/Q-021 goal-maintenance-vs-avoidance ablation, V3-EXQ-857a Q-086 gentler-env confound control) alongside the live conversion-ceiling attack (V3-EXQ-858 ARC-062 GOV-FANOUT-1 P-B f_weight attenuation ladder + V3-EXQ-863 lateral_pfc route ablation replication + V3-EXQ-867 MECH-321 harm-aware selection task-outcome validation). Substrate side minted three new landings (SD-hazard-aware / MECH-122 content-packaging / SD-057 L7 amend) with immediate autopsy-successor followups. Green-board target 2026-07-19 is now **14 days overdue**, but governance throughput was HIGH in the window (13 autopsies + MECH-476 withdrawal + a two-node ceiling graduation in a single day). **ETHICS-PERIMETER Phase 0 datum** stays on the record (Phases 1-3 deferred; NON-BLOCKING).

---

- **2026-08-01T01:10Z nightly read (scheduled `/update-docs`, bot identity).**
  ~4-day window since the 2026-07-28 nightly (the 2026-07-30 and 2026-07-31 slots were absorbed by the daylight-side roadmap.md snapshots; this bullet catches the spec back up). Manifest count on disk: **745 `v3_exq_*` + 3 `v4_exq_*` flat** (+23 v3 vs 2026-07-28, +5 vs 2026-07-30's ~740). **Currently queued (`experiment_queue.json` items[]): 16 items** (up from 4 on 2026-07-29 and 2 on 2026-07-28) -- a big queueing round on top of a successful `/governance` cycle drain. Highest-priority pending: **V3-EXQ-850** (GOV-FANOUT-1 H1: does F1/REM precision recalibration damp SD-076 drift, prio 60), **V3-EXQ-849** (Q-081 cheap reach pre-flight SCAN, prio 55), **V3-EXQ-853** (GOV-FANOUT-1 H2: 5x training exposure closure, prio 55, ~140m), **V3-EXQ-845** (MECH-180 ecological end-to-end SD-MEL-PRODUCER novelty, prio 50), plus the ARC-062 GOV-FANOUT-1 P-A/P-D legs (V3-EXQ-851 prio 45 ~600m; V3-EXQ-847 prio 40 ~600m), the MECH-476 DOSE/NOVELTY/INTERVAL REDESIGN retests (V3-EXQ-836a/d/e), and INV-091 828a / MECH-324 829a / MECH-294 840b re-runs. **Pending review (`pending_review.md`, regenerated 2026-07-31T20:50Z): 1 item** -- 0 PASS, 1 FAIL: **V3-EXQ-841** (MECH-163/Q-085 grain dose-response FAIL 2026-07-31T08:05, non_contributory with in-progress failure autopsy staged as `683b27628e`).
  **Substrate + governance landings in the window:** (a) **`/governance` cycle 2026-07-30T evening APPLIED** (REE_assembly `df210cc8e9`): 14-item pending_review walk (batch-687a-707c-840-748a-833-842-810b-673-614-798afail + 4 standalone confirmed autopsies), plus 3 reconfirmed promotion/demotion agenda holds, 819a/798a follow-ups applied, and a new `mech294_coherence_magnitude_strengthening` substrate_queue entry. Follow-up commit `210ae96695` closed EVB-0444/EVB-0287 as already-answered. (b) **Multiple substrate landings this window:** SD-085 `e3.reality_cost_weight` design doc (REE_assembly `1ea5832061`); MECH-324 reacquisition-window isolation fix (ree-v3 `7747a01c94`; corrects V3-EXQ-829's confirmed FAIL -- reacquisition-window contamination via ChunkAccumulator._tally saturation; validation queued as V3-EXQ-829a); ARC-071 commit-latch persistence fix (ree-v3 `278599a`; falls back to `e3._persistent_committed_trajectory` when both `e3._committed_trajectory` and `e3._closure_committed_trajectory` are None; validation experiment `chip-20260731-arc071-commit-latch-fix`); SD-033b GAP-8-affordability reusable trained-OFC-head cache (REE_assembly `99d3717741`, chip-20260730-ofc-deval-affordable); SD-036 harm-stream decay recurrence recorded (ree-v3 `6ffb2aa5b1`, staleness audit KEEP-all); and a MECH-204/SD-076 waking confidence drift magnitude lit-pull (REE_assembly `fd43046369`) grounding GOV-FANOUT-1 H3. (c) **ARC-062 GOV-FANOUT-1 portfolio structuring landed:** P-A gated_policy route erratum caught (REE_assembly `971b67bf19`); P-B routed to /implement-substrate (no direct F-weight/MECH-090 attenuation knob exists, `85c895119e`); P-C STOP-CHECK not-queued (H1/H2 already resolved by V3-EXQ-813, `53eb2070e3`); P-D queued as V3-EXQ-847 (`6cfc84de5b`). (d) **Additional plan / infra housekeeping:** `runner_git_health` now grades each prepull stash entry for containment (REE_assembly `c0c9437755`) + clears worker STRANDED false-positives via the parent's fresh refs (`499cc17261`); closure-map orphaned-meta lane exposed as a SELF-ASSESSMENT view (`7b2797417b`); CLINICAL lane added for psychiatric failure modes (`e58b751915`); phase3 substrate_commit carried through the flat->pack projection (`a865491d9f`); 20 stale plan Status-table rows reconciled to frontmatter nodes (`8804a8a112`).
  **Bottleneck (shape shifting):** the load-bearing near-term item is the **GOV-FANOUT-1 portfolio** (MECH-204/SD-076 waking-confidence-drift consolidation attack -- H1 F1/REM damping + H2 5x training closure + the ARC-062 P-A/P-D legs) plus the **MECH-476 DOSE/NOVELTY/INTERVAL REDESIGN retests** on noise-scaled margins. Queue capacity is used deliberately -- **16 items pending, 1 in-flight verdict adjudication** (V3-EXQ-841 MECH-163/Q-085 autopsy in progress), so the near-term unblock is running down the fanout portfolio before the next `/governance` cycle. **ETHICS-PERIMETER Phase 0 datum** stays on the record (Phases 1-3 deferred; NON-BLOCKING; V3 green-board target 2026-07-19 now 13 days overdue -- governance cadence recovered strongly on 07-30 with the 14-item walk).

---

- **2026-07-28T01:12Z nightly read (scheduled `/update-docs`, bot identity).**
  1-day window since the 2026-07-27 nightly. Manifest count on disk: **722 `v3_exq_*` + 3 `v4_exq_*` flat** (+8 v3 vs the 2026-07-27 nightly's 714 -- 5 of the 8 are the autopsy-driven successors from yesterday's 07-27 batch: V3-EXQ-032d MECH-102 corrected re-issue, V3-EXQ-819a MECH-457/INV-088 zworld_trained_vs_random gate-fix, V3-EXQ-816d MECH-321 policy_decomposition harshened env, V3-EXQ-827a INV-091 phase-sync redesign, V3-EXQ-824a Q-081 landmark-removal successor). **Currently queued (`experiment_queue.json` items[]): 2 items** -- both CLAIMED, both `claim_ids=[]` (diagnostic): **V3-EXQ-833** (Stage-H STRICT goal isolation DV: does silencing the E3 goal term change LEARNED avoidance? -- follow-on to the 2026-07-27 `scaffolded_sd054_onboarding` strict-goal-isolation amend documented in `ree-v3/CLAUDE.md`; priority 45) + **V3-EXQ-798a** (SD-MEL-PRODUCER validation, C4 measurable: graded, above-reference, LEARNABLE waking MEL from a non-converging world -- C4-readable successor to the 07-22 SD-MEL-PRODUCER validation; priority 34). **Pending review (`pending_review.md`, regenerated 2026-07-27T05:36:42Z; NO new `/governance` cycle since): 11 items** -- 3 PASS + 8 FAIL awaiting the next `/governance` cycle. PASS: V3-EXQ-825 (MECH-245 generative-dominance), V3-EXQ-032d (ARC-024 / MECH-102 / SD-003), V3-EXQ-819a (INV-088 / MECH-457). FAIL: V3-EXQ-822a (SD-078/SD-082), V3-EXQ-826 (MECH-244), V3-EXQ-817a (SD-004/SD-080), V3-EXQ-827 (INV-091), V3-EXQ-824 (Q-081), V3-EXQ-816d (MECH-321), V3-EXQ-827a (INV-091 phase-sync), V3-EXQ-824a (Q-081 landmark-removal).
  **Substrate + governance landings in the window:** No new `/governance` cycle since the 2026-07-26T16:46Z cycle (5 FAILs from yesterday's 07-27 snapshot completed their autopsy-driven successors and returned FAIL again -- the MECH-321 policy_decomposition dose-escalation retest, INV-091 phase-sync redesign, and Q-081 landmark-removal successor all FAILed a second time and route back to `/failure-autopsy`; V3-EXQ-032d MECH-102 corrected re-issue and V3-EXQ-819a MECH-457/INV-088 gate-fix both PASSed). The V3-EXQ-792a MECH-457 H-retention-consolidation re-run (from the 07-25 snapshot, CLAIMED `ree-cloud-2`) has not returned a verdict yet. See `ree-v3/CLAUDE.md` for the substrate detail (the 2026-07-27 `scaffolded_sd054_onboarding` STRICT goal isolation amend that V3-EXQ-833 tests is the most recent no-op-default substrate change).
  **Bottleneck (shape unchanged, adjudication debt accruing):** the 11-item pending-review backlog is the load-bearing near-term item -- the second FAIL of MECH-321 / INV-091 / Q-081 on autopsy-driven successors argues for a `/failure-autopsy` cluster rather than another single-successor letter, and the `/governance` cycle needs to walk all 11 (3 PASS + 8 FAIL) before the next cycle of queueing. The MECH-457 conversion-ceiling attack continues on the H-retention-consolidation leg (V3-EXQ-792a still in flight); V3-EXQ-833 opens a Stage-H strict-goal-isolation front on the scaffold; V3-EXQ-798a re-attempts the SD-MEL-PRODUCER validation with a C4-readable metric. Green-board target 2026-07-19 is now **9 days overdue**. **ETHICS-PERIMETER Phase 0 datum** stays on the record (Phases 1-3 deferred; NON-BLOCKING).

- **2026-07-27T01:03Z nightly read (scheduled `/update-docs`, bot identity).**
  1-day window since the 2026-07-26 nightly. Manifest count on disk: **714 `v3_exq_*` + 3 `v4_exq_*` flat** (+16 v3 vs the 2026-07-26 nightly's 698 -- the day's 8+ newly-queued experiments drained through the phase3 writer along with the prior snapshot's CLAIMED V3-EXQ-793a). **Currently queued (`experiment_queue.json` items[]): 0 items** -- fully drained (the 07-26 snapshot's 1-item CLAIMED queue for V3-EXQ-793a completed + every new experiment queued in the window either landed a manifest or was replaced by an autopsied successor). **Pending review (`pending_review.md`, regenerated 2026-07-26T18:04:15Z): 6 items** -- 1 PASS + 5 FAIL awaiting the next `/governance` cycle: V3-EXQ-825 (MECH-245 generative-dominance-under-deafferentation, PASS) + V3-EXQ-822a (SD-078/SD-082 rule-selection-consumer chain, FAIL) + V3-EXQ-826 (MECH-244 precision-weighted PE self-sealing, FAIL) + V3-EXQ-817a (SD-004/SD-080 world-effect grounding capability, FAIL) + V3-EXQ-827 (INV-091 cross-stream similarity band, FAIL) + V3-EXQ-824 (Q-081 landmark-removal, FAIL).
  **Substrate + governance landings in the window:** (a) **`/governance` cycle 2026-07-26T16:46Z APPLIED** (REE_assembly master `de0c34a3cb`): 793a/816/816b/816c/817/819/820/822/823 walk applied; **SD-079 PROMOTED** (`hippocampal.common_mode_invariant_goal_anchor_match`); 12 pending_user holds acknowledged; 8 experiments walked. (b) **SD-082 SUBSTRATE LANDED** 2026-07-26 (ree-v3 main `d4f7580`; REE_assembly master `d36fd8ba2f` register + queue): common-mode-invariant trained rule_state -> action-bias read-out consumer for SD-033a lateral_pfc -- the propagation half of the SD-078 centered CandidateRuleField chain (SD-078 differentiated the rule_state; SD-082 propagates it to E3 selection). V3-EXQ-822a re-run queued + completed FAIL in the same window (batch autopsy already applied). (c) **Five failure autopsies APPLIED** in the window: V3-EXQ-824 Q-081 landmark-removal precondition-check defect (REE_assembly `7206ed0160`) -> V3-EXQ-824a redesign queued (ree-v3 `caf3f37`); V3-EXQ-822a/826/817a/827 batch autopsy (REE_assembly `14745f29c9`; SD-078 second-order head-insensitivity + MECH-244 E1-tick-omission script bug + SD-004/SD-080 grounding-real-not-load-bearing split + INV-091 lockstep tick-rate confound) -> V3-EXQ-827a phase-sync redesign queued (ree-v3 `fad29a1`); V3-EXQ-032-family E1/E2-self tick-omission precedent-audit (REE_assembly `b04d79c158`) -> V3-EXQ-032d MECH-102 corrected re-issue queued (ree-v3 `bf4fbaa`); V3-EXQ-059c/533 MECH-102 encoder-untrained depth-check (REE_assembly `043b370094` + label correction `bcdc1689dc`). (d) **StepHarness + stream_recorder bug fixes** (ree-v3 main `4ee2264` + `d673790`): StepHarness.step() never advanced agent._step_count; stream_recorder e3_commitment freshness fix (clock-derived -> value-derived). Precedent audit routed V3-EXQ-032-family through the same defect. (e) **Public research web layer LANDED** (REE_assembly master `74172145d1`) as a new external deliverable class. (f) **Additional queued experiments (all landed or superseded in the window):** V3-EXQ-819a (MECH-457/INV-088 diagnostic worst-seed reconcile, seeds 3->6, supersedes 819), V3-EXQ-816d (MECH-321 policy_decomposition P-A dose escalation, env_drift_interval->1 + world_rule_shift 12/2), V3-EXQ-825 (MECH-245 discriminative pair), V3-EXQ-826 (MECH-244 self-sealing).
  **Bottleneck (shape unchanged, adjudication cadence high):** the MECH-457 conversion-ceiling attack continues on the H-retention-consolidation leg (V3-EXQ-792a still in flight from the 07-25 snapshot); the MECH-321 policy-decomposition falsifier is now on its dose-escalation retest (V3-EXQ-816d); the SD-078 -> SD-082 rule-selection-consumer chain is fully built + validated (822a FAIL adjudicated in the batch autopsy as SD-078 second-order head-insensitivity, routing to the SD-082 propagation read-out re-run); INV-091 phase-sync redesign (827a) supersedes the lockstep tick-rate confound. Governance-cycle throughput remains high (2 cycles in the 07-25..07-26 window, 8 experiments walked each). Green-board target 2026-07-19 is now **8 days overdue**. **ETHICS-PERIMETER Phase 0 datum** stays on the record (Phases 1-3 deferred; NON-BLOCKING).

- **2026-07-26T01:11Z nightly read (scheduled `/update-docs`, bot identity).**
  1-day window since the 2026-07-25 spec update. Manifest count on disk: **698 `v3_exq_*` + 3 `v4_exq_*` flat** (+38 v3 vs the 2026-07-25 nightly's 660 -- the day's queue drained many pending items through the phase3 writer). **Currently queued (`experiment_queue.json` items[]): 1 item** -- V3-EXQ-793a (SD-049 ARM_2 curriculum/density repower; CLAIMED `ree-cloud-4`; priority 15; 6-seed 2x2 repower of V3-EXQ-793 with a new C_JOINT_ROBUST criterion making guard_pass load-bearing for the joint arm per the failure_autopsy_V3-EXQ-793 repair pathway). **Pending review (`pending_review.md`, regenerated 2026-07-26T00:09Z): 3 items** -- 0 PASS + 3 FAIL awaiting the next `/governance` cycle: V3-EXQ-820 (MECH-321 policy-decomposition bottleneck ARM_2 FAIL), V3-EXQ-817 (SD-080 consequence-grounding falsifier FAIL), V3-EXQ-816 (MECH-321 policy-decomposition discriminative FAIL).
  **Substrate + governance landings in the window** (largely captured in the 07-25 spec write; this entry summarises what has run + closed since): (a) **MECH-321 / ARC-070 policy decomposition** IMPLEMENTED 2026-07-24 (`ree_core/policy/policy_decomposition.py`) as the inverse composition operator to ARC-071 -- decomposes chunk candidates at rollout time via MECH-288 EventSegmenter boundaries and R1 V_s + R5 bottleneck triggers; both selectable via `decomposition_trigger_mode`. The 816/820 discriminative + bottleneck ARM_2 diagnostics landed under this substrate. (b) **MECH-457 consummation-binding leg** substrate landed 2026-07-25 across two builds: `mech457_consummatory_act` env extension (a distinct CONSUME action index 5 so contact AFFORDS rather than EFFECTS consumption) + `mech457_approach_extinction` (drive-side extinction-on-contact via `info["on_consumable_resource"]`). Companion `consummatory_aware_reference_policies` in `experiments/_lib/capability_eval.py` so the retention BC-install framing can take in the consummatory env. V3-EXQ-821 (H-consummation-binding leg 4) queued/completed. (c) **SD-080 action-object head frozen-at-init** falsifier V3-EXQ-817 completed FAIL (awaiting adjudication). (d) **Umbrella tooling**: the 07-25 snapshot's 660 disk count -> today's 698 reflects the queue drain (778a and other terminal items landing manifests + the 793a family filling the CLAIMED slot; several MECH-321 / MECH-457 / SD-080 diagnostics closing).
  **Bottleneck (unchanged shape):** the MECH-457 conversion-ceiling attack continues on the consummation-binding + retention legs (821 completed, 792a still in flight per the 07-25 snapshot); the MECH-321 policy-decomposition falsifier pair (816 discriminative + 820 bottleneck) both FAIL and route to `/failure-autopsy` before `/governance` walks them; SD-080 action-object frozen-at-init also FAIL awaiting adjudication. Green-board target 2026-07-19 is now **7 days overdue**. **ETHICS-PERIMETER Phase 0 datum** stays on the record (Phases 1-3 deferred; NON-BLOCKING).

- **2026-07-25T01:33Z nightly read (scheduled `/update-docs`, bot identity).**
  3-day window since the 2026-07-22 spec update. Manifest count on disk: **660 `v3_exq_*` + 3 `v4_exq_*` flat** (+16 v3 vs the 2026-07-21 nightly's 644). **Currently queued (`experiment_queue.json` items[]): 20 items** -- 4 claimed (V3-EXQ-792a on `ree-cloud-2`; 707c on `ree-cloud-4`; 797 on `ree-cloud-3`; 734b on `ree-cloud-1`), 16 pending. New this window: V3-EXQ-792a (MECH-457 H-retention-consolidation re-run with `PolicyDriftMonitor` measuring the control's drift instead of a 0.0 sentinel, per-arm anchor-bound proof, n=3->6 seeds); V3-EXQ-708b (MECH-440 pre-commit distribution SHAPE falsifier, supersedes 708a run); V3-EXQ-791a (channel-routing route_range replication re-run, gate fix only); V3-EXQ-809 (SD-080 prior probe -- action-object init invariance of EXQ-003 TERRAIN-vs-RANDOM); V3-EXQ-808 (return-decomposition diagnostic for H-objective-misspecification / `conversion_ceiling_root`); V3-EXQ-114a / V3-EXQ-120a (re-run scripts superseding EXQ-114/120 on the action-object round-trip DEFECT); V3-EXQ-266b (Q-020 valence geometry pair, supersedes 266/266a); V3-EXQ-800 / 801 dispositioned as false-positives on the action_object_selection lint (exempted); plus the pending Q-081 landmark-removal successor structure-destroying arm. **Pending review (`pending_review.md`, regenerated 2026-07-24T08:05Z after the day's `/governance` cycle): 0 items -- fully cleared** (all experiments reviewed).
  **Substrate + governance landings in the window:** (a) **Action-object round-trip DEFECT diagnosis + CEM elite floor fix** (ree-v3 main `6064f90` + docs refinement `3912d83` 2026-07-22): the composition `a -> E2.action_object(a) -> action_object_decoder` is NOT INVERTIBLE -- `argmax(action_object_decoder(traj.get_action_object_sequence()[:,0,:]))` is a CONSTANT (class 3, measured; a driver selecting that way has an action stream invariant under every candidate-set manipulation = arithmetic no-op). NEITHER component is individually degenerate; the COMPOSITION is (measured session `epic-burnell-995d28`). Correct accessor: `HippocampalModule.candidate_first_action_class(traj)` (reads `trajectory.actions` = ground truth); correct selection: `agent.select_action(candidates, ticks)` (E3's J(zeta), never consults the decoder). Sanctioned decoder use unchanged: the CEM proposal path (`E2.rollout_with_world` consumes the continuous vector). Live diagnostic `action_object_roundtrip_recovery`. DEFECT 2 in the same landing: `num_elite` must be >= 2 (torch's default `std()` is UNBIASED -> NaN on singleton; propagates through SP-CEM `torch.clamp`); fix at `num_elite = min(n, max(2, int(n * elite_fraction)))` -- reachable from the substrate DEFAULT silently (`num_candidates=8 * elite_fraction=0.2 -> int(1.6) == 1`). 9 new contracts pin BOTH defects. (b) **6 lint-fire dispositions** (ree-v3 main `564411e`): 800/801 FALSE POSITIVES exempted; 114/120/266/266a CONFIRMED invalidated + replaced by 114a/120a/266b. (c) **SD-076 headroom repair + `dose_saturation` lint** (ree-v3 main `452f99e` 2026-07-22): the SD-076 rv floor is now scale-relative + softplus-saturating; the new `dose_saturation` lint catches two declared dose levels whose per-level float readouts are equal beyond float noise (the V3-EXQ-794 signature where SD-076 overconfidence_score = -1.004... at BOTH LO and HI so the lever never moved). (d) **SD-080 registered** 2026-07-22 (`e2.action_object_head_frozen_at_init`, candidate; scoping-spike measured on the live substrate: 99.5% of action-object variance explained by the action LABEL alone; parameters bit-identical after 40 warmup episodes). (e) **SD-081 substrate landed + MECH-477 registered** 2026-07-22 (SD-081 = `e3.dualsystem_uncertainty_arbitration`; MECH-477 = the control-allocation mechanism MECH-163 presupposed but never named; both candidate/v3; the 786a corrected-successor falsifier must ablate ONLY the arbitration weight with both pathway reads present). (f) **Governance cycle 2026-07-24 APPLIED** (REE_assembly master `0b64bfbf4e` + `3853becb40`): 5 confirmed autopsies applied to claims.yaml (MECH-440 / MECH-457 / MECH-204 / SD-076 / MECH-163 / MECH-477 / MECH-448 / SD-049) + substrate_queue `sd_waking_confidence_inflation_headroom`. 15 experiments marked reviewed; pending_review 0. `V3-EXQ-786a` DV degeneracy re-adjudicated -- MECH-163 leg1 `weakens` WITHDRAWN, `non_contributory` (`f316757008`). 4 additional targets cleared same cycle (790-run2, 689i, 742b extends cluster, 793) via `454cffcc22`. HPL-1 closure-plan node updated with the 786a withdrawal + a repaired 786b retest queued (`b310745387`). (g) **Runner + writer hardening** 2026-07-23..24: `pack_writer.write_flat_manifest` output-dir resolution and the ree-v3 pre-commit contract gate now resolve REE_assembly paths worktree-aware (ree-v3 main `5623c25` + `b4c23f6`); `test_arm_reuse.py` worktree-aware (ree-v3 `ddc3ead`); Q-081 constrained-realisation surrogate-null validation contract for unequal-rate multi-stream traces + pre-use validation (ree-v3 main `47ee07d`) + structure-destroying (landmark-removal) arm BUILT (ree-v3 main `31e82a8`; REE_assembly master `614c06c815` closure-plan note).
  **Bottleneck (unchanged shape, portfolio SHARPENING under the 07-24 governance cycle):** the MECH-457 conversion-ceiling attack keeps consolidating around the H-retention-consolidation leg via **V3-EXQ-792a** (drift-monitor-corrected KL-anchor re-run); the SD-070 z_world encoder warmup adoption stays in flight via **V3-EXQ-734b** (still claimed on `ree-cloud-1`); the action-object round-trip defect has now been (i) diagnosed at the composition level, (ii) covered by contracts pinning both defects, and (iii) re-issued as replacements for the 4 confirmed-invalidated lint fires (V3-EXQ-114a/120a/266b + the 707c ARC-110 instrument repair). Green-board target 2026-07-19 is now **6 days overdue** but the tempo of substrate correction + governance-cycle throughput has picked up (5 autopsies applied in one cycle 07-24). **ETHICS-PERIMETER Phase 0 datum** stays on the record (Phases 1-3 deferred; NON-BLOCKING).

- **2026-07-21T01:20Z nightly read (scheduled `/update-docs`, bot identity).**
  Live day (SD-070 driver adoption + MECH-204 Phase 7 / SD-076 substrate pair LANDED; MECH-457 z_world-guard-fanout retest portfolio + SD-070 adoption validation portfolio QUEUED; `/governance` cycle 2026-07-20T20:28Z APPLIED (V3-EXQ-728 FAIL adjudicated as z_world per-copy untrained-encoder defect, non_contributory)).
  Manifest count on disk: **644 `v3_exq_*` + 3 `v4_exq_*` flat** (+8 v3 vs the 2026-07-20 nightly's 636; the day's landings are dominated by the MECH-314 curiosity-conversion 5-run cluster synced up from Mac-local runs -- `531e547008` -- plus the run manifests for V3-EXQ-785a/785b/786/787/788/789 landing through the phase3 writer).
  **Currently queued (`experiment_queue.json` items[]) canonical (working-tree, phase3 snapshot fresh on `ea7aeff`): 14 items** -- 10 pending, 4 claimed. The Q-portfolio expanded in the day: **V3-EXQ-707c** (ARC-110 instrument-repair diagnostic, priority 50; CLAIMED `ree-cloud-3`); **V3-EXQ-742m-b** (MECH-457 mech457_actor_critic_onoff baseline OFF-arm re-mint at zworld_p0_episodes=60, priority 46; CLAIMED `ree-cloud-2`); the MECH-457 lineage **z_world-guard re-run set** V3-EXQ-742a / 734a (CLAIMED `ree-cloud-1` / `ree-cloud-4`; both riding the `_lib/zworld_encoder_guard` detector); the MECH-457 lineage **SD-070 adoption-validation set** V3-EXQ-737b / 734b / 742b (all PENDING; re-run of the pre-fix drivers with the P0a SD-070 z_world encoder warmup wired via the `_train_all_on_agent` family adoption landing `b523b9c`); **V3-EXQ-786a** (MECH-163 dual-system recruitment leg 1, supersedes V3-EXQ-786 non_contributory / measurement_test_design_defect; PENDING); **V3-EXQ-790 / 791** (cross-machine-class MAGNITUDE replication of the channel-routing route_range contrast, pinned `ree-cloud-3` and `DLAPTOP-4.local` respectively; PENDING); **V3-EXQ-793** (SD-049 ARM_2 foraging-competence 2x2 calibration probe; gated on `sd049_density_budget_truncated`; PENDING); **V3-EXQ-794** (MECH-204 Phase 7 x SD-076 2x3 substrate-readiness validation; PENDING); **V3-EXQ-689i** (MECH-448 instrument-repair of V3-EXQ-689d; CLAIMED `ree-cloud-2` after re-derive brake CLEARED); **V3-EXQ-699b** (MECH-448 instrument-repair of V3-EXQ-699; PENDING). V3-EXQ-795 (SD-024 benefit-terrain live-path validation, priority 50) landed on ree-v3 main `fc8724d` but is not on the working-tree queue snapshot as of this nightly's read (post-writer drain vs pre-writer state race -- the coordinator DB is authoritative and will resurface it or drop it in the next phase3-queue writer tick).
  **Pending review (`pending_review.md`, regenerated 2026-07-20T20:24:46Z after the day's `/governance` cycle): 0 items** -- fully cleared. V3-EXQ-728 was reviewed and adjudicated non_contributory in the cycle at 20:28Z (bd98ecce59).
  **Substrate + governance landings in the window:** (a) **SD-076 `precision.waking_confidence_inflation` + MECH-204 Phase 7 Option B `sleep.accuracy_anchored_broadcast_recalibration` PAIR IMPLEMENTED** (ree-v3 main `8ac193d` + `062516d70e` SD register + `bac7ae4419` substrate_queue landing record). SD-076 creates a waking overconfidence drift; MECH-204 Phase 7 emits a REM-consolidation broadcast that anchors E3 precision to accuracy so the drift is corrected offline -- deliberately paired (Phase 7 without SD-076 is a tautology; the pair is the falsifier substrate for the confidence-precision offline recalibration claim). V3-EXQ-794 2x3 substrate-readiness diagnostic QUEUED (ree-v3 main `35cf657` + widen `043b2ee`). (b) **SD-070 driver ADOPTION** in the `_train_all_on_agent` family LANDED (ree-v3 main `b523b9c` + doc `bacf74a` + REE_assembly master `bac7ae4419` substrate_queue landing record). SD-070 shipped 2026-07-18 as a z_world encoder warmup trainer but NO DRIVER CALLED IT -- this landing wires it into the shared `_train_all_on_agent` driver family, closing the z_world untrained-encoder confounder that non_contributoried V3-EXQ-728 (adjudicated in the same window's governance cycle) and V3-EXQ-734 / 737. Adoption-validation set V3-EXQ-737b / 734b / 742b queued. (c) **SD-024 benefit-terrain live-path producer** LANDED (ree-v3 main `490af60` + contract test `7f16f25`; REE_assembly master `b832d654a0` doc/claims/queue record). Wires the SD-024 density substrate into the live rollout benefit-terrain read (previously an eval-only readout); V3-EXQ-795 live-path efficacy validation QUEUED (`fc8724d`). (d) **SD-049 Phase 2 density-preserving per-type resource spawn** (ree-v3 main `2819cfa`; REE_assembly master `b0552b1fb7` doc record). Addresses the V3-EXQ-693a ARM_2 3type+novelty contact-ceiling by preserving spawn density per resource type; V3-EXQ-793 diagnostic 2x2 QUEUED. (e) **MECH-465 stage-1 spike RECORDED** (REE_assembly master `0f14f7e77f`): commit-gate boundary regime is margin-reachable by config but NOT gradeable -- running-variance dispersion (1.02-1.07x) is ~30x narrower than the 1.455x urgency range; probe-vs-run discrepancy reconciled as a warmup transient; routes to SD-063 wiring. (f) **MECH-314b/c and Q-044 substrate_ceiling PARKED** (REE_assembly master `4c80191676`): ceiling_decision deferred; audit false positive -- the 604c re-derive brake refuses the retest. (g) **Governance cycle 2026-07-20T20:28Z** APPLIED (REE_assembly master `bd98ecce59`): V3-EXQ-728 FAIL adjudicated non_contributory (z_world per-copy untrained-encoder defect, claim_ids=[], substrate_queue amend); 4 holds APPLIED (INV-047 + MECH-463 hold_candidate_resolve_conflict; MECH-459 + SD-031 hold_pending_v3_substrate; no status change, all stay candidate); audits CLEAN across the board (closure drift 0 drifted / 1 legit-suppressed; hypothesis-space integrity 0 flags / 9 labelled fan-out advisories / 7 git-witnessed pre-regs; substrate-ceiling 0 orphaned / 0 exhausted; GOV-DIAG-1 / GOV-APPLY-1 / GOV-CAT-1 all clean); GOV-FANOUT-1 fan-out recurrence noted on the `competence_floor` question (re-pose applied `c5bd4441ce`; portfolio 4 REFUSED, closed families NOT re-opened, `z_world installability` withdrawn as a hypothesis and re-posed as `sd_zworld_warmup_optimizer_group`); pending 0. (h) **Runner hardening** (ree-v3 main `dfe72ec` + `6b166e1` + `3569405`): make prepull-stash restore non-destructive + self-heal untracked-blocked pull; make prepull-stash matching generation-agnostic (`v3` -> `v[0-9]+`); make stdout verdict + crash-record run_id generation-agnostic. (i) **Re-derive brake refined** (ree-v3 main `d4cffd6`): count the epistemic CATEGORY, not the direction alone (the brake now cleared for MECH-448 V3-EXQ-689i / 699b). (j) **`fresh_select` extracted to `_lib`** (ree-v3 main `08e9955` + `acd48f9` V3-EXQ-699c migration): the E3 sentinel-key freshness instrument is now a shared helper; drops both lint exemptions.
  **Bottleneck (shape SHARPENING toward the MECH-457 conversion-ceiling attack):** the SD-070 adoption landing directly attacks the untrained-z_world-encoder defect that non_contributoried V3-EXQ-728 / 734 / 737 / 742 -- 8 re-run items are now on the queue split across two operationalisations: the ZWORLD-GUARD detector re-run (V3-EXQ-742a / 734a / 728a / 737a; the untrained-encoder guard fires or does not) and the SD-070 adoption validation (V3-EXQ-737b / 734b / 742b; a PROPERLY trained z_world encoder is a DIFFERENT arm from a frozen-random-projection arm). Together these decide whether the H1 REE-latent action-adequacy H1 refutation was substrate-real or a confounded artefact. The MECH-204 Phase 7 / SD-076 pair is now BUILT + queued (V3-EXQ-794) as the deliberately-paired confidence-precision offline-recalibration substrate. MECH-457 GOV-FANOUT-1 retention leg H-retention-auxiliary-decay was ELIMINATED in the 07-20 governance cycle (per e4834626a7); H-retention-consolidation is ALIVE and queued as V3-EXQ-792 on the KL-anchor leg (`4a05ff2c0c`). MECH-463 arousal-variance-amplifier fanout stays walked into holds (INV-047 + MECH-463 hold_candidate_resolve_conflict from the cycle). MECH-465 stage-1 spike found the commit-gate boundary regime is margin-reachable but not gradeable and routes to SD-063 wiring, not a further diagnostic. Green-board target 2026-07-19 is now **2 days overdue**; closure-plane weighted progress carries from the prior nightly (77.5% across 80 non-deferred nodes, per the 2026-07-19T11:18Z snapshot). ETHICS-PERIMETER Phase 0 datum stays on the record (Phases 1-3 deferred; NON-BLOCKING).
- **2026-07-20T01:11Z nightly read (scheduled `/update-docs`, bot identity).**
  Live day (MECH-463 fanout day; +1 MECH-457 retention leg BUILDABLE-AND-QUEUED; MECH-314 curiosity-conversion attack surface cleared FAIL non_contributory).
  Manifest count on disk: **636 `v3_exq_*` + 3 `v4_exq_*` flat** (+15 v3 vs the 2026-07-19 nightly's 621).
  **Currently queued (`experiment_queue.json` items[]) canonical (origin/main phase3-queue snapshot `cad8a7b`): 2 items** -- **V3-EXQ-789** (priority 55; `claim_id=MECH-457`; GOV-FANOUT-1 retention leg **H-retention-auxiliary-decay** on the newly-landed `mech457_retention_trajectory_probe` substrate; PENDING) + **V3-EXQ-699a** (priority 5; `claim_id=MECH-448`; re-run of V3-EXQ-699 for adjudicable preconditions after the 2026-07-19 declaration fix `0bfbb42`; smoke PASS; CLAIMED -- `3065054`). The daylight-time queue also carried **V3-EXQ-787** (MECH-463 hazard-geometry probe H-endogenous-hazard-geometry; CLAIMED; autopsy already ELIMINATED the leg 3/3 -- `b9babbe180`), **V3-EXQ-788** (MECH-457 GOV-FANOUT-1 H-retention-critic PAIR with 789 on the same substrate), plus four MECH-457-lineage z_world-guard-fanout retest items **V3-EXQ-742a / 737a / 728a / 734a** initially queued via session `interesting-noether-3f6326` `zworld guard fanout into 728/734/737/742` -- 787+788 ran through / were drained by the phase3 writer during the day, and 742a/737a/728a/734a existed in the reissuing session's uncommitted working tree but never reached the coordinator DB, so the fleet-authoritative queue is the 2 items above, not the 8 a stale working-tree read shows.
  **Pending review (`pending_review.md`, regenerated 2026-07-19T15:30:59Z): 1 item** -- **V3-EXQ-785a** MECH-463 arousal-exogenous-urgency-decomp FAIL/does_not_support (2026-07-19T13:34) awaiting the next `/governance` cycle. (Note: V3-EXQ-785b MECH-463 channel-agnosticism-decomp FAIL/does_not_support 2026-07-19T21:24 landed after the pending-review regen window; a re-regen is owed. The MECH-314 curiosity-conversion cluster below is already marked reviewed per the day's session.)
  **Substrate + governance landings in the window:** (a) **`mech457_retention_trajectory_probe` substrate IMPLEMENTED** (ree-v3 main `7e4f6e932b`; REE_assembly master `27c7cae0` register + `cfd220098e` validation-pair tie): non-perturbing mid-training competence probe wired into `action_learning.mid_training_competence_probe` -- unblocks the MECH-457 GOV-FANOUT-1 retention legs' trajectory DV without perturbing the training run; 11 new contracts T1-T7 in `test_mech457_retention_trajectory_probe.py`; 1873 pass; SD doc `sd_mech457_retention_trajectory_probe.md` flipped -> implemented. Brings the MECH-457 retention-portfolio buildability from 2/4 to 3/4. (b) **MECH-457 GOV-FANOUT-1 retention pair QUEUED** (ree-v3 main `5a7bcf9`; REE_assembly `cfd220098e`): V3-EXQ-788 H-retention-critic on the distributional-critic substrate (2026-07-18) + V3-EXQ-789 H-retention-auxiliary-decay on the BC-aux-schedule substrate (2026-07-18) -- both riding the newly-landed trajectory probe. PROMOTES NOTHING; diagnostic; excluded from governance scoring. (c) **MECH-463 arousal-variance-amplifier fanout (autopsy-lite day)**: V3-EXQ-785a arousal-exogenous-urgency decomp FAIL/does_not_support + V3-EXQ-785b channel-agnosticism at a second incumbent identity FAIL/does_not_support (ree-v3 main `79149e5` queue + `86f65a8` metrics-lib repair + `e89d3c8` RETRACT of the '785a never seeds torch' claim + REE_assembly `b957b17af` spike amend section-6 RETRACT + `da3b66d632` SD-075 spike replacing the untested warmup hypothesis with the measured warmup-rescue) + V3-EXQ-787 hazard-geometry probe (ree-v3 main `ba09660` build) with **H-endogenous-hazard-geometry ELIMINATED 3/3** in the autopsy (REE_assembly `b9babbe180`) -- three of the arousal-variance-amplifier substrate faces closed uniformly against MECH-463 in a single day. (d) **SD-075 `phasic.ema_episode_continuity` IMPLEMENTED 2026-07-19** (ree-v3 CLAUDE.md §SD-075 line 2585): baseline-continuity repair for the per-episode cold-EMA-reset defect in the SD-069 phasic burst regulator; 22 SD-075 contracts pass, SD-069's 8 contracts + flag-inertness unchanged. Sibling to SD-069 (shared phasic-source lit basis, different consumer). (e) **V3-EXQ-708 RE-ADJUDICATION** (REE_assembly `e0ebcbcecc`): pre-commit entropy DV invalid due to stale-E3 arm-dependent mass-weighting -> the earlier MECH-440 `washes at argmax` finding WITHDRAWN; route stands. (f) **MECH-314 curiosity-conversion attack** cleared 5 runs uniformly FAIL/non_contributory in the day: V3-EXQ-590c novelty goldilocks + V3-EXQ-705 conversion demotion + V3-EXQ-705b demotion adaptive-floor + V3-EXQ-706 conversion double-gated + V3-EXQ-706b double-gated validity -- pending the next `/governance` cycle's cluster adjudication. (g) **V3-EXQ-786 MECH-163 dual-system recruitment** FAIL/non_contributory 2026-07-19T16:39. (h) **precommit gate extended to `experiments/_lib/`** (ree-v3 main `2d32dc7` + `e54c291` test): the precommit gate was fail-open on shared substrate under `_lib`; now covered + test pins the inner interpreter to kill a false pass. Umbrella-repo commit `3dff03a` records the wiring. (i) **stale-E3 lint reconciled to 64** (ree-v3 main `a8568da` WARN on stale-E3-diagnostics pseudo-replication in new drivers + `58016e5` narrow by identity-guard). (j) **guard: untrained-z_world-encoder detection lifted into `_lib`** (ree-v3 main `9f72532`); audit shows all 6 `_train_all_on_agent` callers exposed. (k) **SD-068 anchor audit** (ree-v3 main `c897063` + `180eb13`): vacuous intact-readouts anchor recorded, four mis-declarations corrected; anchor lint gains `already-ran-and-superseded` category + lint-specimen guard. (l) **remote-pytest note correction** (ree-v3 main `69342b5`): multinomial diagnosis + lease + timing.
  **Bottleneck (unchanged shape, MECH-463 substrate faces closing + MECH-457 retention buildability improving):** the MECH-463 F-dominance entrenchment mechanism is being narrowed by systematic elimination -- 785a exogenous-urgency + 785b channel-agnosticism-at-second-incumbent + 787 hazard-geometry all closed against the variance-amplifier framing in one day, per the `/failure-autopsy` decisions. The MECH-457 GOV-FANOUT-1 retention-portfolio now sits on THREE buildable legs (2 queued: V3-EXQ-788 critic + V3-EXQ-789 aux-decay on trajectory-probe substrate; 1 buildable-but-not-yet-queued: distributional critic OR aux-decay's paired ancillary), with 1 leg still awaiting a fourth substrate build. MECH-314 curiosity-conversion 5-run cluster (590c/705/705b/706/706b) all FAIL/non_contributory pending governance adjudication -- likely to route via the MECH-314 ordering-gated-on-INV-088 frame the MECH-458 registration set up on 2026-07-17. Green-board target 2026-07-19 is now **1 day overdue**; closure snapshot 2026-07-19T11:18Z shows **77.5% weighted progress** across 80 non-deferred nodes (23 remaining; 10 assembling; 57 done). ETHICS-PERIMETER Phase 0 datum stays on the record (Phases 1-3 deferred; NON-BLOCKING).
- **2026-07-19T01:10Z nightly read (scheduled `/update-docs`, bot identity).**
  Substrate day. Manifest count on disk: **621 `v3_exq_*` + 3 `v4_exq_*` flat** (+15 v3 vs the 2026-07-18 nightly's 606).
  **Currently queued (`experiment_queue.json` items[]): 1 item** -- **V3-EXQ-779b** (priority 40; `claim_id=MECH-063`; MECH-063 sub-claim (ii) tonic/phasic dissociation ONE-PARAMETER exposure fix -- MAX_EPISODES_PER_CELL now DERIVED from MAX_ENV_STEPS_PER_CELL so the 2400-step cap binds for every cell; supersedes V3-EXQ-779a; `machine_affinity ree-cloud-4`; CLAIMED 2026-07-18T21:07Z; re-derive brake explicitly EXEMPTED per the 779a autopsy because the load-bearing criterion PASSED in 779a and 779b changes only exposure, not thresholds/margins/aggregation).
  **Pending review (`pending_review.md`, regenerated 2026-07-18T18:28Z): 7 items** -- 2 PASS + 5 FAIL awaiting the next `/governance` cycle: V3-EXQ-777a (MECH-063 orthogonal control-axes; FAIL), V3-EXQ-779a (MECH-063 tonic/phasic; FAIL, superseded by 779b), V3-EXQ-780 (MECH-457 BC-prior discrimination; FAIL), V3-EXQ-781 (MECH-457 approach-primitive discrimination; FAIL), V3-EXQ-782 (MECH-459 advantage-composition probe; FAIL).
  **Substrate + governance landings in the window:** (a) **SD-074** (`probe.trained_enough_agent_warmup`) REGISTERED as the composition primitive that lets the 2x2 read-only telemetry-probe family (MECH-063 / MECH-320 / MECH-313) run against agents warmed to a de-saturated E3 action distribution -- closes the V3-EXQ-777a starvation-below-ceiling defect; caching-hazard discipline for H1 (`_running_variance` plain float outside state_dict) / H2 (state_dict shareable across 2x2 arms with no per-arm modules) / H3 (`generate_trajectories` capture is an instance attribute, not in state_dict; `reapply_candidate_capture` post-load). (b) **SD-MECH457-DISTRIBUTIONAL-CRITIC** IMPLEMENTED as a VALUE-ESTIMATOR substitution (symlog / two-hot / HL-Gauss bins) on `ActorCriticPolicy` alongside the scalar + SF critics; unblocks **H-retention-critic**; PROMOTES NOTHING. (c) **mech457_bc_aux_schedule** IMPLEMENTED as a schedulable BC-auxiliary persistence (constant/annealed/off) with the guard-is-the-feature discipline; unblocks **H-retention-auxiliary-decay**; PROMOTES NOTHING. Two of four MECH-457 retention legs are now buildable BUT NOTHING QUEUED per GOV-FANOUT-1 (see `evidence/planning/mech457_retention_portfolio_2026-07-18.md`). (d) **MECH-463** REGISTERED (global-scalar arousal channels -- D1/D2 gain, harm-urgency threshold shrinkage, LC-NE temperature -- as a channel-agnostic VARIANCE AMPLIFIER of the already-dominant selection channel, not a source of behavioural differentiation; the F-dominance entrenchment mechanism) + **MECH-463 diagnostics** (E3 commit-gate scalars + per-candidate channel-term tensors surfaced under the existing `e3_score_decomp_enabled` flag; NOT computable from existing runs before this build) + V3-EXQ-785 two-regime incumbent-share variance decomposition (smoke PASS; not yet queued). (e) **SD-068 REM-leg autopsy** folded into `evidence_quality_note`; the REM leg RETIRED as a functional-damage instrument (fold debt discharged; 778d/e/f REM cluster read via the SD-068 rebuild's REM generative-fidelity readout). (f) **infant_substrate:GAP-14** two readiness-anchor specification defects RECORDED in the 591/ISEF-005 family (record-only). (g) **MECH-063 777a/779a autopsy followup 4** four-carrier survey + resolution recorded; `probe_warmup` c1_robust note pointed at `_lib/robustness_bars` (SEM-denominated + explicitly-named dispersion bars). (h) **validate_claims** gained a narrow `--duplicates-only` commit-guard mode (blocking gate, decoupled from other ERROR rules). (i) **serve.py** hardened against a malformed-request `AttributeError` in `log_message` by pre-parsing `self.path`; **ree-explorer.service** install example repointed at the canonical `REE_Working` clone away from the stale Documents/GitHub path; **daemon_code_drift** now grades the tree each daemon ACTUALLY runs from + flags unfetched clones. (j) **precondition annotations** hardened via a `precondition_recomputability` lint (missing direction; measured/met statistic mismatch; comparator alias fix).
  **Bottleneck (unchanged shape, sharper 24-hour cadence):** the MECH-457 conversion-ceiling attack now sits on a live GOV-FANOUT-1 GENERATION portfolio (V3-EXQ-780 / 781 / 782 all FAILed and awaiting adjudication) + a MECH-459 advantage-composition probe (782 FAIL); MECH-063 sub-claim (ii) sits on V3-EXQ-779b (the one-parameter exposure fix on cloud-4) whose autopsy explicitly EXEMPTS the re-derive brake because the load-bearing criterion already PASSED in 779a and 779b changes only exposure. The 780/781/782 FAIL cluster + the retired REM instrument + the two new MECH-457 retention substrates (distributional critic + BC-aux schedule) collectively re-open the MECH-457 attack surface without a queue-side commit, per GOV-FANOUT-1. Green-board target 2026-07-19 is **today**; the pending-review cluster's `/governance` walk + the 779b MECH-063 verdict are the load-bearing green-board contributors. ETHICS-PERIMETER Phase 0 datum stays on the record (Phases 1-3 deferred; NON-BLOCKING).
- **2026-07-18T01:11Z nightly read (scheduled `/update-docs`, bot identity).**
  Substrate day (SD-068 + SD-069) + REM generative-fidelity readout replaces the terrain-variance proxy.
  Manifest count on disk: **606 `v3_exq_*` + 3 `v4_exq_*` flat** (+13 v3 vs the 2026-07-17 nightly's 593) + **~2535 per-run pack manifests** under `runs/**/manifest.json` (thin-pack recording-provenance fix on 2026-07-16 healed 2519 packs at index time; per-run pack count now reflects the healed accounting rather than the previous under-count).
  **Currently queued (`experiment_queue.json` items[]): 1 item** -- **V3-EXQ-771** (priority 6; `claim_id=MECH-457`; GOV-FANOUT-1 H2 metabolic forage-to-survive env reward-coupling discrimination; `machine_affinity ree-cloud-3`; CLAIMED).
  **Pending review (`pending_review.md`, regenerated 2026-07-17T17:31:47Z): 10 items** -- 4 PASS + 3 FAIL among the 2026-07-17 unclaimed-manifest cohort (V3-EXQ-773 MECH-076 residue basin-geometry PASS/supports; V3-EXQ-775 MECH-086 dopamine selection-gain dose-response PASS/supports; V3-EXQ-776 MECH-279 PAG freeze-gate functional-signature PASS/supports; V3-EXQ-778 + V3-EXQ-778a SD-068 staged-damage diagnostic PASS/supports; V3-EXQ-777 MECH-063 orthogonal control-axes rank-2 falsifier FAIL/weakens; V3-EXQ-770 + V3-EXQ-772 MECH-457 GOV-FANOUT-1 discrimination FAILs; V3-EXQ-774 MECH-173 REM-precision recalibration FAIL/inconclusive) awaiting the next `/governance` cycle -- discussed_experiment_dirs marks pending. The 2026-07-17T18:21Z evening `/failure-autopsy` on V3-EXQ-774 already routed it to `substrate_ceiling / amend MECH-204 Phase 7 trigger`; awaiting Step-5 governance apply.
  **Substrate + governance landings in the window:** (a) **SD-068 consolidation-pipeline lesion harness** IMPLEMENTED 2026-07-17 as the experiment-layer instrument that makes the MECH-168 / INV-047 / MECH-169 staged-decline-under-uniform-damage falsifier buildable across MECH-120 (SWS) -> MECH-121 (NREM, hold RESPECTED) -> MECH-123 (REM) without a fake cross-phase content pipe; V3-EXQ-778 + 778a diagnostic PASS; MECH-121 hold preserved (EXPERIMENT_PURPOSE=diagnostic; NREM leg = plumbing-fidelity only). (b) **REM generative-fidelity readout** replaces the terrain-variance proxy (ree-v3 main `da873a1`; REE_assembly master `c8696cce97`); MECH-168 / INV-047 REM generative-gain non-vacuity banked (`c95b2f4a0f`). (c) **SD-069 phasic_surprise_burst** IMPLEMENTED 2026-07-17 as the LC-NE PHASIC complement to MECH-313 tonic on the SAME E3 softmax temperature channel -- makes MECH-063 sub-claim (ii) (tonic/phasic split) behaviourally testable; 17 SD-069 contracts + full `tests/contracts` 1462 pass; sharp-surprise source `"instantaneous_pe"` added same day so genuine per-tick surprise spikes survive the running-variance EMA smoothing; V3-EXQ-779 validation user-routed `blocked_substrate` (no cloud run). Reuses MECH-104's lit basis but routes to selection softmax, NOT MECH-104's commit gate -- does NOT implement MECH-104. (d) **Governance cycle 2026-07-17 evening**: **ARC-057 candidate -> provisional** via V3-EXQ-768a verified interaction PASS (ARC-057 SD-024 x SD-025 super-additive approach-emergence; verified/non-vacuous; supersedes 768 vacuous cluster; approach-side scoped); V3-EXQ-767a SD-025 margin PASS adjudicated REAL/non-vacuous. **MECH-458 REGISTERED** (curiosity = reward-conditional exploitation amplifier, not a diversity generator; SD-025 is exploitation-tighten under a benefit prior; conversion_ceiling has only selection faces; adds a GENERATION face routing ARC-065 / MECH-314 as a rarity-seeking Bellemare-polarity drive, ordering-gated on INV-088). **V3-EXQ-769** MECH-457 capacity-amend regression FAILed non_contributory (missing-dependency reframe; brake fired 6th time -- GOV-FANOUT-1 GENERATION portfolio spawned as V3-EXQ-770 / 771 / 772). **INV-088 hold applied + INV-090 traceability applied**. (e) **New confirmer-lane traffic (IGW workset)**: V3-EXQ-775 MECH-086 dopamine selection-gain dose-response, V3-EXQ-776 MECH-279 PAG freeze-gate functional-signature, V3-EXQ-773 MECH-076 residue basin-geometry dose-response, V3-EXQ-774 MECH-173 REM-precision, V3-EXQ-777 MECH-063 orthogonal control-axes rank-2 falsifier all queued from IGW slots 205-208; new `/workset` `generation` (V3/V4/V5/V6) filter added. (f) **Scientific Progress Dashboard prototype** (Build/Prove/Narrow/Decide + momentum feed) landed under `evidence/planning/`; CURRENT_FRONT.md now emits its hero-question line from `hypothesis_space.v1.json` (SHP-6 + progress-dashboard §6).
  **Bottleneck (unchanged shape, dashboard-lensed):** the MECH-457 conversion-ceiling attack now sits on a GOV-FANOUT-1 GENERATION portfolio (V3-EXQ-770 / 771 / 772; V3-EXQ-771 live on `ree-cloud-3`) after V3-EXQ-769's missing-dependency reframe brake fired 6th. ARC-057 promotion candidate -> provisional preserves the two-face conversion_ceiling frame: SD-025 = exploitation-amplifier arm; the missing rarity-seeking generator (MECH-458) is ordering-gated on INV-088 substrate reconfirmation. V3-EXQ-774 REM-precision FAIL routes to a MECH-204 Phase-7 amend not another single-axis retest (autopsy applied, awaiting governance Step 5). Green-board target 2026-07-19 is now one day out; the GOV-FANOUT-1 GENERATION portfolio's H2 (metabolic forage-to-survive coupling) is the live discrimination the target date turns on. ETHICS-PERIMETER Phase 0 datum stays on the record (Phases 1-3 deferred; NON-BLOCKING).
- **2026-07-17T01:10Z nightly read (scheduled `/update-docs`, bot identity).**
  Substrate day. Manifest count on disk: **593 `v3_exq_*` + 3 `v4_exq_*` flat** (+6 v3 vs the 2026-07-16 nightly's 587) + **1243 per-run pack manifests** under `runs/**/manifest.json`. **Currently queued (`experiment_queue.json` items[]): 2 items** -- **V3-EXQ-768a** (priority 7; `claim_id=ARC-057`; ARC-057 SD-024 x SD-025 interaction spike RE-OPERATIONALIZED with the continuous CEM score-margin gate, supersedes V3-EXQ-768; `machine_affinity ree-cloud-4`; 4-arm 2x2 interaction over 8 seeds; PENDING) + **V3-EXQ-769** (priority 6; `claim_id=MECH-457`; CAPACITY-AMEND retest of the mech457 bootstrap-explorer, supersedes V3-EXQ-765; three JOINT knobs on ONE build -- capacity `actor_critic_hidden 128->256` + budget 3x->5x, reliability `warm_start_fraction 0.2` + credit-replay passes/topk raised, integration z_world DETACHED; `machine_affinity ree-cloud-2`; CLAIMED by ree-cloud-2 2026-07-16T17:15Z, ~720min ETA). **Pending review (`pending_review.md`, regenerated 2026-07-16T18:09Z): 0 items -- clean board** (last review 2026-07-16T18:08Z; the 2026-07-16d governance cycle folded the ARM_INTACT V3-EXQ-680e PASS/supports MECH-423 promotion + the V3-EXQ-767 / 768 vacuous-pass adjudications into the record). **Substrate + governance landings in the window:** (a) **SD-024 (DA-modulated RBF density)** IMPLEMENTED as the diagnostic instrument that RESOLVES MECH-232 (ree-v3 main `402285a`; 13 SD-024 contracts + bit-identical OFF; V3-EXQ-766 diagnostic); the **cloud PASS V3-EXQ-766a** on `ree-cloud-2` (linux-x86_64-py3.10) PROMOTED **MECH-232 candidate->provisional** in the 2026-07-16 cloud re-gate + closed the SD-024 cloud-authoritative gate (REE_assembly master `46db726a00`, autopsy `a5dd155c53`). (b) **SD-025 (hippocampal curiosity drive)** IMPLEMENTED as the 2nd component of ARC-057 on the SD-024 substrate (ree-v3 main `c886d7c`; 7 SD-025 contracts). (c) **MECH-457 mech457_competence_bootstrap_explorer** SUBSTRATE IMPLEMENTED via /implement-substrate closing the GOV-FANOUT-1 four-axis walk-out (RND success-independent drive + first-class actor-critic + prioritized credit-replay converter + developmental intrinsic-coef/entropy schedule-anneal; composes only landed pieces, honours SYNTHESIS.md duplication objection); the post-build retest **V3-EXQ-765 FAILed** (`bootstrap_explorer_plateaus_capacity_gap_remains`: DRIVE works on raw ON 6.48 vs OFF 0.62 +5.87, actor-critic plateaus at ~13% of the 48.05 ceiling, z_world cotrain destructive ON 0.35 < OFF 5.22); the autopsy routed ONE capacity-side amend (three joint knobs on the SAME build, NOT a discrimination fanout; re-derive brake HELD-BUT-SANCTIONED per the autopsy's explicit exception) -> V3-EXQ-769 queued 2026-07-16 (ree-v3 main `61c0fd9`; REE_assembly master `3c340499b3` claims.yaml MECH-457 evidence_quality_note + granularity_debt_recurrence_note UPDATE + substrate_queue amend + `validation_exq V3-EXQ-769`). (d) **MECH-423 cross-model super-additivity RE-CALIBRATED gate** (V3-EXQ-680e supersedes 680d): 680d's co-training stabiliser WORKED (0/5 PAIR seeds diverged) but its readiness gate `min(cos over 5 usable seeds)>=0` fired `r1_grad_cosine_net_negative` on -0.0371 gradient-ORTHOGONALITY noise and self-excluded a run whose suppressed FORK-2 verdict would PASS; replaced with a magnitude-floored / tolerance-band gate `|cos|<=BAND (0.15)` = near-orthogonal = readiness-MET; the resulting cloud (`ree-cloud-2`, linux-x86_64-py3.10) super_additive PASS/supports (all 7 integration-readiness preconditions met, load-bearing `superadditivity_margin_pair` cleared) PROMOTED **MECH-423 candidate->provisional** in the 2026-07-16d governance cycle (REE_assembly master `e40362ebb7`); 680d flat manifest re-classified `non_contributory->superseded` + `superseded_by 680e`. (e) **V3-EXQ-767 (SD-025) + V3-EXQ-768 (ARC-057) VACUOUS-PASS CLUSTER**: both PASS/supports but flagged `vacuous_pass` -- load-bearing binary argmin `pref_dense` pinned at exactly 1.0 across all 8 seeds with zero cross-seed variance (structural saturation of the shared `_pref_dense` argmin selector under the dominant SD-025 curiosity term; density ~84 vs ~9 in 767, ~28 vs ~10 in 768, vs near-zero empty-harm terrain). Adjudicated `non_contributory / measurement_degeneracy` (NOT a substrate ceiling; NOT a degenerate mechanism -- clean OFF/both_off = 0.5 controls confirm the null selection rule gives chance; 768's C1 super-additive interaction contrast is variance-bearing and passes; single-drive arms at chance). PROMOTES/DEMOTES NOTHING. Re-operationalized as V3-EXQ-767a + V3-EXQ-768a (supersede 767/768; continuous CEM score-margin `sparse_score - dense_score`, already computed inside `_pref_dense` and discarded; every load-bearing criterion now rides the margin; `criteria_non_degenerate` guard on margin-variance-across-seeds so neither can re-flag vacuous), cloud-pinned `ree-cloud-3`/`ree-cloud-4` (ree-v3 main `521ad16`). SD-025 + ARC-057 stay candidate. (f) **GAP-A resolution route recorded**: the ARC-057 ecological env-enablement node (Test C) classified `complex(probe-gated)/mystery(known data)` -- frame-blocked (5x5x7 local_view invariant) AND co-blocked on the F-dominance conversion ceiling -> DEFERRED-V4; the buildable Test B env-FREE SD-024xSD-025 interaction spike is now the live substrate-abstract falsifier (768/768a). REE_assembly `2b376e5344` (arc_057_ecological_env_decision doc). (g) **Governance auxiliaries**: GOV-GRAN-1 granularity-debt recurrence detector gained a co-tagged-bystander exclusion (REE_assembly `c6782c3e74`) + coherent-campaign adjudication-marker (`50452771aa`); phase3 thin-pack recording-provenance fix (`a09600807c` + `59e9f69f1f`; producer carries machine/machine_class/substrate_hash + metrics.values, indexer unconditional flat-fallback, thin-pack regression check; 2519 packs healed at index time, no rewrite); explorer queue-count "nothing vs one item" contradiction fixed (`1016d919b5`; EXPLORER_VERSION 2026-07-16.1). **Bottleneck (unchanged shape, sharpened lever):** the MECH-457 conversion-ceiling attack now rests on a single-run capacity-amend retest (V3-EXQ-769, 3 seeds x 7 conditions, ~720min on `ree-cloud-2`); a contributory PASS moves MECH-457 candidate->provisional and clears INV-088 `pending_substrate_reconfirmation`. A preconditions-met FAIL routes to the synthesis-doc V4 directions per the ARC-107 selector-constitution design note, NOT another single-axis probe (the re-derive brake refuses that). ARC-057 Test-B interaction (768a) is the live cloud diagnostic for SD-024 x SD-025 super-additivity. ETHICS-PERIMETER Phase 0 datum stays on the record (Phases 1-3 deferred; NON-BLOCKING for the V3 green-board target 2026-07-19, two days out).
- **2026-07-16T01:10Z nightly read (scheduled `/update-docs`, bot identity).**
  Quiet nightly. Manifest count on disk: **587 `v3_exq_*` + 3 `v4_exq_*`** (+8 v3 vs the 2026-07-15 nightly). **Currently queued (`experiment_queue.json` items[]): 0 items** — the MECH-457 GOV-FANOUT-1 portfolio drained overnight. All five queue items from the 2026-07-15 nightly ran to completion: **V3-EXQ-742-m** PASS 20260715T013328Z (matched-noise baseline mint, ancillary); **V3-EXQ-753** FAIL 20260715T020417Z (H-return Go-Explore archive + return-to-frontier); **V3-EXQ-754** FAIL 20260715T060558Z (H-curriculum goal-frontier IMGEP/AMIGo-lite); **V3-EXQ-755** FAIL 20260715T081923Z (H-mode critic-utility explore/exploit gate); **V3-EXQ-756** FAIL 20260715T170212Z (H-credit × H-return additivity PAIR cell mandated by user directive 2026-07-14). Also completed in the window: **V3-EXQ-746b/746c** FAIL 20260715T134512Z/151705Z (INV-089 harm_eval z_harm bound retries); **V3-EXQ-763** PASS 20260715T081944Z (MECH-304 conditioned-inhibition behavioural falsifier). **Pending review (`pending_review.md`, regenerated 2026-07-15T13:50Z): 0 items — clean board** (last review 2026-07-15T13:44Z; all four provisional promotions and the four MECH-457 FAILs folded into the 2026-07-15 governance cycle). **Bottleneck (unchanged shape, sharpened verdict):** the MECH-457 GOV-FANOUT-1 four-axis fanout (memory/frontier vs teaching-signal vs policy-control vs credit-propagation) closed uniformly FAIL on the three non-additivity legs (H-return / H-curriculum / H-mode) and the PAIR cell (H-credit × H-return). Combined with the 2026-07-14 finding that H-optim (V3-EXQ-751) RND alone clears the floor unsupervised, the fanout's converging read is that the wall is NOT a novelty/exploration class — memory-organisation, teaching-signal-density, gate-control, and credit-propagation have all been refuted as the ceiling-lifting mechanism on the current substrate. Next-step brake to fire per the four-axis-brake protocol (memory entry `project_mech457_competence_floor_coldstart`). ETHICS-PERIMETER Phase 0 datum stays on the record (Phases 1-3 deferred; NON-BLOCKING for the V3 green-board target 2026-07-19, three days out).
- **2026-07-15T01:11Z nightly read (scheduled `/update-docs`, bot identity).**
  Live day. Manifest count on disk: **579 `v3_exq_*` + 3 `v4_exq_*`** (roughly flat vs the 2026-07-14 nightly's 580 — same shape; single-file drift). **Currently queued (`experiment_queue.json` items[]): 5 items** — the MECH-457 GOV-FANOUT-1 portfolio EXPANDED after the 2026-07-14 evening `/failure-autopsy` reroute (751 H-optim RND clears floor unsupervised while ICM fails + 750 non_contributory + lit-pull refuted the novelty class → four fresh legs H-credit/H-return/H-curriculum/H-mode + one mandated additivity PAIR cell, all diagnostic / excluded from governance scoring): **V3-EXQ-742-m** (priority 5; ancillary matched-noise mint arm pinned `ree-cloud-4`), **V3-EXQ-753** (priority 4; MECH-457 H-return Go-Explore archive + return-to-frontier vs sparse, both z_world + raw 5x5 reps; claimed `ree-cloud-3`), **V3-EXQ-754** (priority 4; MECH-457 / INV-088 H-curriculum goal-frontier IMGEP/AMIGo-lite vs sparse; pending `ree-cloud-4`), **V3-EXQ-755** (priority 4; MECH-457 H-mode critic-utility explore/exploit gate vs fixed-RND; claimed `ree-cloud-2`), **V3-EXQ-756** (priority 3; MECH-457 H-credit x H-return additivity PAIR cell mandated by user directive 2026-07-14; pending `ree-cloud-3`). **Pending review (`pending_review.md`, last regenerated 2026-07-14T07:15Z, PRE-GOVERNANCE-CYCLE): 2 items** — both MECH-457 lineage `unclaimed manifest`s (V3-EXQ-751 PASS + V3-EXQ-750 FAIL); the same day's evening `/governance` cycle folded them into the failure-autopsy that spawned the 752..756 portfolio, so this pending count is dated relative to that reroute (a `generate_pending_review.py` refresh is owed in the next cycle to clear). **Substrate + governance landings in the window:** (a) **SD-065 conditioned-safety cue channel** landed as the MECH-304 behavioural falsifier substrate (ree-v3 main `5e03aa6`; REE_assembly master `ccc1d976e8`); (b) THREE promotions candidate→provisional in one evening — **MECH-288 + MECH-287** (V3-EXQ-757 event-boundary trigger wall-independent functional-signature test, REE_assembly master `8918ee9e99`), **MECH-284** (V3-EXQ-758 V_s staleness accumulator monotonicity/locality/leak test, `547e477784`), **MECH-304** (V3-EXQ-759 cue-specific conditioned safety prediction representation-level confirming probe, `422b632a3a`); all four cleared `v3_pending` — the wall-independent functional-signature confirming-test pattern (precedent V3-EXQ-455/447/448) is working as the confirmer lane the new GOV-CONFIRM-1 rule was built for; (c) **GOV-CONFIRM-1** generative evidence-confirmer detector plan-of-record REGISTERED + P0 (`aa4b2c5a3a`) + P1 output-only shadow (`6ba6e80157`) + P2 low-priority autospawn cap 3 (`fcc08cb03e`) LANDED — the missing IGW-workset-consume-only surface for wall-independent confirmers is now automated; (d) MECH-046 CeA mode_prior confirming probe queued (V3-EXQ-762) + MECH-092 quiescent-E3 SWR-equivalent replay (V3-EXQ-761) + MECH-303 contextual passive safety terrain discrimination (V3-EXQ-760) + MECH-304 behavioural falsifier (V3-EXQ-763) all queued and running through the new confirmer lane. **Bottleneck (shape SHIFTED):** the MECH-457 conversion-ceiling attack fans out to 4 substrate axes (memory/frontier vs teaching-signal vs policy-control vs credit-propagation) — the "which class of intrinsic-motivation mechanism is the wall" question is now under decisive combination-aware fanout instead of a single trainable-actor bet, per the 2026-07-14 failure-autopsy reroute. The MECH-288/287/284/304 provisional wave demonstrates the confirmer lane can promote long-parked candidate/v3_pending claims cheaply when a wall-independent functional-signature test is available — a promotion-throughput lever the green-board target 2026-07-19 (four days out) can lean on. ETHICS-PERIMETER Phase 0 datum stays on the record (Phases 1-3 deferred; NON-BLOCKING).
- **2026-07-14T01:11Z nightly read (scheduled `/update-docs`, bot identity).**
  MECH-457 GOV-FANOUT-1 leg cluster expanded during the day. Manifest count on disk: **580 `v3_exq_*` + 3 `v4_exq_*`** (+14 v3 net vs the 2026-07-13 nightly). **Currently queued (`experiment_queue.json` items[]): 3 items** — **V3-EXQ-751** (priority 10; `claim_ids=[MECH-457]`; H-optim leg D of the MECH-457 GOV-FANOUT-1 portfolio — stronger unsupervised explorer, PPO+entropy PLUS RND novelty + ICM curiosity arms, NO expert/BC; z_world cotrain only; SAME-run reuse-eligible sparse-cotrain baseline; diagnostic; excluded from governance scoring; self-routes exploration-was-the-wall vs expert-teaching-signal-necessary), **V3-EXQ-750** (priority 2; `claim_ids=[MECH-457, INV-088]`; single-step strategy-diversity readout on the 742/747/748/749 2x2 factorial — reruns all 4 cells via `mech457_fanout` and co-emits H_greedy + effective_actions + mean-hedging-entropy alongside competence; user-chosen full 2x2; diagnostic; excluded from governance scoring), and **V3-EXQ-742-m** (priority 5; ancillary matched-noise/methodology arm; `claim_ids=[]`; pinned `machine_affinity="ree-cloud-4"`). **Pending review (`pending_review.md`, regenerated 2026-07-13T22:37:56Z): 3 items** — 2 PASS + 1 FAIL, all MECH-457 lineage: **V3-EXQ-747** FAIL (`v3_exq_747_mech457_hrep_rawview_actor_critic`; 2026-07-13T06:22), **V3-EXQ-748** PASS (`v3_exq_748_mech457_hexplore_reward_density_actor_critic`; 2026-07-13T11:19), **V3-EXQ-749** PASS (`v3_exq_749_mech457_hrep_x_hexplore_rawview_dense_actor_critic`; 2026-07-13T06:30). All three are the GOV-FANOUT-1 747/748/749 H-rep x H-explore portfolio the 2026-07-13 lit-pull + failure-autopsy adjudicated as "H-rep + reward-density REFUTED; RL-bootstrap/teaching-signal is the wall" — governance handoff for the trio is in flight (active claim `nifty-cerf-c06cab` was running `/governance` when this snapshot was taken; not applied at snapshot time). Substrate landings in the window: none (2026-07-13 was a scaffolding + fanout leg design + queue-experiment day: MECH-457 leg-C build/land activity concentrated on the substrate_queue plus fanout adjudication + a pack_writer §7.3 hardening of the manifest-writer lint to a commit-time gate; 683/686 non-canonical-dir HYBRID tail cleared; no new no-op-default ree_core substrate). **Bottleneck (unchanged shape):** V3-EXQ-751 is the live 4th leg of the MECH-457 GOV-FANOUT-1 portfolio and — with 747 refuted-on-substrate + 748/749 PASSing but pending governance — the emerging live test of the "exploration was the wall vs expert teaching-signal necessary" fork the 2026-07-13 failure-autopsy pre-registered. MECH-457 substrate landed 2026-07-12 stays candidate/v3_pending pending the fanout adjudication + 751 verdict. ETHICS-PERIMETER Phase 0 datum stays on the record (Phases 1-3 deferred; NON-BLOCKING for the V3 green-board target 2026-07-19, five days out).
- **2026-07-13T01:11Z nightly read (scheduled `/update-docs`, bot identity).**
  Idle nightly (no new substrate landings or governance verdicts since the 2026-07-12 nightly). Manifest count: **566 `v3_exq_*` + 3 `v4_exq_*`** on disk (+7 v3 net vs the 2026-07-12 snapshot). **Currently queued (`experiment_queue.json` items[]): 2 items** -- **V3-EXQ-742** (priority 49; `claim_ids=[MECH-457]`; MECH-457 first-class actor-critic action-learning ON/OFF validation; 4-arm cotrain x SF matrix + bias_head baseline + local-view/oracle/random anchors; foraging teacher; denominator = V3-EXQ-738 local-view ceiling 48.05@D3; claimed `ree-cloud-2` 2026-07-12T00:05Z per the 2026-07-12 snapshot) + **V3-EXQ-742-m** (priority 5; ancillary matched-noise/methodology arm associated with the 742 lineage). Coordinator queue also holds queued-but-not-in-this-file items from the 2026-07-11 autopsy cohort (see the 2026-07-12 snapshot below): V3-EXQ-047m / V3-EXQ-517d / V3-EXQ-669b / V3-EXQ-733b / V3-EXQ-740a / V3-EXQ-741. **Pending review (`pending_review.md`, regenerated 2026-07-12T13:27Z):** 0 items (clean board -- last review 2026-07-12T13:26Z). Bottleneck unchanged: V3-EXQ-742 is the SINGLE live falsifier of the entire conversion-ceiling attack route; MECH-457 substrate landed 2026-07-12 stays candidate/v3_pending pending its verdict. ETHICS-PERIMETER Phase 0 datum stays on the record (Phases 1-3 deferred; NON-BLOCKING for the V3 green-board target 2026-07-19, six days out).
- **2026-07-12T01:12Z nightly read (scheduled `/update-docs`, bot identity).**
  **Post-724 competence-recovery campaign collapsed to a single live H1 substrate build: the FIVE 24-hour verdicts from the 5-track portfolio ran to terminal FAIL and were adjudicated -- V3-EXQ-737 FAILed twice on the "trainable REE-latent policy head" H1 discriminator, V3-EXQ-734/735/736 competence-recovery arms all FAILed, V3-EXQ-740 INV-064 FAILed on measurement_degeneracy (IV never moved); MECH-456 rebinding-under-perturbation GOV-FANOUT-1 2-leg portfolio (733a-a + 733a-b) walked FAIL non_contributory with a decisive cluster autopsy (P-B directed-traversal PASSED every mechanism criterion -- DV1 6/6, DV2 6/6, binder 6/6, overtakes 62-808 -- but was blocked by a granularity-blind survival-coupled P0-coverage floor + P-A survival-onboarded fixed survival but starved overtakes via competent-forager coverage concentration; re-derive brake FIRED 2nd MECH-456 non_contributory; test-bed operationalisation ceiling CONFIRMED, mechanism ceiling REFUTED; route implement-substrate on a bounded rebinding_functional_harness P0-coverage decoupling); V3-EXQ-047l/m MECH-095 agency-routing retests on SD-047 FAILed non_contributory on TWO different measurement degeneracies (047l probe-partition label saturation + 047m training-label env_events-fold saturation); re-derive brake FIRED 2nd MECH-095 non_contributory -> routed /implement-substrate agency_comparator_testbed_sd047 (BUILT 2026-07-11: SD-047 amended with tag_env_caused_multisource_ttype non-saturating self/world label + V3-EXQ-741 test-bed queued with BOTH ROUTED_A gradient-head + ROUTED_B efference-copy read-out operationalisations + dual probe-partition + self-world-balance guards). MECH-457 (actor-critic action-learning) SUBSTRATE IMPLEMENTED 2026-07-12 via /implement-substrate: ree_core/action_learning/actor_critic.py landed (dorsal-striatal ActorCriticPolicy + value critic + optional SF critic V_SF=psi.w per MECH-229 cand-B; no-op-default cotrain_encoder co-shape lever; agent hooks actor_critic_step / actor_critic_reward / actor_critic_parameters / actor_critic_encoder_parameters; smoke PASS; ree-v3 main a8379a8 + 1eb1610). V3-EXQ-742 (MECH-457 ON/OFF validation) queued and claimed by ree-cloud-2 2026-07-12T00:05Z. REE_convergence intake landed action-learning/representation-for-action co-shaping synthesis (5 sources: CURL/UNREAL/successor-features/MuZero-boundary/Dreamer-boundary/Decision-Transformer-boundary; directly informs the MECH-457 cotrain arm as the SD-056 encoder co-shaping amendment; REE_convergence master 42646d6). SHP program CLOSED (all SHP-0..7 done); SHP retrospective + 3 hardening fixes LANDED (session-land skill git-commit hygiene).**
  Manifest count: **559 `v3_exq_*` + 3 `v4_exq_*`** on disk (+14 v3 net vs 2026-07-11). **Currently queued (`experiment_queue.json` items[]): 1 item** -- **V3-EXQ-742** (priority 49; `claim_ids=[MECH-457]`; MECH-457 first-class actor-critic action-learning ON/OFF validation; 4-arm matrix A0-A3 (cotrain_encoder{F,T} x use_sf_critic{F,T}) + bias_head_baseline (724 all-ON incompetence control) + local_view_greedy (738 fair denominator, 48.05@D3) + greedy_oracle + random_walk anchors; D0+D3 rungs; seeds [42,43,44]; phased P0 warmup -> P1 A2C actor-critic (GAE; cotrain adds latent_stack to optimizer + live z_world, frozen detaches) -> P2 unshaped foraging eval; DV = capability_eval foraging_competence vs 1.0 floor, strict majority @D3; experiment_purpose=evidence; PROMOTES/DEMOTES NOTHING pending /failure-autopsy adjudication after run; RPE teacher = FORAGING reward per user 2026-07-12 -- substrate benefit-harm teacher NOT used, no ARC-030 grounding phase; claimed `ree-cloud-2` 2026-07-12T00:05:44Z, ETA ~10 h). Coordinator queue also holds queued-but-not-in-this-file items: **V3-EXQ-047m** (MECH-095 corrected retest w/ probe-partition + PROBE_NEG_FLOOR=5 + PROBE_NEG_BALANCED_TARGET=20 advisory; supersedes 047l), **V3-EXQ-517d** (MECH-302 emit_outcome instrumentation fix; supersedes 517c), **V3-EXQ-669b** (nursery re-issue), **V3-EXQ-733b** (MECH-456 P-B brake-permitted retest after rebinding_functional_harness substrate), **V3-EXQ-740a** (INV-064 corrected retest with task-relevant harm-decodability observable + immature anchor arm + mature-anchor positive control; supersedes 740), **V3-EXQ-741** (MECH-095 agency-comparator test-bed on the new SD-047 non-saturating label; both ROUTED_A gradient BCE head + ROUTED_B efference-copy read-out + dual guards; supersedes 047l/m).
  **Governance verdicts (all applied 2026-07-11T21:07Z; PROMOTES NOTHING; 6 chips spawned to /failure-autopsy):** V3-EXQ-538a reviewed (autopsy applied 2026-07-10; 5 claims non_contributory -- was never marked reviewed); V3-EXQ-517d PASS/**supports MECH-302** (clean discriminative pair, ON 686.7 vs OFF 0.0 relief-completion events, 3/3 seeds; MECH-302 stays candidate below promotion threshold); 8 left pending as chipped for autopsy: 047m (MECH-095 ceiling_confirmed_route_autopsy; task_bb6d8496), 733a-a + 733a-b (MECH-456 GOV-FANOUT-1 both legs FAIL, cluster autopsy landed 22:04Z; task_b889fa0b), 734/735/736/737 (conversion-ceiling GOV-FANOUT-1 synthesis autopsy landed 22:26Z with H1 refuted-on-current-substrate => deficit is DEEPER = REE-latent action-adequacy, MOVES MECH-457 actor-critic substrate; task_2c15deab), 740 (INV-064 degenerate FAIL adjudicated 22:26Z as measurement_degeneracy = eff_rank/participation-ratio of alpha_world=0.9 EMA z_world collapses to rank-1 at every arm + onset window 2..22 entirely post-saturation -> NEVER a valid maturational contrast; task_caa2ef77). Governance agenda (1 pending_user -> 0): **MECH-456** hold_pending_v3_substrate APPLIED. EXP-0336 marked executed. Audits: substrate-ceiling MECH-095 ceiling-may-have-lifted -> 047m autopsy chip; **GOV-DIAG-1** 724->732->732a recurrence -> synthesis chip (fanout ran); GOV-FANOUT-1 clean; closure-drift 0/0/0; status-plane 0/98; demand-queue no new rows; substrate_queue freshness bumped. verify_governance_cycle.py exit 0.
  **Substrate builds LANDED 2026-07-11 / 2026-07-12 (all PROMOTES NOTHING; MECH-457 substrate landing GOVERNANCE UPDATE PENDING):** (a) **MECH-457 first-class actor-critic action-learning substrate** (`ree_core/action_learning/actor_critic.py`; ree-v3 main a8379a8 + 1eb1610; dorsal-striatal ActorCriticPolicy + value critic + optional SF critic V_SF=psi.w per MECH-229 cand-B; config no-op knobs use_actor_critic / actor_critic_cotrain_encoder [co-shape lever] / actor_critic_use_sf_critic / actor_critic_hidden / actor_critic_sf_feature_dim; agent hooks actor_critic_step / actor_critic_reward / actor_critic_parameters / actor_critic_encoder_parameters [latent_stack co-shape seam]; SD doc sd_actor_critic_action_learning.md flipped PROPOSED->IMPLEMENTED + section 7 impl record; MECH-457 stays candidate/v3_pending pending V3-EXQ-742 verdict + /failure-autopsy; smoke PASS; KEY FINDING documented in SD doc section 7: benefit_eval / harm_eval are random-init in the all-ON recipe [ARC-030 warmup gate never lifted] -> the substrate-signal teacher needs ARC-030 grounding + readiness gate; user chose foraging reward as RPE teacher). (b) **SD-047 amendment: non-saturating self/world transition_type tag** (`ree_core/environment/causal_grid_world.py` env-only flat kwarg `tag_env_caused_multisource_ttype`, default False bit-identical OFF; adds env_caused_multisource label filling residual "none" steps under multi_source_dynamics; V3-EXQ-741 test-bed queued using both ROUTED_A gradient BCE head + ROUTED_B query-time efference-copy read-out; re-derive brake CLEARED for MECH-095 [new EXQ number + different operationalisation + pre-registered SD-047 intensity sweep + enabling substrate now BUILT]; NOT a 3rd 047 letter). (c) **REE_convergence action-representation co-shaping synthesis** (REE_convergence master 42646d6; 5 canonical entries covering CURL / UNREAL / successor-features Dayan-1993 / Barreto-2017 + MuZero / Dreamer / Decision-Transformer boundary markers; directly informs the MECH-457 cotrain arm as the SD-056 encoder co-shaping amendment; PROMOTES NOTHING; registers nothing into claims.yaml). (d) **SHP program CLOSED** (all SHP-0..7 done); retrospective + 3 hardening fixes LANDED (evidence/planning/shp_process_retrospective.md; scripts/shp2_collapse_and_verify.py wrapper; session-land skill git-commit-hygiene block -- `-m` before `--` + stale-index.lock clear ONLY when >5min + no live git + post-commit dropped-file pointer; REE_assembly master 62c5c27e1d + REE_Working umbrella master 9b8c4eb).
  **Pending review (`pending_review.md`, regenerated 2026-07-11T22:37Z after the /governance cycle + 4 confirmed autopsies + 2 requeues): 8 items pending** (ALL FAIL, all chipped for /failure-autopsy per the 2026-07-11T21:07Z /governance cycle; V3-EXQ-538a reviewed as non_contributory; V3-EXQ-517d reviewed as supports MECH-302).
  **Bottleneck (shape SHIFTED again):** With the H1 discriminator (V3-EXQ-737) FAILing twice on the current substrate + the 734/735/736 competence-recovery portfolio also failing + the 738 H2 already refuted, the 724 conversion-ceiling cluster autopsy adjudicated the deficit as DEEPER than the initial H1/H2 framing = REE-latent action-adequacy: the substrate produces a competent world-model latent but the bias-head REINFORCE that rides it is not a first-class action-learning system. **MECH-457 actor-critic substrate BUILT 2026-07-12** as the substrate-side answer + the /lit-pull actor-critic dossier grounds it (Schultz 1997 + O'Doherty 2004 + Collins & Frank 2014 + Sutton 2000 + Stooke 2021, lit_conf 0.89). V3-EXQ-742 is now the SINGLE live falsifier of the entire conversion-ceiling attack route -- H1 provisionally REFUTED-on-bias-head-substrate but MECH-457 is the redesigned H1 discriminator with a first-class RPE-driven actor-critic instead of a bias-head over a prediction-trained representation; the co-shape ablation arm (cotrain_encoder=T vs F) directly tests the REE_convergence synthesis reading that SD-056 encoder co-shaping is the missing lever. MECH-456 rebinding-under-perturbation ROUTED to /implement-substrate rebinding_functional_harness (bounded P0-coverage decoupling + starvation-proof P0 + coverage-floor scaled to K -- substrate_queue action=create, priority 1); ONE brake-permitted P-B retest V3-EXQ-733b sits queued after the harness build. MECH-095 ROUTED to V3-EXQ-741 on the newly-built SD-047 non-saturating self/world label. INV-064 corrected V3-EXQ-740a redesign incorporates task-relevant harm-decodability + immature anchor + mature-anchor positive control. MECH-441 stays HELD on a competent substrate. INV-050 stays `ceiling_decision: deferred`. SD-063 (E2 conditional predictive-uncertainty head) remains the first V3-tractable confidence-channel promotion (candidate -> provisional 2026-07-09). **ETHICS-PERIMETER Phase 0 datum** stays on the record (Phases 1-3 deferred; NON-BLOCKING for V3 green-board 2026-07-19; the green-board target date is now ONE WEEK away, gated on the 742 verdict + subsequent MECH-457 governance decision).
- **2026-07-11T01:10Z nightly read (scheduled `/update-docs`, bot identity).**
  **Post-724 competence-recovery campaign expanded to a 5-track diagnostic portfolio + first WS-1/H2 leg CLEARED; MECH-456 walked FAIL non_contributory + re-queued as GOV-FANOUT-1 2-leg portfolio; MECH-457 (actor-critic action-learning) registered candidate/v3_pending from the /lit-pull dossier; GOV-FANOUT-1 governance_rule registered; work-graph debt vocabulary (Cynefin/Treverton) wired through the whole substrate (CLAUDE.md + /governance + /failure-autopsy + /queue-experiment + substrate_queue.node_class schema); SHP-2 collapse of 2 more plan docs + SHP-7 status-doc stub retirement LANDED.**
  Manifest count: **545 `v3_exq_*` + 3 `v4_exq_*`** on disk (+8 v3 net vs 2026-07-10). **Currently queued (`experiment_queue.json` items[]): 6 items, ALL are the post-V3-EXQ-724 competence-recovery campaign** (all `experiment_purpose=diagnostic`, `claim_ids=[]` on the four tracks; MECH-456 legs claim-tagged; brake-EXEMPT competence localization + methodology) --
  (a) **V3-EXQ-737** (priority 155; `claim_ids=[]`; Track-1d = trainable policy/action head bolted onto the REE world-model latent -- the direct H1 discriminator of the "prediction-rich, action-poor translation gap" the 724 competence-localization diagnostic localized; feeds `/implement-substrate f_dominance_conversion_ceiling` competence build; supersedes V3-EXQ-724 as the live H1 test);
  (b) **V3-EXQ-734** (priority 150; `claim_ids=[]`; Track-1a = env-difficulty competence-recovery sweep; holds the 724 A0 all-ON recipe fixed and sweeps env difficulty D0->D3 x 4 arms via `capability_eval` yardstick + greedy-oracle readiness anchor + matched vanilla-PPO learner-adequacy control folding in 732a; forks env-difficulty-recoverable | REE-substrate-ceiling | learner/observability-ceiling);
  (c) **V3-EXQ-733a-a** (priority 48; `claim_ids=[MECH-456]`; Leg P-A of the GOV-FANOUT-1 rebinding-functional 2-leg portfolio; survival-onboarded agent via `scaffolded_sd054_onboarding` 603m/603n recipe; supersedes V3-EXQ-733; MECH-456 stays candidate/v3_pending);
  (d) **V3-EXQ-733a-b** (priority 48; `claim_ids=[MECH-456]`; Leg P-B; cold 733 agent + softened survival + finer lattice G4 + scripted directed-traversal teleport tour decoupling overtakes from foraging survival; supersedes V3-EXQ-733);
  (e) **V3-EXQ-735** (priority 42; `claim_ids=[]`; Track-1b = drive/reward-balance approach-weighting sweep on the 724/728 all-ON stack; A0 balance vs A1/A2/A3 progressively approach-weighted on `cfg.e3` benefit_weight/goal_weight UP + lambda_ethical/rho_residue DOWN; forks reward_balance_is_the_lever | reward_balance_not_the_lever; brake-EXEMPT);
  (f) **V3-EXQ-736** (priority 40; `claim_ids=[]`; Track-1c = 4-arm two-stage CURRICULUM competence-recovery -- Stage-A easy env forage training then Stage-B fine-tune into the hard 724 env; A0 flat-hard control / C1 curriculum e2-frozen / C2 curriculum e2-unfrozen / C3 3-rung easy->med->hard; DV hard-env P2 `mean_resources_per_episode` vs the 1.0 floor).
  **V3-EXQ-738** (Track-1c local-view-achievable ceiling anchor; the H2 falsifier) **PASSed** 2026-07-10 (5x5 local-view forager clears 1.0 floor -- 48/ep D3, 6/ep D0, 3/3 seeds -> **H2 REFUTED**), removing the "observation-observability ceiling" branch and MAKING V3-EXQ-737 THE LIVE H1 DISCRIMINATOR: does a REE-latent-policy-head recover competence (H1 confirmed; MECH-457 actor-critic supported) or not (H1 refuted; substrate-ceiling to fresh routing).
  **Governance verdicts (all applied 2026-07-10T20:22Z; PROMOTES NOTHING; all candidate/v3_pending):** V3-EXQ-738 (H2 local-view ceiling: PASS reviewed; `claim_ids=[]`, promotes nothing); V3-EXQ-733 (MECH-456 rebinding-under-perturbation; entities.rebinding_under_perturbation): FAIL/non_contributory on the seed-quorum READINESS gate ONLY (ready seed 42 passed BOTH DVs strongly -- align margin 0.57, latency gap 13.1); MECH-456 stays candidate/v3_pending; re-queued as V3-EXQ-733a 2-leg GOV-FANOUT-1 portfolio (chip task_965ea19a spawned). Governance agenda (2 pending_user -> 0): **MECH-457** hold_pending_v3_substrate APPLIED (candidate/v3_pending; sibling to MECH-455; gated on the V3-EXQ-737 H1 discriminator; action learning as first-class RPE-driven actor-critic substrate distinct from bias-head-over-prediction; grounded in the /lit-pull `targeted_review_actor_critic_action_learning` dossier -- Schultz/Dayan/Montague 1997 + O'Doherty 2004 + Collins & Frank 2014 + Sutton 2000 + Stooke 2021, lit_conf 0.89); **Q-080** narrow_open_question KEPT BROAD (730/731 ablations queued; status_note stamped). decision_log + decision_state reconciled; claims.json 878; `validate_claims --strict` OK. Audits: substrate-ceiling 0 actionable (7 mapped / 19 parked); **GOV-DIAG-1** 724->732->732a recurrence ACKNOWLEDGED as already-routed to the 737/738/739 fanout portfolio (user chose acknowledge, no new re-pose); closure-drift 0/0/0; demand-queue no new rows; substrate_queue freshness bumped.
  **Substrate + methodology LANDED 2026-07-10 (all PROMOTES NOTHING; all no-op-default; all bit-identical OFF where a code substrate is involved):** (a) **GOV-FANOUT-1** `governance_rule` registered (REE_assembly master c3e392dd10; twin of GOV-CEIL-1; bottleneck -> diverse-portfolio escalation on a brake-fired discrimination; reuses GOV-CEIL-1/GOV-DIAG-1/re-derive-brake detection + adds the fan-out RESPONSE) + graveyard antibody #4; wired `/failure-autopsy` (`fanout_recommendation` producer on brake-fired discrimination + Step 9 schema), `/queue-experiment` Step 2.5b (portfolio-vs-sequential + discriminator design-audit), `/governance` Step 6a-v-quater (surfacing fanout-candidate lineages). (b) **Work-graph debt vocabulary** (Cynefin `complicated (buildable)` / `complex (probe-gated)` [`puzzle (known rules)` | `mystery (known data)`] / `aleatoric (irreducible)`; spike = REE diagnostic = converts complex->complicated backlog; prefer discovery-debt = EFE epistemic-value on the work graph; avoids the taken "epistemic debt") canonicalized in `docs/architecture/work_graph_debt_vocabulary.md` + applied to MECH-457 readiness note + wired into: (i) umbrella `CLAUDE.md` General Rules startup pointer + razor + prefer-discovery-debt stance; (ii) `/failure-autopsy` routing (classify node before routing: buildable->implement-substrate; probe-gated puzzle->spike, portfolio if >=2 hyps via GOV-FANOUT-1; probe-gated mystery->reframe don't gather; irreducible->refuse re-queue); (iii) `/queue-experiment` Step 2.5 (blocked_substrate == complicated(buildable) -> build not experiment; queue only probe-gated spikes; "gated on X" must name a token; diagnostic converts complex->complicated backlog); (iv) `/governance` Step 3 recommendation/next-work sweep (each open node classified by its always-glossed token; prefer complex(probe-gated) discovery-debt AHEAD of complicated(buildable) execution backlog) + Step 6a autopsy-create field wiring + Step 6a-vi report split; (v) `substrate_queue.json` per-entry `node_class` field + `verify_governance_cycle.py` check I (warn/info only; NEVER blocks); 6 previously-flagged `GATED_ON_NO_TOKEN` entries classified by actual residual (SD-026 complicated / SD-031 puzzle / MECH-269b complicated / MECH-295 puzzle / crf-availability-maintenance complex(probe-gated) / f_dominance_conversion_ceiling complex(probe-gated) campaign node). (c) **SHP-2 two-plane collapse of `goal_pipeline_plan.md` + `sd033_governance_plan.md`** (7 GAP-N + 8 CHK/CLAIMS/EXP/SD/MECH nodes backfill-lifted verbatim to `status_snapshot.v1.jsonl` then blob fields collapsed to derived `live:` + `join:`; body byte-identical below frontmatter; status-plane drift 0; at-risk refs 19->7; REE_assembly master 467b9e7791 + 3c2d0f1604). (d) **SHP-7** retired 3 abandoned manual status docs (`evidence/GOVERNANCE_STATE.md`, `docs/REE_overview.md`, `docs/index.md`) to generated stubs owned by new `scripts/generate_status_stubs.py` wired into `governance.sh` Step 3c-bis-3 (before the nav stamper) -- 0 broken links; idempotent; nav-stamper co-operates in-memory (owns frontmatter, preserves generated body); REE_assembly master c36a5c3e2e. (e) **/lit-pull actor-critic action-learning dossier** landed the missing action-learning grounding for the 724 "prediction-rich, action-poor" translation gap: 5 canonical entries in `targeted_review_actor_critic_action_learning/` supporting the reading that REE lacks a first-class RPE-driven actor-critic action-learning system (bias-head REINFORCE over prediction-trained representations is not the same thing); registered MECH-457 (candidate/v3_pending). Also 2026-07-10 EOD: /governance Step-3 recommendation sweep now surfaces discovery-debt AHEAD of execution backlog + `substrate_queue.node_class` values feed the ordering; 6 unmarked entries backfilled non-destructively (info-only in the validator).
  **Pending review (`pending_review.md`, regenerated 2026-07-10T23:22Z after the 733 governance-apply + 733a re-queue): 0 items** (clean board; 733/733a lineage marked reviewed as non_contributory; 738 marked reviewed as PASS).
  **Bottleneck (unchanged shape, sharpened again):** With the H2 refutation (V3-EXQ-738 PASS), the V3 selection-face + valuation-face attack on MECH-439 F-dominance conversion ceiling now consolidates entirely into the H1 discriminator = **V3-EXQ-737 trainable policy/action head on the REE world-model latent** (the direct positive test of the "prediction-rich, action-poor" gap the 724 competence-localization diagnostic localized; the 5-track post-724 portfolio narrows the failure surface -- learner-adequacy 734, drive/reward-balance 735, curriculum 736 -- while 737 alone tests H1). MECH-457 (actor-critic action-learning) is the substrate-side claim gated on the same 737 verdict + carries the /lit-pull dossier as its literature backing. MECH-441 stays HELD on a competent substrate. INV-050 stays `ceiling_decision: deferred`. SD-063 (E2 conditional predictive-uncertainty head) remains the first V3-tractable confidence-channel promotion (candidate -> provisional 2026-07-09). MECH-456 rebinding-under-perturbation re-tested via the GOV-FANOUT-1 733a portfolio (two orthogonal fixes: training-regime survival onboarding vs test-bed decoupling of overtakes from foraging survival). **ETHICS-PERIMETER Phase 0 datum** stays on the record (Phases 1-3 deferred; NON-BLOCKING for V3 green-board 2026-07-19).
- **2026-07-10T01:37Z nightly read (scheduled `/update-docs`, bot identity).**
  **First V3 PROMOTION in a while:** SD-063 (E2 conditional predictive-uncertainty head, MECH-059 confidence-channel instantiation) walked candidate -> provisional via V3-EXQ-716a PASS in the evening `/governance` cycle (first genuine supporting entry; overall 0.824; 0 opposing; SD-031 caveat carried). **New governance rule LANDED:** GOV-CEIL-1 (`governance_rule`) pre-registered ceiling-exhaustion demotion pass wired into `check_substrate_ceiling_audit.py` + `governance.sh` Step 6a-v-bis. First application demoted 5 ceiling-exhausted candidates (`epistemic_category -> standard`, `ceiling_decision: exhausted`, promotion re-gated on a richer-substrate win, co-equal inert-mechanism null carried): MECH-439 (10 hits), MECH-260 (8), MECH-313 (7), MECH-445 (6), MECH-446 (5). Three kept parked user-approved (MECH-314 umbrella core passed on GAP-A; INV-074 universal invariant; Q-045 open_question). ARC-062 / MECH-309 stay parked EXEMPT (still awaiting the competence build). Manifest count: **537 `v3_exq_*` + 3 `v4_exq_*`** on disk (+10 v3 net vs 2026-07-09). **Substrate builds LANDED 2026-07-09 (all PROMOTES NOTHING; all no-op-default; all byte-identical OFF):** (a) **SD-033e frontopolar de-commit lever** (`pfc.frontopolar_analog`; ree-v3 main; V3-narrow slice = MECH-264 counterfactual-value + function (4) disengagement-for-exploration ONLY; entry-relative counterfactual-value release pressure injected into `NaturalCommitUrgencyRelease`; MECH-265 relative-importance + gateway mode stay V4 stubs; isolated GAP-A validation queued); (b) **cross_stream_binder LEARNED (plastic) mode** + **convergence repair** (V4-leaning; ree-v3 main 5140329 + b86fa3e; contrastive co-encoding InfoNCE with COSINE similarity replacing dot-product to escape near-collinear latent regime; new `binder_converged` diagnostic gates the retest; supersedes fixed-field 720 result; V3-EXQ-725a retest queued + ingested to coordinator DB); (c) **Q-080 effort-dissociating env substrate** (ree-v3 `CausalGridWorld` env extension; DISSOCIATES effort from benefit for open_question Q-080; walled two-corridor grid with equal-length low- vs high-effort corridors; env-only kwargs; no mechanism claim minted; smoke-tested + env contracts green; Q-080.a harm-coupling + Q-080.b least-effort-prior ablations queued). **Claims registered/updated 2026-07-09 (candidate; PROMOTES NOTHING):** **MECH-455** competence-based intrinsic motivation / goal-achievement-progress wanting (`ethics_engine_3.competence_based_intrinsic_motivation`; WS-9 headline -- REE's IM stack is entirely knowledge-based, this is the missing competence-based drive; depends_on MECH-307 + SD-012 + MECH-229 VALENCE_WANTING); **EXT-009** refined asymmetric criterion-placement (animism = health signature; WS-12); **Q-080** narrowed into Q-080.a/b/c embedded testable sub-questions (WS-2/effort-coupling); ARC-063 mature-pool amend (CandidateRuleField). **AI-design critique plan-of-record** authored (WS-1..WS-13 comprehensive audit): formal-ancestor mapping, cognitive-architecture graveyard doc (Soar/ACT-R/LeCun 2022 anti-patterns + REE-specific exposure map + governance:cognitive-mass ratio design note wired into closure dashboard), active-inference bridge, corrigibility positioning, Bitter-Lesson rebuttal, axiom-chain adversarial audit (WS-13, external-ethicist handoff), intrinsic-motivation lit synthesis. **Currently queued (`experiment_queue.json` items[]): 4 items, ALL claimed + running** -- (a) **V3-EXQ-729** (priority 120; `claim_ids=[MECH-268]`; MECH-268 dACC conflict-saturation live-loop diagnostic; ecological chronic-hazard analogue of 463b, proves the DACC `_outcome_history` FIFO is populated by the LIVE `agent.py:select_action` tail c7fc045 with a counting spy -- NO manual `record_outcome` injection; brake-EXEMPT new EXQ# + newly-built substrate; claimed `ree-cloud-4`); (b) **V3-EXQ-723a** (priority 90; `claim_ids=[]`; SD-064 Experiment A J-lens redesign superseding V3-EXQ-723; discriminative-compactness via concentration-curve vs matched-null random-subspace + broadcast-alignment + reactive-bypass gates; committed-action-balanced eval; the 723 vacuous-pass off-ramp; claimed `ree-cloud-2`); (c) **V3-EXQ-538a** (priority 50; `claim_ids=[SD-049]`; re-queue of infra-ERROR predecessor V3-EXQ-538 SD-049 Phase 2 with sleep after the 2026-05-08 cloud-1 SIGTERM cluster; byte-identical logic + `supersedes` field; claimed `ree-cloud-3`); (d) **V3-EXQ-495a** (priority 45; `claim_ids=[MECH-163]`; re-queue of infra-ERROR predecessor V3-EXQ-495 MECH-163 planned-system gate after ~4h-in cloud-1 SIGTERM; claimed `ree-cloud-1`). **Autopsies applied 2026-07-09:** V3-EXQ-720 (fixed-field cross-stream binder lifted 1/6 -> 3/6 SPEC-specificity but did not clear the 4/6 gate; substrate_ceiling partial-lift; routed the learned-binder V4 amend); V3-EXQ-723 (J-lens diffuse_branch=False was reused as a vacuous_pass sentinel -- indexer branch-selector `_compute_adjudication` false-flag fixed; 723 re-adjudicates verified; V3-EXQ-723a redesigned); V3-EXQ-725 (untrained-substrate artifact = binder InfoNCE loss stuck at chance; SPEC regressed 0/6; the 641 -> 641a -> 720 -> 725 lineage's 4th non_contributory; routed convergence repair + gated V3-EXQ-725a with a HARD `learned_binder_converged` precondition); V3-EXQ-724 (WS-1 competence-diffuse; H1/H2 policy-learning discriminator routed as V3-EXQ-732). **/diagnose-errors 2026-07-09** re-queued 2 infra-ERROR predecessors (V3-EXQ-495 -> 495a MECH-163 + V3-EXQ-538 -> 538a SD-049); 250a already resolved (250b ran INCONCLUSIVE, all 250/250a/250b discussed); 606b blocked-upstream on V3-EXQ-543k contributory PASS (543b..543k GAP-B falsifier family FAILED). **Pending review (`pending_review.md`, regenerated 2026-07-09T20:24Z): 0 items** (clean board; 720/721/725/727 + 716a all marked reviewed). **Bottleneck (shape SHIFTED):** With the GOV-CEIL-1 demotion pass the MECH-439 F-dominance conversion ceiling is now formally recorded as `ceiling_decision: exhausted` (10 ceiling hits; carries the inert-mechanism null co-equally; promotion re-gated on a richer-substrate win). The V3 selection-face attack no longer sits on an active MECH-439 falsifier -- it moves ENTIRELY onto (i) V3-EXQ-724 competence-localization + the `/implement-substrate f_dominance_conversion_ceiling` COMPETENCE build, (ii) the V3-EXQ-732 H1/H2 policy-learning discriminator routed by the 724 autopsy, and (iii) MECH-455 competence-based IM as the missing wanting-side driver named by WS-9. MECH-441 stays HELD on a competent substrate. INV-050 stays `ceiling_decision: deferred` (consumer banked, ecological producer environment-blocked). SD-063 promotion opens the first V3-tractable confidence-channel path in a while (MECH-059 substrate now provisional). **ETHICS-PERIMETER Phase 0 datum** stays on the record (Phases 1-3 deferred; NON-BLOCKING for V3 green-board 2026-07-19).
- **2026-07-09T01:10Z nightly read (scheduled `/update-docs`, bot identity).**
  **One new no-op-default V3 substrate build landed since the 2026-07-08 nightly (cross_stream_binding_substrate, 2026-07-08 evening), plus SD-064 candidate registered (REE instantiates a global-workspace-like capacity-limited broadcast/access channel, external corroboration Anthropic J-space/J-lens 2026; PROMOTES NOTHING). V3-EXQ-719a walked terminal FAIL + confirmed autopsy + governance-applied (competence-wall reframe of the retest wall; 21st ARC-062 / 20th MECH-309 substrate-ceiling brake FIRED; V3-EXQ-722 re-gated PARKED on competence substrate); V3-EXQ-718a walked terminal FAIL + confirmed autopsy + governance-applied (SD-MEL-CONSUMER status flipped implemented -- consumer VALIDATED via C3 injection PC; ecological demo RE-PARKED off V3 critical path; INV-050 ceiling_decision:deferred; 6th INV-050 / 3rd MECH-180 brake FIRED).** Manifest count: **527 `v3_exq_*` + 3 `v4_exq_*`** on disk (+3 v3 net vs 2026-07-08). **Currently queued (`experiment_queue.json` items[]): 4 items, ALL claimed + running** -- (a) **V3-EXQ-721** (priority 300; `claim_ids=[]`; MECH-446 M3 lag-distribution diagnostic on the F-independent `use_closure_commit_entry` substrate 460o/460p validated -- re-anchors the recurring-VACUOUS MECH-446 de-commit DV from within-arm occupancy-drop to the LAG DISTRIBUTION between SD-034 closure fires and latch-armed occupancy, with `closure_exclusive_decommit_eval=True` attribution guard; brake-EXEMPT new DV measurement; 6 seeds; claimed `ree-cloud-3` 2026-07-08T21:01Z, ETA ~4 h); (b) **V3-EXQ-723** (priority 250; `claim_ids=[]`; SD-064 Experiment A -- REE-native J-lens dispositional-readout diagnostic; post-hoc ridge z_t -> committed_class_{t+H} + SVD J-space on the all-ON 714/719a substrate; NO new mechanism, measurement only; self-routes present/diffuse/substrate_not_ready; claimed `DLAPTOP-4.local` 2026-07-08T21:41Z); (c) **V3-EXQ-724** (priority 200; `claim_ids=[]`; competence-localization diagnostic named as the brake-EXEMPT next step by the 719a autopsy -- 5-arm OFAT over P1-budget / frozen-encoder / all-ON-mechanism-interference + greedy-oracle positive control, isolates why the integrated all-ON REE-v3 agent forages 0.065/0.0/0.455 resources/ep in P2 below the 1.0 floor; feeds `/implement-substrate f_dominance_conversion_ceiling` competence build; claimed `ree-cloud-1` 2026-07-08T23:43Z, ETA ~7.5 h); (d) **V3-EXQ-720** (priority 100; `claim_ids=[]`; V4-leaning coherence non-reducibility retest on the newly-built `cross_stream_binding_substrate` -- 641a harness with `E2Config.cross_stream_binding_enabled=True`, `strength=0.5` pre-registered; does NOT supersede 641a substrate-ceiling; 6 seeds; claimed `ree-cloud-2` 2026-07-08T20:45Z). Fleet drained overnight -> fleet now fully engaged. **V3-EXQ-722** (cross-bank conversion-ceiling SELECTION-face falsifier) stays PARKED per the 719a governance-apply re-gate: KEEP PARKED, re-gate on a COMPETENCE substrate (do NOT queue; brake fired; SELECTION-face falsifier is wrong probe for a competence gap). **Pending review (`pending_review.md`, regenerated 2026-07-08T23:26Z after the 23:20Z governance-apply): 0 items** (clean board; 719 + 719a marked reviewed).
  **Substrate landed 2026-07-08 (PROMOTES NOTHING; no-op-default; byte-identical OFF):** **cross_stream_binding_substrate** (`latent.cross_stream_binding`; ree-v3 main ad8a3a3 + REE_assembly master b8f29a5; V4-leaning shared-latent-factor cross-stream binding in `E2FastPredictor.rollout_with_world` so `z_world <-> z_self` rollout streams share a common cause -- theta-gated ephaptic-field-analog additive factor from joint `(z_self_t, z_world_t)`; MECH-089 theta-gamma nesting + MECH-270 ephaptic coupling; fixed untrained projections by design (641a retest runs eval() with no P0 curriculum); `E2Config.cross_stream_binding_enabled` default False + strength/theta-period sub-knobs; V4-leaning off V3 critical path; validated by V3-EXQ-720 retest queued same day).
  **Claim registered 2026-07-08 (candidate; PROMOTES NOTHING):** **SD-064 global_workspace_access_channel** (`selection.global_workspace_access_channel`; REE_assembly master a71c0554c9; REE instantiates a global-workspace-like capacity-limited broadcast/access channel per Baars 1988 / Dehaene 1998 / Mashour 2020; the SD-027 "not a single global workspace" hedge conflated GWT with unitary intra-E3 selection -- MECH-007 forbids the latter, GWT requires neither; access-functional NOT phenomenal; external corroboration Anthropic J-space/J-lens 2026; non-destructive SD-027 reframe cross-ref; design note `evidence/planning/global_workspace_jlens_plan.md` covers Experiment A J-lens dispositional readout (runnable now, V3-EXQ-723) + Experiment B workspace-ablation cliff (SUBSTRATE-BLOCKED on the SD-027 boundary gate not built in V3)).
  **Governance verdicts (all applied 2026-07-08; all non_contributory; PROMOTES/DEMOTES NOTHING; both claim clusters stay candidate + pending_retest_after_substrate):** **V3-EXQ-719 -> V3-EXQ-719a lineage** (conversion-ceiling dissociation diagnostic; MECH-309/ARC-062): 719 superseded by 719a via /governance (self-aborted on 3000-tick MI floor with only 1/2 seeds cleared; honest readiness abort, not the 642 trap); 719a walked terminal FAIL with self-route `substrate_not_ready_requeue` on all four readiness gates MET (6005 ticks/seed, 6-10 bins, 3/3 seeds); confirmed `failure_autopsy_V3-EXQ-719a_2026-07-08` (REE_assembly master 05a7679e86) adjudicated the self-route as MISLABEL -- readiness met, code else-branch reused the under-sampled string for the pre-registered "evidence against either conclusion" case; Branch B (decisive_state_appropriate_commitment / metric-artifact reframe) REJECTED; Branch A (genuine_monomodal_collapse) NOT clean; FIRST direct competence measurement 0.065/0.0/0.455 resources/ep (0/3 seeds clear the 1.0 floor); dominant diagnosis = substrate_ceiling / behavioural-competence + training-regime gap (thin P1 90-ep bias-head-only REINFORCE, frozen encoder; all-ON interference a live alt), NOT a further selection lever (MECH-448/449 built + selection-face-lifted on GAP-A); reframes the 654h/485i/625e/460h/460i retest wall as ONE competence root. **/governance apply 2026-07-08T23:20Z** (nifty-booth-b574cf; REE_assembly master 07acd6ad29): PROMOTES/DEMOTES NOTHING; verbatim 719a `evidence_quality_note` prepended to ARC-062 + MECH-309 (44 pure additions, 0 status/ceiling/v3_pending line changes); substrate_queue `f_dominance_conversion_ceiling` amended (+719a failure record + competence-wall reconcile note); review_tracker 719a marked reviewed (0 pending); re-derive brake FIRED (21st ARC-062 / 20th MECH-309); did NOT queue 719b; PARKED V3-EXQ-722 (session trusting-elgamal-f8c2e8, case (c) inconclusive-but-branch-B-rejected) RE-GATED: KEEP PARKED, re-gate on a COMPETENCE substrate. NEXT STEP = competence-localization diagnostic (brake-EXEMPT); queued as V3-EXQ-724 same session. **V3-EXQ-718a** (SD-MEL-CONSUMER measured-MEL cadence re-validation; INV-050/MECH-180): confirmed `failure_autopsy_V3-EXQ-718a_2026-07-08` (REE_assembly master 9506cd5646) adjudicated the self-route `mel_control_degenerate` as REFUTED (OFF control IS pinned all 3 seeds; C2 failed only on `on_gt_off`); corrected reading = mel_consumer_capable_ecological_novelty_not_graded; CONSUMER CAPABILITY PROVEN via C3 injection PC (DV [9,13,18,24,30,38] exact-monotone tracks injected [0.6..2.5]); C1 DV-monotone-in-MEASURED-MEL 3/3 but near-degenerate; ecological producer (link i novelty -> graded-above-reference-MEL) STILL broken, same root as 718: CausalGridWorldV2 converges too completely (conv_rel_drop ~0.98) so residual ecological MEL ~1e-5 noise-level, scrambled vs novelty level = environment/test-bed gap NOT substrate_ceiling NOT falsification. **/governance apply 2026-07-08T06:30Z** (bold-payne-7587d5; REE_assembly master b0e98ba7cd): PROMOTES NOTHING; both claims stay candidate (MECH-180 v3_pending; both pending_retest_after_substrate); shared 718a `evidence_quality_note` appended VERBATIM to INV-050 + MECH-180 (status UNCHANGED); INV-050 RE-PARKED with `ceiling_decision: deferred` + ceiling_routing_note (consumer banked; ecological demo off V3 critical path; brake refuses 718b); substrate_queue SD-MEL-CONSUMER status flipped `implemented_validation_owed -> implemented` (+718a failure record; consumer validated; ecological demo re-parked; no build owed); title/impl-note updated; 718a marked reviewed (0 pending); sleep_substrate:GAP-5b reconciled -- owner_exq chain 718 -> 718a TERMINATES (no 718b), re-parked; re-derive brake FIRED (6th non_contributory INV-050 / 3rd MECH-180) -> did NOT queue 718b.
  **Bottleneck (unchanged shape, sharpened):** **MECH-439 F-dominance conversion ceiling remains the live root choke.** The V3-EXQ-719a competence-wall reframe SHIFTS the V3 selection-face attack focus off the ARC-062/MECH-448-449 arbitration axis (already selection-face-lifted on GAP-A) and off the SD-034/MECH-445-446 de-commit-release axis (retest wall now reframed as one competence root) onto V3-EXQ-724 competence-localization (the brake-EXEMPT next step named by the 719a autopsy; PRE-BUILD diagnostic for the `/implement-substrate f_dominance_conversion_ceiling` competence build). MECH-441 + the curiosity-channel falsifier chain stay HELD on a competent substrate. **INV-050 stays candidate/UNWEAKENED** with ecological demo now RE-PARKED off V3 critical path per the 718a governance-apply: consumer BANKED (C3 injection PC), ecological producer environment-blocked (not substrate_ceiling); MECH-180 stays v3_pending; brake refuses 718b same-environment re-grade. **SD-064 candidate lands the global-workspace claim + external corroboration (Anthropic J-space/J-lens 2026); V3-EXQ-723 J-lens diagnostic in flight as the substrate-existence probe (runnable now) while Experiment B workspace-ablation cliff stays SUBSTRATE-BLOCKED behind the SD-027 V3 boundary-gate retrofit. **ETHICS-PERIMETER Phase 0 datum** stays on the record (Phases 1-3 deferred; NON-BLOCKING for V3 green-board 2026-07-19).
- **2026-07-08T01:10Z nightly read (scheduled `/update-docs`, bot identity).**
  **Big four-day window since the 2026-07-04 nightly: (a) V3-EXQ-711 bounded-parity-controller ran to FAIL + confirmed autopsy landed + amend routed and IMPLEMENTED (bounded parity controller); (b) V3-EXQ-713 bounded-parity validation ran; (c) V3-EXQ-714 (ARC-062/MECH-309 FULLSTACK selection+valuation) ran to FAIL, confirmed non_contributory/substrate_ceiling + governance-applied; (d) MECH-445 cluster V3-EXQ-715/715a/717 walked through non_contributory (M1 selection-face lift) + narrow-weakens (M2 commit-intent regime-scoped) + governance-applied; (e) V3-EXQ-716 clean diagnostic PASS; (f) V3-EXQ-718 SD-MEL-CONSUMER cadence validation FAIL non_contributory (measurement_gap, NOT ceiling) + governance-applied + V3-EXQ-718a queued (measured-MEL test-bed redesign); (g) two new no-op-default substrate LANDINGS: SD-063 (E2 conditional predictive-uncertainty head, ree-v3 main, 2026-07-05) + SD-MEL-CONSUMER (adaptive sleep-cadence MEL consumer, ree-v3 main 909292c, 2026-07-07); (h) INV-050 un-parked (substrate now built, retest owed); (i) /claim-synthesis on MECH-445/446 (STOP verdict + M1-M6 forward-moves menu, 2026-07-06); (j) /governance apply of the 4 confirmed autopsies (REE_assembly master c0e8b1f1de, 2026-07-07); (k) plan-doc reconciliations (BG-4 drive-grounding L2->L3 done, BG-5 goal/wanting L1->L2 done, sleep_substrate:GAP-5b 718->718a owner reconcile, behavioral_diversity_isolation:GAP-I 715a+717 reconcile).**
  Manifest count: **524 `v3_exq_*` + 3 `v4_exq_*`** on disk (+9 v3 net vs 2026-07-04; V3-EXQ-711/713/714/715/715a/716/717/718 all landed as terminal FAILs or PASS in the window; +1 for V3-EXQ-712 SD-063 diagnostic PASS 2026-07-05 that appeared in the count between windows). **Currently queued (`experiment_queue.json` items[]): 0 items** at file-tick (queue file drained; coordinator active queue holds V3-EXQ-718a claimed on ree-cloud-1 -- measured-MEL cadence test-bed redesign of 718, supersedes 718 per the confirmed `failure_autopsy_V3-EXQ-718_2026-07-07`; regresses cadence DV on MEASURED per-arm mean MEL rather than the LABEL 718 used, adds a MEL-injection positive-control arm as the same-statistic readiness gate + capability check, and self-routes `substrate_not_ready_requeue` if measured-MEL gradient degenerate -- NEVER a false weakens). **Pending review (`pending_review.md`, regenerated 2026-07-07T17:21Z after the 2026-07-07T16:53Z governance-apply cycle): 0 items** (clean board; all four autopsies applied + 4 runs marked reviewed).
  **Substrate landed 2026-07-05 / 2026-07-07 (all PROMOTES NOTHING; all no-op-default; all byte-identical OFF):** (a) **SD-063 E2 conditional predictive-uncertainty head** (ree-v3 main, 2026-07-05; MECH-059 confidence-channel instantiation; distribution-free quantile/pinball head over (z_world, action) providing per-input predictive spread for E3's commit gate; V3-EXQ-712 diagnostic PASS; V3-EXQ-716 clean diagnostic PASS 2026-07-06); (b) **SD-MEL-CONSUMER adaptive sleep-cadence MEL consumer** (ree-v3 main 909292c + REE_assembly master 845b89991d, 2026-07-07; sleep_substrate:GAP-5b; the INV-050 THIRD/learning-demand sleep drive; reads accumulated waking Model Error Load and modulates offline-phase duration/entry, replacing the K-episode-deterministic scheduler; ABS_MEL_FLOOR recalibrated 1e-4 -> 1e-6 relative; MECH-094 waking-read-only; unblocks INV-050 retest + MECH-180 v3_pending on a PASS retest).
  **Governance verdicts (all applied 2026-07-07T16:53Z-17:28Z, REE_assembly master c0e8b1f1de + 20885bc6ec + 2008303d0f; ALL non_contributory or narrow_weakens; PROMOTES NOTHING; 6 claims stay candidate/v3_pending):** V3-EXQ-714 (ARC-062/MECH-309 FULLSTACK selection+valuation conversion falsifier; confirmed `failure_autopsy_V3-EXQ-714_2026-07-07`; substrate_ceiling/non_contributory -- aborts at C1 readiness with GAP-A divergence 0.004 + OFC range 0.0007 starved at FULL P0=200, falsifies the P0=200 prediction; brake FIRED, fullstack re-queue REFUSED); V3-EXQ-715a + V3-EXQ-717 (MECH-445 cluster; confirmed `failure_autopsy_MECH-445-cluster-715a-717_2026-07-07`; 715a M1 selection-face lift non_contributory both -- ruled OUT as de-commit-release lever, moderate_f_delta -63; 717 M2 commit-intent regime-scoped NARROW weakens MECH-445 reliability only -- existence PROVEN 2/9 in-regime + prior 398, sub-2/3-reliable, status unchanged); V3-EXQ-718 (SD-MEL-CONSUMER validation; confirmed `failure_autopsy_V3-EXQ-718_2026-07-07`; non_contributory/measurement_gap NOT substrate_ceiling -- SD-MEL-CONSUMER LIVE+functional C2 PASS 3/3, C1 0/3 on non-monotone novelty->MEL test-bed; NOT weakened/cleared; routed /queue-experiment 718a). All 6 drafted evidence_quality_notes applied verbatim; substrate_queue f_dominance_conversion_ceiling amended (+714,+715a,+717) + SD-MEL-CONSUMER amended (+718). claims.json rebuilt (870). Closure drift 0; stale-since-review 0 after last_updated bumps on arc_062:GAP-B + behavioral_diversity_isolation:GAP-B/GAP-I. Ceiling audit 0 orphans / 0 may-have-lifted.
  **Bottleneck (unchanged shape, continuation):** **MECH-439 F-dominance conversion ceiling remains the live root choke.** The V3-EXQ-714 FULLSTACK verdict is decisive on the ARC-062 selection-face route -- the arbitration axis is EXHAUSTED at FULL P0=200 (the pre-registered ceiling-vs-lift falsifier); the 20th ARC-062 / 19th MECH-309 substrate-ceiling brake FIRED and no fullstack same-substrate re-queue is permitted. The V3 selection-face attack now stays on the DA-gated ARC-108/ARC-110 loop-effective-weight repair route (V3-EXQ-711 amend + successor bounded-parity-controller retest is the live open falsifier). MECH-441 + the curiosity-channel falsifier chain stay HELD on the coupling returning contributory. **INV-050 stays candidate/UNWEAKENED** with the forward gate now BUILT (SD-MEL-CONSUMER, 2026-07-07) but still owing a valid measurement (718 failed on the LABEL test-bed; 718a redesigns onto MEASURED MEL + a positive-control same-statistic gate). On a 718a PASS retest + /governance, INV-050 clears retest and MECH-180 clears v3_pending. **ETHICS-PERIMETER Phase 0 datum** stays on the record (Phases 1-3 deferred; NON-BLOCKING for V3 green-board 2026-07-19).
- **2026-07-04T01:17Z nightly read (scheduled `/update-docs`, bot identity).**
  **Two new no-op-default substrate builds landed since the 2026-07-02 nightly + BOTH V3-EXQ-709 AND V3-EXQ-710 ran to terminal FAIL and were confirmed non_contributory by their autopsies + governance-applied + the loop-segregation cluster was reclassified V4->V3.**
  Manifest count: **515 `v3_exq_*` + 3 `v4_exq_*`** on disk (+1 net vs 2026-07-02; V3-EXQ-709 landed as terminal FAIL 2026-07-02T05:25Z; V3-EXQ-710 landed as terminal FAIL 2026-07-03 after the 2026-07-02 evening substrate build). **Currently queued (`experiment_queue.json` items[]): 1 item** -- **V3-EXQ-711** (priority 1; `claim_ids=[MECH-439]`; ARC-110 x ARC-108 ascending-spiral gain validation the 709 + 710 autopsies both routed; 2 arms x 6 seeds `A_ASCENDING_OFF` vs `A_ASCENDING_ON` on the GAP-A reef-bipartite substrate; swept factor `use_ascending_spiral_gain` ONLY -- learned cross-loop arbitration ON on both arms; claimed `ree-cloud-1` 2026-07-03T15:54:06Z, ETA ~4 h). Load-bearing precondition: on `A_ASCENDING_ON` the limbic loop reaches >= motor effective weight on >=3/4 divergent seeds; unmet -> `substrate_not_ready_requeue` (non_contributory, NEVER a false weakens; same non-vacuity discipline as 709 + 710). Then C1: committed-class entropy strict-above OFF. PASS -> MECH-439 weakens / ARC-108 + ARC-110 supports; decisive FAIL (limbic wins but no conversion) -> MECH-439 supports / ARC-108 + ARC-110 weakens. Smoke --dry-run PASS: ascending gain live (ON-arm `strict_exceed_ticks=36`, `m_range=8.38` vs 709 `~0.05`). **Pending review (`pending_review.md`, regenerated 2026-07-03T14:23:04Z after the 2026-07-03T14:13Z governance pass-2): 0 items** (clean board; last review 2026-07-03T14:08:31Z).
  **Substrate landed 2026-07-02 / 2026-07-03 (all PROMOTES NOTHING; all no-op-default; all byte-identical OFF):** (a) **MECH-140 x MECH-450 disinhibitory soft-competitive settling** (ree-v3 main 8cc42bc, 2026-07-02; parameter-free graded lateral-inhibition kernel over the F + MECH-448/449 within-eligible field; both single-arena and segregated-loop paths); (b) **ARC-108 x ARC-110 ascending-spiral gain** (ree-v3 main 6b660a6, 2026-07-03; Haber asymmetric striato-nigro-striatal ascending spiral; scales ONLY the ascending strict-upper-triangle of `M_cross` so `w_eff[limbic]` rises without touching `w_eff[motor]` -- implicit motor de-pin; the loop-effective-weight repair BOTH V3-EXQ-709 AND V3-EXQ-710 autopsies routed to).
  **Governance verdicts (all applied 2026-07-03; all non_contributory + pending_retest_after_substrate; PROMOTES NOTHING):** V3-EXQ-709 (learned cross-loop arbitration validation; confirmed `failure_autopsy_V3-EXQ-709_2026-07-03`; mechanism ENGAGED but the deeper `limbic_loop_can_win` precondition UNMET on 3/4 divergent seeds; REE_assembly master f26e707926 + d496f03fe9 apply) + V3-EXQ-710 (disinhibitory settling validation; confirmed `failure_autopsy_V3-EXQ-710_2026-07-03`; substrate BUILT + LIVE but SHARPENS F over the F-collapsed single-arena field on 2/3 divergent seeds -- single-arena is the WRONG LOCUS, VIP->SOM disinhibition reconfigures competition BETWEEN differentiated loops; REE_assembly master 9e0dfde7e7 + 12d32a7337 apply). Both autopsies AMEND `v4_loop_segregation` `failure_record` (loop-effective-weight ceiling BEYOND the 707b static-arithmetic diagnosis); MECH-140 flipped `epistemic_category` -> `substrate_ceiling` + `pending_retest_after_substrate` (strong first-lit / substrate-blocked, NOT under-supported); MECH-439 narrow corroboration only from 710 (single-arena cannot decide intrinsic-vs-liftable). Governance pass-2 (REE_assembly master 4818354be5, 2026-07-03T14:13Z) overrode the flat V3-EXQ-710 self-route to `non_contributory` per the confirmed autopsy (illusory MECH-140 + MECH-450 conflict hold resolved without weakens spuriously scored). Board clean (0 pending).
  **Loop-segregation cluster reclassified V4 -> V3** (REE_assembly master befa74bd8c, 2026-07-03T14:38Z; phase-follows-dependency invariant): MECH-140 `implementation_phase: v3` in claims.yaml; substrate_queue `v4_loop_segregation` reframed **V3-closure-required**, ARC-109 + MECH-452 as V3 built co-requisites (built no-op-default 2026-06-27); MECH-451 stays as the V3 cheap pre-emption rung. Motivation: lifting the F-dominance single-arena conversion ceiling is a V3-closure requirement -- a substrate_ceiling is NEVER grounds to defer to V4. Ceiling audit unchanged: 31 = 16 mapped / 15 parked / 0 orphaned / 0 may-have-lifted (MECH-140 stays MAPPED via v4_loop_segregation).
  **Bottleneck (unchanged shape, continuation):** **MECH-439 F-dominance conversion ceiling remains the live root choke.** The 709 + 710 verdicts refine (not overturn) the 707b intrinsic-vs-liftable read: the ceiling is INTRINSIC to the current single-arena F-collapsed within-eligible field, but the newly-built LEARNED cross-loop arbitration + ascending-spiral loop-effective-weight repair route through the segregated-loop substrate (V3, not V4). V3-EXQ-711 is the primary open falsifier of that route. `substrate_dependencies.landing` on the ascending-spiral substrate: mechanism_changing=false no-op KEEP. MECH-441 + the curiosity-channel falsifier chain stay HELD on the coupling returning contributory. INV-050 stays candidate / UNWEAKENED with the brake reframed FORWARD onto the absent MECH-180 adaptive sleep-cadence substrate (unchanged since the 701c autopsy). **ETHICS-PERIMETER Phase 0 datum** stays on the record (Phases 1-3 deferred; NON-BLOCKING for V3 green-board 2026-07-19).
...
- **2026-06-29T01:12Z nightly read (scheduled `/update-docs`).** **ARC-110 C2 RELEASE
  ...
  V3-EXQ-707 lineage progressed through three sessions: V3-EXQ-707 (the original 4-arm
  loop-segregation readiness) FAILed self-routing `substrate_not_ready_requeue` with
  the buggy null-LIFT precondition (drove the 707a driver-only gate fix);
  V3-EXQ-707a (driver-only null-LIVENESS gate fix) was queued on cloud-1 14:10Z; then
  the **ARC-110 C2 RELEASE BUILT** (per-named-channel range-preserving routing into
  the segregated loops -- limbic loop now carries per-candidate range so
  ARM_DROP_LIMBIC can differ from A1_LOOPS -> C2 limbic load-bearing FINALLY
  testable) and **V3-EXQ-707b** (supersedes 707a + 707; priority 420; in flight on
  cloud-2 18:15Z) queued on top of the build, plus a MECH-451 finer-gating substrate
  DEFECT FIX (the `agent.py:5238` builder-gate flag mismatch that left the named
  channels DEAD substrate-wide on every prior 704/706/707/708 run). V3-EXQ-708
  (MECH-440 NoisyNet propagation falsifier) FAILed self-routing
  `substrate_not_ready_requeue` on cloud-3. V3-EXQ-700d (ARC-108 same-layer null
  retune) FAILed self-routing `substrate_not_ready_requeue` on cloud-1. All three
  FAILs are clean readiness-gate non-vacuity self-routes; no false weakens; the
  `pending_review.md` file (last regenerated 2026-06-27T10:14Z) is STALE relative to
  live state. `evidence/experiments/` flat top-level now holds **510 `v3_exq_*.json`
  manifests + 1 `v4_exq_*.json`** on disk (+3 since the 2026-06-28T01:10Z read --
  V3-EXQ-707 / 708 / 700d all landed as terminal FAIL). Per-machine Phase-3 cards
  continue to lead the legacy tally (single-file `runner_status.json` retired).
  **Currently queued (`experiment_queue.json` items[]): 2 items, BOTH claimed +
  running** -- **(a) V3-EXQ-707b** (priority 420; `claim_ids=[ARC-110]`; ARC-110 C2
  RELEASE validation enabling `use_named_channel_routing` + the limbic input modules
  + a per-NAMED-channel `named_channel_routing_live` non-degeneracy precondition
  evaluated BEFORE scoring C2 so vacuous DROP==A1 self-routes
  substrate_not_ready_requeue; C1 + 707a in-layer-null LIVENESS gate UNCHANGED;
  claimed ree-cloud-2 18:15Z); **(b) V3-EXQ-707a** (priority 400;
  `claim_ids=[ARC-110]`; the 707-superseding driver-only null-liveness gate fix;
  claimed ree-cloud-1 14:10Z -- still running at write-time, will be superseded
  cleanly by 707b once 707b lands first or 707a finishes first). **Substrate /
  governance landings since the 2026-06-28T01:10Z spec sync:** (1) **`/diagnose-errors`
  V3-EXQ-707 driver-only fix + requeue** -- the 707 `in_layer_null_verified_lifting`
  precondition was MIS-SPECIFIED (imported the 700-lineage TEMPERATURE-null
  liveness proxy "a valid null must RAISE entropy above A0" onto a STRUCTURED-
  ACCUMULATOR null where random magnitude-matched content through the loop path
  lands committed-class entropy near BASELINE, NOT above it; in 707 the null was
  demonstrably LIVE -- disagree 0.37-0.97, flip 0.37-0.66 on every divergent seed
  -- yet sat at/below A0, while A1_LOOPS lifted strict-above A0 on 4/4 divergent
  seeds; the buggy gate spuriously requeued a valid conversion result AND made the
  pre-registered weakens branch unreachable). V3-EXQ-707a replaces null-LIFT with
  null-LIVENESS (the null must actively perturb committed selection: at least one
  of flip OR disagree must exceed the cross-loop-variance floor on a strict-majority
  of divergent seeds). C1 (A1 strict-above max(A0, the live null)+margin) UNCHANGED
  -- it already rules out the zscore-path-artifact hypothesis. Substrate UNCHANGED
  (the `_loop_inlayer_null` mechanism is correct); driver-only gate fix; additive
  C2 non-degeneracy diagnostic `c2_drop_differs_from_a1` records whether
  ARM_DROP_LIMBIC differs from A1 (707 showed them byte-identical -> C2
  limb-load-bearing was untestable; flagged the MECH-191 phasic-externalisation gap
  the same day). (2) **MECH-451 finer-channel-gating substrate DEFECT FIXED**
  (ree-v3 main 81f2898; REE_assembly master cfcb81b5c7) -- the `agent.py:5238`
  builder gate read the TOP-LEVEL `self.config.use_finer_channel_gating` (NEVER
  set anywhere in ree_core; always False) while the consumer gate at the e3.select()
  call site reads `config.e3.use_finer_channel_gating` (correct). Net:
  `_fcg_channels` was ALWAYS None -> the named channels (ofc/dacc/lpfc/vigour/
  liking/gated_policy) NEVER reached the selector; only the lumped residual/mech341/
  route did. EVERY finer-channel experiment (704/704b/706*/707/708) ran with
  MECH-451's NAMED decomposition DEAD. MECH-451 effectively UNTESTED
  (pending_retest_after_substrate). Builder gate fixed to read
  `config.e3.use_finer_channel_gating` matching the consumer + 4 regression tests
  added. (3) **ARC-110 C2 RELEASE BUILT via `/implement-substrate`** (ree-v3 main
  64f20b6; REE_assembly master 728061a5e5; design doc
  `docs/architecture/sd_v4_loop_segregation.md` C2-RELEASE section) -- each named
  channel's loop-arbitration term is sourced from its per-candidate REPRESENTATION
  routed through `project_channel_range` (range-preserving, the SAME GAP-A path
  that keeps `route` phasic) and substituted for the flat bias-head scalar in the
  segregated-loop accumulation ONLY (the `_lcg_terms` eligibility traces / authority
  recompose / F-score commit path are UNCHANGED). So the limbic loop now carries
  REAL per-candidate range and ARM_DROP_LIMBIC can differ from A1. New
  no-op-default flag `use_named_channel_routing`; verified end-to-end (flat-terms
  reproduce 707 byte-identity; routed terms give limbic range 2.08 + DROP != A1).
  V3-EXQ-707b queued on top of the build via `/queue-experiment` (priority 420;
  enables routing + the limbic input modules + the
  per-NAMED-channel `named_channel_routing_live` non-degeneracy gate evaluated
  BEFORE scoring C2 so vacuous DROP==A1 self-routes substrate_not_ready_requeue;
  smoke 4/4 arms exit 0; coordinator `POST /queue/add` applied:true; claimed
  ree-cloud-2 18:15Z). (4) **`/lit-pull` Q-067 + Q-068 morning round landed** (06:01-
  06:12Z; REE_assembly origin/master dd22e2ca28; 6 literature entries -- Q-067
  relief/safety escape decomposition `lit_confidence 0.797` Mohammadi 2014 NAC
  relief/safety dissociation supports + Zhang 2018 eLife active-relief RL=MECH-375
  supports + LeDoux 2016 decompose-fear-reduction supports; Q-068 graded
  action-status vocabulary `lit_confidence 0.738` Shibasaki/Hallett 2006 early/late
  BP stages supports + Munzert 2009 shared-representation mixed + O'Shea/Moran
  2017 terminology-critique mixed). Bot identity REE Automation (Mac). **Bottleneck
  (continuation):** **MECH-439 F-dominance conversion ceiling remains the live root
  choke**, BUT today is the first day the V3 substrate has a STRUCTURAL fix for the
  MECH-191 phasic-externalisation gap (the named cortical bias heads emit per-
  candidate-FLAT output; routing the per-candidate REPRESENTATION restores the
  range). The selection-face attack now has TWO parallel V3-EXQ-707 lineage retests
  in flight on ARC-110 (707a driver-only null-liveness gate fix; 707b C2 release on
  the routed substrate) plus the structural MECH-451 retest deferred behind the
  same MECH-191 gap the C2 release closes for the segregated-loop arbitration.
  MECH-441 + the curiosity-channel falsifier chain stay HELD on the V3-EXQ-707
  lineage returning contributory. **ETHICS-PERIMETER Phase 0 datum** stays on the
  record (Phases 1-3 deferred; NON-BLOCKING for V3 green-board 2026-07-19).

- **2026-06-28T01:10Z nightly read (scheduled `/update-docs`).** **MAJOR SUBSTRATE
  DAY -- the cluster autopsy 704b+706b verdict (conversion ceiling = single-arena
  collapse) was applied + the named V4 endpoint built immediately: ARC-110 (parallel
  segregated cortico-BG-thalamic loops) + ARC-109 (D1/D2 split) + MECH-452 (loop-local
  eligibility traces) ALL landed; the parked CPKT-TONIC-EXPLORATION-NOISE decision
  packet was BUILD-adjudicated by the user (override of adjudicator's defer) and
  MECH-440 (NoisyNet propagating selection-head weight noise) + MECH-441 (model-
  disagreement directed curiosity) BOTH landed on top of ARC-110.** `evidence/
  experiments/` flat top-level holds **507 `v3_exq_*.json` manifests + 1 `v4_exq_*.json`**
  on disk (count unchanged since the 2026-06-27T01:10Z read -- V3-EXQ-707 + 708 +
  700d all in flight, not yet completed). Per-machine Phase-3 cards continue to lead
  the legacy tally. **Pending review queue (`pending_review.md` last regenerated
  2026-06-27T10:14Z): 0 items** -- "All experiments reviewed. Nothing pending." The
  2026-06-27 `/governance` cycle (REE_assembly master c95a4fb128 governance + bfffff21bd
  autopsy) inline-applied the route-A cluster autopsy 704b+706b (conversion ceiling =
  single-arena collapse, ARC-110), superseded 700c + 706, flipped MECH-451 ->
  substrate_ceiling + pending_retest, registered the v4_loop_segregation substrate
  with +=MECH-451, MECH-314 and 2 failure records, ran closure-drift 0/0, and
  reviewed 4 runs (704b, 706b, 700c, 706). **Currently queued (`experiment_queue.json`
  items[]): 2 items, BOTH claimed + running** -- **(a) V3-EXQ-707** (priority 400;
  `claim_ids=[ARC-110]`; substrate-readiness validation for the just-landed
  v4_loop_segregation substrate; 4 arms x 6 seeds A0_SINGLE_ARENA (the 704b-A2
  baseline) / A1_LOOPS (ARC-110 full stack) / ARM_NOISE (A1 + S2 in-layer same-layer
  null) / ARM_DROP_LIMBIC (A1 with limbic loop ablated -- the ARC-106 per-loop
  falsifier); PRE-REGISTERED decisive: A1 committed-class entropy strict-above A0
  AND valid in-layer null with live cross-loop variance => supports ARC-110;
  valid null verify-lifts but loops vary without conversion => weakens ARC-110
  (ceiling intrinsic); non-degeneracy self-routes substrate_not_ready_requeue;
  prefer-cloud; smoke dry-run PASS; claimed ree-cloud-2 11:10Z); **(b) V3-EXQ-708**
  (priority 380; `claim_ids=[MECH-440]`; MECH-440 NoisyNet propagation falsifier on
  the 569i top-k + MECH-448 demotion SOTA stack; 4 arms A0_OFF / ARM_TEMP=687
  matched-pre-commit-variance temperature control / ARM_NOISE_SINGLE / ARM_NOISE_LOOPSEG;
  only swept factor = exploration-injection site; PRIMARY DV committed-action-class
  entropy; SECONDARY pre-commit sampling-class entropy thrash-vs-carve; loop-seg
  OFF vs ON disambiguates injection-site fix vs the single-arena/ARC-110 axis;
  smoke dry-run PASS; claimed ree-cloud-3 12:05Z). **Substrate / governance
  landings since the 2026-06-27T01:10Z spec sync:** (1) **`/governance` cycle
  2026-06-27** (REE_assembly master c95a4fb128 governance + bfffff21bd autopsy)
  -- inline route-A cluster autopsy 704b+706b (conversion ceiling = single-arena
  collapse, route to ARC-110 v4_loop_segregation); 700c/706 superseded; MECH-451
  -> substrate_ceiling + pending_retest; v4_loop_segregation unblocks +=MECH-451,
  MECH-314 with 2 failure records; closure-drift 0/0; pending 0; 4 reviewed.
  (2) **`/implement-substrate` ARC-110 full-scope BUILD landed** (ree-v3 main
  c969621 + REE_assembly master 72b69bc2ba) -- segregated cortico-BG-thalamic
  loops (motor / associative / limbic) + S2 in-layer same-layer null + ARC-109
  D1/D2 split + MECH-452 loop-local traces; all no-op default + bit-identical OFF;
  strict additive extension to `e3_selector.py` (NO parallel module per ARC-106
  G2); third major worked application of ARC-106 (grounding ladder L1 functional
  translation of Alexander/DeLong/Strick + Haber spiral; per-loop divergence ledger
  + per-loop ablation falsifier + named psychiatric failure modes); V3-EXQ-707
  substrate-readiness validation queued, DB-ingested (applied:true), claimed
  ree-cloud-2; smoke PASS (unit + 704b + 707 dry-runs). (3) **`/decide-build`
  CPKT-TONIC-EXPLORATION-NOISE adjudicated BUILD** (REE_assembly master 2509fe4134)
  -- packet hybridize-recommendation; user verdict BUILD BOTH MECH-440/441 (override
  of adjudicator's defer-pending-ARC-110); biology-before-formal gate PASSED;
  un-parked `ceiling_decision` on both claims + routing notes record BUILD APPROVED
  + the same-day single-arena/ARC-110 re-rooting risk (the 704b/706b cluster autopsy
  applied earlier the same day) + falsifier constraints; GAP-C-build deferred ->
  in-progress; GAP-C stale owner_exq/resume_condition refreshed; decision record
  + 2 decision_log entries + receipt added. (4) **`/implement-substrate` MECH-440
  + MECH-441 substrate built** (ree-v3 main a65609b + REE_assembly master 6e096b8448)
  on top of the same-day landed ARC-110: NoisyNet weight-noise propagating into the
  E3 selection head + a small K-head ResidualHarmForward ensemble whose per-candidate
  cross-head variance feeds a propagating curiosity bonus; both bit-identical OFF;
  claims.yaml implementation_notes (NO status change); arch-doc IMPLEMENTED block;
  arc_106 2 divergence rows; substrate_dependencies landing record mechanism_changing=false
  KEEP-all. **V3-EXQ-708** MECH-440 falsifier queued + DB-ingested + claimed
  ree-cloud-3; validate_experiments --strict OK, validate_queue OK, dry-run smoke
  PASS. MECH-441 falsifier HELD (`blocked_substrate`, claims.yaml note + carry-forward
  chip) gated on ARC-110 validation V3-EXQ-707 contributory -- the 706b autopsy
  already proved the curiosity channel works on a sufficient substrate; running
  MECH-441 before ARC-110 is validated would re-derive the arena ceiling (vacuous
  FAIL; user decision). **Bottleneck (continuation):** **MECH-439 F-dominance
  conversion ceiling remains the live root choke**, BUT today is the inflection
  point: the cluster autopsy verdict (single-arena collapse) was applied + the
  named V4 ARC-110 endpoint was BUILT THE SAME DAY + the brake-LOCKED MECH-440/441
  exploration channels were unblocked. The selection-face attack now has THREE
  parallel layers in flight: V3-EXQ-707 (ARC-110 readiness validation, ree-cloud-2 --
  if A1 committed-class entropy strict-above A0 AND the null is valid, supports
  ARC-110 as a binding constraint on MECH-439); V3-EXQ-708 (MECH-440 NoisyNet
  propagation, ree-cloud-3 -- if ARM_NOISE_LOOPSEG converts but ARM_NOISE_SINGLE
  does not, the injection-site fix is gated on ARC-110); V3-EXQ-700d (the ARC-108
  validity letter on cloud-1, still in flight from yesterday). MECH-441 + the
  curiosity-channel falsifier chain stay HELD on V3-EXQ-707 returning contributory.
  **ETHICS-PERIMETER Phase 0 datum** stays on the record (Phases 1-3 deferred;
...
- **2026-06-22T01:12Z nightly read (scheduled `/update-docs`).** ARC-107 BG-selector
  ...
  the channel-adaptive MECH-448 envelope. `evidence/experiments/` flat top-level now
  holds **475 `v3_exq_*.json` manifests + 1 `v4_exq_*.json`** on disk (frontier through
  V3-EXQ-689g + V4-EXQ-001; +17 since the 2026-06-21 read). Per-machine Phase-3 cards
  under `runner_heartbeats/` + `runner_status/` lead the legacy single-file tally (the
  legacy `runner_status.json` was retired from `ree-v3/` in favour of the per-machine
  cards). **Pending review queue (regenerated 2026-06-21T19:28Z): 1 item** -- V3-EXQ-485k
  SD-033b / MECH-263 corrected DEVALUATION arm on the MECH-448 demotion-enabled E3
  selector (FAIL 2026-06-21T19:25Z; left PENDING by the 2026-06-21T19:19Z governance
  cycle after the autopsy reclassified it substrate_not_ready_requeue / non_contributory
  / non_degenerate:false; MECH-263 sig-a / SD-033b devaluation stays
  pending_retest_after_substrate, routed to /implement-substrate MECH-449 (build gate
  cleared) + GATE the corrected V3-EXQ-485l behind the MECH-449 build). **Currently
  queued (`experiment_queue.json` items[]): 1 item, 1 claimed** -- **V3-EXQ-654i**
  arc_062 GAP-B committed-class-entropy falsifier on the MECH-448 F->eligibility
  demotion (654h-successor; 485j-style envelope-floor calibration so the demotion lever
  excludes on the arc_062 bank; claim_ids=[MECH-309, ARC-062]; ree-cloud-1; priority
  410). The earlier 6-item queue (V3-EXQ-460i / 689c / 689d / 693a / 695 / 696)
  drained across the day: 689d PASS (MECH-448 promoted candidate -> provisional with
  BG DN-envelope arm 0.938 vs F-route top-k control 0.371, ~2x); 485j C2 OFC
  discrimination CONVERTED (first cross-substrate MECH-448 corroboration off GAP-A);
  654h non_contributory; the rest reviewed + cleared by the 2026-06-21T19:19Z
  governance cycle. **Substrate / governance landings since the 2026-06-21T01:10Z
  spec sync (the BG opponency leg + the machinery anti-forcing keystone):**
  (1) **MECH-449 / ARC-107: Go/No-Go eligibility constitution landed** (ree-v3 main
  829e51e; REE_assembly master 7deb4a2aff) -- component 1 of the ARC-107 BG
  constitution (the OPPONENCY leg complementing MECH-448's pallidal permission gate).
  Build gate cleared on BOTH faces: selection-face V3-EXQ-689f (No-Go-necessity
  falsifier showing the live `_f_eligibility_envelope` admits high-F undesirable
  candidates only an active No-Go can suppress) + behavioural-face the 485e->485k
  six-autopsy lineage (demotion-alone converts the passive OFC discrimination signature
  but cannot express the active No-Go WITHDRAWAL devaluation requires). Bounded
  Go/No-Go pressure SET as methods on `E3TrajectorySelector._go_nogo_eligibility_gate`
  over the MECH-448 eligible set (NO parallel module per ARC-106 G2); No-Go drops
  candidates on an axis ORTHOGONAL to F-rank (safety / staleness / perseveration /
  low-viability); bounded Go re-admits a demoted channel; fail-open guard (safety
  never overridden). Perseveration axis REUSES MECH-260 (dACC suppression routed in)
  per the reuse-before-duplicate guardrail. SECOND major worked application of ARC-106
  (brain-like construction). No-op default + bit-identical OFF; MECH-094 waking-only;
  6/6 new contracts + 8/8 preflight + 148 E3-cluster contracts + full-agent bit-identical
  OFF PASS. V3-EXQ-689g (selection-face conversion ablation falsifier;
  claim_ids=[MECH-449, ARC-107]; priority 430) queued as the validation falsifier.
  MECH-449 stays candidate / substrate_conditional (the ablation falsifier gates
  promotion). (2) **MECH-448 AMEND: channel-adaptive (mean-relative) eligibility floor
  landed** (ree-v3 main 7b44131; REE_assembly master 9a5ddc0388) -- collapses ~5
  per-channel hand-floor dances (654h all-admit no-op + 485i/485j bespoke OFC
  recalibration + pending 625/445/687 sweeps) into ONE global knob. New E3Config flag
  `use_f_eligibility_adaptive_floor` (+ `f_eligibility_adaptive_mean_factor` default
  1.0) replaces the fixed 0.30 absolute share floor with `mean_factor * elig.mean()` --
  scale-invariant per channel, conflict-graded preserved, excluded_count>0 by
  construction on any non-uniform field, RANK-PRESERVING. No-op default (double-guarded:
  `use_f_eligibility_demotion` also default OFF) -> bit-identical OFF. 16/16 MECH-448
  contracts (8 new) + 8/8 preflight + 48/48 E3-cluster PASS. V3-EXQ-689e
  channel-adaptive envelope readiness diagnostic queued + END-TO-END VALIDATED at toy
  8-ep probe: ADAPTIVE excl>0_frac=1.0 env~17/32 rankpres=1.0 on BOTH channels
  (arc_062 lateral_pfc+gated_policy+mech295 / SD-033b OFC) vs FIXED-0.30
  excl>0_frac~0.01-0.02 env~31/32 (the 654h all-admit no-op reproduced). PROMOTES
  NOTHING (MECH-448 stays candidate at this stage of validation). (3) **ARC-107
  anti-partial-instantiation guards landed** (REE_assembly master 058bb649bc) --
  MECH-449 gate INVERTED in claims.yaml from "build only if MECH-448 insufficient" to
  on-the-roadmap with a POSITIVE No-Go-necessity release trigger; BG completeness
  ledger added to the ARC-107 design doc (5 constitution components + v4 axes + a
  closure criterion requiring more than component 3 so "selection-face lifted" cannot
  masquerade as circuit-complete). V3-EXQ-689f No-Go-necessity falsifier queued + ran
  PASS (admit 0.866 / No-Go 0.0 / rank-preserving 1.0; demotion_insufficient_nogo_necessary)
  as the positive build trigger for MECH-449. (4) **assembly-vs-closure machinery
  keystone + 3 follow-on moves landed** (REE_assembly master c6e4272406 + ebe7df11c8
  + 559a3e1378 + 6dd93ce771) -- diagnosis of the "machinery forces closure before
  substrate is ready" pathology, addressed by a first-class `assembling`/`open_by_design`
  closure-plan node state credited on a SEPARATE assembly-frontier axis (weight None ->
  excluded from the punishing closure %, NOT folded into deferred/remaining; restful
  in drift; opt-in `revisit_after` resume trigger). serve.py /api/closure now carries
  an `assembly` block (bucketing V3 nodes by maturity: mature / mid_construction /
  awaiting_construction / genuinely_blocked / remaining); closure.html
  (`CLOSURE_VERSION 2026-06-21.1`) renders an "Assembly maturity" strip with a TWO-number
  headline (closure % AND assembly-frontier health). MOVE-2 carved
  `session-land` Phase-3 rule 3a (chip an UNBUILT critical-path substrate as a BUILD
  task routed to /implement-substrate when on critical path + not built + in no build
  queue + nobody constructing it); MOVE-3 wired the failure-autopsy granularity-debt
  recurrence hook as a re-derive brake; MOVE-4 added the assembly portfolio view.
  V3 closure % unchanged 79.0%; drift 0/9/0 unchanged. (5) **MECH-263 / SD-033b
  granularity refinement** -- /claim-synthesis decomposed SD-033b into sig-a
  (aversive devaluation) vs sig-b (passive discrimination); 485j (devaluation
  re-rank corrected) PASSed on sig-b C2 discrimination but FAILed on sig-a C1
  devaluation (the 485k autopsy re-classified non_contributory pending the MECH-449
  build, which now closes that gate). MECH-263 sig-a stays
  pending_retest_after_substrate; 485l queued behind the MECH-449 build (V3-EXQ-689g).
  (6) **assembly-vs-closure plan-of-record** (`evidence/planning/assembly_vs_closure_plan.md`,
  `generation: meta`, excluded from V3 %) ledgers the diagnosis +
  4 moves + open follow-ons; the open follow-on is consolidating the 6 scattered
  substrate-blocked conventions at the claims layer. **Bottleneck (continuation):**
  **MECH-439 F-dominance conversion ceiling is the live root choke**; the ARC-107
  BG-constitution build is the response. **MECH-448 lead lever now PROMOTED candidate ->
  provisional** by 689d PASS (BG DN-envelope arm 0.938 vs F-route top-k control 0.371;
  the F-dominance selection-readout route 569i top-k / commit-T / near-tie family is
  EXHAUSTED). **MECH-449 Go/No-Go opponency leg in flight** via V3-EXQ-689g
  (selection-face falsifier) once the runner picks it up. The commit/release-DURATION
  lever (V3-EXQ-460i) drained with the prior queue; the gate is now squarely the
  selection-face BG-constitution + the arc_062 654i GAP-B retest of MECH-309/ARC-062
  on the demotion-amended substrate. Constitutional path PRIMARY across both faces;
  downstream retests all run with `use_f_eligibility_demotion=True` as the new constant.
  **ETHICS-PERIMETER Phase 0 datum** stays on the record (Phases 1-3 deferred;
  NON-BLOCKING for V3 green-board 2026-07-19).
