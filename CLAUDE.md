...
>
> **Hub-runner co-tenancy is unsafe.** Because the heartbeat-write happens locally, a runner on the hub VM (`ree-cloud-1`) will dirty the same REE_assembly checkout the writer is trying to publish from, which breaks the writer's clean-tree refusal. The cloud-1 runner is currently `systemctl disable`-d. Until a new `PHASE3_DISABLE_RUNNER_HEARTBEAT_WRITE` flag lands (chip filed 2026-05-29), keep cloud-1's runner off.
