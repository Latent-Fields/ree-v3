...
Claim: MECH-057a
Proposal: EXP-0065 / EVB-0056

MECH-057a asserts: "Committed action sequences suppress new precision updates until
execution completes."

Reframing (2026-03-15, heartbeat architecture analysis): completion events are the
principal policy-propagation opportunities within an otherwise continuous E3 internal
update stream. Beta drops at sequence completion, E3 state propagates to action
selection. The key mechanism is NOT binary episodic E3 gating, but rather:

  During commitment:
    - E3 updates internally (continuous)
    - Precision (running_variance) update is SUPPRESSED
    - BetaGate: elevated, policy output held
  At completion (uncommitted):
    - Precision update fires (accumulated error is applied)
    - BetaGate: released, E3 state propagates to action selection

The functional prediction: suppressing precision updates during commitment (flushing
only at sequence completion) improves harm avoidance quality compared to continuous
per-step precision updates that interfere with the committed sequence.

Why this is MECH-057a and not MECH-090:
  MECH-090 (EXQ-119) tested that BetaGate elevation TRACKS commitment state -- it
  confirmed the gate is concordant with commitment. MECH-057a asks a different
  question: does suppressing precision updates *during* commitment (completing the
  loop at sequence boundary) improve outcomes? The discriminative manipulation here
  is the precision update timing, not gate concordance.

Discriminative pair
--------------------
COMPLETION_GATE_ON (MECH-057a architecture):
  - running_variance update suppressed while agent is committed
    (is_committed = running_variance < commit_threshold)
  - Accumulated prediction errors are flushed as a single batch at sequence
    boundary (transition to uncommitted state)
  - BetaGate: elevated while committed, released when uncommitted
  - Models: "precision updates are held until sequence completion event"

COMPLETION_GATE_ABLATED (continuous update, ablation):
  ...
  - BetaGate: disabled (always released) -- no policy propagation gating
  - Models: naive continuous E3 that ignores sequence structure

Key difference:
  GATE_ON accumulates prediction errors during commitment and applies them as
  a batch at the completion boundary. This prevents mid-sequence precision
  fluctuations from disrupting the committed trajectory.
  GATE_ABLATED updates precision continuously, allowing mid-sequence noise to
  interfere with commitment decisions.

Measurement: harm_rate (harm events / total eval steps) and committed_sequence_length
  (average steps per committed sequence). Gate-ON should produce lower harm_rate and
  longer stable committed sequences.

Pre-registered acceptance criteria:
  C1: harm_rate_gate_on <= harm_rate_ablated * 0.90 (both seeds)
      Gate-ON reduces harm by >=10% relative to ablation.
  C2: per-seed harm_rate gap >= 0.005 (both seeds)
  ...
  C4: n_committed_min >= 30 committed-sequence entries total across eval (GATE_ON, both seeds)
      Enough committed-state steps to confirm commitment dynamics are present.
  C5: no fatal errors across all cells.

Decision scoring:
  PASS (all 5): retain_ree -- completion-gated precision suppression reduces harm
  C1 fails, C3+C4 pass: retire_ree_claim -- no harm benefit from completion gating
  C4 fails: hold -- agent not reaching committed state; increase warmup or threshold
...
If this experiment PASSes, which claim does that support and why?
  ...
  boundary) produces lower harm rates than continuous per-step updating. This
  supports the assertion that committed sequences should suppress precision updates
  until completion -- the functional benefit is harm avoidance improvement from
  preventing mid-sequence noise from disrupting commitment stability.
  Note: MECH-090 (beta gate concordance) must also hold for this design to work;
  GATE_ON includes BetaGate elevation. But the discriminative manipulation here
  is the precision update timing, making MECH-057a the primary claim tested.
...
EXPERIMENT_TYPE = "v3_exq_139_mech057a_action_loop_gate_pair"
CLAIM_IDS = ["MECH-057a"]
...
def _run_cell(
    ...
    gate_on=True:  COMPLETION_GATE_ON -- precision updates suppressed during commitment,
                   flushed as batch at completion boundary. BetaGate active.
    gate_on=False: COMPLETION_GATE_ABLATED -- continuous per-step precision updates,
                   BetaGate disabled.
    """
    ...
    cond_label = "COMPLETION_GATE_ON" if gate_on else "COMPLETION_GATE_ABLATED"
    ...
    for ep in range(actual_warmup):
        ...
        for _ in range(steps_per_episode):
            ...
            # Commitment state (variance-space)
            is_committed = (
                agent.e3._running_variance < agent.e3.commit_threshold
            )

            if gate_on:
                # COMPLETION_GATE_ON: elevate/release BetaGate based on commitment
                if is_committed:
                    ...
                    if was_committed_ep and pending_errors_ep:
                        # Apply batch variance update using mean accumulated error
                        mean_err = sum(pending_errors_ep) / len(pending_errors_ep)
                        # Update running_variance directly (EMA step with batch mean)
                ...
                # COMPLETION_GATE_ABLATED: BetaGate always released, no gating
                agent.beta_gate.release()
            ...
            # For E1 prediction error as proxy for variance signal
            with torch.no_grad():
    ...
    mean_seq_len = float(sum(seq_lengths) / max(1, len(seq_lengths))) if seq_lengths else 0.0
...
def run(
    ...
    **kwargs,
    ...
    """
    Discriminative pair: COMPLETION_GATE_ON (precision updates suppressed during
    commitment, flushed at completion boundary -- MECH-057a architecture) vs
    COMPLETION_GATE_ABLATED (continuous per-step precision updates, no gating).
    ...
    for seed in seeds:
        for gate_on in [True, False]:
            label = "COMPLETION_GATE_ON" if gate_on else "COMPLETION_GATE_ABLATED"
            print(
                f"\n[V3-EXQ-139] {label} seed={seed}"
    ...
    if all_pass:
        interpretation = (
            ...
            f" {[round(r, 4) for r in per_seed_ratios]} all below"
            f" threshold {THRESH_C1_HARM_RATIO}. Suppressing precision updates during"
            " committed sequences and flushing at completion boundary reduces mid-sequence"
            " noise, improving harm avoidance -- consistent with MECH-057a's assertion"
            " that committed sequences suppress precision updates until execution completes."
        )
    elif c3_pass and c4_pass and not c1_pass:
        interpretation = (
        ...
        interpretation = (
            ...
            " enter committed sequences during eval. Increase warmup episodes or"
            " lower the commit_threshold to increase commitment frequency."
        )
    else:
        interpretation = (
    ...
    summary_markdown = (
        f"# V3-EXQ-139 -- MECH-057a Action Loop Completion Gate Discriminative Pair\n\n"
        f"**Status:** {status}\n"
        f"**Claims:** MECH-057a\n"
        f"**Decision:** {decision}\n"
        f"**Seeds:** {list(seeds)}\n"
        f"**Conditions:** COMPLETION_GATE_ON vs COMPLETION_GATE_ABLATED\n"
        ...
        f"COMPLETION_GATE_ON: running_variance update suppressed while committed;"
        f" accumulated errors flushed as batch at completion boundary (uncommitted"
        f" transition). BetaGate active (elevates when committed, releases at completion).\n\n"
        f"COMPLETION_GATE_ABLATED: continuous per-step running_variance updates;"
        f" BetaGate disabled (always released). No sequence-boundary gating.\n\n"
        ...
        f"|-----------|----------------|---------------|\n"
        f"| COMPLETION_GATE_ON      | {harm_rate_on:.4f}"
        f" | {sum(r['harm_contacts'] for r in results_on)} |\n"
        f"| COMPLETION_GATE_ABLATED | {harm_rate_ablated:.4f}"
        ...
        f"{interpretation}\n\n"
        f"## Per-Seed\n\n"
        f"COMPLETION_GATE_ON:\n{per_on_rows}\n\n"
        f"COMPLETION_GATE_ABLATED:\n{per_abl_rows}\n"
        f"{failure_section}\n"
    )
...
if __name__ == "__main__":
    ...
    result["conditions"]     = ["COMPLETION_GATE_ON", "COMPLETION_GATE_ABLATED"]
    result["dispatch_mode"]  = "discriminative_pair"
    result["backlog_id"]     = "EVB-0056"
