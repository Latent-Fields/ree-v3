...
"""
V3-EXQ-076f -- MECH-116 / ARC-032: E1 Goal Conditioning (10k-step budget)

Claims: MECH-116, ARC-032
Supersedes: V3-EXQ-076e

Root cause of 076e silent failure:
  E1Config has no goal_dim field. E1DeepPredictor.__init__ reads
  getattr(config, 'goal_dim', 0) -- always returns 0 regardless of the
  e1_goal_conditioned flag, so goal_input_proj is None in BOTH conditions.
  The two conditions were therefore identical at the E1 level: E1 never
  received z_goal input in either case. This guaranteed C1 (halflife ratio)
  could never differ. REEConfig.from_dims() sets e1_goal_conditioned on
  config.goal (GoalConfig) but does NOT propagate goal_dim to config.e1.

Fix:
  After calling REEConfig.from_dims(), manually set
    config.e1.goal_dim = world_dim  (for goal_conditioned)
    config.e1.goal_dim = 0          (for goal_unconditioned, default)
  This matches the pattern used correctly in EXQ-147 / EXQ-147a.
  With goal_dim=world_dim=32, E1DeepPredictor builds a Linear(96, 64)
  goal_input_proj and the forward() path applies it when z_goal is passed.
  The agent._e1_tick() already passes z_goal to e1() when e1_goal_conditioned
  and goal_state.is_active() -- that path is correct and unchanged.
...
Two conditions (matched seeds):
  ...
  B. goal_conditioned   -- z_goal_enabled=True, e1_goal_conditioned=True,
                           config.e1.goal_dim=world_dim (goal_input_proj active)
...
EXPERIMENT_TYPE = "v3_exq_076f_mech116_arc032_e1_goal_conditioning"
...
def _run_condition(
    ...
    # FIX (076f): propagate goal_dim to E1Config so E1DeepPredictor builds
    # goal_input_proj when conditioned. REEConfig.from_dims() only sets
    # e1_goal_conditioned on config.goal (GoalConfig); E1Config.goal_dim
    # must be set separately. Pattern from EXQ-147 / EXQ-147a.
    if e1_goal_conditioned:
        config.e1.goal_dim = world_dim
    # else: config.e1.goal_dim defaults to 0 (getattr fallback), goal_input_proj=None

    agent = REEAgent(config)
    device = agent.device

    # Sanity check: confirm goal_input_proj state matches condition
    assert (agent.e1.goal_input_proj is not None) == e1_goal_conditioned, (
        f"[{condition}] goal_input_proj mismatch: "
        f"is_none={agent.e1.goal_input_proj is None} e1_goal_cond={e1_goal_conditioned}"
    )
    print(
        f"  [{condition}] e1_goal_dim={agent.e1._goal_dim}"
        f" goal_proj={'active' if agent.e1.goal_input_proj is not None else 'None'}",
        flush=True,
    )
...
def run(
    ...
    **kwargs,
    ...
    print(
        f"\n[EXQ-076f] MECH-116/ARC-032 E1 Goal Conditioning 10k-step budget"
        f" seed={seed}",
        flush=True,
    )
    ...
    summary_markdown = (
        f"# V3-EXQ-076f -- MECH-116 / ARC-032 E1 Goal Conditioning 10k-step\n\n"
        f"**Status:** {status}\n**Claims:** MECH-116, ARC-032\n"
        f"**Seed:** {seed}  **Steps:** {TOTAL_STEPS}"
        f"  **Resource removal:** step {REMOVAL_STEP}"
        f"  **Checkpoint:** step {REMOVAL_STEP + 1000}\n"
        f"**Bug fixed (076f):** config.e1.goal_dim now set to world_dim={world_dim}"
        f" for goal_conditioned condition. In 076e (and all prior iterations),"
        f" E1Config.goal_dim was never set by from_dims(), so goal_input_proj was"
        f" None in both conditions -- E1 was goal-agnostic in both cases.\n\n"
        f"## Conditions\n\n"
        f"- goal_unconditioned: z_goal_enabled=True, e1_goal_conditioned=False,"
        f" config.e1.goal_dim=0 (goal_input_proj=None)\n"
        f"- goal_conditioned: z_goal_enabled=True, e1_goal_conditioned=True,"
        f" config.e1.goal_dim={world_dim} (goal_input_proj active: Linear(96, 64))\n\n"
        f"## Results\n\n"
