...
"""
V3-EXQ-680b -- EXP-0380 MECH-423 cross-model SUPER-ADDITIVITY 3-arm ablation.
Supersedes V3-EXQ-680a (hardened super-additivity PASS criterion; see the
SUPERSEDES note below the imports). Identical 3-arm ablation + R1/R2/R3 readiness
gate -- the ONLY change vs 680a is the margin definition.
...
Run with:
  /opt/local/bin/python3 experiments/v3_exq_680b_mech423_superadditivity_ablation.py --dry-run
  /opt/local/bin/python3 experiments/v3_exq_680b_mech423_superadditivity_ablation.py
"""
...
EXPERIMENT_TYPE = "v3_exq_680b_mech423_superadditivity_ablation"
QUEUE_ID = "V3-EXQ-680b"
CLAIM_IDS: List[str] = ["MECH-423"]
EXPERIMENT_PURPOSE = "evidence"
# Supersedes V3-EXQ-680a (which itself superseded V3-EXQ-680, the encoder-not-in-
# optimizer bugfix). 680a RAN and recorded a PASS/supports for MECH-423
# (manifest v3_exq_680a..._20260614T191656Z_v3), but that PASS rests on a
# methodologically weak margin: it scaled the noise on the across-seed SD of the
# baseline LEVEL (pstdev(iso_scores)), not the SD of the DELTA. On 680a's own
# data (delta_pair = [+1.57, +1.24, -1.19]) the effect is NOT reliably positive
# -- two seeds strongly super-additive, one strongly sub-additive -- yet it
# cleared the level-scaled margin (0.1024) on 2/3 seeds. Under the hardened
# criterion below (noise = SD of the delta itself, hard-floored by an absolute
# minimum effect size) that same data routes to additive_below_margin /
# inconclusive, which is the more defensible verdict. 680b re-runs the IDENTICAL
# 3-arm ablation + readiness gate; the ONLY change is the super-additivity PASS
# gate. Mark the 680a manifest evidence_direction=superseded at next governance.
SUPERSEDES = "v3_exq_680a_mech423_superadditivity_ablation"
