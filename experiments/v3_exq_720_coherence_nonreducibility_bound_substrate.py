...
"""
V3-EXQ-720 -- Coherence non-reducibility RETEST on the BOUND substrate.
(reuses the V3-EXQ-641a harness verbatim; does NOT supersede it)

WHAT CHANGED vs 641a (the ONLY change): the agent is now built with
cross_stream_binding_enabled=True (strength=0.5) -- the newly-landed
cross_stream_binding_substrate. That substrate injects a shared latent factor
(theta-gated, derived from the JOINT z_self/z_world state) identically into BOTH
E2.rollout_with_world transitions, so the two rollout streams
(trajectory.states = z_self, trajectory.world_states = z_world) now share a
genuine common cause. 641a (FAIL, substrate ceiling; failure_autopsy_V3-EXQ-641a
_2026-06-06) showed that with the two streams UNBOUND, cross-stream phase
alignment C(tau) is functionally redundant with E (SPEC 1/6 seeds; shuffle
diverged >= real in 4/6). THIS run asks the same question against the enriched
substrate: with the streams genuinely bound, does real cross-stream coherence now
carry selection information non-reducible to E and destroyed by a shuffle?

strength=0.5 is PRE-REGISTERED (not tuned to a pass): at 0.5 the injected shared
component is ~34% of the base per-step z_world delta -- a genuine co-determinant,
still subdominant to the base dynamics so E remains the primary driver and
"coherence beyond E" is a fair test. Measured during the substrate smoke test.

The harness, seeds, conditions, reads, control, gate and thresholds are IDENTICAL
to 641a (see below). Everything below this paragraph is inherited verbatim.
...
EXPERIMENT_TYPE = "v3_exq_720_coherence_nonreducibility_bound_substrate"
QUEUE_ID = "V3-EXQ-720"
SUPERSEDES = None  # does NOT supersede 641a; retest on the enriched (bound) substrate
CLAIM_IDS: List[str] = []  # diagnostic -- bears on INV-002/MECH-089/MECH-094/MECH-270/MECH-269, tags NONE

# cross_stream_binding_substrate activation (the ONLY substrate change vs 641a).
# Pre-registered: strength 0.5 makes the injected shared component ~34% of the
# base per-step z_world delta (genuine co-determinant, subdominant to base
# dynamics). See docs/architecture/sd_cross_stream_binding_substrate.md.
...
def _make_agent(env: CausalGridWorldV2) -> REEAgent:
    ...
    cfg = REEConfig.from_dims(
        ...
        # cross_stream_binding_substrate: the ONLY change vs 641a. Genuinely
        # binds the z_world<->z_self rollout streams so cross-stream coherence
        # can carry selection info non-reducible to E.
...
def _interpret(rows: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    # READINESS (load-bearing for THIS retest): the cross_stream_binding_substrate
    # must actually be ACTIVE. If binding is OFF/misconfigured, the run silently
    # collapses to 641a and must NOT be read as a substrate verdict -- it is a
    # not-ready run. Measured = the configured coupling strength; floor > 0.
    binding_active = bool(CROSS_STREAM_BINDING_ENABLED) and (
        CROSS_STREAM_BINDING_STRENGTH > 0.0
    )
    ...
    preconditions = [
        {
            "name": "cross_stream_binding_active",
            "kind": "readiness",
            "description": (
                "cross_stream_binding_substrate enabled with strength > 0 so the "
                "z_world<->z_self streams are genuinely bound; OFF collapses this "
                "retest to 641a and is not a substrate verdict."
            ),
            "measured": float(CROSS_STREAM_BINDING_STRENGTH if binding_active else 0.0),
            "threshold": 0.0,
            "control": "config activation of the bound substrate",
    ...
    if not binding_active:
        # Misconfigured: the bound substrate was not engaged -> not-ready, NOT a
        # substrate verdict. (The one route the readiness rule mandates.)
    ...
    return {
        ...
        "criteria": [
            {"name": "SPEC_coherence_specific", "load_bearing": True,
             "passed": bool(n_seed_pass >= MIN_SEEDS_FOR_PASS)},
        ],
...
def _build_manifest(
    ...
    return {
        ...
        "evidence_direction_note": (
            ...
            "confidence or conflict scoring. RETEST of the V3-EXQ-641a harness on "
            "the cross_stream_binding_substrate (cross_stream_binding_enabled=True, "
            "strength=0.5) -- does NOT supersede 641a (its substrate-ceiling finding "
            "stands; failure_autopsy_V3-EXQ-641a_2026-06-06). The ONLY change vs "
            "641a is that the z_world<->z_self rollout streams are now genuinely "
            "bound by a shared theta-gated latent factor injected into both "
            "E2.rollout_with_world transitions. Settles thought_intake_2026-04-23"
            "_binding.md + _path_integral_constraints_search.md (both OPEN/gated "
            "on a bound substrate): C_nonreducible_coherence_specific_register "
            "converts the candidate Q entities/selection.coherence_nonreducibility "
            "to a registerable claim + closes both intakes positive; F1/F2 close "
            "both intakes (binding hypothesis falsified for this substrate design); "
            "specificity_unproven or substrate_not_ready_requeue route a "
