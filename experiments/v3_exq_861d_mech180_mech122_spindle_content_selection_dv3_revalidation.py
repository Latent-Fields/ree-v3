"""
V3-EXQ-861d -- MECH-122 content-packaging DV3 (spindle-density,
mean_sws_new_slot_diversity) REVALIDATION after the novelty-reference fix.
SUPERSEDES V3-EXQ-861a. IDENTICAL design/arms/seeds/thresholds/gates to
V3-EXQ-861a (5 arms, seeds [42,123,456], C1a/C1b/C1c + C2 grid,
use_mech122_spindle_content_selection=True uniformly across all arms) EXCEPT
for the single substrate change this run validates: the spindle content-
selection novelty reference is now sourced from the SD-MEL prediction-error
channel via mech122_novelty_reference_mode='mel_pe' (ree-v3 6aa97ca,
2026-08-14), pinned explicitly in this driver's config.

THIS RUN vs V3-EXQ-861a (the single change under test):
  V3-EXQ-861a wired ThetaBuffer content-selection into run_sws_schema_pass
  (ree-v3 a7d36429fd) but its DV3 still failed 0/3 seeds. The V3-EXQ-861a
  autopsy traced this to the novelty reference itself: ThetaBuffer's default
  reference was a 10-tick recency buffer, DECOUPLED from the MEL axis, so a
  higher-MEL arm produced no more differentiated selection than a low-MEL arm
  -- the DV3 channel could not track novelty because its selection_weight had
  no MEL-graded input. The fix (6aa97ca) adds mech122_novelty_reference_mode:
  when 'mel_pe' (now the config default, pinned here explicitly), the per-
  prototype selection_weight is additively lifted by
  MELConsumer.relative_novelty()=clamp(mel/ref-1, 0, 1) -- coupling content
  selection to the same graded MEL axis MECH-180's other two DVs already
  track. For the OFF control arm (no MEL consumer) the mel_pe path degrades
  to the per-prototype recency floor, so the arm-symmetry of the OFF/ON
  contrast is preserved. This run re-scores DV3 (mean_sws_new_slot_diversity)
  under the fix, on the same C1b monotone+spread and C2 ON-HIGH>OFF-HIGH
  criteria 861a used. 861a's DV3 result is superseded (decoupled reference);
  its sws_power/replay_rate results are re-measured identically here.

CLAIM UNDER TEST -- does REAL graded environmental novelty (SD-MEL-PRODUCER
world-rule-shift, VALIDATED V3-EXQ-798a) drive graded waking MEL which then
drives graded offline-consolidation activity (SD-MEL-CONSUMER, VALIDATED
V3-EXQ-718a via injection), measured as SWS power, sleep spindle density, and
hippocampal replay rate?
...
EXPERIMENT_TYPE = "v3_exq_861d_mech180_mech122_spindle_content_selection_dv3_revalidation"
QUEUE_ID = "V3-EXQ-861d"
CLAIM_IDS = ["MECH-180", "MECH-122"]
EXPERIMENT_PURPOSE = "evidence"
# Supersedes V3-EXQ-861a: its DV3 (mean_sws_new_slot_diversity) was scored
# under the DECOUPLED ThetaBuffer recency-buffer novelty reference, which the
# V3-EXQ-861a autopsy identified as the reason DV3 failed 0/3 seeds. This run
# re-scores DV3 under the mel_pe-coupled reference (ree-v3 6aa97ca), so 861a's
# DV3 result is superseded rather than additive.
SUPERSEDES = "V3-EXQ-861a"
...
# THE FIX UNDER TEST (ree-v3 6aa97ca, 2026-08-14): source the spindle content-
# selection novelty reference from the SD-MEL prediction-error channel. With
# "mel_pe" the per-prototype selection_weight is additively lifted by
# MELConsumer.relative_novelty()=clamp(mel/ref-1, 0, 1), coupling DV3 selection
# to the graded MEL axis. Pinned explicitly here (it is also the config
# default) so the reference source is recorded and reproducible in the manifest
# rather than relying on the default not drifting. For the OFF arm (no MEL
# consumer) the mel_pe path degrades to the per-prototype recency floor.
MECH122_NOVELTY_REFERENCE_MODE = "mel_pe"
...
def _make_agent(env: CausalGridWorldV2, mel_on: bool, mel_reference: float) -> REEAgent:
    ...
    cfg = REEConfig.from_dims(
        ...
        mech122_novelty_reference_mode=MECH122_NOVELTY_REFERENCE_MODE,
        rem_enabled=True,
...
def run_experiment(steps: int, conv_eps: int, meas_cycles: int,
    ...
    interpretation = {
        ...
        "redesign_note": "spindle_density (C1b/C2) reads mean_sws_new_slot_diversity, "
                         ...
                         "introduced (unchanged readout code). This run SUPERSEDES "
                         "V3-EXQ-861a; its ONLY change from 861a is "
                         "mech122_novelty_reference_mode='mel_pe' (ree-v3 6aa97ca), "
                         "re-sourcing the spindle content-selection novelty "
                         "reference from the SD-MEL PE channel so DV3 selection is "
                         "coupled to the graded MEL axis. Tests whether that fix "
                         "closes 861a's 0/3-seed spindle_density gap; "
                         "use_mech122_spindle_content_selection stays True (uniform "
                         "across every arm), as in 861a.",
