...
"""
V3-EXQ-707 -- ARC-110 PARALLEL SEGREGATED LOOP-SEGREGATION VALIDATION (diagnostic).
...
------------------------------------------------
  ...
  WEAKENED / route-elsewhere (collapse was NOT the binding constraint):
    a VALID same-layer null can now be constructed (ARM_NOISE verify-lifts) and loops carry
    live cross-loop variance, BUT A1_LOOPS does NOT convert committed diversity strict-above
    A0/the null. The conversion ceiling is INTRINSIC, not an artefact of single-arena
    collapse -> weakens ARC-110.
...
NON-VACUITY READINESS GATES (self-route substrate_not_ready_requeue, NEVER a false weakens):
  ...
  (3) the VALID in-layer null (ARM_NOISE) actually verify-lifts committed-class entropy
      strict-above A0 on a strict-majority of divergent seeds (the 569g/700b lesson; this is
      the SAME-LAYER null the single arena could not construct -- if even the in-layer null
      is inert on this substrate, the null-validity problem persists -> requeue, not weakens).
...
EXPERIMENT_TYPE = "v3_exq_707_arc110_loop_segregation_validation"
QUEUE_ID = "V3-EXQ-707"
SUPERSEDES = None   # new NUMBER (different scientific question: loop segregation, not finer-channel)
...
def run_experiment(
    ...
    # ----- Precondition: the VALID in-layer null (ARM_NOISE) verify-lifts committed-class
    # entropy strict-above A0 on a strict-majority of divergent seeds. THIS is the same-layer
    # null the single arena could not construct (the 704b/706b binding constraint); if even
    # the in-layer null is inert on the loop substrate, the null-validity problem persists
    # -> requeue, NOT a weakens. -----
    n_noise_lifts = sum(
    ...
    if not preconditions_met:
        ...
        degeneracy_reason = (
            ...
            "vacuous split pinned to the motor winner / the in-layer null did not verify-lift "
            "/ finer channels not dissociable / delta_t flat). NOT a falsification."
        )
    ...
    interpretation = {
        ...
        "preconditions": [
            ...
            {
                "name": "in_layer_null_verified_lifting",
                "kind": "readiness",
                "description": (
                    "the S2 IN-LAYER same-layer null (ARM_NOISE: magnitude-matched random "
                    "structure at the non-motor loop accumulators) lifts committed-class entropy "
                    "strict-above A0 by margin on a strict-majority of DIVERGENT seeds. This is "
                    "the SAME-LAYER null the single arena could NOT construct (704b/706b binding "
                    "constraint). A null that does NOT verify-lift makes the strict-above bar "
                    "meaningless (569g/700b lesson); on the loop substrate a non-lifting null "
                    "means the null-validity problem PERSISTS => substrate_not_ready_requeue "
                    "(NOT a weakens). measured = n seeds the null lifts."
                ),
                "control": "ARM_NOISE committed-class entropy vs A0, divergent seeds, paired",
                # COUNT-shaped, INCLUSIVE floor. `met` is
                # `enough_divergent and _div_pass(n_noise_lifts, n_primary_div)`, and
                # `_div_pass` ITSELF carries a second leg -- it returns False outright when
                # `n_div < MIN_DIVERGENT_SEEDS`, regardless of n_ok. So the plain `needed`
                # threshold does not reproduce `met` in one real corner: n_div == 2 gives
                # needed == 2 and n_ok can also reach 2, which recomputes MET while the
                # shipped predicate says False (the MISSED_UNMET direction).
                # Closed by making the bound UNREACHABLE when the divergent-seed floor
                # fails, which is what the predicate means there. `met` is unchanged.
                "measured": float(n_noise_lifts),
                "threshold": (
                    float(max(MIN_SEEDS_FOR_PASS,
                              int(math.ceil(DIVERGENT_PASS_FRACTION * max(n_primary_div, 1)))))
                    if enough_divergent else float(n_noise_lifts + 1)
                ),
                "comparator": ">=",
                "direction": "lower",
            ...
            {
                # SPLIT from a single `learning_engaged_..._delta_nonflat` entry whose
                # `met` was `fcg_moved_ok and fcg_delta_nonflat_ok` -- an AND of two
                # INDEPENDENT `_maj` counts reported against a single min-magnitude
                # statistic. No one (measured, threshold) pair can reproduce an AND of
                # two counts, so the indexer was adjudicating that entry on neither of
                # its own legs. Declared as two recomputable counts that must BOTH hold,
                # which is the same predicate; the label still routes off the two
                # booleans directly (`preconditions_met`, L1342), so nothing moves.
                ...
                "description": (
                    ...
                    "seeds clearing it. Below => substrate_not_ready_requeue."
                ),
                "control": "A1 fcg_w_chan_finer_range_max",
                # COUNT-shaped, INCLUSIVE floor: `met` is `_maj(a1_rows, fcg_moved)`,
                # i.e. `n_fcg_moved >= MIN_SEEDS_FOR_PASS`.
                "measured": float(n_fcg_moved),
                "threshold": float(MIN_SEEDS_FOR_PASS),
                "comparator": ">=",
                "direction": "lower",
                "observed_min_fcg_w_chan_finer_range_max": float(
            ...
            },
            {
                ...
                "kind": "readiness",
                "description": (
                    "on A1_LOOPS the signed-RPE delta_t carries cross-tick variance on a "
                    "majority of seeds -- the second leg of the learning-engaged check. "
                    "measured = the NUMBER of A1 seeds clearing it."
                ),
            ...
            },
            {
                ...
                "description": (
                    ...
                    "clears the floor on a majority of seeds (GAP-A non-vacuity). measured = "
                    "the NUMBER of seeds divergent on ALL C1 comparison arms."
                ),
                "control": "SD-056 e2 trained online in P0; candidate_summary_source=e2_world_forward",
                # COUNT-shaped, INCLUSIVE floor: `met` is exactly
                # `n_primary_div >= MIN_DIVERGENT_SEEDS`.
                #
                # This entry previously reported min(consumed_summary_pairwise_dist_mean)
                # across ALL rows against CONSUMED_SPREAD_FLOOR, which is strictly HARSHER
                # than the shipped predicate -- a majority of seeds can clear the floor
                # while the min does not -- so the indexer's AUTHORITATIVE recompute
                # wrongly flagged this sound diagnostic precondition_unmet (confirmed on
                # the shipped run: measured 0.026277 vs 0.05 with met=True).
                #
                # No spread statistic CAN reproduce `met`: the per-seed divergence boolean
                # is itself a CONJUNCTION (`consumed_spread_mean > CONSUMED_SPREAD_FLOOR
                # and consumed_dist_max < CONSUMED_MAGNITUDE_CEIL`, L1082) evaluated on
                # EACH C1 arm, and a count over a conjunction does not distribute into
                # per-leg counts. Hence the count, not a k-th-worst spread.
                "measured": float(n_primary_div),
                "threshold": float(MIN_DIVERGENT_SEEDS),
                "comparator": ">=",
                "direction": "lower",
        ...
        "criteria_non_degenerate": {
            ...
            "in_layer_null_verified_lifting": bool(noise_verified_lifting),
    ...
    manifest_core = {
        ...
        "acceptance_criteria": {
            ...
            "in_layer_null_verified_lifting": noise_verified_lifting,
        ...
        "interpretation_grid": {
            "PASS_loop_segregation_converts_limbic_loop_load_bearing_supports_arc110": (
                ...
                "variance + the in-layer null VERIFY-LIFTS + learning engaged) AND C1 (A1_LOOPS "
            ...
            "FAIL_valid_null_loops_vary_but_no_conversion_ceiling_intrinsic_weakens_arc110": (
                ...
                "ARM_NOISE verify-lifts -- and the loops carry live cross-loop variance) BUT "
                "A1_LOOPS does NOT lift committed-class entropy strict-above A0/the null. The "
                "single-arena collapse was NOT the binding constraint -> the F-dominance "
                "conversion ceiling is INTRINSIC, not an artefact of collapse -> weakens ARC-110 "
                "(the loop-segregation-as-artefact hypothesis)."
            ),
            "FAIL_substrate_not_ready_requeue": (
                ...
                "the ARC-110 what_would_answer guard), OR the S2 in-layer null did not VERIFY-LIFT "
                "above A0 (the same-layer null is still inert even with loops -> null-validity "
                "problem persists), OR learning was not engaged (finer channels not dissociable / "
                "delta_t flat). The conversion question could NOT be measured -- NOT a falsification."
            ),
        },
...
def _build_manifest(
    ...
    return {
        ...
        "evidence_direction_note": (
            f"V3-EXQ-707 ARC-110 PARALLEL SEGREGATED LOOP-SEGREGATION VALIDATION "
            f"(experiment_purpose=diagnostic; claim_ids=[ARC-110]; PROMOTES NOTHING). "
            ...
            f"load-bearing if A1 also strict-above ARM_DROP_LIMBIC); a VALID null verify-lifts + "
            ...
            f"winner), the in-layer null must verify-lift (else the null-validity problem "
            f"persists), the pool divergent (GAP-A), finer channels dissociable + delta_t non-"
        ...
        "env_kwargs": dict(ENV_KWARGS),
        "config_summary": {
            "design": "4-arm ARC-110 loop-segregation validation (A0_SINGLE_ARENA / A1_LOOPS / ARM_NOISE in-layer null / ARM_DROP_LIMBIC) + per-seed-divergent gating + live-cross-loop-variance non-degeneracy gate + verify-lifting in-layer same-layer null bar",
...
def main() -> Tuple[Optional[str], Optional[str], bool]:
    ...
    print(
        ...
        f"in_layer_null_verified_lifting={_ac['in_layer_null_verified_lifting']} "
