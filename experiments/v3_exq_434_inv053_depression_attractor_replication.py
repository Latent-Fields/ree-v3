...
"""
V3-EXQ-434 -- INV-053: Depression Attractor Replication (5-seed systematic study)

Claims: INV-053 (chronic harm-dominated environments produce a self-maintaining
depressive attractor: z_goal seeding failure, terrain collapse, and HABIT/PLANNED
behavioural equivalence).

Dispatch mode: discriminative_pair (HABIT vs PLANNED in LONG_HORIZON).

Context:
  Fourth independent 5-seed replication of INV-053 under the LONG_HORIZON parameter
  regime. Prior results:
    EXQ-249 (2026-04-07): PASS 5/5 seeds, z_goal_norm=0.019, behavioural_equivalence=0.
    EXQ-268 (2026-04-09): PASS 5/5 seeds, z_goal_norm mean=0.022, equivalence=1.0.
    EXQ-406 (2026-04-17): queued but output not yet landed.
  INV-053 promoted to stable (governance 2026-04-09-c). This replication further
  constrains the attractor's reproducibility across independent runs.

  LONG_HORIZON context: 8x8 grid, 1 resource, 3 hazards, hazard_harm=0.02, 150 steps.
  Predicts: z_harm_a tonically elevated -> z_goal seeding suppressed -> terrain
  collapsed -> PLANNED collapses to HABIT behaviourally.
...
PRE-REGISTERED ACCEPTANCE CRITERIA:
  ...
  FAIL (attractor absent): <= 1/5 seeds show A_pass
    -> does_not_support: attractor not reproducible under LONG_HORIZON parameters
...
Decision scoring:
  ...
  FAIL (<= 1/5 seeds):     does_not_support -- prior replications were outliers

Acceptance checks (from EXP-0096 / EVB-0096):
  (1) At least 2 additional runs with distinct seeds -- met: 5 distinct seeds.
  (2) Experiment Pack validates against v1 schema -- manifest includes claim_ids_tested,
      evidence_class, evidence_direction.
  (3) Result links to claim_ids_tested and updates matrix direction counts --
      claim_ids=[INV-053], evidence_direction emitted per outcome.
"""
...
# Experiment metadata
# ---------------------------------------------------------------------------
EXPERIMENT_TYPE    = "v3_exq_434_inv053_depression_attractor_replication"
CLAIM_IDS          = ["INV-053"]
EXPERIMENT_PURPOSE = "evidence"
BACKLOG_ID         = "EVB-0096"
PROPOSAL_ID        = "EXP-0096"
...
# ---------------------------------------------------------------------------
# Grid and episode parameters (LONG_HORIZON only, matches EXQ-237a/249/268/406)
# ---------------------------------------------------------------------------
GRID_SIZE        = 8
LONG_N_RESOURCES = 1
LONG_N_HAZARDS   = 3
...
# Learning rates (identical to EXQ-237a/249/268/406)
LR_E1           = 1e-3
LR_E2_WF        = 1e-3
LR_HARM         = 1e-4
LR_BENEFIT      = 1e-3
...
def _warmup(
    ...
    for ep in range(warmup_episodes):
        ...
        if (ep + 1) % 50 == 0 or ep == warmup_episodes - 1:
            ...
            print(
                f"  [train] label seed={seed} cond={condition}"
...
def _run_seed(seed: int, warmup_episodes: int, eval_episodes: int) -> Dict:
    ...
    for condition in CONDITIONS:
        ...
        print(
            f"[V3-EXQ-434] seed={seed} cond={condition}"
        ...
        # Per-run verdict (progress instrumentation: seeds x conditions = 10 verdicts)
        run_pass = (
            condition == "PLANNED"
            and warmup_res["goal_norm"] < GOAL_NORM_ATTRACTOR
        ) or (
            # HABIT passes as "ran to completion"; scientific criterion is on PLANNED
            condition == "HABIT"
        )
        print(f"verdict: {'PASS' if run_pass else 'FAIL'}", flush=True)
...
def main() -> None:
    ...
    print(
        f"[V3-EXQ-434] INV-053: Depression Attractor Replication"
    ...
    all_results = []
    for seed in seeds:
        print(f"\n[V3-EXQ-434] === seed={seed} ===", flush=True)
    ...
    print(f"\n[V3-EXQ-434] === Aggregate Results ===", flush=True)
    ...
    out_path = out_dir / f"{EXPERIMENT_TYPE}_{ts_compact}.json"

    manifest = {
        "run_id":              f"{EXPERIMENT_TYPE}_{ts_compact}_v3",
        "experiment_type":     EXPERIMENT_TYPE,
        "architecture_epoch":  "ree_hybrid_guardrails_v1",
        "claim_ids":           CLAIM_IDS,
        "claim_ids_tested":    CLAIM_IDS,
        "experiment_purpose":  EXPERIMENT_PURPOSE,
        "evidence_class":      "mechanism_probe",
        "outcome":             outcome,
    ...
    print(f"\n[V3-EXQ-434] Written: {out_path}", flush=True)
