"""
V3-EXQ-466c (EXP-0162 behavioural): SD-034 satisficing / residue discharge,
...
fired (n_closures=0). 466b was built on the committed_mode_curriculum, which trains
commitment but NOT foraging competence, AND never set subgoal_mode=True (so the
GAP-3 waypoint-completion machinery was inert). This rewires the TRAINING harness to
the scaffolded_sd054_onboarding curriculum at the 603n config (the 514n pattern;
ready=true 2026-06-11) and sets subgoal_mode=True so completions occur and closure
can fire its ResidueField.discharge_domain.

WHAT THIS MEASURES (unchanged from 466b): the ocd4 satisficing / over-checking
...
ARMS (one curriculum build per seed, two frozen-policy evals):
  ARM_CLOSURE_ON   -- full closure + bistable. Expect n_closures > 0 AND
                      discharge_events >= 1.
  ARM_CLOSURE_OFF  -- same trained weights, closure OFF. Zero closure fires, zero
                      residue discharge.
...
COMMITMENT / COMPLETION NON-VACUITY GATE: before scoring, assert the ON arm committed
...
predecessor: V3-EXQ-466b (NOT a supersede).
...
EXPERIMENT_TYPE = "v3_exq_466c_sd034_satisficing_residue_discharge_behavioural"
QUEUE_ID = "V3-EXQ-466c"
CLAIM_IDS: List[str] = ["SD-034", "MECH-094"]
EXPERIMENT_PURPOSE = "evidence"
PREDECESSOR = "V3-EXQ-466b (successor, NOT supersede)"

SEEDS = [42, 43, 44]
CONDITION_LABEL = "CURRICULUM_BUILT_SATISFICING_RESIDUE_DISCHARGE"
...
def _eval_residue_discharge(
    ...
    (ported from 466b, using _sense_with_optional_harm). Tracks closure fires,
    discharge events (closures with residue_centers_discharged >= 1), mean active-
    weight reduction, beta-elevated steps, and sequence completions (the closure
    opportunity)."""
    agent.eval()
...
def _run_seed(seed: int, dry_run: bool, total_eps: int,
    ...
    print(f"  [train] discharge_eval seed={seed} ep {done}/{total_eps}"
          ...
          f" completions={arm_on['n_sequence_completions']} beta_elev={arm_on['total_beta_elevated']}", flush=True)
            ...
            "preconditions": [
                ...
                },
                {
                    ...
                    "description": "ON arm committed (total_beta_elevated > 0) AND reached "
                                   ...
                                   "fire its residue discharge (the n_closures=0 *b-cohort gap).",
...
def main(dry_run: bool = False,
    ...
    manifest = {
        ...
        "substrate": "scaffolded_sd054_onboarding (full curriculum; 603n config; ready=true "
                     "2026-06-11) + commitment control-plane (bistable BetaGate + SD-034 "
                     "ClosureOperator + SD-033a LateralPFC + SD-032 dACC/salience) + subgoal_mode "
                     "waypoint tolerance-band completion env primitive.",
        "condition": CONDITION_LABEL,
        "predecessor": PREDECESSOR,
        "method_note": "466b's SD-034 satisficing/residue-discharge mechanism (closure ON "
                       "discharges rule-domain residue at completed-with-stable-rule_state ticks; "
                       "closure OFF accumulates residue unchecked) re-run on a foraging-competent "
                       "agent BUILT THROUGH the scaffolded_sd054_onboarding curriculum. 466b's "
                       "committed_mode_curriculum trained commitment but NOT foraging competence and "
                       "never set subgoal_mode=True, so completions never occurred (n_closures=0).",
        "readiness_note": "TWO non-vacuity gates self-route a substrate-not-engaged read to "
                          ...
                          "commitment+completion non-vacuity gate (closure had an opportunity to fire).",
        "arm_note": "ARM_CLOSURE_ON vs ARM_CLOSURE_OFF (same trained weights, closure off). The "
                    "committed_mode_curriculum O-2 forced-rv contrast arm is dropped.",
