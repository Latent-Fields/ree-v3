"""
V3-EXQ-910a -- MECH-489 (SD-099) Defensive-Orienting Response: decision-normalization
re-test, after the SD-ORIENTING-DECISION-SCALE fix.

Supersedes V3-EXQ-910 in the lettered-iteration sense (CLAUDE.md EXQ Versioning Policy:
bug fix to an existing experiment, same scientific question -> next letter), but does
NOT invalidate V3-EXQ-910's own trigger-alignment finding -- see PURPOSE below.
...
PURPOSE. V3-EXQ-910 found MECH-489's Component 4/5 (approach/withdraw/resume decision)
confounded by a scale-mismatch bug in ree_core/agent.py select_action() -- comparing a
structurally non-negative z_harm_s L2 norm directly against a near-zero-unless-close
benefit RBF value, which structurally biased every override toward "withdraw"
(206/206 logged overrides, 0 approach, 0 resume) regardless of actual event valence
(failure_autopsy_V3-EXQ-910_2026-08-10.md). That bug is now fixed
(docs/architecture/sd_orienting_decision_scale.md, ree_core/agent.py select_action()
Component 4/5) -- both channels are z-scored against their own running distribution
before comparison. This re-test asks ONLY whether that fix resolved the pathological
one-sided decision split; SD-099's own trigger/coupling mechanism (Components 1-3) is
UNCHANGED by the fix and NOT re-tested here.

Original V3-EXQ-910 pre-registered criteria, from
REE_assembly/evidence/planning/observational_review_V3-EXQ-906b_2026-08-09.md
Section 13-A / 12h / 12b, computed unchanged for context (criteria (a)/(b) below) --
the autopsy found (a) fairly falsified on its own pre-registered signature (not
re-litigated by this run) and (b)/(a) together not decisive for MECH-489's
trigger-alignment sub-claim, which stands independent of this fix:
  ...
  (c) ADDED for V3-EXQ-910a -- the decisive criterion for this re-queue: is the
      resolved approach/withdraw/resume decision NOT unconditionally one class? A
      working decision-scale fix must produce a non-degenerate mix tracking actual
      event valence, not reproduce V3-EXQ-910's 0/0/206 split.
...
# Readiness precondition for NEW criterion (c): need at least this many pooled
# Component-4/5 overrides across all ON-arm seeds for decision_alignment to be a
# meaningful measurement of the SD-ORIENTING-DECISION-SCALE fix. V3-EXQ-910 logged 206
# pooled overrides across 2 seeds, so this floor is comfortably conservative.
MIN_OVERRIDES_FOR_DECISION_READINESS = 5
...
def run(seeds: Optional[List[int]] = None, dry_run: bool = False) -> Dict[str, Any]:
    ...
    # Computed here (not inside the criteria-evaluation branch below) because its
    # n_overrides count feeds the NEW readiness precondition for criterion (c).
    decision_alignment = _decision_alignment(on_steps)

    # --- Readiness preconditions: enough pooled ground-truth events / overrides to
    ...
    preconditions.append({
        "name": "pooled_overrides_count_sufficient",
        "description": (
            f"At least {MIN_OVERRIDES_FOR_DECISION_READINESS} pooled Component-4/5 "
            "overrides across all ON-arm seeds are needed for decision_alignment "
            "(criterion (c)) to be a meaningful measurement of the "
            "SD-ORIENTING-DECISION-SCALE fix -- this is the readiness gate for the "
            "criterion this re-queue actually exists to test."
        ),
        "measured": decision_alignment["n_overrides"],
        "threshold": MIN_OVERRIDES_FOR_DECISION_READINESS,
        "direction": "lower",
        "met": decision_alignment["n_overrides"] >= MIN_OVERRIDES_FOR_DECISION_READINESS,
    })
    ...
    if not all_preconditions_met:
        ...
        # --- Pass criterion (c), ADDED for V3-EXQ-910a: decision_alignment
        # non-degeneracy -- the SD-ORIENTING-DECISION-SCALE fix's own acceptance test.
        # Before the fix, every override resolved to "withdraw" (206/206, V3-EXQ-910)
        # regardless of actual event valence -- a scale-mismatch artifact, not a
        # measurement. A working fix must not reproduce that one-sided collapse.
        dec_counts = decision_alignment["decision_counts"]
        dec_n_overrides = decision_alignment["n_overrides"]
        decision_not_collapsed = (
            dec_n_overrides > 0
            and sum(1 for v in dec_counts.values() if v > 0) >= 2
        )
        criteria.append({
            "name": "C_decision_alignment_non_degenerate",
            "load_bearing": True,
            "passed": bool(decision_not_collapsed),
            "decision_counts": dec_counts,
            "n_overrides": dec_n_overrides,
            "note": (
                "PASSES if the resolved decision is NOT unconditionally one class "
                "(V3-EXQ-910's bug signature: 0 approach / 0 resume / 206 withdraw)."
            ),
        })
        ...
        criteria_non_degenerate = {
            c["name"]: (
                ...
                if c["name"].startswith("C_alignment_")
                    ...
                    (dec_n_overrides > 0)
                    if c["name"] == "C_decision_alignment_non_degenerate"
    ...
    summary_markdown = (
        f"# V3-EXQ-910a -- MECH-489 (SD-099) Defensive-Orienting Decision Retest\n\n"
        ...
        f"## Pass criterion (c) [NEW, decisive for this re-queue]: decision_alignment non-degenerate\n"
        f"See interpretation.decision_alignment in the manifest.\n\n"
        f"## Outcome: {outcome}\n"
    )
