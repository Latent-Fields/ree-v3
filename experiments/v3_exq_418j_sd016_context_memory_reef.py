...
"""
V3-EXQ-418j -- SD-016 env-entropy reef fix: 2x2 factorial {SD-016, reef}
...
EXPERIMENT_TYPE    = "v3_exq_418j_sd016_context_memory_reef"
CLAIM_IDS          = ["SD-016"]
EXPERIMENT_PURPOSE = "evidence"
...
def _run_one_arm_seed(
    ...
    combined_z = torch.cat([reef_z, food_z], dim=0).to(device)
    if combined_z.shape[0] == 0:
        combined_z = torch.zeros(4, 32, device=device)
