...
"""
V3-EXQ-725 -- Coherence non-reducibility RETEST on the LEARNED-BINDER substrate.
(reuses the V3-EXQ-641a / V3-EXQ-720 harness; does NOT supersede either)

WHAT CHANGED vs 720 (the ONLY substrate change): the agent is now built with
cross_stream_binding_learned=True (still enabled, strength=0.5) -- the LEARNED
(plastic) mode of the cross_stream_binding_substrate. 720 ran the fixed random
projection field (learned=False) and lifted coherence-specificity from 1/6 (641a,
unbound) to 3/6 seeds but did NOT clear the 4/6 SPEC gate, and n_rebind stayed 0.
failure_autopsy_V3-EXQ-720_2026-07-09 (confirmed) named the residual prerequisite:
a fixed random projection creates correlation but nothing SHAPES the coupling so
that real cross-stream conjunctions are robustly more selection-informative than a
contrast-matched shuffle. The learned binder trains its projections phi_self /
phi_world by contrastive co-encoding (InfoNCE over within-tick observed pairs vs
in-batch shuffle) so genuine conjunctions bind and a shuffle collapses -- the
load-bearing biology divergence (binding-by-synchrony is LEARNED, not a fixed
field). THIS run asks the same question against the plastic substrate: with a
LEARNED binder, does real cross-stream coherence now carry selection information
non-reducible to E and destroyed by a shuffle, AND is the rebinding falsifier
exercisable (n_rebind>0)?
...
EXPERIMENT_TYPE = "v3_exq_725_coherence_nonreducibility_learned_binder"
QUEUE_ID = "V3-EXQ-725"
SUPERSEDES = None  # does NOT supersede 641a/720; retest on the learned-binder substrate
CLAIM_IDS: List[str] = []  # diagnostic -- bears on INV-002/MECH-089/MECH-094/MECH-270/MECH-269, tags NONE

# cross_stream_binding_substrate activation. LEARNED (plastic) mode is the ONLY
# substrate change vs 720. Pre-registered: strength 0.5 carried from 720 (not
# tuned to a pass). See docs/architecture/sd_cross_stream_binding_substrate.md.
CROSS_STREAM_BINDING_ENABLED = True
CROSS_STREAM_BINDING_LEARNED = True   # <- the ONLY substrate change vs 720
...
def _interpret(rows: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    binder_trained = bool(binding_active and all_binder_learned and min_binder_steps > 0)
    ...
    preconditions = [
        ...
        {
            "name": "learned_binder_trained",
            "kind": "readiness",
            "description": (
                "the P0 binder curriculum actually fired: min n_learn_steps across "
                "primary seeds clears a floor > 0. An untrained binder is a "
                "random-projection field (720), not a learned-binder verdict -> "
                "route substrate_not_ready_requeue."
            ),
            "measured": int(min_binder_steps),
            "threshold": 1,
            "control": "InfoNCE learn-step count from the P0 curriculum",
            "met": bool(binder_trained),
        },
        {
    ...
    criteria_non_degenerate = {
        ...
        # AND the binder can express rebinding (learned+trained).
        "rebinding_exercised": bool(by_seed_perturb and binder_trained),
    }

    spec_passed = bool(n_seed_pass >= MIN_SEEDS_FOR_PASS)
    if not binding_active or not binder_trained:
        # Misconfigured / untrained: the LEARNED binder was not engaged+trained ->
        # not-ready, NOT a substrate verdict. (The route the readiness rule mandates.)
    ...
    return {
        ...
        "learned_binder_trained": bool(binder_trained),
