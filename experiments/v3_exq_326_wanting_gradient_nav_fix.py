...
"""
V3-EXQ-326: Wanting-Gradient Navigation (fix of EXQ-259)

experiment_purpose: evidence

Tests SD-015 (z_resource encoder enables goal-directed navigation),
MECH-216 (E1 predictive wanting), SD-012 (homeostatic drive modulation).

Root cause of EXQ-259 FAIL:
  1. schema_wanting_enabled was False (default) -- E1 never produced schema_salience,
     so update_schema_wanting() was a no-op and VALENCE_WANTING was never seeded.
  2. The experiment used update_benefit_salience() (which needs serotonin enabled)
     instead of update_z_goal() + update_schema_wanting() combination.

Fix:
  - Enable schema_wanting_enabled=True in E1Config
  - Enable use_resource_proximity_head=True (SD-018) so z_world encodes resource prox
  - Enable z_goal_enabled=True + drive_weight=2.0 (SD-012) for wanting via goal path
  - Use resource_respawn_on_consume=True and hazard_harm=0.1 (mild) so agent explores
  - Phased training: P0 = encoder + proximity head warmup (no goal update);
    P1 = full pipeline with z_goal and schema_wanting active; P2 = evaluation
...
Both conditions use identical env + training structure (phased).
...
Pass criteria:
  ...
  C3: benefit_ratio WITH_WANTING >= 1.3x (paper gate primary, >= 2/3 seeds)
      where benefit_ratio = mean_benefit_exposure / baseline_benefit_exposure
      (baseline = ABLATED condition mean_benefit_exposure)
...
# Experiment metadata
# ---------------------------------------------------------------------------
EXPERIMENT_TYPE    = "v3_exq_326_wanting_gradient_nav_fix"
CLAIM_IDS          = ["SD-015", "MECH-216", "SD-012"]
EXPERIMENT_PURPOSE = "evidence"
...
DRIVE_WEIGHT   = 2.0   # SD-012 benefit amplification
...
def run_condition(
    ...
    for ep in range(total_eps):
        ...
        for step in range(steps_per):
            ...
            # SD-012 / MECH-216: wanting updates (P1 and P2 only, WITH_WANTING only)
            benefit_raw = float(obs_dict.get("body_state", torch.zeros(12))[11].item()) \
...
def main():
    ...
    run_id = (
        f"v3_exq_326_wanting_gradient_nav_fix_dry_{ts}_v3"
        if args.dry_run
        else f"v3_exq_326_wanting_gradient_nav_fix_{ts}_v3"
    )
    print(f"EXQ-326 start: {run_id}")
    ...
    output = {
        ...
        "evidence_direction_per_claim": {
            ...
            "MECH-216": "supports" if criteria["c1_wanting_populated"] else "does_not_support",
