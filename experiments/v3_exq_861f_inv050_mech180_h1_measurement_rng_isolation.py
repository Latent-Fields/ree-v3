"""
H1 leg (GOV-FANOUT-1, measurement axis): is the V3-EXQ-861e seed-271 HIGH-arm MEL collapse an intervention-isolation defect? Reseed the measurement phase, substrate pinned to 861e's own 17befb8c
...
Hypothesis under test: H1   Design axis: measurement
...
failure_autopsy_V3-EXQ-861e_2026-08-21 withdrew "same seeds isolate the calibration change" as a
...
FANOUT_HYPOTHESIS = "H1"
FANOUT_AXIS = "measurement"
FANOUT_SOURCE_AUTOPSY = "failure_autopsy_V3-EXQ-861e_2026-08-21"
PRIMARY_VARIANT = "reseeded"
CONTROL_VARIANT = "unreseeded"
...
def _control_reproduces_collapse(cell: Dict[str, Any]) -> bool:
    """THE SHIPPED PREDICATE for this leg's readiness anchor. Used both by
    _discrimination() on the live control cell and by the setup-time
    reachability guard on 861e's frozen recorded cell -- one definition, never
    a copy, which is the whole point of the guard."""
    return float(cell.get("mean_duration_factor", 0.0)) <= FACTOR_GRADED_FLOOR


_ANCHOR_REACHABILITY = [assert_anchor_reachable(
    anchor_name="in_run_unreseeded_control_reproduces_861e_collapse",
    reference_cells=[PREDECESSOR_DECISIVE["861e"]],
    score_fn=_control_reproduces_collapse,
    threshold=1.0,
    reference_source=("V3-EXQ-861e recorded seed 271 / ARM_3_HIGH_ON cell "
                      "(factor 0.8845) -- the exact state this leg's control "
                      "must reproduce"),
)]
...
def _discrimination(arm_results: List[Dict[str, Any]],
    ...
    control_reproduces_861e = (c is not None and _control_reproduces_collapse(c))
