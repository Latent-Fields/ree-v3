"""
V3-EXQ-916 -- Relief/Safety Fishtank Showcase
...
Output:
  evidence/experiments/v3_exq_916_relief_safety_fishtank_showcase/
    v3_exq_916_relief_safety_fishtank_showcase_<ts>.json          (manifest)
    v3_exq_916_relief_safety_fishtank_showcase_<ts>_episode_log.json  (fishtank feed)
...
EXPERIMENT_TYPE    = "v3_exq_916_relief_safety_fishtank_showcase"
EXPERIMENT_PURPOSE = "diagnostic"
CLAIM_IDS          = []
...
def run_seed(seed: int, dry_run: bool = False) -> Dict:
    ...
    print(f"\nSeed {seed} Condition relief_safety_showcase", flush=True)
    print(
        f"[EXQ-916] Seed {seed}  warmup={warmup_eps}  eval={eval_eps}"
    ...
    print(
        f"[EXQ-916] Seed {seed} -- world_obs_dim={env.world_obs_dim}"
    ...
    print(
        f"[EXQ-916] Seed {seed} channel std: "
    ...
    print(
        f"[EXQ-916] Seed {seed} freeze_fires={ree['freeze_fires']}"
...
def run(seeds=None, dry_run: bool = False) -> dict:
    ...
    print(
        f"[V3-EXQ-916] Relief/Safety Fishtank Showcase\n"
        f"  Seeds: {seeds}\n"
        ...
        f"  Relief/safety (NEW): MECH-302 MECH-303 MECH-304\n"
    ...
    interpretation = {
        "label": "relief_safety_showcase_channels_live" if passed
                 else "relief_safety_showcase_degenerate_channels",
        "criteria_non_degenerate": {
            **{f"channel_{k}": chan_nondegen.get(k, False) for k in chan_keys},
            ...
            "relief_fires_present": bool(total_relief > 0),
        },
        "criteria": [
        ...
        "note": ("Telemetry showcase, follow-on to V3-EXQ-664. PASS = the core affect "
                 "channels (z_harm_a, z_harm_un, drive) vary across eval, the blocked-"
                 "agency pole engages, and PAG freeze (if it fires) does not permanently "
                 "lock the agent. No claim_ids; does not weight governance. excite/dread/"
                 "override AND relief_event/safety_cue_signal/safety_terrain_read (the "
                 "MECH-302/303/304 channels this experiment newly enables) are reported "
                 "but not load-bearing -- MECH-302 needs a genuine sustained z_harm_a "
                 "descent and MECH-304 needs a prior MECH-302 firing to seed its EMA "
                 "prototype, so either can be legitimately sparse within a bounded eval "
                 "window even when correctly wired."),
    }
    ...
    for r in seed_results:
        rows += (
            ...
            f" | {r['freeze_fires']} | {r['block_steps']} | {r['relief_fires']} |\n"
        )

    summary_markdown = f"""# V3-EXQ-916 -- Relief/Safety Fishtank Showcase
...
(override), MECH-279 (PAG freeze, capped at {PAG_MAX_FREEZE}), MECH-353 (blocked agency,
...
- relief fires (total, MECH-302): {total_relief} / {total_steps} eval steps
