...
"""V3-EXQ-824a -- Q-081: does REE's multi-rate execution produce SHARED
cross-stream organisation (Outcome A), or only the configured rate
separation and wired gates it was built with (Outcome B)?

Same-question redesign of V3-EXQ-824 (alphabetic suffix -- the scientific
question, Outcome A vs B, is unchanged; only the implementation was wrong).
`supersedes` below points at the FAIL run this corrects. Full source-trace
diagnosis: REE_assembly/evidence/planning/failure_autopsy_V3-EXQ-824_2026-07-26.md.

WHAT WAS WRONG WITH 824, AND WHAT THIS FIXES
---------------------------------------------
824's config relied on `q081_profile_kwargs()` alone, which sets
`use_invalidation_trigger=True` but NOT `use_anchor_sets`. Per source trace
(`ree_core/agent.py:4479-4514`), `use_invalidation_trigger` alone only feeds
the MECH-287 broadcast queue, which is drained solely by a tick-boundary
bounding flush (`ree_core/agent.py:4323-4338`) that discards it -- no
consumer in that config reads the queue for any purpose that touches
`z_world`/`operating_mode`. `assert_behavioural_reach()` nonetheless reported
`has_behavioural_reach=True` (it treated any one of four flags as sufficient),
so the run proceeded and produced BIT-IDENTICAL `rv_primary` between the ON
and LANDMARK_REMOVED arms at all 5 seeds -- the signature of the manipulation
never reaching the measured pair, not evidence of true landmark-invariance
(Outcome B). This redesign fixes BOTH the precondition check (library fix,
landed alongside this script: `experiments/_lib/q081_landmark_removal.py`'s
`assert_behavioural_reach` now requires `use_anchor_sets` and/or
`use_per_region_vs` specifically -- `REACH_CONSUMERS`) and this script's own
config (`cfg.hippocampal.use_anchor_sets = True`, mirroring the already-proven
fix in `v3_exq_827_inv091_cross_stream_similarity_band.py:230`). A cheap
sanity check (`rv_bit_identical` per seed, folded into `adjudicate()`'s
preconditions) now catches a recurrence of the exact 824 defect immediately
rather than letting it run to a false Outcome-B verdict.

This is the FIRST corrected Q-081 run. All three preconditions the claim's
...
supersedes: v3_exq_824_q081_shared_organisation_landmark_removal_20260726T165630Z_v3
(V3-EXQ-824, FAIL, non_contributory per failure autopsy -- inert landmark
arm, not a real Outcome B; see module docstring above).
ASCII-only output.
"""
...
EXPERIMENT_TYPE = "v3_exq_824a_q081_shared_organisation_landmark_removal"
QUEUE_ID = "V3-EXQ-824a"
CLAIM_IDS: List[str] = ["Q-081"]
EXPERIMENT_PURPOSE = "evidence"
SUPERSEDES_RUN: Optional[str] = (
    "v3_exq_824_q081_shared_organisation_landmark_removal_20260726T165630Z_v3"
)
...
def make_agent(env: CausalGridWorldV2) -> REEAgent:
    ...
    # THE 824 FIX. q081_profile_kwargs() alone (use_invalidation_trigger=True)
    # gives the landmark scrambler NO reach to z_world/operating_mode -- see
    # module docstring. use_anchor_sets=True gives it real reach via MECH-269
    # write_anchor, mirroring the already-proven fix in
    # v3_exq_827_inv091_cross_stream_similarity_band.py:230.
...
def run_seed(seed: int) -> Dict[str, Any]:
    ...
    # strict=True (824 used strict=False): use_anchor_sets=True in make_agent()
    # gives real reach, so a failure here should stop the run loudly rather
    # than silently produce another vacuous 824-shaped result.
    ...
    return {
        ...
        # V3-EXQ-824 sanity check: bit-for-bit float64 equality between the two
        # independently-simulated arms is the tell that the manipulation never
        # reached rv_primary (confirmed root cause of 824's false Outcome B),
        # not evidence of true landmark-invariance. See adjudicate().
        "rv_bit_identical": bool(analysis_on["rv_primary"] == analysis_rm["rv_primary"]),
...
def adjudicate(per_seed: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for r in per_seed:
        ...
        valid_bit_identical.append(bool(r["rv_bit_identical"]))

    n_valid = len(valid_seeds)
    n_valid_bit_identical = sum(1 for b in valid_bit_identical if b)
    # V3-EXQ-824 sanity check: ALL valid seeds bit-identical is the confirmed
    # signature of the manipulation never reaching rv_primary, not a real
    # Outcome B. A partial pattern is not this defect and is not gated here.
    ...
    preconditions = [
        {
            ...
            "description": "assert_behavioural_reach (tightened post-V3-EXQ-824): the "
                           "boundary stream must reach (z_world, operating_mode) "
                           "specifically, via use_anchor_sets and/or use_per_region_vs "
                           "(REACH_CONSUMERS) -- use_invalidation_trigger alone no longer "
                           "counts, since it only feeds a queue drained by a bounding "
                           "flush with no causal reach.",
            "measured": 1.0 if reach_ok else 0.0, "threshold": 1.0, "direction": "lower",
            "control": "make_agent() sets cfg.hippocampal.use_anchor_sets=True "
                       "(mirrors V3-EXQ-827 _build_cfg)",
            "met": reach_ok,
        },
        {
            "name": "arm_statistics_not_degenerately_bit_identical",
            "description": "V3-EXQ-824 sanity check (failure_autopsy_V3-EXQ-824_2026-07-26): "
                           "rv_primary must NOT be bit-identical between ON and REMOVED at "
                           "every valid seed. Exact float64 equality across independently-"
                           "simulated seeds is the confirmed signature of the manipulation "
                           "never reaching the measured statistic, not evidence of true "
                           "landmark-invariance -- this is the check that would have caught "
                           "824's defect immediately.",
            "measured": float(n_valid - n_valid_bit_identical), "threshold": 1.0,
            "direction": "lower",
            "control": "use_anchor_sets=True gives real reach to z_world",
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="V3-EXQ-824a Q-081 cross-stream shared organisation: ON vs landmark-removal")
