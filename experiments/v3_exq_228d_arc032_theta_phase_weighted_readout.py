...
"""
V3-EXQ-228d -- ARC-032: Theta-Rate Pathway PHASE-WEIGHTED-READOUT retest,
measured at E3-tick granularity on the scaffolded_sd054_onboarding curriculum
(substrate-fix continuation of the V3-EXQ-228/228a/228b/228c lineage).

Claims: ARC-032
EXPERIMENT_PURPOSE = "evidence"
Supersedes: V3-EXQ-228c

=== WHY THIS REDESIGN (228c -> 228d) ===

V3-EXQ-228c (2026-08-10) was the first FAIR, E3-tick-restricted, direct-DV
test of ARC-032 (both 228b-identified measurement defects fixed -- see below).
It failed both primary criteria with a REVERSED trend: theta ACTIVE was
NOISIER than theta bypassed (noise_delta_mean=-0.0671) and showed no
persistence benefit (persist_delta_mean=4.47e-07). The confirmed
failure_autopsy (failure_autopsy_V3-EXQ-228c_2026-08-10) diagnosed this as a
BIOLOGY-DIVERGENCE IMPLEMENTATION GAP, not a claim falsification:
ThetaBuffer.summary()'s flat, unweighted mean is permutation-invariant over
the buffered window, which cannot represent the within-cycle phase/order
structure Dragoi & Buzsaki 2006 (theta-sequence compression) and Colgin et al.
2016 (theta-gamma phase nesting) attribute theta's functional payload to --
converging with MECH-089's own independently-confirmed EXQ-066/EXQ-122
findings that static/uniform theta-averaging measurably hurts E3's
fine-grained discrimination. Routing: SD-100, a phase/sequence-order-aware
ThetaBuffer summary (docs/architecture/sd_100_theta_buffer_phase_aware_summary.md),
landed 2026-08-10 (ree-v3 commit 22b7a3bcff) -- IMPLEMENTED per ree-v3/CLAUDE.md.
claims.yaml ARC-032 carries `pending_retest_after_substrate: true` naming
exactly this re-test.

228d changes ONLY the theta-summary mechanism used by the THETA_ACTIVE arm:
SD-100's forward-sweep phase-weighted summary
(`config.heartbeat.use_theta_phase_weighted_summary=True`,
`theta_phase_concentration=4.0`, the validated default kappa) replaces the old
flat-mean summary. Everything else -- ablation semantics, shared-training
architecture, measurement protocol, thresholds, seeds -- is unchanged from
228c, which was itself the first fair test of the claim's own pre-registered
DVs. This is a substrate fix landing on a proven measurement design, not a
new measurement redesign.

=== BACKGROUND: WHY 228c'S MEASUREMENT DESIGN IS TRUSTED (228b -> 228c) ===
...
for two measurement-adequacy reasons 228c fixed and 228d retains:
...
228c/228d keep 228b's PROVEN substrate + training path byte-identical (the
only lineage member to clear the precondition) and score the claim's OWN
pre-registered readouts directly, restricted to the E3-tick steps where the
ablation actually acts.

=== ABLATION (unchanged from 228a/228b/228c -- pre-registered; THETA_ACTIVE's
    summary MECHANISM changes 228c -> 228d, the ablation semantics do not) ===
...
proposer + goal appraisals -- this monkey-patch bypasses ThetaBuffer.summary()
entirely regardless of the SD-100 flag, so THETA_ZEROED is bit-identical in
mechanism to 228c. THETA_ACTIVE now reaches SD-100's phase-weighted summary()
(the config flag is set on the shared trained agent before the deepcopy split,
so both clones start with it True; only THETA_ZEROED's monkey-patch overrides
it).

=== ARCHITECTURE (unchanged from 228b/228c -- shared training, deepcopy split) ===
...
=== MEASUREMENT PROTOCOL (unchanged from 228c -- the 228b->228c redesign) ===
...
PRIMARY readouts (claims.yaml's pre-registered CONFIRMING DVs -- autopsy (b)):
  ...
  C2  goal-PROXIMITY-SCORE NOISE. On each E3-tick step we compute the
      ...
      primary because theta's function IS temporal smoothing (228d: a
      recency-weighted forward-sweep phase kernel over the fixed 10-step
      window, SD-100 -- not the flat mean 228c used); the CROSS-CANDIDATE
      dispersion (prox_noise_xcand,
...
NOTE ON THRESHOLDS: PERSIST_MARGIN / NOISE_MARGIN are UNCHANGED from 228c --
the same pre-registered thresholds, chosen ahead of the first direct
measurement of these DVs, apply to this substrate-fixed retest (GOV-REUSE-1
checked: 228c's readouts were measured under the OLD flat-mean summary and are
not substitutable for a phase-weighted-summary result -- different
substrate_hash, not recoverable by reanalysis). The primary scientific value
is the DIRECTION on the claim's own named readouts plus the direct-readout
record itself. If 228d again shows a CONSISTENT REVERSED direction (theta
ACTIVE worse -- less persistent and/or noisier) even under the phase-weighted
summary, that is a STRONGER finding than 228c's: it would mean the reversal is
not attributable to the flat-mean's permutation-invariance specifically (the
228c autopsy's diagnosed cause), and ARC-032's implementation-gap routing
would need re-examination rather than a further substrate iteration -- flag
this explicitly for governance rather than silently re-routing to another
`/implement-substrate` cycle.
...
(severity: degrading, status: implemented_pending_validation -- the untrained
reality_scorer/harm_cost_fallback_scorer subnetworks are now gated out of the
E3 trajectory score by default, but the fix is not yet independently
validated) touches ree_core/predictors/e3_selector.py, which this driver's
select_action path exercises. Recorded here per the substrate-path-overlap
gate so a later autopsy sees the run happened under a known (degrading,
non-corrupting, largely-mitigated) limitation.
"""
...
# Experiment metadata
# --------------------------------------------------------------------- #
EXPERIMENT_TYPE = "v3_exq_228d_arc032_theta_phase_weighted_readout"
QUEUE_ID = "V3-EXQ-228d"
SUPERSEDES = "V3-EXQ-228c"
ARCHITECTURE_EPOCH = "ree_hybrid_guardrails_v1"
CLAIM_IDS = ["ARC-032"]
EXPERIMENT_PURPOSE = "evidence"

# SD-100 (docs/architecture/sd_100_theta_buffer_phase_aware_summary.md):
# THETA_ACTIVE's summary() mechanism, 228c's flat mean -> 228d's phase-weighted
# forward-sweep kernel. theta_phase_concentration is SD-100's validated default
# kappa (unweighted-to-phase-weighted degrades gracefully at kappa=0; 4.0 is
# the design doc's chosen operating point, not separately tuned here).
USE_THETA_PHASE_WEIGHTED_SUMMARY = True
THETA_PHASE_CONCENTRATION = 4.0
...
def build_agent_config(env) -> REEConfig:
    ...
    clears the z_goal precondition on this curriculum), PLUS SD-100's
    phase-weighted ThetaBuffer summary switch (228c -> 228d's only config
    change). ARC-032 needs the goal/wanting channel live; the theta-ablation
    manipulation is post-training (the deepcopy + monkey-patch below), not in
    REEConfig -- but the SD-100 flag must be set here, on the ONE trained
    agent, before the deepcopy split, so THETA_ACTIVE's clone inherits
    use_theta_phase_weighted_summary=True on its ThetaBuffer (constructed at
    REEAgent.__init__ from config.heartbeat.*, see ree_core/agent.py). The
    THETA_ZEROED clone's theta_buffer.summary is monkey-patched to return
    zeros unconditionally (_patch_theta_zeroed below), which overrides this
    flag entirely for that clone -- so THETA_ZEROED's ablation semantics are
    bit-identical to 228c regardless of this config value."""
    cfg = REEConfig.from_dims(
        ...
        use_theta_phase_weighted_summary=USE_THETA_PHASE_WEIGHTED_SUMMARY,
        theta_phase_concentration=THETA_PHASE_CONCENTRATION,
    )
...
def run(dry_run: bool = False) -> Tuple[Dict[str, Any], ZGoalStreamAccumulator]:
    print(
        f"\n[{QUEUE_ID}] ARC-032 Theta-Rate Pathway PHASE-WEIGHTED-READOUT test"
        f" (SD-100 phase-weighted summary on THETA_ACTIVE, E3-tick-restricted"
        f" persistence + proximity-noise, supersedes {SUPERSEDES})"
        f" dry_run={dry_run}",
        flush=True,
    )
    ...
    if not precond_pass:
        ...
        degeneracy_reason = (
            ...
            f"{gn_frac:.2f} of seeds -- unexpected regression from 228b/228c, "
            "both of which cleared it 1.00 on this identical shared-training "
            "recipe. The theta pathway could not be tested: no live "
        ...
        interpretation = (
            ...
            "context signal. (228b/228c both cleared this precondition 1.00 on "
            "the same training recipe; a failure here indicates seed/substrate "
            "drift, not an ARC-032 result.)"
        )
        ...
        if reversed_trend:
            interpretation = base + (
                ...
                f"on {rev_persist_frac:.2f}, noisier on {rev_noise_frac:.2f}) -- "
                "EVEN WITH SD-100's phase-weighted, order-sensitive summary in "
                "place of the flat mean 228c used. This is a STRONGER finding than "
                "228c's reversal: it is no longer attributable to the flat mean's "
                "permutation-invariance (the 228c autopsy's diagnosed cause), since "
                "228d's THETA_ACTIVE arm does not use a flat mean. Route: flag for "
                "governance re-examination of ARC-032's implementation-gap disposition "
                "(rather than a further /implement-substrate iteration) -- either the "
                "SD-100 kernel's parameterisation (theta_phase_concentration) is itself "
                "inadequate, or the reversal is not a ThetaBuffer-packaging artifact at "
                "all and the claim-level frontal-hippocampal-synchrony hypothesis "
                "should be reconsidered against this substrate."
            )
        else:
            interpretation = base + (
    ...
    summary_markdown = (
        f"# {QUEUE_ID} -- ARC-032 Theta-Rate Pathway PHASE-WEIGHTED-READOUT test\n\n"
