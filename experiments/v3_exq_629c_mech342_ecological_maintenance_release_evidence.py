...
"""
V3-EXQ-629c: MECH-342 maintenance-time commitment-release -- ECOLOGICAL evidence run.
SUPERSEDES V3-EXQ-629b (same scientific question; PRECONDITION fix, not a redesign).

WHY 629c EXISTS (the one thing that changed):
  629b (2026-06-12) FAILed non_contributory / contact_guard_unmet: the upstream 603n
  foraging contact guard held on only 1/3 seeds (seed 42 contact_rate 0.0; seed 44
  z_goal_at_contact_peak 0.371 < 0.4). The failure was UPSTREAM foraging incompetence,
  NOT a MECH-342 defect -- on the ONE competent seed (43) MECH-342 fired correctly
  (decommit_transitions=34, mech342_fires=1, beta occupancy 1.0 -> 0.217 in the degraded
  window vs 1.0 in ARM_0 OFF). The stated resume condition was: retest once the
  scaffolded_sd054_onboarding nav-competence leg clears on >= 2/3 seeds.

  That condition is now MET, and the cause of 629b's miss is identified. Across ~30
  sibling runs on the IDENTICAL curriculum -- same 603n budgets, same TRAIN_STEPS=200,
  same seeds 42/43/44, same run_p2 guard readout -- the contact guard clears on 3/3
  seeds essentially always, from 2026-06-12T21:33 (V3-EXQ-461c, nine hours AFTER 629b)
  through 2026-07-09 (V3-EXQ-721, 6/6 seeds). Checked: 461c, 468d, 460d, 467d, 464d,
  460e, 460h, 460i/j/k/l, 460m/n/o/p, 466e, 715, 715a, 717 (12/12 seeds), 721 (6/6).
  Seed 42 lands 0.16-0.35 contact_rate in every one of them; 629b is the sole 0.0.

  The discriminating difference is NOT the curriculum, the budget, or the seed: it is
  that 629b built its curriculum agent under the full ECOLOGICAL MEASUREMENT config.
  use_mech090_readiness_conjunction, heartbeat.use_commit_readiness_gate, and
  e3.urgency_interrupt_threshold=1e6 were all live for the entire Stage-0 -> Stage-0b
  -> P0 -> Stage-H -> P1 -> P2 build. NONE of the passing siblings carries ANY of those
  flags during the build (verified by grep across all four families). 629b is the
  unique carrier and the unique guard failure.

  629c's fix is exactly that and nothing else: build the curriculum agent under the
  CLEAN 603n config (_make_curriculum_config), then apply the ecological overlay at
  CLONE time (_apply_ecological_overlay), where the measurement actually needs it.
  This is sound because MECH-090's CommitReadiness and MECH-342's release regulator are
  both pure-arithmetic and carry NO learned parameters (ree_core/policy/commit_readiness.py:
  "Pure-arithmetic regulator ... No learned parameters, no nn.Module"), so a state_dict
  trained under the clean config loads STRICTLY into the overlaid config. The clone-time
  load asserts strict=True and treats failure as fatal -- if the overlay ever stops being
  parameter-free, the run must stop rather than silently drop weights.

NON-VACUITY (629c strengthens this): if the contact guard still holds on < 2/3 seeds,
  the run self-routes substrate_not_ready_requeue / non_contributory with the guard
  recorded as an UNMET precondition, and reports NOTHING as MECH-342 evidence. A repeat
  of 629b's upstream failure therefore reads as precondition-unmet, never as a MECH-342
...
commitment-inducing curriculum + wire the inert degradation driver." 629b took the
commitment-inducing-curriculum branch (629c inherits it): the foraging-competent agent is BUILT THROUGH
...
EXPERIMENT_PURPOSE = "evidence"
PREDECESSOR = "V3-EXQ-629b (SUPERSEDED by this run; 629 and the 592g diagnostic also stand)"
SUPERSEDES = "V3-EXQ-629b"
...
def _make_curriculum_config(env) -> REEConfig:
    """The CLEAN 603n-validated foraging substrate (mirror 514n / 461c / 468e / 460o)
    -- and NOTHING ELSE. This is the 629c fix.

    629b built its curriculum agent under the full ECOLOGICAL config: the MECH-090
    R-c commit-entry conjunction, the heartbeat commit-readiness gate, and
    urgency_interrupt_threshold=URGENCY_DISABLE were all active for the entire
    Stage-0 -> Stage-0b -> P0 -> hazard -> P1 -> P2 curriculum. None of the ~30
    sibling runs that pass the same contact guard on the same curriculum, at the
    same budgets and the same seeds, carry ANY of those four flags during the
    build (checked: 461c, 468e, 460l, 460o, 721). 629b is the unique carrier and
    the unique guard failure -- seed 42 at contact_rate 0.0 where every sibling
    seed 42 lands 0.16-0.35.

    So the overlay is moved OUT of the build and applied at CLONE time
    (_apply_ecological_overlay, below), where it belongs: MECH-090 and MECH-342
    are both pure-arithmetic regulators with no learned parameters
    (ree_core/policy/commit_readiness.py: "no learned parameters, no nn.Module"),
    so a state_dict trained under the clean config loads into the overlaid config
    without any shape change. The curriculum therefore reproduces the substrate
    the guard was validated on, and the ecological measurement still runs under
    the full R-c conjunction it needs."""
    ...
    # NOTE (629c): no MECH-090 conjunction, no commit-readiness gate, no urgency
    # override here. That is the whole point -- see the docstring above.
    return cfg


def _apply_ecological_overlay(cfg: REEConfig, use_maintenance_release: bool) -> REEConfig:
    """Apply the ecological MEASUREMENT overlay to an already-trained agent's config.

    Everything 629b carried through the curriculum, applied instead at clone time:
      - MECH-090 R-c commit-entry conjunction (identical across BOTH arms), plus the
        heartbeat commit-readiness gate at the ecological score-margin floor.
      - MECH-091 held inert via urgency_interrupt_threshold = URGENCY_DISABLE
        (the distinct-from control; the manifest still asserts max_z_harm_a_norm
        below it).
      - MECH-342 maintenance release -- the ARM AXIS, the only field that differs
        between ARM_0 and ARM_1.

    All of these are pure-arithmetic / gate-threshold settings: none adds, removes
    or reshapes a learned parameter, so the clean-built state_dict loads STRICTLY
    (asserted in _clone_with_overlay -- no silent non-strict fallback, which would
    mask exactly the shape drift this claim depends on not happening)."""
    cfg.use_mech090_readiness_conjunction = True
    cfg.mech090_readiness_floor = NAV_COMPETENCE_FLOOR
    cfg.commit_readiness_initial = 1.0
    # REEConfig.__post_init__ auto-arms use_commit_readiness from the conjunction
    # master flag when built via from_dims; set it explicitly here because we are
    # mutating an already-constructed config rather than re-running from_dims.
    cfg.use_commit_readiness = True
    cfg.use_maintenance_release = bool(use_maintenance_release)
    cfg.maintenance_release_score_margin_floor = MR_SCORE_MARGIN_FLOOR
    cfg.maintenance_release_score_margin_reengage = MR_SCORE_MARGIN_REENGAGE
    cfg.maintenance_release_nav_floor = MR_NAV_FLOOR
    cfg.maintenance_release_nav_reengage = MR_NAV_REENGAGE
    cfg.maintenance_release_accumulation_rate = MR_ACCUMULATION_RATE
    cfg.maintenance_release_leak_rate = MR_LEAK_RATE
    cfg.maintenance_release_bound = MR_RELEASE_BOUND
    cfg.maintenance_release_pressure_cap = MR_PRESSURE_CAP
...
def _build_p2_env(scaffold_cfg: ScaffoldedSD054OnboardingConfig, degraded: bool,
...
def _clone_with_overlay(trained_agent: REEAgent, use_maintenance_release: bool,
                        device: torch.device) -> REEAgent:
    """Clone the CLEAN-BUILT trained weights into an agent carrying the full
    ecological measurement overlay, with use_maintenance_release as the arm axis.

    629c change: the overlay (MECH-090 conjunction + commit-readiness gate +
    urgency-disable) is applied HERE rather than during the curriculum. Both arms
    receive an IDENTICAL overlay; only use_maintenance_release differs, so the arms
    stay bit-identical in weights and differ in exactly one flag."""
    cfg = _apply_ecological_overlay(copy.deepcopy(trained_agent.config),
                                    use_maintenance_release)
    agent = REEAgent(cfg).to(device)
    state = {k: v.detach().clone() for k, v in trained_agent.state_dict().items()}
    # STRICT, and a failure here is FATAL (629c). 629b fell back to strict=False on
    # RuntimeError, which would silently drop any parameter whose shape moved -- and
    # the whole design rests on the two arms sharing bit-identical weights. A shape
    # change means the overlay is NOT parameter-free after all, which invalidates the
    # clone-time-overlay premise and must stop the run rather than be papered over.
...
def _build_curriculum_agent(scaffold_cfg, device, seed, dry_run, total_eps,
    ...
    # either. idx is the construction order within this driver (0 = this
    # world_obs_dim parity probe env, 1 = HEALTHY window, 2 = DEGRADED window).
    probe_env = _build_p2_env(
        scaffold_cfg, degraded=False,
        seed=_derive_env_seed(seed_env_base, stream=2, idx=0),
    )
    probe_env.reset()
    agent = REEAgent(_make_curriculum_config(probe_env)).to(device)
...
def _run_seed(seed: int, dry_run: bool, total_eps: int,
    ...
    # curriculum envs and this driver's own P2 envs on OS entropy, exactly as in
    # the landed run. When the knob is set, the run seed is folded in so the
    ...
    # Built once and SHARED by both arms, as in the landed run: the arms are
    # compared WITHIN a common pair of worlds, so they take one idx each here
    # (1 = HEALTHY, 2 = DEGRADED) rather than one idx per arm.
    healthy_env = _build_p2_env(
        scaffold_cfg, degraded=False,
        seed=_derive_env_seed(seed_env_base, stream=2, idx=1),
    )
    degraded_env = _build_p2_env(
        scaffold_cfg, degraded=True,
        seed=_derive_env_seed(seed_env_base, stream=2, idx=2),
    )
    for a in ARMS:
        print(f"Seed {seed} Condition {a['name']}", flush=True)
        arm_agent = _clone_with_overlay(agent, a["use_maintenance_release"], device)
...
def run_experiment(dry_run: bool = False,
    ...
    # 629c non-vacuity hardening: when the run is non_contributory because an
    # UPSTREAM precondition failed (contact guard / C1 commits / C2 degradation),
    # exclude it from governance confidence + conflict scoring outright rather than
    # relying on evidence_direction alone. A repeat of the 629b upstream miss then
    # reads as precondition-unmet and contributes NOTHING to MECH-342 in either
    # direction. Only an explicit False excludes; absent/True is a no-op.
    out: Dict[str, Any] = {
        "outcome": outcome,
        "evidence_direction": evidence_direction,
        "evidence_direction_per_claim": {"MECH-342": evidence_direction},
        ...
        "interpretation": {
            ...
            "preconditions": [
                {"name": "foraging_contact_guard",
                 ...
                 "measured": guard_frac, "threshold": MIN_FRACTION,
                ...
                {"name": "baseline_commits_C1",
                 ...
                 "measured": 1.0 if c1 else 0.0, "threshold": 1.0,
                 "direction": "lower", "comparator": ">=", "met": bool(c1)},
                {"name": "degradation_occurred_C2",
                 ...
                 "measured": 1.0 if c2 else 0.0, "threshold": 1.0,
                 "direction": "lower", "comparator": ">=", "met": bool(c2)},
            ],
            "criteria": [
    ...
    if not crit_non_degenerate:
        out["non_degenerate"] = False
        out["non_degenerate_per_claim"] = {"MECH-342": False}
        out["degeneracy_reason"] = (
            f"upstream precondition unmet ({route_reason}): contact_guard_frac="
            f"{guard_frac:.3f} (floor {MIN_FRACTION:.3f}), C1_baseline_commits={bool(c1)}, "
            f"C2_degradation_occurred={bool(c2)}. MECH-342 release authority was never "
            f"exercised under a valid harness, so this run is excluded from MECH-342 "
            f"confidence/conflict scoring rather than counted as evidence."
        )
    return out
...
def main(dry_run: bool = False,
    ...
    manifest = {
        ...
        "substrate": "scaffolded_sd054_onboarding (full curriculum; 603n config; ready=true "
                     "2026-06-11) built under the CLEAN 603n config, then overlaid at clone "
                     "time with MECH-090 R-c commit-entry conjunction (identical across arms) "
                     "+ MECH-342 maintenance release (arm axis) + SD-047 multi_source_dynamics "
                     "degraded window. harm stream ON (curriculum requires); MECH-091 held "
                     "inert via urgency_interrupt_threshold=URGENCY_DISABLE.",
        "condition": CONDITION_LABEL,
        "predecessor": PREDECESSOR,
        "supersedes": SUPERSEDES,
        # None = the landed run's OS-entropy env seeding (both the scheduler's
        # curriculum envs and this driver's own P2 envs). An int here means the
        ...
        "precondition_fix_note": "629c differs from 629b in ONE respect: the ecological "
                                 "measurement overlay (MECH-090 conjunction, heartbeat "
                                 "commit-readiness gate, urgency_interrupt_threshold=1e6) is "
                                 "applied at CLONE time instead of being carried through the "
                                 "curriculum build. 629b was the sole run in its family to "
                                 "build under that overlay and the sole one to miss the 603n "
                                 "contact guard (1/3 seeds); ~30 sibling runs on the identical "
                                 "curriculum/budgets/seeds clear it on 3/3 from 2026-06-12T21:33 "
                                 "(461c) through 2026-07-09 (721, 6/6). MECH-090 and MECH-342 "
                                 "are both parameter-free regulators, so the clean-built "
                                 "state_dict loads strict=True into the overlaid config.",
...
if __name__ == "__main__":
    ...
    ap.add_argument(
        ...
        help="Opt-in env-seed base. Omitted (the default) reproduces the landed "
             ...
             "builds -- the scheduler's curriculum envs AND this driver's own P2 "
