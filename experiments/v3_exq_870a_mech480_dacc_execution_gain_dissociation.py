...
"""V3-EXQ-870a -- MECH-480 dACC-analog execution-gain dissociation (EXP-0413).

Supersedes V3-EXQ-870 (ERROR, crash-before-manifest). Diagnosed via
/diagnose-errors 2026-08-02.

ROOT CAUSE (870): RuntimeError "probability tensor contains either `inf`,
`nan` or element < 0" at ree_core/predictors/e3_selector.py:3498
(torch.multinomial), reached from agent.select_action -> e3.select, during
Seed 42 Condition ON, P1 training ep 20/90 (traceback captured from the
ree-cloud-4 systemd journal -- the crash predates any manifest write, so no
run pack carries it; see journalctl -u ree-runner --since "2026-08-02
10:30:00" --until "2026-08-02 11:00:00" on ree-cloud-4 for the full trace).

870 carried DACC_WEIGHT=3.0 / DACC_PRECISION_SCALE=500.0 from v3_exq_450's
calibration but did NOT set dacc_bias_max_abs (defaults to 0.0 = unclamped;
see DACCConfig.dacc_bias_max_abs / DACCtoE3Adapter.forward in
ree_core/cingulate/dacc.py). Unclamped, dacc_weight >= 1.0 is a POSITIVE
FEEDBACK loop, not just a magnitude-tuning knob: DACCtoE3Adapter.forward's
`mode_ev` term is fed from `payoffs = -self.e3.last_scores`, and
`self.e3.last_scores` is the PREVIOUS tick's *already-biased* composed score
(dacc_score_bias is added into `scores` before `self.last_scores =
scores.detach()` is set, e3_selector.py ~line 3078). So bias_N is
approximately dacc_weight * bias_{N-1} in the dominant-term limit: with
dacc_weight=3.0 that is unconditionally divergent (gain > 1), and 870 hit it
at ep 20 of P1. v3_exq_450 used the identical unclamped weight/precision
pair and PASSED (2026-04-20), which is NOT evidence the config is safe --
only that the run-length/seed combination there did not surface the
divergence before P1 ended. The established, precedented mitigation (used at
lower dacc_weight in v3_exq_445g/445h/597/597b/799, all of which needed a
clamp specifically because unclamped bias either degenerated E3's
discrimination or, as here, diverged) is DACCConfig.dacc_bias_max_abs -- it
truncates the bias EVERY tick, which breaks the exponential feedback
regardless of dacc_weight. This experiment adds DACC_BIAS_MAX_ABS=2.0 (the
445g/445h/597 precedent value -- "the range where E3 softmax can actually
discriminate candidates", per 445g's docstring: natural inter-candidate
score variation is O(0.5-2.0)), leaving DACC_WEIGHT/DACC_PRECISION_SCALE and
every other constant unchanged from 870, so the calibration intent (enough
gain to shift E3's argmax) is preserved -- only the runaway tail is cut.

Everything below this point is UNCHANGED from V3-EXQ-870 except: (a) the
dacc_bias_max_abs clamp above, (b) EXPERIMENT_TYPE/QUEUE_ID/SUPERSEDES, and
(c) a preflight assertion that the clamp actually reached the agent's dACC
adapter config (so a future REEConfig.from_dims signature change that
silently swallows the kwarg -- see ree-v3/CLAUDE.md/from_dims memory note --
fails loud here instead of re-diverging silently).
...
E3's argmax distribution in a small-seed budget -- now paired with the
dacc_bias_max_abs clamp per the root-cause note above).
...
EXPERIMENT_TYPE = "v3_exq_870a_mech480_dacc_execution_gain_dissociation"
QUEUE_ID = "V3-EXQ-870a"
SUPERSEDES = "v3_exq_870_mech480_dacc_execution_gain_dissociation"
...
# FIX (870 -> 870a, see module docstring root-cause note): unclamped +
# dacc_weight >= 1.0 is a positive-feedback loop over successive ticks
# (mode_ev is fed from the PREVIOUS tick's already-biased score), diverging
# to inf/nan in the E3 softmax. Value matches the v3_exq_445g/445h/597
# precedent -- "the range where E3 softmax can actually discriminate
# candidates" -- which truncates the bias every tick and so cannot diverge
# regardless of dacc_weight, while leaving DACC_WEIGHT/DACC_PRECISION_SCALE
# (and therefore the calibrated effect size) unchanged from 870.
...
def _preflight() -> None:
    ...
    # FIX regression guard (870 -> 870a): confirms the clamp actually reached
    # the dACC adapter's config, not just REEConfig -- from_dims has a known
    # failure mode where a kwarg not also added to __init__'s signature is
    # silently swallowed (see ree-v3/CLAUDE.md from_dims memory note). If
    # this ever fires, the runaway-bias crash 870 hit is unguarded again.
    assert on.dacc_adapter is not None
    assert abs(float(on.dacc_adapter.config.dacc_bias_max_abs) - DACC_BIAS_MAX_ABS) < 1e-9
    ...
    print(
        ...
        "dacc_bias_max_abs clamp reached the adapter, last_scores/select "
        "present, resource relocation confirmed in both regions with "
        "non-empty proximity field",
        flush=True,
    )
    ...
    for seed in seeds:
        ...
        for cond in CONDITIONS:
            ...
            config_slice = {
                ...
                "dacc_bias_max_abs": DACC_BIAS_MAX_ABS if cond == "ON" else 0.0,
