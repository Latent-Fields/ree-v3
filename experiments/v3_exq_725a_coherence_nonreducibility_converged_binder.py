...
"""
V3-EXQ-725a -- Coherence non-reducibility RETEST on the REPAIRED (converging)
LEARNED-BINDER substrate. Supersedes V3-EXQ-725 (an untrained-substrate artifact).
(reuses the V3-EXQ-641a / V3-EXQ-720 harness; does NOT supersede 641a/720)

WHY THIS SUPERSEDES 725: V3-EXQ-725 ran the SAME harness with the learned binder
but the binder NEVER CONVERGED -- its InfoNCE loss stayed at chance (log(64)=4.16;
observed 3.75-3.96) because the observed (z_self,z_world) latents are near-collinear
buffer-wide (cos ~0.99) and the un-normalized dot-product logit is magnitude-
dominated. SPEC regressed 3/6 (720 fixed field) -> 0/6, an UNTRAINED-substrate
artifact, NOT a coherence verdict (failure_autopsy_V3-EXQ-725_2026-07-09, confirmed).
The substrate was REPAIRED 2026-07-09 (ree-v3 b86fa3e, /implement-substrate):
L2-normalize phi_self/phi_world before the InfoNCE dot (cosine InfoNCE, SimCLR-
standard) -> loss drops to 0.65-0.80 of chance across seeds; the substrate now
exposes binder_converged = loss_ema < conv_frac*log(batch).

WHAT CHANGED vs 725 (this run):
  (1) cross_stream_binding_temperature=0.2 (725 used the default 0.5; 0.2 deepens
      the convergence margin to ~0.65 of chance).
  (2) HARD CONVERGENCE GATE. The vacuous learned_binder_trained precondition
      (n_learn_steps>1 -- "the P0 curriculum FIRED") that green-lit the untrained
      binder in 725 is REPLACED by a hard learned_binder_converged precondition:
      the substrate's binder_converged (loss_ema < 0.85*log(batch)) must be True on
      every primary seed. If the binder does NOT converge, the run self-routes
      substrate_not_ready_requeue (-> /failure-autopsy, an environment-adequacy
      question), NEVER a coherence verdict. A naive 725b that reran the same
      non-converging curriculum is REFUSED by the 725 autopsy.

THIS run asks the same question against the CONVERGED plastic substrate: with a
learned binder that actually trained, does real cross-stream coherence now carry
selection information non-reducible to E and destroyed by a shuffle, AND is the
rebinding falsifier exercisable (n_rebind>0)?

PHASED TRAINING (mandatory, satisfied here): the learned binder requires a P0
binder curriculum. During P0 warmup episodes ONLY, each step calls
agent.update_cross_stream_binder(z_self, z_world) for both paired agents (one
contrastive InfoNCE optimizer step on DETACHED observed pairs -- no gradient into
E1/E2 encoders). In P1 the binder is FROZEN (the update is not called; agent.eval()
is already set) and the 641a measurement runs against the trained-then-frozen
binder. This is the P0->P1 discipline; joint-training collapse is avoided by
training the binder P0-only.

SUBSTRATE-LEVEL REBINDING PROBE (folded in, per the 720 autopsy): n_rebind is now
read through agent.cross_stream_binder.rebinding_probe -- does a competing
world-config OVERTAKE the currently-bound one under an ANCHOR (z_self) perturbation
-- NOT the incidental selection-flip 720 used. A fixed field cannot express this
(binding_score is undiscriminating -> n_rebind=0 across 641/641a/720); the learned
binder makes the binding intake's own falsifier exercisable AT the substrate.

strength=0.5 is PRE-REGISTERED (carried from 720, not tuned to a pass). The
harness, seeds, conditions, reads, control, gate and thresholds are otherwise
IDENTICAL to 641a/720. Everything below this paragraph is inherited from that
harness.
...
EXPERIMENT_TYPE = "v3_exq_725a_coherence_nonreducibility_converged_binder"
QUEUE_ID = "V3-EXQ-725a"
# Supersedes the untrained-binder artifact 725 (run on the pre-repair substrate).
SUPERSEDES = "v3_exq_725_coherence_nonreducibility_learned_binder"
CLAIM_IDS: List[str] = []  # diagnostic -- bears on INV-002/MECH-089/MECH-094/MECH-270/MECH-269, tags NONE

# cross_stream_binding_substrate activation. LEARNED (plastic) mode + REPAIRED
# convergence (cosine InfoNCE, ree-v3 b86fa3e). Pre-registered: strength 0.5
# carried from 720/725; temperature 0.2 deepens the convergence margin (725 used
# the default 0.5). See docs/architecture/sd_cross_stream_binding_substrate.md.
CROSS_STREAM_BINDING_ENABLED = True
CROSS_STREAM_BINDING_LEARNED = True
CROSS_STREAM_BINDING_STRENGTH = 0.5
CROSS_STREAM_BINDING_TEMPERATURE = 0.2   # <- change vs 725 (was default 0.5)
# HARD convergence gate. The substrate reports binder_converged = loss_ema <
# CONV_FRAC*log(batch); this retest gates readiness on it (replaces 725's vacuous
# n_learn_steps>1 check). Must match E2Config.cross_stream_binding_conv_frac.
...
def _make_agent(env: CausalGridWorldV2) -> REEAgent:
    ...
    cfg = REEConfig.from_dims(
        ...
        # cross_stream_binding_substrate LEARNED mode: the ONLY change vs 720.
        # Plastic phi_self/phi_world trained by contrastive co-encoding (P0
        # curriculum below) so genuine cross-stream conjunctions bind and a
        # shuffle collapses -- coherence non-reducible to E.
        ...
        # Repaired convergence: cosine InfoNCE + temperature 0.2 for margin;
        # conv_frac drives the substrate's binder_converged stat.
...
def _candidate_world_rep(cand) -> Optional[torch.Tensor]:
    """Representative z_world config for a candidate = terminal rollout world_state.
    The binding affinity of a candidate is read at this (z_self-anchor, z_world)
    conjunction. Returns None if the candidate has no usable world stream.
    """
    seq = getattr(cand, "world_states", None)
    if seq is None or len(seq) == 0:
        return None
    return seq[-1].detach().reshape(1, -1)


def _substrate_rebinding(
    agent: REEAgent,
    z_self_anchor: torch.Tensor,
    cands,
    magnitude: float,
    gen: torch.Generator,
) -> Optional[bool]:
    """Substrate-level rebinding probe (folded in per failure_autopsy_V3-EXQ-720).

    Reads n_rebind through agent.cross_stream_binder.rebinding_probe: does a
    competing world-config OVERTAKE the currently-bound one under an ANCHOR
    (z_self) perturbation? Anchor (not per-candidate) perturbation is load-bearing
    -- binding_score is bilinear, so a uniform candidate perturbation shifts every
    score by a candidate-independent constant and can NEVER flip the argmax; an
    anchor perturbation gives a per-candidate shift that can. Returns None when the
    binder is absent / fixed-mode / the pool is degenerate (a fixed field cannot
    rebind, which is exactly why n_rebind stayed 0 across 641/641a/720).
    """
    binder = getattr(agent, "cross_stream_binder", None)
    if binder is None or not getattr(binder, "learned", False):
        return None
    reps = [_candidate_world_rep(c) for c in cands]
    reps = [r for r in reps if r is not None]
    if len(reps) < 2:
        return None
    zs = z_self_anchor.detach().reshape(1, -1)
    rms = float(zs.norm().item()) / max(zs.numel(), 1) ** 0.5
    pert = torch.randn(zs.shape[-1], generator=gen, device=zs.device) * (magnitude * rms)
    probe = binder.rebinding_probe(zs, reps, perturbation=pert)
    return bool(probe.get("rebound", False))
...
def _decision(
    ...
    # P0 binder curriculum: train the LEARNED cross-stream binder ONLY during P0
    # warmup on the observed (waking) joint state; FREEZE in P1 (this call is
    # skipped) so the 641a measurement reads a trained-then-frozen binder. No-op
    # in fixed mode / when the substrate is OFF. Inputs detached inside the binder
    # -> no gradient leaks into E1/E2 encoders.
    if not is_p1:
    ...
    if arm == "B" and is_p1:
        ...
        # Substrate-level rebinding read (folded in): does a competing world-config
        # overtake under an anchor z_self perturbation? This is the n_rebind source
        # (replaces 720's incidental selection-flip). None when perturb==0 or the
        # binder cannot express rebinding.
        substrate_rebound = None
        if effective_perturb > 0.0:
            substrate_rebound = _substrate_rebinding(
                agent, latent.z_self, exec_candidates, effective_perturb, perturb_gen
            )
        diag = {
            "energies": energies,
            ...
            "pick_a": pick_a,
            "pick_b": pick,
            "clean_pick_b": clean_pick_b,   # 720 selection-flip (secondary/context)
            "substrate_rebound": substrate_rebound,  # PASS-relevant rebinding read
...
def _run_pair(
    ...
    n_rebind = 0                 # SUBSTRATE-probe rebinding (PASS-relevant)
    n_rebind_at_tie = 0
    n_rebind_selection_flip = 0  # 720 selection-flip read (secondary/context)
    contacts_a = 0
    contacts_b = 0
                ...
                if diag_b is not None:
                    ...
                    for g in AUTHORITY_GAIN_SWEEP:
                        if _select_pref_weighted(energies, prefs, g) != pick_a:
                            gain_flip[f"{g:g}"] += 1
                    if perturb > 0.0:
                        # PASS-relevant n_rebind = the SUBSTRATE probe (competing
                        # config overtakes under anchor perturbation). Folded in
                        # per failure_autopsy_V3-EXQ-720; replaces the incidental
                        # selection-flip (kept as n_rebind_selection_flip context).
                        if diag_b.get("substrate_rebound"):
                            n_rebind += 1
                            if diag_b.get("is_tie_tick"):
                                n_rebind_at_tie += 1
                        if (
                            diag_b.get("clean_pick_b") is not None
                            and diag_b["clean_pick_b"] != pick_b
                        ):
                            n_rebind_selection_flip += 1
    ...
    # Binder training telemetry (readiness): both paired agents train identically
    # in P0; agent_b is representative. n_learn_steps > 0 confirms the P0 binder
    # curriculum actually fired (the learned-binder readiness precondition).
    binder_b = getattr(agent_b, "cross_stream_binder", None)
    binder_learned = bool(binder_b is not None and getattr(binder_b, "learned", False))
    binder_learn_steps = int(getattr(binder_b, "n_learn_steps", 0)) if binder_b else 0
    binder_last_loss = getattr(binder_b, "last_loss", None) if binder_b else None
    # Convergence telemetry (the HARD readiness gate this retest routes on). The
    # substrate exposes loss_ema (EMA-smoothed InfoNCE loss), chance_floor
    # (log(effective_batch)), and binder_converged (loss_ema < conv_frac*chance).
    ...
    return {
        ...
        "n_rebind_under_perturb": int(n_rebind),          # SUBSTRATE probe (PASS-relevant)
        "n_rebind_at_tie_tick": int(n_rebind_at_tie),
        "n_rebind_selection_flip": int(n_rebind_selection_flip),  # 720 read (context)
        "binder_learned": binder_learned,
        "binder_learn_steps": binder_learn_steps,
        "binder_last_loss": binder_last_loss,
...
def _interpret(rows: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    # Rebinding sub-signal. UNLIKE 720, this is now PASS-relevant (read through the
    # substrate rebinding_probe): PASS = >=4/6 SPEC AND n_rebind>0.
    n_rebind_seeds = sum(
        1 for r in by_seed_perturb.values()
        if r["n_rebind_under_perturb"] > 0
    )
    n_rebind_total = sum(
        int(r["n_rebind_under_perturb"]) for r in by_seed_perturb.values()
    )
    rebinding_exercised = bool(n_rebind_total > 0)
    ...
    # --- Diagnostic adjudication: readiness preconditions -------------------
    # READINESS (load-bearing for THIS retest): the LEARNED cross_stream_binding
    # substrate must be ACTIVE (enabled + strength > 0) AND actually TRAINED (the
    # P0 binder curriculum fired: n_learn_steps > 0). If the binder is OFF / fixed
    # / untrained, the run does not test the learned-binder hypothesis and must NOT
    # be read as a substrate verdict -- it is a not-ready run.
    binding_active = (
        bool(CROSS_STREAM_BINDING_ENABLED)
        and bool(CROSS_STREAM_BINDING_LEARNED)
        and (CROSS_STREAM_BINDING_STRENGTH > 0.0)
    )
    # Binder actually trained: min n_learn_steps across primary seeds must clear a
    # floor > 0 (the P0 curriculum ran). Read the same statistic the substrate
    # depends on -- learn-step count, not a proxy.
    min_binder_steps = (
        min(int(pr.get("binder_learn_steps", 0)) for pr in by_seed_primary.values())
        if by_seed_primary else 0
    )
    all_binder_learned = bool(
        by_seed_primary
        and all(bool(pr.get("binder_learned", False)) for pr in by_seed_primary.values())
    )
    # HARD CONVERGENCE GATE (replaces 725's vacuous n_learn_steps>1 check). Every
    # primary seed's binder must report binder_converged = loss_ema <
    # conv_frac*log(batch). This is the load-bearing readiness: an unconverged
    # binder is the 725 untrained-substrate artifact, NOT a coherence verdict.
    all_binder_converged = bool(
        by_seed_primary
        and all(bool(pr.get("binder_converged", False)) for pr in by_seed_primary.values())
    )
    # Worst (largest) loss_ema / chance across primary seeds, for the precondition
    # numeric read. Fall back to +inf loss / 0 chance if unavailable (-> not met).
    _emas = [pr.get("binder_loss_ema") for pr in by_seed_primary.values()]
    _emas = [e for e in _emas if e is not None]
    max_loss_ema = float(max(_emas)) if _emas else float("inf")
    _chances = [pr.get("binder_chance_floor") for pr in by_seed_primary.values()]
    _chances = [c for c in _chances if c is not None]
    min_chance_floor = float(min(_chances)) if _chances else 0.0
    conv_threshold = float(CROSS_STREAM_BINDING_CONV_FRAC) * min_chance_floor
    # Readiness for the retest = active learned binder AND every seed converged.
    binder_ready = bool(binding_active and all_binder_learned and all_binder_converged)
    # Gating adequacy (measurement-diversity readiness): the SPEC/D1 denominator
    # (two-mode-active P1 steps) must be non-degenerate on the primary seeds.
    min_gated = (
        min(pr["n_p1_steps_gated"] for pr in by_seed_primary.values())
        if by_seed_primary else 0
    )
    preconditions = [
        {
            "name": "learned_binder_active",
            "kind": "readiness",
            "description": (
                "cross_stream_binding_substrate enabled in LEARNED mode with "
                "strength > 0 so the plastic binder is engaged; OFF/fixed collapses "
                "this retest to 720/641a and is not a learned-binder verdict."
            ),
            "measured": float(CROSS_STREAM_BINDING_STRENGTH if binding_active else 0.0),
            "threshold": 0.0,
            "control": "config activation of the learned binder (enabled+learned)",
        ...
        },
        {
            ...
            "kind": "readiness",
            "description": (
                "HARD convergence gate (replaces 725's vacuous n_learn_steps>1 "
                "check). Every primary seed's binder must converge: loss_ema < "
                "conv_frac*log(batch). The routed statistic is the WORST (max) "
                "loss_ema across primary seeds vs the convergence threshold "
                "(upper-bound: met when measured <= threshold). An unconverged "
                "binder is the 725 untrained-substrate artifact (InfoNCE at "
                "chance), NOT a learned-binder verdict -> route "
                "substrate_not_ready_requeue -> /failure-autopsy "
                "(environment-adequacy question), never a coherence verdict."
            ),
            # Upper-bound precondition: met when measured (worst loss_ema) <=
            # threshold. Unavailable loss_ema -> large sentinel so the indexer's
            # recompute reads NOT-met (never a false convergence pass).
            "measured": (max_loss_ema if max_loss_ema != float("inf") else 1e9),
            "threshold": float(conv_threshold),
            "direction": "upper",
            "control": (
                "substrate binder_converged (loss_ema<conv_frac*chance) on the "
                "trained-then-frozen P0 binder; smoke-confirmed True on the real "
                "725 latent stream (loss_ema 3.33 < gate 3.54 at default temp)"
            ),
            "met": bool(binder_ready),
        },
        {
            "name": "gating_denominator_adequate",
            "kind": "readiness",
            "description": (
                "min two-mode-active P1 steps across primary seeds clears the "
                "adequacy floor so gated D1/SPEC are not fractions over an empty "
                "denominator (same SP-CEM-diversity caveat as 641a)."
            ),
            "measured": int(min_gated),
            "threshold": int(MIN_GATED_STEPS_FOR_ADEQUACY),
            "control": "two-mode-active step count on the primary condition",
            "met": bool(min_gated >= MIN_GATED_STEPS_FOR_ADEQUACY),
        },
    ]
    # criteria_non_degenerate: did each pre-registered criterion have the
    # structure to fire, or would it pass/fail vacuously?
    any_gated = any(pr["n_p1_steps_gated"] > 0 for pr in by_seed_primary.values())
    d3_measurable = any(
        pr["abs_spearman_corr_e_rawc"] is not None for pr in by_seed_primary.values()
    )
    spec_matched = any(s in by_seed_control for s in by_seed_primary)
    criteria_non_degenerate = {
        "D1_behavioural_divergence_gated": bool(any_gated),
        "D3_c_nonreducible": bool(d3_measurable),
        "SPEC_coherence_specific": bool(spec_matched and any_gated),
        # rebinding is non-degenerate only if the perturb condition actually ran
        # AND the binder can express rebinding (learned + CONVERGED).
        "rebinding_exercised": bool(by_seed_perturb and binder_ready),
    }

    spec_passed = bool(n_seed_pass >= MIN_SEEDS_FOR_PASS)
    if not binder_ready:
        # Misconfigured / unconverged: the LEARNED binder was not engaged, or did
        # NOT converge on every primary seed -> not-ready, NOT a substrate verdict.
        # (The route the hard-convergence readiness rule mandates; the 725 lesson.)
        label = "substrate_not_ready_requeue"
    elif not by_seed_primary:
        label = "no_clean_completion"
    elif spec_passed and rebinding_exercised:
        # PASS = >=4/6 SPEC AND n_rebind>0 (both axes exercised on the learned binder).
        label = "C_nonreducible_coherence_specific_register"
    elif spec_passed and not rebinding_exercised:
        # SPEC clears but the rebinding falsifier was never exercised (n_rebind=0):
        # divergence axis routes positive but the binding intake stays OPEN on the
        # rebinding axis -> route /failure-autopsy, do NOT register.
        label = "C_specific_but_rebinding_unexercised_route_followup"
    ...
    return {
        ...
        "criteria": [
            {"name": "learned_binder_converged", "load_bearing": True,
             "passed": bool(binder_ready)},
            {"name": "SPEC_coherence_specific", "load_bearing": True,
             "passed": spec_passed},
            {"name": "rebinding_exercised", "load_bearing": True,
             "passed": bool(rebinding_exercised)},
        ],
        ...
        "learned_binder_converged": bool(binder_ready),
        "all_binder_converged": bool(all_binder_converged),
        "max_binder_loss_ema": (max_loss_ema if max_loss_ema != float("inf") else None),
        "binder_conv_threshold": float(conv_threshold),
        "learned_binder_trained": bool(all_binder_learned and min_binder_steps > 0),
        "min_binder_learn_steps": int(min_binder_steps),
        ...
        "n_rebind_total": int(n_rebind_total),
        "rebinding_exercised": bool(rebinding_exercised),
...
def run_experiment(
    ...
    return {
        ...
        "selection": {
            ...
            "cross_stream_binding_learned": bool(CROSS_STREAM_BINDING_LEARNED),
            "cross_stream_binding_strength": float(CROSS_STREAM_BINDING_STRENGTH),
            "rebinding_read": "substrate_rebinding_probe_anchor_perturbation",
        },
...
def _build_manifest(
    ...
    review_caveats = [
        ...
        "REBINDING (n_rebind_under_perturb, read through the SUBSTRATE "
        "rebinding_probe on an anchor z_self perturbation -- NOT 720's incidental "
        "selection-flip) is now PASS-RELEVANT for this learned-binder retest: "
        "PASS = >=4/6 SPEC AND n_rebind>0. A fixed field could not express "
        "rebinding (n_rebind=0 across 641/641a/720); the learned binder makes the "
        "binding intake's falsifier exercisable at the substrate. If SPEC clears "
        "but n_rebind==0 the run self-routes "
        "C_specific_but_rebinding_unexercised_route_followup (do NOT register; "
        "the binding intake stays OPEN on the rebinding axis).",
    ]
    ...
    return {
        ...
        "evidence_direction_note": (
            ...
            "confidence or conflict scoring. RETEST of the V3-EXQ-641a/720 harness "
            "on the LEARNED-binder cross_stream_binding_substrate "
            "(cross_stream_binding_learned=True, strength=0.5) -- does NOT supersede "
            "641a/720 (their findings stand; failure_autopsy_V3-EXQ-720_2026-07-09 "
            "routed the learned binder as the residual prerequisite). The ONLY "
            "change vs 720 is that the shared-factor projections are now PLASTIC, "
            "trained by a P0 contrastive co-encoding curriculum, so genuine "
            "cross-stream conjunctions bind and a shuffle collapses; n_rebind is "
            "read through the substrate rebinding_probe. Settles "
            "thought_intake_2026-04-23_binding.md + "
            "_path_integral_constraints_search.md (both OPEN/gated on the "
            "learned-binder substrate): C_nonreducible_coherence_specific_register "
            "(>=4/6 SPEC AND n_rebind>0) converts the candidate Q "
            "entities/selection.coherence_nonreducibility to a registerable claim + "
            "closes both intakes positive; F1/F2 close both intakes; "
            "specificity_unproven / rebinding_unexercised / "
            "substrate_not_ready_requeue route a /failure-autopsy before any "
            "registration. majority_label="
            f"{result.get('interpretation', {}).get('majority_label')}."
        ),
        "bears_on_not_tagged": [
            "INV-002", "MECH-089", "MECH-094", "MECH-270", "MECH-269",
        ],
