...
"""V3-EXQ-893 -- MECH-232 DA-modulated representational expansion (SCORING-ELIGIBLE confirmer).

Claim: MECH-232 (hippocampus.da_representational_expansion)
Purpose: EVIDENCE (experiment_purpose="evidence"; scoring-eligible -- a PASS contributes
  genuine experimental confidence for MECH-232, a FAIL weakens it).

WHY THIS EXISTS (GOV-CONFIRM-1)
------------------------------
MECH-232 is provisional with literature_confidence 0.779 but genuine_exp_count == 0 in
claim_evidence.v1.json: the only run that read its representation signature -- the SD-024
diagnostic V3-EXQ-766a -- was experiment_purpose="diagnostic", so the indexer marks it
scoring_excluded="diagnostic_probe" and it contributes ZERO scoring-eligible experimental
evidence (build_experiment_indexes.py line ~2701). GOV-CONFIRM-1 (the generative
evidence-confirmer detector) surfaces exactly this class -- a candidate/provisional claim
with built substrate + thin/zero exp evidence whose confirming DV is a representation-level /
functional signature, NOT a competence-wall-bound committed behaviour. This run is that
confirmer: it re-reads the SAME pre-registered representation signature 766 read, as a
scoring-eligible EVIDENCE run, on INDEPENDENT seeds (8-15 vs 766's 0-7) so the readout is a
genuine replication whose value is recorded in no existing manifest (GOV-REUSE-1: the
diagnostic readout at seeds 0-7 is recorded but scoring-excluded; a reanalysis artifact
would NOT move claim_evidence scoring -- the indexer does not ingest reanalysis/ -- so a
scoring-eligible run is required to close the gap).

WALL-INDEPENDENCE
-----------------
Every DV here reads a STRUCTURAL / FUNCTIONAL signature of the benefit terrain, never an
agent's committed behaviour in an episode. No grid-world, no episodes, no reward collection,
no F-dominated selection -- so this confirmer is immune to the competence-conversion wall
that gates behavioural DVs. (Precedent: GOV-CONFIRM-1 seed confirmers 757/758/759, all
experiment_purpose="evidence".)

WHAT THIS TESTS (identical pre-registered design to V3-EXQ-766)
--------------------------------------------------------------
MECH-232's falsifiable prediction: dopamine at reward-associated locations produces
representational EXPANSION (higher information density / finer resolution / more stable place
fields) in the hippocampal benefit terrain, and this expansion -- NOT an explicit
positive-valence gradient -- is what lets approach valence enter hippocampal terrain.

SD-024 is the instrument: DA-modulated accumulate_benefit allocates a local CLUSTER of RBF
centers (representational expansion) with the total benefit intensity SPLIT across the
cluster, so the summed benefit VALUE at the reward location is CONSERVED while the
representational DENSITY rises. compute_benefit_density reads that density WEIGHT-INDEPENDENTLY.
...
LEG 2 -- CRUX: approach emerges from representational QUALITY alone, not a valence gradient.
  A density-following hill-climber (reads ONLY compute_benefit_density, never the RBF weights)
  ...
  compute_benefit_density is WEIGHT-INDEPENDENT, so the density field is UNCHANGED and the
...
ACCEPTANCE (PASS -> evidence_direction "supports"): L1a AND L2a AND L2b AND L2c.
FAIL (-> "weakens") otherwise. A below-floor readiness reading self-routes to
substrate_not_ready_requeue (evidence_direction "non_contributory", non_degenerate=false) --
NEVER a substrate verdict -- exactly as the workset item instructs for the substrate-not-ready
case.

No training occurs (the benefit terrain is a non-parametric RBF accumulator). Phased training
N/A. MECH-094: DA expansion inherits accumulate_benefit's hypothesis_tag gate (not exercised
here -- all accumulation is waking).
"""
...
EXPERIMENT_TYPE = "v3_exq_893_mech232_da_representational_expansion_confirmer"
EXPERIMENT_PURPOSE = "evidence"
ARCHITECTURE_EPOCH = "ree_hybrid_guardrails_v1"
CLAIM_IDS = ["MECH-232"]

# ---- fixed design constants (pre-registered; identical to V3-EXQ-766) ----
WORLD_DIM = 16
...
# ---- walker (leg-2 functional-signature instrument) ----
N_STARTS = 24
...
# INDEPENDENT of V3-EXQ-766's [0..7] -- genuine replication, readout recorded nowhere.
DEFAULT_SEEDS = [8, 9, 10, 11, 12, 13, 14, 15]
ARMS = ["da_off", "da_on"]
EPS = 1e-8
...
def run_seed(seed: int) -> Dict:
    ...
    # LEG 1 -- representational expansion signature
    ...
    # compute_benefit_density is weight-independent -> density field UNCHANGED (approach
...
def evaluate(per_seed: List[Dict]) -> Dict:
    ...
    # ---- route ----
    if not ready:
        # substrate not ready -> non-scoring, per the workset item's explicit fallback.
        label = "substrate_not_ready_requeue"
        outcome = "FAIL"
        direction = "non_contributory"
        non_degenerate = False
        degeneracy_reason = (degeneracy_reason or
                             "readiness precondition below floor -- substrate not ready")
    else:
...
def main(dry_run: bool = False) -> Dict:
    ...
    manifest = {
        ...
        "substrate": "SD-024",
        "confirms": "V3-EXQ-766a",
        "notes": (
            "GOV-CONFIRM-1 scoring-eligible confirmer for MECH-232. Reads the SAME "
            "pre-registered representation signature as the SD-024 diagnostic V3-EXQ-766a "
            "(density expansion ratio; weight-independent density-follower approach with the "
            "value gradient zeroed) but as experiment_purpose=evidence, so it contributes "
            "genuine experimental confidence (766a was diagnostic -> scoring_excluded, "
            "genuine_exp_count=0). Seeds 8-15 are INDEPENDENT of 766's 0-7, so this is a "
            "genuine replication whose readout is recorded in no existing manifest (GOV-REUSE-1: "
            "reanalysis of 766 would not move claim_evidence scoring -- the indexer does not "
            "ingest reanalysis/). Wall-independent: every DV reads a structural/functional "
            "signature of the benefit terrain, never an agent's committed episode behaviour. "
            "Benefit-terrain-only; harm/safety fields never DA-modulated (MECH-233 asymmetry). "
            "Total benefit intensity split across the DA cluster -> summed VALUE conserved "
            "while representational DENSITY expands; compute_benefit_density is weight-"
            "independent. No training (non-parametric RBF); phased training N/A."
        ),
    }
