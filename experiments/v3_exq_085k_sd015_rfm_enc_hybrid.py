...
=== ROOT CAUSE OF 085j C2 FAILURE ===

085j C3 PASS (goal_resource_r_enc=0.819), C4 PASS (enc_acc=0.927).
Encoder is excellent. BUT C2 catastrophically failed (benefit_ratio=0.034x):
  benefit_present=0.020 vs absent=0.587.

Root cause: E2Resource action selection is the problem.
  e2r_final_loss=0.0414 (vs RFM's 0.0078 in 085i).
  E2Resource learns dynamics in 16-dim latent space, which is hard:
  z_resource barely changes step-to-step (EMA-smoothed encoder output),
  so E2Resource converges to near-identity with a small learned bias.
  That bias becomes a constant directional preference in latent space,
  causing the agent to always pick the same action and get stuck
  (benefit_present=0.020 ~= 0, the agent barely moves toward resources).

=== THE FIX: RFM + Encoder Hybrid ===

Two-module action selection:
  (1) RFM (raw 25-dim -> 25-dim): learned in 085h/i, loss=0.0078. Reliable.
      Predicts rf_next = RFM(rf_curr, action).
  (2) ResourceEncoder (25->16): learned via contact-prediction BCE. Spatial invariance.
      Compares predicted rf_next to z_goal in encoded space.

Action selection: argmax cosine_sim(enc.encode(RFM(rf_curr, a)), z_goal)

Why this works:
  - RFM captures fine-grained rf dynamics (resource moves into/out of proximity
    cells as agent moves) -- this is what it was designed for and learned well.
  - Encoder maps predicted rf_next to z_resource space: "how resource-like is
    the predicted state?"
  - z_goal = EMA of enc.encode(rf) at contacts -- spatially invariant (085j
    confirmed this works: r=0.819).
  - Combined: "which action leads to a rf_next that looks like resource contact,
    compared to my invariant z_goal?" This is the correct question.

No E2Resource. RFM replaces it entirely for dynamics.

=== PASS CRITERIA (same as 085j) ===
...
EXPERIMENT_TYPE = "v3_exq_085k_sd015_rfm_enc_hybrid"
...
class ResourceEncoder(nn.Module):
    ...
    Spatial invariance: all contact positions share is_contact=1 label.
    Validated in 085j: enc_acc=0.927, goal_resource_r_enc=0.819.
    """
...
# ------------------------------------------------------------------ #
# ResourceForwardModel (raw rf space, from 085h/085i)                 #
# ------------------------------------------------------------------ #

class ResourceForwardModel(nn.Module):
    """
    Predicts resource_field_view_next from (resource_field_view_curr, action).
    Operates in raw 25-dim rf space -- reliable (loss=0.0078 in 085h/i).
    Used for action selection via hybrid: cosine_sim(enc.encode(RFM(rf,a)), z_goal).
    """
...
def _rfm_enc_guided_action(
    ...
    """
    Hybrid action selection: cosine_sim(enc.encode(RFM(rf_curr, a)), z_goal).

    RFM predicts next rf in raw 25-dim space.
    Encoder maps predicted rf_next to z_resource (16-dim).
    Goal comparison in encoded space for spatial invariance.
...
def _run_single(
    ...
    # z_world-seeded goal state (comparison baseline)
    ...
    # ResourceEncoder-seeded goal state (SD-015, dim=ENCODER_DIM)
    ...
    contact_buf_pos: List[torch.Tensor] = []
    contact_buf_neg: List[torch.Tensor] = []
    MAX_BUF = 2000
        ...
        for _ in range(steps_per_episode):
            ...
            if goal_present:
                if ttype == "resource":
                    ...
                    # Secondary seeding for z_goal_world only (not enc).
    ...
    for _ in range(eval_episodes):
        ...
        for _ in range(steps_per_episode):
            ...
            # Hybrid RFM+Encoder action selection in GOAL_PRESENT; random otherwise
            if goal_present and goal_state_enc.is_active():
                action_idx = _rfm_enc_guided_action(
...
def run(
    ...
    **kwargs,
    ...
    """GOAL_PRESENT (RFM+Encoder-guided eval) vs GOAL_ABSENT (random eval)."""
    ...
    print(f"\n[V3-EXQ-085k] Final results:", flush=True)
    print(
    ...
    if not c2_pass:
        if c3_pass:
            failure_notes.append(
                ...
                " RFM+enc goal representation works; navigation policy gap."
                " Check: RFM quality (rfm_loss should be < 0.01);"
                " consider nav_bias multiplier or stronger eval action selection."
            )
        else:
            failure_notes.append(
    ...
    if not c3_pass:
        failure_notes.append(
            ...
            f" enc_acc={avg_enc_final_acc:.3f}."
            " If acc < 0.7: encoder not converged. If acc > 0.7 but r < 0.2:"
            " RFM predictions not useful for goal tracking."
        )
    if not c4_pass:
        failure_notes.append(
            f"C4 FAIL: enc_contact_acc={avg_enc_final_acc:.3f} <= 0.7."
            f" enc_loss={avg_enc_final_loss:.4f}."
            " Try: increase warmup to 1000+ or lr_enc to 5e-3."
        )
    ...
    summary_markdown = (
        f"# V3-EXQ-085k -- SD-015 RFM+Encoder Hybrid Action Selection\n\n"
        f"**Status:** {status}\n"
        f"**Claims:** SD-015, SD-012, MECH-112\n"
        f"**Decision:** {decision}\n"
        f"**Seeds:** {list(seeds)}\n"
        f"**Supersedes:** V3-EXQ-085j\n\n"
        f"## Architecture\n\n"
        f"Hybrid action selection: `cosine_sim(enc.encode(RFM(rf, a)), z_goal)`\n\n"
        f"- **RFM** (25->25, raw rf space): reliable dynamics (085h/i loss=0.0078)\n"
        f"- **ResourceEncoder** (25->16, BCE contact prediction): spatial invariance"
        f" (085j: acc=0.927, r=0.819)\n"
        f"- **z_goal**: EMA of enc.encode(rf) at contacts (not raw rf)\n\n"
        f"**Rationale:** 085j E2Resource (dynamics in 16-dim latent space) had"
        f" e2r_loss=0.0414 and degenerated to a constant action policy"
        f" (benefit_present=0.020). RFM dynamics in raw 25-dim space are reliable"
        f" (fine-grained position info). Encoder handles goal comparison with"
        f" spatial invariance. Hybrid gets both.\n\n"
        ...
        f" (GOAL_PRESENT: RFM+enc hybrid; GOAL_ABSENT: random)\n\n"
        f"## Diagnostic Series Summary\n\n"
        f"| Variant | goal_resource_r | benefit_ratio | Note |\n"
        f"|---|---|---|---|\n"
        f"| 085g (z_world) | 0.066 | 0.37x | baseline |\n"
        f"| 085i (raw rf, contact-only) | 0.218 | 1.03x | spatial position issue |\n"
        f"| 085j (enc z_goal, E2Resource) | 0.819 | 0.034x | E2Resource degenerate |\n"
        f"| 085k (enc z_goal, RFM+enc hybrid) | {avg_goal_resource_r_e:.3f}"
