...
Fixes vs EXQ-073:
  - WORLD_OBS_DIM: 54 -> 200 (CausalGridWorld with use_proxy_fields=False, size=10)
  - record_transition: was passing z_self_next as action arg; fixed to pass actual
    action tensor. generate_trajectories/select_action moved before record_transition
    so action is available.
"""
...
EXPERIMENT_TYPE = "v3_exq_073b_mech111_novelty_signal"
CLAIM_IDS = ["MECH-111"]
...
def _run_single(
    ...
    print(f"\n[EXQ-073b] TRAIN {cond_label} seed={seed}", flush=True)
    agent.train()
    ...
    for ep in range(warmup_episodes):
        ...
        for _ in range(steps_per_episode):
            ...
            # generate_trajectories/select_action before record_transition so action
            # is available as a proper argument (fixes bug in EXQ-073)
...
def run(
    ...
    **kwargs,
    ...
    print(f"\n[EXQ-073b] MECH-111 Novelty Signal", flush=True)
    ...
    summary_markdown = (
        f"# V3-EXQ-073b -- MECH-111 Novelty Signal\n\n"
        f"**Status:** {status}\n"
        f"**Claims:** MECH-111\n"
