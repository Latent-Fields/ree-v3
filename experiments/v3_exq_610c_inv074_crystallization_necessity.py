...
"""V3-EXQ-610c: INV-074 crystallization necessity test (post-substrate Phase-3 enrichment retest).

Supersedes V3-EXQ-610b (FAIL; D2=False; control did not show predicted collapse ->
non_contributory per 2026-06-01 governance). This retest follows substrate
implementation IGW-20260601-023 (test_bed_enrichment_crystallization_necessity).
...
SUBSTRATE DEPENDENCIES (2026-06-01 retest):
  - test_bed_enrichment_crystallization_necessity (IGW-20260601-023) implemented
    2026-06-01T17:42:58Z: infant_curriculum.InfantCurriculumScheduler Phase 3
    now enables destabilizing environmental dynamics that force continued
    forward-model adaptation post-Phase-3:
      * SD-047 multi_source_dynamics (weather_field + transient_events +
        background_drift with n_drift_sources=2, intensity_scale=0.8)
      * SD-048 interoceptive_noise (autonomic + sensitisation + fatigue at
        scale=0.6)
      * Accelerated env_drift (interval=3 vs default 5, prob=0.5 vs default 0.3)
  - MECH-341 (E3 score diversity preservation) retuned 2026-05-27 commit a45ca7f:
    entropy_lambda 0.05 -> 0.5, bias_scale 0.1 -> 1.0.
  - V3-EXQ-610b (2026-06-01) ran WITHOUT the Phase-3 enrichment and showed
    FAIL (D1=False, D2=False): crystallization did NOT preserve diversity, and
    control did NOT show collapse -> non_contributory (CausalGridWorldV2 supplied
    no post-Phase-3 destabilising pressure). failure_autopsy_V3-EXQ-610a_2026-06-01
    diagnosed this as test_bed_enrichment_crystallization_necessity missing.
  - This experiment (V3-EXQ-610c) retests with the new Phase-3 destabilizing
    pressure to verify that the control arm now shows the predicted collapse.

Arms
----
  ARM_0 (control): crystallize_at_phase3=FALSE
    - Diversity mechanisms active (MECH-313, MECH-260, MECH-341 with RETUNED
      parameters, gated_policy with differential_heads for ARC-062 fix)
    - No crystallization -> diversity should COLLAPSE post-Phase-3 under
      destabilizing pressure (INV-074 predicts established discrimination is
      overwritten as F consolidates + env dynamics force reconsolidation)
  ...
  ARM_1 (test): crystallize_at_phase3=TRUE
    ...
    - Crystallization (plasticity injection + residue EWC) -> diversity should
      PERSIST post-Phase-3 despite destabilizing pressure (INV-074 predicts
      discrimination is protected by the plasticity lock)

Both arms:
  - infant_curriculum=True, 4-phase training (Phases 0-3)
  - Phase 3 WITH destabilizing pressures (SD-047 + SD-048 + accelerated drift)
  ...
  - MECH-313 (noise_floor) + MECH-260 (dACC anti-recency) + MECH-341 (E3 score
    diversity preservation with RETUNED parameters) all enabled
...
distribution) measured at:
  ...
  - end_phase_3 (post-closure + destabilizing pressure)

Pre-registered acceptance criteria
-----------------------------------
  D1 (crystallization preserves diversity):
      ARM_1.end_phase_3_entropy - ARM_0.end_phase_3_entropy >= +0.10
      (with crystallization, diversity persists despite pressure)

  D2 (control shows collapse):
      ARM_0.end_phase_2_entropy - ARM_0.end_phase_3_entropy >= +0.10
      (without crystallization, diversity regresses from Phase-2 peak under
      destabilizing pressure -- THIS IS THE CRITICAL FIX FROM 610b)
...
--------------
  ...
  (b) D1 FAIL (ARM_1 does not preserve diversity):
      -> crystallization insufficient under destabilizing pressure; escalate to
         /diagnose-errors {INV-074 weakens, MECH-334 weakens}

  (c) D2 FAIL (ARM_0 does not show collapse even with destabilizing pressure):
      -> control does not exhibit the predicted failure mode even under
         environmental pressure; either (i) the diversity signals (MECH-341
         retuned parameters) are sufficiently robust that crystallization is
         unnecessary, or (ii) the destabilizing pressure is still insufficient.
         Non_contributory for INV-074/MECH-334 (test is non-discriminative), but
         potentially strengthens MECH-341 (diversity preservation working well
         enough to prevent collapse even without crystallization and with pressure).
...
Estimated runtime: ~800 min (2 arms x 3 seeds x ~2500 episodes @ 200 steps/ep)
  -- V3-EXQ-610b actual_secs=47729 (~13.25h) on ree-cloud-2.
"""
...
EXPERIMENT_TYPE = "v3_exq_610c_inv074_crystallization_necessity"
EXPERIMENT_PURPOSE = "evidence"
QUEUE_ID = "V3-EXQ-610c"
CLAIM_IDS = ["INV-074", "MECH-334", "MECH-333"]
BACKLOG_ID = "EVB-0270"
SUPERSEDES = "V3-EXQ-610b"
...
def _make_agent_and_env(
    ...
    config = REEConfig.from_dims(
        ...
        # NOTE: Uses RETUNED parameters (entropy_lambda 0.5, bias_scale 1.0 per 2026-05-27 commit a45ca7f).
        # The substrate now reads these from ree_core/agent.py defaults.
...
def _train_infant_curriculum(
    ...
    for ep in range(max_eps):
        ...
        # NOTE: Phase 3 now includes destabilizing pressures (SD-047 + SD-048 + accelerated drift)
        # via the infant_curriculum.InfantCurriculumScheduler.env_kwargs() Phase-3 path.
...
def _run_arm(arm_config: Dict, seed: int, dry_run: bool) -> Dict:
    ...
    print(
        f"[V3-EXQ-610c] Starting {arm_label} seed={seed} crystallize={crystallize}",
        flush=True,
    )
...
if __name__ == "__main__":
    ...
    # Run experiment
    print(
        f"[V3-EXQ-610c] INV-074 crystallization necessity test (EVB-0270, post-Phase-3-enrichment retest)",
        flush=True,
    )
    ...
    if not dry:
        ...
        print(f"[V3-EXQ-610c] Manifest written: {manifest_path}", flush=True)
