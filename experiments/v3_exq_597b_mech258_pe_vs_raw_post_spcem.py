...
"""
V3-EXQ-597b -- MECH-258: precision-weighted affective PE vs raw magnitude (post-SP-CEM).

Bug-fix iteration of V3-EXQ-597 (failure autopsy 2026-05-21: measurement_gap).
597 C2 used post-clip |dACC bias| (dacc_bias_max_abs=2.0), which saturated at
2.0 on every step and made Pearson r undefined. This run keeps the same agent
config and phased training as 597 but records pre-clip bias for C2 correlation.
...
Pre-registered PASS (MECH-258 evidence):
  ...
  C2: corr(|bias_pre_clip|, model_pe) > corr(|bias_pre_clip|, raw_norm) + 0.05
      in >=2/3 seeds (PE_FORWARD; pre-clip bias from DACCtoE3Adapter bundle,
      before dacc_bias_max_abs clamp; model_pe = ||z - z_pred||, raw_norm = ||z||)
...
EXPERIMENT_TYPE = "v3_exq_597b_mech258_pe_vs_raw_post_spcem"
CLAIM_IDS = ["MECH-258"]
EXPERIMENT_PURPOSE = "evidence"
QUEUE_ID = "V3-EXQ-597b"
SUPERSEDES_QUEUE_ID = "V3-EXQ-597"
...
def _dacc_bias_pre_clip_tensor(bundle: dict, adapter) -> torch.Tensor:
    """Mirror DACCtoE3Adapter.forward without dacc_bias_max_abs clamp (C2 telemetry)."""
    cfg = adapter.config
    mode_ev = bundle["mode_ev"]
    device = mode_ev.device
    dtype = mode_ev.dtype
    weight = cfg.dacc_weight * float(bundle.get("drive_gain", 1.0))
    if weight == 0.0:
        return torch.zeros_like(mode_ev)
    bias = -mode_ev.clone()
    if cfg.dacc_interaction_weight != 0.0:
        bias = bias + cfg.dacc_interaction_weight * (-bundle["harm_interaction"])
    if cfg.dacc_foraging_weight != 0.0:
        fv = torch.tensor(bundle["foraging_value"], dtype=dtype, device=device)
        bias = bias + cfg.dacc_foraging_weight * fv
    if cfg.dacc_suppression_weight != 0.0:
        bias = bias + cfg.dacc_suppression_weight * bundle["suppression"]
    return weight * bias
...
def _run_condition(seed: int, condition: str, verbose: bool = True) -> Dict:
    ...
    for ep_idx in range(P0_EPS + P1_EPS + P2_EPS):
        ...
        for _step in range(STEPS_PER_EP):
            ...
            if is_p2:
                ...
                if agent._dacc_last_bundle is not None and agent.dacc_adapter is not None:
                    pre_bias = _dacc_bias_pre_clip_tensor(
                        agent._dacc_last_bundle, agent.dacc_adapter
                    )
                    b_pre = float(pre_bias.abs().mean().item())
                    bias_pre_sum += b_pre
                    bias_cnt += 1
                    bias_trace.append(b_pre)
                    if agent._dacc_last_bias is not None:
                        b_post = float(agent._dacc_last_bias.abs().mean().item())
                        bias_post_sum += b_post
                        max_abs = float(agent.dacc_adapter.config.dacc_bias_max_abs)
                        if max_abs > 0.0 and b_post >= max_abs - 1e-6:
                            clip_hits += 1
    ...
    mean_bias_pre = bias_pre_sum / bias_cnt if bias_cnt > 0 else 0.0
    mean_bias_post = bias_post_sum / bias_cnt if bias_cnt > 0 else 0.0
    clip_frac = clip_hits / bias_cnt if bias_cnt > 0 else 0.0
    corr_delta = (
        corr_pe - corr_raw
    ...
    result = {
        ...
        "mean_score_bias_pre_clip_abs": float(mean_bias_pre),
        "mean_score_bias_post_clip_abs": float(mean_bias_post),
        "mean_score_bias_abs": float(mean_bias_post),
        "bias_clip_saturation_frac": float(clip_frac),
        "c2_bias_telemetry": "pre_clip",
...
def main(dry_run: bool = False, output_dir: Optional[str] = None) -> Dict:
    ...
    output = {
        ...
        "queue_id": QUEUE_ID,
        "supersedes": "v3_exq_597_mech258_pe_vs_raw_post_spcem_20260520T063328Z_v3",
        "claim_ids": CLAIM_IDS,
