...
Root cause of EXQ-049 FAIL:
  Same bug as EXQ-048 -- agent.select_action() was bypassed. EXQ-049 called
  agent.e3.select(candidates, temperature=1.0) directly and set agent._last_action
  manually. This bypasses the only code path that calls beta_gate.elevate() and
  beta_gate.propagate(). As a result, gate.is_elevated was always False,
  hold_count=0 and propagation_count=0, so C3 and C4 both FAILed regardless
  of commitment state. This was an experiment instrumentation error.
...
Fix:
  ...
  agent.select_action() reads result.committed, calls beta_gate.elevate() or
  beta_gate.release(), then beta_gate.propagate(action). Gate state is now
  updated on every e3_tick.
...
Motivation (2026-03-20):
  ...
  Agent sets beta state via agent.select_action():
  ...
  MECH-090 distinction from MECH-057b (EXQ-048b):
    - EXQ-048b measured completion timing (gate fires at completion, not initiation)
    - EXQ-049b measures the POLICY CONTENT: is the held vs propagated content
...
EXPERIMENT_TYPE = "v3_exq_049b_mech090_beta_gate_fixed"
CLAIM_IDS = ["MECH-090"]
...
def run(
    ...
    **kwargs,
    ...
    print(
        f"[V3-EXQ-049b] MECH-090: Beta-Gated Policy Propagation (Fixed)\n"
    ...
    print(f"\n[V3-EXQ-049b] Eval -- beta gate concordance...", flush=True)
    ...
    print(f"\nV3-EXQ-049b verdict: {status}  ({criteria_met}/5)", flush=True)
    ...
    summary_markdown = f"""# V3-EXQ-049b -- MECH-090: Beta-Gated Policy Propagation (Fixed)
