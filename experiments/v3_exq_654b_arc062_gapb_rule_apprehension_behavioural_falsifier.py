...
"""
V3-EXQ-654b -- arc_062_rule_apprehension:GAP-B behavioural falsifier for
MECH-309 / ARC-062, LONGER-MATURATION RE-RUN of V3-EXQ-654a.

SUPERSEDES V3-EXQ-654a (FAIL non_contributory, substrate_not_ready_requeue,
2026-06-10): even with crf_persist_rules_across_episode_reset=True the persisted
CandidateRuleField did NOT mature past the C1c floor at 654a's budget --
crf_frac_active = 0.137 < 0.30, 0/3 ON seeds matured (rules minted 268/549/220),
so the C2 committed-class falsifier DV never ran. The within-arm rule_state
counterfactual was 0/3 and the committed-class entropy lift was ~0 (OFF 0.9375 ->
ON 0.9377), but with C1c unmet that readout is NOT scored (correctly
non_contributory). See REE_assembly/evidence/planning/failure_autopsy_569f-661-654a_2026-06-10.{md,json}.

THE ONLY DESIGN CHANGE vs V3-EXQ-654a: a LONGER CRF maturation budget. The
maturation window (P0 + P1, both before the frozen P2 measurement, with
crf_persist carrying the pool across the per-episode agent.reset()) is extended
from 654a's 60 + 40 = 100 episodes to 150 + 90 = 240 episodes (2.4x), so the
persisted differentiated pool can clear crf_frac_active >= 0.30 BEFORE the
behavioural criteria are scored. Everything else is matched to 654a:
crf_persist_rules_across_episode_reset=True, the trained-bias-head P1 REINFORCE
window (GAP-D), the within-arm rule_state counterfactual instrumentation
(mean_prop_counterfactual_delta per seed), and the C1-style non-vacuity gate
(C1c matured + C1d propagation non-vacuity) that self-routes
substrate_not_ready_requeue -> non_contributory if maturation STILL fails.

DEEPER FINDING THIS RE-QUEUE INSTRUMENTS (failure_autopsy_569f-661-654a): if a
matured-and-firing field (C1c MET) STILL shows a within-arm rule_state
counterfactual of ~0 AND no committed-class lift, 654b promotes to a full member
of the E3-commit-readout cluster (with 569f / 661) and confirms the selection-
authority COUPLING -- not maturation -- is the ceiling. So the within-arm
rule_state counterfactual is recorded per seed (mean_prop_counterfactual_delta).

THREE changes vs the ORIGINAL V3-EXQ-654 (inherited from 654a; everything else matched):
...
Phases / budget
---------------
P0 (150 ep, e2 TRAINED online via SD-056 contrastive; bias head NOT trained; field
   ...
   seeds a matured rule pool. 654b LONGER maturation (was 60).
P1 (90 ep, encoder FROZEN, bias head TRAINED via outcome-coupled REINFORCE; field
   ...
   654b LONGER maturation (was 40).
P2 (60 ep, all FROZEN -- e2 + bias head; field persists; instrumentation ON): the
   behavioural measurement window.
Budget: 2 arms x 3 seeds x 300 ep x 200 steps = 360k steps total. ~8-9 h (2.25x
654a's 192k). The maturation window (P0+P1) grows 100 -> 240 ep so the persisted
pool clears crf_frac_active >= 0.30 before P2 scoring. The C2 paired lift + C1d
propagation comparison are within-seed (same seed, same machine) -> machine_affinity "any".
...
EXPERIMENT_TYPE = "v3_exq_654b_arc062_gapb_rule_apprehension_behavioural_falsifier"
QUEUE_ID = "V3-EXQ-654b"
SUPERSEDES = "V3-EXQ-654a"
...
SEEDS = [42, 43, 44]
P0_WARMUP_EPISODES = 150         # 654b LONGER maturation: SD-056 e2 warmup; field matures (crf_persist)
P1_BIAS_TRAIN_EPISODES = 90      # 654b LONGER maturation: frozen-encoder bias-head REINFORCE (GAP-D); field keeps maturing
...
def _build_manifest(
    ...
    return {
        ...
        "evidence_direction_note": (
            f"V3-EXQ-654b arc_062 GAP-B behavioural falsifier (MECH-309 / ARC-062), "
            f"LONGER-MATURATION re-run of V3-EXQ-654a (P0+P1 maturation window 100->240 ep) "
            f"so crf_frac_active clears the 0.30 C1c floor before scoring. Single-variable: "
...
def main() -> Tuple[Optional[str], Optional[str]]:
    parser = argparse.ArgumentParser(description="V3-EXQ-654b GAP-B falsifier (longer-maturation re-run of 654a)")
