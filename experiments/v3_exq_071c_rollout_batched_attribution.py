"""
V3-EXQ-071c -- Rollout-Batched SD-003 Attribution (GPU-Optimized Variant)

Supersedes: V3-EXQ-071b (error on 2026-03-23, pre-E2-detach-bug-fix era)
...
EXPERIMENT_TYPE = "v3_exq_071c_rollout_batched_attribution"
...
def _eval_paired(
    ...
    if len(all_seq) > 10:
        ...
        try:
            rho, _ = spearmanr(all_seq, all_batch)
            rank_corr = float(rho)
        except Exception:
            rank_corr = 0.0
    else:
...
def run(
    ...
    **kwargs,
    ...
    print(
        f"[V3-EXQ-071c] Rollout-Batched SD-003 Attribution\n"
    ...
    print(f"\n[V3-EXQ-071c] Training ({warmup_episodes} episodes)...", flush=True)
    train_out = _train(
    ...
    for n_haz in DISTRACTOR_LEVELS:
        print(f"\n[V3-EXQ-071c] Eval: n_hazards={n_haz} ({eval_episodes} eps)...", flush=True)
    ...
    print(f"\n[V3-EXQ-071c] Verdict: {status}  ({n_criteria_met}/5 criteria)", flush=True)
    ...
    summary_markdown = f"""# V3-EXQ-071c -- Rollout-Batched SD-003 Attribution
