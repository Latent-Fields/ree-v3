"""
V3-EXQ-517: MECH-302 relief-completion behavioural discriminative pair.
...
Acceptance criteria (both seeds must pass for PASS fraction >= 2/3):
...
Gate (c) for MECH-304 conditioned-inhibition experiment.
"""
...
EXPERIMENT_TYPE = "v3_exq_517_mech302_relief_completion_discriminative_pair"
CLAIM_IDS = ["MECH-302"]
...
COMP_WINDOW_LENGTH = 5
COMP_DROP_THRESHOLD = 0.05
COMP_MIN_INITIAL_NORM = 0.02
...
def step_agent(agent, env, obs_dict):
    ...
    Returns diagnostics dict including event_fired, valence_writes, done, next_obs."""
    ...
    # Monkey-patch valence writes BEFORE sense (some writes may happen in sense path)
    ...
    with torch.no_grad():
        ...
        # Capture event flag AFTER sense() sets it, BEFORE select_action() clears it
...
def run_arm(seed: int, arm_label: str, arm_on: bool, dry_run: bool) -> dict:
    ...
    if arm_on:
        ...
        c3_pass = True   # not applicable for ON arm
        c4_pass = True
        seed_pass = c1_pass and c2_pass
    else:
        c1_pass = True   # not applicable for OFF arm
        c2_pass = True
        c3_pass = p1_events == 0
        c4_pass = p1_writes == 0
        seed_pass = c3_pass and c4_pass

    verdict_str = "PASS" if seed_pass else "FAIL"
    ...
    return {
        ...
        "arm_label": arm_label,
        "arm_on": arm_on,
        "p1_episodes": 0 if dry_run else P1_EPISODES,
        "p1_events": p1_events,
        "p1_writes": p1_writes,
        "c1_pass": c1_pass,
        "c2_pass": c2_pass,
...
def main():
    ...
    for seed in SEEDS:
        res_a = run_arm(seed, "ARM_A_mech302_on", arm_on=True, dry_run=dry_run)
        res_b = run_arm(seed, "ARM_B_mech302_off", arm_on=False, dry_run=dry_run)
    ...
    manifest = {
        ...
        "notes": (
            "MECH-302 relief-completion behavioural discriminative pair. "
            "ARM_A (ON): use_suffering_derivative_comparator=True + valence_liking_enabled=True; "
            "ARM_B (OFF): both False. limb_damage_enabled=True (SD-022) provides clear "
            "z_harm_a rise/fall dynamics (damage accumulates on hazard contact, heals 0.002/step). "
            "C1: ARM_A events >= 3 per seed. C2: ARM_A writes >= 2 per seed. "
