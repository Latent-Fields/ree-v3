"""
V3-EXQ-514t -- SD-049 Phase-2 MECH-436 drive-coupling RETEST (514s successor; BOUNDED kappa raise).

The pre-registered retest routed by failure_autopsy_V3-EXQ-514s_2026-06-18 (confirmed,
user-adjudicated: clean self-route, substrate-ceiling PARTIALLY LIFTED). 514s ran the
2026-06-17 levers at kappa_scale=6.0 + restoration=0.3 and MET all five non-vacuity
preconditions, proving lever (b) standing-differential-depletion WORKED (enriched_spread_met
1.0, mean_drive_spread_max 0.211 vs 514q's equalised ~0.006). The residual shortfall is
LOCALIZED to lever (a): the WL metric is argmax-flip-gated and on 3/5 seeds the real object
base_value gaps exceed what kappa=6.0 x standing-spread~0.2 flips at natural magnitude
(natural delta mean 0.064 < margin 0.15; 2/5 seeds clear; overshoot mag 5.0 still flips 4/5).
The autopsy routed a BOUNDED raise of the existing no-op-default levers (implement-substrate
2026-06-19; NO new flag, NO ree_core default change):
  (a) RAISE GoalConfig.incentive_drive_kappa_scale 6.0 -> 12.0 (effective kappa
      = incentive_drive_kappa_weight(2.0) * 12.0 = 24). BOUNDED: at eff_kappa=24 a MODERATE
      ...
      object; OFF floor stays hard-zero; MECH-229 leg-(a) wanting!=liking intact). Contracts
      C7/C8 in tests/contracts/test_sd049_phase2_drive_coupling.py guard both invariants.
  (b) DEEPEN the standing spread: per_axis_restoration_fraction 0.3 -> 0.15 on the WL-scoring
      ...
      preserved), leaving ~0.43 standing drive on a just-consumed axis (vs ~0.35 at 0.3) so
      the divergent-decay per-axis differences -- and the argmax-relevant spread -- are larger
      at the WL scoring moment; less kappa needed per flip.

514t re-runs the 514r/514s overshoot + OFF/bank-disabled + recalibrated-argmax-relevance +
enriched-spread readiness controls on the BOUNDED-KAPPA + DEEPER-STANDING-DEPLETION substrate.
The agent config sets incentive_drive_kappa_scale=KAPPA_SCALE; the WL SCORING env (the
...
SUBSTRATE + instrument chain: IDENTICAL training stack to 514s (full
scaffolded_sd054_onboarding 603n curriculum; 681-C4 info-tag liking source; SD-057
IncentiveTokenBank wanting). NEW vs 514s: BOUNDED higher agent kappa_scale (6.0 -> 12.0) +
deeper enriched WL-scoring env (per_axis_restoration_fraction 0.3 -> 0.15 + divergent decay).

experiment_purpose: evidence
supersedes: V3-EXQ-514s (514s proved lever (b) WORKED but kappa_scale=6.0 was short on 3/5
  seeds; 514t tests the natural-delta supports path on the BOUNDED higher-kappa
  (12.0) + deeper-standing-depletion (restoration 0.15) substrate).
predecessor: V3-EXQ-514s.
...
EXPERIMENT_TYPE = "v3_exq_514t_sd049_phase2_mech436_drive_coupling_retest"
QUEUE_ID = "V3-EXQ-514t"
CLAIM_IDS: List[str] = ["MECH-436"]
EXPERIMENT_PURPOSE = "evidence"
PREDECESSOR = "V3-EXQ-514s (514s: lever (b) standing-differential-depletion WORKED [enriched_spread 1.0] but kappa_scale=6.0 short on 3/5 seeds [natural delta 0.064 < 0.15; overshoot flips 4/5] -> BOUNDED kappa raise amend landed)"
SUPERSEDES = "V3-EXQ-514s"
...
# BOUNDED-RAISE amend (failure_autopsy_V3-EXQ-514s; implement-substrate 2026-06-19): 514s
# proved lever (b) WORKED but kappa_scale=6.0 (lever a) was short on 3/5 seeds. This retest
# RAISES kappa above 6.0 + DEEPENS the standing spread, BOUNDED so drive does not swamp
# base_value (contracts C7 OFF-floor-hard-zero / C8 bounded-no-domination guard the invariants).
# (a) kappa scale: effective kappa = incentive_drive_kappa_weight(2.0) * KAPPA_SCALE = 24.
#     At 24, a MODERATE base_value gap (1.0 vs 0.6) flips under a realistic standing spread
#     while a CLEARLY-LARGER 10x gap (1.0 vs 0.10) does NOT -- carves near-ties without
#     overriding a decisive gap. Kappa-alone could not close it boundedly (natural spread ~0.2
#     is ~25x smaller than the mag-5.0 overshoot), hence pairing with the deeper spread in (b).
KAPPA_SCALE = 12.0
...
RESTORATION_FRACTION = 0.15      # deeper partial restore -> ~85% standing drive left on contact
                                 # (~0.43 standing vs ~0.35 at 0.3 -> deeper argmax-relevant spread)
