...
"""
V3-EXQ-847a -- ARC-062 GOV-FANOUT-1 portfolio Leg P-D (H4: MEASUREMENT ALIASING --
context-conditioned mode-switch). DIAGNOSTIC (claim_ids=[]; brake-exempt; a
measurement-adequacy probe, NOT a same-substrate re-test of MECH-309/ARC-062).
SUPERSEDES V3-EXQ-847 (denominator fix; see PREMISE CORRECTION below).

Design source (build exactly this): REE_assembly/evidence/planning/
arc_062_conversion_fanout_2026-07-29.md Section 2 "Leg P-D" (H4) + the P-A/P-B/P-C
erratum sections appended to that same doc 2026-07-31 + the `active_frac`
denominator sweep addendum appended 2026-08-02. Provenance chip
chip-20260729-arc062-pd-context-modeswitch (original P-D build);
chip-20260802-v3exq847a-queue (this redesign).

PREMISE CORRECTION vs V3-EXQ-847 (this build, 2026-08-02; CONFIRMED by
`REE_assembly/evidence/planning/failure_autopsy_V3-EXQ-847_2026-08-01.md`,
governance-ratified 2026-08-01T22:24:00Z). V3-EXQ-847's readiness-gate
preconditions `mech448_demotion_lever_live_and_excluding_both_arms` /
`mech449_active_nogo_live_and_suppressing_both_arms` computed `active_frac` as
`demotion_active_ticks / n_p2_ticks` (resp. `nogo_active_ticks / n_p2_ticks`).
The numerator is correctly latch-guarded to genuine fresh `E3.select()` ticks
only (`n_latched_ticks` bookkeeping, below), but the denominator `n_p2_ticks`
counted EVERY P2 tick including held/replayed commitments -- a basis mismatch
between a fresh-only numerator and an all-ticks denominator that silently
deflated `active_frac` by the held-tick fraction (documented 40-86% across
847's 6 cells), producing a FALSE `substrate_not_ready_requeue` self-route even
though MECH-448/449 were actually 100% active on genuine fresh selections
(recomputed correctly from 847's own manifest: every cell corrects to exactly
1.0 against `n_p2_ticks - n_latched_ticks`). THE FIX (this build): both
`demotion_active_frac` and `nogo_active_frac` now divide by
`n_p2_fresh_select = n_p2_ticks - n_latched_ticks`, matching the
reference-correct pattern already used by sibling script V3-EXQ-859
(`v3_exq_859_arc062_lateral_pfc_route_mech448_449_ablation.py`, which tracks
an equivalent `n_p2_fresh_select` counter). This is purely a post-hoc
aggregation fix -- it does not touch the training loop, RNG seeding, env
dynamics, ARMS, or hyperparameters, all of which are otherwise byte-identical
to V3-EXQ-847.

SEED SET (UNCHANGED from V3-EXQ-847: [42, 43, 44]). Investigated explicitly
this build session, not carried over by default. The 847 autopsy names two
sanctioned options for seed 44 (documented environmentally-degenerate for this
exact env geometry, per `failure_autopsy_V3-EXQ-690_2026-06-20.md`'s "R3
environmental-degeneracy lesson" and confirmed again in 847's own run,
n_reef_ticks=0 both arms): "avoid 44" or "accept it will always self-exclude".
This build accepts the latter, for three reasons. (1) No third
documented-good seed exists to substitute -- every script in the
654/690/847/858/859/863 lineage uses exactly [42, 43, 44], so there is no
precedent seed to draw on. (2) A standalone probe run this session
(CausalGridWorldV2 with 847's exact ENV_KWARGS, pure random-action rollout,
20 episodes x 200 steps, seeds 42-52) found ALL candidates -- including seed
44 -- visit the reef half 22-38% of the time under a RANDOM policy (seed 44:
21.9%, n=4000). Yet seed 44 is documented ZERO reef-ticks under the REAL
TRAINED P2 policy in two independent runs (V3-EXQ-690 and V3-EXQ-847). This
proves the seed-44 degeneracy is a TRAINED-POLICY property (e.g.
hazard/food-attraction-driven avoidance of the reef half), not a raw
environment-geometry property -- so no cheap pre-screen can verify whether an
untested seed (45, 46, ...) would also turn out degenerate under the real
P0/P1/P2 training pipeline. Swapping in an unverified seed would be a blind
gamble, not a verified improvement, and would also break direct seed-for-seed
comparability with the rest of the GOV-FANOUT-1 P-A/P-B/P-C/P-D portfolio and
the 654-lineage. (3) Seeds 42 and 43 are independently confirmed
context-scorable (n_reef_ticks/n_open_ticks both clear MIN_CONTEXT_TICKS=10)
on BOTH arms in V3-EXQ-847's own manifest, so C1g's "majority (>=2/3) of
seeds in BOTH arms" floor is met by 42+43 alone regardless of seed 44. Extra
seeds were deliberately NOT added either: MIN_SEEDS_FOR_PASS=2 and
TV_MIN_LIFT_SEEDS=2 are fixed absolute counts (not proportions) throughout
C1a-g, so appending seeds without rescaling those constants would silently
loosen the "majority of seeds" semantics for every C1 subgate -- out of scope
for a denominator-only fix.

EXPECTED OUTCOME (stated in advance so a future reader is not surprised).
Because the fix touches only post-hoc aggregation and SEEDS/ENV_KWARGS/ARMS
are unchanged, re-running is expected to reproduce numerically identical (or
near-identical, modulo hardware/torch nondeterminism) values for n_p2_ticks,
n_latched_ticks, committed_class_counts, reef/open tick counts, and
tv_context_divergence per seed x arm as V3-EXQ-847's own manifest already
recorded: seed 43 bit-identical across arms (a washout -- the CRF manipulation
produced zero detectable trajectory difference for that seed), seed 44 zero
reef ticks (degenerate self-exclusion, expected and accepted per above), seed
42 the only informative cell, showing a lift in the direction OPPOSITE H4's
hypothesis (CRF-ON reduces context-divergence, 0.046 vs 0.085). The point of
this re-run is NOT to gamble on a different outcome -- it is to let the run's
OWN readiness gate correctly recognize MECH-448/449 as live (instead of
falsely self-routing substrate_not_ready_requeue) so the pipeline produces a
properly-labeled manifest with the correct C1/C2/C3 verdict computed
end-to-end by the protocol itself, rather than living only as a
hand-annotated correction on a broken run's autopsy. Expected outcome branch:
FAIL / non_contributory / label
"context_divergence_not_greater_arm_on_measurement_aliasing_ruled_out" (H4
refuted as an explanation for the pooled-DV null).

THE QUESTION (H4). Every prior rung of the 654-lineage (654g/h/i/j and the P-A/P-B/
P-C legs of this same fanout) reads its committed-class DV as the POOLED MARGINAL
entropy of the committed first-action class over the whole P2 window. H4 asks
whether that pooled-marginal DV is BLIND to a narrower, context-CONDITIONED effect:
the ARC-062 weak reading asserts the rule-apprehension channel differentiates >= 2
CONTEXT-GATED regimes (reef-exposed vs open/forage), not that it moves the marginal
class frequency. If the differentiated rule_state shifts WHICH class is committed
DIFFERENTLY depending on context (e.g. class 2 in reef, class 5 in the open) while
leaving the POOLED marginal roughly unchanged, the 654-lineage's own DV would read a
null even though the rule-creator's signal is real -- a measurement artifact, not a
substrate ceiling. H4 tests this directly: DV = divergence between the reef-context
and open-context committed-class distributions (TV distance), compared ARM_ON vs
ARM_OFF, paired by seed.

PREMISE CORRECTION vs the plan doc's Section 2 text (STOP-CHECK finding, this build
session, 2026-07-31; CODE-CONFIRMED). Section 2's P-D design line reads "P-A's
matched stack + gated_policy routing". Leg P-A was separately found (erratum
appended to the plan doc after its Section 2, same date) to rest on a FALSE premise:
modulatory_channel_route_source="gated_policy" does NOT route the rule-apprehension
/ CandidateRuleField (CRF) channel -- e3_selector's "gated_policy" route source
identity-routes `_bdc_gp` (ree_core/agent.py:7414), the output of the SEPARATE
GatedPolicy module (ree_core/policy/gated_policy.py), whose own docstring states
"There is no connection to SD-033a LateralPFCAnalog in Phase 1". The CRF-differentiated
signal P-D's swept variable (use_candidate_rule_field) actually varies reaches
committed action ONLY via the pre-existing `_bdc_lpfc` -> dacc_score_bias summation
path (ree_core/agent.py:6557) -- exactly the path the ENTIRE 654g/h/i/j lineage
already uses, unchanged. Adding the "gated_policy" routing here would therefore only
inject an unrelated, equally-present-on-BOTH-arms bias term with zero bearing on the
H4 measurement-aliasing question (a constant across arms cannot move a PAIRED-BY-SEED
ARM_ON-vs-ARM_OFF comparison either way) -- so THIS BUILD OMITS IT ENTIRELY. P-D is
therefore a straight adaptation of the 654j matched stack (unchanged) plus a NEW
context-partitioned readout layered on top of the existing measurement window; the
plan doc's own erratum block confirms "P-D unaffected by this finding (does not
depend on gated_policy routing mattering)" and this build makes that concrete by
never wiring the route.

GOV-REUSE-1 check (this build session): the decisive readout -- paired-by-seed TV(
committed-class | reef) vs TV(committed-class | open), ARM_ON vs ARM_OFF on the
use_candidate_rule_field sweep -- is NOT recorded anywhere. V3-EXQ-690 (Q-054)
already computes and reports the identical `tv_reef_forage` statistic per seed-arm
(reused verbatim as `_tv_distance` below) but its swept variable is
noise_floor_min_temperature (an entropy-sweep ladder), never use_candidate_rule_field
-- a different manipulation entirely, so 690's manifest cannot answer H4. No other
manifest or queue entry combines the reef/open partition with the CRF ON/OFF sweep.
Not recoverable by reanalysis; proceeds to a new run.

WHY THIS IS BUILDABLE (code-confirmed, this build session). The reef/open context
signal is a REAL substrate primitive, not aspirational: `ree_core/environment/
causal_grid_world.py` implements SD-054 `reef_enabled` / `reef_bipartite_layout`
(midline-split bipartite geometry, `reef_bipartite_axis` + `reef_bipartite_agent_
band_radius`), already present unchanged in 654j's own ENV_KWARGS below. The
per-tick partition helper `_is_reef_half(env)` and the TV-distance helper
`_tv_distance(...)` are ported verbatim from `v3_exq_690_q054_arc062_diversity_
floor_sweep.py` (lines ~439-446 / ~449-464), which already demonstrates the
partition is live and non-degenerate on exactly this env config: seeds 42 and 43
both clear a >=10-tick floor in BOTH reef and forage partitions (n_reef_ticks=271/
8909, n_forage_ticks=4950/2962 in the 690 manifest), while seed 44 recorded
n_reef_ticks=0 (the agent's trajectory under that seed never crosses into the reef
half) -- the documented "690 R3 environmental-degeneracy lesson" (`failure_autopsy_
V3-EXQ-690_2026-06-20.md` line 50). SEEDS below is kept at [42, 43, 44] to match the
654-lineage exactly; the new C1g readiness precondition (below) self-routes
substrate_not_ready_requeue if fewer than a majority of seeds turn out scorable
under 654j's different (MECH-448/449-armed) policy, rather than assuming 690's
finding transfers unchanged.

Single-variable arm contrast (UNCHANGED from 654j -- the matched stack, the ONLY
swept variable, is identical)

Both arms run the SAME matched stack as V3-EXQ-654j (unchanged; not re-derived here) --
SP-CEM (Layer A), GAP-A shared candidate-summary source candidate_summary_source=
"e2_world_forward" (V3-EXQ-649) with e2 TRAINED ONLINE in P0 (SD-056 contrastive;
rollout-norm clamp ON per the 643a numerical-stability lesson), the modulatory-bias-
selection-authority gate (V3-EXQ-643a float32 fix) + channel routing
(cand_world_summary -- NOT "gated_policy"; see the premise correction above) + the
MECH-448 RANK-PRESERVING F->ELIGIBILITY DEMOTION conversion (channel-adaptive floor,
V3-EXQ-689e) + the MECH-449 (ARC-107) GO/NO-GO ELIGIBILITY CONSTITUTION active No-Go
opponency leg (V3-EXQ-689g), MECH-341 stratified-select, MECH-313 noise floor, the V_s
minimal stack, AND use_lateral_pfc_analog=True + use_gated_policy=True with the
SD-033a bias head UN-ZEROED (lateral_pfc_train_rule_bias_head=True) so the rule_state
reaches a non-zero per-candidate E3 score-bias. These MECH-448/449 conversion
constants are kept ARMED (matched on BOTH arms, per 654j) so that P-D reads the SAME
"does the differentiated bias reach + convert" substrate state the 654-lineage's own
committed-class entropy DV was measured under -- P-D is a re-READING of that exact
substrate state through a context-partitioned lens, not a different substrate config.

The ONLY swept variable is use_candidate_rule_field (which auto-sets
use_candidate_rule_source on the agent) -- UNCHANGED from 654j:
  ARM_OFF: use_candidate_rule_field=False -> LateralPFCAnalog rule_state is the
           ...
           EMA source -- the 543/598b COLLAPSED rule_state. (Matched-stack ON, matched.)
  ARM_ON:  use_candidate_rule_field=True (+ crf_persist=True) -> LateralPFCAnalog
           ...
           rule_state, MATURED across episodes. (Matched-stack ON, matched.)

Dependent variables -- POOLED (inherited, reported) + CONTEXT-PARTITIONED (NEW, PRIMARY)
------------------------------------------------------------------------------------------
Inherited from 654j, reported as context (NOT load-bearing here; claim_ids=[]):
committed_class_entropy_nats, the POOLED marginal entropy of the committed first-action
class over the whole P2 window.

NEW PRIMARY DV (H4): per P2 tick, partition by `_is_reef_half(env)` (the SD-054
bipartite-layout geometry test, ported from V3-EXQ-690) into a reef-context committed-
class distribution and an open/forage-context committed-class distribution, and
compute their TOTAL-VARIATION distance `tv_context_divergence` (`_tv_distance`, also
ported from V3-EXQ-690). Read PAIRED BY SEED, ARM_ON vs ARM_OFF: does the
differentiated rule_state increase context-conditioned divergence MORE than the
legacy collapsed rule_state does? A positive paired lift here, in a run where the
POOLED committed_class_entropy_nats lift (654j's own C2) was null or weak, is exactly
the measurement-aliasing signature H4 predicts.

DV-SYMMETRY INVARIANCE DECLARATION (mandatory per the fanout doc + this skill).
(1) The ARM manipulation (differentiated vs collapsed rule_state) is NOT a uniform
broadcast scalar -- it is a per-first-action-class differentiated bias (compute_bias
broadcasts one rule_state across the K candidates WITHIN a tick, but which rule_state
fires depends on the matured CRF pool's per-context match, so across ticks/contexts
the bias is not a single additive constant). It is therefore not annihilated by the
argmax symmetry of the committed-class DV -- this is the SAME finding 654j's own
docstring already established (the differentiated rule-creator "moves WHICH CLASS is
committed"), inherited unchanged. (2) `tv_context_divergence` is itself a difference
of two SET-AGGREGATE DVs (entropy-shaped categorical distributions), each already
downstream of the argmax; TV distance is not further symmetric under any additional
manipulation this design applies -- the reef/open partition is defined by env geometry
(agent position), which is READ (not manipulated) identically in both arms, so it
carries no arm-dependent symmetry of its own to check.

Phases / budget (UNCHANGED from 654j -- same substrate state, new readout only)
---------------------------------------------------------------------------------
...
P2 (60 ep, all FROZEN -- e2 + bias head; field persists + maintains; instrumentation
   ON, including the context partition + the MECH-448/449 diagnostics): the
   behavioural measurement window.
Budget: 2 arms x 3 seeds x 350 ep x 200 steps = 420k steps total. Diagnostic; cloud
worker (machine_affinity: any; prefer cloud per the 689/654 lineage convention).

Pre-registered acceptance criteria
----------------------------------
  C1 (READINESS / non-vacuity -- protects against a false weakens; ANY C1 fail
      self-routes substrate_not_ready_requeue, NOT a verdict). C1a-f are UNCHANGED
      from 654j (same matched stack, same substrate-readiness bar):
     ...
     (d) PROPAGATION non-vacuity: paired-by-seed |mean_lateral_pfc_bias_abs(ARM_ON)
         ...
         seeds.
     (e) MECH-448 DEMOTION non-vacuity (the matched conversion constant is LIVE), on
         a majority of seeds in BOTH arms.
     (f) MECH-449 ACTIVE NO-GO non-vacuity (the matched opponency constant is LIVE
         AND SUPPRESSING), on a majority of seeds in BOTH arms.
     (g) NEW -- CONTEXT PARTITION ADEQUATE: n_reef_ticks >= MIN_CONTEXT_TICKS AND
         n_open_ticks >= MIN_CONTEXT_TICKS (`context_scorable`, ported from V3-EXQ-690)
         on a majority of seeds in BOTH arms. Below-floor (the seed-44-style
         zero-reef-visits degeneracy) means the context partition itself could not be
         read for that seed -> excluded from the paired comparison; if fewer than a
         majority of seeds clear this in BOTH arms, the whole run self-routes
         substrate_not_ready_requeue -- the context DV was never actually exercised,
         which must NOT be read as "no aliasing" (that would be exactly the
         precondition-vs-falsification confusion the 643/785 nets exist to prevent).
  C2 (PRIMARY -- the H4 discriminator, diagnostic, NOT load-bearing for any claim):
     paired-by-seed lift in tv_context_divergence of ARM_ON over ARM_OFF of at least
     TV_LIFT_MARGIN (strict-above-by-margin) on a majority of the CONTEXT-SCORABLE
     paired seeds (denominator capped by C1g).
  C3 (SECONDARY, reported not load-bearing): the INHERITED 654j pooled
     committed_class_entropy_nats lift (context for interpreting a P-D verdict against
     the rest of the fanout) + within-class-representative entropy negative control.

Overall outcome (THREE branches; diagnostic -- NEVER a MECH-309/ARC-062 verdict,
claim_ids=[])
--------------------------------------------------------------------------------
  PASS  = C1 (incl. C1g context-partition adequacy) AND C2 (context-divergence lift)
          -> H4 CONFIRMED: the pooled-marginal DV was aliasing a real context-
          conditioned effect. Motivates an `evidence` re-pose of MECH-309/ARC-062
          using the context-partitioned DV as primary (a NEW queue entry, not this
          one). Reported context: whether 654j's own pooled C2 was null on the SAME
          run informs how strong the aliasing story is.
  FAIL (C1 holds incl. C1g, C2 fails) = the context partition was adequately
          exercised on both arms but ARM_ON shows no greater cross-context divergence
          than ARM_OFF -> H4 REFUTED as the explanation for the pooled-DV null: the
          marginal-entropy DV was NOT merely aliasing a context-switch, and the
          conversion ceiling read by the 654-lineage / P-A / P-B / P-C legs is not a
          measurement artifact of this kind. Strengthens the H2 (F-dominance) / H3
          (competence) reading. non_contributory (diagnostic; no claim to weaken).
  FAIL (C1 fails, incl. C1g) = substrate not exercisable/matured, OR the context
          partition itself could not be read on a majority of seeds (context floor
          unmet) -> substrate_not_ready_requeue; the H4 question was never actually
          tested. Do NOT read as either confirming or refuting H4.

Claims: [] (diagnostic; claim_ids intentionally empty -- see GOV-FANOUT-1 in the fanout
doc: P-D is a measurement-adequacy probe discriminating WHY the pooled-DV legs read
what they read, not a same-substrate re-test of MECH-309/ARC-062 itself).
experiment_purpose = "diagnostic". Brake-exempt (a different question from P-A's
same-claim retest; per the fanout doc Section 4). PROMOTES / WEAKENS NOTHING by
itself in either branch.

See REE_assembly/evidence/planning/arc_062_conversion_fanout_2026-07-29.md (Section 2
Leg P-D design + the P-A/P-B/P-C erratum sections this build's premise correction
above matches),
REE_assembly/evidence/planning/arc_062_rule_apprehension_plan.md (GAP-B, the parent
lineage this diagnostic reads the substrate state of),
experiments/v3_exq_654j_arc062_gapb_rule_apprehension_nogo_behavioural_falsifier.py
(the matched-stack template this script is a straight adaptation of -- unchanged
training loop, unchanged C1a-f readiness, unchanged pooled DV reported as context),
experiments/v3_exq_690_q054_arc062_diversity_floor_sweep.py (source of `_is_reef_half`
+ `_tv_distance` + the context_scorable readiness pattern; DIFFERENT swept variable
(noise_floor_min_temperature), so its manifest cannot itself answer H4),
REE_assembly/evidence/planning/failure_autopsy_V3-EXQ-690_2026-06-20.md (the
seed-44-zero-reef-ticks environmental-degeneracy lesson the C1g gate guards against),
ree-v3/CLAUDE.md (MECH-448/449, modulatory-bias-selection-authority routing, ARC-062
GAP-A/B/C/D, SD-056 entries).
"""
...
EXPERIMENT_TYPE = "v3_exq_847a_arc062_pd_context_modeswitch_committed_class_divergence"
QUEUE_ID = "V3-EXQ-847a"
SUPERSEDES: Optional[str] = "V3-EXQ-847"
CLAIM_IDS: List[str] = []
...
# C1(g) NEW -- context partition adequacy (ported from V3-EXQ-690's context_scorable
# gate). Below-floor on a seed means the reef/open partition could not be READ for
# that seed (e.g. the seed-44-style zero-reef-visits degeneracy documented in
# failure_autopsy_V3-EXQ-690_2026-06-20.md) -- excluded from the C2 paired comparison
# for that seed. If fewer than a majority of seeds clear this in BOTH arms, the H4
# question was never actually exercised -> substrate_not_ready_requeue.
MIN_CONTEXT_TICKS = 10   # V3-EXQ-690-matched floor per partition (reef, open) per seed

# C2 (PRIMARY, H4 discriminator): paired-by-seed lift in tv_context_divergence of
# ARM_ON over ARM_OFF, over the context-scorable paired seeds. TV distance is bounded
# [0, 1]; TV_LIFT_MARGIN is a modest minimal-detectable-effect floor (same order as
# the pooled-DV C2_LIFT_MARGIN_NATS above, different units -- these are NOT the same
# statistic and are not meant to be numerically comparable).
TV_LIFT_MARGIN = 0.05
TV_MIN_LIFT_SEEDS = 2  # of up to 3 context-scorable paired seeds (majority; see C1g)
...
# ---------------------------------------------------------------------------
# NEW (V3-EXQ-847, H4 context-partition DV) -- ported VERBATIM (logic unchanged)
# from experiments/v3_exq_690_q054_arc062_diversity_floor_sweep.py, which already
# demonstrated both helpers are live and non-degenerate on this exact env config.
# ---------------------------------------------------------------------------
...
def _run_seed_arm(
    ...
    # NEW (V3-EXQ-847, H4 PRIMARY DV): reef/open context-partitioned committed-class
    # counts, ported from V3-EXQ-690's context_scorable pattern.
    reef_committed_class_counts: Dict[int, int] = {}
    open_committed_class_counts: Dict[int, int] = {}
    n_reef_ticks = 0
    n_open_ticks = 0
    ...
    # NEW (V3-EXQ-847, code-review fix per the fanout doc's mandatory latch-clearing
    # instruction, Section 2 preamble): e3.last_score_diagnostics is populated ONLY
    # inside e3.select(), which runs on ~1 P2 tick in heartbeat.e3_steps_per_tick
    # (default 10) -- on a held tick it still carries the PREVIOUS selection's values.
    # The demotion/nogo diagnostics below are cleared immediately before every
    # select_action() call and counted as latched (not accumulated) when the clear
    # was not overwritten, so C1e/C1f read a true per-fresh-selection denominator
    # rather than re-recording one selection ~10x (the V3-EXQ-785 defect). This does
    # NOT affect the PRIMARY DVs (committed_class / tv_context_divergence), which are
    # read from the `action` select_action() itself returns every tick (a legitimate
    # per-tick observation of the committed action, held or fresh) -- only the
    # secondary MECH-448/449 readiness diagnostics are latch-sensitive.
    n_latched_ticks = 0
    ...
    for ep in range(total_train_eps):
        ...
        for _step in range(steps_per_episode):
            ...
            # NEW (V3-EXQ-847): clear the E3 diagnostics latch immediately before every
            # select_action() call (fanout doc Section 2 preamble mandate; see the
            # n_latched_ticks declaration above for why).
            ...
            if is_p2:
                ...
                # NEW (V3-EXQ-847, H4 PRIMARY DV): partition this tick's committed
                # class by the SD-054 reef/open context geometry (read, not
                # manipulated -- identical test on both arms).
                if _is_reef_half(env):
                    n_reef_ticks += 1
                    reef_committed_class_counts[committed_class] = (
                        reef_committed_class_counts.get(committed_class, 0) + 1
                    )
                else:
                    n_open_ticks += 1
                    open_committed_class_counts[committed_class] = (
                        open_committed_class_counts.get(committed_class, 0) + 1
                    )
                ...
                # MECH-448/449 diagnostics (read LIVE at the select tick; the matched
                # conversion constants fire on BOTH arms). NEW (V3-EXQ-847): the latch
                # was cleared to None immediately before select_action() above -- a
                # None here means this tick was a HELD commitment (e3.select() did not
                # run), so it is counted as latched and NOT accumulated as a fresh
                # observation (the V3-EXQ-785 ~9x pseudo-replication fix).
                diag_raw = getattr(agent.e3, "last_score_diagnostics", None)
                if diag_raw is None:
                    n_latched_ticks += 1
                    diag: Dict[str, Any] = {}
                else:
                    diag = diag_raw
                    # C1e checks the envelope is active AND actually excludes on the
                    # divergent pool.
                    ...
                    # C1f checks the constitution is active AND actually SUPPRESSES
                    # (safety + soft No-Go removed candidates from the F-eligible set).
    ...
    # NEW (V3-EXQ-847, H4 PRIMARY DV + C1g readiness): context_scorable ported
    # verbatim from V3-EXQ-690. Below-floor => the reef/open partition could not be
    # read for this seed => tv_context_divergence reported as 0.0 (NOT a genuine
    # measurement) and this seed is excluded from the C1g/C2 seed counts downstream.
    context_scorable = bool(
        n_reef_ticks >= MIN_CONTEXT_TICKS and n_open_ticks >= MIN_CONTEXT_TICKS
    )
    tv_context_divergence = (
        _tv_distance(reef_committed_class_counts, open_committed_class_counts)
    ...
    # V3-EXQ-847a FIX (failure_autopsy_V3-EXQ-847_2026-08-01.md): the numerator
    # (demotion_active_ticks) is latch-guarded to genuine fresh E3.select() ticks
    # only (see n_latched_ticks above), but V3-EXQ-847 divided by n_p2_ticks --
    # EVERY P2 tick, including held/replayed commitments where the latch was not
    # repopulated. That is a basis mismatch (numerator subset of a denominator
    # population it doesn't match) that deflated active_frac by the held-tick
    # fraction (40-86% across 847's 6 cells), producing a false
    # substrate_not_ready_requeue self-route even though MECH-448 was 100% active
    # on genuine fresh selections. The correct denominator is the fresh-selection
    # count (n_p2_ticks - n_latched_ticks), matching the n_p2_fresh_select pattern
    # already used correctly by sibling script V3-EXQ-859.
    n_p2_fresh_select = n_p2_ticks - n_latched_ticks
    ...
    # V3-EXQ-847a FIX: same basis-mismatch correction as demotion_active_frac
    # above -- divide by fresh-selection ticks only (n_p2_fresh_select), not all
    # P2 ticks.
    nogo_active_frac = (
    ...
    return {
        ...
        # (V3-EXQ-847): count of P2 ticks where the E3 diagnostics latch was NOT
        # repopulated (a held commitment).
        "n_latched_ticks": int(n_latched_ticks),
        # NEW (V3-EXQ-847a): the true fresh-selection denominator for the
        # MECH-448/449 diagnostics, n_p2_ticks - n_latched_ticks, recorded
        # explicitly (matches V3-EXQ-859's n_p2_fresh_select convention) so the
        # corrected active_frac basis is auditable without recomputation.
        "n_p2_fresh_select": int(n_p2_fresh_select),
        "error_note": error_note,
        # ----- PRIMARY DV (H4, NEW) -----
        "n_reef_ticks": int(n_reef_ticks),
        "n_open_ticks": int(n_open_ticks),
        "context_scorable": context_scorable,
        "reef_committed_class_counts": {
            str(k): int(v) for k, v in sorted(reef_committed_class_counts.items())
        },
        "open_committed_class_counts": {
            str(k): int(v) for k, v in sorted(open_committed_class_counts.items())
        },
        "tv_context_divergence": round(tv_context_divergence, 6),
        # ----- SECONDARY: pooled marginal (654j's own PRIMARY DV, reported context) -----
...
def run_experiment(
    ...
    # C1(g) NEW (H4): context partition adequate (context_scorable) on a majority of
    # seeds in BOTH arms. COUNT-shaped, INCLUSIVE floor (same recomputability pattern
    # as C1a-f above): measured = min(n_off_context_scorable, n_on_context_scorable).
    n_off_context = sum(1 for r in off_rows if r["context_scorable"])
    n_on_context = sum(1 for r in on_rows if r["context_scorable"])
    c1g_holds = bool(
        n_off_context >= MIN_SEEDS_FOR_PASS and n_on_context >= MIN_SEEDS_FOR_PASS
    )

    c1_holds = bool(
        c1a_holds and c1b_holds and c1c_holds and c1d_holds and c1e_holds
        and c1f_holds and c1g_holds
    )

    # C2 pooled (SECONDARY, reported context -- 654j's own PRIMARY DV, inherited
    # unchanged; NOT load-bearing here, claim_ids=[]).
    ...
    for seed in sorted(set(off_by_seed) & set(on_by_seed)):
        ...
        if lift >= C2_LIFT_MARGIN_NATS:
            n_lift_seeds += 1
    pooled_lift_holds = bool(n_lift_seeds >= C2_MIN_LIFT_SEEDS)
    ...
    # C2-CONTEXT (PRIMARY, H4 discriminator, NEW): paired-by-seed lift in
    # tv_context_divergence, ARM_ON over ARM_OFF, restricted to seeds
    # context_scorable in BOTH arms (a seed unscorable in either arm cannot form a
    # meaningful pair -- its tv_context_divergence is a reported 0.0 placeholder,
    # not a genuine measurement, per the per-seed computation above).
    off_tv_by_seed = {
        int(r["seed"]): r["tv_context_divergence"] for r in off_rows if r["context_scorable"]
    }
    on_tv_by_seed = {
        int(r["seed"]): r["tv_context_divergence"] for r in on_rows if r["context_scorable"]
    }
    paired_tv_lifts: Dict[int, float] = {}
    n_tv_lift_seeds = 0
    for seed in sorted(set(off_tv_by_seed) & set(on_tv_by_seed)):
        tv_lift = on_tv_by_seed[seed] - off_tv_by_seed[seed]
        paired_tv_lifts[seed] = round(tv_lift, 6)
        if tv_lift >= TV_LIFT_MARGIN:
            n_tv_lift_seeds += 1
    c2_context_holds = bool(n_tv_lift_seeds >= TV_MIN_LIFT_SEEDS)

    off_tv_mean = _mean([r["tv_context_divergence"] for r in off_rows if r["context_scorable"]])
    on_tv_mean = _mean([r["tv_context_divergence"] for r in on_rows if r["context_scorable"]])
    ...
    # ----- Outcome map (THREE branches; diagnostic, claim_ids=[] -- NEVER a
    # MECH-309/ARC-062 verdict in either direction) -----
    if not c1_holds:
        # C1 fail (incl. C1g: the context partition itself could not be read on a
        # majority of seeds in both arms) -> the H4 question was never actually
        # exercised -> re-queue, NOT a reading of H4 either way.
        outcome = "FAIL"
        direction = "non_contributory"
        label = "substrate_not_ready_requeue"
    elif c2_context_holds:
        # H4 CONFIRMED: the context-conditioned DV shows a real ARM_ON-over-ARM_OFF
        # lift that the pooled marginal DV (reported above as context) may have
        # missed -> the pooled-DV null seen elsewhere in this fanout may be a
        # measurement-aliasing artifact, not a substrate ceiling. Motivates an
        # `evidence` re-pose with the context DV as primary (a NEW queue entry).
        outcome = "PASS"
        direction = "non_contributory"
        label = "context_conditioned_divergence_detected_motivates_evidence_repose"
    else:
        # C1 holds (incl. context partition adequately exercised) but the
        # context-conditioned DV shows NO greater ARM_ON-vs-ARM_OFF divergence than
        # the pooled DV would suggest -> H4 REFUTED: the marginal-entropy DV was NOT
        # merely aliasing a context-switch. Strengthens the H2 (F-dominance) / H3
        # (competence) reading over a measurement artifact. Pre-registered
        # informative null (see docstring); non_contributory (diagnostic, no claim).
        outcome = "FAIL"
        direction = "non_contributory"
        label = "context_divergence_not_greater_arm_on_measurement_aliasing_ruled_out"
    ...
    interpretation = {
        ...
        "preconditions": [
            ...
            {
                "name": "context_partition_adequate_both_arms",
                "kind": "readiness",
                "description": (
                    "NEW (V3-EXQ-847, H4): the reef/open context partition itself is "
                    "READABLE for a majority of seeds in BOTH arms -- n_reef_ticks >= "
                    "MIN_CONTEXT_TICKS AND n_open_ticks >= MIN_CONTEXT_TICKS "
                    "(context_scorable, ported from V3-EXQ-690). The 690 manifest "
                    "showed this degenerates for some seeds (seed 44: n_reef_ticks=0 "
                    "under 690's policy) purely from env-geometry trajectory variance, "
                    "independent of the rule-apprehension manipulation. Below-floor on "
                    "a majority of seeds means the H4 context-conditioned DV was never "
                    "actually exercisable for those seeds -> exclude them from C2-context "
                    "(the paired comparison below already restricts to jointly-scorable "
                    "seeds); if fewer than a majority clear this in EITHER arm, the whole "
                    "run self-routes substrate_not_ready_requeue (do NOT read a below-"
                    "floor run as confirming OR refuting H4)."
                ),
                "control": "SD-054 reef/open bipartite geometry, both arms (env config identical to 654j)",
                # COUNT-shaped, INCLUSIVE floor: `met` is c1g_holds, i.e.
                # `n_off_context >= MIN_SEEDS_FOR_PASS and n_on_context >=
                # MIN_SEEDS_FOR_PASS`, and min(counts) >= k iff every count >= k, so
                # `measured >= threshold` reproduces `met` EXACTLY (same pattern as
                # C1a/C1b/C1c/C1e/C1f above).
                "measured": float(min(n_off_context, n_on_context)),
                "threshold": float(MIN_SEEDS_FOR_PASS),
                "comparator": ">=",
                "direction": "lower",
                "observed_n_off_context_scorable": int(n_off_context),
                "observed_n_on_context_scorable": int(n_on_context),
                "met": bool(c1g_holds),
            },
        ],
        "criteria": [
            {
                # PRIMARY (H4 discriminator, NEW). Diagnostic purpose (claim_ids=[]):
                # "load_bearing" governs the indexer's vacuous_pass check on THIS run's
                # own outcome map, not any claim's confidence score.
                "name": "C2_context_divergence_lift",
                "load_bearing": True,
                "passed": bool(c2_context_holds),
            },
            {
                # SECONDARY (reported context; 654j's own PRIMARY DV, inherited
                # unchanged, NOT load-bearing here).
                "name": "C2_pooled_committed_class_entropy_lift",
                "load_bearing": False,
                "passed": bool(pooled_lift_holds),
            },
        ],
        "criteria_non_degenerate": {
            ...
            "C1g_context_partition_adequate": bool(c1g_holds),
            "C2_context_divergence_lift": bool(c2_context_holds),
            "C2_pooled_paired_lift": bool(pooled_lift_holds),
        },
    }
    ...
    return {
        ...
        # empty dict (not omitted) so _build_manifest's lookup below stays a no-KeyError
        ...
        "decision_rule_thresholds": {
            ...
            "tv_lift_margin": float(TV_LIFT_MARGIN),
            "tv_min_lift_seeds": int(TV_MIN_LIFT_SEEDS),
        },
        "acceptance_criteria": {
            ...
            "C1g_context_partition_adequate_both_arms": c1g_holds,
            "C1g_n_off_context_scorable": int(n_off_context),
            "C1g_n_on_context_scorable": int(n_on_context),
            # ----- PRIMARY (H4 discriminator, NEW) -----
            "C2_context_divergence_lift": c2_context_holds,
            "C2_context_n_lift_seeds": int(n_tv_lift_seeds),
            "C2_context_n_scorable_paired_seeds": int(len(paired_tv_lifts)),
            "C2_context_paired_lifts_by_seed": paired_tv_lifts,
            "C2_context_off_mean_tv_divergence": round(off_tv_mean, 6),
            "C2_context_on_mean_tv_divergence": round(on_tv_mean, 6),
            # ----- SECONDARY (654j's own PRIMARY DV, reported context, NOT load-bearing) -----
            "C2_pooled_committed_class_lift": pooled_lift_holds,
            "C2_pooled_n_lift_seeds": int(n_lift_seeds),
            "C2_pooled_paired_lifts_by_seed": paired_lifts,
            "C2_pooled_off_mean_committed_class_entropy": round(off_mean_dv, 6),
            "C2_pooled_on_mean_committed_class_entropy": round(on_mean_dv, 6),
        },
        "secondary_negative_control_not_load_bearing": {
            "note": (
                "Within-class-representative entropy is a NEGATIVE CONTROL inherited "
                "from 654j: the rule bias is class-keyed (per-candidate summary "
                "first-action-keyed; compute_bias broadcasts one rule_state across K), "
                ...
                "committed-class axis (the axis both the pooled-marginal DV and the "
                "NEW context-partitioned DV read, at different granularities)."
            ),
        ...
        "interpretation_grid": {
            "PASS_C1_C2_context_divergence_detected": (
                "The context partition was adequately exercised on a majority of "
                "seeds in BOTH arms (C1g), AND the differentiated rule_state's "
                "committed-class distribution diverges MORE strongly between the reef "
                "and open contexts than the collapsed legacy baseline's does, "
                "paired-by-seed, by at least TV_LIFT_MARGIN. H4 CONFIRMED: the "
                "pooled-marginal committed-class-entropy DV (654j's own primary, "
                "reported here as context) can be blind to a real context-conditioned "
                "effect. This is a DIAGNOSTIC finding (claim_ids=[]) -- it does NOT by "
                "itself support or weaken MECH-309/ARC-062. It motivates a NEW "
                "`evidence`-purpose re-pose of MECH-309/ARC-062 using the "
                "context-partitioned DV as primary (a separate /queue-experiment "
                "session), and it means a null pooled-DV result elsewhere in this "
                "fanout (P-A/654-lineage) should be read with more caution: it may be "
                "measuring the wrong axis, not the absence of an effect."
            ),
            "FAIL_C1_holds_C2_context_fails": (
                "The context partition was adequately exercised on a majority of "
                "seeds in BOTH arms (C1g -- the H4 question WAS actually tested), but "
                "the differentiated rule_state shows NO greater cross-context "
                "committed-class divergence than the collapsed legacy baseline, "
                "paired-by-seed. H4 REFUTED as the explanation for a pooled-DV null: "
                "the marginal-entropy DV was NOT merely aliasing a context-switch -- "
                "the conversion ceiling read elsewhere in this fanout is not a "
                "measurement artifact of THIS kind. Strengthens the H2 (F-dominance) / "
                "H3 (competence) reading. non_contributory (diagnostic; claim_ids=[], "
                "nothing to weaken). This is a pre-registered INFORMATIVE null, not a "
                "failed run -- GOV-FANOUT-1's stated price for an early, "
                "un-confounded answer."
            ),
            "FAIL_C1_substrate_not_ready_requeue": (
                "Either the inherited 654j substrate-readiness gates (C1a-f: class "
                "axis exercisable, GAP-A divergence, ARM_ON CRF matured, propagation "
                "non-vacuous, MECH-448 demotion live, MECH-449 active No-Go live and "
                "suppressing) were not met, OR (C1g, NEW) the reef/open context "
                "partition itself could not be READ on a majority of seeds in either "
                "arm (n_reef_ticks or n_open_ticks below MIN_CONTEXT_TICKS -- the "
                "seed-44-style environmental-degeneracy lesson from V3-EXQ-690). The "
                "H4 question was never actually exercised in this run -- do NOT read "
                "this as confirming OR refuting H4. Route to substrate enrichment / "
                "re-queue with a wider seed set if C1g specifically is the blocker."
            ),
        },
...
def _build_manifest(
    ...
    return {
        ...
        "evidence_direction_note": (
            f"V3-EXQ-847a (supersedes V3-EXQ-847 -- corrected active_frac denominator) "
            f"ARC-062 GOV-FANOUT-1 Leg P-D (H4: measurement aliasing --"
            f"context-conditioned mode-switch). DIAGNOSTIC, claim_ids=[]; brake-exempt "
            f"(a different question from the same-claim P-A retest). Design source: "
            f"REE_assembly/evidence/planning/arc_062_conversion_fanout_2026-07-29.md "
            f"Section 2 Leg P-D, chip chip-20260729-arc062-pd-context-modeswitch. "
            f"PREMISE CORRECTION (this build session): the plan doc's P-D design line "
            f"names 'gated_policy routing', which Leg P-A's own erratum (same doc, "
            f"appended 2026-07-31) found does NOT route the rule-apprehension/CRF "
            f"channel (it identity-routes the unrelated GatedPolicy module output). "
            f"This build OMITS that routing entirely -- P-D is a straight adaptation of "
            f"the 654j matched stack (unchanged: SP-CEM + GAP-A e2_world_forward + "
            f"MECH-448 demotion + MECH-449 active No-Go, all matched-stack constants "
            f"on BOTH arms; swept variable use_candidate_rule_field) plus a NEW "
            f"context-partitioned readout layered on top of the SAME P2 measurement "
            f"window. GOV-REUSE-1 check: no existing manifest combines the reef/open "
            f"partition (V3-EXQ-690's _is_reef_half/_tv_distance, ported verbatim) with "
            f"the use_candidate_rule_field sweep -- 690's own swept variable is "
            f"noise_floor_min_temperature, a different manipulation; not recoverable, "
            f"proceeded to a new run. PRIMARY DV (NEW) = paired-by-seed lift in "
            f"tv_context_divergence (TV distance between reef-context and open-context "
            f"committed-class distributions) of ARM_ON over ARM_OFF, restricted to "
            f"seeds context_scorable in BOTH arms (C1g, ported from 690's readiness "
            f"pattern; guards the seed-44-style zero-reef-ticks degeneracy documented "
            f"in failure_autopsy_V3-EXQ-690_2026-06-20.md). C1 (readiness) = the "
            f"INHERITED 654j non-vacuity gates C1a-f (class axis exercisable, GAP-A "
            f"divergence, ARM_ON CRF matured >= {CRF_FRAC_ACTIVE_FLOOR}, propagation "
            f"non-vacuous, MECH-448 demotion live, MECH-449 active No-Go live and "
            f"suppressing, all both arms) PLUS the NEW C1g context-partition-adequate "
            f"gate (n_reef_ticks/n_open_ticks >= {MIN_CONTEXT_TICKS} on a majority of "
            f"seeds in both arms). C2 (PRIMARY, H4 discriminator) = paired tv_context_"
            f"divergence lift >= {TV_LIFT_MARGIN} on >= {TV_MIN_LIFT_SEEDS} "
            f"context-scorable paired seeds. interpretation_label="
            f"{result['interpretation_label']}. "
            f"C1={result['acceptance_criteria']['C1_substrate_exercisable_and_manipulation_live']}, "
            f"C1g={result['acceptance_criteria']['C1g_context_partition_adequate_both_arms']}, "
            f"C2_context={result['acceptance_criteria']['C2_context_divergence_lift']}, "
            f"C2_pooled(reported, not load-bearing)="
            f"{result['acceptance_criteria']['C2_pooled_committed_class_lift']}. "
            f"C1-fail (incl. C1g) self-routes substrate_not_ready_requeue -- the H4 "
            f"question was never exercised, read as neither confirming nor refuting "
            f"H4. C1-holds/C2-context-fail self-routes "
            f"context_divergence_not_greater_arm_on_measurement_aliasing_ruled_out -- "
            f"a pre-registered INFORMATIVE null (H4 refuted; strengthens the H2/H3 "
            f"reading over a measurement artifact). C1-holds/C2-context-PASS self-"
            f"routes context_conditioned_divergence_detected_motivates_evidence_repose "
            f"-- H4 confirmed; motivates a NEW evidence-purpose re-pose of MECH-309/"
            f"ARC-062 with the context DV as primary (a separate queue entry, not this "
            f"one). ALL THREE branches are non_contributory (diagnostic, claim_ids=[]) "
            f"-- this run PROMOTES / WEAKENS NOTHING by itself in any branch."
        ),
        ...
        "config_summary": {
            ...
            "primary_dv": "tv_context_divergence: paired-by-seed TV(committed-class|reef) vs TV(committed-class|open), ARM_ON vs ARM_OFF (NEW, H4)",
            "secondary_reported_dv_not_load_bearing": "committed_class_entropy_nats pooled marginal (654j's own primary DV, inherited unchanged)",
            "secondary_negative_control": "within-class-representative entropy (expected ~null)",
            "context_partition_source": "V3-EXQ-690 _is_reef_half + _tv_distance, ported verbatim (SD-054 reef_bipartite geometry, already present in ENV_KWARGS below)",
            "min_context_ticks": int(MIN_CONTEXT_TICKS),
...
def main() -> Tuple[Optional[str], Optional[str], bool]:
    parser = argparse.ArgumentParser(description="V3-EXQ-847a ARC-062 GOV-FANOUT-1 Leg P-D (H4 context-conditioned mode-switch diagnostic; 654j matched stack + reef/open TV-divergence DV; corrected active_frac denominator, supersedes V3-EXQ-847)")
