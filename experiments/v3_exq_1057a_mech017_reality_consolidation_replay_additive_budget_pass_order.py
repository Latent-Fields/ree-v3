...
"""
V3-EXQ-1057a -- MECH-017 reality consolidation: is the RECENCY COST of offline replay
INTRINSIC to replay, or an artefact of REALLOCATING a fixed gradient budget? The same
question V3-EXQ-1057 asked, now with CONSOLIDATION PASS ORDER controlled as an explicit
factor: the three V3-EXQ-1048 arms unchanged, plus BOTH additive-budget pass orders.
...
Under a matched budget every step spent replaying old traces is a step NOT spent on
recent ones, so a recency cost is exactly what a pure reallocation would produce even
if replay itself is harmless to recency. The two hypotheses are only separable by
ADDING the replay pass to the online budget instead of substituting it.

WHY A SECOND ITERATION, AND WHAT 1057 SETTLED
----------------------------------------------
V3-EXQ-1057 built that additive arm and RAN (manifest
evidence/experiments/v3_exq_1057_..._20260918T211119Z_v3.json). It did NOT answer the
question, and it did not fail to: it gated out, cleanly and by design. 14 of its 15
preconditions passed; the one that failed was `additive_arm_retains_replay_gain` at
0 of 5 seeds against a threshold of 4.

That gate existed because this script's own red-team (fable, verdict CONTESTED,
finding F2) predicted exactly this: the additive arm ran the whole-buffer pass FIRST
and the recent-window pass LAST, and the recent-window pass is byte-identical to
ARM_B's only pass -- same window, same lr, same freshly-built per-module Adam. If it
simply overwrites the replay pass, the arm collapses onto B and C4 becomes a B-vs-B
comparison that passes trivially. The recorded numbers are worse than the prediction:

  per seed, held-out EARLY-probe E1 MSE (lower is better)
  seed |  ARM_A      ARM_B      ARM_D      (D-B)/B
    42 | 9.792e-05  1.616e-04  2.069e-04   +0.280
    45 | 9.168e-05  1.675e-04  1.878e-04   +0.121
   123 | 9.789e-05  1.819e-04  2.234e-04   +0.228
   456 | 4.185e-05  7.580e-05  8.434e-05   +0.113
  2026 | 7.190e-05  1.481e-04  1.675e-04   +0.131

ARM_A carries a large, seed-consistent replay benefit (~40% better than B on 5/5).
ARM_D did not merely LOSE it -- D came in 11% to 28% WORSE than B itself on every
seed. So this is not clean last-writer-wins (which would predict D == B); running the
two differently-windowed passes in that order actively degrades early-probe fidelity
below what the recent-window pass alone achieves.

And the gate was load-bearing, not decorative. On the LATE probes D was within
LATE_TOL of B on 4 of 5 seeds -- i.e. C4 would have PASSED and the run would have
recorded "recency_cost_is_reallocation_artefact" / `supports`, off an arm that had
lost replay entirely. The premise also replicated: A was worse than B on the late
probes on 5/5 seeds, +9.3% to +95%, matching V3-EXQ-1048's recorded range.

So 1057 established that PASS ORDER MATTERS and left the ratified question open. The
user's decision (2026-09-18, on decision chip chip-20260918-mech017-arm-d-pass-order,
recorded verbatim in the queue entry) was: let 1057 run, then run BOTH orders as
explicit arms if order turns out to matter. It did. This is that run.
  ...
  ARM_D1_ADDITIVE_WHOLE_THEN_RECENT -- V3-EXQ-1057's ARM_D exactly. At each
                                 consolidation point: ARM_A's whole-buffer pass,
                                 THEN ARM_B's recent-window pass. Replay FIRST.
                                 2 x CMC_STEPS. The ORDER CONTROL -- measured at
                                 0/5 replay-gain retention on 1057, and re-run here
                                 so the contrast with D2 is within one run, on one
                                 seed set, rather than across runs.
  ARM_D2_ADDITIVE_RECENT_THEN_WHOLE -- the reverse: ARM_B's recent-window pass,
                                 THEN ARM_A's whole-buffer pass. Replay LAST.
                                 Identical budget, identical windows, identical lr,
                                 identical schedule -- ONLY the order differs. This
                                 is the arm the load-bearing C4 is read off, because
                                 it is the order that can preserve replay's benefit
                                 and therefore the only additive arm that can answer
                                 the reallocation question.

CONFIRMING (the recency cost was a REALLOCATION ARTEFACT):
    e1_holdout_mse_late(D2) <= (1 + LATE_TOL) * e1_holdout_mse_late(B) on at least
    SIGN_CONSISTENCY_REQUIRED of the seeds, WITH D2 having passed the gate that it
    still carries replay's early-probe gain -- the deficit DISAPPEARS once the
    budget is added rather than reallocated.
FALSIFYING (the recency cost is INTRINSIC to replay):
    it does not -- D2 is still worse than B by more than LATE_TOL at the same
    majority, so replaying stale traces costs recency even when it takes nothing
    away from the online stream.
NEITHER (routes to substrate_not_ready_requeue, not to a claim verdict):
    D2 does not retain replay's gain either. Then NO pass order delivers "A's
    benefit plus B's" at this dose, and the additive construction -- not MECH-017 --
    is what needs rethinking. That is the state 1057 reached on the D1 order.

CRITERION PROVENANCE -- NOTHING NEW WAS INVENTED, AGAIN. C4 is V3-EXQ-1048's C2 with
ARM_D2 substituted for ARM_A: same DV (`e1_holdout_mse_late`), same comparator arm
(B), same LATE_TOL = 0.10, same SIGN_CONSISTENCY_REQUIRED = 4. The gain-retention
gate is V3-EXQ-1048's C1 form with the same substitution and the same majority
constant. The additive budget is 2 x CMC_STEPS because that is the only value
derivable as "A's budget plus B's". No threshold in this script was introduced,
moved, or relaxed relative to V3-EXQ-1057; the ONLY structural change is that the
single ARM_D becomes two arms differing in pass order, and that the gain-retention
gate is therefore evaluated PER ORDER ARM.
...
Waking training is IDENTICAL in all five arms: after every training episode each
...
tensors and requires all five arms of a seed to agree exactly.
...
E1/E2 WEIGHTS evaluated at fixed inputs.
  ...
  - ARM_D1 / ARM_D2: the manipulation is the ADDITION of a second,
    differently-windowed pass, and between them, the ORDER of the two. Both are
    changes in the NUMBER, COMPOSITION and SEQUENCE of gradient steps, so both move
    the weights; same reading as A/B. Note the addition is emphatically NOT an
    additive constant on the DV -- nothing is added to the measured MSE, only to the
    training budget, and the two are related through the optimizer, not by
    arithmetic. Order is likewise not a permutation of interchangeable units in the
    invariance sense: gradient steps do not commute, which is precisely the
    hypothesis under test and is why D1 and D2 are separate arms rather than a
    nuisance factor to be averaged over.
  The DV is also not a rank/argmax statistic, so the monotone-transform class is
  inapplicable to all five arms.
...
V3-EXQ-1057'S RED-TEAM (Step 4.5, model: fable), INHERITED IN FULL -- every fix
below is carried into this script unchanged, and F2 is the reason this run exists:
VERDICT **CONTESTED**, four findings, ALL verified against source,
...
----------------------------------------------------------------
V3-EXQ-1048 (FAIL, evidence_direction `mixed`, autopsied and CONFIRMED): C1 5/5, C3
mean relative gain 0.4542 against a 0.05 floor, C2 1/5 against a threshold of 4, all
12 preconditions green. Replay counters forgetting by a large margin and charges a
recency cost at matched budget.

V3-EXQ-1057 (FAIL, evidence_direction `inconclusive`, label
substrate_not_ready_requeue): 14/15 preconditions green; the sole failure is
`additive_arm_retains_replay_gain` 0/5 against a threshold of 4, on the whole-then-
recent order. Numbers in the section above. It is an INSTRUMENT result, not a
MECH-017 verdict, and it is cited here, not superseded -- its finding (that order
matters, and that the D1 order is degenerate) is load-bearing for THIS design and is
why ARM_D1 is retained as the control. It is separately owed a `/failure-autopsy`
pass; this script does not adjudicate it.

WHAT IS STILL GENUINELY UNRUN. `e1_holdout_mse_late(D2)` -- the recent-then-whole
order -- has never been measured on any substrate. Its direction is unknown and both
branches of C4 are live:
  - D2 retains replay's gain AND lands within tolerance of B on the late probes ->
    the deficit was the opportunity cost of reallocated compute.
  - D2 retains the gain but is STILL worse than B on the late probes -> the deficit
    is a property of replaying stale traces as such. A genuine weakening, and a much
    stronger one than 1048's, because both the budget confound and the pass-order
    confound have now been removed.
  - D2 does not retain the gain either -> neither order works at this dose; routes to
    substrate_not_ready_requeue (see NEITHER above).
No threshold in this script was chosen or moved with any knowledge of D2's value.

Run with:
  /opt/local/bin/python3 experiments/v3_exq_1057a_mech017_reality_consolidation_replay_additive_budget_pass_order.py --dry-run
  /opt/local/bin/python3 experiments/v3_exq_1057a_mech017_reality_consolidation_replay_additive_budget_pass_order.py
"""
...
EXPERIMENT_TYPE = "v3_exq_1057a_mech017_reality_consolidation_replay_additive_budget_pass_order"
QUEUE_ID = "V3-EXQ-1057a"
...
ARM_D1 = "ARM_D1_ADDITIVE_WHOLE_THEN_RECENT"
ARM_D2 = "ARM_D2_ADDITIVE_RECENT_THEN_WHOLE"
...
def run_cell(
    ...
    def _consolidation_point(block: str) -> None:
        ...
        ARM_D1 and ARM_D2 are the ADDITIVE-budget arms: at each point each takes
        ARM_A's whole-buffer pass AND ARM_B's recent-window pass, as two
        INDEPENDENT `consolidator.consolidate(...)` calls run SEQUENTIALLY --
        never one accumulated backward. They differ ONLY in ORDER: D1 runs replay
        FIRST (whole -> recent), D2 runs replay LAST (recent -> whole). Budget,
        windows, lr and schedule are identical, which is what makes D1-vs-D2 a
        clean isolation of the ordering factor. `_consolidate` wraps each call in
        its own `_BufferWindow`, so each pass has fully exited the context manager
        (buffers restored) before the next one begins; neither pass can inherit
        the other's narrowing, in either order. Total extra budget per point is
        exactly 2 x cmc_steps = A's budget + B's, for both arms.
        """
        if arm == ARM_C:
            ...
            return
        if arm == ARM_D1:
            # 1057's ARM_D exactly: replay FIRST, recent-window LAST.
            _one_pass(block, "whole_buffer", None)            # ARM_A's pass
            _one_pass(block, "recent_window", recent_window)  # ARM_B's pass
            return
        if arm == ARM_D2:
            # The reverse: recent-window FIRST, replay LAST. Identical budget,
            # identical windows, identical lr -- ONLY the order differs, which is
            # what makes D1-vs-D2 a clean isolation of the ordering factor.
            _one_pass(block, "recent_window", recent_window)  # ARM_B's pass
            _one_pass(block, "whole_buffer", None)            # ARM_A's pass
            return
    ...
    window_label = {ARM_A: "ALL", ARM_B: str(recent_window), ARM_C: "none",
                    ARM_D1: "ALL->" + str(recent_window),
                    ARM_D2: str(recent_window) + "->ALL"}[arm]
...
def main(dry_run: bool = False):
    ...
    d1_rows = [rows[(ARM_D1, s)] for s in seeds]
    d2_rows = [rows[(ARM_D2, s)] for s in seeds]
    all_rows = a_rows + b_rows + c_rows + d1_rows + d2_rows
    ...
    budget_d1 = _mean([r["total_extra_gradient_steps"] for r in d1_rows])
    budget_d2 = _mean([r["total_extra_gradient_steps"] for r in d2_rows])
    # BOTH order arms carry the identical additive dose; the gate takes the worse.
    additive_budget_error = max(abs(budget_d1 - (budget_a + budget_b)),
                                abs(budget_d2 - (budget_a + budget_b)))
    ...
    def _retains_replay_gain(arm_name: str) -> int:
        """Seeds where this order arm still carries replay's early-probe benefit."""
        return sum(
            1 for s_ in seeds
            if rows[(arm_name, s_)]["e1_holdout_mse_early"]
            < rows[(ARM_B, s_)]["e1_holdout_mse_early"]
        )

    # EVALUATED PER ORDER ARM, and deliberately gating on D2 ONLY. V3-EXQ-1057
    # MEASURED this at 0/5 for the whole-then-recent order, so a gate that ranged
    # over both arms would fire on D1 every time and void D2 -- destroying the very
    # comparison this run exists to make. D1's value is REPORTED (it is the order
    ...
    # The WORSE of the two order arms -- strictly stronger than 1057's single-arm
    # form, since a bit-identical pair in EITHER order arm is a dead manipulation.
    n_seeds_additive_dv_moved = min(_dv_moved(ARM_D1), _dv_moved(ARM_D2))
    # Order must actually CHANGE something, or D1 and D2 are the same run.
    n_seeds_order_separates = sum(
        1 for s_ in seeds
        if rows[(ARM_D1, s_)]["e1_holdout_mse_early"]
        != rows[(ARM_D2, s_)]["e1_holdout_mse_early"]
    )
    ...
    try:
        preconditions = p0_readiness_gate([
            ...
            {"name": "additive_budget_is_a_plus_b", "measured": additive_budget_error,
             "threshold": 0.5, "direction": "upper",
             "control": "worst over the two order arms of |extra gradient steps - (ARM_A + "
                        "ARM_B)|. Both order arms are deliberately "
                        ...
                        "Ranges over BOTH order arms (the worse of the two), since they carry the "
                        "identical additive dose and differ only in the order it is spent. "
                        f"Measured: D1={budget_d1:.0f}, D2={budget_d2:.0f}, A={budget_a:.0f}, "
                        f"B={budget_b:.0f}."},
            {"name": "additive_arm_retains_replay_gain_d2",
             "measured": float(n_seeds_d2_retains_replay_gain),
             "threshold": float(SIGN_CONSISTENCY_REQUIRED), "direction": "lower",
             "control": "seeds with e1_holdout_mse_early(ARM_D2) < e1_holdout_mse_early(ARM_B) -- "
                        "V3-EXQ-1048's C1 form with ARM_D2 substituted for ARM_A, same comparator "
                        "arm and same seed-majority constant. ARM_D2 is only an ADDITIVE-BUDGET "
                        "arm if it is still doing the REPLAY the budget was added for, and the "
                        "load-bearing C4 is read off D2, so this gate protects the verdict. "
                        "GATES ON D2 ONLY, on purpose: V3-EXQ-1057 MEASURED the whole-then-recent "
                        "order at 0/5 on this exact quantity, so a gate ranging over BOTH order "
                        "arms would fire on D1 every time and void D2 -- destroying the very "
                        "comparison this run exists to make. D1's counterpart is REPORTED below "
                        "and in the readout, never gated.",
             "arm_d1_reported_not_gated": float(n_seeds_d1_retains_replay_gain),
             "arm_d1_mean_early": _mean([r["e1_holdout_mse_early"] for r in d1_rows]),
            ...
            {"name": "pass_order_separates_the_arms",
             ...
             "threshold": float(len(seeds)), "direction": "lower",
             "control": "seeds where ARM_D1 and ARM_D2 produced DIFFERENT early-probe E1 MSE. D1 "
                        "and D2 run the SAME two passes with the SAME budget, windows and lr, "
                        "differing ONLY in order, so a bit-identical pair would mean the ordering "
                        "factor this run isolates does not exist in the instrument -- a dead "
                        "manipulation, not a finding about order.",
             "d1_minus_d2_mean_early": (
                 _mean([r["e1_holdout_mse_early"] for r in d1_rows])
                 - _mean([r["e1_holdout_mse_early"] for r in d2_rows]))},
            {"name": "additive_manipulation_reaches_dv",
             "measured": float(n_seeds_additive_dv_moved),
             "threshold": float(len(seeds)), "direction": "lower",
             "control": "min over the two order arms of the seeds where that arm and ARM_B "
                        "produced DIFFERENT LATE-probe E1 MSE -- the exact leg the load-bearing "
                        "C4 reads. Same form and same threshold as "
                        "manipulation_reaches_dv above, re-pointed at the compared pairs: a "
                        "bit-identical pair is a dead manipulation, and would otherwise be "
                        "scored as a clean C4 PASS ('D is not worse than B') on no manipulation "
                        "at all. Takes the WORSE of the two order arms (min), so it is strictly "
                        "stronger than V3-EXQ-1057's single-arm form."},
            ...
            {"name": "probe_set_identical_across_arms", "measured": float(n_seeds_probe_match),
             ...
             "control": "sha256 over each cell's concatenated probe [z_self, z_world] tensors; all "
                        "five arms of a seed must agree EXACTLY, which is what makes the probe set "
    ...
    early_d1 = [r["e1_holdout_mse_early"] for r in d1_rows]
    late_d1 = [r["e1_holdout_mse_late"] for r in d1_rows]
    early_d2 = [r["e1_holdout_mse_early"] for r in d2_rows]
    late_d2 = [r["e1_holdout_mse_late"] for r in d2_rows]
    ...
    # THE load-bearing leg, read off ARM_D2 -- the order that PRESERVES replay, and
    # therefore the only additive arm that can answer the reallocation question.
    n_d2_not_worse_late = sum(
        1 for x, y in zip(late_d2, late_b) if x <= y * (1.0 + LATE_TOL)
    )
    c4 = bool(n_d2_not_worse_late >= SIGN_CONSISTENCY_REQUIRED)
    # The SAME statistic on the other order, reported so the two are comparable.
    # V3-EXQ-1057 scored 4/5 here on D1 -- which would have PASSED, on an arm that had
    # lost replay entirely. That near-miss is exactly why C4 is gated on gain retention.
    n_d1_not_worse_late = sum(
        1 for x, y in zip(late_d1, late_b) if x <= y * (1.0 + LATE_TOL)
    )
    ...
    criteria = [
        {"name": "C4_additive_budget_no_cost_on_late_probes", "load_bearing": True,
         "passed": c4, "measured": float(n_d2_not_worse_late),
         "threshold": float(SIGN_CONSISTENCY_REQUIRED), "comparator": ">=",
         "detail": f"seeds with e1_holdout_mse_late(D2) <= (1+{LATE_TOL})*e1_holdout_mse_late(B), "
                   f"where D2 is the RECENT-then-WHOLE order (replay last). "
                   ...
                   "seed-majority constant -- with ARM_D2 substituted for ARM_A. Read off D2 and not D1 because "
                   "V3-EXQ-1057 measured the D1 order at 0/5 on replay-gain retention: on D1 this "
                   "same statistic scored 4/5 and WOULD have passed, on an arm that had lost "
                   "replay entirely. The gate additive_arm_retains_replay_gain_d2 is what makes "
                   "a D2 pass mean 'replay was added at no recency cost' rather than 'replay was "
                   "erased, so there was nothing to cost anything'."},
    ...
    criteria_non_degenerate = {
        "C4_additive_budget_no_cost_on_late_probes": bool(
            len({round(x, 12) for x in late_d2 + late_b}) > 1
        ),
    ...
    degeneracy = check_degeneracy({
        ...
        "e1_holdout_mse_late_additive_d2": {
            "groups": [[d, b] for d, b in zip(late_d2, late_b)]},
        "e1_holdout_mse_early_pass_order": {
            "groups": [[x, y] for x, y in zip(early_d1, early_d2)]},
    })
    ...
    if not c2:
        ...
        if all_pass:
            label = "recency_cost_is_reallocation_artefact_when_replay_runs_last"
            direction = "supports"
        else:
    ...
    note = (
        ...
        f"gradient budget, with PASS ORDER as an explicit factor. Both order arms take "
        f"ARM_A's whole-buffer consolidation pass AND ARM_B's recent-window pass at every "
        f"consolidation point, as two sequential independent consolidate() calls, differing "
        f"ONLY in order: ARM_D1 = whole->recent (replay first), ARM_D2 = recent->whole "
        f"(replay last). D1={budget_d1:.0f} and D2={budget_d2:.0f} extra steps vs "
        f"A={budget_a:.0f}, B={budget_b:.0f}; worst |D-(A+B)|={additive_budget_error:.1f}. "
        f"LATE-probe held-out E1 MSE, the load-bearing leg, read off D2: within "
        f"(1+{LATE_TOL}) of B on {n_d2_not_worse_late}/{len(seeds)} seeds "
        f"(threshold {SIGN_CONSISTENCY_REQUIRED}). "
        ...
        f"PASS ORDER, the factor this run isolates: ARM_D2 (recent->whole, replay LAST) "
        f"retained replay's early-probe gain over B on "
        f"{n_seeds_d2_retains_replay_gain}/{len(seeds)} seeds and is the arm C4 is read off; "
        f"ARM_D1 (whole->recent, replay FIRST) retained it on "
        f"{n_seeds_d1_retains_replay_gain}/{len(seeds)} seeds -- REPORTED, never gating, and "
        f"V3-EXQ-1057 measured that same order at 0/5. Mean early relative gain over B: "
        f"D2 {rel_gain_early_d2:+.4f}, D1 {rel_gain_early_d1:+.4f}, A {rel_gain_early:+.4f}. "
        f"Late relative cost vs B: D2 {rel_cost_late_d2:+.4f} "
        f"({n_d2_not_worse_late}/{len(seeds)} within tolerance), D1 {rel_cost_late_d1:+.4f} "
        f"({n_d1_not_worse_late}/{len(seeds)}), A {rel_cost_late_a:+.4f}. "
    ...
    manifest.update({
        ...
        "interpretation": {
            "label": label,
            "criteria": criteria,
            "combination_rule": "PASS iff the single LOAD-BEARING criterion C4 passes (plain all() over the load_bearing subset, no any()/OR branching), where C4 is read off ARM_D2 -- the recent-then-whole order, i.e. the one that runs replay LAST and therefore preserves it. C1/C2/C3 are retained with load_bearing:false as replication of V3-EXQ-1048's matched-budget legs and CANNOT move the verdict. The A-vs-B gradient-budget match, the assertion that both order arms' budgets are exactly A+B, the assertion that the two orders actually separate, and the requirement that ARM_D2 still carries replay's early-probe gain, are all PRECONDITIONS, so any of them failing routes to substrate_not_ready_requeue instead of to a MECH-017 verdict. The corresponding gain-retention figure for ARM_D1 is REPORTED and never gates: V3-EXQ-1057 measured that order at 0/5, so gating on it would void ARM_D2 and destroy the comparison this run exists to make.",
        ...
        "readout": _flat_scalar({
            ...
            "extra_gradient_steps_d1": budget_d1,
            "extra_gradient_steps_d2": budget_d2,
            "additive_budget_error_worst_d_minus_a_plus_b": additive_budget_error,
            "n_seeds_d2_not_worse_late": float(n_d2_not_worse_late),
            "n_seeds_d1_not_worse_late": float(n_d1_not_worse_late),
            "n_seeds_additive_dv_moved": float(n_seeds_additive_dv_moved),
            "n_seeds_pass_order_separates": float(n_seeds_order_separates),
            "rel_cost_late_d2_over_b": rel_cost_late_d2,
            "rel_cost_late_d1_over_b": rel_cost_late_d1,
            "rel_cost_late_a_over_b": rel_cost_late_a,
            "rel_gain_early_d2_over_b": rel_gain_early_d2,
            "rel_gain_early_d1_over_b": rel_gain_early_d1,
            "mean_e1_holdout_mse_early_d1": _mean(early_d1),
            "mean_e1_holdout_mse_early_d2": _mean(early_d2),
            "mean_e1_holdout_mse_late_d1": _mean(late_d1),
            "mean_e1_holdout_mse_late_d2": _mean(late_d2),
            "mean_e2_selfforward_holdout_mse_late_d1": _mean(
                [r["e2_selfforward_holdout_mse_late"] for r in d1_rows]),
            "mean_e2_selfforward_holdout_mse_late_d2": _mean(
                [r["e2_selfforward_holdout_mse_late"] for r in d2_rows]),
            ...
            "mean_replay_early_share_whole_buffer_pass_d1": _mean(
                [r["replay_early_regime_share_whole_buffer_pass"] for r in d1_rows]),
            "mean_replay_early_share_whole_buffer_pass_d2": _mean(
                [r["replay_early_regime_share_whole_buffer_pass"] for r in d2_rows]),
            "c4_pass": c4,
            ...
            "min_e1_skill_over_persistence_late_d1": min(
                r["e1_skill_over_persistence_late"] for r in d1_rows),
            "min_e1_skill_over_persistence_late_d2": min(
                r["e1_skill_over_persistence_late"] for r in d2_rows),
            "min_per_seed_best_skill_late_d2b": min(
                max(rows[(ARM_D2, s_)]["e1_skill_over_persistence_late"],
        ...
        "per_seed_results": [
            {
                ...
                "e1_early_d1": rows[(ARM_D1, s)]["e1_holdout_mse_early"],
                "e1_early_d2": rows[(ARM_D2, s)]["e1_holdout_mse_early"],
                "e1_late_d1": rows[(ARM_D1, s)]["e1_holdout_mse_late"],
                "e1_late_d2": rows[(ARM_D2, s)]["e1_holdout_mse_late"],
                "e2_late_d1": rows[(ARM_D1, s)]["e2_selfforward_holdout_mse_late"],
                "e2_late_d2": rows[(ARM_D2, s)]["e2_selfforward_holdout_mse_late"],
                "e2_late_b": rows[(ARM_B, s)]["e2_selfforward_holdout_mse_late"],
                "extra_steps_d1": rows[(ARM_D1, s)]["total_extra_gradient_steps"],
                "extra_steps_d2": rows[(ARM_D2, s)]["total_extra_gradient_steps"],
