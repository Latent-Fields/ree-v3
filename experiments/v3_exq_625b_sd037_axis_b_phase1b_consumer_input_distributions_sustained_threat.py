...
"""
V3-EXQ-625b: SD-037 axis (b) Phase 1b substrate-readiness diagnostic
(SUPERSEDES V3-EXQ-625 -- affective-harm-stream-disabled measurement artifact).

ROOT-CAUSE FIX (diagnosed 2026-06-01, /diagnose-errors):
  V3-EXQ-625 reported a vacuous headline PASS while acceptance_pass=false with
  z_harm_a_norm identically 0.0 across all 1027 ticks / 3 seeds -- even on the
  17 external-hazard-event ticks in seed 42. The cause was NOT "harm magnitude
  below the affective noise floor" (the plan's Section 5.1 hypothesis). It was a
  CONFIG defect: the measurement reads agent._current_latent.z_harm_a, which is
  None unless cfg.latent.use_affective_harm_stream=True. ARM_PHASE1B_BASELINE
  sets gap4_operating=True -> build_config routed to REEConfig.goal_stream(),
  which never forwarded the SD-011 flags, so the AffectiveHarmEncoder was never
  instantiated and z_harm_a stayed None (every consumer-input quantity read
  exactly 0.0). The env emits harm_obs_a[50] and StepHarness threads it; only the
  config flag was off. V3-EXQ-620 (axis a) shared the identical gap4 path, so its
  zero distributions -- the result that justified this axis-b plan -- were the
  same artifact (see also V3-EXQ-620b).
  ...
  default False keeps every other gap4 caller bit-identical). Outcome is now tied
  to the substrate-readiness acceptance gate (Section 3.4) instead of the vacuous
  "ran cleanly" gate, so a zero-distribution run can no longer report PASS.
...
QUEUE_ID = "V3-EXQ-625b"
SUPERSEDES = "V3-EXQ-625"
CLAIM_IDS: List[str] = []
EXPERIMENT_PURPOSE = "diagnostic"
...
def run_experiment(
    *,
    ...
    # V3-EXQ-625b reporting fix: the run-level outcome is now TIED to the
    # substrate-readiness acceptance gate (Section 3.4), not the vacuous
    # "ran cleanly + all distributions populated" gate that let V3-EXQ-625
    # report a headline PASS while acceptance_pass=false. The data-populated
    # check is retained as a precondition so a crashed/empty run still FAILs.
    ...
    return {
        ...
        "outcome": outcome,
        "outcome_basis": "acceptance_gate_section_3_4 (C1 AND C2 AND C3) AND all six pooled distributions populated",
...
def main(argv: Optional[List[str]] = None):
    ...
    manifest: Dict[str, Any] = {
        ...
        "supersedes_run_type": "v3_exq_625_sd037_axis_b_phase1b_consumer_input_distributions_sustained_threat",
