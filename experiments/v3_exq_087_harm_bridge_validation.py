...
Tests the new HarmBridge class (ree_core/latent/stack.py): a 2-layer MLP that maps
z_world -> z_harm directly, enabling counterfactual harm queries without raw harm_obs.

The EXQ-058c pipeline used nn.Linear(world_dim, harm_obs_dim) as an intermediate step
(z_world -> harm_obs_approx -> harm_enc -> z_harm). HarmBridge collapses this:
    z_harm_cf = harm_bridge(E2.world_forward(z_world, a_cf))
rather than:
    z_harm_cf = harm_enc(harm_bridge_linear(E2.world_forward(z_world, a_cf)))

Protocol (three phases):
  Phase 1 -- terrain training (300 eps): train E2.world_forward, HarmEncoder,
    HarmBridge. HarmBridge trained on MSE(HarmBridge(z_world), HarmEncoder(harm_obs).detach()).
  Phase 2 -- E3 calibration (150 eps, frozen terrain): train harm_eval_z_harm_head
    with stratified replay (none/approach/contact).
  Phase 3 -- attribution eval (100 eps): freeze all, measure causal signature.
...
PASS criteria (ALL):
  ...
  C5: harm_bridge_alignment_r2 > 0.5  (HarmBridge predicts HarmEncoder output)
"""
...
EXPERIMENT_TYPE = "v3_exq_087_harm_bridge_validation"
...
def run(
    ...
    **kwargs,
    ...
    print(
        f"[V3-EXQ-087] HarmBridge validation (SD-010 / SD-003)\n"
        f"  HarmBridge: z_world({world_dim}) -> z_harm({Z_HARM_DIM})\n"
        f"  Pipeline: z_harm_cf = harm_bridge(E2.world_forward(z_world, a_cf))\n"
    ...
    opt_harm    = optim.Adam(harm_enc.parameters(), lr=1e-3)
    ...
    for ep in range(phase1_episodes):
        ...
        for _ in range(steps_per_episode):
            ...
            # Train HarmEncoder on normalized proximity label
        ...
        for _ in range(steps_per_episode):
            with torch.no_grad():
                ...
                for ci in range(num_actions):
                    ...
                    z_harm_cf = harm_bridge(z_w_cf)  # HarmBridge: z_world -> z_harm
    ...
    print(f"\nV3-EXQ-087 verdict: {status}  ({n_met}/5)", flush=True)
    ...
    summary_markdown = f"""# V3-EXQ-087 -- HarmBridge Counterfactual Validation
...
HarmBridge is a 2-layer MLP: z_world({world_dim}) -> z_harm({Z_HARM_DIM}).
