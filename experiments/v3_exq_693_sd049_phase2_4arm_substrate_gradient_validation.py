"""
V3-EXQ-693 -- SD-049 Phase-2 4-ARM SUBSTRATE-GRADIENT behavioural validation on a
scaffolded_sd054_onboarding CURRICULUM-BUILT agent. The V3-EXQ-514l successor
(NEW number: the 514 letter space is exhausted through 514t).

WHY A NEW NUMBER / WHAT THIS IS: SD-049's falsifiable spec (claims.yaml SD-049) pre-registers
a 4-arm substrate gradient -- ARM_0 OFF / ARM_1 2-type homeostatic / ARM_2 3-type-with-novelty
(the default) / ARM_3 5-type overshoot -- with four lifts: goal_resource_r (ARM_2-ARM_0 >= 0.4),
wanting!=liking trajectory fraction (>= 0.6 ARM_2, near-zero ARM_0/ARM_1), per-axis drive ANOVA
on the z_goal cluster (significant in ARM_2), and encoder identity-recovery accuracy (> 0.6 ARM_2,
at-chance ARM_0). The 514j/k/l runs FAILED this on the reef substrate from a ~0.2% consumption
rate (23 vs 818 neighborhood samples; failure_autopsy_V3-EXQ-514l_2026-06-03) -- a foraging-
competence substrate ceiling, NOT a falsification. The 514l autopsy + the 514m/514n lineage then
established TWO things this experiment depends on:
  (1) The scaffolded_sd054_onboarding curriculum (603n config) LIFTS that ceiling: V3-EXQ-514m on
      the curriculum reached n=3455 identity samples, C_ID probe 0.926, C_ANOVA F~1019 -- all
      GENUINE and contact-guarded. 603n PASSED its readiness gate 2026-06-11
      (substrate_queue.scaffolded_sd054_onboarding.ready flipped true). So the consumption
      degeneracy that sank 514j/k/l is fixed by building the agent through the FULL curriculum.
  (2) The 514m FAIL was a VACUOUS C_WL=0.0 (failure_autopsy_V3-EXQ-514m): the legacy SD-014
      residue VALENCE_WANTING/VALENCE_LIKING magnitude read is substrate-degenerate (channels
      never written -> |w-l|>0.1 can never fire, independent of behaviour). The substrate-faithful
      read (V3-EXQ-514n) is the SD-057 OBJECT-BOUND dissociation: WANTING target = the MECH-346
      most-wanted z_goal pointer (IncentiveTokenBank.most_wanted), LIKING target = the last-consumed
      type. This run uses that read, with the same-statistic WL non-vacuity readiness gate 514n
      introduced.
The 514n-t lineage dropped the 4-arm structure to focus on the MECH-229/MECH-436 object-bound /
drive-coupling test (514t tests MECH-436, NOT SD-049's 4-arm validation). This experiment is the
distinct, still-owed SD-049/SD-015 4-arm SUBSTRATE-GRADIENT validation, done on the foraging-
competent curriculum with the WL read fixed and the 514 C5 metric defect corrected.

WHAT THIS BUILDS (per goal_pipeline_plan.md GAP-2 Stage B + the 603n substrate harness):
  For EACH arm (ARM_0/1/2/3) and EACH seed, build a fresh REEAgent sized to that arm's env and
  train it through the FULL scaffolded_sd054_onboarding curriculum at the 603n config
  (Stage-0 -> Stage-0b -> P0 -> Stage-H -> P1 -> P2-guard), then run a frozen-policy SD-049
  Phase-2 behavioural eval on that arm's SD-049-enabled (per arm) P2 env.

ARM RENDERING (matched to the V3-EXQ-513 / V3-EXQ-514l validated arm configs):
  ARM_0  SD-049 OFF       cue bridge OFF -> legacy single-resource env (the negative control:
                          no resource identity to encode, no wanting/liking to dissociate).
  ARM_1  2-type           cue bridge ON, scaffold_cue_n_resource_types=2 (homeostatic only).
  ARM_2  3-type+novelty   cue bridge ON, scaffold_cue_n_resource_types=3 (the default; PRIMARY
                          measurement arm; food/water/novelty, novelty = non-homeostatic channel).
  ARM_3  5-type overshoot cue bridge ON, scaffold_cue_n_resource_types=5.
  The FORAGING-COMPETENCE curriculum (harm-pathway training + Stage-H survival + reef-spawn
  weaning + 634c seeding + SD-057 cue bridge) is held CONSTANT across ARM_1/2/3; ARM_0 turns the
  SD-049 substrate (and therefore its cue lever) off because there is no resource identity to
  perceive. CONFOUND MITIGATION: the goal_resource_r lift uses a non-saturating IDENTITY-
  DISCRIMINATION MARGIN (below), which is ~0 in ARM_0 by ABSENCE OF TYPE STRUCTURE regardless of
  the cue lever; and the per-arm consumption non-vacuity guard (R1) requires ARM_0 AND ARM_2 to
  forage+consume above a floor, so a foraging-collapsed ARM_0 self-routes substrate_not_ready_requeue
  rather than manufacturing a false lift.
...
constants, NOT post-hoc):
  ...
  C_GR    goal_resource_r as an IDENTITY-DISCRIMINATION MARGIN (the 514 C5 FIX): at goal-active
          steps, margin = mean_i[ cos(z_goal_i, mean_zres[type=t_i]) - mean_{t'!=t_i} cos(z_goal_i,
          mean_zres[type=t']) ], where t_i is the neighborhood-dominant type. NON-SATURATING:
          ARM_0 (no type structure) -> margin ~0 (no diff-type set); a z_resource-encoder-driven
          z_goal that points at the right type -> margin > 0. Avoids the 514l C5 defect where the
          legacy cosine(z_goal, z_world) saturated ARM_0 to ~1.0 making a +0.4 lift impossible.
          Lift criterion: margin_ARM2 - margin_ARM0 >= GR_LIFT (0.4, the SD-049-spec value applied
          to the non-saturating margin). The legacy saturating cosine is reported as a DIAGNOSTIC
          only.                                                                       [SD-049 + SD-015]
  C_WL    object-bound wanting!=liking dissociation fraction (SD-057 read; the 514m vacuity FIX):
          fraction of scored consumption events where MECH-346 most-wanted type != last-consumed
          type. ARM_2 mean >= WL_FRACTION (0.6, the SD-049-spec value; near-zero in ARM_0/ARM_1
          reported advisory).                                                          [SD-049]
  C_ANOVA per-axis homeostatic drive one-way ANOVA (z_goal-cluster proxy): F > F_CRIT_P01 (4.605
          = F(0.01, df1=n_axes-1, df2~inf)) in ARM_2 (scipy-free pre-registered critical).  [SD-049]

THREE NON-VACUITY SELF-ROUTE GUARDS (the load-bearing safety; an unmet precondition self-routes
substrate_not_ready_requeue, FAIL, BOTH claims non_contributory -- NEVER a false weakens):
  R1 CONSUMPTION (fixes the 514l ~0.2% / 23-vs-818 degeneracy): ARM_0 AND ARM_2 each have
     >= MIN_FRACTION (2/3) seeds that (a) pass the per-seed P2 contact guard (contact_rate > 0 AND
     z_goal_norm_at_contact_peak > P2_ZGOAL_GATE) AND (b) reach behav_contact_rate > CONSUMPTION_FLOOR.
  R2 IDENTITY-PROBE-FIREABLE: ARM_2 pooled neighborhood identity samples >= N_ID_SAMPLES_FLOOR, so
     C_ID gets a fair test (the 514l C2c "23<30" starvation guard).
  R3 WL-CHANNEL-FIREABLE (the 514n same-statistic gate): leg-1 positive control -- a constructed
     2-token bank from THIS run's GoalConfig must produce a drive-favored most-wanted DIFFERING
     from a designated last-consumed type on 2 deterministic probes (pc_separation == 1.0); AND
     leg-2 in-run population -- ARM_2 has >= MIN_FRACTION seeds where the bank reached >= 2 distinct
     tokens AND >= MIN_SCORED_STEPS scored WL events AND a per-axis-drive spread > DRIVE_SPREAD_FLOOR.
     Catches the exact 514m gap (the WL instrument present but never written this run).
...
PER-CLAIM EVIDENCE DIRECTION (len(claim_ids) > 1 -> evidence_direction_per_claim REQUIRED):
  ...
  Non-vacuity met:
    SD-015 = supports if (C_ID and C_GR); mixed if exactly one; weakens if NEITHER (the encoder
             neither encodes identity nor drives navigation on a forage-competent rich substrate).
    SD-049 = supports if (C_ID and C_ANOVA and C_GR and C_WL); mixed if some-but-not-all (a
             WL-flat-only outcome is the SD-049 falsifier-branch substrate_conditional re-route
             candidate, NOT a standalone weakens -- routed by /failure-autopsy); weakens only if
             ALL FOUR discrimination criteria fail on a forage-competent rich substrate.

claim_ids: SD-049, SD-015
experiment_purpose: evidence
predecessor: V3-EXQ-514l (SD-049 Phase-3 4-arm; FAIL/non_contributory -- foraging-competence
             ceiling now lifted by the onboarding curriculum) / V3-EXQ-514m (curriculum-built;
             vacuous C_WL FIX adopted here from V3-EXQ-514n).
...
EXPERIMENT_TYPE = "v3_exq_693_sd049_phase2_4arm_substrate_gradient_validation"
QUEUE_ID = "V3-EXQ-693"
...
# --- 4-arm substrate gradient (matched to the V3-EXQ-513 / 514l validated arm configs) ---
...
# --- Goal-pipeline / encoder dims (mirror 603n / 514m / 514n exactly) ---
WORLD_DIM = 32
HARM_A_DIM = 16
HARM_OBS_A_DIM = 7
HARM_HISTORY_LEN = 10
...
# --- Curriculum budgets (mirror 603n / 514m / 514n exactly) ---
STAGE0_BUDGET = 20
STAGE0B_BUDGET = 10
P0_BUDGET = 100
...
def _make_config(env) -> REEConfig:
    """Mirror 514n: full SD-049 Phase-2 + SD-057 substrate. z_resource encoder on (SD-015);
...
def _neighborhood_dominant_type(obs_dict: Dict[str, Any], type_names: Tuple[str, ...]) -> int:
    ...
    signal). Mirrors V3-EXQ-514l / 514m."""
    if not type_names:
        return -1
...
def _identity_recovery_probe(z_samples: List[np.ndarray], targets: List[int]) -> Tuple[float, int]:
    ...
    Mirrors V3-EXQ-514l / 514m identity_recovery_probe."""
    n = len(z_samples)
    if n < 10:
        return 0.0, n
...
def _one_way_anova_f(groups: List[np.ndarray]) -> Tuple[float, int, int]:
    ...
    scipy-free; gate uses a pre-registered F-critical. Mirrors V3-EXQ-514m."""
...
def _run_arm_eval(agent, scaffold_cfg, device: torch.device, n_eps: int, n_axes: int) -> Dict[str, Any]:
    ...
    (object-bound most-wanted vs last-consumed), C_ANOVA (per-axis drive). z_goal is refreshed at
    genuine contact via agent.update_z_goal (the substrate's own L2/L4 path)."""
    ...
    for _ep in range(n_eps):
        ...
        for _step in range(steps_per_ep):
            ...
            # --- post-step contact: bind token + seed z_goal, then read WL targets ---
...
def run_experiment(dry_run: bool = False) -> Dict[str, Any]:
    ...
    print(f"[{EXPERIMENT_TYPE}] per_claim SD-049={dir_sd049} SD-015={dir_sd015}", flush=True)
    ...
    return {
        ...
        "interpretation": {
            ...
            "non_vacuity_note": "R1 consumption + R2 identity-probe-fireable + R3 WL-channel-fireable "
                                ...
                                "(FAIL; both claims non_contributory), NEVER a false weakens. A WL-flat-"
                                "only outcome with non-vacuity met is the SD-049 falsifier-branch "
                                "substrate_conditional re-route candidate (route via /failure-autopsy), "
                                "NOT a standalone weakens.",
        },
        "per_run": per_run,
    }
...
def main(dry_run: bool = False) -> Dict[str, Any]:
    ...
    manifest = {
        ...
        "substrate": "scaffolded_sd054_onboarding full curriculum (Stage-0 -> Stage-0b -> P0 -> "
                     ...
                     "substrate gradient OFF / 2-type / 3-type+novelty / 5-type.",
        ...
        "method_note": "Per arm + seed: build an agent through the full onboarding curriculum on that "
                       ...
                       "wanting!=liking is the SD-057 object-bound most-wanted-vs-last-consumed read "
                       "(514m vacuity fix, from V3-EXQ-514n). Three non-vacuity guards self-route "
                       "substrate_not_ready_requeue, never a false weakens (the 514/603e cluster confound).",
        ...
        "predecessor": "V3-EXQ-514l (SD-049 Phase-3 4-arm; FAIL/non_contributory -- foraging-competence "
                       "ceiling now lifted by the curriculum) / V3-EXQ-514m (curriculum-built; vacuous "
                       "C_WL FIX adopted from V3-EXQ-514n).",
    }
