...
"""V3-EXQ-655: INV-074 crystallization necessity test (TASK-DISTRIBUTION SHIFT redesign).

Supersedes V3-EXQ-610f (FAIL; non_contributory / substrate_ceiling per
failure_autopsy_V3-EXQ-610f_2026-06-08, user-confirmed route (a)). 610f was the 6th
necessity-test iteration and the FIRST behaviourally-live + true-negative control
(harness-no-op lineage 543h/i/k/l + 610a/b/c/d RESOLVED; ARM_0 confound RESOLVED) --
yet it STILL re-FAILed D2: the stripped REINFORCE control did NOT collapse its action
entropy under the IGW-20260601-023 phase-3 NOISE regime (delta -0.012; threshold map
ARM_0/2/3 all -0.010..-0.012, no entropy floor load-bearing). The 610f autopsy's
load-bearing learning: adding observation/interoceptive NOISE is NOT the same
destabilising pressure as a task-distribution SHIFT. INV-074's necessity premise
requires the latter -- a phase-3 whose OPTIMAL policy CONFLICTS with phases 0-2,
forcing the overwriting that crystallization is meant to resist. With a noise-only
phase-3 there is no single dominant new optimum for REINFORCE to collapse onto, so
the control never collapses and D1 (crystallization-preserves) is unreadable.

WHAT 655 CHANGES (the 610f autopsy's pre-registered route-(a) redesign)
----------------------------------------------------------------------
655 keeps the 610f ARM STRUCTURE and the VERIFIED WIRING unchanged, and replaces
ONLY the phase-3 environment: the IGW-023 noise-only phase-3 becomes a genuine
TASK-DISTRIBUTION SHIFT whose optimal first-action distribution is INCOMPATIBLE with
the phase-0-2 forage policy.

  1. PHASE-3 TASK SHIFT (the sole change vs 610f). Phases 0-2 are the 610f infant
     curriculum (harm gradient + transient benefit + microhabitat) MINUS the noise;
     foraging is multi-directional, so the policy reaches Phase-3 entry with genuine
     action diversity (D3 non-vacuity). At Phase-3 entry the env reconfigures to a
     conflicting contingency via LANDED SD-054 substrate:
       reef_bipartite_layout=True (reef -> bottom rows, food/hazards -> top half,
         agent spawns in the midline band) + hazard_food_attraction=0.8 (hazards
         actively chase food). Foraging the top half becomes lethal; the ONLY
         viable policy is FLEE-TO-REEF -- a SINGLE dominant first-action class.
     This is a genuine catastrophic-forgetting pressure: the phase-3 optimum (one
     escape direction) conflicts with the phase-0-2 forage-diverse optimum, so the
     un-crystallized control must OVERWRITE its phase-2 diversity to satisfy it
     (entropy collapse), while the EWC-anchored + crystallized arm resists.
     reef_enabled=True is set in ALL phases (constant world_obs_dim=275) so the agent
     is built once; bipartite + food-attraction toggle on only at phase 3
     (placement/behaviour-only, no obs-dim change). NO multi_source / interoceptive
     noise -- the 610f threshold map showed no entropy floor was load-bearing under
     noise, so the task-shift is the SOLE, clean phase-3 pressure.

  2. PHASE-DEPENDENT POLICY ENTROPY BONUS (unchanged from 610f). The entropy bonus
     is the diversity GENERATOR during phases 0-2 (ENTROPY_BONUS_TRAIN=0.02 in EVERY
     arm so D3 is non-vacuous and crystallization has real diversity to preserve) and
     the diversity FLOOR during phase 3, SWEPT {0.0, 0.005, 0.02} across the control
     arms to map whether an entropy floor can resist collapse under a REAL task
     gradient (the 610f map showed it could not resist noise -- this re-maps it
     against genuine forgetting pressure):
       ...
       ARM_3 phase-3 eb = 0.02   (entropy-only regime)

  3. EXPLICIT MECH-341/313 FLOOR CONTRAST (ARM_4, unchanged from 610f). ARM_4 is a
     control (crystallize=False, phase-3 eb=0) with use_noise_floor=True +
     use_e3_score_diversity=True, so D4 = ARM_4.end_p3 - ARM_0.end_p3 measures whether
     the MECH-341/313 floor alone resists the Phase-3 collapse the stripped ARM_0
     suffers. MECH-341 and MECH-313 are in claim_ids (with evidence_direction_per_claim).

MANDATORY PRE-RUN ASSERTION (carried forward verbatim from 610f)
----------------------------------------------------------------
...
Any failure raises AssertionError and the run ERRORs.
...
infant curriculum, Phase-3 pressure = a TASK-DISTRIBUTION SHIFT (SD-054 reef-bipartite
layout + hazard_food_attraction; NO noise) replacing the 610f IGW-023 noise regime.
...
floors per-arm, reef present but corner-placed + no food-attraction). Only the
Phase-3 boundary behaviour differs (the task shift applies to every arm's env).
...
-----------------------------------------------------------------
  ...
  (ii) D2 FAIL (the STRIPPED control still does NOT collapse, even under a GENUINE
       conflicting-task phase-3) WHILE D3 holds (policy verifiably diverse at
       Phase-2):
       -> THE DECISIVE SUBSTRATE-INCAPACITY FORK (610f autopsy, user-confirmed). The
          610f noise-only D2 FAIL already left open whether noise simply was not the
          right pressure (route a) or the V3 REINFORCE-on-grid substrate cannot
          exhibit catastrophic plasticity collapse at all (route b). If the
          stripped control does NOT collapse even when the phase-3 OPTIMAL policy
          conflicts with phases 0-2 (foraging lethal -> flee-to-reef single dominant
          optimum), that CONFIRMS reading (b): INV-074's V3-applicability is
          genuinely blocked. This does NOT weaken the universal invariant INV-074
          (biology-faithful, OD critical period, lit_conf 0.82); it confirms the
          existing epistemic_category substrate_ceiling. Disposition:
          non_contributory-for-INV-074; ACCEPT INV-074 substrate_ceiling and STOP
          the 610 necessity-test cascade (decisive-with-evidence, not assumption).
          {INV-074 unknown, MECH-334 unknown, MECH-333 unknown}.
  ...
  (v)  D4 reading (independent of the INV-074 PASS rule):
       D4 PASS -> the MECH-341/313 floor resists the Phase-3 collapse the stripped
         ...
         diversity-floor reading scored against a genuine task gradient).
...
supersedes: V3-EXQ-610f (and transitively the 610a-f noise-only lineage).
"""
...
# reef_enabled=True in ALL phases so the agent is built ONCE at a constant
# world_obs_dim (=275; reef adds +25). The reef-bipartite layout + food-attracted
# hazards (the phase-3 TASK SHIFT) are placement/behaviour-only -- they do NOT
# change world_obs_dim -- so they can toggle on at phase 3 without rebuilding the
# agent. In phases 0-2 the reef is corner-placed and non-channeling (a benign safe
# zone), so phase-2 foraging stays multi-directional (D3 non-vacuity).
ENV_BASE_KWARGS = dict(
    ...
    # SD-054 reef substrate -- present in EVERY phase for constant obs dim.
    reef_enabled=True,
    n_reef_patches=3,
    reef_patch_radius=2,
)

# Phase-3 TASK-DISTRIBUTION SHIFT (the SOLE change vs the 610f noise-only phase-3).
# Spread on top of the phase-2 geography env to make the phase-3 OPTIMAL first-action
# distribution (flee-to-reef, a single dominant class) CONFLICT with the phase-0-2
# forage-diverse policy -> genuine catastrophic-forgetting pressure for INV-074.
#   reef_bipartite_layout: reef -> bottom rows, food/hazards -> top half, agent ->
#     midline band (categorically-opposite first actions for reef vs forage).
#   hazard_food_attraction: hazards actively chase food, so foraging the top half is
#     lethal and the only viable policy is flee-to-reef.
# No multi_source / interoceptive noise (the 610f threshold map showed no entropy
# floor was load-bearing under noise; the task shift is the clean, sole pressure).
TASK_SHIFT_PHASE3_ENV_KWARGS = dict(
...
#   Phase 3: ep 2000+ (pre-gate readiness + TASK-DISTRIBUTION SHIFT: reef-bipartite + food-attracted hazards)
MAX_EPISODES = 2500
...
def _train_infant_curriculum(
    ...
    for ep in range(max_eps):
        # Phases 0-2: the 610f infant-curriculum geography MINUS noise (scheduler's
        # phase 0-2 env_kwargs carry no noise). Phase 3: replace the scheduler's
        # noise-only phase-3 kwargs with the TASK-DISTRIBUTION SHIFT (reef-bipartite
        # + food-attracted hazards). reef_enabled (in ENV_BASE_KWARGS) holds obs dim
        # constant across the switch.
        if scheduler.current_phase == 3:
            phase_kwargs = TASK_SHIFT_PHASE3_ENV_KWARGS
        else:
            phase_kwargs = scheduler.env_kwargs()
        env_kwargs = {**ENV_BASE_KWARGS, **phase_kwargs}
...
def _run_arm(arm_config: Dict, seed: int, dry_run: bool) -> Dict:
    ...
    print(
        f"[V3-EXQ-655] Starting {arm_label} seed={seed} crystallize={crystallize} "
        f"p3_eb={entropy_bonus_phase3} noise_floor={use_noise_floor} "
        f"e3_div={use_e3_diversity}",
        flush=True,
    )

    # Per-(seed x arm) cell: arm_cell() resets ALL RNG to `seed` on entry (so the
    # cell is a pure function of (substrate, config_slice, seed), independent of arm
    # iteration order) and stamps a content-addressed fingerprint onto the row.
    # Arms are independent (each builds its own agent/env/optimizers; no shared
    # mutable state across cells).
    config_slice = {
        "arm": dict(arm_config),
        "env_base_kwargs": ENV_BASE_KWARGS,
        "task_shift_phase3_env_kwargs": TASK_SHIFT_PHASE3_ENV_KWARGS,
        "max_episodes": MAX_EPISODES,
        "steps_per_episode": STEPS_PER_EPISODE,
        "entropy_bonus_train": ENTROPY_BONUS_TRAIN,
        "residue_ewc_lambda": RESIDUE_EWC_LAMBDA,
    ...
    with arm_cell(seed, config_slice=config_slice, script_path=Path(__file__)) as cell:
        ...
        cell.stamp(metrics)  # writes metrics["arm_fingerprint"]
...
def _assert_fixes_wired() -> Dict:
    ...
    print("[V3-EXQ-655][ASSERT] verifying the three fixes are wired...", flush=True)
    ...
    print(
        f"[V3-EXQ-655][ASSERT] FIX 1 OK: grad_seen={grad_seen} params_changed={params_changed} "
    ...
    print(
        f"[V3-EXQ-655][ASSERT] FIX 2 OK: crystallized={agent.gated_policy.crystallized} "
    ...
    print(
        f"[V3-EXQ-655][ASSERT] FIX 3 OK: ewc_anchored={agent.residue_field.ewc_anchored} "
        f"ewc_penalty={float(penalty):.6g} residue_grad_sum={res_grad:.6g} "
        f"(fisher_sum={anchor_info.get('fisher_sum')})",
        flush=True,
    )

    print("[V3-EXQ-655][ASSERT] all three fixes wired + verified.", flush=True)
    return {
...
if __name__ == "__main__":
    ...
    print(
        "[V3-EXQ-655] INV-074 crystallization necessity test (EVB-0270, TASK-DISTRIBUTION "
        "SHIFT phase-3 [reef-bipartite + food-attracted hazards] + entropy sweep + "
        "MECH-341/313 floor contrast; supersedes V3-EXQ-610f)",
        flush=True,
    )
