"""
V3-EXQ-672b -- MECH-057b: Hippocampal Trajectory Completion Gate

Claims: MECH-057b
Supersedes: V3-EXQ-672a (governance-reclassified non_contributory 2026-06-14).
            672a's gate fired correctly (C3 PASS, ~40% of candidates filtered),
            but C6_baseline_not_collapsed FAILED: goal_success_rate was 0.0 in
            BOTH the gate-on (ARM_1) and gate-off (ARM_0) arms, so the C1
            (harm-reduction) and C2 (completion-improvement) cross-arm contrasts
            could not be measured against a floor-collapsed baseline.

            ROOT CAUSE (672b diagnosis, instrumentation -- NOT a substrate
            ceiling): the eval loop read goal success as
                info.get("resource_collected", False)
            but CausalGridWorldV2.step() NEVER populates an "resource_collected"
            key. On resource consumption the env sets
            info["transition_type"] == "resource"
            (ree_core/environment/causal_grid_world.py:1608 / 2227). So the prior
            success metric read a key that is always absent -> goal_success_rate
            was STRUCTURALLY 0.0 in every arm regardless of agent behaviour. C6
            could never pass; the run was non_contributory by construction, not
            because the baseline agent fails to forage.

            A standalone probe (default from_dims agent, this env, eval over
            transition_type=='resource') confirmed the baseline DOES collect
            resources: ~0.05-0.10 per episode at a short warmup. The agent is a
            functional incidental forager (SP-CEM main-path exploration); the
            FAIL was the broken readout, not collapse.

            672b FIX (both scope-preserving; the MECH-057b completion gate --
            _filter_trajectories_by_completion -- is BYTE-IDENTICAL to 672a):
              (1) Read goal success from info["transition_type"] == "resource"
                  (the key the env actually emits). This is the load-bearing fix.
              (2) num_resources 1 -> 3 (functional-baseline margin lever; the
                  user-sanctioned "larger resource density" option). Identical in
                  BOTH arms; raises incidental foraging-contact probability so the
                  baseline clears the C6 floor (>=0.05) with comfortable margin
                  rather than sitting exactly on it. num_hazards stays 2 so the
                  C1 harm-reduction contrast remains measurable.
            Nothing about what MECH-057b tests changes: ARM_1 still gates
            trajectory candidates by hippocampal sequence-completion verification
            before E3 candidacy; ARM_0 still admits all candidates.
...
Motivation (2026-06-11):
  ...
  Key metrics:
    ...
    goal_success_rate: fraction of episodes reaching a resource
      (info["transition_type"] == "resource")
...
EXPERIMENT_TYPE = "v3_exq_672b_mech057b_trajectory_promotion_gate"
CLAIM_IDS = ["MECH-057b"]
    ...
    for ep_idx in range(num_episodes):
        ...
        for step_idx in range(steps_per_episode):
            ...
            try:
                ...
                # Check goal reach (resource consumed). The env signals a resource
                # contact via info["transition_type"] == "resource"
                # (causal_grid_world.py:1608/2227); it does NOT emit a
                # "resource_collected" key -- reading that (the V3-EXQ-672a bug)
                # made goal_success structurally 0.0 in every arm.
                if not reached_goal and info.get("transition_type") == "resource":
                    reached_goal = True
                    goal_successes += 1
...
def _run_one_seed(seed: int, arm: str, use_completion_gate: bool, dry_run: bool) -> Dict:
    ...
    # num_resources=3 (vs 672a's 1): functional-baseline margin lever -- raises
    # incidental foraging-contact probability so ARM_0 clears the C6 floor
    # (goal_success_rate >= 0.05) with margin. num_hazards stays 2 so the C1
    # harm-reduction contrast remains measurable. Identical in BOTH arms; the
    # MECH-057b completion gate is unaffected.
    env = CausalGridWorldV2(
        seed=seed,
        size=8,
        num_hazards=2,
...
def main():
    ...
    print(f"V3-EXQ-672b: MECH-057b Trajectory Promotion Gate (relative completion bar)", flush=True)
    ...
    if not args.dry_run:
        ...
        manifest = {
            ...
            "run_id": run_id,
            "queue_id": os.environ.get("REE_QUEUE_ID", "V3-EXQ-672b"),
            "supersedes": "v3_exq_672a_mech057b_trajectory_promotion_gate",
            "claim_ids": CLAIM_IDS,
            ...
            "params": {
                "seeds": seeds,
                "size": 8, "num_hazards": 2, "num_resources": 3,
        ...
        print(f"[V3-EXQ-672b] Manifest written: {manifest_path}", flush=True)
