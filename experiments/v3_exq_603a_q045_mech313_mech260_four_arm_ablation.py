...
"""V3-EXQ-603a -- Q-045 MECH-313 vs MECH-260 four-arm anti-monostrategy ablation (repair).

Supersedes V3-EXQ-603 (measurement gap).

IGW-20260521-003 / arc_062_rule_apprehension:GAP-H.

Root cause of V3-EXQ-603 (failure autopsy 2026-05-23):
  act_with_split_obs() calls sense(obs_body, obs_world) without obs_harm_a, so
  use_affective_harm_stream was never enabled and z_harm_a stayed None. dACC
  forward() and DACCtoE3Adapter suppression never ran (MECH-260 inert). MECH-313
  noise floor is stateless and was the only live axis (+0.048 sub-threshold).

Fixes applied vs V3-EXQ-603:
  Fix 1 (mandatory): Training loop uses sense()+generate()+select_action() (not
    act_with_split_obs), passing obs_harm_a so z_harm_a is set and
    select_action() calls dacc.record_action() each step.
  Fix 2 (required): FIFO_WARMUP_STEPS=75 (Kennerley / SD-054 gate: accumulate
    action history before entropy measurement; pre-registered in [50, 100]).
    Action counts for acceptance exclude the first 75 steps of each episode.
  Fix 3 (recommended): evidence_direction_per_claim compares each arm vs
...
Smoke:
  /opt/local/bin/python3 experiments/v3_exq_603a_q045_mech313_mech260_four_arm_ablation.py --dry-run
"""
...
EXPERIMENT_TYPE = "v3_exq_603a_q045_mech313_mech260_four_arm_ablation"
QUEUE_ID = "V3-EXQ-603a"
CLAIM_IDS = ["Q-045", "MECH-313", "MECH-260"]
EXPERIMENT_PURPOSE = "evidence"
...
def run_experiment(dry_run: bool = False) -> Dict[str, Any]:
    ...
    manifest: Dict[str, Any] = {
        ...
        "fixes_applied": [
            "Fix1: select_action() path with obs_harm_a + use_affective_harm_stream",
            f"Fix2: FIFO_WARMUP_STEPS={FIFO_WARMUP_STEPS} before entropy measurement",
            "Fix3: evidence_direction_per_claim vs ARM_0 baseline",
        ],
    }
