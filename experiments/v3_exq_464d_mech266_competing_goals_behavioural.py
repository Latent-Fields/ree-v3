"""
V3-EXQ-464d (EXP-0160 behavioural): MECH-266 competing goals / switch-cost
...
V3-EXQ-464c (NOT a supersede).

ROUTING: 464c landed on the foraging-competent 603n substrate but FAILed
non_contributory (substrate_ceiling): fraction_in_external_task = 0.0 on BOTH arms
all seeds, and the n_switches>=1 non-vacuity gate passed VACUOUSLY (it counted one
episode-initial settle into internal_planning per episode; n_switches == n_episodes),
so the MECH-266 exit-rail had no contested mode to bind (failure_autopsy_SD-034-closure
-cluster-ext_2026-06-12 sub-cluster B). The mode-governance-engagement substrate
(landed 2026-06-13, ree-v3 REEConfig.use_external_task_drive) adds the external_task
salience SOURCE the SalienceCoordinator lacked: a committed-goal-pursuit engagement
signal registered in the coordinator's affinity (-> external_task) AND salience
(switch-into-external_task aggregate). 464d enables it.

TWO SUBSTRATE CHANGES vs 464c:
  (1) use_external_task_drive=True in the agent config (the ONLY agent-config change --
      everything else is bit-identical to 464c so the comparison isolates the drive).
  (2) the non-vacuity readiness gate is RE-STATED: the contested mode must actually be
      occupied on BOTH arms (min_across_arms(fraction_in_external_task) > OCCUPANCY_FLOOR)
      BEFORE the asymmetric-hysteresis criteria are scored -- replacing the 464c
      n_switches>=1 gate that certified episode-boundary settles.

WHAT THIS MEASURES (unchanged from 464c): the MECH-266 asymmetric per-mode hysteresis
...
ARMS (one curriculum build per seed, two frozen-policy evals on clones):
  ...
  MECH-266 mode-governance measurement (464b/c omitted it for the same reason).
...
EXTERNAL_TASK OCCUPANCY NON-VACUITY GATE (re-states the 464c n_switches gate -- the SAME
  statistic the load-bearing C1 criterion routes on, per the V3-EXQ-643 same-statistic
  rule): before scoring C1/C2, assert the contested external_task mode is genuinely
  occupied on BOTH arms -- per-seed occupancy_min = min over arms of
...
predecessor: V3-EXQ-464c (NOT a supersede).
...
EXPERIMENT_TYPE = "v3_exq_464d_mech266_competing_goals_behavioural"
QUEUE_ID = "V3-EXQ-464d"
CLAIM_IDS: List[str] = ["MECH-266", "SD-032a"]
EXPERIMENT_PURPOSE = "evidence"
PREDECESSOR = "V3-EXQ-464c (successor, NOT supersede)"
...
# occupied on BOTH arms before the asymmetric-hysteresis criteria are scored. Replaces
# the 464c n_switches>=1 gate (which counted episode-boundary settles).
...
def _make_config(env) -> REEConfig:
    ...
    cfg = REEConfig.from_dims(
        ...
        # SOURCE the SalienceCoordinator lacked on the foraging substrate. The ONLY
        # agent-config change vs 464c. Calibrated weights (the no-op-default substrate
        # knobs default to 1.0; this experiment opts into a stronger setting so the
        # engagement during a committed foraging step decisively contests external_task
        # against the dACC internal_planning push -- verified non-vacuous: committed
        # engagement ~1.0 -> aggregate 3.2 > enter_threshold 1.0 -> external_task wins
        # + switches; uncommitted engagement ~0.1 -> internal_planning wins. The
        # commit (beta-elevated) term is the decisive driver of genuine competition).
...
def _run_seed(seed: int, dry_run: bool, total_eps: int,
    ...
    c2 = arm_asym["n_switches"] <= arm_sym["n_switches"]
    # mode-governance-engagement occupancy non-vacuity gate (replaces the 464c
    # n_switches gate): the contested mode must be occupied on BOTH arms.
    occupancy_min = min(
        ...
        "interpretation": {
            ...
            "preconditions": [
                ...
                },
                {
                    ...
                    "description": "the contested external_task mode is genuinely occupied on "
                                   ...
                                   "on (V3-EXQ-643 same-statistic rule), re-stating the 464c "
                                   "n_switches>=1 gate that certified episode-boundary settles. A "
                                   "zero-occupancy read is mode-governance-not-engaged (possibly the "
                                   "unresolved nav-competence dependency), NOT a MECH-266 falsification.",
            ...
            "occupancy_non_vacuity_gate": {
                "definition": "per-seed min over arms of fraction_in_external_task > "
                              ...
                              "(non_contributory), NEVER a false weakens. Re-states the 464c "
                              "n_switches>=1 gate.",
                "min_fraction": MIN_FRACTION,
...
def main(dry_run: bool = False,
    ...
    manifest = {
        ...
        "substrate": "scaffolded_sd054_onboarding (full curriculum; 603n config; ready=true "
                     ...
                     "competing-goal env primitive. use_closure_operator OFF (closure would inject "
                     "a confounding mode-switch signal).",
        "condition": CONDITION_LABEL,
        "predecessor": PREDECESSOR,
        "method_note": "464c's MECH-266 competing-goals / switch-cost asymmetry (a sticky exit rail "
                       "on external_task produces over-binding -- more task occupancy, fewer switches "
                       "-- vs the symmetric MECH-259 baseline) re-run WITH the mode-governance-"
                       "engagement substrate enabled (use_external_task_drive=True). 464c FAILed "
                       "non_contributory because fraction_in_external_task=0.0 on both arms (no "
                       "external_task salience SOURCE), so the exit-rail had no contested mode to "
                       "bind. The drive is the ONLY agent-config change vs 464c.",
        "readiness_note": "TWO non-vacuity gates self-route a substrate-not-engaged read to "
                          ...
                          "replacing the 464c n_switches>=1 gate).",
