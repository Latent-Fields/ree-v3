...
"""
V3-EXQ-704b (EXP-0391) -- MECH-451 FINER-CHANNEL-GRANULARITY VALIDATION FALSIFIER.

LETTER ITERATION that SUPERSEDES V3-EXQ-704 (scientific question UNCHANGED; the
implementation control was mis-calibrated -- a test-design/config re-tune fix; substrate
already built, substrate action=none). Routed by the CONFIRMED, user-gated
REE_assembly/evidence/planning/failure_autopsy_V3-EXQ-704_2026-06-25.{md,json}.

WHY V3-EXQ-704 FAILED (precondition_unmet, NOT a falsification): the ARM_NOISE same-layer
null injected a FROZEN random w_chan_finer at a FIXED absolute scale (FCG_NOISE_SCALE=0.5).
A2's learned finer-channel dissociation came in TINY (range 0.00148, ~15x the 1e-4 floor),
so the fixed-scale null's range was ~176.9x the A2 LEARNED range -> the precondition
`fcg_noise_magnitude_matched` ([0.25, 4.0]x band) FAILED, and the C1 "A2 strict-above
ARM_NOISE" bar was meaningless (a 177x-magnitude perturbation is not a null). V3-EXQ-642
invalid-precondition family (control-calibration artifact). MECH-451 UNWEAKENED. Re-derive
brake NOT fired (1st MECH-451 autopsy; brake-exempt config re-tune).

THE FIX (the ONE scientific change; everything else from 704 is kept VERBATIM):
  (1) The ARM_NOISE frozen-random w_chan_finer is now magnitude-matched to the REALISED
      per-(arm, seed) A2 LEARNED w_chan_finer range AT RUN TIME (target = 1.0x the realised
      range = the centre of the [0.25, 4.0]x band on a log scale), instead of a fixed
      absolute scale. Arms run A0,A1,A2,ARM_NOISE in order, so when an ARM_NOISE seed runs,
      that seed's A2 learned range is already measured: the injected random vector is
      RESCALED so its range EQUALS the matched A2 range BY CONSTRUCTION -> the magnitude
      ratio lands inside the band. A degenerate / missing A2 range falls back to the old
      fixed scale, which correctly trips the magnitude-match precondition -> requeue.
  (2) A SUBSTANTIVE-dissociation near-vacuity guard (autopsy Step 7): the A2 LEARNED finer-
      channel range must clear a SUBSTANTIVE floor (W_CHAN_FINER_SUBSTANTIVE_RANGE_FLOOR,
      well above the bare dissociability floor and above 704's near-vacuous 0.00148) on a
      strict-majority of seeds. If the finer channels are only barely above the floor (the
      "compressed blend re-labelled" near-vacuity), the test self-routes
      substrate_not_ready_requeue + non_degenerate=False rather than producing a MISLEADING
      no-lift weakens on a knife-edge tiny-structure-vs-tiny-matched-noise comparison.
...
ARC-108 learned-gating layer. MECH-451 is a DIFFERENT scientific question than the
ARC-108/MECH-450 settling lineage (claim_ids=[MECH-451]). supersedes V3-EXQ-704.
...
weakens):
  ...
  (5) ARM_NOISE frozen-random w_chan_finer magnitude MATCHED to the REALISED per-(arm,seed)
      A2 learned w_chan_finer range AT RUN TIME (704b fix: target 1.0x by construction,
      replacing 704's fixed FCG_NOISE_SCALE that mis-matched 176.9x).
  (6) A2 learned finer-channel range is SUBSTANTIVE, not merely floor-clearing
      (W_CHAN_FINER_SUBSTANTIVE_RANGE_FLOOR; 704b near-vacuity guard) -- else a tiny
      dissociation against a tiny matched null is a knife-edge non-test -> requeue.
...
See REE_assembly/docs/architecture/mech_451_finer_channel_granularity.md,
    ...
    REE_assembly/evidence/planning/failure_autopsy_V3-EXQ-704_2026-06-25.{md,json} (routes this re-queue),
    REE_assembly/docs/architecture/sd_v4_loop_segregation.md (ARC-110 -- pre-empted on PASS),
    experiments/v3_exq_704_mech451_finer_channel_granularity_falsifier.py (scaffold; superseded),
...
EXPERIMENT_TYPE = "v3_exq_704b_mech451_finer_channel_granularity_falsifier"
QUEUE_ID = "V3-EXQ-704b"
SUPERSEDES = "V3-EXQ-704"   # LETTER iteration; ARM_NOISE magnitude-match re-tune + near-vacuity guard
...
#
# 704b FIX-1: the injected range is matched AT RUN TIME to the REALISED per-seed A2 LEARNED
# w_chan_finer range, NOT a fixed presumed scale. ARMS run A0,A1,A2 FIRST; for each ARM_NOISE
# seed the frozen-random deviation is rescaled so its range equals FCG_NOISE_MATCH_TARGET_RATIO
# x that seed's realised A2 learned range -- so the null is magnitude-matched to the structure
# A2 actually learned, BY CONSTRUCTION (704's fixed scale gave a 176.9x mismatch because A2's
# realised range was tiny). A missing / degenerate A2 range falls back to FCG_NOISE_FALLBACK_SCALE,
# which correctly trips the magnitude-match precondition -> substrate_not_ready_requeue.
FCG_NOISE_MATCH_TARGET_RATIO = 1.0   # target injected range = this x realised A2 learned range (centre of [LO,HI])
FCG_NOISE_FALLBACK_SCALE = 0.5       # only used if no realised A2 range is available (then ratio is out-of-band -> requeue)
...
def _inject_fcg_noise(
    ...
    random w_chan_finer once, magnitude-matched to the REALISED A2 learned range (704b).

    Writes a random vector (around the softplus-unity init) into agent.e3.w_chan_finer
    (the register_buffer the MECH-451 finer recompose reads). This is ONLY meaningful when
    learned_channel_gating_eta=0.0 (w_chan_finer frozen) -- which _make_agent guarantees on
    the noise arm -- so the vector is a stable random per-channel re-weighting of the SAME
    finer channels A2 learns, NOT a learned object.

    704b magnitude match: a zero-centred random deviation is RESCALED so its (max-min)
    range equals `target_range` (= FCG_NOISE_MATCH_TARGET_RATIO x the realised A2 learned
    range for this seed). The injected range therefore equals the matched A2 range BY
    CONSTRUCTION, so the magnitude ratio lands inside [LO, HI]. If `target_range` is None or
    non-positive (no realised A2 range available -- e.g. A2 errored / toy dry-run), fall
    back to FCG_NOISE_FALLBACK_SCALE; the resulting unmatched range then correctly trips the
    fcg_noise_magnitude_matched precondition -> substrate_not_ready_requeue (never a weakens).
    ...
    dev = torch.randn((n,), generator=g)
    dev = dev - dev.mean()  # zero-centre so the rescale controls range, not the unity offset
    raw_range = float((dev.max() - dev.min()).item())
    if target_range is not None and target_range > 0.0 and raw_range > 0.0:
        # Rescale the deviation so its range EQUALS the matched A2-learned target.
        alpha = (FCG_NOISE_MATCH_TARGET_RATIO * float(target_range)) / raw_range
    else:
        # No realised A2 range to match -> fixed fallback (ratio out-of-band -> requeue).
        alpha = FCG_NOISE_FALLBACK_SCALE
    w = _FCG_W_INIT + dev * alpha
...
def _run_seed_arm(
    ...
    p2_episodes: int,
    steps_per_episode: int,
    fcg_noise_target_range: Optional[float] = None,
) -> Dict[str, Any]:
    reset_all_rng(seed)
    env = _make_env(seed)
    ...
    # 704b: the injected range is matched to this seed's realised A2 LEARNED range
    # (fcg_noise_target_range, supplied by run_experiment after A2 ran for this seed).
    fcg_noise_injected_range = 0.0
    if arm.get("fcg_noise"):
        fcg_noise_injected_range = _inject_fcg_noise(
            agent, seed, fcg_noise_target_range
        )
...
def run_experiment(
    ...
    for arm in ARMS:
        ...
        for s in seeds:
            ...
            if row is None:
                # 704b FIX-1: for ARM_NOISE, match the injected frozen-random w_chan_finer
                # range to THIS seed's REALISED A2 learned range. A2 runs before ARM_NOISE
                # (arm order A0,A1,A2,ARM_NOISE), so the A2 row for this seed is already in
                # arm_results. None (A2 errored / missing) -> _inject_fcg_noise falls back to
                # the fixed scale -> magnitude ratio out-of-band -> requeue (never a weakens).
                fcg_noise_target_range: Optional[float] = None
                if arm.get("fcg_noise"):
                    a2_row = next(
                        (
                            r for r in arm_results
                            if r["arm_id"] == "A2_FINER_CHANNELS"
                            and int(r["seed"]) == int(s)
                            and r.get("error_note") is None
                        ),
                        None,
                    )
                    if a2_row is not None:
                        fcg_noise_target_range = float(
                            a2_row["fcg_w_chan_finer_range_max"]
                        )
                row = _run_seed_arm(
                    arm, s, p0_episodes, p1_episodes, p2_episodes, steps_per_episode,
                    fcg_noise_target_range=fcg_noise_target_range,
                )
                ...
                row["arm_fingerprint"] = compute_arm_fingerprint(
                    config_slice={
                        **_arm_config_slice(
                        ...
                        "fcg_noise_match_target_ratio": float(FCG_NOISE_MATCH_TARGET_RATIO),
                        "fcg_noise_match_mode": "runtime_matched_to_a2_learned_range",
    ...
    # the structure A2 learns. 704b matches the injected range to the REALISED per-seed A2
    # range AT RUN TIME (target 1.0x), so this lands in-band BY CONSTRUCTION; it still
    # verifies the construction (a degenerate / fallback path lands out-of-band -> requeue).
    ...
    # ----- Precondition (6) [704b near-vacuity guard]: A2 dissociation is SUBSTANTIVE -----
    # The A2 LEARNED finer-channel range must clear the SUBSTANTIVE floor (well above the
    # bare dissociability floor AND above 704's near-vacuous 0.00148) on a strict-majority
    # of seeds. If the finer channels only barely clear the floor ("the compressed blend
    # re-labelled"), a strict-above bar against an equally-tiny matched null is a knife-edge
    # non-test -> substrate_not_ready_requeue (never a misleading no-lift weakens).
    fcg_dissociation_substantive_ok = _maj(
        a2_rows, lambda r: float(r["fcg_w_chan_finer_range_max"]) >= W_CHAN_FINER_SUBSTANTIVE_RANGE_FLOOR
    )
    median_a2_fcg_range_substantive = _median(
        [float(r["fcg_w_chan_finer_range_max"]) for r in a2_rows]
    )
    ...
    n_crf_differentiated_per_arm = [
        ...
        for rows in (a0_rows, a1_rows, a2_rows, noise_rows)
    ]
    ...
    preconditions_met = bool(
        ...
        and fcg_dissociation_substantive_ok
    ...
    # exact, because min(counts) >= k iff every count >= k. The old min/median numbers
    # are preserved on each entry as NON-BOUND diagnostic keys (extra keys are ignored
    # by the recompute), so no information is lost.
    ...
    n_fcg_substantive = _n_seeds(
        a2_rows,
        lambda r: float(r["fcg_w_chan_finer_range_max"]) >= W_CHAN_FINER_SUBSTANTIVE_RANGE_FLOOR,
    )
    ...
    if not preconditions_met:
        ...
        degeneracy_reason = (
            ...
            "(too few divergent seeds / finer channels not dissociable / A2 dissociation "
            "below the SUBSTANTIVE near-vacuity floor / delta_t flat / weights never moved / "
            "matched-noise control did not verify-lift / fcg-noise magnitude unmatched). "
            "NOT a falsification."
        )
    ...
    interpretation = {
        ...
        "preconditions": [
            {
                ...
                "description": (
                    ...
                    "seeds carrying crf_differentiated, over arms (a0_rows, a1_rows, a2_rows, noise_rows)."
                ),
            ...
            {
                "name": "finer_channels_dissociation_substantive",
                "kind": "readiness",
                "description": (
                    "704b NEAR-VACUITY GUARD (autopsy Step 7): the A2 LEARNED finer-channel "
                    "range must be SUBSTANTIVE -- clear W_CHAN_FINER_SUBSTANTIVE_RANGE_FLOOR "
                    "(well above the bare dissociability floor AND above V3-EXQ-704's near-"
                    "vacuous 0.00148) on a strict-majority of seeds. A dissociation only "
                    "barely above the floor is 'the compressed blend re-labelled'; a strict-"
                    "above bar against an equally-tiny matched null is a knife-edge non-test. "
                    "measured = median A2 learned range. Below floor => "
                    "substrate_not_ready_requeue (NEVER a misleading no-lift weakens)."
                ),
                "control": "median A2 fcg_w_chan_finer_range_max vs SUBSTANTIVE floor",
                # COUNT-shaped: `met` is `_maj(a2_rows, range >=
                # W_CHAN_FINER_SUBSTANTIVE_RANGE_FLOOR)`, a k-of-n count of A2 seeds --
                # NOT a bound on the MEDIAN of that range. The median is neither
                # equivalent to nor conservative against the count (with 6 seeds the
                # median can clear the floor on 3 of 6 while `_maj` needs only 2, and
                # vice versa), so the recompute was adjudicating a different predicate.
                # Reported as the satisfying-seed count vs MIN_SEEDS_FOR_PASS.
                "measured": float(n_fcg_substantive),
                "threshold": float(MIN_SEEDS_FOR_PASS),
                "comparator": ">=",
                "direction": "lower",
                # Non-bound diagnostic: the median preserved verbatim from the pre-fix
                # declaration, plus the per-seed floor the count is taken against.
                "observed_median_a2_fcg_range": float(round(median_a2_fcg_range_substantive, 8)),
                "observed_w_chan_finer_substantive_range_floor": float(
                    W_CHAN_FINER_SUBSTANTIVE_RANGE_FLOOR
                ),
                "met": bool(fcg_dissociation_substantive_ok),
            },
            {
            ...
            },
            {
                ...
                "description": (
                    ...
                    "requeue (704b matches the null to the realised A2 range at the correct layer)."
                ),
            ...
            },
            {
                ...
                "description": (
                    ...
                    "to the structure A2 learns at the SAME layer. measured = the ratio. 704b "
                    "matches the injected range to the REALISED per-seed A2 learned range at "
                    "RUN TIME (target 1.0x), so this lands in-band by construction; an out-of-"
                    "band ratio (degenerate / fallback path) => substrate_not_ready_requeue."
                ),
        ...
        "criteria_non_degenerate": {
            ...
            "finer_channels_dissociation_substantive": bool(fcg_dissociation_substantive_ok),
    ...
    manifest_core = {
        ...
        "decision_rule_thresholds": {
            ...
            "w_chan_finer_substantive_range_floor": float(W_CHAN_FINER_SUBSTANTIVE_RANGE_FLOOR),
            ...
            "fcg_noise_match_target_ratio": float(FCG_NOISE_MATCH_TARGET_RATIO),
            "fcg_noise_fallback_scale": float(FCG_NOISE_FALLBACK_SCALE),
        ...
        "acceptance_criteria": {
            ...
            "finer_channels_dissociation_substantive": fcg_dissociation_substantive_ok,
            "median_a2_fcg_range_substantive": round(float(median_a2_fcg_range_substantive), 8),
        ...
        "interpretation_grid": {
            ...
            "FAIL_substrate_not_ready_requeue": (
                ...
                "re-labelled), OR the A2 dissociation is NOT SUBSTANTIVE (704b near-vacuity "
                "guard -- only barely above the floor, a knife-edge non-test), OR delta_t flat "
                "(no outcome variance), OR the learned weights never moved (eligibility never "
                "credited), OR the SAME-LAYER matched-noise control did not VERIFY-LIFT above "
                "A0 on a strict-majority of divergent seeds, OR the frozen random w_chan_finer "
                "magnitude did not MATCH the A2 learned range. The conversion question could "
                "NOT be measured -- NOT a falsification. 704b already matched the null to the "
                "realised A2 range at run time; a persistent SUBSTANTIVE-dissociation failure "
                "(finer channels cannot pull apart at this substrate) routes to the V4 ARC-110 "
                "loop-segregation build, NOT another V3 re-tune."
            ),
        },
...
def _build_manifest(
    ...
    return {
        ...
        "same_layer_null": {
            ...
            "mechanism": (
                "frozen random w_chan_finer with learned_channel_gating_eta=0.0 (the finer "
                "three-factor update a no-op), use_finer_channel_gating=True so the recompose "
                "uses the random vector; 704b: range matched AT RUN TIME to the realised "
                "per-seed A2 learned range (target 1.0x), replacing 704's fixed absolute scale"
            ),
            "match_target_ratio": FCG_NOISE_MATCH_TARGET_RATIO,
            "match_mode": "runtime_matched_to_realised_per_seed_a2_learned_range",
            "fallback_scale": FCG_NOISE_FALLBACK_SCALE,
        },
        "reuse_baseline_from": REUSE_BASELINE_FROM,
        "evidence_direction_note": (
            f"V3-EXQ-704b (EXP-0391) MECH-451 FINER-CHANNEL-GRANULARITY VALIDATION FALSIFIER "
            f"(supersedes V3-EXQ-704; experiment_purpose=evidence; claim_ids=[MECH-451]). "
            f"LETTER iteration routed by the confirmed failure_autopsy_V3-EXQ-704_2026-06-25: "
            f"704 self-routed substrate_not_ready_requeue/non_contributory because the "
            f"ARM_NOISE same-layer null (fixed FCG_NOISE_SCALE=0.5) came in ~176.9x the tiny "
            f"A2 LEARNED w_chan_finer range (fcg_noise_magnitude_matched [0.25,4.0] FAILED) -- "
            f"a V3-EXQ-642 invalid-precondition control-calibration artifact, MECH-451 "
            f"UNWEAKENED. 704b FIX (the one scientific change; substrate action=none): (1) the "
            f"ARM_NOISE frozen-random w_chan_finer range is matched AT RUN TIME to the realised "
            f"per-(arm,seed) A2 learned range (target 1.0x, in-band by construction); (2) a "
            f"SUBSTANTIVE-dissociation near-vacuity guard self-routes substrate_not_ready_"
            f"requeue if A2's learned range is only barely above the floor (704's 0.00148 was "
            f"~15x the bare floor; a tiny-structure-vs-tiny-matched-null strict-above bar is a "
            f"knife-edge non-test) rather than a misleading no-lift weakens. SIBLING of the V3-EXQ-700 "
        ...
        "config_summary": {
            ...
            "fcg_noise_match_target_ratio": FCG_NOISE_MATCH_TARGET_RATIO,
            "fcg_noise_match_mode": "runtime_matched_to_realised_per_seed_a2_learned_range",
            "fcg_noise_fallback_scale": FCG_NOISE_FALLBACK_SCALE,
            "fcg_noise_magnitude_match_band": [FCG_NOISE_MAGNITUDE_MATCH_LO, FCG_NOISE_MAGNITUDE_MATCH_HI],
            "w_chan_finer_substantive_range_floor": W_CHAN_FINER_SUBSTANTIVE_RANGE_FLOOR,
            "fix_704b": "ARM_NOISE magnitude matched to realised A2 range at run time + substantive-dissociation near-vacuity guard (autopsy_V3-EXQ-704)",
            "reuse_baseline_from": REUSE_BASELINE_FROM,
...
def main() -> Tuple[Optional[str], Optional[str], bool]:
    parser = argparse.ArgumentParser(
        description="V3-EXQ-704b (EXP-0391) MECH-451 finer-channel-granularity falsifier"
    )
