"""
V3-EXQ-786b -- MECH-163 dual-system recruitment signature (DV-DEGENERACY REPAIR).

Claims: MECH-163
Supersedes: v3_exq_786a_mech163_dual_system_recruitment_20260721T113809Z_v3
...
None of these blockers caused the V3-EXQ-786 or V3-EXQ-786a FAILs and this run exercises
neither.

===========================================================================
WHY THIS ITERATION EXISTS (read failure_autopsy_V3-EXQ-786a_2026-07-24_dv_degeneracy.
{json,md} -- confirmed, user-adjudicated 2026-07-24)
===========================================================================
V3-EXQ-786a ran to completion, self-reported `non_degenerate: true`, and its manipulation
check (familiarity AUC) cleared with real headroom (mean AUC 0.79 against a 0.70 floor).
Its recorded verdict -- a flat recruitment response, `no_differential_recruitment`, Cohen's
d 0.047 -- was read as the "no-arbitrator" signature and stood as `weakens` for over 60
hours. It was WITHDRAWN 2026-07-24, not because the manipulation failed, but because the
LOAD-BEARING STATISTIC ITSELF was degenerate by construction, independent of the
manipulation, the sample size, or anything the run measured.

THE BUG (786a's `_depth_scores`, mirrored below in this file before the fix):
    full.append(...residue_field.evaluate_trajectory(world_seq)...)          # fine
    first.append(...residue_field.evaluate_trajectory(world_seq[:, :1, :])...)  # BUG
`world_seq[:, :1, :]` is index 0 of the world-state sequence -- the CURRENT state, IDENTICAL
across every candidate before any trajectory diverges (verified: n_unique=1 across all 32
candidates on 786a's own re-derived per-tick data, range exactly 0 on every scored tick).
`recruitment = 1 - spearman(full, first)` therefore compared the genuine full-horizon
ranking against a CONSTANT vector -- which does not degrade gracefully to "no signal", it
produces NOISE with a specific, simulatable distribution (200-draw simulation: mean 1.0173,
sd 0.1871), because a double-argsort of a constant vector is a stable-sort tie-break
permutation, not a zero vector. 786a's manifest recruitment_rate of 1.01725 matched that
simulated noise distribution to FIVE SIGNIFICANT FIGURES -- the "flat response" was the
DV reporting exactly what a degenerate correlation reports, not a scientific null.

WHY 786a's OWN READINESS GATE COULD NOT CATCH THIS. `candidate_score_range_non_degenerate`
gated the cross-candidate RANGE of the FULL-HORIZON score only. It never checked the OTHER
input to the same rank-correlation statistic -- the first-step vector -- so a constant
first-step vector passed readiness cleanly while the DV it fed was pure noise. Compounding
this, `_spearman`'s own internal guard (rank-variance == 0) could not fire either: a
double-argsort of a constant array is a PERMUTATION of 0..K-1 (an arbitrary but non-zero-
variance stable-sort ordering), so the guard that exists specifically to catch this class of
degeneracy read the constant vector as fine. Both facts generalise: a readiness gate must
check EVERY input to a difference/ratio/correlation statistic, and a rank-based degeneracy
guard must be paired with an explicit RAW-VALUE range/distinctness check, because rank
machinery can silently launder a constant into a well-formed-looking permutation.

THE REPAIR PRECEDENT (V3-EXQ-811, `failure_autopsy_V3-EXQ-811_2026-07-24.md`). 811 built the
SD-081 arbitration substrate on top of this exact recruitment design and hit the identical
class of defect one layer down -- and *that* build is what first surfaced 786a's degeneracy,
before 786a's own autopsy existed. 811's fix, adopted verbatim below:
  1. HABIT_DEPTH = 2, never 1. Score the myopic pathway at world_seq[:, :HABIT_DEPTH, :]
     (two world-states) instead of world_seq[:, :1, :] (one, always the shared current
     state). Depth 2 is the first depth at which candidate trajectories can differ at all,
     since depth 0 (the pre-action state) is shared by construction and 786a's depth-1 read
     was the pre-action state alone. Measured on this substrate (811's own re-derivation):
     depth-2 habit scores are fully distinct across candidates (worst distinct-fraction
     0.875, worst cross-candidate range 9.34e-6), where depth-1 was n_unique=1 with range 0.
  2. GATE THE HABIT VECTOR'S OWN RANGE AND DISTINCT-FRACTION, not just the full-horizon
     range -- the direct fix for "a readiness gate must check every input to the statistic".
     Two new preconditions below, `habit_score_range_non_degenerate` and
     `habit_score_distinct_fraction_non_degenerate`, mirror 811's `min_habit_score_range` /
     `min_habit_distinct_frac` fields exactly.
  3. GATE AT THE POINT OF COMPUTATION, not only at the readiness-precondition level: a tick
     is counted toward `recruitment_rate` only when BOTH the full-horizon range AND the
     habit-depth range clear the floor, so even if a readiness precondition were somehow
     bypassed, a degenerate tick could not enter the reported statistic.
  4. AGGREGATE WITH None, NEVER 0.0, FOR EMPTY CELLS. 811's OWN fix still shipped a bug the
     autopsy separately found: its worst-case aggregation defaulted an empty (zero-scored-
     tick) cell's range/distinct-fraction to 0.0 instead of None, so a single empty cell
     (present in 15.6% of 811's cells, mostly short novel-layout episodes) forged a false
     "degenerate" reading across the WHOLE run, even though 108/128 cells were genuinely
     fine and the correctly-None-defaulted `recruitment_rate` field was never fooled. This
     iteration's aggregation (see `_probe_layout` / `_run_seed` below) follows the CORRECT
     pattern throughout: a zero-tick cell reports None and is EXCLUDED from every worst-case
     min, never coerced into a spurious zero.

MECH-163 leg (1) reverted to EXPERIMENTALLY UNTESTED on the DV-degeneracy re-adjudication
(its pre-786a state: literature-only, 9 supports, 0 experimental entries). This run is that
retest: same scientific question as 786 and 786a, repaired instrument. V3-EXQ-811 tests a
DIFFERENT (though related) question -- whether an EXPLICIT ARBITRATOR (MECH-477) produces
differential recruitment -- and does not substitute for this leg-(1) retest; its own OFF-arm
control additionally runs at curiosity_weight=0.05 (required for the arbitrator's u_habit
path), a materially different config from this experiment's curiosity_weight=0 (required
here for CEM-scorer non-circularity, unchanged from 786a -- see `_familiarity` docstring
below). GOV-REUSE-1 checked and NOT recoverable from 811's manifest for that reason.

===========================================================================
HISTORICAL CONTEXT -- CHANGES 1-3 BELOW ARE 786a's OWN REPAIR OF V3-EXQ-786, CARRIED FORWARD
UNCHANGED (read failure_autopsy_V3-EXQ-786_2026-07-20.{json,md} for the original diagnosis)
===========================================================================
...
substrate gap in that run's causal path. The scientific question is unchanged, so 786a was
a lettered iteration, and it changed exactly two things (both still in force in 786b):
...
agent's OWN scorer is applied at two depths:
    ...
    habit-depth score   = residue_field.evaluate_trajectory(world_seq[:, :HABIT_DEPTH, :])
(both lower = better; mirrors HippocampalModule.score_trajectory). HABIT_DEPTH = 2, NEVER 1
-- see "WHY THIS ITERATION EXISTS" above. Depth 0 (the pre-action state) is shared by every
candidate by construction, so a depth-1 read is that shared state alone and is CONSTANT;
depth 2 is the first depth at which candidates can differ. RECRUITMENT is the rate at which
deep rollout REORDERS the candidate set relative to a myopic (habit-depth) read of the same
machinery: recruitment = 1 - spearman(full_horizon_scores, habit_depth_scores). 0 = lookahead
...
0.0 by arithmetic.) 786a's own defect was a DIFFERENT failure mode than 604c's: not an
arithmetic identity under the DV's declared symmetry group, but a degenerate INPUT (a
structurally constant habit-depth vector) that the group-invariance analysis alone does not
catch -- which is exactly why the readiness check below gates the RAW range/distinctness of
BOTH scoring depths, not only the DV-symmetry declaration.
...
EVIDENCE DIRECTIONS (both declared, per the diagnostic-descriptions rule):
  ...
  * WEAKENS MECH-163 (leg 1): recruitment equal or LOWER on novel layouts, on a substrate
    that passed ALL FOUR readiness preconditions below. That is a genuine negative: the
    planned system is engaged no more by novelty than by familiarity.
  * NEITHER (self-routes substrate_not_ready_requeue, weighting NOTHING): a readiness
    ...
    either scoring depth's cross-candidate RANGE or DISTINCT-FRACTION is degenerate so the
    ranking (or the thing it is compared against) is arbitrary noise.

READINESS (P0). Four preconditions, each asserting the statistic its consumer routes on --
two for the manipulation, two NEW ones (the DV-degeneracy repair) for the DV's OWN two
inputs:
  * familiarity_discriminability_auc -- THE MANIPULATION CHECK. Gating, per the V3-EXQ-786
    autopsy's fourth required change ("keep the manipulation check as a GATING precondition
    -- it worked as intended, catching an uninterpretable run before it could weigh against
    a claim"). A future miss self-routes substrate_not_ready_requeue rather than silently
    producing a degenerate C1.
  * familiarity_discriminability_above_chance -- the SEM lower bound of the same AUC, so the
    discrimination is reliable across seeds and not carried by one seed's point estimate.
  * full_score_range_non_degenerate -- READINESS FOR THE FULL-HORIZON INPUT to C1's rank
    correlation. C1 routes on a RANKING over candidates, meaningful only if candidates are
    separable, so this asserts the cross-candidate RANGE clears a floor -- deliberately
    RANGE, not mean-abs/magnitude (the V3-EXQ-643 same-statistic lesson: a uniform per-tick
    offset has large magnitude and ~0 range, so a magnitude check passes while the ranking
    is arbitrary). This is the precondition 786a HAD, unchanged.
  * habit_score_range_non_degenerate -- READINESS FOR THE HABIT-DEPTH INPUT to the SAME rank
    correlation -- the precondition 786a LACKED, and whose absence is the entire 786a
    defect. Same statistic, same floor, applied to the OTHER input. A constant habit-depth
    vector (786a's depth-1 read) has cross-candidate range exactly 0 and is caught here
    directly, at the raw-value level, rather than relying on `_spearman`'s rank-variance
    guard, which cannot see this class of degeneracy (see "WHY THIS ITERATION EXISTS").
  * habit_score_distinct_fraction_non_degenerate -- a SECOND, independent check on the same
    input: the fraction of candidates whose habit-depth score is numerically distinct from
    every other candidate's, floor 0.5. Catches the case where range is nominally non-zero
    (e.g. floating-point noise on an otherwise-constant computation) but the values still
    collapse to a small number of ties -- the exact secondary signature V3-EXQ-811's own
    re-derivation reported (worst distinct-fraction 0.875 alongside worst range 9.3e-6).
Below-floor on ANY of the four routes to substrate_not_ready_requeue -- NEVER to a substrate
verdict label and never to an evidence direction. Every per-cell aggregate feeding these four
preconditions is WORST-CASE (min), never a mean -- a mean would hide exactly the single
degenerate tick this repair exists to catch -- and every empty (zero-scored-tick) cell
contributes None to that min, never a coerced 0.0. That second point is 811's OWN bug (its
`min_habit_score_range` etc. defaulted an empty cell to 0.0, unlike its correctly-None-
defaulted `recruitment_rate`), fixed here so the DV-degeneracy repair does not reintroduce
the aggregation-degeneracy repair from the same session.
...
EXPERIMENT_TYPE = "v3_exq_786b_mech163_dual_system_recruitment"
CLAIM_IDS = ["MECH-163"]
...
# --- DV-degeneracy repair (786a -> 786b; see "WHY THIS ITERATION EXISTS"). HABIT_DEPTH
# mirrors V3-EXQ-811's E3Config.dualsystem_habit_depth precedent, though nothing here reads
# that config field -- this experiment scores world_seq[:, :HABIT_DEPTH, :] directly via the
# agent's own residue_field, same as 786a did (bug and all) at depth 1. 2, never 1: index 0
# is the shared pre-action state, so a depth-1 vector is constant by construction.
HABIT_DEPTH = 2
...
AUC_SEM_K = 1.0
# Applied to BOTH the full-horizon score and the habit-depth score (786a applied this only
# to the full-horizon score, which is the entire 786a defect -- see "WHY THIS ITERATION
# EXISTS"). Deliberately RANGE, not magnitude (V3-EXQ-643 lesson).
CANDIDATE_SCORE_RANGE_FLOOR = 1e-6
# Fraction of candidates whose habit-depth score is numerically distinct from every other
# candidate's. A second, independent check on the SAME input as the range floor above --
# catches near-constant-but-not-exactly-zero-range cases the range floor alone could miss.
# Matches V3-EXQ-811's min_habit_distinct_frac floor (its own re-derivation: worst 0.875).
HABIT_DISTINCT_FRACTION_FLOOR = 0.5
...
def _worst(values: List[Optional[float]]) -> Optional[float]:
    """Min of the non-None values, or None if every value is None (or the list is empty).

    THE V3-EXQ-811 AGGREGATION FIX. A zero-scored-tick cell must contribute NOTHING to a
    worst-case min, not a coerced 0.0 -- 811's own `min_full_score_range` etc. defaulted
    empty cells to 0.0, which is indistinguishable from a genuinely degenerate reading of
    exactly zero and let a single empty cell (15.6% of 811's cells, mostly short
    unpracticed-layout episodes) zero the whole run's readiness verdict. Every caller of this
    helper in this file follows the pattern `recruitment_rate` already used correctly in
    786a: None in, excluded from the min, None out only if nothing at all was scored.
    """
...
def _depth_scores(agent: REEAgent, candidates: List[Any]) -> Tuple[List[float], List[float]]:
    ...
    residue_field.evaluate_trajectory over the sequence) at two depths of the SAME
    machinery -- so the habit read is a depth-limited view of the planned read, not a second
    scorer that could drift from it. Lower = better. Returns ([], []) when any candidate
    lacks a usable world-state sequence.

    HABIT_DEPTH = 2, NEVER 1 (see module docstring "WHY THIS ITERATION EXISTS"): index 0 of
    world_seq is the pre-action state, shared by every candidate, so a depth-1 read
    (786a's bug) is CONSTANT by construction regardless of anything the manipulation does.
    """
    full: List[float] = []
    habit: List[float] = []
    with torch.no_grad():
...
def _probe_layout(
    ...
    RECRUITMENT = 1 - spearman(full_horizon_scores, habit_depth_scores) over the candidate
    set: how much multi-step lookahead REORDERS the candidates relative to a myopic
    (HABIT_DEPTH) read of the same machinery. 0 = deep rollout changes nothing (the habit
    ...
    GATED AT THE POINT OF COMPUTATION (the V3-EXQ-811 precedent): a tick counts toward
    `recruitment_rate` only when BOTH the full-horizon range AND the habit-depth range clear
    CANDIDATE_SCORE_RANGE_FLOOR. This is a second, independent line of defence alongside the
    top-level readiness preconditions -- even if those were bypassed, a degenerate tick could
    not enter the reported statistic.
        ...
        for _step in range(STEPS_PER_EPISODE):
            ...
            if ticks.get("e3_tick", False) and candidates and len(candidates) >= 2:
                ...
                if full and habit:
                    ...
                    # The direct 786a check: a constant vector has distinct-frac 1/K, and
                    # no rank-variance guard (inside _spearman) can see it -- double-argsort
                    # of a constant array is a non-zero-variance stable-sort permutation.
                    habit_distinct_fracs.append(float(len(set(habit))) / float(k))
                    if f_rng > CANDIDATE_SCORE_RANGE_FLOOR and h_rng > CANDIDATE_SCORE_RANGE_FLOOR:
                        rho = _spearman(full, habit)
                        if rho is not None:
                            ticks_scored += 1
    ...
    return {
        ...
        # WORST tick, never a mean: the degeneracy this exists to catch is a single pinned
        # tick, which a mean hides. None (not 0.0) when this layout scored no E3 ticks at
        # all -- 811's own bug, fixed here: an empty-cell 0.0 would be indistinguishable
        # from a genuinely degenerate reading of exactly zero once it enters a downstream
        # min-across-cells.
        "min_full_score_range": float(min(full_ranges)) if full_ranges else None,
        "min_habit_score_range": float(min(habit_ranges)) if habit_ranges else None,
        "min_habit_distinct_frac": (
            float(min(habit_distinct_fracs)) if habit_distinct_fracs else None
        ),
...
def _run_seed(seed: int, practice_episodes: int, probe_episodes: int,
    ...
    for cond, env_seeds, env_kwargs in (
        ...
        per_condition[cond] = {
            ...
            # WORST layout, None-safe (see _worst docstring): a zero-tick layout must not
            # masquerade as a degenerate-zero reading.
            "min_full_score_range": _worst([r["min_full_score_range"] for r in rows]),
            "min_habit_score_range": _worst([r["min_habit_score_range"] for r in rows]),
            "min_habit_distinct_frac": _worst([r["min_habit_distinct_frac"] for r in rows]),
...
def run_experiment(dry_run: bool = False) -> Dict[str, Any]:
    ...
    # WORST across every seed, None-safe (see _worst docstring). Falls back to 0.0 only if
    # EVERY seed scored zero ticks everywhere -- a genuine "no data at all" state, which
    # correctly reads as below-floor rather than as a false pass on an empty aggregate.
    full_range_worst = _worst([r["min_full_score_range"] for r in per_seed])
    habit_range_worst = _worst([r["min_habit_score_range"] for r in per_seed])
    habit_distinct_worst = _worst([r["min_habit_distinct_frac"] for r in per_seed])
    ...
    full_range_measured = float(full_range_worst) if full_range_worst is not None else 0.0
    habit_range_measured = float(habit_range_worst) if habit_range_worst is not None else 0.0
    habit_distinct_measured = (
        float(habit_distinct_worst) if habit_distinct_worst is not None else 0.0
    )
    ...
    preconditions = [
        ...
        },
        {
            ...
            "description": (
                "Cross-candidate RANGE of the full-horizon score (NOT magnitude), WORST "
                "tick across all seeds/conditions/layouts (never a mean): the load-bearing "
                "criterion routes on a rank correlation over candidates, which is arbitrary "
                "noise unless candidates are separable. This is the precondition 786a HAD."
            ),
            "measured": full_range_measured,
            "threshold": CANDIDATE_SCORE_RANGE_FLOOR,
            "direction": "lower",   # FLOOR: met when measured >= threshold
            "control": "candidate set on a real E3 tick",
            "met": bool(full_range_measured >= CANDIDATE_SCORE_RANGE_FLOOR),
        },
        {
            "name": "habit_score_range_non_degenerate",
            "description": (
                "Cross-candidate RANGE of the HABIT_DEPTH score (NOT magnitude), WORST tick "
                "across all seeds/conditions/layouts. THE DV-DEGENERACY REPAIR: this is the "
                "precondition 786a LACKED -- it gated the full-horizon input to the rank "
                "correlation but never the OTHER input, so a constant depth-1 vector (786a's "
                "bug) passed readiness cleanly while the DV it fed was pure noise. A "
                "below-floor reading here means the habit-depth read is constant across "
                "candidates -- exactly 786a's failure mode -- and routes to "
                "substrate_not_ready_requeue rather than a false weakens/supports."
            ),
            "measured": habit_range_measured,
            "threshold": CANDIDATE_SCORE_RANGE_FLOOR,
            "direction": "lower",   # FLOOR: met when measured >= threshold
            "control": "candidate set on a real E3 tick, same ticks as the full-horizon check",
            "met": bool(habit_range_measured >= CANDIDATE_SCORE_RANGE_FLOOR),
        },
        {
            "name": "habit_score_distinct_fraction_non_degenerate",
            "description": (
                "Fraction of candidates whose habit-depth score is numerically distinct "
                "from every other candidate's, WORST tick across all seeds/conditions/"
                "layouts. A second, independent check on the SAME input as the range "
                "precondition above: catches near-constant-but-not-exactly-zero-range cases "
                "(e.g. floating-point noise on an otherwise-constant computation collapsing "
                "to a small number of tied values) that a pure range floor could miss. "
                "Matches the V3-EXQ-811 precedent's min_habit_distinct_frac gate."
            ),
            "measured": habit_distinct_measured,
            "threshold": HABIT_DISTINCT_FRACTION_FLOOR,
            "direction": "lower",   # FLOOR: met when measured >= threshold
            "control": "candidate set on a real E3 tick, same ticks as the range checks",
            "met": bool(habit_distinct_measured >= HABIT_DISTINCT_FRACTION_FLOOR),
        },
    ]
    ...
    manifest: Dict[str, Any] = {
        ...
        "pre_registered": {
            ...
            "habit_distinct_fraction_floor": HABIT_DISTINCT_FRACTION_FLOOR,
        ...
        manifest["interpretation"] = {
            ...
            "criteria_non_degenerate": {
                "C1_recruitment_higher_on_novel": bool(
                    len(deltas) >= SEM_MIN_N
                    and full_range_measured > CANDIDATE_SCORE_RANGE_FLOOR
                    and habit_range_measured > CANDIDATE_SCORE_RANGE_FLOOR
                    and habit_distinct_measured >= HABIT_DISTINCT_FRACTION_FLOOR
                )
            },
        }

    # NOTE: habit_score_distinct_frac is deliberately NOT included here. It is a coarsely
    # quantized statistic (multiples of 1/K for K candidates) sitting near its ceiling by
    # design -- ties across seeds at the same discrete value (e.g. two seeds both landing on
    # 31/32) are the EXPECTED healthy outcome, not a degeneracy signature, and including it
    # would make check_degeneracy's cross-seed zero-spread test fire on quantization rather
    # than on anything scientifically informative (observed in this script's own --dry-run
    # smoke: two seeds tied at 0.96875, correctly non-informative). full_score_range and
    # habit_score_range are continuous quantities where cross-seed zero-spread genuinely
    # would be suspicious, so both stay -- mirroring 786a's own candidate_score_range entry.
    manifest.update(check_degeneracy({
        "recruitment_delta": deltas,
        "familiarity_auc": aucs,
        "full_score_range": {
            "values": [r["min_full_score_range"] for r in per_seed
                       if r["min_full_score_range"] is not None],
            "floor": CANDIDATE_SCORE_RANGE_FLOOR,
        },
        "habit_score_range": {
            "values": [r["min_habit_score_range"] for r in per_seed
                       if r["min_habit_score_range"] is not None],
            "floor": CANDIDATE_SCORE_RANGE_FLOOR,
        },
    }))
