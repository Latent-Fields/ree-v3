"""
V3-EXQ-514p -- SD-049 Phase-2 MECH-229 wanting != liking, measured as the SD-057
OBJECT-BOUND wanting-target != liking-target dissociation on a curriculum-built agent.

CONFIRMATORY REPLICATION of V3-EXQ-514o (NOT a supersede). 514o LANDED PASS/supports
2026-06-15 (mean_object_bound_wl_dissoc_fraction=0.80 >= 0.6, both readiness gates met,
non_degenerate=true) and CLOSED goal_pipeline:GAP-2 + GAP-7. This run is the independent
4th genuine experimental entry the user requested so MECH-229 clears the provisional->stable
governance gate (min_experimental_entries=4; 514o was the 3rd genuine exp support, exp_conf
0.868 already > the 0.80 floor). BYTE-FAITHFUL to 514o except the seed triple is INDEPENDENT
([45, 46, 47] vs 514o's [42, 43, 44]) so this is a genuine replication, not a re-run of the
same seeds. All gates, the substrate-marginal contact-guard self-route, and the falsifier
off-ramp are identical to 514o.

ROUTING (inherited from the 514o lineage): V3-EXQ-514n FAILed (non_contributory, route_reason=contact_guard_unmet): the
...
predecessor: V3-EXQ-514o (confirmatory replication, NOT a supersede -- 514o is a genuine
  PASS/supports; this independent-seed replication is the 4th genuine experimental entry to
  clear MECH-229 provisional->stable).
...
EXPERIMENT_TYPE = "v3_exq_514p_sd049_phase2_mech229_object_bound_wanting_liking"
QUEUE_ID = "V3-EXQ-514p"
CLAIM_IDS: List[str] = ["MECH-229"]
EXPERIMENT_PURPOSE = "evidence"
PREDECESSOR = "V3-EXQ-514o (confirmatory replication, NOT supersede)"
