"""V3-EXQ-544a: MECH-313 (ARC-065) stochastic_noise_floor substrate-readiness diagnostic.

Supersedes V3-EXQ-544 (ran 2026-05-10, exit=0, manifest PASS for all 5 UC sub-tests,
but the runner classified the run ERROR because no sentinel was emitted under
_runner_signals/ -- pre-experiment_protocol-wiring infrastructure glitch). Script
unchanged from the predecessor; this re-queue produces a clean PASS record via the
current experiment_protocol.emit_outcome handshake.
...
def main() -> None:
    ...
    run_id = "v3_exq_544a_mech313_noise_floor_substrate_readiness_v3"
    ts = datetime.utcnow().strftime("%Y%m%dT%H%M%SZ")
    manifest = {
        "schema_version": "v1",
        "run_id": f"{run_id}_{ts}",
        "experiment_type": "v3_exq_544a_mech313_noise_floor_substrate_readiness",
        "architecture_epoch": "ree_hybrid_guardrails_v1",
        "timestamp_utc": datetime.utcnow().isoformat() + "Z",
        "experiment_purpose": "diagnostic",
        "supersedes": "v3_exq_544_mech313_noise_floor_substrate_readiness",
