"""
Substrate-defect CONTROL leg (GOV-FANOUT-1 Step 2.5b(iv) coverage gap; 2.5c corrupting overlap): is the decisive seed's MEL readout trustworthy at all? Repeat the 861e protocol on 17befb8c with the already-built non-degenerate ContextMemory write selection ON
...
Hypothesis under test: CONTROL   Design axis: representation
Substrate pinned to: 17befb8c
...
================================================================================
failure_autopsy_V3-EXQ-861e_2026-08-21 froze H1 + H3 (count 2), with H2 a labelled follow-on. This leg is
...
legacy -- the repair must actually fire.
 ...
 - CONTROL FAILS (the reading that matters most) if the refractory arm restores
   ...
   against qid inv050_mech180_861e_producer_vs_intervention_isolation, and that is a governance call, not this leg's.
...
SUBSTRATE_PIN_REF = "17befb8c46f0b7352f74a6b6e3ee4fc9715878fc"
...
FANOUT_HYPOTHESIS = "CONTROL"
FANOUT_AXIS = "representation"
FANOUT_SOURCE_AUTOPSY = "failure_autopsy_V3-EXQ-861e_2026-08-21"
PRIMARY_VARIANT = "refractory"
CONTROL_VARIANT = "argmin_legacy"
...
def _make_agent(env: CausalGridWorldV2, mel_on: bool, mel_reference: float,
...
def _write_selection_for(variant: str) -> str:
...
def _cell_is_write_locked(cell: Dict[str, Any]) -> bool:
    """THE SHIPPED PREDICATE for the LEGACY half of this leg's readiness anchor.
    Used both by _discrimination() on the live argmin control cell and by the
    setup-time reachability guard on 861e's frozen recorded cell.

    Only the LEGACY half is anchorable: no recorded run of the refractory write
    path on this lineage exists, so there is no frozen positive control for the
    "repair fires" half. That asymmetry is stated rather than papered over -- a
    guard scored against a reference that does not exist would be theatre."""
    return (_insufficient_frac(cell) or 0.0) >= WRITE_LOCK_INSUFFICIENT_FRAC


_ANCHOR_REACHABILITY = [assert_anchor_reachable(
    anchor_name="write_repair_engaged_on_decisive_seed__legacy_half",
    reference_cells=[PREDECESSOR_DECISIVE["861e"]],
    score_fn=_cell_is_write_locked,
    threshold=1.0,
    reference_source=("V3-EXQ-861e recorded seed 271 / ARM_3_HIGH_ON cell "
                      "(6 of 6 measurement cycles with < 2 touched slots) -- "
                      "the write-address lock this leg's legacy control must "
                      "reproduce"),
)]
...
def _discrimination(arm_results: List[Dict[str, Any]],
    ...
    repair_engaged = (p is not None and c is not None
                      and p_lock is not None
                      and p_lock < WRITE_LOCK_INSUFFICIENT_FRAC
                      and _cell_is_write_locked(c))
