...
Context:
  EXQ-062 FAIL -- C4 FAIL: surprise-gated condition produced MORE uncommitted
  steps than always-spike (selectivity_ratio > 1.0; threshold < 0.80).

  Root cause: positive feedback loop. The interrupt fired on any harm step
  regardless of committed state. Once the agent became uncommitted (variance
  elevated), its predictions degraded -> |actual - predicted| increased ->
  more surprise-gated spikes -> agent stayed uncommitted longer. Surprise-gated
  fires more often uncommitted precisely because noisier predictions amplify
  surprise signal.

  Fix: restrict both interrupt policies to COMMITTED steps only.
  If agent.e3._running_variance >= commit_threshold (already uncommitted),
  no spike is applied -- the gate is already open, no interrupt needed.
  This breaks the positive feedback loop for surprise and makes always-spike
  also operationally correct (you don't spike if already interrupted).

Design -- same as EXQ-062, with committed-only gating:
  ...
  Condition C -- SURPRISE-GATED SPIKE (committed steps only)
    ...
    Expected: selective de-commitment -- fewer uncommitted than always-spike.
...
EXPERIMENT_TYPE = "v3_exq_062a_mech104_surprise_gate_committed_only"
...
def _eval_interrupt_policy(
    ...
    KEY FIX vs EXQ-062: interrupt only fires on COMMITTED steps.
    If agent is already uncommitted (variance >= commit_threshold),
    no spike is applied -- the gate is already open.
    This breaks the positive feedback loop where elevated variance
    causes noisier predictions -> larger surprise -> more spikes.
    """
    agent.eval()
    ...
    for _ in range(num_episodes):
        ...
        for _ in range(steps_per_episode):
            ...
            try:
                ...
                # If already uncommitted, gate is open; no interrupt needed.
                if actual_harm < -0.01:
                    n_harm_steps += 1
...
def run(
    ...
    **kwargs,
    ...
    print(
        f"[V3-EXQ-062a] MECH-104: Surprise-Gated Volatility Interrupt (Committed-Only Fix)\n"
        f"  Fix: interrupt fires on committed steps only (breaks positive feedback loop)\n"
    ...
    # -- Train single agent --------------------------------------------------
    print(f"\n{'='*60}", flush=True)
    print(f"[V3-EXQ-062a] Training ({warmup_episodes} eps)...", flush=True)
    print('='*60, flush=True)
    ...
    for policy, label in [
        ...
        print(f"\n{'='*60}", flush=True)
        print(f"[V3-EXQ-062a] {label}", flush=True)
        print('='*60, flush=True)
    ...
    c4_pass = (
        r_always["n_uncommitted"] > 0 and
        r_surprise["n_uncommitted"] < r_always["n_uncommitted"] * 0.80
    )
    c5_pass = (
        r_none["fatal_errors"] +
    ...
    print(f"\nV3-EXQ-062a verdict: {status}  ({criteria_met}/5)", flush=True)
    ...
    metrics = {
        ...
        "selectivity_ratio": float(
            r_surprise["n_uncommitted"] / max(1, r_always["n_uncommitted"])
        ),
    ...
    summary_markdown = f"""# V3-EXQ-062a -- MECH-104: Surprise-Gated Volatility Interrupt (Committed-Only Fix)
...
| B | Always-spike (committed only) | {r_always['n_uncommitted']} | {r_always['n_committed_harm_steps']} | {r_always['n_spikes']} |
| C | Surprise-gated (committed only) | {r_surprise['n_uncommitted']} | {r_surprise['n_committed_harm_steps']} | {r_surprise['n_spikes']} |

Selectivity ratio (C/B): {metrics['selectivity_ratio']:.3f}  (target: < 0.80)
