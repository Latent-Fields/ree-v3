...
"""V3-EXQ-514g: SD-049 BG gating wider-seed sweep under StepHarness.
...
QUEUE_ID = "V3-EXQ-514g"
EXPERIMENT_TYPE = "v3_exq_514g_sd049_bg_gating_wider_seeds_stepharness"
CLAIM_IDS: List[str] = []
EXPERIMENT_PURPOSE = "diagnostic"
SUPERSEDES = "V3-EXQ-514d"
...
def main_run(dry_run: bool = False) -> Dict:
    ...
    t0_total = time.time()
    for seed in seeds:
        print(f"[514g] seed={seed} episodes={n_eps}", flush=True)
        t0 = time.time()
...
def write_manifest(result: Dict, run_id: str) -> str:
    ...
    print(f"[514g] written -> {out_path}", flush=True)
    return str(out_path)
