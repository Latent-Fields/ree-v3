...
"""
V3-EXQ-436e -- SD-017/ARC-045/MECH-166 slot-differentiation retest on an
OCCUPIED-SLOTS-ONLY slot_cosine_sim DV (repairs the C1 instrument, not the
substrate)
...
INTERPRETATION GRID:
  P0 UNMET (any of the five gates) -> label names the specific gate
              (sleep_cycle_recording_gap_still_present /
              waking_writepath_not_engaged / insufficient_occupancy_for_c1 /
              adam_drift_neutralization_failed). Route to /failure-autopsy
              on the named mechanism -- C1/C4 are not evidence either way.
...
EXPERIMENT_TYPE = "v3_exq_436e_sd017_mech166_occupied_slot_retest"
QUEUE_ID = "V3-EXQ-436e"
SUPERSEDES = "V3-EXQ-436d"
CLAIM_IDS = ["SD-017", "ARC-045", "MECH-166"]
EXPERIMENT_PURPOSE = "evidence"
...
def run(dry_run: bool = False) -> Tuple[dict, ZGoalStreamAccumulator]:
    ...
    # --- P0 READINESS GATE (5 checks; all must clear) ------------------------
    ...
    summary = {
        "P0_sleep_cycle_write_counters": {
            ...
            "ready": ready,
            "desc": ("P0 GATE (5 checks, all must clear). 2 unchanged from "
                     "436c/436d (sleep-pass write/rollout counters); 3 NEW "
                     "in 436e (waking writepath engagement, occupancy "
                     "sufficiency, Adam-drift-neutralization calibration) "
                     "-- see failure_autopsy_V3-EXQ-436d-methodology-check_"
                     "2026-08-07 Recommendation #4."),
        },
    ...
    result = {
        ...
        "notes": (
            "SD-017/ARC-045/MECH-166 slot-differentiation retest on an "
            "OCCUPIED-SLOTS-ONLY slot_cosine_sim DV. Discharges the "
            "DV-repair obligation recorded by "
            "failure_autopsy_V3-EXQ-436d-methodology-check_2026-08-07 "
            "(confirmed, user-confirmed), which found 436d's C1 metric "
            "(whole-bank mean off-diagonal cosine, no occupancy mask) "
            "fatally confounded on five independent grounds: WAKING_ONLY "
            "performed zero ContextMemory writes (F1); an unwritten bank "
            "reads ~0 not ~1.0, so ARC-045's near-1.0 calibration point was "
            "an artifact of the write-gate defect 436d itself repaired "
            "(F2); the statistic is a PRODUCT of similarity and occupancy, "
            "not similarity alone (F3); this makes it anti-correlated with "
            "the differentiation mechanism it was written to detect -- a "
            "12-slot near-orthogonal arm scored 8x WORSE than a 1-slot arm "
            "(F4); WAKING_ONLY's entire observed 0.0009-0.47 spread was "
            "reproduced by Adam-optimizer drift alone on "
            "context_memory.memory with ZERO writes (F5); and 2/5 seeds sat "
            "inside the untouched-bank null, making C1 unsatisfiable there "
            "by construction (F6). This letter implements all six items of "
            "the autopsy's Recommendation #4: (1) statistic over occupied "
            "slots only, tracking write()'s own min_idx; (2) similarity and "
            "occupancy reported separately, never as their product; (3) "
            "WAKING_ONLY given a real write path (sd016_writepath_mode="
            "'sense_only', held constant vs SWS_THEN_REM, which shares it "
            "plus the offline consolidation writes); (4) Adam drift "
            "neutralized (context_memory.memory.requires_grad_(False), all "
            "conditions); (5) the predicted baseline re-derived empirically "
            "at run time rather than reusing ARC-045's near-1.0 figure; (6) "
            "a NO_WRITES negative control added, checked against the "
            "re-derived null as a P0 calibration gate. The manipulation "
            "under test is unchanged from every prior letter: SWS_THEN_REM "
            "(offline consolidation) vs WAKING_ONLY (online writes only). "
            "The OLD whole-bank statistic is still recorded verbatim "
            "(slot_cosine_sim_raw_whole_bank) for audit continuity, but "
            "never gates. Conditions, training loop, action-selection "
            "wiring, seeds and C1's shape (paired, directional, >=3/5 "
            "seeds) are otherwise unchanged from 436d."
        ),
    }
    return result, zg
