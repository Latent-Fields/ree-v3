...
"""
V3-EXQ-063a -- ARC-029: Committed vs Ablated Operating Mode Harm Outcomes
              (R-c commit-entry readiness gate ACTIVE; supersedes V3-EXQ-063)

Claims: ARC-029

SUBSTRATE-STALENESS REVALIDATION (queued 2026-06-02). The predecessor V3-EXQ-063
(2026-03-22) measured committed-mode harm outcomes BEFORE the MECH-090 R-c
commit-entry readiness conjunction landed (2026-05-28, ree-v3/CLAUDE.md "MECH-090
Commit-Entry Predicate"). That landing added a readiness pre-condition on commit
ENTRY -- which changes WHICH episodes/ticks enter committed mode -- so the
2026-03-22 measurement of "what committed mode produces" is mechanistically stale.
The 063 evidence was de-weighted today (scoring_excluded=stale_substrate,
superseded_by_substrate=MECH-090@2026-05-28; see
REE_assembly/evidence/planning/epoch_stale_evidence_review_2026-06-02.md). This
re-run measures the SAME ARC-029 question on the CURRENT substrate with the R-c
commit-entry gate active. Single claim ARC-029 (re-evaluated from scratch per the
claim_ids accuracy rule: the gate change does not add or remove any claim under
test -- ARC-029 is still exactly what this 2x2 design measures).
...
Context:
  ...
  R-c gate activation (this revalidation): config.heartbeat.use_commit_readiness_gate
  = True with commit_readiness_floor = 0.05. This is the WITHIN-TICK DECISIVENESS
  axis of the MECH-090 R-c predicate (BetaGate.should_admit_elevation, fires
  automatically on per-candidate score_margin every committed tick -- no harness
  outcome-push needed). The ACROSS-TICK nav_competence axis
  (use_mech090_readiness_conjunction) is intentionally LEFT OFF: it requires a
  per-tick harness-pushed notify_outcome signal that this eval design does not
  emit, so it would sit fail-open (readiness=1.0) and add no measurement -- only
  the decisiveness axis is exercisable in this 2x2 harness. The gate composes with
  the bistable beta latch at the elevate site: elevation is admitted only when
  result.committed AND score_margin >= floor.

  Dependencies:
    MECH-090 ACTIVE (commit-entry R-c predicate landed 2026-05-28): BG beta gate
      controls policy propagation; entry now gated by readiness decisiveness.
...
EXPERIMENT_TYPE = "v3_exq_063a_arc029_committed_mode_harm_outcomes_rc_gate"
CLAIM_IDS = ["ARC-029"]
SUPERSEDES = "v3_exq_063_arc029_committed_mode_harm_outcomes"
...
def _make_agent(
    ...
    # V3-EXQ-063a: activate the MECH-090 R-c commit-entry readiness gate
    # (within-tick decisiveness axis). These HeartbeatConfig flags are NOT
    # surfaced through from_dims (matches the beta_gate_bistable precedent --
    # ree-v3/CLAUDE.md "MECH-090 Commit-Entry Predicate"); set them directly on
    # config.heartbeat after the dataclass build. agent.py forwards them to
    # BetaGate via getattr at REEAgent.__init__, and select_action consults
    # should_admit_elevation(score_margin, K) at the elevate site, so commit
    # ENTRY is gated by per-candidate score decisiveness. The across-tick
    # nav_competence axis is left off (no harness outcome-push in this design).
    config.heartbeat.use_commit_readiness_gate = True
...
def run(
    ...
    **kwargs,
    ...
    print(
        f"[V3-EXQ-063a] ARC-029: Committed vs Ablated Gate -- Harm Outcomes (2x2)\n"
    ...
    for seed in seeds:
        print(f"\n{'='*60}", flush=True)
        print(f"[V3-EXQ-063a] Seed {seed}", flush=True)
        print('='*60, flush=True)
        ...
        for env_name, env_obj in [("stable", stable_env), ("volatile", volatile_env)]:
            for gate_label, ablated in [("gate_active", False), ("gate_ablated", True)]:
                ...
                print(f"\n{'='*60}", flush=True)
                print(f"[V3-EXQ-063a] {label}", flush=True)
                print('='*60, flush=True)
    ...
    print(f"\nV3-EXQ-063a verdict: {status}  ({criteria_met}/5)", flush=True)
    ...
    summary_markdown = f"""# V3-EXQ-063a -- ARC-029: Committed vs Ablated Mode Harm Outcomes (2x2, R-c gate)
