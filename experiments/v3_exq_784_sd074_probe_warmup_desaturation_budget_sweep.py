...
def _config_slice(train_spe: int, read_spe: int, budgets: List[int], selects: int) -> Dict[str, Any]:
    ...
    # A SET env-seed base changes the env layout this cell was computed on, so it
    # must key the fingerprint -- otherwise a pinned cell and an OS-entropy cell
    # share a hash and a consumer could reuse the wrong world (false cache HIT).
    # Conditional so the default path's slice is byte-identical to every existing
    # mint of this driver.
