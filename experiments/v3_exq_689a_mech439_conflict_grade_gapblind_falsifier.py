"""V3-EXQ-689a: MECH-439 F-dominance conflict-grade GAP-BLIND control-arm
discriminating falsifier -- supersedes V3-EXQ-689. Same scientific question
(MECH-439's FIRST falsifier).

WHY 689a (redesign, not a re-queue at higher P0). 689 self-routed
substrate_not_ready_requeue on the grading-non-vacuity precondition (gap_spread
0/3 seeds; F-gap pinned in the near-tie bin, a1b1_gap_bins_used=2 < 3). The
adjudication (failure_autopsy_V3-EXQ-689_2026-06-19) found this is a
test-design / MEASUREMENT gap, NOT substrate-under-trained: the GAP-A readiness
preconditions PASSED (route_range 0.566, e2 pairwise_dist 0.164, C1 divergent)
and BOTH conflict-grade levers ENGAGED (k_effective varied 3/3, t_eff varied
3/3) -- the grading DID act. The true cause is that the foraging top-F gap is
STRUCTURALLY concentrated in the near-tie bin (itself a manifestation of the
F-dominance MECH-439 asserts), so the load-bearing C_FGAP per-gap-bin
entropy-vs-gap regression is UNCOMPUTABLE (<= 2 populated fixed-width bins; the
slope needs >= 3). Re-queueing at higher P0 cannot open an ecology-pinned gap.

THE REDESIGN (autopsy S6, user-ratified). Replace the uncomputable per-gap-bin
regression with an ARM CONTRAST that is computable on the F-dominated foraging
regime: GAP-BLIND CONTROL ARMS.
  LEAD -- conflict-grade is load-bearing IFF ARM_A1B1 (gap-SCALED both levers)
  beats BOTH gap-blind controls on committed-action-class entropy (strict-above
  on >= 2/3 seeds):
    ARM_FIXED_KMAX  -- k pinned at k_max=6, GAP-BLIND (Factor A's fixed-large-k
      control: a uniformly-wide shortlist, not graded by the gap).
    ARM_FIXED_HOT_T -- commit-T pinned at the gap-scaled peak base+alpha=2.5,
      GAP-BLIND, over the e2_world_forward DIVERGENT source (Factor B's
      fixed-hot-T control: a uniformly-hot flat softmax over the eligible set --
      distinct from ARM_MATCHED_NOISE which keeps the COLLAPSED proposer source).
  If a fixed-large-k / fixed-hot-T arm lifts committed entropy AS MUCH AS the
  gap-scaled version, the lift is NOT conflict-grading -- it is "a bigger fixed
  shortlist / hotter flat softmax". ARM_A1B1 strict-above BOTH gap-blind
  controls isolates the GAP-CONCENTRATION of the heat/width (the load-bearing
  property), sidestepping the uncomputable regression.

  SUPPORTING (secondary, REPORTED not gated):
    (i) Quantile-adaptive gap binning -- bin per cell by gap QUANTILES matched to
      the near-tie distribution so even a concentrated gap yields >= 3 populated
      bins; retains the C_FGAP entropy-vs-gap slope as a SECONDARY readout. (The
      fixed-width 689 binning is kept too, also non-gating, for continuity.)
    (ii) DECOUPLED readiness gate -- "levers engaged" (k_effective AND t_eff vary
      across P1 ticks) is SEPARATED from "gap-spread for the regression". A
      levers-engaged + regression-uncomputable run now reaches the arm-contrast
      verdict; it NO LONGER self-routes substrate_not_ready_requeue on gap-spread.
...
substrate_not_ready_requeue, NEVER a false weakens):
  ...
  (c) levers-engaged: ARM_A1B1 k_effective varies (Factor A) AND t_eff varies
      (Factor B) across P1 ticks. (gap-spread is REMOVED from this gate.)

THE GAP-BLIND CONTROL MAPPING ONTO THE LANDED MECH-439 SUBSTRATE:
  ARM_FIXED_KMAX  = factor_a OFF (NOT conflict-graded) + modulatory_shortlist_k=6
    (the fixed large eligible set) + hard argmin. Contrast vs ARM_A1B0 (Factor A
    graded k 1..6): A1B0 grades k by the gap; FIXED_KMAX is flat k=6.
  ARM_FIXED_HOT_T = factor_b ON (use_gap_scaled_commit_temperature) but with
    gap_scaled_commit_entropy_alpha=0.0 AND base temperature=2.5 -> T_eff =
    2.5 + 0*(1 - gap_norm) = 2.5 CONSTANT (gap-blind). Same multinomial-over-
    eligible-set commit machinery as ARM_A0B1/ARM_A1B1, but flat. Contrast vs
    ARM_A0B1 (Factor B gap-scaled, alpha=1.5, T=1.0 peak 2.5 at near-tie):
    A0B1 scales the heat by the gap; FIXED_HOT_T is flat 2.5.

ALL eight arms share the GAP-A-ready conversion constant: SD-056 online-trained
...
Layer A + shared E3-side bias channels (lateral_pfc + mech295). CRF stack off.
  ...
  ARM_A1B0           e2wf,      T=1.0, A GRADED, B off    (Factor A only -- gap-graded k)
  ARM_A0B1           e2wf,      T=1.0, A off, B GAP-SCALED (Factor B only -- gap-scaled commit-T, alpha=1.5)
  ARM_A1B1 (PRIMARY) e2wf,      T=1.0, A GRADED, B GAP-SCALED (BOTH levers; the gap-SCALED cell)
  ARM_FIXED_KMAX     e2wf,      T=1.0, A off, B off, k=6  (GAP-BLIND Factor-A control -- flat large shortlist)
...
ACCEPTANCE (evidence, claim_ids=[MECH-439]; supersedes V3-EXQ-689):
  ...
  NEVER a weakens -- RANGE statistics, the same-statistic discipline):
    (a) IN-ARM ROUTE-RANGE: ARM_A1B1 modulatory_channel_route_range mean (read
        LIVE at the select tick) > ROUTE_RANGE_FLOOR on >= MIN_SEEDS_FOR_PASS seeds.
    (b) E2-DIVERGENCE: ARM_A1B1 cand_world_pairwise_dist > C1_PAIRWISE_DIST_FLOOR
        on >= MIN_SEEDS_FOR_PASS seeds.
    (c) LEVERS-ENGAGED (DECOUPLED from gap-spread): ARM_A1B1 k_effective varies
        (spread >= 1) AND t_eff varies (spread > eps) on >= MIN_SEEDS_FOR_PASS
        seeds. (gap-spread is computed + REPORTED, NOT gated.)
  C_PRIMARY (gate, secondary off-ramp discriminator): ARM_A1B1
    ...
    ARM_A1B1 mean > C3_SELECTED_ENTROPY_FLOOR (no-lift-at-all off-ramp).
  C_GAPBLIND (LOAD-BEARING -- the 689 C_FGAP replacement): ARM_A1B1
    selected_action_class_entropy STRICTLY ABOVE BOTH ARM_FIXED_KMAX AND
    ARM_FIXED_HOT_T on the SAME seed, on >= MIN_SEEDS_FOR_PASS seeds, AND
    ARM_A1B1 mean > C3_SELECTED_ENTROPY_FLOOR. The lift is gap-CONCENTRATED
    (conflict-grading), not uniform (a bigger fixed shortlist / hotter flat
    softmax).
  PASS = READINESS AND C_PRIMARY AND C_GAPBLIND ->
    conflict_grade_converts_committed_diversity; evidence_direction=supports.

Interpretation grid:
| outcome                                              | label                                              | evidence_direction | next                                                                                       |
|------------------------------------------------------|----------------------------------------------------|--------------------|--------------------------------------------------------------------------------------------|
| READINESS + C_PRIMARY + C_GAPBLIND                   | conflict_grade_converts_committed_diversity        | supports           | MECH-439 first falsifier PASS; conflict-grade moves committed behaviour, lift is gap-concentrated |
| route/e2-div below floor OR k/T_eff not varied       | substrate_not_ready_requeue                        | non_contributory   | routing/SD-056 under-trained OR levers inert; re-queue; NOT a weakens                       |
...
| READINESS + C_PRIMARY met, C_GAPBLIND fail (uniform) | uniform_lift_grading_non_load_bearing              | non_contributory   | the lift matches a gap-blind fixed-k / flat-hot-T -> NOT conflict-grading -> route to rank_preserving_F_to_eligibility_demotion |
...
on ARM_A1B1 vs the gap-blind controls. claim_ids=[MECH-439] only;
ARC-065/MECH-341/ARC-062/MECH-309/MECH-294 untouched.

Usage:
  /opt/local/bin/python3 experiments/v3_exq_689a_mech439_conflict_grade_gapblind_falsifier.py --dry-run
"""
...
EXPERIMENT_TYPE = "v3_exq_689a_mech439_conflict_grade_gapblind_falsifier"
QUEUE_ID = "V3-EXQ-689a"
SUPERSEDES = "V3-EXQ-689"
CLAIM_IDS: List[str] = ["MECH-439"]  # MECH-439's FIRST falsifier (gap-blind redesign)
...
ARMS: List[Dict[str, Any]] = [
    ...
    {
        # GAP-BLIND Factor-A control: flat large shortlist (k pinned at k_max=6),
        # hard argmin, NO gap grading. Isolates "a bigger fixed shortlist" from
        # the gap-CONCENTRATED width of ARM_A1B0 / ARM_A1B1.
        "arm_id": FIXED_KMAX_ARM,
        "label": "e2wf_fixed_kmax6_gap_blind_factor_a_control",
        "candidate_summary_source": "e2_world_forward",
        "temperature": 1.0,
        "factor_a": False,
        "factor_b": False,
        "shortlist_k": MODULATORY_SHORTLIST_K_MAX,   # pinned wide, gap-blind
        "gap_scaled_alpha": GAP_SCALED_COMMIT_ENTROPY_ALPHA,
    },
    {
...
def _evaluate(arm_results: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    fixed_kmax = _arm_rows(arm_results, FIXED_KMAX_ARM)
    ...
    fixed_kmax_by_seed = {r["seed"]: r for r in fixed_kmax}
    ...
    # READINESS (c): levers-engaged non-vacuity on the PRIMARY cell. DECOUPLED
    # from gap-spread (the 689 fix): only k_effective AND t_eff must vary. A flat
    # k / flat T_eff means the grading could not act -> substrate_not_ready_requeue.
    ...
    # C_GAPBLIND (LOAD-BEARING -- the 689 C_FGAP replacement): ARM_A1B1 STRICTLY
    # ABOVE BOTH gap-blind controls (fixed-k=6 AND flat-hot-T=2.5) on the same
    # seed. If a gap-blind control matches ARM_A1B1, the lift is uniform (a
    # bigger fixed shortlist / hotter flat softmax), NOT gap-concentrated.
    gapblind_seeds_ok = _n_seeds(
        a1b1, lambda r: _strict_above_pair(r, fixed_kmax_by_seed, fixed_hot_t_by_seed)
    )
    ...
    # Control-engagement diagnostics (informational; confirm the gap-blind arms
    # applied their fixed levers -- a wider shortlist / a hotter flat commit-T).
    fixed_kmax_shortlist_mean = _mean_key(fixed_kmax, "modulatory_shortlist_size_mean")
    ...
    all_arms = [proposer, noise, a0b0, a1b0, a0b1, a1b1, fixed_kmax, fixed_hot_t]
    non_degenerate = bool(
    ...
    # VERDICT resolver (689a): readiness -> C_PRIMARY -> C_GAPBLIND. The
    # load-bearing test is the GAP-BLIND ARM CONTRAST, not the regression.
    if not readiness_ok:
    ...
    # 2x2 + gap-blind dissociation (informational): each cell's entropy + how many
    # seeds it is strict-above BOTH gap-blind controls.
    def _cell_summary(rows: List[Dict[str, Any]]) -> Dict[str, Any]:
        return {
            ...
            "seeds_strict_above_both_gap_blind_controls": int(
                _n_seeds(rows, lambda r: _strict_above_pair(r, fixed_kmax_by_seed, fixed_hot_t_by_seed))
            ),
        }
    ...
    return {
        "readiness": {
            ...
            "gap_spread_diagnostic": {
                ...
                "note": (
                    "DIAGNOSTIC ONLY in 689a -- DECOUPLED from the readiness gate. "
                    "A levers-engaged + gap-pinned run reaches the gap-blind arm "
        ...
        "c_primary": {
            ...
            "note": (
                "Off-ramp discriminator: ARM_A1B1 strict-above BOTH collapsed "
                "controls (proposer + matched-noise). Fail = no lift at all -> "
                "conversion_ceiling_persists_despite_conflict_grade (V4 off-ramp)."
            ),
        },
        "c_gapblind": {
            "a1b1_seeds_strict_above_both_gap_blind_controls": int(gapblind_seeds_ok),
            "a1b1_selected_entropy_mean": round(a1b1_sel_mean, 6),
            "fixed_kmax_selected_entropy_mean": round(_mean_key(fixed_kmax, SENT), 6),
            "fixed_hot_t_selected_entropy_mean": round(_mean_key(fixed_hot_t, SENT), 6),
            "selected_entropy_floor": C3_SELECTED_ENTROPY_FLOOR,
            "c_gapblind_pass": gapblind_pass,
            "note": (
                "LOAD-BEARING (the 689 C_FGAP replacement): ARM_A1B1 (gap-SCALED "
                "both levers) STRICTLY ABOVE BOTH gap-blind controls -- "
                "ARM_FIXED_KMAX (flat k=6) AND ARM_FIXED_HOT_T (flat T=2.5 over "
                "the divergent source). PASS = the lift is gap-CONCENTRATED "
                "(conflict-grading). Fail = a gap-blind fixed-k / flat-hot-T "
                "matches it -> uniform lift, NOT conflict-grading."
            ),
        },
        "control_engagement_diagnostic": {
            "fixed_kmax_shortlist_size_mean": round(fixed_kmax_shortlist_mean, 6),
            "a0b0_shortlist_size_mean": round(a0b0_shortlist_mean, 6),
            "fixed_hot_t_teff_mean": round(fixed_hot_t_teff_mean, 6),
            "note": (
                "Informational: confirm the gap-blind controls applied their fixed "
                "levers -- ARM_FIXED_KMAX shortlist wider than ARM_A0B0 (k=6 vs 3); "
                "ARM_FIXED_HOT_T t_eff ~ 2.5 (flat hot commit-T). Does NOT gate."
            ),
        },
        "c_fgap_secondary": {
            "fixed_width": {
            ...
            "note": (
                "SECONDARY readout, NON-GATING in 689a. The fixed-width regression "
                ...
                "The LOAD-BEARING test is the gap-blind arm contrast (c_gapblind)."
            ),
        },
        "negative_control": {
        ...
        "two_by_two_dissociation": {
            ...
            "ARM_A1B0_factor_a_only": _cell_summary(a1b0),
            "ARM_A0B1_factor_b_only": _cell_summary(a0b1),
            "ARM_A1B1_both_on": _cell_summary(a1b1),
            "ARM_FIXED_KMAX_gap_blind_a": _cell_summary(fixed_kmax),
            "ARM_FIXED_HOT_T_gap_blind_b": _cell_summary(fixed_hot_t),
            "note": (
                "Informational dissociation. The PRIMARY gates on ARM_A1B1 vs the "
                "two gap-blind controls (c_gapblind); these per-cell readouts "
                "dissociate Factor A (graded width) vs Factor B (gap-scaled "
                "commit-T) vs both vs their gap-blind counterparts."
            ),
        },
        "selected_action_entropy_per_arm_mean": {
            ...
            FIXED_KMAX_ARM: round(_mean_key(fixed_kmax, SENT), 6),
        ...
        "route_range_per_arm_mean": {
            ...
            FIXED_KMAX_ARM: round(_mean_key(fixed_kmax, RDIST), 6),
        ...
        "preconditions": [
            {
                ...
                "kind": "readiness",
                "description": (
                    "ARM_A1B1 IN-ARM RAW cross-candidate RANGE of the modulatory "
                    "bias routed into the E3 selection authority "
                ...
                "control": (
                    "ARM_A1B1: candidate_summary_source=e2_world_forward, "
            ...
            },
            {
                ...
                "kind": "readiness",
                "description": (
                    "ARM_A1B1 e2.world_forward(z0, a_i) per-candidate prediction "
            ...
            {
                "name": "a1b1_grading_levers_engaged_non_vacuous",
                "kind": "readiness",
                "description": (
                    "ARM_A1B1 conflict-graded k_effective varies across P1 ticks "
                    "(spread >= 1) AND the gap-scaled commit T_eff varies (spread "
                    "> eps). DECOUPLED from gap-spread (the 689 fix): a flat k / "
                    "flat T_eff means the grading could not act => "
                    "substrate_not_ready_requeue, but a gap-pinned distribution "
                    "(689 failure) NO LONGER trips this gate. Count statistic "
                    "(seeds meeting both)."
                ),
                "control": (
                    "ARM_A1B1: both MECH-439 levers ON; k_effective_spread / "
                    "t_eff_spread read live"
                ),
                "measured": int(min(k_varies_seeds, t_varies_seeds)),
        ...
        "criteria": [
            ...
            {"name": "C_PRIMARY_a1b1_selected_entropy_strict_above_both_collapsed_controls",
             "load_bearing": True, "passed": primary_pass},
            {"name": "C_GAPBLIND_a1b1_selected_entropy_strict_above_both_gap_blind_controls",
...
def run_experiment(dry_run: bool = False) -> Dict[str, Any]:
    ...
    manifest: Dict[str, Any] = {
        ...
        "evidence_direction_note": (
            "MECH-439 F-dominance conflict-grade GAP-BLIND control-arm falsifier -- "
            "supersedes V3-EXQ-689 (same scientific question, redesigned measurement). "
            "689 self-routed substrate_not_ready_requeue on the grading-non-vacuity "
            "gap-spread precondition (gap_spread 0/3; F-gap pinned in the near-tie bin; "
            "the per-gap-bin C_FGAP regression uncomputable). ADJUDICATED "
            "(failure_autopsy_V3-EXQ-689_2026-06-19) as a test-design/measurement gap, "
            "NOT substrate-under-trained: GAP-A readiness PASSED + both levers ENGAGED "
            "(k 3/3, t_eff 3/3); the gap is structurally near-tie-pinned (itself a "
            "manifestation of the F-dominance MECH-439 asserts). 689a replaces the "
            "uncomputable regression with a GAP-BLIND ARM CONTRAST: ARM_A1B1 (gap-SCALED "
            "both levers) must beat BOTH gap-blind controls -- ARM_FIXED_KMAX (flat k=6) "
            "AND ARM_FIXED_HOT_T (flat T=2.5 over the divergent e2_world_forward source) "
            "-- on committed-action class entropy on >=2/3 seeds (C_GAPBLIND, "
            "load-bearing). PASS (label=conflict_grade_converts_committed_diversity) = "
            "READINESS (IN-ARM route-range + e2-divergence + levers-engaged k/T_eff-vary; "
            "gap-spread DECOUPLED) AND C_PRIMARY (strict-above both collapsed controls) "
            "AND C_GAPBLIND. Route/e2-div below floor OR k/T_eff not varied self-routes "
            "substrate_not_ready_requeue (non_contributory, NEVER a weakens). Readiness "
            "met, no lift vs collapsed => conversion_ceiling_persists_despite_conflict_grade "
            "(V4 off-ramp). Readiness + lift but a gap-blind control matches it => "
            "uniform_lift_grading_non_load_bearing (a bigger fixed shortlist / hotter "
            "flat softmax, NOT conflict-grading -> rank_preserving_F_to_eligibility_demotion). "
        ...
        "interpretation": {
            ...
            "routing": {
                "conflict_grade_converts_committed_diversity": "PASS -> MECH-439 first falsifier supports; the committed-entropy lift is gap-CONCENTRATED (ARM_A1B1 beats both gap-blind controls); toward supports",
                "substrate_not_ready_requeue": "routing/SD-056 under-trained OR k/T_eff levers not varied; re-queue; do NOT weaken MECH-439",
                ...
                "uniform_lift_grading_non_load_bearing": "the lift is real but a gap-blind fixed-k / flat-hot-T control matches it -> NOT gap-concentrated -> route to the rank_preserving_F_to_eligibility_demotion V3 fallback",
            },
        },
        ...
        "acceptance_criteria": {
            ...
            "C_PRIMARY_a1b1_strict_above_both_collapsed_controls": summary["c_primary"]["c_primary_pass"],
            "C_GAPBLIND_a1b1_strict_above_both_gap_blind_controls": summary["c_gapblind"]["c_gapblind_pass"],
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="V3-EXQ-689a MECH-439 conflict-grade GAP-BLIND control-arm falsifier (supersedes V3-EXQ-689)"
    )
