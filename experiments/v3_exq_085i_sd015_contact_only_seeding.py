...
EXQ-085h result (2026-03-30):
  RFM converged well (loss=0.0078) but goal_resource_r_rfm=-0.169 (average).
  Seed-level breakdown:
    seed=42: r=-0.503  seed=7: r=-0.227  seed=13: r=+0.223

  Seed 13 PASSED C3 (r=0.223 > 0.2). Seeds 42 and 7 had strongly negative r.
  The average was dragged negative by secondary seeding contamination.

ROOT CAUSE:
  085h secondary seeding path fired on EVERY step where benefit_exposure exceeded
  threshold (i.e. whenever the agent was near a resource). This accumulated
  resource_field_view patterns from hundreds of non-contact positions, pushing
  z_goal_norm_resource to ~2.7 (vs z_goal_norm_world ~0.38). The resulting
  z_goal_resource represented the AVERAGE proximity field across random near-resource
  positions rather than the specific "resource at contact" pattern. Cosine-sim
  action selection then guided the agent toward states matching this diffuse average,
  which was anti-correlated with resource proximity in most seeds.

  Seed 13 worked because its random initialisation produced a contact-dominated
  seeding trajectory despite the noise.

THE FIX:
  Remove secondary seeding for z_goal_resource entirely.
  z_goal_resource is updated ONLY on ttype=="resource" (actual contact events).
  z_goal_world retains secondary seeding (needed to prevent decay between contacts
  in z_world space).

  alpha_goal for goal_state_resource raised 0.3 -> 0.5 to compensate for the
  reduced seeding frequency (fewer updates per episode, each must contribute more).

=== DESIGN (unchanged from 085h) ===

ResourceForwardModel (RFM): resource_field_view_t + action -> resource_field_view_{t+1}
TWO goal states: z_world-seeded (085h baseline) and resource_field_view-seeded (SD-015).
Action selection in eval: RFM + goal_state_resource.
KEY DIAGNOSTIC: goal_resource_r_rfm > 0.2.

=== PASS CRITERIA ===

C1: z_goal_resource_norm > 0.1  (contact-only seeding produces non-trivial goal)
C2: benefit_ratio >= 1.3x       (RFM-guided nav beats random by 30%)
C3: goal_resource_r_rfm > 0.2   (z_goal_resource correlates with resource proximity)
C4: no fatal errors
...
EXPERIMENT_TYPE = "v3_exq_085i_sd015_contact_only_seeding"
...
def _run_single(
    ...
    # resource_field_view-seeded goal state (SD-015 -- contact-only, no secondary)
    goal_config_resource = GoalConfig(
        goal_dim=RESOURCE_DIM,
        alpha_goal=0.5,   # raised from 0.3: fewer updates, each must count more
        decay_goal=0.003,
        benefit_threshold=0.05,
        goal_weight=1.0,
        drive_weight=drive_weight,
    ...
    for ep in range(warmup_episodes):
        ...
        for _ in range(steps_per_episode):
            ...
            if goal_present:
                if ttype == "resource":
                    # Contact-gated seeding for BOTH goal states
                    goal_state_world.update(z_world_curr, benefit_exposure=1.0,
                                            drive_level=1.0)
                    # SD-015 FIX: z_goal_resource updated ONLY on contact.
                    # 085h secondary seeding contaminated z_goal_resource with
                    # hundreds of non-contact proximity patterns, making it
                    # represent the average proximity field (not "resource type").
                    ...
                    # z_goal_resource does NOT use secondary seeding (key fix).
                    ...
                    # (no goal_state_resource update here -- contact-only)
...
def run(
    ...
    **kwargs,
    ...
    print(f"\n[V3-EXQ-085i] Final results:", flush=True)
    print(
    ...
    if not c2_pass:
        if c3_pass:
            failure_notes.append(
                ...
                " Next: full ResourceEncoder + E2_resource forward model (EXQ-085j)."
            )
        else:
            failure_notes.append(
    ...
    summary_markdown = (
        f"# V3-EXQ-085i -- SD-015 Contact-Only Seeding Fix\n\n"
        f"**Status:** {status}\n"
        f"**Claims:** SD-015, SD-012, MECH-112\n"
        f"**Decision:** {decision}\n"
        f"**Seeds:** {list(seeds)}\n"
        f"**Supersedes:** V3-EXQ-085h\n\n"
        f"## Diagnostic Design\n\n"
        f"Fixes 085h secondary seeding contamination. 085h: z_goal_resource seeded from"
        f" both contact events AND any near-resource step (secondary path), producing"
        f" z_goal_norm~2.7 and goal_resource_r=-0.169 (seeds 42/7). Seed 13 (contact-"
        f"dominated) achieved r=+0.223. Fix: z_goal_resource updated ONLY on resource"
        f" contact (ttype=='resource'). z_goal_world retains secondary seeding."
        f" alpha_goal_resource raised 0.3->0.5 to compensate for reduced update frequency.\n\n"
