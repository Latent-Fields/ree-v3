"""V3-EXQ-822f -- SD-082: the raw-stage discrimination index scored TRAINED-MINUS-INIT-HEAD.
Design-changed successor to V3-EXQ-822e (NOT a replay).

supersedes: V3-EXQ-822e

=============================== 822f HEADER ===============================
WHY THIS LETTER EXISTS. The confirmed autopsy failure_autopsy_V3-EXQ-822e_2026-09-05
(Fable red-team CONTESTED at Step 7c, ratified at the Step 8 gate: "no replay; 822f =
init-head control + new seeds") found two independent, load-bearing defects in 822e:
  (1) its verdict was voided by ONE readiness gate, `bias_head_actually_trained`, whose
      statistic was a SIGNED difference of two L2 norms at a 1e-3 floor -- an optimizer-
      LIVENESS test wearing a trainedness name (one Adam step at lr 5e-4 moves the
      32-parameter last layer ~2.8e-3), and it failed exactly the cells that trained INWARD;
  (2) worse, the criteria it gated (C1: raw index >= 1.0 on ARM_ON; C2: pooled flips) have
      NO init-head control, and a RANDOM-INIT head of this driver's exact shape clears C1's
      floor in 99 percent of 200 Monte-Carlo trials (median index 3.93, p5/p95 1.68/5.41)
      -- ABOVE 822e's measured ON-arm mean of 1.846. Clearing C1 is therefore not evidence
      about training, and a counterfactual PASS would have recorded `supports` for a claim
      about a TRAINED readout on criteria init structure satisfies.
  And 822e was a bit-for-bit deterministic replay of 822d's P0-P2 (28/28 shared per-cell
  statistics identical in all ten cells), so a byte-identical 822f is REFUSED: it would
  re-print every number. This letter is licensed ONLY because it changes the load-bearing
  criterion, adds the control that criterion needs, and changes the seeds.

THE FIVE RATIFIED REQUIREMENTS (autopsy routing_detail.recommended_action), AND HOW EACH IS MET:
  (a) LOAD-BEARING CRITERION = TRAINED-MINUS-INIT DISCRIMINATION INDEX (C1').
      At cell construction the substrate's own rule_bias_head is DEEP-COPIED before any
      optimizer step (`init_head`). At EVERY live P2 measurement tick the raw pre-tanh
      discrimination index (cross-candidate range / mean-abs of bias_raw(rule_state) -
      bias_raw(0); 822e's DV, unchanged) is read TWICE on the SAME candidate summaries and
      the SAME live rule_state: once through the trained head, once through the init head.
      The per-tick PAIRED difference D_tick = index_trained - index_init is the DV; the
      per-cell statistic is its median over ticks (right-skewed ratio, as in 822e).
      C1' (SD-082, load-bearing, ARM_ON): mean over seeds of D >= max(TRAINED_MINUS_INIT_ABS_FLOOR,
      1 sd of D across seeds, 1 sd of the INIT-head per-cell medians across seeds -- the
      in-run init distribution's own spread, the margin the autopsy prefers over the
      Monte-Carlo p95) AND D > 0 on >= 4/5 seeds (paired sign clause). MIRRORED for the
      weakens direction (D <= -req on >= 4/5 seeds). Everything in between is INCONCLUSIVE
      for SD-082's direction and reads, for the registry, as "discrimination within the
      init head's own spread" -- H-init-structure consistent, H1 not supported at this n.
      The 822e absolute floor (C1, index >= 1.0) and C2 (pooled flips) are still computed
      and REPORTED, now alongside the same statistics on the init head, but gate NOTHING.
  (b) NEW SEEDS: [611, 622, 633, 644, 655]. None of 101/202/303/404/505 (822a/822c/822d/822e).
  (c) TRAINEDNESS GATE RE-SPECIFIED AS A REAL DISTANCE, SCOPED TO THE TREATMENT ARM, AND
      DESCRIBED AS LIVENESS. `||W_p1 - W_init||_F` is recorded PER PARAMETER TENSOR of the
      head (head_param_distance_init_to_p1) and the readiness gate
      `bias_head_optimizer_live_on_arm` tests the last-layer WEIGHT distance on ARM_ON cells
      only against HEAD_PARAM_DISTANCE_FLOOR. It certifies that the optimizer stepped; it is
      NOT the trained-vs-init question, which (a) owns. The pool-control arm holds no veto
      (822e's second-order defect).
  (d) C4 (SD-078 contrast) KEPT, emitted through three-valued logic with its DETECTABILITY
      pre-registered: at n=5 with 822e's measured contrast_sd 1.694 the one-sided detectable
      effect at alpha 0.05 (t_(4) = 2.132) is 2.132 * 1.694 / sqrt(5) = 1.62, i.e. 6.5x the
      0.25 margin; the seed count that would deliver the margin is ~207. C4 is therefore
      DIAGNOSTIC-DETECTABILITY-SCOPED: dir_078 = supports only if the seed clause holds AND
      contrast_t_like >= SD078_T_DETECT; weakens only if the mirrored clause holds AND
      t <= -SD078_T_DETECT; `unknown` when C4's control-arm headroom fails; otherwise
      INCONCLUSIVE by construction (never a sign read off one outlier -- 822e's seed 101).
  (e) THE OFFLINE RE-SCORE OF 822e's RECORDED ROWS IS CARRIED AS PROVENANCE, NOT EVIDENCE
      (manifest `provenance_822e_offline_rescore`): with abs() on the signed delta the only
      unmet 822e precondition falls away (min |delta| 0.002076 vs 1e-3) and C1/C2 read
      PASS/PASS -- which establishes the INSTRUMENT defect and licenses nothing about SD-082.

REGISTRY (sd082_candidate_discriminating_readout_locus): this run adjudicates EXACTLY the two
complementary legs H1-trained-discriminating-readout and H-init-structure (adjudicating_runs
already carry V3-EXQ-822f). H2-uniform-common-mode, H3-upstream-summary-starvation and
H4-dead-head-capacity are NOT adjudicated here (their own probe sketches; empty adjudicating_runs)
and its verdict must not be read as bearing on them.

READINESS-FAILURE CLASS (autopsy learning 8): `substrate_not_ready_requeue` stays the label the
indexer expects, but the manifest now also carries `readiness_failure_class` = "instrument" |
"substrate" | "mixed" so a governance reader does not book substrate debt for a broken instrument.

dv_headroom (W5 rule): C1' gets `dv_headroom_trained_minus_init_index`, measured as the WORST
ARM_ON cell's tick-to-tick RANGE of the trained index (floor_headroom, signed, worst cell) against
2x TRAINED_MINUS_INIT_ABS_FLOOR: if the index cannot move by even 1.0 across ticks on this head, a
0.5 difference criterion is unresolvable. 822e's dv_headroom entries for the absolute C1 and C2
are kept as recorded entries but no longer gate anything load-bearing.

red-team (Step 4.5, DIFFERENT model, model=fable, 2026-09-08) verdict CONTESTED, 3 findings + 4
notes, every one verified against the source: F1 (the requirement imported the between-seed init-
median spread into a PAIRED criterion, predetermining `inconclusive` for a consistent moderate
effect) -> requirement is max(0.5, 1 sd of D); init spread recorded as reference; F2 (headroom read
the trained index's tick DISPERSION, monotone in n, failing a perfectly stable cell) -> headroom is
the reachable bound |D| <= max(trained, init) per cell, worst cell, margin 1.0; F3 (`weakens` fired
on H1's wording against SD-082's registered text, with the init head's dead-ReLU mask unrecorded and
the trained head possibly still discriminating) -> init-head dead-ReLU recorded per tick and the
claim-direction weakens route requires comparable masks AND the trained head failing the absolute C1
floor, while the registry state reads the raw mirrored clause; N4 (inconclusive emitted as FAIL, one
label for three paths) -> labels split, `outcome_semantics` + C1' detectability recorded; N5 (pairing
is conditional on the trained agent's state distribution) -> recorded, init-median-vs-summary-spread
correlation emitted; N6 (a flat P1 reward reads as instrument; last-layer-only distance) -> gate sums
every tensor's distance, `n_p1_optimizer_updates` recorded, `training_signal_absent` failure class;
N7 (n=5 sd noise; reachable branch fixed by which head is larger) -> recorded. No second pass (not
BLOCKING; the skill forbids iterating to CLEAR).

=== 822e HEADER (inherited design; unchanged unless the 822f header above says so) ===
...
EXPERIMENT_TYPE = "v3_exq_822f_sd082_candidate_discriminating_init_head_control"
QUEUE_ID = "V3-EXQ-822f"
SUPERSEDES = "V3-EXQ-822e"
...
SEEDS = [611, 622, 633, 644, 655]  # NEW (autopsy (b)): none of 101/202/303/404/505 -- 822e replayed 822d bit-for-bit on those.
ARMS = ["ARM_OFF", "ARM_ON"]
...
DEAD_RELU_SHIFT_CEIL = 0.25   # 822f (red-team F3): max |trained - init| dead-ReLU fraction for the weakens route
LAST_LAYER_WEIGHT_DELTA_FLOOR = 1e-3   # 822e's signed difference-of-norms statistic: RECORDED as a diagnostic only in 822f.

# --- 822f: the TRAINED-MINUS-INIT discrimination index (C1', load-bearing) -----
# D = index_trained - index_init, paired per tick on the same summaries and the same live
# rule_state; per-cell median. Requirement = max(abs floor, 1 sd of D across seeds, 1 sd of the
# init-head per-cell medians across seeds). The abs floor is half the index unit at which 822e's
# absolute bar sat ("range = mean-abs" is 1.0); the in-run init spread is the autopsy's preferred
# margin source (Monte-Carlo reference: init median 3.93, p5/p95 1.68/5.41, i.e. a spread ~1.1).
TRAINED_MINUS_INIT_ABS_FLOOR = 0.5
TRAINED_MINUS_INIT_SD_MULT = 1.0
TRAINED_MINUS_INIT_SEED_MAJORITY = 4       # of 5: paired sign clause (D > 0), one-sided sign test p = 6/32
# --- 822f: optimizer LIVENESS gate (autopsy (c)) -- a real distance, ARM_ON only ----
HEAD_PARAM_DISTANCE_FLOOR = 1e-3           # ||W_p1 - W_init||_F on the last-layer weight; liveness, not trainedness
# --- 822f: C4 detectability (autopsy (d)) ----------------------------------------
SD078_T_DETECT = 2.132                     # one-sided t, df = 4, alpha 0.05
SD078_CONTRAST_SD_822E = 1.694086          # 822e's measured contrast_sd at n=5, the detectability anchor
# --- 822f: provenance, NOT evidence (autopsy (e)) --------------------------------
PROVENANCE_822E_OFFLINE_RESCORE = {
    "source_run_id": "v3_exq_822e_sd082_candidate_discriminating_bias_spread_20260905T025644Z_v3",
    "what": "822e's recorded rows re-scored with abs() on last_layer_weight_delta_init_to_p1 (the sign-convention repair)",
    "unmet_preconditions_as_recorded": ["bias_head_actually_trained"],
    "min_abs_last_layer_delta_all_cells": 0.0020761340856552124,
    "floor": 1e-3,
    "readiness_after_abs_repair": "19/19 met",
    "c1_as_recorded": True, "c2_as_recorded": True, "c4_as_recorded": False,
    "on_arm_mean_median_ratio": 1.8458181978630435,
    "pooled_flip_frac": 0.05834242093784078,
    "sd078_contrast_mean_sd_t": [0.054073332278493515, 1.694086313904243, 0.07137277820631725],
    "reading": ("Establishes the INSTRUMENT defect only: the one unmet gate falls away under the correct "
                "statistic. It does NOT license supports/PASS for SD-082, because C1/C2 are satisfied by an "
                "untrained head (autopsy Section 3b). Recorded per autopsy requirement (e)."),
}
...
def _raw_stage_prop(lpfc, summaries: torch.Tensor, head=None, check_replica: bool = True
                    ) -> Optional[Tuple[float, float, float, float, float, float]]:
    """THE RAW PRE-TANH PROPAGATION. Returns
    (raw_spread, raw_delta, raw_ratio, replica_atanh_abs_err, max_bias_over_scale, argmax_flip).

    822f: `head` selects WHICH head reads the summaries -- the substrate's own trained
    `lpfc.rule_bias_head` (default) or the deep-copied INIT head. Both read the SAME centered
    summaries and the SAME live `lpfc.rule_state`, so the only thing that differs between the
    two calls at a tick is the head's weights. The atanh replica cross-check is a property of
    the substrate's OWN output stage and is skipped (err/sat = 0.0) for the init head, which
    has no substrate output to compare against. `argmax_flip` (stage-invariant, see
    _prop_delta_and_flip_with_diag) is returned so the init head's flip rate can be reported.
    ...
    with torch.no_grad():
        ...
        # 822f red-team F3: the hidden dead-ReLU fraction of THIS head on THIS input (the
        # substrate only latches it for its own head), so the init head's mask is recorded too.
        _joined = torch.cat([lpfc.rule_state.expand(k, -1), s], dim=-1)
        _hidden = head_mod[1](head_mod[0](_joined))
        dead_frac = float((_hidden <= 0).float().mean().item())
        b1 = lpfc.compute_bias(summaries).detach().clone().reshape(-1) if check_replica else None
    ...
    vals = (raw_spread, raw_delta, raw_ratio, err, sat, flip, dead_frac)
...
def _prop_delta_and_flip_with_diag(
    agent: REEAgent, summaries: torch.Tensor, init_head=None
    ...
    try:
        ...
        # 822f (autopsy (a)): the SAME index through the INIT head on the SAME summaries and
        # the SAME live rule_state -- the control C1' is scored against, paired per tick.
        if init_head is not None:
            raw_i = _raw_stage_prop(lpfc, summaries, head=init_head, check_replica=False)
            if raw_i is None:
                return None
            head_diag["init_raw_spread"] = raw_i[0]
            head_diag["init_raw_ratio"] = raw_i[2]
            head_diag["init_flip"] = raw_i[5]
            head_diag["init_dead_relu_frac"] = raw_i[6]
            head_diag["trained_dead_relu_frac_replica"] = raw[6]
            head_diag["trained_minus_init_ratio"] = raw[2] - raw_i[2]
...
def _run_cell(arm: str, seed: int) -> Dict[str, Any]:
    ...
    with arm_cell(seed, config_slice=_config_slice(centering),
        ...
        # 822f: the INIT head, deep-copied BEFORE any optimizer step (P0 never steps the head:
        # bias_opt.step() runs only in P1). Frozen, eval-mode, never trained -- the control C1'
        # reads at every measurement tick. The named init tensors give the real distance
        # ||W_p1 - W_init||_F per tensor for the liveness gate (autopsy (c)).
        init_head = copy.deepcopy(agent.lateral_pfc.rule_bias_head).eval()
        for _p in init_head.parameters():
            _p.requires_grad_(False)
        init_params = {n: t.detach().clone()
                       for n, t in agent.lateral_pfc.rule_bias_head.named_parameters()}

        head_diag_by_phase: Dict[str, Any] = {"init": _init_positive_control(agent)}
        head_param_distance_init_to_p1: Dict[str, float] = {}
        ...
        prop_ratios_raw: List[float] = []       # raw pre-tanh DISCRIMINATION INDEX (822e DV; reported).
        init_ratios_raw: List[float] = []       # 822f: the SAME index through the INIT head.
        trained_minus_init: List[float] = []    # 822f: paired per-tick D = trained - init (C1' DV).
        init_flips: List[float] = []            # 822f: init-head argmax flips (reported).
        init_dead: List[float] = []             # 822f: init-head dead-ReLU fraction per tick (F3 control).
        n_p1_updates = 0                        # 822f: optimizer steps actually taken in P1 (F6).
        ...
        for ep in range(total_eps):
                    ...
                    if n_active >= 1:
                        ...
                        if candidates and len(candidates) >= 2:
                            ...
                            if summ is not None and torch.isfinite(summ).all():
                                pf = _prop_delta_and_flip_with_diag(agent, summ, init_head=init_head)
                                if pf is not None:
                                    ...
                                    init_ratios_raw.append(hd["init_raw_ratio"])
                                    trained_minus_init.append(hd["trained_minus_init_ratio"])
                                    init_flips.append(hd["init_flip"])
                                    init_dead.append(hd["init_dead_relu_frac"])
            ...
            if (ep + 1) == P0_WARMUP_EPISODES:
                ...
                with torch.no_grad():
                    head_param_distance_init_to_p1 = {
                        n: float((t.detach() - init_params[n]).norm().item())
                        for n, t in agent.lateral_pfc.rule_bias_head.named_parameters()}
            elif (ep + 1) == total_eps:
        ...
        # 822f: the last Linear of the head is index 2 of the Sequential (Linear, ReLU, Linear).
        last_layer_weight_distance_init_to_p1 = float(
            head_param_distance_init_to_p1.get("2.weight", 0.0))
        total_param_distance_init_to_p1 = float(sum(head_param_distance_init_to_p1.values()))
        init_dead_relu_p2_mean = statistics.fmean(init_dead) if init_dead else 0.0
        init_ratio_raw_median = statistics.median(init_ratios_raw) if init_ratios_raw else 0.0
        tmi_median = statistics.median(trained_minus_init) if trained_minus_init else 0.0
        tmi_mean = statistics.fmean(trained_minus_init) if trained_minus_init else 0.0
        init_flip_frac = statistics.fmean(init_flips) if init_flips else 0.0
        trained_ratio_tick_range = ((max(prop_ratios_raw) - min(prop_ratios_raw))
                                    if len(prop_ratios_raw) >= 2 else 0.0)
        ...
        row = {
            ...
            # --- 822f: init-head control statistics and the real parameter distances ---
            "init_ratio_raw_median": float(init_ratio_raw_median),
            "trained_minus_init_ratio_median": float(tmi_median),
            "trained_minus_init_ratio_mean": float(tmi_mean),
            "n_init_ratio_samples": len(init_ratios_raw),
            "init_rule_flip_frac": float(init_flip_frac),
            "n_init_flips": int(sum(int(f) for f in init_flips)),
            "trained_ratio_tick_range": float(trained_ratio_tick_range),
            "head_param_distance_init_to_p1": head_param_distance_init_to_p1,
            "last_layer_weight_distance_init_to_p1": last_layer_weight_distance_init_to_p1,
            "total_param_distance_init_to_p1": total_param_distance_init_to_p1,
            "init_hidden_dead_relu_frac_p2_mean": float(init_dead_relu_p2_mean),
            "n_p1_optimizer_updates": int(n_p1_updates),
        }
        cell.stamp(row)
    _ZG.observe(agent)
...
def run_experiment() -> Dict[str, Any]:
    ...
    # 822f (autopsy (c)): the 822e statistic above is RECORDED as a diagnostic only. The gate is
    # now a real distance ||W_p1 - W_init||_F on the last-layer weight, scoped to ARM_ON (the
    # pool-control arm holds no veto over the treatment arm's criteria), and it certifies
    # optimizer LIVENESS only -- it does not and cannot test trainedness (one Adam step at lr
    # 5e-4 already moves the 32-parameter tensor ~2.8e-3). The trained-vs-init question is C1'.
    last_layer_worst, last_layer_cell = _worst_cell(
        rows, "last_layer_weight_delta_init_to_p1", mode="min")
    head_dist_worst, head_dist_cell = _worst_cell(
        on, "total_param_distance_init_to_p1", mode="min")
    head_live = bool(head_dist_worst >= HEAD_PARAM_DISTANCE_FLOOR)
    p1_updates_worst, p1_updates_cell = _worst_cell(on, "n_p1_optimizer_updates", mode="min")
    head_trained = head_live   # name kept for the metrics/reasons plumbing below; semantics = LIVENESS

    # 822f C1' headroom: the worst ARM_ON cell's tick-to-tick RANGE of the trained index must
    # exceed 2x the C1' floor -- if the index cannot move by that much across ticks on this
    # head, a 0.5 difference criterion is unresolvable. floor_headroom: signed, worst cell.
    def _headroom_m1(name, **kw):
        e = dv_headroom_check(name, margin=1.0, **kw)
        e["met"] = bool(e["measured"] >= e["threshold"])
        return e

    hr_c1p = _headroom_m1(
        "dv_headroom_trained_minus_init_index",
        dv_name="trained_minus_init_ratio_median (ARM_ON)",
        criterion_threshold=TRAINED_MINUS_INIT_ABS_FLOOR,
        control_values=[float(max(r["prop_ratio_raw_median"], r["init_ratio_raw_median"])) for r in on],
        statistic="floor_headroom", dv_bounds=(0.0, 1.0e6),
        description=("C1' reads a DIFFERENCE of two non-negative per-cell medians, so |D| is "
                     "bounded above by max(trained median, init median) in that cell; the worst "
                     "ARM_ON cell's bound must reach the C1' floor (red-team F2: the first draft "
                     "used the trained index's tick range, a dispersion that grows with n and "
                     "fails a perfectly stable cell). Signed, worst cell (floor_headroom), "
                     "margin 1.0 = bare reachability."),
        control="ARM_ON per-cell max(trained, init) raw-index median",
        leg="C1prime")
    headroom_entries.append(hr_c1p)
    # C1' is the ONLY load-bearing SD-082 criterion in 822f, so its headroom is the one that
    # gates run-level readiness; 822e's hr_c1/hr_c2 remain recorded entries (their criteria
    # are reported, not load-bearing) and no longer gate.
    headroom_ok = bool(hr_c1p["met"])
    ...
    ready = (cone_present and on_diff_pool and off_pinned_pool and on_active
             ...
             and flip_resolution_ok and head_live and headroom_ok)
    # autopsy learning 8: which KIND of readiness failed, so a governance reader does not
    # book substrate debt for a broken instrument.
    _instrument_gates = {"replica_ok": replica_ok, "uniform_reads_uniform": uniform_reads_uniform,
                         "controls_sampled": controls_sampled, "flip_resolution_ok": flip_resolution_ok,
                         "head_live": head_live, "headroom_ok": headroom_ok,
                         "capture_active": capture_active, "head_diag_sufficient": head_diag_sufficient,
                         "spread_measurable": spread_measurable, "prop_samples_sufficient": prop_samples_sufficient}
    _substrate_gates = {"cone_present": cone_present, "on_diff_pool": on_diff_pool,
                        "off_pinned_pool": off_pinned_pool, "on_active": on_active, "fix_engaged": fix_engaged}
    _inst_fail = [k for k, v in _instrument_gates.items() if not v]
    _sub_fail = [k for k, v in _substrate_gates.items() if not v]
    readiness_failure_class = ("none" if ready else
                               ("training_signal_absent" if (_inst_fail == ["head_live"] and p1_updates_worst == 0 and not _sub_fail) else
                                ("mixed" if (_inst_fail and _sub_fail) else
                                 ("instrument" if _inst_fail else "substrate"))))
    ...
    # --- 822f C1': TRAINED-MINUS-INIT discrimination index, ARM_ON (LOAD-BEARING) ---
    # The autopsy's requirement (a). Everything above about C1 is kept and REPORTED; C1 no
    # longer gates anything because an untrained head clears its floor 99 percent of the time.
    per_arm_tmi: Dict[str, Any] = {}
    for label_arm, group in (("ARM_ON", on), ("ARM_OFF", off)):
        d_vals = [float(r["trained_minus_init_ratio_median"]) for r in group]
        i_vals = [float(r["init_ratio_raw_median"]) for r in group]
        d_mean = statistics.fmean(d_vals) if d_vals else 0.0
        d_sd = statistics.stdev(d_vals) if len(d_vals) >= 2 else 0.0
        i_sd = statistics.stdev(i_vals) if len(i_vals) >= 2 else 0.0
        # Red-team F1: D is PAIRED per tick (same summaries, same rule_state), so the between-
        # seed spread of the init medians is summary geometry both heads see identically --
        # nuisance variance the pairing removed. It is RECORDED (init_median_sd) as the
        # in-run reference for the init distribution, never part of the requirement.
        req = max(TRAINED_MINUS_INIT_ABS_FLOOR, TRAINED_MINUS_INIT_SD_MULT * d_sd)
        det_effect = (SD078_T_DETECT * d_sd / math.sqrt(len(d_vals))) if (d_vals and d_sd > 0.0) else float("nan")
        n_pos = sum(1 for d in d_vals if d > 0.0)
        n_neg = sum(1 for d in d_vals if d < 0.0)
        per_arm_tmi[label_arm] = {
            "per_seed_trained_minus_init_median": {r["seed"]: r["trained_minus_init_ratio_median"]
                                                   for r in sorted(group, key=lambda x: x["seed"])},
            "per_seed_init_ratio_median": {r["seed"]: r["init_ratio_raw_median"]
                                           for r in sorted(group, key=lambda x: x["seed"])},
            "per_seed_trained_ratio_median": {r["seed"]: r["prop_ratio_raw_median"]
                                              for r in sorted(group, key=lambda x: x["seed"])},
            "d_mean": d_mean, "d_sd": d_sd, "init_median_sd": i_sd, "requirement": req,
            "detectable_effect_at_this_n": det_effect,
            "paired_t_like": (d_mean / (d_sd / math.sqrt(len(d_vals)))) if (d_vals and d_sd > 0.0) else float("nan"),
            "init_dead_relu_p2_mean": {r["seed"]: r["init_hidden_dead_relu_frac_p2_mean"] for r in sorted(group, key=lambda x: x["seed"])},
            "trained_dead_relu_p2_mean": {r["seed"]: r["hidden_dead_relu_frac_p2_mean"] for r in sorted(group, key=lambda x: x["seed"])},
            "n_seeds_positive": int(n_pos), "n_seeds_negative": int(n_neg), "n_seeds": len(d_vals),
            "supports": bool(len(d_vals) == len(SEEDS) and d_mean >= req
                             and n_pos >= TRAINED_MINUS_INIT_SEED_MAJORITY),
            "weakens": bool(len(d_vals) == len(SEEDS) and d_mean <= -req
                            and n_neg >= TRAINED_MINUS_INIT_SEED_MAJORITY),
        }
    c1p = bool(per_arm_tmi["ARM_ON"]["supports"])
    c1p_negative_raw = bool(per_arm_tmi["ARM_ON"]["weakens"])
    # Red-team F3: a negative D has two uncontrolled routes -- training re-allocating dead
    # hidden units (H4, not adjudicated here) and a trained head that is still discriminating
    # in absolute terms (SD-082's registered text asserts a trainable readout that
    # discriminates; it does not assert training INCREASES discrimination over init). The
    # mirrored clause therefore counts toward `weakens` ONLY when (i) both heads' dead-ReLU
    # fractions are below DEAD_RELU_MED_FLOOR and differ by <= DEAD_RELU_SHIFT_CEIL in every
    # ARM_ON cell, and (ii) the trained head FAILS the absolute C1 floor on ARM_ON (it is not
    # discriminating on its own). Otherwise a negative D is recorded, the registry reads
    # H-init-structure as supported, and SD-082's direction stays `inconclusive`.
    _dead_ok = all(
        r["hidden_dead_relu_frac_p2_mean"] < DEAD_RELU_MED_FLOOR
        and r["init_hidden_dead_relu_frac_p2_mean"] < DEAD_RELU_MED_FLOOR
        and abs(r["hidden_dead_relu_frac_p2_mean"] - r["init_hidden_dead_relu_frac_p2_mean"]) <= DEAD_RELU_SHIFT_CEIL
        for r in on)
    weakens_route_controls = {"dead_relu_masks_comparable_all_on_cells": bool(_dead_ok),
                              "trained_head_fails_absolute_c1": bool(not c1)}
    c1p_negative = bool(c1p_negative_raw and _dead_ok and (not c1))
    c1pb_off = per_arm_tmi["ARM_OFF"]   # reported only, never gating (same reasoning as C1b)
    # init-head flip fraction, pooled -- C2's own init control (reported)
    init_total_flips = sum(int(r["n_init_flips"]) for r in rows)
    ...
    # 822f: overall PASS is SD-082's question, now asked against the init control -- C1'
    # alone. C1 and C2 are reported.
    overall = bool(ready and c1p)
    ...
    c1p_non_degenerate = bool(
        base_non_degenerate
        and all(r["n_init_ratio_samples"] > 0 for r in on)
        and per_arm_tmi["ARM_ON"]["d_sd"] > 0.0)
    ...
    # 822f label ladder: the trained-vs-init question first (it is the only load-bearing one).
    if not ready:
        label = "substrate_not_ready_requeue"
    elif c1p:
        label = "sd082_trained_readout_discriminates_beyond_init_head"
    elif c1p_negative:
        label = "trained_readout_less_discriminating_than_init_head"
    elif c1p_negative_raw:
        label = "init_structure_dominates_but_weakens_route_uncontrolled_inconclusive"
    elif per_arm_tmi["ARM_ON"]["d_mean"] >= per_arm_tmi["ARM_ON"]["requirement"]:
        label = "c1prime_mean_clears_but_sign_majority_fails_inconclusive"
    elif per_arm_tmi["ARM_ON"]["n_seeds_positive"] >= TRAINED_MINUS_INIT_SEED_MAJORITY:
        label = "c1prime_consistent_sign_but_mean_below_requirement_inconclusive"
    else:
        label = "discrimination_within_init_head_spread_inconclusive"
    ...
    # 822f: dir_082 is a function of C1' ONLY (trained-minus-init, ARM_ON). `supports` needs
    # positive transfer beyond the init head's own spread; `weakens` needs the trained head
    # to be LESS discriminating than its init by the same margin (training subtracted
    # discrimination -- the init structure IS the discrimination); everything else is
    # inconclusive. Note `reads_uniform` (822e's weakens route) is recorded but no longer
    # sets a direction: an absolute index near 0 in BOTH heads is a statement about the
    # test bed, not about training.
    # dir_078 is a function of C4 ONLY, through the DETECTABILITY clause (autopsy (d)).
    sd078_detectable_effect = (SD078_T_DETECT * contrast_sd / math.sqrt(len(deltas))
                               if deltas and contrast_sd > 0.0 else float("nan"))
    sd078_n_for_margin = (int(math.ceil((SD078_T_DETECT * contrast_sd / SD078_CONTRAST_MARGIN) ** 2))
                          if contrast_sd > 0.0 else None)
    c4_seed_clause = bool(len(per_seed_contrast) == len(SEEDS) and n_contrast_seeds >= SD078_CONTRAST_SEEDS)
    c4_neg_seed_clause = bool(len(per_seed_contrast) == len(SEEDS) and n_contrast_seeds_neg >= SD078_CONTRAST_SEEDS)
    if not ready:
        ...
        if not sd078_headroom_ok:
            dir_078 = "unknown"
        elif c4_seed_clause and contrast_t_like >= SD078_T_DETECT:
            dir_078 = "supports"
        elif c4_neg_seed_clause and contrast_t_like <= -SD078_T_DETECT:
            dir_078 = "weakens"
        else:
            dir_078 = "inconclusive"
        overall_dir = dir_082
    registry_adjudication = {
        "question": "sd082_candidate_discriminating_readout_locus",
        "hypotheses": {
            "H1-trained-discriminating-readout": {
                "adjudicated": bool(ready),
                "state": ("supported" if (ready and c1p) else ("eliminated" if (ready and c1p_negative_raw)
                          else ("alive_not_supported_at_this_n" if ready else "unadjudicated")))},
            "H-init-structure": {
                "adjudicated": bool(ready),
                "state": ("eliminated" if (ready and c1p) else ("supported" if (ready and c1p_negative_raw)
                          else ("consistent_within_init_spread" if ready else "unadjudicated"))),
                "note": "registry state reads the raw mirrored clause; the CLAIM direction additionally requires the weakens_route_controls (red-team F3)"},
            "H2-uniform-common-mode": {"adjudicated": False, "state": "alive"},
            "H3-upstream-summary-starvation": {"adjudicated": False, "state": "alive"},
            "H4-dead-head-capacity": {"adjudicated": False, "state": "alive"},
        },
        "note": ("H1 and H-init-structure are complementary under one measurement (C1'); the other "
                 "three legs are NOT adjudicated by this run (autopsy fanout not_assigned_to_822f)."),
    }
    ...
    metrics = {
        ...
        "last_layer_weight_delta_note": "822e's SIGNED difference-of-norms statistic, RECORDED ONLY in 822f (not a gate)",
        "readiness_head_live": head_live,
        "head_param_distance_last_layer_worst_on_arm": float(head_dist_worst),
        "head_param_distance_worst_cell": head_dist_cell,
        "head_param_distance_by_cell": {f"{r['arm']}/seed{r['seed']}": r["head_param_distance_init_to_p1"] for r in rows},
        "readiness_failure_class": readiness_failure_class,
        # --- 822f LOAD-BEARING: trained-minus-init index ---------------------------
        "trained_minus_init_per_arm": per_arm_tmi,
        "c1prime_pass": c1p,
        "c1prime_negative": c1p_negative,
        "init_head_pooled_flip_frac": float(init_total_flips / total_flip_ticks) if total_flip_ticks else 0.0,
        "n_init_flips_pooled": int(init_total_flips),
        "sd078_detectable_effect_at_this_n": sd078_detectable_effect,
        "sd078_n_seeds_for_margin": sd078_n_for_margin,
        "sd078_t_detect": SD078_T_DETECT,
        "weakens_route_controls": weakens_route_controls,
        "c1prime_negative_raw": c1p_negative_raw,
        "init_median_vs_summary_spread_corr_on_arm": (lambda xs, ys: (float(np.corrcoef(xs, ys)[0, 1]) if len(xs) >= 3 and np.std(xs) > 0 and np.std(ys) > 0 else None))(
            [r["init_ratio_raw_median"] for r in on], [r["summary_spread_ratio_p2_median"] for r in on]),
        "n_p1_optimizer_updates_worst_on_arm": int(p1_updates_worst),
        "n_p1_optimizer_updates_worst_cell": p1_updates_cell,
        "outcome_semantics": "outcome FAIL covers both a null and every inconclusive path (family convention since 822e); read evidence_direction_per_claim and interpretation.label, never outcome alone",
    ...
    result: Dict[str, Any] = {
        ...
        "interpretation": {
            ...
            "preconditions": [
                ...
                {"name": "bias_head_optimizer_live_on_arm",
                 "description": (
                     "LIVENESS, not trainedness (822f, autopsy (c)): the real distance "
                     "||W_p1 - W_init||_F SUMMED over every head parameter tensor (red-team F6: "
                     "the first layer moves ~8x more than the last and is what changes ReLU masks), worst "
                     "ARM_ON cell, against HEAD_PARAM_DISTANCE_FLOOR. Replaces 822e's "
                     "signed difference-of-norms gate, which failed exactly the cells "
                     "that trained inward. Scoped to the TREATMENT arm: the pool-control "
                     "arm holds no veto. This certifies the optimizer stepped and nothing "
                     "more -- the trained-vs-init question is C1'. Per-tensor distances "
                     "are recorded in per_seed_rows[].head_param_distance_init_to_p1."),
                 "measured": float(head_dist_worst),
                 "threshold": float(HEAD_PARAM_DISTANCE_FLOOR),
                 "direction": "lower", "offending_cell": head_dist_cell,
                 "control": "init tensors captured before the first optimizer step",
                 "met": head_live},
            ...
            "criteria": [
                {"name": "C1prime_trained_minus_init_discrimination_index_on_arm",
                 "load_bearing": True, "passed": c1p,
                 "detail": (
                     f"822f's LOAD-BEARING SD-082 criterion (autopsy (a)). At every live P2 "
                     f"tick the raw pre-tanh discrimination index is read through the "
                     f"TRAINED head and through the deep-copied INIT head on the SAME "
                     f"summaries and the SAME live rule_state; D = trained - init, paired "
                     f"per tick, per-cell median. ARM_ON: mean D "
                     f"{per_arm_tmi['ARM_ON']['d_mean']:.4g} (sd "
                     f"{per_arm_tmi['ARM_ON']['d_sd']:.4g}; init-median sd "
                     f"{per_arm_tmi['ARM_ON']['init_median_sd']:.4g}) against requirement "
                     f"max({TRAINED_MINUS_INIT_ABS_FLOOR}, sd_D, sd_init) = "
                     f"{per_arm_tmi['ARM_ON']['requirement']:.4g}; seeds with D > 0: "
                     f"{per_arm_tmi['ARM_ON']['n_seeds_positive']}/{per_arm_tmi['ARM_ON']['n_seeds']} "
                     f"(required {TRAINED_MINUS_INIT_SEED_MAJORITY}). Mirrored weakens clause: "
                     f"c1prime_negative={c1p_negative}. ARM_OFF (reported, never gating): mean D "
                     f"{c1pb_off['d_mean']:.4g}, positive {c1pb_off['n_seeds_positive']}/"
                     f"{c1pb_off['n_seeds']}. An absolute index a random-init head clears 99 "
                     f"percent of the time is NOT a criterion; this is.")},
                {"name": "C1_raw_discrimination_index_on_arm",
                 "load_bearing": False, "passed": c1,
                 "detail": (
                ...
                {"name": "C1b_same_floor_on_pool_control_arm",
                 ...
                 "detail": (
                     ...
                     f"image.")},
                {"name": "C2_argmax_flip_pooled", "load_bearing": False, "passed": c2,
                 "detail": (
            ...
            "combination_rule_822f": (
                "overall PASS = readiness AND C1' (trained-minus-init discrimination index, "
                "ARM_ON, noise-aware against both the D spread and the in-run init-head "
                "spread, plus a 4/5 paired sign clause). dir_082 = supports | weakens "
                "(mirrored clause) | inconclusive, from C1' ONLY. C1 (absolute index) and C2 "
                "(pooled flips) are REPORTED with their init-head counterparts and gate "
                "nothing. dir_078 = C4 through the detectability clause: supports/weakens "
                "need the seed clause AND |t_like| >= SD078_T_DETECT; otherwise inconclusive; "
                "unknown when C4's control-arm headroom fails. Readiness now includes the "
                "ARM_ON-scoped liveness distance gate and dv_headroom_trained_minus_init_index; "
                "822e's hr_c1/hr_c2 are recorded entries only. The 822e combination_rule text "
                "below is retained for provenance and is SUPERSEDED by this paragraph."),
            "combination_rule": (
            ...
            "criteria_non_degenerate": {
                "C1prime_trained_minus_init_discrimination_index_on_arm": c1p_non_degenerate,
            ...
            "claim_criterion_map": {
                "SD-082": {
                    "criteria": ["C1prime_trained_minus_init_discrimination_index_on_arm"],
                    "reported_only": ["C1_raw_discrimination_index_on_arm", "C2_argmax_flip_pooled"],
                    "role": "primary",
        ...
        "provenance_822e_offline_rescore": PROVENANCE_822E_OFFLINE_RESCORE,
        "readiness_failure_class": readiness_failure_class,
    }
    ...
    if not head_live:
        reasons.append(f"bias head optimizer NOT LIVE on the treatment arm (worst ARM_ON cell "
                       f"{head_dist_cell}: last-layer weight distance {head_dist_worst:g} < "
                       f"{HEAD_PARAM_DISTANCE_FLOOR:g}) -- instrument, not substrate")
    if not headroom_ok:
        unmet = [e["name"] for e in (hr_c1p,) if not e["met"]]
        reasons.append("dv_headroom unmet -- a load-bearing bar lies outside the "
                       "range its DV can reach: " + ", ".join(unmet))
    if reasons or not (c1p_non_degenerate and c4_non_degenerate):
        if not c1p_non_degenerate:
            reasons.append("C1' degenerate: init-head index not sampled on every ARM_ON cell, "
                           "or the per-seed trained-minus-init medians have no spread")
...
def main():
    ...
    full_config = {
        ...
        "trained_minus_init_abs_floor": TRAINED_MINUS_INIT_ABS_FLOOR,
        "trained_minus_init_sd_mult": TRAINED_MINUS_INIT_SD_MULT,
        "trained_minus_init_seed_majority": TRAINED_MINUS_INIT_SEED_MAJORITY,
        "head_param_distance_floor": HEAD_PARAM_DISTANCE_FLOOR,
        "sd078_t_detect": SD078_T_DETECT,
        "sd078_contrast_sd_822e": SD078_CONTRAST_SD_822E,
        ...
        "supersedes_note_822f": ("SUPERSEDES V3-EXQ-822e as a DESIGN CHANGE (trained-minus-init "
                                 "index load-bearing, new seeds, ARM_ON-scoped liveness distance gate, "
                                 "C4 detectability-scoped) per confirmed failure_autopsy_V3-EXQ-822e_2026-09-05; "
                                 "a byte-identical replay was REFUSED. The 822e note below is inherited."),
    ...
    manifest = {
        ...
        "registry_adjudication": result["registry_adjudication"],
        "provenance_822e_offline_rescore": result["provenance_822e_offline_rescore"],
        "readiness_failure_class": result["readiness_failure_class"],
        "h_legs_not_adjudicated": ["H2-uniform-common-mode", "H3-upstream-summary-starvation", "H4-dead-head-capacity"],
    }
    ...
    _t = m["trained_minus_init_per_arm"]["ARM_ON"]
    print(f"C1' trained-minus-init (ARM_ON): mean D={_t['d_mean']:.4g} sd={_t['d_sd']:.4g} "
          f"init_sd={_t['init_median_sd']:.4g} req={_t['requirement']:.4g} "
          f"pos={_t['n_seeds_positive']}/{_t['n_seeds']} C1'={m['c1prime_pass']} "
          f"neg={m['c1prime_negative']}", flush=True)
    print(f"init-head index per seed (ARM_ON): {_t['per_seed_init_ratio_median']}", flush=True)
    print(f"liveness: last-layer distance worst ARM_ON={m['head_param_distance_last_layer_worst_on_arm']:.4g} "
          f"(floor {HEAD_PARAM_DISTANCE_FLOOR:g}) live={m['readiness_head_live']} "
          f"readiness_failure_class={m['readiness_failure_class']}", flush=True)
    print(f"registry: {result['registry_adjudication']['hypotheses']['H1-trained-discriminating-readout']['state']} / "
          f"init-structure {result['registry_adjudication']['hypotheses']['H-init-structure']['state']}", flush=True)
