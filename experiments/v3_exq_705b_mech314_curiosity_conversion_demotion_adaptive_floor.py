"""V3-EXQ-705b: MECH-314 structured-curiosity conversion to committed-action-class
diversity WITH the MECH-448/ARC-107 rank-preserving F->eligibility demotion ON --
CHANNEL-ADAPTIVE envelope floor (use_f_eligibility_adaptive_floor) ENABLED.

SUPERSEDES V3-EXQ-705 (a LETTER iteration, NOT a new number: the scientific
question -- does the MECH-314 curiosity channel convert to committed-action-class
diversity once F is demoted out of the committed argmin -- is UNCHANGED; the
IMPLEMENTATION was wrong). Re-queue routed by the confirmed, user-gated
failure_autopsy_V3-EXQ-705_2026-06-25.

=== WHY 705 SELF-ROUTED non_contributory / substrate_not_ready_requeue ===

705 self-routed substrate_not_ready_requeue / non_contributory / non_degenerate=
false because readiness leg C (f_eligibility_demotion_non_degeneracy) FAILED on
2/3 seeds: on the harm-free foraging pool (num_hazards=0) the F-score over the
curiosity-divergent candidates is near-tied, so under the FIXED
f_eligibility_envelope_floor=0.30 absolute share floor EVERY candidate's merit-
share fell below the floor -> empty-eligible -> WIDE all-admit fallback (seeds
43/44: excluded_count==0.0, envelope_size==32.0). The MECH-448 demotion lever
FIRED (active_frac 1.0) but never NARROWED, so MECH-314 was not fairly tested
under a narrowing demotion. The below-control ARM_CURIOSITY committed entropy was
a degeneracy of the all-admit (novelty argmin over a near-whole pool = mono-
strategy), NOT a MECH-314 weakening. This is the V3-EXQ-642 invalid-precondition
family (the correctly-CAUGHT case), NOT a substrate ceiling.

=== THE FIX (config only; substrate already built + validated) ===

Enable the CHANNEL-ADAPTIVE mean-relative envelope floor on every arm's
REEConfig.from_dims: use_f_eligibility_adaptive_floor=True +
f_eligibility_adaptive_mean_factor=1.0 (landed + validated 2026-06-21,
V3-EXQ-689e: adaptive_excluded_count_mean 14.8-15.7 of 32 candidates reliably, vs
the fixed floor's 0.0-0.73). The adaptive floor is computed RELATIVE to the
field's own mean merit-share (floor = mean_factor * elig.mean(); e3_selector.py),
so by construction mean_factor >= 1.0 on any NON-uniform field excludes at least
one candidate (excluded_count > 0) while staying rank-preserving (still a
threshold on the merit-share -> F-rank prefix). The 705 fixed-floor all-admit
cannot recur. NOTHING ELSE in the 705 config changes (the 590c confounded-
precondition fix is retained verbatim -- see the CONFOUNDED-PRECONDITION FIX
block below).

=== HARDENED legC READINESS GATE (autopsy requirement) ===

Readiness leg C is the load-bearing non-vacuity gate: it requires
f_eligibility_excluded_count_mean > EXCLUDED_COUNT_FLOOR (==0) AND
demotion_active_frac >= floor on >= MIN_SEEDS seeds. The verdict resolver checks
`not readiness_ok` FIRST, so the committed_class_entropy LIFT criterion is scored
ONLY after legC is met -- a residual all-admit (excluded_count==0) still
self-routes substrate_not_ready_requeue, NEVER a false reading. This is the
re-stated precondition the autopsy mandates.

=== RE-DERIVE BRAKE: EXEMPT now, BRAKE-LOCK trigger on 705b ===

705 was brake-EXEMPT (a confounded / un-exercised-substrate reading: the demotion
all-admitted, so it is NOT a clean substrate_ceiling at the same granularity
against the same EXERCISED substrate; firing the brake on a confounded run would
be illusory-resolution -- precedent failure_autopsy_V3-EXQ-701a). 4 prior MECH-314
substrate_ceiling autopsies exist. BRAKE-LOCK (recorded on the 705 autopsy
target): if 705b CLEARS legC (the channel-adaptive demotion genuinely excludes,
excluded_count>0 on >=2/3 seeds, MECH-314 fairly tested under a NARROWING
demotion) AND committed_class_entropy STILL shows no lift over the demotion-ON
F-only control + matched-noise control -> THAT is the genuine
conversion_ceiling_persists_despite_demotion off-ramp -> the 705b autopsy fires
the re-derive brake (6th) + routes the curiosity-conversion question to MECH-449
Go/No-Go (double-gated) / V4. So 705b is the brake-LOCK trigger, not exempt
forever.

=== ORIGINAL 705 DESIGN (substrate the demotion enables; unchanged) ===

Routed to by failure_autopsy_V3-EXQ-590c_2026-06-24 (a DIFFERENT-SUBSTRATE
redesign, not another curiosity_novelty_weight sweep against the bare F-dominated
argmin -- that 590a/590b/590c family is brake-FIRED, 4th MECH-314 ceiling
autopsy). The upstream substrate the 590c autopsy routed to
...
Smoke:
  /opt/local/bin/python3 experiments/v3_exq_705b_mech314_curiosity_conversion_demotion_adaptive_floor.py --dry-run
"""
...
EXPERIMENT_TYPE = "v3_exq_705b_mech314_curiosity_conversion_demotion_adaptive_floor"
QUEUE_ID = "V3-EXQ-705b"
CLAIM_IDS: List[str] = ["MECH-314"]
EXPERIMENT_PURPOSE = "evidence"
SUPERSEDES: Optional[str] = "V3-EXQ-705"  # config-only fix: enable channel-adaptive floor
...
# MECH-448 (ARC-107) demotion lever config (ON every arm).
# THE 705->705b FIX: channel-adaptive mean-relative floor (689e-validated) replaces
# the FIXED absolute share floor that all-admitted on the near-tie-F foraging pool.
# With use_f_eligibility_adaptive_floor=True the fixed F_ELIGIBILITY_ENVELOPE_FLOOR
# is INERT (the adaptive branch reads f_eligibility_adaptive_mean_factor instead);
# kept here only for the audit record of what 705 used.
...
def _make_agent(env: CausalGridWorldV2, arm: Dict[str, Any]) -> REEAgent:
    ...
    cfg = REEConfig.from_dims(
        ...
        # THE 705->705b FIX: channel-adaptive mean-relative floor (689e-validated) so
        # the demotion ACTUALLY EXCLUDES on the near-tie-F foraging pool (excluded_count
        # > 0 by construction at mean_factor>=1.0) instead of the 705 fixed-floor
        # all-admit. The fixed envelope_floor below is INERT under the adaptive branch.
...
def run_experiment(dry_run: bool = False) -> Dict[str, Any]:
    ...
    manifest: Dict[str, Any] = {
        ...
        "claim_ids": CLAIM_IDS,
        "supersedes": "v3_exq_705_mech314_curiosity_conversion_demotion",
        "evidence_direction": evidence_direction,
        "evidence_direction_note": (
            "V3-EXQ-705b SUPERSEDES V3-EXQ-705 (config-only fix, routed by the "
            "confirmed failure_autopsy_V3-EXQ-705_2026-06-25). MECH-314 structured-"
            "curiosity conversion to committed-action-class diversity WITH the "
            "MECH-448/ARC-107 rank-preserving F->eligibility demotion ON (every arm), "
            "on the 648a/590c GAP-A-ready substrate (curiosity_candidate_source="
            "'e2_world_forward' + SD-056 online + rollout clamp + SP-CEM + V_s), "
            "MECH-314 curiosity the SOLE modulatory channel. THE 705->705b FIX: the "
            "CHANNEL-ADAPTIVE mean-relative envelope floor (use_f_eligibility_adaptive_"
            "floor=True + f_eligibility_adaptive_mean_factor=1.0, 689e-validated) "
            "replaces 705's FIXED f_eligibility_envelope_floor=0.30, which all-admitted "
            "on the near-tie-F harm-free foraging pool (705 legC excluded_count==0.0 / "
            "envelope==32.0 on 2/3 seeds -> demotion fired but never narrowed -> "
            "MECH-314 was not fairly tested = V3-EXQ-642 invalid-precondition family, "
            "NOT a ceiling). The adaptive floor excludes (excluded_count>0) by "
            "construction on any non-uniform field while staying rank-preserving. The "
            "contrast is channel PRESENCE (ARM_CURIOSITY w=0.25 vs ARM_FONLY w=0.0) "
            "under demotion, NOT two curiosity scales (argmin(w*curiosity) is scale-"
            "invariant; that is why the 590c sweep was vacuous). PASS (supports "
            "MECH-314) = ARM_CURIOSITY committed_class_entropy lifts over the demotion-"
            "ON F-only control by >= margin on >= 2/3 seeds AND strict-above the "
            "matched-noise control (ARM_NOISE). MANDATORY non-vacuity (HARDENED legC, "
            "load-bearing BEFORE the lift criterion is scored): if the GAP-A pool is "
            "not divergent, the curiosity bias carries no per-candidate range at the "
            "measured NON-saturation arm (the 590c confounded-precondition FIX), or the "
            "demotion envelope does NOT actually exclude on the divergent pool "
            "(f_eligibility_excluded_count_mean==0 = residual all-admit), the run self-"
            "routes substrate_not_ready_requeue (non_contributory), NEVER a weakens. A "
            "readiness-MET no-lift is conversion_ceiling_persists_despite_demotion "
            "(non_contributory; the genuine off-ramp) -> fires the BRAKE-LOCK: the 705b "
            "autopsy fires the re-derive brake (6th) + routes to MECH-449 Go/No-Go "
            "(double-gated) / V4. Also NOT a weakens. claim_ids=[MECH-314] (re-evaluated "
            "from scratch: the demotion is the enabling substrate, MECH-314 is the claim "
            "under test). PROMOTES NOTHING."
        ),
        "interpretation": {
            "label": summary["label"],
            ...
            "routing": {
                ...
                "conversion_ceiling_persists_despite_demotion": (
                    "BRAKE-LOCK off-ramp (705b is the trigger): legC MET (demotion "
                    "genuinely excludes, excluded_count>0 on >=2/3 seeds) but no lift "
                    "over the demotion-ON F-only + matched-noise controls -> the 705b "
                    "autopsy fires the re-derive brake (6th) + routes to MECH-449 "
                    "Go/No-Go (double-gated) / V4. non_contributory; NOT a "
