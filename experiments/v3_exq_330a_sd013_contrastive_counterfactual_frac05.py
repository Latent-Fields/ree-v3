...
"""
V3-EXQ-330a: SD-013 Contrastive Counterfactual Training -- interventional_fraction=0.5

experiment_purpose: evidence

Retest of EXQ-330 with interventional_fraction raised from 0.3 to 0.5.

EXQ-330 FAIL diagnosis: cf_gap_ratio=1.29-1.49x (threshold 2.0) in CONTRASTIVE condition.
The contrastive margin loss was applied to only 30% of P1 training steps. Hypothesis:
insufficient interventional pressure -- observational MSE dominated, preventing the
forward model from developing action-sensitive discrimination.

Fix: interventional_fraction=0.5 (50% of steps apply the margin loss alongside MSE).
This increases interventional pressure while keeping the MSE loss dominant enough
to maintain forward model accuracy.

Same scientific question as EXQ-330 (SD-013 primary test):
Does contrastive interventional training (margin loss on a_actual vs a_cf) improve
counterfactual attribution quality compared to observational-only training?

Two conditions per seed:
  CONTRASTIVE:   use_interventional=True, interventional_fraction=0.5
  OBSERVATIONAL: use_interventional=False (MSE-only training)

Phased training (P0: encoder warmup, P1: forward model training, P2: evaluation).

Pass criteria:
  C1: CONTRASTIVE forward_r2 >= 0.7 (forward model not degraded by increased margin loss)
  C2: CONTRASTIVE cf_gap > OBSERVATIONAL cf_gap * 2.0 (stronger threshold than EXQ-330)
  C3: attribution_sign_correct >= 0.65 in CONTRASTIVE condition

PASS: C1 AND C2 AND C3 across >= 2/3 seeds.

Supersedes: V3-EXQ-330 (same hypothesis, interventional_fraction bug: 0.3 too low).
Claims: SD-013 (primary). ARC-033 tagged for C1 (forward model integrity check).
Per-claim direction required (SD-013 and ARC-033 may differ).

Note: SD-003 is intentionally NOT tagged here. EXQ-329 already provides strong
PASS evidence for SD-003/ARC-033. This experiment tests SD-013 specifically
(interventional training benefit). See claim_ids accuracy rule.
"""
...
# Experiment metadata
# ---------------------------------------------------------------------------
EXPERIMENT_TYPE    = "v3_exq_330a_sd013_contrastive_counterfactual_frac05"
CLAIM_IDS          = ["SD-013", "ARC-033"]
EXPERIMENT_PURPOSE = "evidence"
...
# Pass thresholds
C1_FORWARD_R2_THRESH   = 0.7    # forward model intact
C2_CF_GAP_RATIO        = 2.0    # stronger than EXQ-330 (1.5) to confirm genuine improvement
C3_SIGN_CORRECT_THRESH = 0.65
MIN_SEEDS_PASS         = 2

# Interventional fraction: key change from EXQ-330 (was 0.3)
...
def run_condition(
    ...
    for ep in range(total_eps):
        ...
        for step in range(steps_per):
            ...
            # Collect z_harm_s transitions for replay
            ...
            if in_p1 and len(replay_buf) >= BATCH_SIZE:
                ...
                if use_interventional and random.random() < harm_fwd_cfg.interventional_fraction:
                    # Counterfactual actions: different from a_actual for each item
    ...
    c1_ok = forward_r2 >= C1_FORWARD_R2_THRESH
    verdict = "PASS" if c1_ok else "FAIL"
    print(
        f"verdict: {verdict} forward_r2={forward_r2:.3f} cf_gap={mean_cf_gap:.4f} "
...
def evaluate_criteria(all_results: List[Dict]) -> Dict:
    ...
    # C1: forward_r2 >= 0.7 in CONTRASTIVE (model not degraded by 50% margin loss rate)
    ...
    # C2: CONTRASTIVE cf_gap > OBSERVATIONAL cf_gap * 2.0 (interventional improves CF gap)
    c2_seeds  = 0
    c2_ratios = []
    ...
    # C3: attribution_sign_correct >= 0.65 in CONTRASTIVE
...
def main():
    ...
    run_id = (
        f"v3_exq_330a_sd013_contrastive_counterfactual_frac05_dry_{ts}_v3"
        if args.dry_run
        else f"v3_exq_330a_sd013_contrastive_counterfactual_frac05_{ts}_v3"
    )
    print(f"EXQ-330a start: {run_id}")
    ...
    print(f"C2 cf_gap_ratio x{C2_CF_GAP_RATIO}: {criteria['c2_cf_gap_ratio_pass']} "
    ...
    print(f"interventional_fraction used: {INTERVENTIONAL_FRACTION}")

    # Per-claim direction
    # SD-013: interventional benefit demonstrated (C2 primary criterion)
    # ARC-033: forward model integrity (C1)
    sd013_pass  = criteria["c2_cf_gap_ratio_pass"] and criteria["c3_sign_correct_pass"]
