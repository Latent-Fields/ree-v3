"""
V3-EXQ-591h -- LIVE (closed-loop) validation of the 591f crossing-count Phase 0->1 gate.

SUPERSEDES V3-EXQ-591g (ERROR, 2026-09-02, ree-cloud-2). 591g was scientifically complete
-- all 20 cells ran, all five readiness preconditions were met (gate_green True) -- and then
crashed in the aggregation block with
    UnboundLocalError: local variable 'dropout_bypassed' referenced before assignment
after ~5h46m of compute, before any manifest was written. The F4 telemetry-dropout guard
added during 591g's pre-queue red-team pass DEFINED `dropout_bypassed` seven lines BELOW the
`c_discriminates` expression that CONSUMES it. Under `--dry-run` the leading `gate_green`
conjunct is always False (a 3-episode run cannot reach PHASE_EP_MIN[1], so no seed advances
and P1 fails), so Python short-circuited and the mandatory pre-queue smoke test never
evaluated the undefined name. The live run met the preconditions, evaluated it, and died.

591h is that statement-ordering fix and nothing else: the F4 guard block moves ABOVE
`c_discriminates`. No DV, criterion, threshold, seed, oracle, or verdict-grid change --
`c_discriminates` computes exactly what 591g's own docstring, `criteria_non_degenerate`
and `label` branch already specified.
...
RED-TEAM VERDICT
----------------
red-team (fable): recorded in the queue entry note for V3-EXQ-591g and inherited
unchanged -- 591h alters no manipulation, DV, criterion or verdict mapping.
red-team: SKIP for 591h itself (pure crash / statement-ordering re-queue).
"""
...
QUEUE_ID = "V3-EXQ-591h"
# 591g ran all 20 cells and died in aggregation (UnboundLocalError) before writing a
# manifest; 591h is its statement-ordering fix. See the module docstring.
SUPERSEDES = "V3-EXQ-591g"
EXPERIMENT_TYPE = "v3_exq_591h_isef005_phase01_gate_live"
...
def run_experiment(*, dry_run: bool = False) -> Dict[str, Any]:
    ...
    # NOTE (591h): `dropout_bypassed` MUST be defined above this expression. In 591g it was
    # defined seven lines below, and the leading `gate_green` conjunct short-circuited past
    # the UnboundLocalError on every --dry-run, so the pre-queue smoke test could not see it.
