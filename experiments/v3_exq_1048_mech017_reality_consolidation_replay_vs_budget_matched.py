...
"""
V3-EXQ-1048 -- MECH-017 reality consolidation: offline REPLAY vs BUDGET-MATCHED
waking-only extra training vs NO extra training, read on held-out early/late
...
THE QUESTION (MECH-017, claims.yaml `what_would_answer`)
--------------------------------------------------------
Does sleep-time reality consolidation improve world-model fidelity BEYOND what
the SAME compute delivers online? Three seed-matched arms:
...
CONFIRMING (claim's own grid): A < B on the EARLY-state probes (replay counters
forgetting) with A <= B on the LATE-state probes (no cost to recency), sign-
consistent across seeds.
FALSIFYING: A == B (sleep consolidation is just more gradient steps -- nothing
specific to offline replay), or A > B (replaying stale traces DEGRADES the model
against the online comparator -- the anti-prediction).
...
Waking training is IDENTICAL in all three arms: after every training episode each
...
tensors and requires all three arms of a seed to agree exactly.
...
E1/E2 WEIGHTS evaluated at fixed inputs.
  ...
  inapplicable to all three arms.

RED-TEAM (Step 4.5, model: fable -- model diversity from this session's Opus 5):
VERDICT **CONTESTED**, three findings, all verified against source and all disposed:
  F1 (Family 3, CONFIRMED, FIXED) -- the verdict grid's `else` catch-all absorbed two
     unlike states: a C1+C3-pass / C2-fail TRADE-OFF (replay counters forgetting but
     charges a recency cost) and a C4 budget-mismatch INSTRUMENT failure, labelling
     both "replay_indistinguishable_from_matched_compute" / `weakens`. Fixed two ways:
     the budget match moved OUT of the criteria into the preconditions (a mismatch now
     routes to substrate_not_ready_requeue, not to a claim verdict), and the trade-off
     got its own branch + label + `mixed` direction. At full scale seed 42 the run
     lands in exactly that new branch, so the fix is load-bearing, not hypothetical.
  F2 (Family 4, CONFIRMED, FIXED) -- the persistence-skill gate read ARM_C only, the
     one arm the A-vs-B verdict never uses, while exempting A and B. Re-scoped to the
     compared arms as a per-seed max; see the long comment at its construction for why
     a min-over-all-arms would have been the opposite error.
  F3 (provenance, CONFIRMED, ACTED ON) -- the full-scale numbers quoted to the reviewer
     did not reproduce. Root cause found afterwards and it invalidates BOTH sides'
     figures: `reset_all_rng(seed)` runs in `arm_cell.__enter__`, which `main()` applies
     and a direct `run_cell(...)` call does NOT, so every measurement either party took
     by calling `run_cell` directly was off-contract. All numbers quoted in this
     docstring are re-measured through the real `arm_cell` wrapper.
  Minor notes: (a) the E2 triple keyed the not-yet-executed action -- FIXED, both
  training and probe now key the action that carried z_{t-1} -> z_t, which also drops
  the cross-episode teleport transition the reviewer flagged in (c); (b)
  `_e1_action_one_hot` is always zero here -- correct and inert, since
  `action_conditioned_transition` defaults False, so the probe never uses it.
  NOT fixed, with reason: nothing. No finding was dismissed.

DESIGN-VALIDATION MEASUREMENTS (disclosed in full; no threshold was changed after
seeing them -- a pre-registered bound that fails is a design-time result, never a
reason to move the bound)
---------------------------------------------------------------------------------
3 of the 5 seeds (42, 123, 45) were run at FULL scale through the real `arm_cell`
wrapper while validating the instrument. Per-seed early-probe skill over the
persistence baseline, and the criteria directions:

  seed | A_skill B_skill C_skill | A<B early | A<=1.1B late | rel gain | forget(C)
    42 | +0.170  -0.370  -0.611  |   True    |    False     |  +0.394  |  2.271
   123 | +0.105  -0.663  -1.201  |   True    |    False     |  +0.462  |  3.354
    45 | +0.286  -0.305  -0.524  |   True    |    False     |  +0.453  |  2.503

So on the evidence so far C1 holds 3/3 and C3 clears its floor by ~9x, while C2
fails 3/3: replay counters forgetting on the early probes by a large, reproducible
margin AND charges more than LATE_TOL for it on the recent ones. That is the
`replay_counters_forgetting_at_a_recency_cost` / `mixed` branch -- the one MECH-017's
own grid has no slot for, and the one the red-team's Finding 1 fix exists to record
honestly instead of sweeping into "A == B" / `weakens`. The skill ordering
A > B > C is monotone on every seed measured.

The remaining 2 seeds and the recorded manifest are what the queued run produces;
these scratch cells wrote no manifest and are not citable evidence.

Run with:
  /opt/local/bin/python3 experiments/v3_exq_1048_mech017_reality_consolidation_replay_vs_budget_matched.py --dry-run
  /opt/local/bin/python3 experiments/v3_exq_1048_mech017_reality_consolidation_replay_vs_budget_matched.py
"""
...
EXPERIMENT_TYPE = "v3_exq_1048_mech017_reality_consolidation_replay_vs_budget_matched"
QUEUE_ID = "V3-EXQ-1048"
...
def run_cell(
    ...
    def _consolidation_point(block: str) -> None:
        """The manipulation. Arm C skips it entirely."""
        if arm == ARM_C:
            cmc_records.append({"block": block, "skipped": True, "n_updates": 0.0})
            return
        window = None if arm == ARM_A else recent_window
    ...
    print(
        ...
        f"updates_e2={updates_e2:.0f} window={recent_window if arm != ARM_A else 'ALL'} "
...
def main(dry_run: bool = False):
    ...
    min_jaccard = min(r["early_late_jaccard_distance"] for r in a_rows + b_rows + c_rows)
    min_probe_var = min(
    ...
    n_writepath_off = sum(
        1 for r in a_rows + b_rows + c_rows if r["sd016_writepath_mode"] == "off"
    )
        ...
        preconditions = p0_readiness_gate([
            ...
            {"name": "probe_set_identical_across_arms", "measured": float(n_seeds_probe_match),
             ...
             "control": "sha256 over each cell's concatenated probe [z_self, z_world] tensors; all "
                        "three arms of a seed must agree EXACTLY, which is what makes the probe set "
    ...
    criteria = [
        {"name": "C1_replay_beats_budget_matched_on_early_probes", "load_bearing": True,
         "passed": c1, "measured": float(n_a_better_early),
         "threshold": float(SIGN_CONSISTENCY_REQUIRED), "comparator": ">=",
         "detail": "seeds with e1_holdout_mse_early(A) < e1_holdout_mse_early(B)"},
        {"name": "C2_no_cost_on_late_probes", "load_bearing": True,
         "passed": c2, "measured": float(n_a_not_worse_late),
         "threshold": float(SIGN_CONSISTENCY_REQUIRED), "comparator": ">=",
         "detail": f"seeds with e1_holdout_mse_late(A) <= (1+{LATE_TOL})*e1_holdout_mse_late(B)"},
        {"name": "C3_early_effect_size_clears_floor", "load_bearing": True,
         "passed": c3, "measured": rel_gain_early, "threshold": MIN_REL_GAIN_EARLY,
         "comparator": ">",
         "detail": "mean over seeds of (early_B - early_A)/early_B; this is the criterion the "
                   "claim's FALSIFYING 'A == B' branch fails"},
    ]
    criteria_non_degenerate = {
    ...
    if all_pass:
        label = "reality_consolidation_beyond_matched_compute_confirmed"
        direction = "supports"
    elif n_a_worse_early >= SIGN_CONSISTENCY_REQUIRED:
        label = "replay_degrades_against_matched_online_comparator"
        direction = "weakens"
    elif c1 and c3 and not c2:
        # Red-team Finding 1 (CONFIRMED): replay DID counter forgetting on the early
        # probes -- by a large margin -- but charged more than LATE_TOL for it on the
        # recent ones. MECH-017's grid has no branch for this: its FALSIFYING clause is
        # "A = B" or "A > B", and neither describes "A << B early, A > 1.1B late". The
        # old code swept it into the catch-all and recorded a measured trade-off as
        # `weakens` under a label asserting A == B. `mixed` is the honest reading here,
        # and is NOT the disallowed use of `mixed` for an unmeasured arm: both legs of
        # this trade-off are genuine per-seed measurements.
        label = "replay_counters_forgetting_at_a_recency_cost"
        direction = "mixed"
    else:
        label = "replay_indistinguishable_from_matched_compute"
        direction = "weakens"

    note = (
        f"MECH-017 discriminative pair at matched gradient budget "
        f"(A={budget_a:.0f} vs B={budget_b:.0f} extra steps). EARLY-probe held-out E1 MSE: "
        f"A better on {n_a_better_early}/{len(seeds)} seeds, mean relative gain "
        f"{rel_gain_early:.4f} (floor {MIN_REL_GAIN_EARLY}); LATE-probe no-cost held on "
        f"{n_a_not_worse_late}/{len(seeds)} seeds. Control (ARM_C, no extra training) "
        f"forgetting ratio early/late = {forgetting_ratio:.3f}. "
        f"ARM_A replay early-regime coverage {min_replay_share_a:.3f} vs ARM_B "
        f"{replay_share_b:.3f}. E2 numbers in this manifest are SELF-forward, NOT the "
    ...
    manifest.update({
        ...
        "interpretation": {
            "label": label,
            "criteria": criteria,
            "combination_rule": "PASS iff ALL of C1, C2, C3 pass (plain AND, no any()/OR branching). The gradient-budget match that used to be C4 is now a PRECONDITION, so a mismatch routes to substrate_not_ready_requeue instead of to a MECH-017 verdict.",
