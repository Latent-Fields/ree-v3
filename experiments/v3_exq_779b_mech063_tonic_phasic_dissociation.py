"""V3-EXQ-779b: MECH-063 sub-claim (ii) tonic-vs-phasic behavioural DISSOCIATION.
...
Supersedes: V3-EXQ-779a (same scientific question; ONE-PARAMETER exposure fix only).

WHY 779b (adjudicated by failure_autopsy_MECH-063-777a-779a-cluster_2026-07-18,
REE_assembly origin/master 0a5cd3f24b, targets[1]):

V3-EXQ-779a's LOAD-BEARING CRITERION ALREADY PASSED. double_dissociation_C1_and_C2 =
passed true, criteria_non_degenerate true, robust true, diss_seed_count 4 meeting
min_seeds 4:
    mean_dS_tonic  = +0.265 (sd 0.072) against SUSTAINED_MARGIN 0.05
    mean_dR_phasic = -0.048 (sd 0.037) against TRANSIENT_MARGIN 0.02
It was withheld SOLELY because the precondition phasic_fires_real_events measured 6
against a threshold of 10 -- a MIN across cells sourced ENTIRELY from seed 23 / arm T1P1.
MECH-063 sub-claim (ii) is one exposure fix from adjudication.

ROOT CAUSE -- an EPISODE-DENOMINATED CAP under a SAMPLE-DRIVEN STOPPING RULE. Capability
is RULED OUT and must not be re-investigated: burst_level_max = 1.00 in EVERY PHASIC-ON
cell including both seed-23 cells, and seed 23's firing RATE (7.2-11.9 event ticks per
1000 env steps) sits INSIDE the range the other seeds span (seed 29: 8.8-9.6). SD-069
fires normally on the offending seed. Seed 23 simply received only 835 of the 2400
available env steps, because it dies in ~7 steps per episode, so the 120-EPISODE cap bound
before the 2400-STEP cap. Seed 23's four cells are the ONLY cells in the run at/near the
episode cap (n_episodes 116/116/120/120 against 8-52 everywhere else), and its two
PHASIC-ON cells are the ONLY two of sixteen reporting stop_reason 'episode_cap' -- the
other fifteen report 'step_cap' or 'floors_met'.

779a's fix F1 made the stopping RULE sample-driven but left MAX_EPISODES_PER_CELL
denominated in EPISODES, so the original V3-EXQ-777/779 starvation defect reappeared one
layer down, on the same seed. Fixing a stopping rule does not fix an episode-denominated
cap.

THE FIX IN 779b -- ONE PARAMETER, made STRUCTURAL (nothing else changes):

 1. STEP-DENOMINATE THE PER-CELL CAP. MAX_EPISODES_PER_CELL is no longer an independent
    magic number (779a: 120); it is DERIVED from MAX_ENV_STEPS_PER_CELL. Since every
    episode consumes at least one env step, an episode cap equal to the step cap can
    NEVER bind before it -- so MAX_ENV_STEPS_PER_CELL (2400) is the binding constraint for
    EVERY cell regardless of how short that seed's episodes are, and the episode cap
    retains only its intended role: a finite backstop against unbounded resets. This is
    exactly the shared helper's own documented default (RolloutBudget.__post_init__ sets
    max_episodes = max_env_steps when it is not overridden); 779a's explicit 120 was an
    override of an already-correct default. A module-level assert now pins the invariant
    so a future edit cannot silently re-introduce an episode-denominated bound.

    Expected effect on the vetoing cell: seed 23 / T1P1 goes from 835 to 2400 env steps
    (~2.9x), taking its event ticks from 6 to ~17 at its own measured rate -- clearing
    MIN_EVENT_TICKS (10) with margin, without touching the threshold.

 2. STEP-DENOMINATED PROGRESS. The runner's `ep N/M` progress token now carries env
    steps against MAX_ENV_STEPS_PER_CELL, because under sample-driven stopping the env
    step IS the loop bound and the only cross-cell-comparable unit: realised episodes per
    cell span 8 (seed 29/37, 300-step episodes) to ~343 (seed 23, ~7-step episodes), so an
    episode denominator cannot describe progress for both. The printed line labels the
    unit explicitly, and the queue entry's episodes_per_run matches this denominator.

DELIBERATELY UNCHANGED from 779a (the autopsy directs that all three be KEPT):
  - MIN-ACROSS-CELLS readiness aggregation. It did LEGITIMATE work here and must NOT be
    relaxed: unlike in V3-EXQ-779, the vetoing cell IS the criterion's largest
    contributor -- seed 23 carries dR_phasic -0.108 (the largest phasic effect of any
    seed) on just 6 event ticks, and excluding it leaves 3 dissociating seeds against
    min_seeds 4. The PASS genuinely depended on the under-exposed cell, so the
    precondition correctly refused to bank it.
  - The SAMPLE-vs-CAPABILITY precondition kind classification
    (readiness_unmet_sample_kind / readiness_unmet_capability_kind). It is why 779's
    mislabelling did not recur: 779a correctly self-routed sample_starvation_requeue with
    preconditions[0].kind = "sample" and an EMPTY capability list.
  - PER-CELL stop_reason recording. It is what separated the one episode_cap cell from
    the fifteen step_cap cells and made this a one-command diagnosis.
  - Every readiness FLOOR and verdict threshold (MIN_*, *_MARGIN, DOMINANCE_K) and the
    surprise-EMA telemetry. No threshold is nudged; only exposure changes.

OPEN QUESTION this run answers: whether seed 23's dR_phasic (-0.108) survives adequate
exposure. Adjudication is expected on this run either way.

NOT BLOCKED on SD-PROBE-WARMUP / SD-073 (the sub-claim (i) substrate build). The
re-derive brake fired on the 777 lineage ONLY and EXPLICITLY EXEMPTS this one --
targets[1].re_derive_brake.exempt = true: this lineage is not circling a substrate
ceiling, its load-bearing criterion passed non-degenerately and robustly, and refusing
V3-EXQ-779b would discard a nearly-complete result rather than prevent a re-derive loop.
The autopsy's recommended_substrate_queue_entry for this target is action "none".
...
# 779b FIX: STEP-DENOMINATED. Derived from the step cap, NOT an independent magic number
# (779a hardcoded 120, which bound before the step cap on any seed whose episodes are
# shorter than MAX_ENV_STEPS_PER_CELL / 120 = 20 steps -- seed 23 dies in ~7). Every
# episode consumes >= 1 env step, so an episode cap equal to the step cap can never bind
# first: MAX_ENV_STEPS_PER_CELL is the binding constraint for EVERY cell, and this keeps
# only its intended role as a finite backstop against unbounded resets. This reproduces
# the shared helper's own default (RolloutBudget.__post_init__ sets
# max_episodes = max_env_steps when not overridden); we state it explicitly because it is
# declared in the fingerprint config_slice and recorded in the manifest.
MAX_EPISODES_PER_CELL = MAX_ENV_STEPS_PER_CELL
# Per-episode step cap, unchanged from 779/779a, so the episode structure (and therefore
# the phasic EMA's per-episode reset cadence, via agent.reset -> phasic_burst.reset)
# matches the superseded design. 779b changes HOW MANY episodes may run, not what an
# episode is.
STEPS_PER_EPISODE = 300

# 779b FIX: STEP-DENOMINATED PROGRESS. Under sample-driven stopping the env step is the
# loop bound and the only cross-cell-comparable progress unit -- realised episodes per
# cell span 8 (300-step episodes) to ~343 (~7-step episodes), so no episode denominator
# describes both. The runner's `ep N/M` token therefore carries env steps against
# MAX_ENV_STEPS_PER_CELL; the printed line labels the unit explicitly.
...
# Invariant of 779b's fix: the per-cell cap must be STEP-denominated. Because every
# episode consumes at least one env step, max_episodes >= max_env_steps guarantees the
# EPISODE cap can never bind before the STEP cap on any seed, however short its episodes.
# This assert is the structural guard: it fails loudly if a future edit re-introduces an
# independently-denominated episode bound (the exact V3-EXQ-779a defect, which starved
# seed 23 to 835 of 2400 available env steps and vetoed an otherwise-passing run).
assert MAX_EPISODES_PER_CELL >= MAX_ENV_STEPS_PER_CELL, (
    "MAX_EPISODES_PER_CELL must be step-denominated (>= MAX_ENV_STEPS_PER_CELL) so the "
    "step cap is the binding constraint for every cell -- see V3-EXQ-779b fix 1"
)
...
def _config_slice(arm: str) -> Dict[str, Any]:
    ...
    # A SET env-seed base changes the env layout this cell was computed on,
    # so it must key the fingerprint -- otherwise a pinned cell and an
    # OS-entropy cell share a hash and a consumer could reuse the wrong
    # world (false cache HIT). Conditional so the default path's slice is
    # byte-identical to every existing mint of this driver.
...
def _budget(arm: str) -> RolloutBudget:
    ...
    the readiness gates -- and, from 779b, so are its CAPS.

    779b: max_episodes is passed as MAX_EPISODES_PER_CELL == MAX_ENV_STEPS_PER_CELL, so
    the STEP cap is what binds for every cell. 779a passed an independent 120 here, which
    bound first on any seed averaging under 20 steps per episode and starved seed 23 to
    835 of 2400 available steps -- a sample-driven stopping RULE governed by an
    episode-denominated CAP is still episode-denominated where it matters.
...
def _run_cell(arm: str, seed: int) -> Tuple[Dict[str, Any], RolloutOutcome]:
    ...
    with arm_cell(
        ...
        def _observe(ctx: TickContext) -> Optional[Dict[str, int]]:
            ...
            # Runner progress line, 779b: STEP-denominated. The `ep N/M` token the
            # runner parses carries ENV STEPS against MAX_ENV_STEPS_PER_CELL (the loop
            # bound under sample-driven stopping), not episodes -- realised episodes per
            # cell span 8 to ~343 here, so an episode denominator cannot describe
            # progress for both. The unit is named inline so the line is unambiguous,
            # and the episode index is still reported.
            if (
                ctx.n_env_steps == 1
...
def run_experiment(dry_run: bool = False) -> Dict[str, Any]:
    ...
    if dry_run:
        ...
        # 779b: the smoke path keeps the STEP-denominated invariant too, so a dry-run
        # exercises the same cap relationship the real run uses (a dry-run with an
        # independently-denominated episode cap would not smoke-test the fix at all).
        MAX_EPISODES_PER_CELL = MAX_ENV_STEPS_PER_CELL
    ...
    interpretation = {
        ...
        "summary": (
            ...
            "substrate CAPABILITY check failed. Supersedes V3-EXQ-779a, whose load-bearing "
            "criterion PASSED (mean_dS_tonic +0.265 vs margin 0.05; mean_dR_phasic -0.048 "
            "vs margin 0.02; 4 dissociating seeds = min_seeds 4; robust, non-degenerate) "
            "but was withheld because phasic_fires_real_events measured 6 vs 10 -- sourced "
            "entirely from seed 23 / T1P1, whose 120-EPISODE cap bound before the "
            "2400-STEP cap at ~7 steps per episode. 779b step-denominates that cap so the "
            "step cap binds for every cell; no threshold and no aggregation rule changed."
        ),
    }
    ...
    manifest: Dict[str, Any] = {
        ...
        "notes": (
            ...
            "SUPERSEDES V3-EXQ-779a (same scientific question; ONE-PARAMETER exposure fix "
            "per failure_autopsy_MECH-063-777a-779a-cluster_2026-07-18, REE_assembly "
            "origin/master 0a5cd3f24b, targets[1]). 779a's load-bearing criterion "
            "double_dissociation_C1_and_C2 ALREADY PASSED -- criteria_non_degenerate true, "
            "robust true, diss_seed_count 4 = min_seeds 4, mean_dS_tonic +0.265 (sd 0.072) "
            "vs SUSTAINED_MARGIN 0.05, mean_dR_phasic -0.048 (sd 0.037) vs TRANSIENT_MARGIN "
            "0.02 -- and was withheld SOLELY because phasic_fires_real_events measured 6 vs "
            "threshold 10, a min-across-cells sourced entirely from seed 23 / T1P1. "
            "ROOT CAUSE (capability RULED OUT: burst_level_max 1.00 in every PHASIC-ON cell "
            "including both seed-23 cells, and seed 23's firing RATE of 7.2-11.9 event ticks "
            "per 1000 env steps sits INSIDE the range other seeds span): seed 23 dies in ~7 "
            "steps per episode, so 779a's 120-EPISODE cap bound before its 2400-STEP cap and "
            "the cell received only 835 of 2400 available steps. Seed 23's cells are the only "
            "ones at/near the episode cap (n_episodes 116/116/120/120 vs 8-52 elsewhere) and "
            "its two PHASIC-ON cells the only two of sixteen reporting stop_reason "
            "'episode_cap'. 779a's F1 made the stopping RULE sample-driven but left the CAP "
            "episode-denominated, so the V3-EXQ-777/779 starvation defect reappeared one "
            "layer down on the same seed. THE FIX: MAX_EPISODES_PER_CELL is now DERIVED from "
            "MAX_ENV_STEPS_PER_CELL (equal to it) rather than an independent 120, so the "
            "step cap binds for EVERY cell however short its episodes; a module-level assert "
            "pins the invariant. This restores the shared helper's own documented default "
            "(RolloutBudget sets max_episodes = max_env_steps when not overridden) -- 779a's "
            "120 was an override of already-correct behaviour. Expected: seed 23 / T1P1 goes "
            "835 -> 2400 steps, taking event ticks 6 -> ~17 at its own measured rate, "
            "clearing MIN_EVENT_TICKS 10 without touching the threshold. Progress prints are "
            "step-denominated to match. DELIBERATELY UNCHANGED (autopsy directs all three be "
            "KEPT): min-across-cells readiness aggregation -- it did legitimate work here, "
            "since seed 23 IS the criterion's largest contributor (dR_phasic -0.108 on 6 "
            "ticks) and excluding it leaves 3 dissociating seeds vs min_seeds 4, so the PASS "
            "genuinely depended on the under-exposed cell; the sample-vs-capability "
            "precondition kind classification (why 779's mislabelling did not recur); and "
            "per-cell stop_reason recording (what made this a one-command diagnosis). No "
            "threshold, margin, or aggregation rule is nudged -- only exposure changes. "
            "TONIC = MECH-313 noise_floor (sustained temperature lift); PHASIC = SD-069 "
            "phasic_surprise_burst with the sharp instantaneous_pe source. Complements "
            "V3-EXQ-777/777a (sub-claim i). GOV-REUSE-1: decisive readout = event ticks and "
            "dR_phasic for seed 23 / T1P1 at adequate exposure; checked 779 and 779a (the "
            "only runs of this family, substrate_hash 765ce9d248 for 779a) -- 779a records "
            "that cell at 835 of 2400 steps with 6 event ticks, i.e. it records the "
            "SHORTFALL, not the answer, and no reprocessing can manufacture unobserved ticks "
            "-> run. Re-derive brake: claim-level count reaches threshold 2 but this lineage "
            "is EXEMPT (targets[1].re_derive_brake.exempt=true) -- it is not circling a "
            "substrate ceiling, its load-bearing criterion passed non-degenerately, and the "
            "autopsy's recommended_substrate_queue_entry for this target is action 'none'. "
            "NOT blocked on SD-PROBE-WARMUP / SD-073, which brakes the 777 lineage only."
        ),
    }
