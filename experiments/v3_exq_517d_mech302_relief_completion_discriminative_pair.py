"""
V3-EXQ-517d: MECH-302 relief-completion discriminative pair -- env curriculum.

Supersedes V3-EXQ-517c (2026-05-30: ran to completion in ~105 min, exit 0, but
emitted NO runner sentinel -- it never called emit_outcome, so the runner
recorded the finished run as a no-result ERROR and discarded it). This iteration
is instrumentation-only: the science, comparator, curriculum, and acceptance
criteria are byte-identical to 517c; the sole change is importing and calling
experiment_protocol.emit_outcome so the runner registers the PASS/FAIL verdict.

Original 517c header follows.
...
EXPERIMENT_TYPE = "v3_exq_517d_mech302_relief_completion_discriminative_pair"
CLAIM_IDS = ["MECH-302"]
...
def main():
    ...
    print(f"EXQ-517d: MECH-302 env-curriculum discriminative pair", flush=True)
    print(
    ...
    manifest = {
        ...
        "claim_ids": CLAIM_IDS,
        "queue_id": "V3-EXQ-517d",
        "supersedes": "v3_exq_517c_mech302_relief_completion_discriminative_pair",
        "outcome": outcome,
    ...
    # Emit the runner-conformance sentinel. The V3-EXQ-517c predecessor omitted
    # this, so a completed ~105-min run was recorded as a no-result ERROR and
    # discarded by the runner. emit_outcome(dry_run=True) relocates the manifest
    # out of evidence/experiments/ so smoke runs don't pollute the indexer.
    emit_outcome(
        outcome,
