...
Validation experiment for SD-037 (broadcast_override regulator). Factorial 2x2:
{use_gabaergic_decay, use_broadcast_override} x {OFF, ON}. SD-036 is the GABAergic
decay layer (committed 2026-04-22); SD-037 is the orexin-analog override that
sits on top. The matched 4-arm design isolates whether SD-037 contributes effects
distinct from SD-036 alone.

Acceptance criteria (from orexin kinetics lit-pull synthesis,
REE_assembly/evidence/literature/targeted_review_orexin_kinetics/synthesis.md):
  PWS-hyperphagia analog (saturated override): with SD-037 ON and the override
    decay tuned so the system saturates under sustained drive+harm, approach-
    commit rate should reach >=2x the balanced (SD-036-only) arm.
  Narcolepsy/cataplexy analog (lost override): with SD-037 OFF under sustained
    threat, approach-commit rate should fall below 30% of the SD-036-only arm,
    confirming the override system carries motivational committment.
  EXQ-471 catatonic lock-in: with both SD-036 and SD-037 ON, freeze release
    rate under sustained drive+harm should exceed the SD-036-only arm (override
    raises PAG exit_threshold via alpha_override).

Output:
  evidence/experiments/v3_exq_483_sd037_broadcast_override_4arm/
    v3_exq_483_sd037_broadcast_override_4arm_<ts>.json         (manifest)
    v3_exq_483_sd037_broadcast_override_4arm_<ts>_episode_log.json  (fishtank feed)

Estimated runtime: ~50 min/arm on Mac CPU
  (3 seeds x [60 warmup + 5 eval] eps x 200 steps x 4 arms)
"""
...
EXPERIMENT_TYPE    = "v3_exq_483_sd037_broadcast_override_4arm"
EXPERIMENT_PURPOSE = "diagnostic"
...
def run_arm_seed(arm: Dict, seed: int, dry_run: bool = False) -> Dict:
    ...
    print(f"[EXQ-483 {arm['id']} s{seed}] -- Phase 0: warmup", flush=True)
    warmup = _warmup_train(agent, env, warmup_eps, steps)

    print(f"[EXQ-483 {arm['id']} s{seed}] -- Phase 1: eval", flush=True)
...
def run(seeds=None, dry_run: bool = False) -> dict:
    ...
    print(
        f"[V3-EXQ-483] SD-037 Broadcast Override Regulator -- 4-arm validation\n"
        f"  Seeds: {seeds}\n"
    ...
    # Acceptance: PWS-hyperphagia analog (saturated override -> >=2x approach-commit
    # vs balanced SD-036-only arm) and narcolepsy/cataplexy analog (lost override
    # -> <30% approach-commit vs balanced arm). "Balanced arm" = ON_OFF (SD-036 only).
    ...
    overall_status = "PASS" if (pws_pass and narco_pass) else "FAIL"
    ...
    summary_markdown = f"""# V3-EXQ-483 -- SD-037 Broadcast Override Regulator (orexin-analog) 4-arm validation
