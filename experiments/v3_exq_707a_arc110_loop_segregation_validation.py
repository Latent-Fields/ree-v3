...
"""
V3-EXQ-707a -- ARC-110 PARALLEL SEGREGATED LOOP-SEGREGATION VALIDATION (diagnostic).

SUPERSEDES V3-EXQ-707. 707 self-routed substrate_not_ready_requeue on a SINGLE unmet
precondition (in_layer_null_verified_lifting=false: the S2 ARM_NOISE null lifted committed-
class entropy strict-above A0 on only 1/4 divergent seeds, threshold 2). Diagnosis: that
precondition was MIS-SPECIFIED. It imported the 700-lineage TEMPERATURE-null liveness proxy
("a valid null must RAISE entropy above A0" -- trivially true when the null is a softmax-
temperature knob) onto a STRUCTURED-ACCUMULATOR null. Randomising the non-motor loop
accumulators with magnitude-matched gaussian content and routing it through the SAME loop
path (per-loop zscore + Haber ascending-spiral arbitration, motor never nulled) lands
committed-class entropy near BASELINE -- structured loops beat random; random ~ A0 -- NOT
above it. In 707 the null was demonstrably LIVE (it perturbed the committed selection:
loop_frac_disagree 0.37-0.97, loop_frac_committed_neq_motor 0.37-0.66, loop_noise_active_ticks
in the thousands on every divergent seed) yet sat at/below A0, while A1_LOOPS lifted strict-
above A0 on 4/4 divergent seeds. The buggy gate therefore (i) spuriously requeued a valid
structured-conversion result and (ii) made the pre-registered WEAKENS branch unreachable
(it required the random null to exceed A0 by luck). 707a replaces the null-LIFT proxy with a
null-LIVENESS gate (the null must actively PERTURB committed selection, the property that
makes C1's "A1 strict-above the null" non-vacuous). C1 (A1 strict-above max(A0, null)+margin)
is UNCHANGED -- it already rules out the path-artifact hypothesis (if A1's lift were a zscore-
path artefact, the live magnitude-matched null -- same zscore path -- would match A1 and fail
C1). Substrate UNCHANGED (the _loop_inlayer_null mechanism is correct); this is a driver-only
gate fix.
...
EXPERIMENT_TYPE = "v3_exq_707a_arc110_loop_segregation_validation"
QUEUE_ID = "V3-EXQ-707a"
SUPERSEDES = "V3-EXQ-707"   # 707a fixes the mis-specified in-layer-null validity gate (null-LIVENESS, not null>A0)
...
def run_experiment(
    ...
    if not preconditions_met:
        ...
        degeneracy_reason = (
            ...
            "vacuous split pinned to the motor winner / the in-layer null is INERT = does not "
            "perturb the committed selection / finer channels not dissociable / delta_t flat). "
            "NOT a falsification."
        )
    ...
    manifest_core = {
        ...
        "interpretation_grid": {
            ...
            "FAIL_substrate_not_ready_requeue": (
                ...
                "the ARC-110 what_would_answer guard), OR the S2 in-layer null is INERT (does NOT "
                "perturb the committed selection even with loops -> null-validity problem "
                "persists), OR learning was not engaged (finer channels not dissociable / delta_t "
                "flat). The conversion question could NOT be measured -- NOT a falsification."
            ),
        },
...
def _build_manifest(
    ...
    return {
        ...
        "evidence_direction_note": (
            f"V3-EXQ-707a ARC-110 PARALLEL SEGREGATED LOOP-SEGREGATION VALIDATION "
            f"(supersedes V3-EXQ-707; experiment_purpose=diagnostic; claim_ids=[ARC-110]; "
            f"PROMOTES NOTHING). 707a FIX: the in-layer-null validity precondition is null-"
            f"LIVENESS (the S2 ARM_NOISE null must PERTURB the committed selection), NOT null-LIFT "
            f"(707's retired requirement that the null raise entropy strict-above A0 -- the "
            f"700-lineage temperature-null proxy, invalid for a structured-accumulator null; "
            f"random magnitude-matched loop content lands at baseline, not above it). C1 (A1 "
            f"strict-above max(A0, the live null)+margin) UNCHANGED and already rules out the "
            f"zscore-path-artifact hypothesis. Substrate unchanged (driver-only gate fix). "
            ...
            f"winner), the in-layer null must be LIVE (perturb the committed selection, else the "
            f"null-validity problem persists), the pool divergent (GAP-A), finer channels "
            f"dissociable + delta_t non-"
