...
"""V3-EXQ-543d: ARC-062 x MECH-260 2x2 Factorial Falsifier on bipartite SD-054.

Supersedes V3-EXQ-543c. Per user direction 2026-05-11 after the 543c FAIL
landed in the second pre-registered outcome (probe-gate FAIL on all 3 seeds
with TV ~1e-7, well below INERT_GATING_THRESHOLD=0.05).

Why this iteration is needed
----------------------------
V3-EXQ-543c was the second iteration in the arc_062 GAP-B falsifier lineage
(543 -> 543b script-correction -> 543c bipartite-substrate). 543c applied
two fixes from the 2026-05-11 diagnose-errors session:
  Cause 1 (script-level): probe-buffer construction switched from
    world_states[0] (= initial_z_world, identical across all K candidates)
    to world_states[1] (POST-action z_world, differs per K).
  Cause 2 (substrate-level): SD-054 bipartite-layout extension enabled
    (reef bottom rows / food top rows / agent midline-band).
Both fixes engaged correctly: TV went from exact-zero (543b) to ~1e-7
non-zero (543c). But that's still 5 orders of magnitude below the
INERT_GATING_THRESHOLD. Probe-gate fired on all 3 seeds; all claims
returned non_contributory.

The post-543c review surfaced a third gap that neither 543b nor 543c
addressed: 543c ran ARC-062 in architectural isolation from the BG
cluster it was designed to live in. Specifically OFF in 543c:
  - MECH-260 dACC anti-recency / No-Go FIFO
  - SD-032a SalienceCoordinator (mode-conditioned write-gate registry)
  - SD-032b DACCAdaptiveControl (mode_ev / foraging_value / suppression bundle)
  - SD-033a LateralPFCAnalog (the downstream consumer gated_policy's bias
    output is *supposed* to feed via the arc_062 GAP-C wiring; Phase 1
    intentionally deferred this wiring to Phase 3)
  - SD-033b OFCAnalog
  - SD-034 ClosureOperator (No-Go injection at rule-completion)
  - SD-035 amygdala BLA / CeA mode-priors
  - MECH-313 noise floor / MECH-314 curiosity / MECH-320 tonic vigor
With gated_policy's bias having no architecturally-intended consumer
AND no No-Go anti-recency pressure to alternate AND no closure signal,
the falsifier was structurally amputated. "Always head south to food"
is locally optimal on the bipartite substrate without anti-recency
suppression; the agent has no architectural force pushing it to
alternate reef vs forage.

This iteration tests whether the inert-gating result reflects an
actual substrate failure (option 2 from the 2026-05-11 decision-log:
ARC-062 head input is too narrow, needs augmentation with first-action
one-hot) OR just the absence of cluster-wiring (the simpler explanation:
the BG cluster's anti-recency was the missing piece).
...
EXPERIMENT_TYPE = "v3_exq_543d_arc062_mech260_factorial_falsifier"
EXPERIMENT_PURPOSE = "evidence"
QUEUE_ID = "V3-EXQ-543d"
SUPERSEDES = "V3-EXQ-543c"
CLAIM_IDS = ["ARC-062", "MECH-309"]

# V3-EXQ-543d pre-registered cross-arm attribution thresholds.
...
def run(seeds: Optional[List[int]] = None, dry_run: bool = False) -> Dict:
    ...
    print(
        f"[V3-EXQ-543d] ARC-062 x MECH-260 2x2 Factorial Falsifier"
        f" (bipartite SD-054 + world_states[1] fix + dACC anti-recency factorial)"
...
def write_manifest(result: Dict, dry_run: bool, elapsed: float) -> Tuple[Path, str]:
    ...
    manifest = {
        ...
        "dry_run": dry_run,
        "notes": (
            "ARC-062 x MECH-260 2x2 factorial monomodal-collapse falsifier on "
            "bipartite SD-054. Supersedes V3-EXQ-543c. 4 arms x 3 seeds: "
            ...
            "0.0 to isolate MECH-260 anti-recency. Inherited from 543c: "
            ...
            "probe (kept as diagnostic; NON-FATAL in 543d). Pre-registered "
            ...
            "decision-log entries 2026-05-11."
        ),
    }
...
if __name__ == "__main__":
    ...
    print("\n=== V3-EXQ-543d SUMMARY ===", flush=True)
    ...
    print(
        ...
        f"  (NON-FATAL in 543d; reported only)",
        flush=True,
    )
