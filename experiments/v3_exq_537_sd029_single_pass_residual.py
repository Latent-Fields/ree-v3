...
"""V3-EXQ-537: SD-029/MECH-256 single-pass comparator residual on z_harm_s.

Replaces the EXQ-535a two-pass cf_gap formulation. Per the SD-029 spec
(claims.yaml):
...
EXQ-535a computed ||E2(z, a_actual)|| - ||E2(z, a_opposite)||, which is the
superseded SD-003 two-pass design. EXQ-537 computes the actual single-pass
residual the spec requires.

Design (post-discussion 2026-05-07):
    Single substrate: reef_food_ext curriculum (SD-029). ARM_0 baseline
    dropped -- it produced 0 ext events in EXQ-535a, useless for measuring
    dissociation.
    ...
    ARM_2_scrambled (falsification):
        ...
        => no dissociation). Confirms the dissociation in ARM_1, when present,
        is driven by efference copy, not by some other artefact of the
        substrate.

PASS criteria:
    C1 forward_r2 (held-out):       harm_forward_r2 >= 0.85 in ARM_1
                                    (relaxed from spec 0.9 to account for
                                    reef_food_ext-induced ext events
                                    dragging R^2 down; see analysis below)
...
C1 threshold note:
    EXQ-195 hit harm_forward_r2 = 0.914 with SD-008 (alpha_world=0.9) and
    z_harm_dim=32 on the BASELINE substrate (no reef_food_ext). EXQ-320
    interventional training reached ~0.98. EXQ-537 adds the reef_food_ext
    curriculum on top, which injects unpredictable-by-design ext events
    into the eval pool and therefore depresses raw R^2. We relax C1 to
    0.85 to allow for that. If C2 PASSes, the residual dissociation is
    informative even at R^2=0.85; if C2 FAILs at C1>=0.9, that is a
    cleaner refutation. Both outcomes are interpretable.

Training scale: P0 collects more episodes than EXQ-535a; P2 graduation gate
raised from 0.65 to 0.85 with more max_windows to give the forward model
adequate fit. Interventional training (SD-013) enabled to push R^2 toward
the EXQ-320 scale.

claim_ids: ["SD-029", "MECH-256"]
experiment_purpose: evidence
supersedes: V3-EXQ-535a
"""
...
def main(dry_run=False):
    ...
    # SD-029 substrate: reef_food_ext curriculum on (matches EXQ-535a ARM_1 P0 config).
    # P3 eval phase overrides interval/prob to be more aggressive (in
    # _p3_collect_events_and_heldout). Here we set the P0 training-phase config.
    env_kwargs = {
        "size": 12,
        "num_hazards": 3,
        "num_resources": 5,
        ...
        # P0 curriculum injection (lighter than P3 eval phase)
    ...
    manifest = {
        ...
        "experiment_purpose": "evidence",
        "supersedes": "v3_exq_535a_sd029_p3_eval_fix",
