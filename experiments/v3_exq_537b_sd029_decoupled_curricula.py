...
"""V3-EXQ-537b: SD-029/MECH-256 single-pass comparator with DECOUPLED training/eval curricula.

Supersedes V3-EXQ-537 / V3-EXQ-537a (both FAIL non_contributory 2026-05-08;
manifests at REE_assembly/evidence/experiments/v3_exq_537_sd029_single_pass_residual/).
...
DIAGNOSIS OF V3-EXQ-537 / 537a (2026-05-08, both runs identical script):
    Both runs produced the same FAIL pattern across 6 seeds:
      C1 forward_r2 (held-out):  0.65 / 0.71 / 0.67 (target 0.85) -- FAIL
      C2 dissociation:           ratio ~0.88 (ext < self; backwards) -- FAIL
      C3 falsification:          PASS (scrambled arm shows similar ratio)
      C4 event sufficiency:      PASS (n_self ~400, n_ext ~250 per seed)
      P2 graduation:             False on every seed at p2_steps=48000
                                 (P2_MAX_WINDOWS fully exhausted, p2_r2 ceiling 0.65-0.71)

    Root cause: training-curriculum and eval-curriculum coupled.
    The original 537 trained P0/P1/P2 with scheduled_external_hazard
    enabled (interval=20, prob=0.5), which injects by-design-unpredictable
    ext events into the training pool. The forward model cannot fit them
    by construction, so p2_r2 plateaus at ~0.7 even after 48000 steps.
    EXQ-195 hit harm_forward_r2=0.914 on the BASELINE substrate (no
    scheduled_external_hazard); EXQ-320 interventional training reached ~0.98.
    Mixing the two regimes in 537 lost ~25 R^2 points and rendered
    C2 uninterpretable (the comparator is too underfit to distinguish
    Shergill efference-driven dissociation from noise).

DESIGN CHANGE (537b only): decouple training and eval curricula.
    P0 / P1 / P2 (TRAINING):
      scheduled_external_hazard_enabled = False
      All other env state matches 537 (reef_enabled, hazard_food_attraction,
      limb_damage_enabled, harm_history_len, env_drift). Forward model
      should now reach EXQ-195's ~0.91 capacity without curriculum-induced
      capacity ceiling.
    P3 (EVAL):
      scheduled_external_hazard_enabled = True (interval=5, prob=1.0)
      Same as 537 -- this is what produces the balanced n_self / n_ext
      pool that SD-029's C2/C3 measurements require. No code change to
      _p3_collect_events_and_heldout (it already overrides via p3_kwargs).

C1 threshold restored to 0.85 (script default in 537 was already 0.85,
but with decoupled training the spec target 0.9 is now reachable;
0.85 retains a small margin for the P3 substrate switch causing minor
distribution shift on held-out R^2 eval).

Single substrate: reef_food_ext curriculum (SD-029) ON in eval, OFF in
training. ARM_0 baseline still dropped -- the ARM_1 intact / ARM_2
scrambled split is still the comparator design.
...
PASS criteria:
    C1 forward_r2 (held-out):       harm_forward_r2 >= 0.85 in ARM_1
                                    (with decoupling, >= 0.9 expected;
                                    0.85 retained as floor for distribution
                                    shift between training and P3 eval substrate)
...
Per-claim direction grid (unchanged from 537):
    C1 PASS + C2 PASS + C3 PASS + C4 PASS:
        SD-029 = supports, MECH-256 = supports.
    C1 FAIL:
        SD-029 = non_contributory, MECH-256 = non_contributory.
        (Forward fit prereq missing -- dissociation not interpretable.
         If 537b also FAILs C1 with decoupling, the substrate is
         genuinely incapable of supporting the test and route to
         /diagnose-errors for substrate redesign rather than another
         training-protocol tweak.)
...
EXPERIMENT_TYPE = "v3_exq_537b_sd029_decoupled_curricula"
QUEUE_ID = "V3-EXQ-537b"
CLAIM_IDS = ["SD-029", "MECH-256"]
...
def main(dry_run=False):
    ...
    manifest = {
        ...
        "experiment_purpose": "evidence",
        "supersedes": "v3_exq_537_sd029_single_pass_residual",
