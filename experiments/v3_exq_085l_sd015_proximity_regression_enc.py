...
=== ROOT CAUSE OF 085k C2 FAILURE ===

085k benefit_ratio=0.977x (essentially neutral). Per-seed:
  seed=42: present=0.540, absent=0.500  (ratio=1.08x)
  seed=7:  present=0.560, absent=0.650  (ratio=0.86x)
  seed=13: present=0.620, absent=0.610  (ratio=1.02x)

All three seeds near 1.0x. The goal guidance is not helping.

Infrastructure all works: enc_acc=0.927, rfm_loss=0.0069, goal_resource_r_enc=0.582.
The failure is that the RFM+enc hybrid produces near-identical cosine_sim scores
across candidate actions, so action selection is effectively random.

ROOT CAUSE: The ResourceEncoder is a BINARY classifier (contact vs not-contact).
Both "3 cells from resource" and "10 cells from resource" get label=0 (not-contact).
The encoder learns to separate the contact cluster from everything else but does NOT
learn to distinguish near-resource from far-resource within the not-contact class.

Result: z_resource at "3 cells away" ~= z_resource at "10 cells away".
cosine_sim(enc.encode(RFM(rf, a)), z_goal) is nearly identical for all actions
because the encoder maps most non-contact states to the same region of latent space.

=== THE FIX: Proximity Regression Encoder ===

Replace binary contact BCE with resource_proximity MSE regression.

Training target: resource_proximity = 1 / (1 + manhattan_dist_to_nearest_resource)
  - At contact: proximity ~ 1.0  (same position as resource)
  - 1 cell away: proximity ~ 0.5
  - 3 cells away: proximity ~ 0.25
  - 10 cells away: proximity ~ 0.09

The encoder learns: z_resource encodes HOW CLOSE the agent is to any resource.
z_goal = EMA of z_resource at contacts (high proximity states).

Now: cosine_sim(z_resource_current, z_goal) is HIGH near resources, LOW far away.
RFM predicts rf_next. enc.encode(rf_pred) encodes predicted proximity.
Action selection: pick action maximising cosine_sim(enc.encode(RFM(rf,a)), z_goal).
This CORRECTLY ranks actions: "which action leads closer to a resource?"

No binary buffers needed -- every step provides (rf, proximity) training signal.
...
EXPERIMENT_TYPE = "v3_exq_085l_sd015_proximity_regression_enc"
...
class ProximityEncoder(nn.Module):
    ...
    Enables cosine_sim(z_resource_current, z_goal) to smoothly track resource
    proximity, giving action selection a useful gradient.

    Regression head (16 -> 1) forces encoder to learn distance-like features.
    Only encoder layers used for goal seeding and action selection.
    """
...
def _rfm_enc_guided_action(
    ...
    """
    Hybrid: cosine_sim(enc.encode(RFM(rf_curr, a)), z_goal).
    With proximity regression encoder, z_goal is in "high proximity" region.
    enc.encode(rf_pred) is graded by proximity -> action selection has gradient.
    """
    ...
    for ep in range(warmup_episodes):
        ...
        for _ in range(steps_per_episode):
            ...
            if len(prox_buf_rf) >= 16:
                # Sample with overweighting of high-proximity steps for better coverage
                # (high proximity is rare -- resources are consumed and respawn)
...
def run(
    ...
    **kwargs,
    ...
    print(f"\n[V3-EXQ-085l] Final results:", flush=True)
    print(
    ...
    if not c2_pass:
        if c3_pass:
            failure_notes.append(
                ...
                " Proximity regression gives graded representation but nav gap remains."
                " Consider: more eval eps, stronger goal bias in action selection,"
                " or wiring z_goal into REEAgent action scoring directly."
            )
        else:
            failure_notes.append(
    ...
    summary_markdown = (
        f"# V3-EXQ-085l -- SD-015 Proximity Regression Encoder\n\n"
        f"**Status:** {status}\n"
        f"**Claims:** SD-015, SD-012, MECH-112\n"
        f"**Decision:** {decision}\n"
        f"**Seeds:** {list(seeds)}\n"
        f"**Supersedes:** V3-EXQ-085k\n\n"
        f"## Architecture\n\n"
        f"ProximityEncoder (25->16) trained via MSE regression on resource_proximity"
        f" = 1/(1+manhattan_dist). Replaces binary contact BCE (085j/k).\n\n"
        f"**Rationale:** Binary BCE (085j/k) trained encoder to separate contact"
        f" (is_contact=1) from everything else. Near-resource steps labelled 0 --"
        f" encoder didn't distinguish near-resource from far-resource."
        f" Proximity regression forces smooth graded z_resource: high near resources,"
        f" low far away. cosine_sim(z_resource_current, z_goal) now a smooth"
        f" resource-proximity gradient for action selection.\n\n"
        f"Action selection: cosine_sim(enc.encode(RFM(rf,a)), z_goal) -- same as 085k.\n\n"
        ...
        f"| Variant | Enc type | goal_r | benefit_ratio | Note |\n"
        f"|---|---|---|---|---|\n"
        f"| 085i | raw rf (no enc) | 0.218 | 1.03x | spatially specific |\n"
        f"| 085j | BCE contact enc | 0.819 | 0.034x | E2Resource degenerate |\n"
        f"| 085k | BCE contact enc + RFM | 0.582 | 0.977x | near-random action selection |\n"
        f"| 085l | Prox regression enc + RFM | {avg_goal_resource_r_e:.3f}"
