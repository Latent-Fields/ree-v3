...
CHANGE vs EXQ-385 (one bug fixed):
  EXQ-385 used shy_decay_rate=0.85 with 15 sleep cycles (P1=150 eps,
  SLEEP_INTERVAL=10). This produces 0.85^15 ~= 0.09 -- slot collapse.
  WITH_OFFLINE showed slot_diversity ~= 0.12 vs WITHOUT_OFFLINE ~= 1.0
  (reversed comparison, invalidating C1).

  Fix: shy_decay_rate=0.85 -> shy_decay_rate=0.98
  0.98^15 ~= 0.74 -- gentle homeostasis, preserves slot diversity.

  SHY is kept enabled (shy_enabled=with_offline) because SHY is part of
  the offline consolidation mechanism being tested (MECH-120). Only the
  decay rate changes.

SCIENTIFIC QUESTION (unchanged from EXQ-385):
  Is offline consolidation a computational necessity for stable world-model
  formation? (INV-049)

  WITH_OFFLINE -- SWS+REM sleep cycles every SLEEP_INTERVAL episodes.
  WITHOUT_OFFLINE -- waking-only, no offline consolidation.

ACCEPTANCE CRITERIA (unchanged from EXQ-385):
  C1: slot_diversity(WITH_OFFLINE) > slot_diversity(WITHOUT_OFFLINE) in >= 2/3 seeds
  C2: late_pred_loss(WITH_OFFLINE) < late_pred_loss(WITHOUT_OFFLINE) in >= 2/3 seeds
  C3: eval_harm_rate(WITH_OFFLINE) < eval_harm_rate(WITHOUT_OFFLINE) in >= 2/3 seeds
...
EXPERIMENT_TYPE    = "v3_exq_385a_inv049_offline_consolidation_necessity"
SUPERSEDES_ID      = "V3-EXQ-385"
CLAIM_IDS          = ["INV-049"]
EXPERIMENT_PURPOSE = "evidence"

P0_EPISODES       = 50          # encoder warmup (no sleep in either condition)
P1_EPISODES       = 150         # P1: WITH_OFFLINE gets sleep; WITHOUT does not
...
def _make_agent(env: CausalGridWorldV2, with_offline: bool) -> REEAgent:
    ...
    Fix vs EXQ-385: shy_decay_rate changed from 0.85 to 0.98.
    0.85^15 ~= 0.09 caused slot collapse (WITH_OFFLINE reversed vs WITHOUT).
    0.98^15 ~= 0.74 allows gentle homeostasis without collapsing slots.
    """
    cfg = REEConfig.from_dims(
        ...
        # MECH-120: SHY normalisation (only with sleep; fixed decay rate)
        shy_enabled=with_offline,
        shy_decay_rate=0.98,   # fix: was 0.85 (0.85^15=0.09 collapses slots)
    )
    return REEAgent(cfg)
...
def run_condition(
    ...
    # ---- P1: WITH_OFFLINE gets sleep; WITHOUT continues waking ----
...
def main(dry_run: bool = False) -> Dict:
    ...
    output = {
        ...
        "fix_description": (
            "SHY decay rate changed from 0.85 to 0.98. "
            "EXQ-385 used 0.85 with 15 cycles -> 0.85^15=0.09 -> slot collapse "
            "reversed the C1 diversity comparison. 0.98^15=0.74 allows gentle "
            "homeostasis without destroying slot differentiation."
        ),
        "acceptance_checks": {
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="V3-EXQ-385a: INV-049 offline consolidation necessity (SHY fix)"
    )
    parser.add_argument(
