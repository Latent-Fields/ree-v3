"""
V3-ONBOARD-smoke-ree-cloud-2 -- Cloud Worker Onboarding Smoke Test

Purpose: verify the full experiment pipeline round-trip on the second Hetzner
cloud worker AND benchmark CPU throughput so experiment routing can match
experiment type to the best available machine.

Mirrors v3_onboard_smoke_ree_cloud_1.py exactly -- only identifiers differ.
The two cloud workers are nominally identical Hetzner CX22 instances; any
significant throughput divergence between their smoke results is a signal
of per-instance variation (shared-vCPU neighbour noise) worth flagging.
...
Machine affinity: ree-cloud-2 (set in experiment_queue.json).
...
EXPERIMENT_TYPE = "v3_onboard_smoke_ree_cloud_2"
...
def run(seed: int = 42) -> dict:
    ...
    print(
        f"[ONBOARD] Cloud worker: ree-cloud-2 (Hetzner CX22)\n"
