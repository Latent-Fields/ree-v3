"""V3-EXQ-936a: MECH-439 -- does the CONVERTING F->eligibility demotion reduce
F's committed-selection variance share? BUG-FIX RE-RUN of V3-EXQ-936 (SAME
QUESTION, SAME CLAIM -- letter, not a new number).

WHY THIS IS A BUG FIX, NOT A NEW EXPERIMENT
--------------------------------------------
V3-EXQ-936 (run v3_exq_936_..._20260817T062038Z) measured this exact question
and got a corrupted answer: SD-056's online E2 contrastive training was armed
on every arm (as intended) but the E2 rollout-norm clamp
(`e2_rollout_output_norm_clamp_enabled`) that V3-EXQ-689i paired it with was
never declared in this lineage's baseline module, despite the module claiming
"689i parity" elsewhere in its own docstring. Without the clamp,
`world_forward_contrastive_loss` rollout magnitudes overflowed to 1e16-1e18
(matching V3-EXQ-569e's independently-measured overflow band on the same code
path), which saturated `f_variance_share` to 1.0 in all 8 cells and destroyed
the load-bearing C2 criterion -- var(non-F)/var(F) fell below float32 eps in
7 of 8 cells, additively annihilating the entire modulatory stack in the E3
score sum.

`failure_autopsy_V3-EXQ-936_2026-08-18` confirmed this as an INSTRUMENT defect
(severity `corrupting`, category `standard`/`non_contributory`, NOT
`substrate_ceiling` -- the run measured nothing usable, it did not refute
anything) and named the fix: enable the clamp with 689i's own parameters
(`e2_rollout_output_norm_clamp_enabled=True`, `e2_rollout_output_norm_clamp_ratio=2.0`).
That is now done in `experiments/_lib/baselines/mech439_f_variance_share.py`'s
`CONFIG_FLAGS` (fires on every arm, OFF included, exactly like the SD-056
training constants it sits beside). The clamp MECHANISM itself is BUILT and
VALIDATED (`substrate_queue.json` SD-056, `status: implemented`; already
exercised for real by V3-EXQ-689i) -- this change only turns it on for this
lineage. It does NOT flip the REEConfig class default (still False globally;
the autopsy's own `recommended_substrate_queue_entry` explicitly vetoed a
default flip as a separate governance decision, not something owed here).

The autopsy's OTHER recommendation -- a corpus-wide WARN-only lint
(`sd056_training_without_rollout_clamp`) catching any driver missing this
flag -- is deliberately NOT part of this change; it is a governance/tooling
improvement orthogonal to whether THIS re-test produces valid science, and is
left for a separate `/implement-substrate` pass if wanted.

RE-DERIVE BRAKE (Step 2.5b): MECH-439 carries a count of 12 prior
substrate_ceiling-or-non_contributory autopsies (689/689a, the f-dominance
cluster, 700/700a/700b/700c/700d-708, 709, 711, 713, and 936 itself) --
threshold triggering. The most recent (936's own autopsy) names the required
upstream substrate as SD-056, `status: implemented` (built) -- so per Step
2.5b's release condition, the brake RELEASES: the substrate this specific
re-test needs is built, and the failure being corrected is instrumentation
(the clamp not being turned on), not the F-demotion mechanism under test.
substrate_queue.json's 2.5c open-corrupting-entry gate does not block either:
SD-056's `status` field reads exactly `implemented` (matches the CLOSED set),
because what remains open per that entry is the CORPUS-WIDE lint, not the
per-lineage fix this run applies.

SUPERSEDES: v3_exq_936_mech439_f_variance_share_under_f_demotion_20260817T062038Z_v3
(non_contributory instrument-defect result; corrected here.)
...
INHERITED RE-DERIVE BRAKE CONTEXT (V3-EXQ-936's original release, still valid)
-------------------------------------------------------------------------------
The f-dominance-conversion-ceiling brake that V3-EXQ-936 itself released still
holds released -- nothing about the SD-056 clamp fix touches that substrate.
Kept verbatim for context. MECH-439 carries 12 counted substrate_ceiling
autopsies -- 689/689a, the f-dominance-conversion cluster,
700-cluster/700b/700c/700d-708, 709, 710, 711, 713 and the 711-713
re-adjudication (936 itself is now also a 13th, per this file's own
RE-DERIVE BRAKE section above, which is the one that actually governs
936a). The most recent (2026-07-20) names
...
# applied via kwargs.update(CONFIG_FLAGS) inside make_agent_kwargs(). That is
# a **kwargs-splat two-level indirection, one of the lint's own documented
# blind spots (validate_experiments.py sd056_training_without_rollout_clamp_lint
# docstring: "**kwargs splats, indirection beyond one level"), not an actual
# omission -- this IS the fix this experiment exists to apply.
SD056_ROLLOUT_CLAMP_EXEMPT = (
    ...
    "this driver IS the V3-EXQ-936a fix for the missing clamp, see module docstring"
)

EXPERIMENT_TYPE = "v3_exq_936a_mech439_f_variance_share_rollout_clamp_fix"
EXPERIMENT_PURPOSE = "evidence"
ARCHITECTURE_EPOCH = "ree_hybrid_guardrails_v1"
CLAIM_IDS = ["MECH-439"]
SUPERSEDES_RUN_ID = (
    "v3_exq_936_mech439_f_variance_share_under_f_demotion_20260817T062038Z_v3"
)
    ...
    manifest: Dict[str, Any] = {
        ...
        "custom_information": {
            ...
            "re_derive_brake_release": (
                "MECH-439 brake count 12 (13 counting V3-EXQ-936 itself); most "
                "recent counted autopsy (failure_autopsy_V3-EXQ-936_2026-08-18) "
                "names upstream substrate SD-056, status=implemented (the "
                "rollout-norm clamp mechanism is BUILT+VALIDATED, already "
                "exercised for real by V3-EXQ-689i). Category is standard / "
                "non_contributory (instrument defect), not substrate_ceiling -- "
                "the prior run measured nothing usable rather than refuting "
                "anything. Brake releases: the substrate this specific re-test "
                "needs (the clamp) is built; only the corpus-wide WARN lint the "
                "autopsy separately recommends remains unbuilt, and that lint "
                "is orthogonal to this run's validity. Original "
                "f_dominance_conversion_ceiling release (936's own) still "
                "holds unchanged -- see the inherited-context section above."
            ),
            "gov_reuse_1_check": (
                "The corrected f_variance_share readout is not recoverable "
                "from any existing manifest: V3-EXQ-936 itself is the only "
                "prior run of this exact question and its own reading is the "
                "corrupted one this run corrects (saturated to 1.0 by the "
                "unclamped-rollout defect). No other manifest carries a "
                "variance-share readout on a compatible (clamp-enabled) "
                "substrate_hash. Not recoverable -> run."
            ),
        },
        "ethics_preflight": {
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description=(
            "V3-EXQ-936a MECH-439: does a converting selection-face lever reduce "
            "F's committed-selection variance share? (rollout-clamp bug-fix "
            "re-run of V3-EXQ-936)"
        )
    )
