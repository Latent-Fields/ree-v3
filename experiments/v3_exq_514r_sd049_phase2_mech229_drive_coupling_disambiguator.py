"""
V3-EXQ-514r -- SD-049 Phase-2 MECH-229 drive-coupling DISAMBIGUATOR.

The pre-registered disambiguator routed by failure_autopsy_V3-EXQ-514q_2026-06-16
(confirmed, user-adjudicated 2026-06-16). 514q's load-bearing criterion
C_WL_DRIVE = mean(WL_drive - WL_nodrive) returned EXACTLY 0.0 on all 5 guard-passing
seeds (WL_drive == WL_nodrive byte-identical, 0.7021) -- the drive-favored most-wanted
argmax never differed from the drive-uniform argmax -- because the in-run per-axis
drive spread (~0.0055-0.0085) is far below the magnitude that could flip an
object-base_value-dominated most_wanted. The 514q readiness gate verified the
machinery CAN separate (a constructed positive control, separation 1.0) and that some
spread > 1e-3 exists, but NOT that the in-run spread is ARGMAX-RELEVANT, and no
overshoot control was run -- so the zero delta could not be read as
"drive does not carve wanting" vs "drive never varied enough to carve".

THIS RUN DISAMBIGUATES THAT NULL with three additions on the 514q-identical substrate:

  (i)  OVERSHOOT computation: per scored consumption event, also compute most_wanted
       under an ARTIFICIALLY LARGE per-axis drive (OVERSHOOT_DRIVE_MAGNITUDE on the
       most-depleted axis). If the overshoot argmax DIFFERS from the drive-uniform
       argmax (overshoot_flip_frac >= FLIP_FLOOR on >= 2/3 guard-passing seeds), then
       drive CAN carve wanting given adequate magnitude -- so the 514q FAIL is a
       DRIVE-MAGNITUDE / ENVIRONMENT artifact (substrate_ceiling), NOT a MECH-229
       weakens. If overshoot CANNOT flip even at maximal drive, that IS the genuine
       MECH-229 drive-coupling weakens.

  (ii) OFF / bank-disabled FLOOR control: per scored event, compute the wanting target
       with the bank bypassed (wanting := the consumed/liking type), establishing the
       wanting==liking floor. wl_off_frac must read ~0 (a non-vacuity sanity check: the
       scoring harness produces 0 when wanting is structurally equal to liking, so a
       non-zero natural/overshoot dissociation is genuinely bank-driven).

  (iii) RECALIBRATED readiness gate: the leg-2 in-run non-vacuity floor is replaced by
       an ARGMAX-RELEVANCE positive control -- a constructed 2-token bank with a
       realistic cross-token base_value gap on which the OVERSHOOT drive MUST flip the
       argmax while the NATURAL-magnitude drive must NOT. This asserts the overshoot
       magnitude is in the decision-moving regime for this run's base_value spread; a
       below-floor reading means OVERSHOOT_DRIVE_MAGNITUDE is mis-calibrated (fix the
       test -> substrate_not_ready_requeue), NEVER a verdict on the claim.

DECISION GRID (after gates (a) contact + (b) recalibrated readiness + (b2) OFF floor +
(b3) bank populated are met; any unmet -> substrate_not_ready_requeue / non_contributory):
  PASS / supports         : natural drive delta >= effect_margin (drive carves naturally
                            now -- the 514q criterion clears; preserves the supports path).
  FAIL / non_contributory : overshoot_flip_frac >= FLIP_FLOOR on >= 2/3 seeds -> drive
    (substrate_ceiling)     CAN carve at adequate magnitude -> 514q FAIL is env/magnitude
                            limited -> /implement-substrate amend SD-049-PHASE-2 (env
                            differential-depletion / kappa-scaling). NOT a weakens.
  FAIL / weakens          : overshoot CANNOT flip even at maximal drive -> the genuine
                            MECH-229 drive-coupling weakens (drive cannot carve wanting
                            in this substrate even at overshoot).

CLAIM SCOPE: claim_ids=[MECH-229], and this tests SUB-LEG (b) DRIVE-MODULATED wanting
ONLY. Sub-leg (a) wanting != liking (object-bound dissociation) is ALREADY ESTABLISHED
by V3-EXQ-514o PASS (0.80) -- this run MUST NOT be allowed to weaken (a). The (a)/(b)
split is the /claim-synthesis routing the autopsy also recorded; (b) is the only thing
under test here.

SUBSTRATE + instrument chain: IDENTICAL to 514q (full scaffolded_sd054_onboarding 603n
curriculum; 681-C4 info-tag liking source; SD-057 IncentiveTokenBank wanting). Only the
SCORING (overshoot + OFF + recalibrated readiness) changes.

experiment_purpose: evidence
supersedes: V3-EXQ-514q (the readiness gate green-lit a vacuous comparison + ran no
  overshoot control; 514r makes the overshoot-flip the disambiguating statistic).
predecessor: V3-EXQ-514q.
...
EXPERIMENT_TYPE = "v3_exq_514r_sd049_phase2_mech229_drive_coupling_disambiguator"
QUEUE_ID = "V3-EXQ-514r"
CLAIM_IDS: List[str] = ["MECH-229"]
EXPERIMENT_PURPOSE = "evidence"
PREDECESSOR = "V3-EXQ-514q (drive-coupled delta returned exactly 0.0; disambiguate magnitude vs weakens)"
SUPERSEDES = "V3-EXQ-514q"
...
# Supports path (514q load-bearing criterion, retained as the PASS gate)
...
def run_experiment(dry_run: bool = False) -> Dict[str, Any]:
    ...
    readiness_ok = bool(
        contact_non_vacuity_met and wl_readiness_met and off_floor_met and run_populated_met
    )
    ...
    if not contact_non_vacuity_met:
        ...
        # the 514q criterion clears -> drive carves naturally now.
        outcome = "PASS"; readiness_route = "mech229_drive_coupled_dissociation_isolated"
        evidence_direction = "supports"; route_reason = "natural_drive_delta_cleared_effect_margin"
    elif overshoot_resolves:
        # drive CAN carve at adequate magnitude -> 514q FAIL is env/magnitude-limited.
        outcome = "FAIL"; readiness_route = "mech229_drive_magnitude_artifact_substrate_ceiling"
        evidence_direction = "non_contributory"
        route_reason = "overshoot_flips_argmax_implement_substrate_sd049_phase2_differential_depletion"
    else:
        # overshoot cannot flip even at maximal drive -> genuine drive-coupling weakens.
        outcome = "FAIL"; readiness_route = "mech229_drive_coupling_genuine_weakens_overshoot_cannot_flip"
        evidence_direction = "weakens"
        route_reason = "overshoot_cannot_flip_argmax_genuine_mech229_drive_coupling_weakens"

    # Non-degeneracy scoring net (2026-06-11): the disambiguator got a fair, non-vacuous
    # test only when the gates (a)+(b)+(b2)+(b3) are met. A weakens/non_contributory verdict
    # before those is degenerate (the overshoot statistic could not be trusted).
    non_degenerate = bool(readiness_ok)
    degeneracy_reason = ("" if non_degenerate
                         else f"disambiguator not non-vacuously testable: {route_reason} "
    ...
    print(f"[{EXPERIMENT_TYPE}] readiness contact={contact_non_vacuity_met}"
          ...
          f" bank_pop={run_populated_met} (frac={run_populated_frac:.3f})", flush=True)
    ...
    return {
        ...
        "interpretation": {
            ...
            "route_reason": route_reason,
            "disambiguation": {
                "supports_if": "natural drive delta >= max(K_SD*pstdev(delta), DELTA_FLOOR)",
                "substrate_ceiling_if": "overshoot flips argmax on >= 2/3 seeds (drive can carve "
                                        "at adequate magnitude; 514q FAIL is env/magnitude-limited "
                                        "-> /implement-substrate amend SD-049-PHASE-2)",
                "genuine_weakens_if": "overshoot CANNOT flip even at OVERSHOOT_DRIVE_MAGNITUDE "
                                      "(drive cannot carve wanting in this substrate)",
                "claim_scope_note": "tests MECH-229 sub-leg (b) drive-modulated wanting ONLY; "
                                    "sub-leg (a) wanting!=liking is established by V3-EXQ-514o PASS "
