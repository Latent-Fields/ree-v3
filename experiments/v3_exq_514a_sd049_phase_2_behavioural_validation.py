...
"""V3-EXQ-514a -- SD-049 Phase 2 behavioural validation, fixed instrumentation.

Supersedes: V3-EXQ-514 (FAIL 7/10 on 2026-05-04T10:06Z due to instrumentation
issues, not architectural failure).
...
FAIL on C2b but C2c PASS (sufficient samples but probe still fails):
  ...
  OR fundamental architectural mismatch. Recommendation: V3-EXQ-514b
...
FAIL on C1b only:
  ...
  OR detach trunk for classifier-only loss in V3-EXQ-514b. This narrows
...
experiment_purpose = "evidence" (governance evidence; supersedes V3-EXQ-514).
supersedes: v3_exq_514_sd049_phase_2_behavioural_validation_20260504T100649Z_v3

Run with:
  /opt/local/bin/python3 experiments/v3_exq_514a_sd049_phase_2_behavioural_validation.py
  /opt/local/bin/python3 experiments/v3_exq_514a_sd049_phase_2_behavioural_validation.py --dry-run
"""
...
EXPERIMENT_TYPE = "v3_exq_514a_sd049_phase_2_behavioural_validation"
...
def make_env(seed: int, arm_cfg: Dict) -> CausalGridWorld:
    """Build env per arm config with V3-EXQ-514a high-contact-rate tuning."""
    kwargs = dict(
...
def run_evaluation(
    ...
    Fix (B): TWO probe targets reported.
      ...
      probe_acc_neighborhood: NEW -- sampled at every step, labeled by argmax
        ...
        acceptance test for V3-EXQ-514a.
    """
    ...
    # Neighborhood-target probe samples (V3-EXQ-514a primary methodology)
...
def main(dry_run: bool = False) -> int:
    ...
    manifest = {
        ...
        "supersedes_note": (
            "V3-EXQ-514 ran 2026-05-04T10:06Z and FAILed 7/10 due to: "
            "(1) sparse contact rate -- only 8-9 identity samples per "
            "arm-seed for a 3-class probe; (2) goal_resource_r metric "
            "saturated at ~1.0 across all arms; (3) classifier loss "
            "diverging at unit weight. V3-EXQ-514a fixes all three: "
            "smaller grid + no hazards + more resources + longer episodes "
            "for higher contact rate; neighborhood-dominant-type probe "
            "for dense sampling; classifier_loss_weight=0.1 for stable "
            "joint training."
        ),
    }
