"""
V3-EXQ-812a -- HARNESS-REPAIR successor to V3-EXQ-812 (supersedes it). Same
MECH-295 Layer-C cue-authority question, same C0-C3 pre-registered gates, same
scaffolded_sd054_onboarding substrate and curriculum -- ONE substantive change,
documented in the "812a HARNESS FIX" section below.
...
812a HARNESS FIX (chip-20260808-v3exq812-cue-authority-harness-repair):
V3-EXQ-812 was confirmed-autopsied INVALID_HARNESS 2026-07-24
(failure_autopsy_backlog_2026-07-24.json, targets[] V3-EXQ-812): C1_cue_fires
passed but C0_evaluability/C2_reaches_scoring/C3_selection_authority all
failed, with mean_proximity_range EXACTLY 0.0 across every seed and arm and
c3_diff_in_diff_per_seed ~1e-9-1e-10 (numerically zero) -- the cue fires but
never reaches the scoring path, so selection authority was unmeasurable, not
absent. Root cause: 812's REEConfig trains SD-056 action-conditional
divergence (e2_action_contrastive_enabled=True) but never sets
candidate_summary_source="e2_world_forward", so REEAgent._candidate_world_summaries
(ree_core/agent.py) returns None and MECH-295's per-candidate proximity read
(agent.py ~line 7091, goal_state.goal_proximity(m295_summaries)) silently falls
back to the collapsed proposer first-step z_world, whose cross-candidate spread
is ~0 under monostrategy by construction (ARC-065 GAP-A, ree_core/utils/config.py
candidate_summary_source docstring; V3-EXQ-614e autopsy) -- exactly the observed
symptom. This landed 2026-06-07, a month before 812's 2026-07-24 run; 812 simply
never set it. Fix (the ONLY substantive change vs the V3-EXQ-812 driver):
candidate_summary_source="e2_world_forward" added to _make_config()'s
REEConfig.from_dims(...) call. Validated precedent for this exact fix on a
comparable substrate: V3-EXQ-649 (the original GAP-A validation experiment,
candidate_summary_source="e2_world_forward" + e2_action_contrastive_enabled=True
together) and V3-EXQ-682 (outcome PASS, cause_label="no_collapse_reproduced",
summary_pairwise_dist_mean=0.065 -- confirms cross-candidate spread reliably
clears the floor at the live select tick under this exact config combination,
so this is a low-risk, well-precedented harness repair, not a substrate
redesign). No other pre-registered gate, threshold, curriculum parameter, or
seed was changed.
...
supersedes: V3-EXQ-812 (INVALID_HARNESS; see "812a HARNESS FIX" above)
predecessor (not supersedes, in the 812 sense): V3-EXQ-490j (weakens/evidence on
  the necessity claim, gap4 regime) and V3-EXQ-490k (diagnostic, pre-603n
  substrate, could not test authority: mech295_bias_range_mean=0.0). 812/812a is
  the Layer-C authority measurement neither could complete, run on the substrate
  governance actually gated it to.
"""
...
EXPERIMENT_TYPE = "v3_exq_812a_mech295_cue_authority_sd054"
QUEUE_ID = "V3-EXQ-812a"
SUPERSEDES = "V3-EXQ-812"
CLAIM_IDS: List[str] = []
EXPERIMENT_PURPOSE = "diagnostic"
...
def _make_config(env) -> REEConfig:
    cfg = REEConfig.from_dims(
        ...
        # 812a HARNESS FIX (see module docstring): route the E3-side bias
        # channels -- including MECH-295's per-candidate proximity read -- through
        # the SD-056-trained action-conditional e2.world_forward predictions
        # instead of the default collapsed-proposer path, whose cross-candidate
        # spread is ~0 under monostrategy (ARC-065 GAP-A; V3-EXQ-614e autopsy).
        # e2_action_contrastive_enabled=True above was already training SD-056;
        # this is the missing consumer-side wire. Validated combination:
        # V3-EXQ-649 / V3-EXQ-682.
...
CONFIG_SLICE = {
    ...
    "candidate_summary_source": "e2_world_forward",  # 812a HARNESS FIX
}
...
def main(dry_run: bool = False) -> Dict[str, Any]:
    ...
    manifest: Dict[str, Any] = {
        ...
        "predecessor": "V3-EXQ-812 (INVALID_HARNESS, confirmed autopsy 2026-07-24: cue fired "
                       "but candidate_proximity range was exactly 0.0 every seed/arm because "
                       "candidate_summary_source was left at its 'proposer' default -- see "
                       "module docstring '812a HARNESS FIX'; this run adds "
                       "candidate_summary_source='e2_world_forward' and nothing else). Earlier: "
                       "V3-EXQ-490j (weakens/evidence, gap4, necessity layer); V3-EXQ-490k "
