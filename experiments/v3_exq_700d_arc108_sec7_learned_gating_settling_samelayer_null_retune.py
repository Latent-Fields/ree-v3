...
"""
V3-EXQ-700d -- ARC-108 sec-7 LEARNED-GATING CONVERSION FALSIFIER, brake-EXEMPT
VALIDITY RE-TUNE of the SAME-LAYER null. SUPERSEDES V3-EXQ-700c.

THE ONE CHANGE FROM 700c (config-only validity fix; no ree_core change)
-----------------------------------------------------------------------
700c proved the SAME-LAYER null is the correct LAYER (coupled to the committed-class
DV, unlike 700b's policy-temperature null) but mis-CALIBRATED its MAGNITUDE: the fixed
FIELD_NOISE_WLAT_SCALE=0.5 produced a frozen-random W_lat range of 2.16 = 41.42x the
realised learned A2/A3 settling range of 0.052 (field_noise_magnitude_matched band
[0.25,4.0] FAILED), and the oversized null DROVE committed-class entropy BELOW A0
(0.859 < 0.909; noise_verified_lifting 0/3) -- the V3-EXQ-642 invalid-precondition
family (same shape as V3-EXQ-704's 176.9x). 700d AUTO-COMPUTES the null magnitude: the
frozen random [C,C] structure is RESCALED so its range == FIELD_NOISE_TARGET_RATIO (1.0)
x the in-run median LEARNED W_lat range of the A2/A3 SIGNED settling arms ONLY (the C3
UNSIGNED arm's W_lat blows up to range 1.9-4.8 via abs-RPE accumulation and is EXCLUDED
from both the match target and the magnitude-match denominator). So the null is
magnitude-matched (ratio ~1.0, in band) BY CONSTRUCTION. Two readiness guards are added:
a NEAR-VACUITY guard (the median A2/A3 learned range -- the match target -- must clear
LEARNED_RANGE_VACUITY_FLOOR, else a matched null tests structure against noise-of-noise)
and an explicit BELOW-A0 destructive-null guard; both self-route substrate_not_ready_requeue.
Everything else is identical to 700c.

THE 700b->700c LAYER FIX (retained context)
-------------------------------------------
...
WHY THIS IS BRAKE-EXEMPT (user-adjudicated 2026-06-26 Reading B, re_derive_brake.exempt=true):
MECH-439 has multiple prior non_contributory/substrate_ceiling autopsies (the mechanical
re-derive brake), but 700c's self-route was an INVALID-PRECONDITION (magnitude-mismatched
41.42x null), NOT a clean ceiling -- so the brake did NOT fire. 700d is a measurement-
VALIDITY fix (auto-magnitude-match the same-layer null + vacuity/below-A0 guards), the same
class as 704->704b and 705->705b, and is the ONE validity-fixed letter the brake-LOCK
permits. The brake REFUSES any further same-lever requeue on a VALID-null no-lift.

TERMINALITY
-----------
PRE-REGISTERED TERMINAL for the VALIDITY question. The null magnitude is now AUTO-COMPUTED
from the in-run learned A2/A3 range (no fixed scale left to re-tune). A magnitude-matched
null that VERIFY-LIFTS while the learned settling arms still do NOT lift is the DECISIVE
"no lift" outcome -> escalate to the V4 ARC-110 loop-segregation substrate
(implement-substrate), NEVER another V3 letter. This TRIPS the brake-LOCK recorded by the
700c autopsy (the next VALID-null no-lift fires the re-derive brake).
...
--------------------------------------------------
  self-route substrate_not_ready_requeue if ANY precondition is unmet:
   ...
   [LO,HI] x the median LEARNED A2/A3 SIGNED settling-arm W_lat range (auto-matched by
   construction; the C3 unsigned tail is excluded); (f) learned_settling_range_non_vacuous
   -- that median A2/A3 learned range itself clears LEARNED_RANGE_VACUITY_FLOOR; (g)
   same_layer_null_not_destructive -- the matched null does not drive committed-class
   entropy BELOW A0 on a majority of divergent seeds.
...
See REE_assembly/evidence/planning/failure_autopsy_V3-EXQ-700c_2026-06-26.{md,json} (the routing autopsy),
    REE_assembly/evidence/planning/failure_autopsy_V3-EXQ-700b_2026-06-24.{md,json},
    REE_assembly/evidence/planning/dopamine_into_gating_design_2026-06-22.md,
    experiments/v3_exq_700c_arc108_sec7_learned_gating_settling_samelayer_null.py (the superseded harness),
...
EXPERIMENT_TYPE = "v3_exq_700d_arc108_sec7_learned_gating_settling_samelayer_null_retune"
QUEUE_ID = "V3-EXQ-700d"
SUPERSEDES = "V3-EXQ-700c"
...
# 700d VALIDITY RE-TUNE: the frozen-random W_lat scale is no longer a fixed pre-registered
# constant (700c's FIELD_NOISE_WLAT_SCALE=0.5 produced range 2.16 = 41.42x the realised
# learned A2/A3 range 0.052 -> field_noise_magnitude_matched FAILED, the V3-EXQ-642
# invalid-precondition family). Instead the injected null range is AUTO-COMPUTED at run
# time to equal FIELD_NOISE_TARGET_RATIO x the realised median LEARNED settling range of
# the A2/A3 SIGNED arms only (the C3 UNSIGNED arm's W_lat blows up to range 1.9-4.8 via
# abs-RPE accumulation without sign cancellation and is EXCLUDED from the match target).
# ratio 1.0 => the null has the SAME magnitude as the learned structure (the fairest
# attribution bar: a lift is learned STRUCTURE only if it beats matched-magnitude random
# structure at the same layer). The [LO,HI] band remains the validity acceptance window
# (robust to per-seed jitter); by construction the realised ratio lands ~1.0, in band.
FIELD_NOISE_TARGET_RATIO = 1.0        # inject range == 1.0 x median A2/A3 signed learned W_lat range
FIELD_NOISE_SEED_OFFSET = 100000      # reproducible-per-cell random W_lat distinct from the cell seed's other RNG
FIELD_NOISE_MAGNITUDE_MATCH_LO = 0.25  # ARM_NOISE wlat_range_max must be within [LO,HI] x median A2/A3 signed learned range
FIELD_NOISE_MAGNITUDE_MATCH_HI = 4.0
# 700d near-vacuity guard: the match TARGET itself (median A2/A3 signed learned W_lat range)
# must clear this floor. If the learned settling arms barely move W_lat, a magnitude-matched
# null tests learned-structure against noise-of-noise -- a vacuous match -> substrate_not_ready_requeue
# (NOT a verdict). Realised 700c learned median was 0.052 (~520x WLAT_RANGE_FLOOR); this floor
# (50x the bare detection floor, ~10x below the realised value) rejects a collapsed-to-floor
# learned range while passing a genuinely-moved one.
LEARNED_RANGE_VACUITY_FLOOR = 5e-3
# Fallback scale used ONLY if the realised learned target is unavailable (e.g. all A2/A3
# cells errored) so the injection is still reproducible; the vacuity/match gates then self-route.
FIELD_NOISE_FALLBACK_SCALE = 0.05
...
def _inject_field_noise_wlat(
    agent: REEAgent, seed: int, target_range: Optional[float]
) -> float:
    """SAME-LAYER null injection (700d VALIDITY RE-TUNE): seed a FROZEN random W_lat once,
    RESCALED so its range (max - min) equals `target_range`.

    700c injected randn(C,C) * FIELD_NOISE_WLAT_SCALE (fixed 0.5), giving range ~2.16 =
    41.42x the realised learned A2/A3 range -> magnitude mismatch -> invalid precondition.
    700d instead generates the random [C,C] STRUCTURE, measures its RAW range, then rescales
    so the injected range == target_range (= FIELD_NOISE_TARGET_RATIO x the in-run median
    LEARNED A2/A3 signed-settling W_lat range, computed by run_experiment before ARM_NOISE
    runs). The random STRUCTURE is preserved; only its MAGNITUDE is matched. Falls back to
    FIELD_NOISE_FALLBACK_SCALE if no target is available (the vacuity/match gates then
    self-route). Reproducible per cell via FIELD_NOISE_SEED_OFFSET (independent of the cell
    RNG). Returns the injected range.

    Only meaningful when learned_settling_eta=0.0 (W_lat frozen) -- which _make_agent
    guarantees on the field-noise arm -- so the matrix is a stable random-structure
    lateral-inhibition operator the settling step injects into the within-eligible field.
    """
    C = int(SETTLING_N_ACTION_CLASSES)
    g = torch.Generator(device="cpu").manual_seed(int(seed) + FIELD_NOISE_SEED_OFFSET)
    W = torch.randn((C, C), generator=g)
    raw_range = float((W.max() - W.min()).item())
    tgt = (
        float(target_range)
        if (target_range is not None and target_range > 0.0)
        else FIELD_NOISE_FALLBACK_SCALE
    )
    if raw_range > 0.0:
        W = W * (tgt / raw_range)
...
def _run_seed_arm(
    ...
    # SAME-LAYER null (700d): inject the frozen random W_lat ONCE on the field-noise arm,
    # RESCALED so its range == noise_target_range (= the in-run median learned A2/A3 signed
    # range, computed by run_experiment before ARM_NOISE runs). eta=0 keeps it frozen across
    # all P0/P1/P2 episodes (W_lat is not reset by agent.reset()).
    field_noise_wlat_range = 0.0
    if arm.get("field_noise"):
        field_noise_wlat_range = _inject_field_noise_wlat(agent, seed, noise_target_range)
...
def run_experiment(
    ...
    for arm in ARMS:
        ...
        # 700d: auto-compute the same-layer null magnitude from the realised LEARNED A2/A3
        # SIGNED-settling W_lat ranges (the C3 UNSIGNED arm's abs-RPE blow-up is EXCLUDED).
        # ARM_NOISE runs LAST in the arm-major loop, so A2/A3 rows are complete here.
        noise_target_range: Optional[float] = None
        if arm["arm_id"] == "ARM_NOISE":
            a2a3_learned = [
                float(r["wlat_range_max"])
                for r in arm_results
                if r["arm_id"] in ("A2_SETTLING_SIGNED", "A3_BOTH_SIGNED")
                and r["error_note"] is None
            ]
            median_a2a3 = float(statistics.median(a2a3_learned)) if a2a3_learned else 0.0
            noise_target_range = (
                FIELD_NOISE_TARGET_RATIO * median_a2a3 if median_a2a3 > 0.0 else None
            )
            _tgt_print = (
                noise_target_range if noise_target_range is not None
                else FIELD_NOISE_FALLBACK_SCALE
            )
            print(
                f"  [700d same-layer null] median A2/A3 signed learned wlat_range="
                f"{median_a2a3:.6f} -> target injected range={_tgt_print:.6f}",
                flush=True,
            )
        ...
        for s in seeds:
            ...
            if row is None:
                row = _run_seed_arm(
                    ...
                    noise_target_range=noise_target_range,
                )
            ...
            if reusable:
                ...
                row["arm_fingerprint"] = compute_arm_fingerprint(
                    config_slice={
                        ...
                        "noise_on": bool(arm["noise_on"]),
                        "field_noise": bool(arm.get("field_noise", False)),
                        "field_noise_target_ratio": float(FIELD_NOISE_TARGET_RATIO),
                        "field_noise_match_mode": "auto_from_a2a3_signed_learned_median",
    ...
    # 700d explicit guard: a magnitude-matched null that DRIVES committed-class entropy
    # strictly BELOW A0 on a majority of divergent seeds is pathological (an oversized/
    # destructive perturbation -- the 700c failure: noise 0.859 < A0 0.909). Framed as a
    # FLOOR on the not-below count so the indexer's default-floor met-recompute agrees with
    # the author met. (verified_lifting already requires strict-ABOVE, so this is a legible
    # safety net, not an independent binding gate.)
    n_noise_below_a0 = sum(
        1 for s in primary_div if noise_ent.get(s, 0.0) < a0_ent.get(s, 0.0)
    )
    n_noise_not_below_a0 = n_primary_div - n_noise_below_a0
    need_not_below = max(
        MIN_SEEDS_FOR_PASS, int(math.ceil(DIVERGENT_PASS_FRACTION * max(n_primary_div, 1)))
    )
    noise_not_destructive = bool(n_noise_not_below_a0 >= need_not_below)

    # ----- Precondition (e): field_noise_magnitude_matched (auto-matched by construction) -----
    # ARM_NOISE's median frozen-random W_lat range must be within [LO,HI] x the median LEARNED
    # A2/A3 SIGNED settling-arm W_lat range. 700d auto-rescales the injected null to that same
    # A2/A3 median, so the realised ratio lands ~1.0 in band; an out-of-band ratio (the auto-
    # scale failed to land in band) => substrate_not_ready_requeue. No fixed scale to re-tune.
    # 700d: match denominator = A2/A3 SIGNED learned arms ONLY. The C3 UNSIGNED arm's W_lat
    # blows up (range 1.9-4.8) via abs-RPE accumulation without sign cancellation, so including
    # it would distort the median (the autopsy's secondary calibration note). The injected null
    # range was auto-scaled to this same A2/A3 median, so the realised ratio lands ~1.0 in band.
    noise_wlat_ranges = [float(r["wlat_range_max"]) for r in noise_rows]
    learned_a2a3_wlat_ranges = [float(r["wlat_range_max"]) for r in (a2_rows + a3_rows)]
    # Full A2/A3/C3 median retained for reporting/continuity with 700c only.
    settling_wlat_ranges_full = [
        float(r["wlat_range_max"]) for r in (a2_rows + a3_rows + c3u_rows)
    ]
    median_noise_wlat_range = _median(noise_wlat_ranges)
    median_learned_a2a3_wlat_range = _median(learned_a2a3_wlat_ranges)
    median_settling_wlat_range = _median(settling_wlat_ranges_full)
    if median_learned_a2a3_wlat_range > 0.0:
        field_noise_magnitude_ratio = median_noise_wlat_range / median_learned_a2a3_wlat_range
    else:
        field_noise_magnitude_ratio = 0.0
    field_noise_magnitude_matched = bool(
        median_learned_a2a3_wlat_range > 0.0
        and FIELD_NOISE_MAGNITUDE_MATCH_LO <= field_noise_magnitude_ratio <= FIELD_NOISE_MAGNITUDE_MATCH_HI
    )
    # 700d near-vacuity guard: the match TARGET itself must clear the vacuity floor, else a
    # magnitude-matched null is testing learned-structure against noise-of-noise.
    learned_range_non_vacuous = bool(
        median_learned_a2a3_wlat_range > LEARNED_RANGE_VACUITY_FLOOR
    )
    ...
    n_crf_differentiated_per_arm = [
        ...
        for rows in (a0_rows, a2_rows, a3_rows, c3u_rows, noise_rows)
    ]
    n_crf_differentiated_min = int(min(n_crf_differentiated_per_arm))

    preconditions_met = bool(
        enough_divergent
        and learned_range_non_vacuous
        and noise_verified_lifting
        and noise_not_destructive
    ...
    # it is strictly harsher than "a majority of seeds" (2-of-3 seeds can clear the
    # floor while the min does not, flagging a sound run precondition_unmet), and a
    # count over a conjunction does not distribute into per-leg counts. Reported
    # instead as the satisfying-seed COUNT minimised over arm groups vs
    # MIN_SEEDS_FOR_PASS with comparator ">=" -- exact, because
    # min(counts) >= k iff every count >= k. The old min-spread numbers are preserved
    # on each entry as NON-BOUND diagnostic keys (extra keys are ignored by the
    ...
    # on its own bounds -- see the entry below. (`need_not_below` above is the same
    # quantity for the not-destructive guard; kept separate for legibility.)
    ...
    interpretation = {
        ...
        "preconditions": [
            {
                ...
                "description": (
                    ...
                    "seeds carrying crf_differentiated, over arms (a0_rows, a2_rows, a3_rows, c3u_rows, noise_rows)."
                ),
            ...
            },
            {
                ...
                "kind": "readiness",
                "description": (
                    "700d SAME-LAYER null magnitude match: ARM_NOISE's median frozen-random "
                    "W_lat range is within [FIELD_NOISE_MAGNITUDE_MATCH_LO, "
                    "FIELD_NOISE_MAGNITUDE_MATCH_HI] x the median LEARNED A2/A3 SIGNED "
                    "settling-arm W_lat range (the C3 UNSIGNED abs-RPE tail is EXCLUDED) -- and "
                    "the null is AUTO-RESCALED to that median, so the realised ratio lands ~1.0 "
                    "BY CONSTRUCTION. measured = the realised ratio. An out-of-band ratio => "
                    "substrate_not_ready_requeue (the scale is auto-computed; no fixed knob to "
                    "re-tune)."
                ),
                "control": "median ARM_NOISE wlat_range_max vs median A2/A3 SIGNED learned wlat_range_max",
                ...
                # `LO <= ratio <= HI`. The guard leg (`median_learned_a2a3_wlat_range >
                # 0.0`) needs no separate declaration: when it fails the ratio is set
                # to 0.0, which is below LO=0.25, so the interval already reports unmet.
                ...
                "threshold_low": float(FIELD_NOISE_MAGNITUDE_MATCH_LO),
                "threshold_high": float(FIELD_NOISE_MAGNITUDE_MATCH_HI),
                "comparator_low": ">=",
                "comparator_high": "<=",
                "direction": "interval",
            ...
            {
                "name": "learned_settling_range_non_vacuous",
                "kind": "readiness",
                "description": (
                    "700d near-vacuity guard: the median LEARNED A2/A3 signed-settling W_lat "
                    "range (the auto-scale match TARGET) must clear LEARNED_RANGE_VACUITY_FLOOR. "
                    "If the learned settling arms barely move W_lat, a magnitude-matched null "
                    "tests learned-structure against noise-of-noise (a vacuous match) => "
                    "substrate_not_ready_requeue, NOT a verdict."
                ),
                "control": "median A2/A3 signed-settling wlat_range_max vs vacuity floor",
                # FLOOR-shaped, and STRICTLY so: the source is
                # `median_learned_a2a3_wlat_range > LEARNED_RANGE_VACUITY_FLOOR`, and
                # `measured` is that same median, so `measured > threshold` reproduces
                # `met` exactly. Declared so the recompute mirrors the source rather
                # than taking the inclusive default, which would flip the
                # measured == threshold boundary.
                "measured": float(round(median_learned_a2a3_wlat_range, 8)),
                "threshold": float(LEARNED_RANGE_VACUITY_FLOOR),
                "comparator": ">",
                "direction": "lower",
                "met": bool(learned_range_non_vacuous),
            },
            {
                "name": "same_layer_null_not_destructive",
                "kind": "readiness",
                "description": (
                    "700d explicit guard: the magnitude-matched ARM_NOISE must NOT drive "
                    "committed-class entropy strictly BELOW A0 on a strict-majority of divergent "
                    "seeds (700c's oversized null drove noise 0.859 < A0 0.909). measured = the "
                    "count of divergent seeds whose noise entropy is NOT below A0 (FLOOR on the "
                    "not-below count). A destructive null cannot serve as the verify-lifting "
                    "floor => substrate_not_ready_requeue."
                ),
                "control": "ARM_NOISE committed-class entropy vs A0, divergent seeds, paired (not-below count)",
                # COUNT-shaped and already correct; comparator declared so the recompute
                # mirrors the source (`n_noise_not_below_a0 >= need_not_below`,
                # inclusive) rather than taking the default.
                "measured": float(n_noise_not_below_a0),
                "threshold": float(need_not_below),
                "comparator": ">=",
                "direction": "lower",
                "met": bool(noise_not_destructive),
            },
            {
            ...
            },
            {
                ...
                # COUNT-shaped, and the settling leg is a per-seed CONJUNCTION
                # (`wlat_moved and settling_moved_field`) -- settling_moved_field was
                # absent from the reported statistic entirely, so the old declaration
                # was adjudicating on a strict subset of the shipped predicate.
                ...
                "observed_min_settling_round_delta": float(min(
                    [r["mean_settling_round_delta"] for r in (a2_rows + a3_rows + c3u_rows)] or [0.0]
                )),
                "observed_settling_round_delta_floor": float(SETTLING_ROUND_DELTA_FLOOR),
            ...
            },
            {
                ...
                # seeds divergent on the focus arms -- NOT a bound on the pool-spread
                # statistic. Two reasons the old min-spread declaration could not
                # reproduce it: (1) min(spread) over all rows is strictly HARSHER than
                # "a majority of seeds", so a sound run with 3 of 6 seeds divergent was
                # flagged precondition_unmet; (2) per-seed `gapa_divergence` is a
                # CONJUNCTION (`consumed_spread_mean > CONSUMED_SPREAD_FLOOR and
                # consumed_dist_max < CONSUMED_MAGNITUDE_CEIL`), and a count over a
                # conjunction does not distribute into per-leg counts, so no single
                # spread statistic exists that could carry it. Reported as the same
                # divergent-seed count `enough_divergent` is defined on -- exact by
                # construction. (It therefore duplicates `enough_divergent_seeds`
                # above; that is the shipped predicate, which is unchanged.)
                "measured": float(n_primary_div),
                "threshold": float(MIN_DIVERGENT_SEEDS),
                "comparator": ">=",
                "direction": "lower",
        ...
        "criteria_non_degenerate": {
            ...
            "learned_range_non_vacuous": bool(learned_range_non_vacuous),
            "noise_verified_lifting": bool(noise_verified_lifting),
            "noise_not_destructive": bool(noise_not_destructive),
    ...
    return {
        ...
        "decision_rule_thresholds": {
            ...
            "field_noise_target_ratio": float(FIELD_NOISE_TARGET_RATIO),
            "field_noise_match_mode": "auto_from_a2a3_signed_learned_median",
            "learned_range_vacuity_floor": float(LEARNED_RANGE_VACUITY_FLOOR),
            "field_noise_fallback_scale": float(FIELD_NOISE_FALLBACK_SCALE),
        ...
        "acceptance_criteria": {
            ...
            "learned_range_non_vacuous": learned_range_non_vacuous,
            "noise_not_destructive": noise_not_destructive,
            "n_noise_below_a0": int(n_noise_below_a0),
            "median_noise_wlat_range": round(float(median_noise_wlat_range), 8),
            "median_learned_a2a3_wlat_range": round(float(median_learned_a2a3_wlat_range), 8),
            "noise_match_target_range": round(float(FIELD_NOISE_TARGET_RATIO * median_learned_a2a3_wlat_range), 8),
        ...
        "interpretation_grid": {
            ...
            "FAIL_substrate_not_ready_requeue": (
                "700d VALIDITY RE-TUNE (the ONE letter the brake-LOCK permits). A precondition "
                "is unmet: too FEW divergent seeds (pool collapsed), OR delta_t flat (no outcome "
                "variance), OR the learned weights never moved / the settling never moved the "
                "field, OR the median LEARNED A2/A3 signed W_lat range is BELOW the vacuity floor "
                "(a magnitude-matched null would test learned-structure against noise-of-noise), "
                "OR the auto-magnitude-matched SAME-LAYER null did not VERIFY-LIFT above A0 on a "
                "strict-majority of divergent seeds / DROVE entropy BELOW A0 (destructive null), "
                "OR the realised match ratio fell outside [LO,HI] (the auto-scale failed to land "
                "in band). The conversion question could NOT be measured -- NOT a falsification. "
                "The null scale is now AUTO-COMPUTED from the in-run learned A2/A3 range (no fixed "
                "scale to re-tune); a persistent failure to verify-lift / match magnitude / clear "
                "vacuity at the correct layer ESCALATES to V4 ARC-110 loop-segregation, NEVER "
                "another V3 letter (this TRIPS the brake-LOCK)."
            ),
        },
...
def _build_manifest(
    ...
    return {
        ...
        "terminal_escalation": (
            "V4 ARC-110 loop-segregation substrate (implement-substrate); NO further V3 letters "
            "(a valid-null verify-lift with no learned-settling lift trips the brake-LOCK)"
        ),
        "re_derive_brake": {
            "exempt": True,
            "rationale": (
                "user-adjudicated 2026-06-26 (Reading B): 700c was an invalid-precondition "
                "self-route (magnitude-mismatched 41.42x null), NOT a clean ceiling -- so the "
                "brake did not fire. 700d is a measurement-VALIDITY fix (auto-magnitude-match the "
                "same-layer null + vacuity/below-A0 guards), the same class as 704->704b and "
                "705->705b, and is the ONE validity-fixed letter the brake-LOCK permits."
            ),
            "brake_lock": (
                "the NEXT 700-lineage result with a VALID magnitude-matched same-layer null that "
                "still shows no learned-settling lift FIRES the brake -> V4 ARC-110 loop-"
                "segregation, no more V3 letters"
            ),
            "refuses": "any further same-lever requeue on a valid-null no-lift",
        },
        "same_layer_null": {
            "layer": (
            ...
            "mechanism": (
                "frozen random W_lat with learned_settling_eta=0.0, AUTO-RESCALED so its range "
                "== FIELD_NOISE_TARGET_RATIO x the in-run median LEARNED A2/A3 signed-settling "
                "W_lat range (700d validity re-tune; 700c used a fixed scale 0.5 -> 41.42x "
                "overshoot). Gated behind per-arm field_noise so A0/A2/A3/C3 stay byte-identical "
                "to 700b/700c."
            ),
            "match_mode": "auto_from_a2a3_signed_learned_median",
            "target_ratio": FIELD_NOISE_TARGET_RATIO,
            "realised_match_ratio": result.get("acceptance_criteria", {}).get("field_noise_magnitude_ratio"),
            "realised_target_range": result.get("acceptance_criteria", {}).get("noise_match_target_range"),
            "median_learned_a2a3_wlat_range": result.get("acceptance_criteria", {}).get("median_learned_a2a3_wlat_range"),
            "vacuity_floor": LEARNED_RANGE_VACUITY_FLOOR,
        },
        "reuse_baseline_from": REUSE_BASELINE_FROM,
        "evidence_direction_note": (
            f"V3-EXQ-700d ARC-108 sec-7 LEARNED-GATING CONVERSION FALSIFIER -- VALIDITY RE-TUNE "
            f"of the SAME-LAYER null (experiment_purpose=evidence; claim_ids=[MECH-439, ARC-108, "
            f"MECH-450]; supersedes V3-EXQ-700c). Routed by the user-adjudicated (Reading B) "
            f"failure_autopsy_V3-EXQ-700c_2026-06-26: 700c's same-layer frozen-random W_lat null "
            f"came in 41.42x the realised learned settling range (median noise 2.16 vs learned "
            f"0.052; field_noise_magnitude_matched band [0.25,4.0] FAILED) and drove committed-"
            f"class entropy BELOW A0 (0.859 vs 0.909; noise_verified_lifting 0/3) -- a calibration "
            f"artifact, the V3-EXQ-642 invalid-precondition family (same shape as 704's 176.9x). "
            f"THE ONE CHANGE FROM 700c (config only; no ree_core change): the frozen-random W_lat "
            f"scale is no longer a fixed 0.5 -- it is AUTO-COMPUTED at run time so the injected "
            f"range equals FIELD_NOISE_TARGET_RATIO (1.0) x the in-run median LEARNED W_lat range "
            f"of the A2/A3 SIGNED settling arms ONLY (the C3 UNSIGNED arm's abs-RPE blow-up to "
            f"range 1.9-4.8 is excluded from both the match target and the magnitude-match "
            f"denominator). A near-vacuity guard (median A2/A3 learned range must clear "
            f"LEARNED_RANGE_VACUITY_FLOOR) and an explicit below-A0 destructive-null guard now "
            f"self-route substrate_not_ready_requeue rather than scoring. All 700c design is "
            f"retained (5 arms, settling layer, matched arithmetic envelope, reef-bipartite size "
            f"12, phased P0/P1/P2, committed-action-class entropy DV; A0/A2/A3/C3 byte-identical "
            f"to 700b/700c). RE-DERIVE BRAKE EXEMPT (autopsy: invalid-precondition, not a clean "
            f"ceiling; the ONE validity-fixed letter the brake-LOCK permits, same class as "
            f"704->704b / 705->705b). PRE-REGISTERED TERMINAL for the validity question: a "
            f"magnitude-matched null that VERIFY-LIFTS while learned settling still does NOT lift "
            f"-> escalate V4 ARC-110 loop-segregation (no more V3 letters; TRIPS the brake-LOCK). "
            f"PROMOTES NOTHING (MECH-439 substrate_ceiling; ARC-108/MECH-450 substrate_conditional; "
            f"pending_retest_after_substrate). outcome={result['outcome']}; "
        ...
        "config_summary": {
            ...
            "the_one_change_from_700c": (
                "VALIDITY RE-TUNE: ARM_NOISE's frozen random W_lat is now AUTO-RESCALED so its "
                "range == the in-run median LEARNED A2/A3 signed-settling range (ratio ~1.0, in "
                "band), instead of 700c's fixed scale 0.5 which produced range 2.16 = 41.42x the "
                "learned 0.052 (field_noise_magnitude_matched FAILED + the oversized null drove "
                "entropy BELOW A0). The magnitude-match denominator excludes the C3 unsigned "
                "abs-RPE tail; a near-vacuity guard + an explicit below-A0 guard self-route "
                "substrate_not_ready_requeue. Everything else (5 arms, settling layer, matched "
                "envelope, reef-bipartite, phased P0/P1/P2, committed-class-entropy DV) is "
                "identical to 700c; A0/A2/A3/C3 stay byte-identical to 700b/700c."
            ),
            ...
            "field_noise_match_mode": "auto_from_a2a3_signed_learned_median",
            "field_noise_target_ratio": FIELD_NOISE_TARGET_RATIO,
            "learned_range_vacuity_floor": LEARNED_RANGE_VACUITY_FLOOR,
...
def main() -> Tuple[Optional[str], Optional[str], bool]:
    parser = argparse.ArgumentParser(
        description="V3-EXQ-700d ARC-108 sec-7 learned-gating validity re-tune (auto-matched same-layer null)"
    )
