...
Root cause of EXQ-049b FAIL:
  Same as EXQ-048b: agent.select_action() was correctly used (gate wired), but
  agent.e3.update_after_execution() was never called. Without this, E3's
  _running_variance stays frozen at precision_init=0.5 forever. Since
  ...
  never fires → hold_count=0, propagation_count=0, C1-C4 all FAIL.

Fix:
  At the start of each step, after agent.sense(), call:
    agent.e3.update_after_execution(latent.z_world.detach(), harm_prev < 0)
  This feeds back the actual z_world (outcome of previous action) into E3's
  prediction error EMA, allowing _running_variance to decrease during training.
  Once running_variance drops below commit_threshold=0.40, commitment fires.

  Sequence:
    step t:   encode obs_t → z_world_t
              update_after_execution(z_world_t, harm_{t-1} < 0)  ← fix
              select action_t
              env.step(action_t) → harm_t, obs_{t+1}

Prerequisite: EXQ-042 PASS (HippocampalModule functional).
Prior: EXQ-049 FAIL (bypassed select_action), EXQ-049b FAIL (update_after_execution missing).
...
Motivation:
  ...
  This measures concordance between commitment state and gate state:
    committed + gate elevated = correct hold (MECH-090 prediction)
    uncommitted + not elevated = correct release

  MECH-090 distinction from MECH-057b (EXQ-048c):
    - EXQ-048c: tests completion TIMING (gate fires at completion, not initiation)
    - EXQ-049c: tests CONCORDANCE (committed ↔ elevated, uncommitted ↔ released)
...
EXPERIMENT_TYPE = "v3_exq_049c_mech090_beta_gate_v2"
CLAIM_IDS = ["MECH-090"]
...
def run(
    ...
    **kwargs,
    ...
    print(
        f"[V3-EXQ-049c] MECH-090: Beta-Gated Policy Propagation (running_variance fix)\n"
    ...
    print(f"\nV3-EXQ-049c verdict: {status}  ({criteria_met}/5)", flush=True)
    ...
    summary_markdown = f"""# V3-EXQ-049c -- MECH-090: Beta-Gated Policy Propagation (running_variance fix)

**Status:** {status}
**Claim:** MECH-090 -- beta gate holds E3 policy output during committed action
**Fix:** Adds agent.e3.update_after_execution() each step so _running_variance evolves
**Prior attempts:** EXQ-049 (bypassed select_action), EXQ-049b (update_after_execution missing)
**alpha_world:** {alpha_world}
...
1. **EXQ-049**: `agent.e3.select()` bypassed → beta_gate never exercised
2. **EXQ-049b**: `agent.select_action()` used → gate wired, BUT
   `update_after_execution()` never called → `_running_variance` frozen at 0.5
   → above commit_threshold=0.40 → never commits → gate never elevates
3. **EXQ-049c**: `update_after_execution(z_world_t, harm_{t-1})` called each step
   → running_variance evolves during training → commitment fires
