...
"""V3-EXQ-598b: commitment_closure GAP-1 -- SD-033a bias-head trainable ablation
(post-543l autopsy substrate-enrichment-first routing).

Supersedes V3-EXQ-598a. Routing per failure_autopsy_V3-EXQ-543l_2026-05-27
sections 7 + 9: the V3-EXQ-543l ARC-062 GAP-B falsifier landed FAIL/branch-e
(2026-05-26, all four diff-ON gated arms 3/3 inert despite escalated
mode_separation_floor=0.5 and P1_W_DEVIATION_AUX_WEIGHT=0.3) per autopsy
section 4 -- but the autopsy user-confirmed routing is substrate-enrichment
first, NOT to commission another floor/aux ablation. ARC-062 GAP-C
(discriminator_proj into LateralPFCAnalog source vector) and GAP-D
(rule_bias_head parameters in P1 optimiser) substrate landed 2026-05-17; this
EXQ is the empirical test of whether that GAP-C/D routing consumer rescues
gated-policy head differentiation under the same outcome-coupled REINFORCE
training signal that 543l demonstrated cannot break MECH-309 collapse alone.

The discriminating EXQ between two readings (autopsy section 5):
  (a) Substrate-enrichment reading -- predicts CONTRIBUTORY PASS:
      routing consumer (GAP-C discriminator output -> SD-033a rule_state EMA;
      GAP-D rule_bias_head trainable via E3 gradient) rescues differentiation
      where shared-return REINFORCE alone collapsed in 543l. C2 + C3 both
      clear; MECH-262 rule-selective persistence supported.
  (b) Test-design ceiling reading -- predicts FAIL/weakens: REINFORCE on
      shared return is structurally insufficient regardless of consumer;
      ARC-062 weak-reading dead, ARC-063 V4 distributed CandidateRule field
      required. C3 collapses despite intact substrate.

2-arm validation on the ARC-062 + SD-054 bipartite stack (post-SP-CEM main-path
defaults landed 2026-05-17, differential heads + mode_separation_floor inherited
from the 543k/543l substrate line). Single varied factor:
lateral_pfc_train_rule_bias_head (frozen-zero vs trainable). The frozen-zero
arm is the SD-033a substrate baseline (zero-init head -> mechanically silent
Go-side); the trainable arm exercises GAP-D rule_bias_head parameters in the
P1 optimiser and (via GAP-C in agent.select_action ordering) the discriminator
output -> LateralPFCAnalog source vector routing.

Pre-registered acceptance (commitment_closure_plan Phase 1 / arc_062 Phase 3):
  C1_frozen_silent       : ARM_0 mean |score_bias| after P1 < FROZEN_BIAS_MAX
                           all seeds (SD-033a substrate baseline: zero-init
                           head must remain silent when train_rule_bias_head
                           is False).
  C2_trainable_nonzero   : ARM_1 mean |score_bias| after P1 >= TRAINABLE_BIAS_MIN
                           in >= MIN_PASS_SEEDS of 3 seeds (SD-033a substrate
                           trainable hypothesis: head moves under E3 gradient
                           through compute_bias).
  C3_trainable_not_monomodal: ARM_1 P2 reef_visit_fraction in (REEF_LO, REEF_HI)
                           in >= MIN_PASS_SEEDS seeds (MECH-262 rule-selective
                           persistence proxy: rule_state EMA holds long enough
                           and routes context-discriminably enough for the
                           bias to produce non-trivial reef/forage split).
  PASS = C1 AND C2 AND C3.

Per-claim evidence mapping (multi-claim experiment; evidence_direction_per_claim
mandatory per CLAUDE.md "claim_ids accuracy rule"):
  SD-033a: supports iff C1 AND C2 (substrate-side intactness -- silent baseline
           AND trainable head); else weakens.
  MECH-262: non_contributory if SD-033a substrate fails (prerequisite gap);
            otherwise supports iff C3, weakens iff not C3.

Scientific gate (PERMISSIVE, per autopsy routing): require that V3-EXQ-543l
ran to completion -- any manifest with outcome in {PASS, FAIL} clears the gate.
NOT "contributory PASS" (the stale 598/598a gate semantic from before the 543l
autopsy landed -- 543l ran FAIL/mixed 2026-05-26 and the autopsy explicitly
routes 598b to run anyway as the discriminator between substrate-enrichment
and test-design-ceiling readings). Dry-run + --skip-scientific-gate honour
the same escape hatches as 598a.

claim_ids: SD-033a (primary, substrate-side); MECH-262 (rule-selective
persistence -- behavioural test via C3). SD-034 NOT tagged: closure_operator
is not exercised by this 2-arm ablation (use_closure_operator left at default
False); honestly tagging SD-034 would require closure-pulse / disengagement
metrics that are out of scope for 598b's minimal-supersession-of-598a charter.
...
QUEUE_ID = "V3-EXQ-598b"
SUPERSEDES = "V3-EXQ-598a"
GATES_ON_EXQ = "V3-EXQ-543l"
GATE_PERMISSIVE_OUTCOMES = frozenset({"PASS", "FAIL"})
CLAIM_IDS = ["SD-033a", "MECH-262"]
...
def _require_scientific_gate(gate_exq: str, skip: bool) -> None:
    """Permissive 543l gate: require a non-dry-run manifest with outcome in
    {PASS, FAIL} (i.e., the experiment actually ran without runner ERROR).
    Per failure_autopsy_V3-EXQ-543l_2026-05-27 sections 7 + 9, V3-EXQ-598b is
    the *discriminator* between substrate-enrichment and test-design-ceiling
    readings; it does NOT require contributory PASS on 543l (which ran FAIL/
    mixed 2026-05-26)."""
    if skip:
        print(
    ...
    if not best_label:
        raise RuntimeError(
            ...
            f"Run {gate_exq} to completion (PASS or FAIL) before {QUEUE_ID}."
        )
    if best_outcome not in GATE_PERMISSIVE_OUTCOMES:
        raise RuntimeError(
            f"Scientific gate blocked: {gate_exq} latest={best_label} "
            f"outcome={best_outcome}. Need outcome in {sorted(GATE_PERMISSIVE_OUTCOMES)} "
            f"(ran to completion, not ERROR/dry-run). Per autopsy routing, "
            f"contributory PASS on {gate_exq} is NOT required."
        )
    print(
        f"Scientific gate PASS (permissive): {gate_exq} from {best_label} "
        f"(outcome={best_outcome}, direction={best_direction}). "
        f"Per failure_autopsy_V3-EXQ-543l_2026-05-27 routing, "
        f"{QUEUE_ID} is the substrate-enrichment-vs-test-design-ceiling discriminator.",
        flush=True,
    )
...
def _preflight() -> None:
    ...
    assert frozen.lateral_pfc.config.use_discriminator_source is True
    assert trainable.lateral_pfc.config.use_discriminator_source is True
    del agent, frozen, trainable
    print(
        "Preflight PASS: gated_policy + lateral_pfc + use_discriminator_source "
        "+ train_rule_bias_head flag",
        flush=True,
    )
...
def _per_claim_directions(c1: bool, c2: bool, c3: bool) -> Dict[str, str]:
    """Per-claim evidence mapping per module docstring.

    SD-033a: substrate-side intactness (silent baseline AND trainable head).
    MECH-262: rule-selective persistence (gated by SD-033a substrate intactness).
    """
    sd033a_ok = c1 and c2
    sd033a_dir = "supports" if sd033a_ok else "weakens"
    if not sd033a_ok:
        mech262_dir = "non_contributory"
    elif c3:
        mech262_dir = "supports"
    else:
        mech262_dir = "weakens"
    return {"SD-033a": sd033a_dir, "MECH-262": mech262_dir}


def write_manifest(result: Dict, dry_run: bool, elapsed: float) -> Tuple[Path, str]:
    acc = result["acceptance"]
    outcome = "PASS" if acc["pass"] else "FAIL"
    per_claim = _per_claim_directions(
        bool(acc["C1_frozen_silent"]),
        bool(acc["C2_trainable_nonzero"]),
        bool(acc["C3_trainable_not_monomodal"]),
    )
    if outcome == "PASS":
        direction = "supports"
    elif per_claim["SD-033a"] == "supports":
        direction = "mixed"
    else:
        direction = "weakens"
    ...
    manifest = {
        ...
        "gate_semantic": "permissive_manifest_exists_outcome_pass_or_fail",
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Smoke/dry-run only: do not require V3-EXQ-543l manifest.",
    )
    ...
    print("\n=== V3-EXQ-598b SUMMARY ===", flush=True)
    ...
    print(f"  per_claim_direction: SD-033a={per_claim['SD-033a']} MECH-262={per_claim['MECH-262']}", flush=True)
    print(f"  gates_on_exq: {GATES_ON_EXQ} (permissive: manifest exists, ran to PASS/FAIL)", flush=True)
