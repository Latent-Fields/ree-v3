...
"""
V3-EXQ-614b -- GAP-B Phase P3 behavioural falsifier: B_only / ablate_B / ALL_ON,
re-run on the SD-056-amended substrate.
...
def run_experiment(
    ...
    return {
        ...
        "interpretation_grid_header_note_614b": (
            "V3-EXQ-614b context: 614a already landed PASS via C2+C3 only "
            "(MECH-341 supports + load-bearing in stack only). On the "
            "SD-056-amended substrate (multi-step contrastive h=5 + "
            "per-step output norm clamp ratio=2.0), PASS via C1 (R2.c "
            "MECH-341 isolation) is now the load-bearing target -- a fresh "
            "C1 fire would clear the R2.c isolation reading 614a deferred. "
            "If 614b lands PASS via C2+C3 only again, the result is read as "
            "substrate-stable-but-isolation-still-not-clearing under the "
            "same routing as 614a (Q-054 entropy_bias_scale sweep + "
            "stratified_temperature revisit, NOT a fresh /diagnose-errors "
            "loop)."
        ),
        "interpretation_grid": {
            "PASS_C1_C2_C3": (
