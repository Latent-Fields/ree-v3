...
"""
V3-EXQ-085c -- MECH-071 / INV-034 Goal-Directed Calibration (Curiosity + SD-010)

Claims: MECH-071, INV-034

Root-cause analysis of EXQ-085 / EXQ-085b FAIL (2026-03-23):
  delta_n_agent_events = 0.0 exactly -- both conditions ran identically.
  Root cause: CIRCULAR SEEDING. z_goal starts at zero (GoalState._z_goal=0).
  It only updates when benefit_exposure > 0.1. But the agent never approaches
  resources (no benefit exposure) because z_goal is zero (no goal to approach).
  GOAL_PRESENT and GOAL_ABSENT were functionally identical -- z_goal stayed at
  zero throughout.

  Second flaw: the pass criterion (n_agent_events > baseline * 1.5) tested
  whether z_goal increases PER-STEP harm event count. But z_goal is PARALLEL
  to the harm pathway (it biases TRAJECTORY SELECTION, not per-step events).
  The right metric is trajectory quality and benefit acquisition, not harm counts.

Fixes in EXQ-085c:
  1. MECH-111 curiosity: novelty_bonus_weight=0.1 enabled. The agent explores
     via novelty even before z_goal is seeded. Encounters benefit -> seeds z_goal.
     Breaks the circular dependency.
  2. SD-010 wired: use_proxy_fields=True -> env emits harm_obs. harm_obs passed
     to agent.sense() as obs_harm. This is the first experiment to fully wire the
     ARC-027 parallel harm stream.
  3. Metric redesign: measure BENEFIT ACQUISITION RATE (not harm event count).
     With curiosity, both conditions explore. With z_goal seeded, GOAL_PRESENT
     should acquire MORE benefit than GOAL_ABSENT (goal attractor biases trajectory
     selection toward resource regions). This tests INV-034 directly.
  4. Extended warmup: 500 eps for curiosity-driven exploration to seed z_goal.
     Eval phase: 100 eps measuring benefit_acquired per episode.

Mechanism under test:
  GOAL_PRESENT: curiosity active + z_goal_enabled=True
    -> agent explores (MECH-111) -> encounters benefit -> z_goal seeds
    -> trajectory scoring biased toward goal region -> more benefit acquired
...
PASS criteria (ALL required):
  C1: z_goal_norm_goal_present > 0.1
      (z_goal was successfully seeded during exploration warmup)
...
EXPERIMENT_TYPE = "v3_exq_085c_mech071_curiosity_goal"
...
def _run_single(
    ...
    for ep in range(warmup_episodes):
        ...
        # Track z_goal norm at end of episode
        if goal_present:
...
def run(
    ...
    **kwargs,
    ...
    """GOAL_PRESENT vs GOAL_ABSENT with curiosity (MECH-111) + SD-010."""
    ...
    print(f"\n[V3-EXQ-085c] Final results:", flush=True)
    print(
    ...
    if not c1_pass:
        failure_notes.append(
            ...
            " (goal never seeded -- curiosity insufficient to encounter benefit)"
        )
    if not c2_pass:
        failure_notes.append(
    ...
    summary_markdown = (
        f"# V3-EXQ-085c -- MECH-071 / INV-034 Curiosity + Goal Calibration\n\n"
        f"**Status:** {status}\n"
        ...
        f"**Root cause fix:** Circular seeding broken via MECH-111 curiosity."
        f" Metric redesigned: benefit acquisition rate (not harm event count).\n\n"
        f"## Pre-Registered Thresholds\n\n"
        f"C1: z_goal_norm > 0.1 (goal seeded via exploration)\n"
