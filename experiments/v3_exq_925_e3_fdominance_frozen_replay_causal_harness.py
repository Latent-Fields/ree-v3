"""V3-EXQ-925: E3 F-dominance frozen-state causal-replay harness (H1/H2/H3/H4).
...
themselves -- those stay untouched by this run's evidence_direction).

WHY THIS RUN EXISTS (session mech357-pressure-scoping-11e9c9, F-dominance causal
localisation, 2026-08-12). User-requested causal discrimination among:
  H1 literal-F dominance      -- literal F itself has candidate-specific causal
                                  authority sufficient to suppress useful competitors.
  H2 primary-field dominance  -- not literal F specifically; the broader pre-
                                  modulatory/eligibility pathway retains privileged
                                  authority even when literal F alone is attenuated.
  H3 upstream insufficiency   -- apparent dominance is mainly downstream of
                                  inadequate candidate generation/representation;
                                  competing channels don't yet carry useful
                                  candidate-specific evidence.
  H4 conditional specialisation -- F controls where informative, competitors can
                                  gain control where THEIR information is useful;
                                  this is substantially appropriate, not a defect.

PRIOR STEPS THIS RUN DEPENDS ON (all this session, 2026-08-12):
  1. V3-EXQ-924 (ree-v3 d7856aed8a): confirmed literal F still dominates E3
     committed-selection temporal variance on the CURRENT (post-SD-E3-SCORER-
     COMPLETION) substrate -- 96.0-96.1% fixed vs 88.2-89.5% legacy (the historical
     V3-EXQ-571 figure), i.e. the scorer-noise-contamination hypothesis for the
     88-89% figure is REFUTED; F-dominance is real, not a measurement artifact.
  2. A direct empirical probe (this session, not yet an EXQ) found NO E3 modulatory
     channel produces genuine per-candidate signal on the current substrate,
     including gated_policy (ARC-062's own "rule apprehension" channel) even with
     use_gated_policy=True enabled: std_across_K=0.0 (uniform broadcast), because
     E2's world-forward model collapses candidates differing only in first action
     to near-identical z_world (the same root cause V3-EXQ-571's own 2026-05-25
     postmortem found: cand_world_pairwise_dist ~ 0.0000). This is a direct hit on
     the "candidate support/differentiation is inadequate" stop condition -- user
     confirmed: train SD-056 (E2 world-forward action-conditional divergence, an
     auxiliary online contrastive loss, already built but requires an explicit
     per-experiment training loop -- REEConfig.e2_action_contrastive_enabled is
     confirmed DEAD CODE, never read by any ree_core/ training path per the
     2026-08-11 nested-flag-inertness audit) as a P0 precursor before building the
     causal-replay harness, using the validated V3-EXQ-643a recipe (SD-056 online
     contrastive + e2_rollout_output_norm_clamp for numerical stability, since
     V3-EXQ-569e found SD-056 training destabilises z_world to ~1e16-1e18 without
     the clamp).

DESIGN CHOICES AND WHAT IS DEFERRED (stated explicitly, not silently omitted):
  - Competitor channel: gated_policy only (ARC-062's rule-apprehension mechanism),
    read via last_channel_terms (requires use_finer_channel_gating=True -- the
    DEFAULT last_channel_terms only carries the COMBINED score_bias sum, not
    per-channel breakdown; verified against e3_selector.py _FCG_CHANNEL_INDEX vs
    _LCG_CHANNEL_INDEX before writing this script). Other channels (ofc,
    lateral_pfc, dacc, mech295_liking, curiosity, tonic_vigor) are NOT exercised
    here -- ofc/lateral_pfc need their own head training (zeroed at init, separate
    from SD-056), dacc's input is documented as informationally downstream of F
    itself (not independent), curiosity/tonic_vigor are structurally broadcast
    unless fed a genuine per-candidate source. A full multi-channel portfolio is
    future work, not attempted here.
  - Independent criterion: ENV-NATIVE ground-truth safety (Euclidean distance from
    the candidate's resulting position, after its first action, to the nearest
    known hazard in env.hazards). This is genuinely independent of F/harm/gated_
    policy (computed from env.agent_x/agent_y + env._action_map + env.hazards
    directly, never touching any network output) and matches the user's own
    "safety/harm" example axis. "Value/OFC", "goal evidence" and "contextual rule
    evidence" axes are not attempted -- ofc/goal need active training/goal state
    this harness does not set up.
  - Counterfactuals implemented: C0 (factual replay self-consistency check), C1
    (literal-F lesion, f_weight=0), C2 (literal-F association scramble -- fixed
    per-event permutation of F across candidate indices, preserving F's marginal
    distribution), C3 (attenuation ladder f_weight in {1.0,0.5,0.25,0.0}, matching
    V3-EXQ-858 for comparability), C4 (primary-field/eligibility toggle -- MECH-448
    _f_eligibility_envelope applied vs bypassed, at factual f_weight), C5
    (competitor lesion -- gated_policy's term zeroed, at both factual and
    F-lesioned f_weight, to establish whether it had ANY causal influence in
    either condition). MECH-449 Go/No-Go is NOT exercised (use_go_nogo_
    constitution stays at its default; the go_nogo_signals this harness would
    need to capture -- specifically agent._dacc_last_bundle["suppression"] -- are
    not populated in the default waking loop per the technical grounding this
    script was designed from).
  - Unit of analysis: a GENUINE FRESH E3 selection event only. Uses the same
    clear-the-latch-before-the-call pattern V3-EXQ-924 needed (E3 fires only every
    heartbeat.e3_steps_per_tick env steps; agent.e3.last_score_decomp/last_channel_
    terms otherwise hold the PREVIOUS fresh selection's values verbatim). Captured
    once per fresh tick, never per env step.

DESIGN EVOLUTION DURING AUTHORING (2026-08-12) -- three defects caught by the
harness's own checks on SMOKE runs, before any multi-hour real run. Recorded here
because each was invisible to the lints and would have produced a confident,
wrong answer:
  1. The readiness gate hand-rolled a pairwise distance over the PROPOSER's
     candidate summaries instead of calling `e2.cand_world_pairwise_dist()`, the
     substrate's own SD-056 API. It read 4.5e-5 where the correct statistic reads
     0.0141 UNTRAINED -- a different quantity, not a weaker one -- and three
     separate "SD-056 does not converge" conclusions were drawn from it before the
     API was found. With the correct statistic, SD-056 trains cleanly:
     0.0165 -> 0.1531 over 60 P0 episodes (the ~0.1147 figure ree-v3/CLAUDE.md
     documents for SD-056-trained world_forward is independent corroboration).
  2. `candidate_summary_source` was left at its "proposer" default, which pins the
     competitor channel's per-candidate bias to EXACTLY 0.0 regardless of SD-056 --
     see the flag comment in `_make_agent`. That alone would have made the whole
     H1/H2/H3/H4 discrimination vacuous and read as "H3 confirmed".
  3. The counterfactual replay rebuilt the score from a component subset, which
     reproduced the live selection ~2.5% of the time (chance at K=32). It now
     perturbs the REAL captured score vector; reconstruction of the factual
     distribution is exact to ~6e-9 against the selector's own recorded probs.

THE DV FRAMING THAT CAME OUT OF (3), AND WHY IT MATTERS SCIENTIFICALLY. Fixing the
replay exposed that this substrate runs the selector UNCOMMITTED: `committed` is
False on every observed tick, so the factual choice is a SAMPLE from
softmax(-scores/T), not an argmin. "Did the argmin flip?" is therefore ill-posed
against the factual draw. The primary DVs are consequently DISTRIBUTIONAL -- change
in selection probability on the independently-preferred candidate, change in its
rank, change in expected ground-truth hazard distance, and total variation -- which
are also three of the four DVs the requesting brief itself named. Argmin flips are
still recorded, as the deterministic-selector counterpart.

Two SCALE readouts are recorded for the same reason, and they are the ones to read
first: `selection_entropy_normalised` (1.0 = uniform, i.e. nothing is steering the
choice at all) and `f_contribution_range` vs `gated_policy_contribution_range`.
A variance decomposition (V3-EXQ-924: F = ~96% of E3 score TEMPORAL variance) says
how much a term moves ACROSS TIME; only CROSS-CANDIDATE spread within a decision can
move a choice. Those can diverge sharply, and if they do, "F-dominance" grounded in
variance-share alone does not license a claim about selection authority. This run
measures both rather than assuming they agree.

WHAT THIS RUN DOES NOT DO: no new substrate mechanism is built (only a diagnostic
harness over EXISTING mechanisms: SD-056, E3Config.f_weight/SD-085, MECH-448's
_f_eligibility_envelope). No developmental contingency-switch experiment (deferred
per the user's brief, pending this run's own findings). Per the user's brief: "Do
not proceed automatically to a fix" and "do not optimise REE toward looking
balanced" -- this reports the pattern found, not a target.
"""
...
EXPERIMENT_TYPE = "v3_exq_925_e3_fdominance_frozen_replay_causal_harness"
SUPERSEDES = None
...
def _make_agent(env: CausalGridWorld, obs_dict: Dict[str, Any]) -> REEAgent:
...
def run_p0_warmup(agent: REEAgent, env: CausalGridWorld, obs_dict, seed: int,
                   n_episodes: int, steps_per_episode: int) -> Tuple[Dict, Any]:
...
def run_p1_capture_and_replay(agent: REEAgent, env: CausalGridWorld, obs_dict, seed: int,
                               n_episodes: int, steps_per_episode: int) -> List[Dict[str, Any]]:
...
def run_experiment(dry_run: bool = False, started_at: Optional[float] = None) -> Dict[str, Any]:
    ...
    for seed in SEEDS:
        ...
        np.random.seed(seed)
        config_slice = {
            "seed": seed, "p0_episodes": p0_eps, "p1_episodes": p1_eps,
        ...
        with arm_cell(seed, config_slice=config_slice, script_path=Path(__file__)) as cell:
            ...
            _, obs_dict = env.reset()
            agent = _make_agent(env, obs_dict)

            print(f"== seed {seed}: P0 SD-056 warmup ({p0_eps} episodes) ==", flush=True)
            _, obs_dict = run_p0_warmup(agent, env, obs_dict, seed, p0_eps, steps)
            print(f"    [decomp] seed={seed} ep {p0_eps}/{p0_eps} P0 warmup complete", flush=True)

            readiness, obs_dict = measure_readiness(agent, env, obs_dict, n_probe_ticks=15 if not dry_run else 5)
            per_seed_readiness[str(seed)] = readiness
            print(f"    readiness: cand_world_pairwise_dist={readiness['cand_world_pairwise_dist_mean']:.4f} "
                  f"gated_policy_std_across_K={readiness['gated_policy_std_across_K_mean']:.6f}", flush=True)

            try:
                p0_readiness_gate([
                    {"name": "cand_world_pairwise_dist", "measured": readiness["cand_world_pairwise_dist_mean"],
                     "threshold": CAND_WORLD_PAIRWISE_DIST_FLOOR, "direction": "lower"},
                    {"name": "gated_policy_std_across_K", "measured": readiness["gated_policy_std_across_K_mean"],
                     "threshold": GATED_POLICY_STD_ACROSS_K_FLOOR, "direction": "lower"},
                ])
            except P0NotReady as e:
                # SMOKE-TEST REACHABILITY: a --dry-run uses DRY_RUN_P0 (3) episodes,
                # which cannot train SD-056 to the readiness floor (the real run uses
                # 60, verified to clear: cand_world_pairwise_dist 0.0165 -> 0.1531).
                # Aborting the seed here on a dry run would leave the entire P1
                # capture + C0-C5 replay path NEVER SMOKE-TESTED, so a crash in the
                # replay arithmetic would only surface on the real multi-hour run.
                # On a dry run we therefore record the (expected) readiness miss and
                # CONTINUE, purely to exercise the code path. This can never leak
                # into a real result: the real run does not take this branch, and
                # `n_ready_seeds` -- computed independently from per_seed_readiness
                # against the same floors -- still governs the recorded outcome.
                if not dry_run:
                    cell.stamp({"seed": seed, "readiness_failed": True})
                    print(f"    seed {seed}: substrate_not_ready_requeue -- {e}", flush=True)
                    print("verdict: FAIL", flush=True)
                    continue
                print(f"    seed {seed}: DRY RUN -- readiness unmet as expected "
                      f"({e}); continuing to exercise the P1 replay path", flush=True)

            print(f"== seed {seed}: P1 capture + inline replay ({p1_eps} episodes) ==", flush=True)
            events = run_p1_capture_and_replay(agent, env, obs_dict, seed, p1_eps, steps)
            for e in events:
                e["seed"] = seed
            ...
            cell.stamp({"seed": seed, "n_events": len(events), "readiness": readiness})
            cell_fingerprints.append(cell.fingerprint if hasattr(cell, "fingerprint") else None)
    ...
    # Non-degeneracy self-report. The load-bearing readout is the disagreement-
    # conditioned delta_p_safer under the F-lesion. Degenerate if it is structurally
    # pinned -- most importantly if C1 moves the distribution by EXACTLY zero on every
    # event, which would mean F contributed nothing to the score vector and the
    # "lesion" was a no-op (a precondition failure that must not read as a null result).
    if agg.get("n_disagreement_events", 0) > 0:
        dis_events = [e for e in all_events if e["disagreement_event"]]
        degeneracy = check_degeneracy({
            "c1_delta_p_safer_across_disagreement_events": {
                "values": [float(e.get("c1_delta_p_safer") or 0.0) for e in dis_events]
            },
            "c1_total_variation_across_events": {
                "values": [float(e.get("c1_total_variation_vs_factual") or 0.0) for e in all_events]
            },
        })
    else:
        degeneracy = {"non_degenerate": False, "degeneracy_reason": "zero disagreement events", "degenerate_metrics": {}}

    n_ready_seeds = sum(1 for s in SEEDS if str(s) in per_seed_readiness
                        and per_seed_readiness[str(s)]["cand_world_pairwise_dist_mean"] >= CAND_WORLD_PAIRWISE_DIST_FLOOR
                        and per_seed_readiness[str(s)]["gated_policy_std_across_K_mean"] >= GATED_POLICY_STD_ACROSS_K_FLOOR)

    if n_ready_seeds == 0:
        outcome = "FAIL"
        outcome_note = (
            "No seed cleared the SD-056/gated_policy readiness gate -- substrate_"
            "not_ready_requeue across all seeds. See per_seed_readiness."
        )
    elif len(all_events) == 0:
        outcome = "FAIL"
        outcome_note = "Readiness cleared but zero fresh E3 selection events were captured -- instrumentation defect, investigate before re-running."
    else:
        outcome = "PASS"
        dc = agg.get("disagreement_conditioned") or {}
        _dps = dc.get("c1_delta_p_safer_mean")
        outcome_note = (
            f"{n_ready_seeds}/{len(SEEDS)} seeds ready; {agg['n_events']} genuine fresh "
            f"E3 selection events captured, {agg['n_disagreement_events']} disagreement "
            f"events ({agg['disagreement_rate']:.1%}); committed_fraction "
            f"{agg['committed_fraction']:.3f}. "
            f"Replay validity (reconstructed vs selector's own distribution, worst abs err): "
            f"{agg['c0_probs_reconstruction_max_abs_err_worst']:.2e}. "
            f"PRIMARY: C1 (literal-F lesion) mean delta_p_safer on disagreement events = "
            f"{_dps:+.5f}" if _dps is not None else "PRIMARY: C1 delta_p_safer unavailable"
        )
        outcome_note += (
            f"; C1 mean total variation {agg['c1_total_variation_mean']:.4g}; "
            f"competitor influence at factual F {agg['competitor_had_influence_factual_f_rate']:.3f}, "
            f"at F-lesioned {agg['competitor_had_influence_lesioned_f_rate']:.3f}. "
            f"See hypothesis_evaluation for the H1-H4 pattern read (READ THE SELECTOR "
            f"REGIME NOTE FIRST)."
        )
    ...
    return {
        ...
        "per_seed_event_counts": {str(s): sum(1 for e in all_events if e["seed"] == s) for s in SEEDS},
    }


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="V3-EXQ-925: E3 F-dominance frozen-replay causal harness")
    ...
    manifest = {
        ...
        "n_ready_seeds": result["n_ready_seeds"],
        ...
        "methodology_note": (
            "Frozen-state causal-replay diagnostic. P0 trains SD-056 (E2 world-forward "
            "action-conditional divergence, verbatim V3-EXQ-643a recipe) so candidates "
            "genuinely differentiate in z_world space -- verified empirically this "
            "session that without it, gated_policy (ARC-062's rule-apprehension "
            "channel) reads std_across_K=0.0 even when enabled, an upstream-"
            "representation problem, not a selector-authority problem. P1 captures "
            "genuine fresh E3 selections only (latch-cleared, per V3-EXQ-924's fix) "
            "and replays C0-C5 counterfactuals INLINE on the captured per-candidate "
            "f/harm/gated_policy vectors -- pure arithmetic, no re-touching the live "
            "agent/env. Independent criterion: env-native ground-truth distance to the "
            "nearest known hazard after the candidate's first action (never a network "
            "output). Deferred: ofc/lateral_pfc/dacc/curiosity/tonic_vigor as "
            "competitors (see module docstring for why); MECH-449 Go/No-Go; a "
            "developmental contingency-switch follow-on (pending this run's findings, "
            "per the user's brief -- not built automatically)."
        ),
    }

    config_for_recording = {
        "seeds": SEEDS, "p0_warmup_episodes": P0_WARMUP_EPISODES,
        "p1_capture_episodes": P1_CAPTURE_EPISODES, "steps_per_episode": STEPS_PER_EPISODE,
