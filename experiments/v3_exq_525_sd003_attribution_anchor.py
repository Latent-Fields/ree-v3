"""
V3-EXQ-525 -- SD-003 / ARC-033: Harm Attribution Anchor (current substrate reconfirmation)

Claims: SD-003, ARC-033
Supersedes: V3-EXQ-205

Purpose:
  Reconfirmation of the SD-003 z_harm_s counterfactual pipeline on the current
  substrate (post SD-019/020/021 harm stream extensions 2026-04-10).
  V3-EXQ-205 (2026-04-02) returned status="PASS" but outcome=None, making it
  invisible to governance confidence scoring. This run fixes the output format:
    - outcome field ("PASS"/"FAIL") for governance indexer
    - timestamp_utc in datetime format (not Unix int) for sync_v3_results.py
    - verdict: PASS/FAIL per seed for runner progress display
    - [train] ep N/M progress prints for runner ETA

SD-003 pipeline (unchanged from EXQ-205):
...
PRE-REGISTERED ACCEPTANCE CRITERIA (ALL required for PASS):
  C1 (attribution_gap > 0.002):
    ...
    Agent approach has higher attribution than baseline locomotion.
...
EXPERIMENT_TYPE = "v3_exq_525_sd003_attribution_anchor"
...
# Pre-registered thresholds (unchanged from EXQ-205)
...
def _phase0_train_encoder(
    ...
    """P0: warm up HarmEncoder. total_train_eps is the full P0+P1 count for progress prints."""
    device = agent.device
    ...
    for ep in range(num_episodes):
        ...
        if (ep + 1) % 50 == 0 or ep == num_episodes - 1:
            print(
                f"  [train] ep {ep+1}/{total_train_eps}"
                f"  [P0 encoder] ae_loss={ae_loss.item():.5f}",
                flush=True,
            )
...
def _phase1_train_heads(
    ...
    for ep in range(num_episodes):
        ...
        if (ep + 1) % 50 == 0 or ep == num_episodes - 1:
            print(
                f"  [train] ep {p0_episodes + ep + 1}/{total_train_eps}  [P1 heads]",
                flush=True,
            )
    ...
    for _ in range(num_episodes):
        ...
        for _ in range(steps_per_episode):
            ...
            with torch.no_grad():
                ...
                z_null_cf = harm_fwd(z_harm_s, action)
...
def run_seed(
    ...
    # Runner seed boundary line
    print(f"Seed {seed} Condition default", flush=True)

    torch.manual_seed(seed)
    random.seed(seed)
    np.random.seed(seed)

    total_train_eps = p0_episodes + p1_episodes  # denominator for [train] ep N/M
    ...
    print(f"\n[seed {seed}] P0: train HarmEncoder ({p0_episodes} eps)", flush=True)
    _phase0_train_encoder(
        agent, env, harm_enc, p0_episodes, steps_per_episode, total_train_eps,
    )

    print(f"\n[seed {seed}] P1: freeze encoder, train heads ({p1_episodes} eps)", flush=True)
    p1_out = _phase1_train_heads(
        agent, env, harm_enc, harm_fwd, p1_episodes, steps_per_episode,
        p0_episodes, total_train_eps,
    )
    ...
    passed  = all([c1_pass, c2_pass, c3_sig, c4_pass, c5_pass, c6_discrim])
    ...
    print(f"  seed {seed} verdict: {outcome}  ({n_met}/6)", flush=True)
    for note in failure_notes:
        print(f"    {note}", flush=True)

    # Runner verdict line (increments runs_done in the runner)
...
def run(
    ...
    **kwargs,
    ...
    print(
        f"[V3-EXQ-525] SD-003 Harm Attribution Anchor (supersedes V3-EXQ-205)\n"
    ...
    print(f"\n{'='*60}", flush=True)
    print(f"V3-EXQ-525 OVERALL: {outcome}  ({criteria_met}/6)", flush=True)
    ...
    summary_markdown = f"""# V3-EXQ-525 -- SD-003 Harm Attribution Anchor
...
Criteria met: {criteria_met}/6 -> **{outcome}**
{failure_section}
"""
...
if __name__ == "__main__":
    ...
    if args.dry_run:
        print("[dry-run] Using 1 seed, 5 eps P0, 5 eps P1, 5 eps eval, 10 steps", flush=True)
        args.seeds    = [42]
        args.p0_eps   = 5
        args.p1_eps   = 5
        args.eval_eps = 5
