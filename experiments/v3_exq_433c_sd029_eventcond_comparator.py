...
"""
V3-EXQ-433c -- SD-029 Comparator Attenuation: Event-Balanced Training + Eval.

Claims: SD-029, MECH-256
Supersedes: V3-EXQ-433b (agent_caused_hazard r2=0.0 across all 4 seeds).

Root cause of EXQ-433b (diagnose-errors 2026-04-22):
  Reported: agent_caused_hazard r2=0.0 in all 4 seeds; c2_sd029_attenuation_ratio
  null; overall c1 r2 dropped to 0.58-0.68.
  Actual: trials_collected["agent_caused_hazard"] == 0 in all 4 seeds. The
  r2=0.0 is the empty-list fallback in _safe_r2_from_pairs. SD-013 interventional
  training IS correctly wired (use_interventional=True, fraction=0.7, margin=0.2).
  The forward model's action->harm channel was simply never exercised at eval
  because the env+policy combination produces ~0 agent-caused events organically
  and the eval loop had no curriculum / scripted event-eliciting mechanism.
  The MECH-256 comparator question ("can forward-model attenuate self-caused
  residuals?") is UNTESTED, not falsified.

Fix (two axes, both required):
  (A) Enable SD-029 curriculum (scheduled_external_hazard) during BOTH P0
      training and P1 interventional training AND eval so env_caused events
      populate reliably across seeds.
  (B) Run scripted agent-caused event elicitation during eval: repeatedly reset,
      then deterministically walk the agent toward an adjacent hazard for
      several steps per trial until n_agent_caused >= TARGET_TRIALS_PER_TYPE
      per seed, or we hit EVAL_MAX_EPS.
  (C) Emit manifest flags agent_caused_trials_sufficient and
      env_caused_trials_sufficient so downstream evidence assessment can
      distinguish "r2=0 from empty pairs" from "r2=0 from model failure".

Adds diagnostic metric C2b: per-event-type forward_r2 (from EXQ-433b).
Adds diagnostic metric C2c: per-event-type causal_sig (from EXQ-433b).
...
PASS criteria:
  C0 (gate): agent_caused_trials_sufficient AND env_caused_trials_sufficient
             in >=3/4 seeds (must have enough data to evaluate at all).
...
If C0 fails, the run is FAIL but the evidence direction flips to
"inconclusive_insufficient_events" on SD-029 and MECH-256 -- signalling that
the governance pipeline should NOT downgrade these claims' confidence from
this run (the experiment did not produce interpretable data).
...
EXPERIMENT_TYPE = "v3_exq_433c_sd029_eventcond_comparator"
CLAIM_IDS = ["SD-029", "MECH-256"]
EXPERIMENT_PURPOSE = "evidence"
...
# Curriculum (SD-029) enabled throughout
CURRICULUM_INTERVAL = 25
CURRICULUM_PROB = 1.0
CURRICULUM_ADJACENT_ONLY = False  # False so injection falls back to any empty cell -- more reliable
...
def _make_env(seed: int) -> CausalGridWorldV2:
    return CausalGridWorldV2(
        ...
        harm_history_len=10,
        # SD-029 curriculum ON throughout training and eval
...
def _run_seed(seed: int, verbose: bool = True) -> Dict:
    ...
    print(f"  P0 ({P0_EPS} eps) encoder warmup [curriculum ON]...", flush=True)
    ...
    print(f"  P1 ({P1_EPS} eps) interventional [curriculum ON]...", flush=True)
    prev_z_harm = None
    ...
    # events from the curriculum. Each episode: walk toward any adjacent hazard
    # when one is present (elicits agent_caused event); otherwise random policy
    # (allows organic sampling of hazard_approach / none and curriculum-injected
    # env_caused events).
    print("  Balanced eval [curriculum ON + scripted agent-caused]...", flush=True)
    ...
    # Only include types with sufficient trials in mean_r2
...
def main():
    ...
    # C0 gate: trials sufficient in >=3/4 seeds for BOTH agent_caused and env_caused
    c0_wins = sum(
    ...
    # C1: forward_r2_mean >= 0.9 in >=3/4 seeds (only meaningful when C0 holds)
    c1_wins = sum(1 for r in all_results if safe(r["forward_r2_mean"]) >= 0.9)
    c1 = c1_wins >= 3

    # C2: attenuation_ratio in [0.2, 0.8] in >=3/4 seeds
    c2_wins = sum(
        1 for r in all_results
    ...
    # Evidence-direction logic: if C0 fails, the experiment did not produce
    # interpretable data; signal inconclusive rather than weakens so the
    # governance pipeline does not downgrade SD-029/MECH-256 from this run.
    if not c0:
