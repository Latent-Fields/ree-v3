...
Proposal: validates SD-012 fix as causal
...
SD-012 context:
  ...
  seeding to work (confirmed by EXQ-189: z_goal_norm = 0.30).
...
# Metadata
# ---------------------------------------------------------------------------
EXPERIMENT_TYPE = "v3_exq_233_sd012_zgoal_seeding_validation"
...
def run(dry_run: bool = False) -> dict:
    print(f"\n[EXQ-233] SD-012 z_goal Seeding Causal Validation (dry_run={dry_run})",
          flush=True)
    ...
    summary_markdown = (
        f"# V3-EXQ-233 -- SD-012 z_goal Seeding Causal Validation\n\n"
        f"**Status:** {status}  **Criteria met:** {crit_met}/3\n"
        f"**Claims:** SD-012, MECH-112  **Purpose:** evidence\n\n"
        f"## Context\n\n"
        f"SD-012 implemented drive_weight=2.0 (GoalConfig default). EXQ-189"
        f" showed z_goal_norm=0.30 confirming the fix works. This experiment"
        f" provides clean ablation confirming drive_weight is causal.\n\n"
        f"## Conditions\n\n"
