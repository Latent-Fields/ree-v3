...
"""
V3-EXQ-445c -- SD-032b dACC: Larger Multi-Resource Environment.

Claims: SD-032b, MECH-258, MECH-260

Follows up V3-EXQ-445 (FAIL: monostrategy collapse) with a richer environment.
Hypothesis: the 10x10 grid with 3 hazards / 4 resources is too sparse -- the
agent learns to ignore hazards because the optimal strategy (move to nearest
resource) is always the same action. A larger environment with multiple resource
clusters and hazard corridors creates genuine decision heterogeneity, requiring
the agent to choose between competing navigation paths.

Uses epsilon-greedy (eps=0.1) during training (same as 445b) to prevent lock-in.
Also adds full P0 pipeline training (E3 harm_eval, same as 445a) to ensure E3
can evaluate the richer environment.

Environment: size=14, num_hazards=5, num_resources=7. Resources distributed
to require navigation through different corridors. Hazard proximity scale
increased to 0.20 (more penalty for being near hazards).

Score_bias scale: dacc_precision_scale=5000.0, dacc_weight=0.5 (same as 445a/b).

2-arm design (OFF vs ON_INDEPENDENT), 3 seeds.

Phased training:
  P0 (150 eps): Full pipeline (E1, E3 harm_eval, E2 world-forward) + epsilon=0.1
...
EXPERIMENT_TYPE = "v3_exq_445c_sd032b_dacc_larger_env"
CLAIM_IDS = ["SD-032b", "MECH-258", "MECH-260"]
EXPERIMENT_PURPOSE = "evidence"
