"""
V3-EXQ-811a -- MECH-477 dual-system uncertainty-arbitration falsifier (SD-081 OFF vs ON).

Claims: MECH-477 (primary), MECH-163 leg (1) (secondary)
Supersedes: V3-EXQ-811 (v3_exq_811_mech477_dualsystem_arbitration_falsifier)

===========================================================================
WHY THIS LETTER EXISTS -- REPAIRED INSTRUMENT, SAME QUESTION
===========================================================================
V3-EXQ-811's own manifest self-routed FAIL / substrate_not_ready_requeue on all three
non-vacuity readiness gates (full_score_range_non_degenerate, habit_score_range_non_
degenerate, habit_score_distinct_fraction), even though its load-bearing C1 criterion
(C1_recruitment_delta_greater_with_arbitrator) passed cleanly (Cohen's d 0.999 ON vs -0.158
OFF, 6/7 divergent seeds ON-greater) and every arbitration-liveness/coupling check read at
or near 100%.

REE_assembly/evidence/planning/failure_autopsy_V3-EXQ-811_2026-07-24.{md,json} (confirmed,
user-cleared 2026-07-24) traced this to an aggregation bug in the canonical lineage module
`experiments/_lib/baselines/mech477_dualsystem_arbitration.py`: 20/128 cells (15.6%) scored
zero E3 ticks (shorter episodes on unpracticed layouts), and the empty-cell default for the
three range/distinctness fields was `0.0` -- indistinguishable from a genuine degenerate
reading of exactly zero -- instead of `None`, which is what the adjacent `recruitment_rate`
field already correctly defaults to for an empty cell (filtered out downstream). The
worst-case-across-cells aggregation (`_worst()`, deliberately a true min so a single
out-of-band cell cannot hide inside an in-band mean) then propagated that spurious `0.0`
through every level, zeroing the whole run's readiness verdict for BOTH arms regardless of
how the other 108/128 (84%) cells read. Manually re-aggregating the 811 manifest's own
per-cell data with zero-tick cells correctly excluded gave: full range 0.0022, habit range
9.34e-6, habit distinct fraction 0.875 -- all three clear their floors. The autopsy's own
re-derivation needed no new run; this letter exists so a clean manifest self-routes the same
verdict going forward, per the autopsy's Section 10 repair routing (`/queue-experiment`,
same question, new letter, `recommended_substrate_queue_entry.action = "none"` -- no
substrate gap, SD-081 works as designed).

THE FIX (in the lineage module, not here): the three empty-cell defaults now propagate
`None` through all three aggregation levels (per-probe, per-condition, per-arm-per-seed) via
two small helpers (`_worst_or_none` / `_mean_or_none`), mirroring `recruitment_rate`'s
existing None-for-unmeasured convention. This driver is otherwise UNCHANGED from V3-EXQ-811
-- same scientific question, same pre-registered thresholds, same schedule, same arms -- so
a corrected manifest exists for the same design that was already confirmed sound.
...
EXPERIMENT_TYPE = "v3_exq_811a_mech477_dualsystem_arbitration_falsifier"
SUPERSEDES = "v3_exq_811_mech477_dualsystem_arbitration_falsifier"
...
def run_experiment(dry_run: bool = False) -> Dict[str, Any]:
    ...
    manifest: Dict[str, Any] = {
        ...
        "supersedes_note": (
            "V3-EXQ-811's own manifest self-routed substrate_not_ready_requeue on all three "
            "non-vacuity readiness gates due to a confirmed empty-cell aggregation bug in "
            "experiments/_lib/baselines/mech477_dualsystem_arbitration.py (0.0 default "
            "instead of None for zero-tick cells, unlike the adjacent recruitment_rate "
            "field). failure_autopsy_V3-EXQ-811_2026-07-24 manually re-derived a clean PASS "
            "on C1 from the same manifest's per-cell data with zero-tick cells excluded; "
            "this run repeats the identical design with the aggregation bug fixed so a "
            "correct manifest exists going forward without hand re-derivation."
        ),
    }
