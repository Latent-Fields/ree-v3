...
"""
V3-EXQ-944b -- MECH-091: do salient events reset E3's cycle boundary, and does
...
Supersedes: V3-EXQ-944a (crashed, ERROR, exit 1, no manifest -- see below)

DIAGNOSIS OF V3-EXQ-944a's ERROR (/diagnose-errors, 2026-08-25)
----------------------------------------------------------------
944a ran on ree-cloud-3 2026-08-22T13:05:01Z -> 15:10:58Z (125.9 min against
an estimated_minutes of 120), exit code 1, no manifest, no runner sentinel.
The coordinator DB stores no stderr, so the traceback was NOT recoverable
from any git-tracked artefact -- ree-worker-3 was powered off (the cloud
scaler idles it; a pytest run or a diagnose pass creates no queue claim to
keep it up). Powered it on by hand (hcloud, user-authorised) and read its
systemd journal directly (persists across power cycles); it still held the
crash window intact:

    Aug 22 15:10:58 ree-worker-3 ree-runner[19266]: Traceback (most recent
      call last):
      File ".../v3_exq_944a_mech091_salient_event_cycle_boundary.py",
        line 794, in <module>
        result, zg, last_agent = run(dry_run=args.dry_run)
      File ".../v3_exq_944a_..._boundary.py", line 713, in run
        [r["reset_execution_frac"] for r in arm_results
      File ".../v3_exq_944a_..._boundary.py", line 715, in <listcomp>
        and r["reset_execution_frac"] is not None] or [0.0])),
    KeyError: 'reset_execution_frac'

CONFIRMED ROOT CAUSE: a coupled-producer/consumer landing-order defect,
CLAUDE.md's remedy (a2) shape exactly. 944a's driver (committed e1a56b0,
2026-08-22T12:00:59Z) reads `r["reset_execution_frac"]` by DIRECT dict
indexing in the metrics block (bypassing the driver's own `val()` safe
getter, which every OTHER read of a `measure_cell` row goes through). At
the time 944a actually ran, the base module's `measure_cell()` -- still
V3-EXQ-944's ORIGINAL version, since the RATE_MATCHED fix commit that adds
`reset_execution_frac` to every row (f34e17b) did not land until
2026-08-22T21:38:31Z, ~6.5h AFTER the crash and ~9.5h after the driver went
live -- did not populate that key at all. Every one of the 18 cells
completed successfully (the journal shows "Seed 45 Condition NO_RESET ...
verdict: PASS" immediately before the traceback); the crash was in the
final metrics aggregation, after all real compute had already run, which is
exactly why exit code 1 arrived at ~126 min against a 120 min estimate.

f34e17b's OWN commit message explicitly (and, per this journal, WRONGLY)
ruled this out: "the missing counter does NOT crash the driver... all 20
base.X references resolve against origin's baseline... so the effect was a
silently NULL metric, not an error." That statement is true for every
access that goes through `val()` -- true of 19 of the 20 references -- but
misses the one at line 713-715, a direct `r["..."]` index with no `.get()`
fallback. That investigation's own conclusion is corrected here from
ground truth, not superseded speculation.

WHY THIS CANNOT RECUR AS-IS: f34e17b already landed and now unconditionally
populates `reset_execution_frac` on every `measure_cell()` row -- 944b
builds on that current base module, so the missing-key KeyError is already
closed by a commit that predates this diagnosis. FIX D below hardens the
one direct-index read anyway (matching the defensive pattern every other
row-read in this driver already uses), so a FUTURE coupled producer/
consumer split on this file cannot reintroduce the same crash shape.

FIXES IN THIS REVISION (944a -> 944b)
--------------------------------------
  FIX D (the confirmed crash fix). `r["reset_execution_frac"]` (direct
    index, the crash site) -> `r.get("reset_execution_frac")` (matching
    every other row-read in this driver, all of which already go through
    the safe `val()` helper or an explicit `.get`). Purely defensive given
    the base module already guarantees the key today; closes the coupled-
    producer/consumer landing-order hazard structurally rather than only
    incidentally.
  FIX A (independent real bug, found by code review). The cross-arm delta
    computation built `aligned_st` and `matched_st` as TWO INDEPENDENTLY
    None-filtered lists over the scored seeds, then zipped them
    positionally. If ALIGNED's straddle_frac is present for a seed where
    RATE_MATCHED's is None (or vice versa -- plausible for RATE_MATCHED
    specifically, since its resets are deliberately deferred late and so
    more likely to have no following tick in some episodes), zip() silently
    pairs values from DIFFERENT seeds. Fixed: deltas are now computed per
    seed, only when both arms have a value for that seed.
  FIX B (independent real bug, blocks Step 4e for any successor). The
    dry-run smoke assertion `RATE_MATCHED executed zero resets in every
    cell -> SMOKE FAIL` is structurally guaranteed to fail at dry-run's toy
    STEPS_PER_EPISODE=20: the decouple window is U{K..2K}=U{10..20}, so a
    reset requested at any global_step > 0 cannot land inside a 20-step
    (19-drainable) episode. This is exactly the P0 territory the run
    already names (episode_admits_cycle_contrast) -- the smoke check was
    asserting a real-scale-only guarantee at toy scale. It never ran before
    944a went live (e1a56b0's own commit message says the driver was landed
    unrun, "validates OK under --strict --paths", not "smoke-tested"), so
    the mismatch went unnoticed -- and the SAME "never actually smoke-tested"
    gap is why FIX D's crash site went unnoticed too. Fixed: the smoke check
    now verifies the re-issue path RECORDED requests (the mechanism
    engaged), and reports -- without failing -- when toy scale left none of
    them executable.
  FIX C (diagnostic hardening, not a functional change). Each arm cell is
    now wrapped so a crash inside it prints which (seed, arm) was running
    before re-raising unchanged. This specific crash was in the post-loop
    aggregation (outside any per-cell scope), so FIX C would not have
    caught it, but it closes the same blind spot for any future per-cell
    crash on this driver.
...
EXPERIMENT_TYPE = "v3_exq_944b_mech091_salient_event_cycle_boundary"
QUEUE_ID = "V3-EXQ-944b"
CLAIM_IDS: List[str] = ["MECH-091"]
EXPERIMENT_PURPOSE = "evidence"
ARCHITECTURE_EPOCH = "ree_hybrid_guardrails_v1"
SUPERSEDES = "v3_exq_944a_mech091_salient_event_cycle_boundary"
...
def run(dry_run: bool = False):
        ...
        for arm in ARM_NAMES:
            ...
            try:
                with arm_cell(
                    ...
                    # The three arms are measured from ONE per-seed trained
                    # snapshot so they differ only in cycle-boundary alignment.
                    # That is shared mutable cross-cell state, so the cell is
                ...
                # FIX C (944a -> 944b): 944a's ERROR crash-before-manifest left
                # no attributable trace anywhere reachable (coordinator DB has
                # no stderr column; the worker was unreachable). Never suppress
                # -- re-raise unchanged so the runner still classifies ERROR --
                # but name which cell was running so stdout carries the clue
                # this run's own diagnosis could not recover.
                print("[FATAL] cell crashed: seed=%d arm=%s" % (seed, arm),
                      flush=True)
                raise
    ...
    # FIX A (944a -> 944b): 944a built aligned_st/matched_st as two
    # INDEPENDENTLY None-filtered lists, then zipped them positionally --
    # silently pairing different seeds' values whenever one arm had a None
    # straddle_frac for a seed the other arm didn't (plausible for
    # RATE_MATCHED specifically, since its resets are deliberately deferred
    # late). Pair explicitly by seed instead.
    aligned_st, matched_st, no_reset_st = [], [], []
    aligned_lag, no_reset_lag = [], []
    deltas = []
    for s in sc:
        a_st = val(base.ARM_ALIGNED, s, "straddle_frac")
        m_st = val(base.ARM_RATE_MATCHED, s, "straddle_frac")
        n_st = val(base.ARM_NO_RESET, s, "straddle_frac")
        if a_st is not None:
            aligned_st.append(a_st)
        if m_st is not None:
            matched_st.append(m_st)
        if n_st is not None:
            no_reset_st.append(n_st)
        if a_st is not None and m_st is not None:
            deltas.append(m_st - a_st)
        a_lag = val(base.ARM_ALIGNED, s, "mean_integration_lag")
        n_lag = val(base.ARM_NO_RESET, s, "mean_integration_lag")
        if a_lag is not None:
            aligned_lag.append(a_lag)
        if n_lag is not None:
            no_reset_lag.append(n_lag)
    ...
    metrics = {
        ...
        # FIX D (944a -> 944b): direct r["reset_execution_frac"] indexing was
        # 944a's actual crash site (KeyError, ree-cloud-3, 2026-08-22T15:10:58Z)
        # -- at that time the base module's measure_cell() did not yet
        # populate this key (added later by f34e17b). .get() matches every
        # other row-read in this driver and survives a future producer/
        # consumer split on this same field.
        "min_reset_execution_frac_rate_matched": _fmt(min(
            [r.get("reset_execution_frac") for r in arm_results
             if r["arm"] == base.ARM_RATE_MATCHED
             and r.get("reset_execution_frac") is not None] or [0.0])),
    }
    ...
    diagnostics = {
        ...
        "supersedes_run_note": (
            "Chain: V3-EXQ-944 measured the same DV cleanly on all 15 cells "
            "but aggregated its preconditions with a max/min OVER SEEDS, so "
            "one seed's failure vacated four clean seeds and the run "
            "self-routed substrate_not_ready_requeue with every criterion "
            "passing (fixed by the per-seed gate, below). Its successor "
            "V3-EXQ-944a then crashed (ERROR, exit 1, no manifest, ree-cloud-3, "
            "2026-08-22) before completing; /diagnose-errors 2026-08-25 could "
            "not conclusively determine the crash site (see this script's "
            "own docstring) but found and fixed two independent real defects "
            "and added per-cell crash attribution. This run (944b) is that "
            "retry."),
...
if __name__ == "__main__":
    ...
    if args.dry_run:
        ...
        # The RATE_MATCHED re-issue path must actually be ENGAGED (recording
        # requests). Whether any of them get EXECUTED cannot be asserted at
        # this toy scale: FIX B (944a -> 944b) -- the decouple window is
        # U{K..2K}=U{10..20} against dry-run's STEPS_PER_EPISODE=20 (19
        # drainable steps), so a request from any global_step > 0 is
        # structurally unable to land inside the episode. That is exactly
        # the P0 territory this run pre-registers (episode_admits_cycle_
        # contrast), not a defect -- asserting nonzero EXECUTED here made the
        # smoke check unpassable by construction and (confirmed) had never
        # actually been run before 944a went live.
        rm = [r for r in result["arm_results"] if r["arm"] == base.ARM_RATE_MATCHED]
        assert any((r["n_resets_requested"] or 0) > 0 for r in rm), (
            "SMOKE FAIL: RATE_MATCHED recorded zero reset requests in every "
            "cell -- the ledger is not observing phase_reset() calls for "
            "this arm at all (a wiring regression, not a scale artefact).")
        if any((r["n_resets_executed"] or 0) > 0 for r in rm):
            print("[smoke] RATE_MATCHED executed >=1 deferred reset", flush=True)
        else:
            print("[smoke] RATE_MATCHED requested resets but executed none at "
                  "toy dry-run scale (STEPS_PER_EPISODE=20 < decouple window "
                  "K..2K=10..20) -- expected at this scale per P0; the real "
                  "run's STEPS_PER_EPISODE=72 gives resets room to land. Not "
                  "a smoke failure.", flush=True)
