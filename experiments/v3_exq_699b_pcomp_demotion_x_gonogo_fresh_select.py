...
# experiments/_lib/fresh_select.py and constructs its probe -- migrated off the
# hand-rolled sentinel-key marker onto the shared helper, matching the 699c
# reference implementation (experiments/_lib/fresh_select.py; landed acd48f9).
#
# That is the point of the shared helper. This file previously declared both lint
...
# the narrowing above.
#
# The freshness MECHANISM is unchanged from before this migration: a sentinel key
# rather than an `agent.e3.<attr> = None` clear, because nulling
...
# step via agent.py:8006), which is unacceptable in a composition experiment. Full
# rationale in the helper's module docstring; the substrate assumption it rests on
# is pinned by tests/contracts/test_fresh_select_wholesale_reassign.py.
#
...
# The namespace is REQUIRED and yields the private key "_exq699b_stale_marker",
...
_FRESH_SELECT = FreshSelectProbe("exq699b")
