...
"""V3-EXQ-875a -- MECH-471 LOCAL-UPDATE INTERFERENCE TEST (EXP-0399), CORRECTED REQUEUE.

SUPERSEDES V3-EXQ-875 (self-routed substrate_not_ready_requeue, NOT a verdict on MECH-471).
875's readiness gate failed: both baseline competences (env A "corner", env B "banded")
landed far below the required survival floor (a_baseline_worst_seed=19.67, b=15.5 vs
survival_floor_ticks=90.0; random-walk margins also missed) and the phase-2 targeted update
moved competence B by 0.0 (below the 7.5-tick floor) -- phase-1 never installed a real
competence to interfere with in the first place.

ROOT CAUSE, VERIFIED (not guessed): 875's `_train_all_on_agent` calls never passed
`zworld_p0_episodes`, so it defaulted to 0 -- the exact SD-070 defect documented in
...
FROZEN RANDOM PROJECTION, with no error and no warning. This is not a hypothesis -- it is the
SAME driver family (`experiments._lib.allon_training._train_all_on_agent`), and the CLAUDE.md
record for V3-EXQ-728 states plainly: with this defect present, 728 measured "3 of 3 seeds
failed"; after wiring `zworld_p0_episodes=60` (SD-070's validated operating point) into the
SAME script with the SAME otherwise-unchanged config, 728 measured "arm_green=True
seeds_failed=0" and outcome PASS. 875 cited 728 as establishing survival is learnable on this
stack, but 875's own driver never actually enabled the fix that made 728 work.

FIX (two changes, both directly informed by the verified 728 remedy, not by guesswork):
  1. Add the P0a SD-070 z_world encoder warmup (`zworld_p0_episodes=ZWORLD_P0_EPISODES=60`,
     SD-070's validated operating point) to the FIRST `_train_all_on_agent` call per cell
     (competence-A acquisition) -- the stage that trains the shared agent's z_world encoder
     BEFORE the e2 forward-model warmup regresses on it (ordering is load-bearing: e2 fitted
     to a frozen random projection is the same defect one phase later). Competence B's
     acquisition (phase 1b) and the phase-2 targeted update reuse the SAME agent with an
     already-trained z_world, so they do NOT repeat the P0a stage (zworld_p0_episodes=0,
     matching the existing p0_episodes=0 "no re-warmup" convention for those calls).
  2. Align P0_WARMUP_EPISODES/P1_ACQUIRE_EPISODES/STEPS_PER_EPISODE/EVAL_EPISODES to
     V3-EXQ-728's validated recipe (200/90/200/20, vs 875's 120/50/150/12) rather than
     guessing a partial bump -- this is the ONE combination with direct empirical evidence of
     producing above-floor survival competence on this exact all-ON stack post-SD-070-fix.
     875's own design (acquire TWO competences sequentially on one shared agent, then a
     dose-response perturbation) has no direct precedent for whether a smaller budget would
     also clear the floor once z_world is fixed, and a second multi-hour under-trained run is
     a real cost (875 itself ran ~20.5h on ree-cloud-3 to reach its degenerate result) -- so
     this iteration reuses the fully-validated numbers rather than re-deriving new ones.
  Seeds, strength levels, and the local-update-interference design itself are UNCHANGED --
  the scientific question 875 asked is still the right question; only its training-readiness
  instrumentation was broken.

--- ORIGINAL V3-EXQ-875 DOCSTRING BELOW (env/task design, acceptance criteria, and polarity
are UNCHANGED by this fix and still apply verbatim) ---
...
EXPERIMENT_TYPE = "v3_exq_875a_mech471_competence_provenance"
QUEUE_ID = "V3-EXQ-875a"
SUPERSEDES = "V3-EXQ-875"
CLAIM_IDS: List[str] = ["MECH-471"]
EXPERIMENT_PURPOSE = "evidence"
ARCHITECTURE_EPOCH = "ree_hybrid_guardrails_v1"

# --- Budget (real run) -- ALIGNED TO V3-EXQ-728'S VALIDATED POST-SD-070-FIX RECIPE, NOT
# 875's under-trained numbers. See module docstring "FIX" section for why. ------------------
SEEDS: List[int] = [42, 43, 45]   # 44 skipped per repo convention (recurring early-death
                                  # instability on this env family; substitute 45).
ZWORLD_P0_EPISODES = 60           # NEW: P0a SD-070 z_world ENCODER warmup, run AHEAD of the
                                  # e2 warmup below, on competence-A acquisition ONLY (the
                                  # shared agent's z_world stays trained through B's
                                  # acquisition and the phase-2 perturbation). 60 is SD-070's
                                  # validated operating point (matches V3-EXQ-728/734/737/742).
                                  # THIS IS THE FIX -- see module docstring. 875 had this at
                                  # the implicit default 0, which left z_world a frozen random
                                  # projection for the whole run.
P0_WARMUP_EPISODES = 200          # SD-056 e2 forward-model warmup, run ONCE on env A only
                                  # (A0 recipe: e2 frozen through every subsequent P1 call).
                                  # 875 used 120; aligned to 728's validated 200.
P1_ACQUIRE_EPISODES = 90          # REINFORCE episodes to acquire EACH of competence A and B.
                                  # 875 used 50; aligned to 728's validated 90.
STRENGTH_LEVELS: List[int] = [0, 25, 50, 150]   # additional TARGETED REINFORCE episodes on B
                                                 ...
                                                 # UNCHANGED from 875 -- the dose-response
                                                 # design is not what was broken.
STEPS_PER_EPISODE = 200            # 875 used 150; aligned to 728's validated 200.
EVAL_EPISODES = 20                 # 875 used 12; aligned to 728's validated 20.
...
def _run_cell(
    ...
    # Phase 1a: acquire competence A. P0a SD-070 z_world warmup (zworld_p0>0, THE FIX -- see
    # module docstring) runs AHEAD of the P0b e2 forward-model warmup, on a DEDICATED env
    # instance (the P0a rollout consumes env RNG; reusing env_a_train would shift the layout
    # sequence P0b/P1 then see). Then P1 REINFORCE, e2 frozen thereafter.
    ...
    _train_all_on_agent(
        ...
        zworld_p0_env=(_make_env(seed, kw_a) if zworld_p0 > 0 else None),
    )
    ...
    for strength in strengths:
        for seed in seeds:
            ...
            with arm_cell(
                ...
                row = _run_cell(
                    seed, strength, p0, p1_acquire, eval_eps, steps, kw_a, kw_b, zg, zworld_p0,
                )
                cell.stamp(row)
...
def _build_manifest(result: Dict[str, Any], timestamp_utc: str, dry_run: bool,
    ...
    return {
        ...
        "notes": (
            "MECH-471 EXP-0399 local-update interference test. Supersedes V3-EXQ-875, which "
            "self-routed substrate_not_ready_requeue: its _train_all_on_agent calls never "
            "enabled the SD-070 z_world P0a warmup (zworld_p0_episodes defaulted to 0), so "
            "z_world stayed a frozen random projection and neither competence A nor B cleared "
            "the survival floor (verified defect, not a redesign -- see module docstring; "
            "matches the documented V3-EXQ-728 pre-fix 'seeds_failed=3/3' signature on the "
            "same driver family). This iteration adds zworld_p0_episodes=60 (SD-070's "
            "validated operating point) to competence-A acquisition and aligns "
            "P0/P1/steps/eval to V3-EXQ-728's validated post-fix recipe (200/90/200/20). "
            "Design, seeds, and strength levels are otherwise UNCHANGED from V3-EXQ-875: two "
            "separable competences (survival/avoidance under two reef-placement GEOMETRIES -- "
            "corner safe-zones vs a persistent banded reef corridor, same n_reef_patches -- on "
            "the SAME shared all-ON REE bias_head/OFC-devaluation REINFORCE policy substrate, "
            "e2 frozen throughout per the 724-A0 recipe). Survival (not foraging) was chosen "
            "as the DV specifically to avoid the well-documented MECH-457 foraging-competence "
            "ceiling (719a/724/728/734/742/780/788), which V3-EXQ-728 showed the same all-ON "
            "stack does NOT share for survival, once z_world is correctly trained. Testable on "
            "existing substrate; no new architecture. Does not build or exercise any "
            "provenance/rollback machinery (that is what MECH-471 would motivate building, "
...
def main() -> Tuple[Optional[str], Optional[str], bool]:
    parser = argparse.ArgumentParser(
        description="V3-EXQ-875a MECH-471 local-update interference test (EXP-0399, corrected requeue)"
    )
    ...
    result = run_experiment(
        ...
        eval_eps=eval_eps, steps=steps, zworld_p0=zworld_p0,
    )
