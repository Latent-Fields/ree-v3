...
"""
V3-EXQ-245a -- MECH-120 SHY Wiring Validation (diagnostic)

Claims: MECH-120

Scientific question:
  Does the newly wired enter_sws_mode() -> shy_normalise() path produce
  measurably different ContextMemory slot differentiation when shy_enabled=True
  vs shy_enabled=False?

  This is a DIAGNOSTIC substrate-readiness check, not governance evidence.
  EXQ-245 (x2) was non_contributory because shy_normalise() was never called
  from enter_sws_mode(). Now that it is wired (2026-04-08), this experiment
  validates that the wiring works end-to-end through the agent API.

Design:
  Two conditions, 3 seeds, 100 training episodes.

  SHY_ENABLED:
    shy_enabled=True, shy_decay_rate=0.85. enter_sws_mode() calls
    shy_normalise() before serotonin.enter_sws(). Then alternating
    SAFE/DANGEROUS context writes (Phase 2).

  SHY_DISABLED:
    shy_enabled=False. enter_sws_mode() does NOT call shy_normalise().
    Same alternating context writes.

  Environment: alternating SAFE (num_hazards=1) / DANGEROUS (num_hazards=8).
...
Metrics:
  ...
  - slot_norm_std: std of slot norms (SHY should reduce this)

PASS criteria:
  S1: slot_norm_std(SHY_ENABLED) < slot_norm_std(SHY_DISABLED) after
      enter_sws_mode() in >=2/3 seeds (SHY reduces slot norm variance)
  S2: slot_cosine_sim(SHY_ENABLED) < slot_cosine_sim(SHY_DISABLED)
      in >=2/3 seeds (SHY improves differentiation)
...
EXPERIMENT_TYPE = "v3_exq_245a_mech120_shy_wiring_validation"
CLAIM_IDS = ["MECH-120"]
EXPERIMENT_PURPOSE = "diagnostic"
...
CONDITIONS = ["SHY_ENABLED", "SHY_DISABLED"]
...
def _make_agent(env: CausalGridWorldV2, shy_enabled: bool) -> REEAgent:
...
def _compute_slot_norm_std(agent: REEAgent) -> float:
    """Std of slot L2 norms (SHY should reduce this)."""
    with torch.no_grad():
...
def _sws_pass(
    ...
    """Alternating SAFE/DANGEROUS context_memory writes (SWS Phase 2)."""
...
def _run_condition(
    ...
    shy_enabled = (condition == "SHY_ENABLED")

    env_safe = _make_env_safe(seed)
    env_dang = _make_env_dangerous(seed)
    agent = _make_agent(env_safe, shy_enabled=shy_enabled)
    ...
    for ep in range(training_episodes):
        ...
        if (ep + 1) % SLEEP_INTERVAL == 0:
            if len(safe_z_buf) >= 2 and len(dang_z_buf) >= 2:
                ...
                # Temporarily allow writes (SWS offline mode blocks them)
...
def main():
    ...
    enabled = [r for r in all_results if r["condition"] == "SHY_ENABLED"]
    disabled = [r for r in all_results if r["condition"] == "SHY_DISABLED"]

    # S1: norm_std(ENABLED) < norm_std(DISABLED) in >=2/3 seeds
    s1_wins = sum(1 for a, b in zip(disabled, enabled)
                  if b["slot_norm_std"] < a["slot_norm_std"])
    s1 = s1_wins >= 2

    # S2: slot_cosine_sim(ENABLED) < slot_cosine_sim(DISABLED) in >=2/3 seeds
    s2_wins = sum(1 for a, b in zip(disabled, enabled)
    ...
    summary = {
        "s1_shy_reduces_norm_std": {
            "wins": s1_wins, "pass": s1,
            "desc": "slot_norm_std(SHY_ENABLED) < slot_norm_std(SHY_DISABLED) in >=2/3 seeds",
        },
        "s2_shy_improves_differentiation": {
            "wins": s2_wins, "pass": s2,
            "desc": "slot_cosine_sim(SHY_ENABLED) < slot_cosine_sim(SHY_DISABLED) in >=2/3 seeds",
        },
    }

    print(f"\nFINAL OUTCOME: {outcome}")
    for k, v in summary.items():
        print(f"  {k}: {v['pass']} [{v['wins']}/3 seeds]")
