"""
V3-EXQ-460f (supersedes V3-EXQ-460e): SD-034 closure-control-plane de-commit
authority, re-run on the foraging-competent scaffolded_sd054_onboarding 603n agent
with the rule_bias_head trained (commitment_closure:GAP-4 Leg C) AND the
BETA-ENGAGEMENT amend live (use_closure_commit_beta_coupling, ree-v3 main f4ceea4).

WHY THIS SUPERSEDES 460e (failure_autopsy_V3-EXQ-460e_2026-06-17, confirmed): 460e ran
terminal FAIL/non_contributory, self-routing substrate_not_ready_requeue on
beta_engagement_not_met_both_arms (0.333, 1/3 seeds) BEFORE the C2 de-commit DV ran.
Legs A/B/C are all BUILT and WORKING (rule_bias_trained 1.0 3/3; closures fire 5-7), but
with both MECH-090 R-c gates OFF the bistable BetaGate elevated ONLY on E3
result.committed (running_variance < commit_threshold) -- a decisive natural commit-entry
that fired on only 1/3 seeds on the 603n foraging substrate. The closure control-plane
set committed_trajectory + fired closures regardless (the commit-without-beta
dissociation: seeds 42/43 committed_steps 2415/2019 but total_beta_elevated=0); seed 44
engaged beta on BOTH arms and C2 PASSED (ON mean_beta_elevated 11.73 < OFF 14.87,
non-cap-pinned) -- the positive existence proof that the DV is sound. The trained
rule_bias_head biases per-candidate SCORING, not the running_variance that gates
commit-ENTRY, so it cannot rescue engagement.

THE FIX (the beta-engagement amend, mechanism (a); landed ree-v3 main f4ceea4
2026-06-17): use_closure_commit_beta_coupling makes the bistable elevate block treat
(result.committed OR e3._committed_trajectory is not None) as the commit-for-beta
trigger -- keeping the full should_admit_elevation / _readiness_admits conjunction -- so
beta occupancy now tracks the closure-plane commitment on EVERY seed where one forms.
BOTH arms inherit the flag (the OFF clone deepcopies the config), so both engage beta;
only the closure-driven Leg-B de-commit refractory differs, producing the measurable
ON<OFF latch-occupancy drop the 460e self-route never reached. The new BetaGate
diagnostic sd034_n_closure_coupled_elevations records that the coupling -- not a fluke
natural commit-entry -- engaged the latch (non-vacuity). All four readiness gates are
unchanged; with the coupling live the beta_engagement_both_arms gate is now satisfiable,
so a still-unmet gate self-routes substrate_not_ready_requeue (mechanism (b)
commit_threshold adaptation would be the next experiment-config lever), NEVER a false
weakens.

WHAT CHANGED (commitment_closure:GAP-4 Leg C, landed 2026-06-16): the scaffold has a
scaffold_train_rule_bias_head leg that TRAINS agent.lateral_pfc.bias_head_parameters()
...
no-op default, bit-identical OFF; ree_core untouched). 460f keeps it enabled
...
EXPERIMENT_TYPE = "v3_exq_460f_sd034_closure_control_plane_decommit_behavioural"
QUEUE_ID = "V3-EXQ-460f"
CLAIM_IDS: List[str] = ["SD-034", "MECH-260", "MECH-261"]
EXPERIMENT_PURPOSE = "evidence"
SUPERSEDES = "V3-EXQ-460e"
...
def _make_config(env) -> REEConfig:
    ...
    cfg = REEConfig.from_dims(
        ...
        # closure-plane commitment (e3._committed_trajectory) to bistable beta elevation
        # so the de-commit latch-occupancy DV is readable on every seed where a closure
        # commitment forms -- decoupled from the fragile natural running_variance crossing.
...
def _eval_closure_behaviour(
    ...
    with torch.no_grad():
        for _ in range(n_eps):
            ...
            # >0 across the arm confirms the coupling (not a fluke natural commit-entry)
            # engaged the latch -- the de-commit DV non-vacuity readout.
...
def _aborted_seed_record(seed: int, stage: str, reason: str) -> Dict[str, Any]:
    empty = {
        "n_closures": 0, "sd034_n_closure_coupled_elevations": 0, "n_hook_fires": 0, "n_automatic_fires": 0, "beta_release_events": 0,
