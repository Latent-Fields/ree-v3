...
"""
V3-EXQ-258b -- MECH-205: Surprise-Gated Replay Validation (Threshold Calibration Fix)

Supersedes: V3-EXQ-258a
...
Does surprise-gated replay prioritisation (MECH-205) produce
...
=== CHANGES FROM EXQ-258a ===

EXQ-258a FAIL root cause: pe_surprise_threshold=0.001 miscalibrated.
Observed PE magnitudes in this architecture are order 1e-5 (mean_pe_mag=4.16e-5,
mean_surprise=1.86e-5). Threshold was ~53x above actual signal level so
VALENCE_SURPRISE never fired regardless of whether MECH-205 works correctly.
P2 and P3 passed (PE tracking and EMA work); only P1 failed.

Fix:
  - pe_surprise_threshold=0.001 -> pe_surprise_threshold=1e-5
    (just below observed mean_surprise=1.86e-5; allows surprise events
    above the EMA to write to VALENCE_SURPRISE)
...
# Experiment metadata
# ---------------------------------------------------------------------------
EXPERIMENT_TYPE    = "v3_exq_258b_mech205_surprise_gated_replay"
CLAIM_IDS          = ["MECH-205"]
EXPERIMENT_PURPOSE = "evidence"
...
def _eval(
    ...
    if hasattr(agent.residue_field, 'rbf_field') and agent.residue_field.rbf_field is not None:
        vv = agent.residue_field.rbf_field.valence_vecs
        if vv is not None:
...
def _run_seed(
    ...
    for label, surprise_gated in CONDITIONS:
        print(
            f"\n[V3-EXQ-258b] Seed {seed} Condition {label}"
...
def main() -> None:
    ...
    print(
        f"[V3-EXQ-258b] MECH-205: Surprise-Gated Replay Validation"
    ...
    all_results = []
    for seed in seeds:
        print(f"\n[V3-EXQ-258b] === Seed {seed} ===", flush=True)
    ...
    print(f"\n[V3-EXQ-258b] === Results ===", flush=True)
    ...
    manifest = {
        ...
        "supersedes":         "v3_exq_258a_mech205_surprise_gated_replay",
        "outcome":            outcome,
    ...
    print(f"\n[V3-EXQ-258b] Output written: {out_path}", flush=True)
