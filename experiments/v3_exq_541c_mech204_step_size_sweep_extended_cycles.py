...
V3-EXQ-541b PASSed C1+C2+C3 in every step-size arm but FAILed C4
(cross-arm divergence) with a clean monotone dose-response curve:
step 0.05 -> 0.31%, step 0.10 -> 0.63%, step 0.25 -> 1.56%,
step 0.50 -> 3.13%. None reached the pre-registered 5% threshold.
Tracking_quality monotonically improved with step (0.877 -> 0.939);
zero overshoot across all arms.

Hypothesis under test: the small cross-arm divergence is because waking
drift between cycles re-randomises rv enough to wash out most of the
per-cycle recalibration effect. With 4 cycles per run (sleep_loop_K=2,
8 episodes) the cumulative effect doesn't have time to grow. If F1 is
sufficient given enough exposure, then increasing the cycle count
should produce proportionally larger cross-arm divergence (i.e. the
divergence should scale roughly linearly with cycle count under fixed
waking drift). If F1 is intrinsically limited, divergence should
plateau regardless of cycle count and Phase 7 / Option B becomes the
load-bearing architectural lever per the REM-precision lit-pull
SYNTHESIS dispatch case #3.

This run uses sleep_loop_K=1 (sleep every episode) and EPISODES_PER_RUN=16
to give 16 cycles per run -- 4x V3-EXQ-541b's 4 cycles. Together,
541b and 541c form the cycle-count dose-response for F1.

Five-arm sweep on rem_precision_recalibration_step (same as 541b):
...
also fails C4 AT 16 CYCLES (4x the exposure of 541b), F1+step-tuning
alone is insufficient and the diagnosis points at Phase 7 / Option B
as the next architectural lever per the REM-precision lit-pull
SYNTHESIS dispatch case #3.

Comparison framing (the scientifically primary outcome of 541c, beyond
the binary PASS/FAIL):
- For each step arm, compare C4 cross-arm divergence at 4 cycles
  (541b) vs 16 cycles (541c). If divergence scales approximately
  linearly with cycle count, F1 is sufficient given enough exposure
  and Phase 7 stays deferred.
- If divergence plateaus at 16 cycles (i.e. ARM_4_step_0_50 still
  caps near 3-5%), F1 is at its ceiling -- waking drift fully washes
  per-cycle recalibration regardless of cumulative cycle count, and
  Phase 7 / Option B becomes load-bearing.
"""
...
EXPERIMENT_TYPE = "v3_exq_541c_mech204_step_size_sweep_extended_cycles"
CLAIM_IDS = ["MECH-204"]
...
SLEEP_LOOP_K = 1  # sleep fires every episode -> 16 cycles per run
...
def main(dry_run: bool = False):
    ...
    print(
        f"V3-EXQ-541c MECH-204 step-size sweep extended cycles -- {outcome} in {elapsed:.1f}s",
        flush=True,
    )
    if dry_run:
    ...
    manifest = {
        ...
        "notes": (
            "Extended-cycle companion to V3-EXQ-541b (4 cycles per run); "
            "this run uses sleep_loop_K=1 + EPISODES_PER_RUN=16 to give "
            "16 cycles per run -- 4x the cumulative exposure. Five arms "
            "vary rem_precision_recalibration_step in {0.0_off, 0.05, "
            "0.10, 0.25, 0.50} with precision_zero_point_ema_alpha=0.1 "
            "held constant. Same env, same seeds, same per-cycle "
            "telemetry as 541b. Tests whether F1 cross-arm divergence "
            "scales with cycle count (F1 sufficient given enough "
            "exposure -> Phase 7 stays deferred) or plateaus at 4-cycle "
            "level (F1 at its ceiling -> Phase 7 / Option B becomes "
            "load-bearing per REM-precision lit-pull SYNTHESIS dispatch "
            "case #3). NOT a supersession of 541b; together 541b+541c "
            "form the cycle-count dose-response for F1. Same C1-C4 "
            "acceptance criteria as 541b; primary scientific outcome "
            "is the 4-cycle vs 16-cycle divergence comparison."
        ),
    }
