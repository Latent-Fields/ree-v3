...
"""V3-EXQ-648a -- MECH-314a Phase-2 substrate-readiness diagnostic (CORRECTED).

SUPERSEDES V3-EXQ-648 (run v3_exq_648_..._20260607T025417Z, FAIL
precondition_unmet). Same scientific question (does the Phase-2 substrate
produce per-candidate-varying curiosity bias?), corrected implementation ->
alphabetic suffix. Autopsy:
REE_assembly/evidence/planning/failure_autopsy_V3-EXQ-648_2026-06-07.{md,json}.

=== WHAT 648 GOT WRONG (and what 648a fixes) ===

648's MECH-314a per-candidate novelty AND the auto-augmentation _candidate_spread
key were computed from the hippocampal proposer's first-step z_world
(trajectory.world_states[:,0,:]), whose cross-candidate spread is <0.01 under
monostrategy -> curiosity_bias_range_mean=0.0 and curiosity_std_across_K_mean=0.0
in EVERY arm incl the ARM_1 positive control (c2_arm1_seeds_ok=0/3). The SD-056
trained e2.world_forward(z0,a_i) predictions carry spread ~0.1147, but were NOT
the consumed signal. 648's readiness precondition measured e2.cand_world_pairwise_dist
(0.1147, PASS) while C2 routed on the proposer-derived bias (<0.01) -> false READY
-> the self-route mislabelled a collapsed-INPUT artefact as a wiring null
(canonical V3-EXQ-642 same-statistic pattern).

648a applies two corrections, both now expressible at the script level because the
MECH-314a substrate amend landed 2026-06-07 (ree-v3 commit 84c091c):
  (1) curiosity_candidate_source="e2_world_forward" on EVERY arm. The new
      no-op-default REEConfig flag makes agent.select_action rebuild the
      per-candidate novelty signature from e2.world_forward(z0, a_i) -- so BOTH
      the 314a RBF novelty AND the auto-augmentation _candidate_spread key key on
      the SD-056-divergent, action-conditional representation (design-doc
      Candidate 1 source on the landed Candidate-5A machinery).
  (2) The readiness precondition is re-targeted to the CONSUMED representation:
      it now ALSO asserts the per-candidate curiosity_bias_range (the SAME
      statistic C2 routes on) clears a floor on the ARM_1 positive control --
      not only the e2 pairwise spread. A collapsed consumed-input (e2 untrained,
      or the amend not wired) self-routes substrate_not_ready_requeue instead of
      a false READY. Post-amend, e2.cand_world_pairwise_dist IS the consumed
      representation's spread, so the two readiness legs are causally aligned
      (e2 spread -> per-candidate novelty range -> curiosity bias range).

Validates the LANDED MECH-314a Phase-2 substrate (rolling z_world visitation
buffer + first-action one-hot augmentation; landed 2026-06-05) AS AMENDED
(e2.world_forward novelty source, 2026-06-07; substrate_queue entry
MECH-314a-Phase-2-impl, status amend_implemented_pending_validation). This run
...
(section 3 Candidate 1 + section 6 falsifier sketch + the V3-EXQ-648a amend
note: novelty fed from e2.world_forward).
...
=== READINESS PRECONDITION (substrate-not-ready guard) -- 648a re-targeted ===

C2 routes on a per-candidate cross-candidate RANGE statistic. Per the V3-EXQ-643
same-statistic rule, the readiness precondition asserts that range on the CONSUMED
representation, on the ARM_1 positive control (SD-056-trained, multi-first-action-
class SP-CEM pool). THREE legs:
  Leg 1 -- ARM_1 cand_world_pairwise_dist (e2.world_forward per-candidate spread)
    > C0_PAIRWISE_DIST_FLOOR on >= MIN_SEEDS. Post-648a-amend
    (curiosity_candidate_source="e2_world_forward") this IS the representation the
    curiosity novelty consumes -- a cross-candidate RANGE, not a magnitude.
  Leg 2 (the 648 fix) -- ARM_1 curiosity_bias_range (the EXACT statistic C2 routes
    on) > C0_BIAS_RANGE_FLOOR on >= MIN_SEEDS. 648's precondition measured the e2
    spread but C2 consumed a DIFFERENT (proposer) representation, so a collapsed
    consumed input read as a wiring null (false READY). Leg 2 gates on the consumed
    bias range directly: below-floor -> the consumed signal still collapsed (e2
    untrained / amend not wired) -> substrate STARVED, not falsified.
  Leg 3 -- rolled-out z_world magnitude finite/bounded (643a explosion guard).
If any leg is below floor / non-finite the run self-routes BELOW-FLOOR to
...
|-----------------------------------------|------------------------------------|-------------|
| any readiness leg below floor/non-finite| substrate_not_ready_requeue        | re-queue as 648b at higher P0 budget (or fix e2/amend wiring); do NOT weaken Phase-2 |
...
Smoke:
  /opt/local/bin/python3 experiments/v3_exq_648a_mech314a_phase2_substrate_readiness.py --dry-run
"""
...
EXPERIMENT_TYPE = "v3_exq_648a_mech314a_phase2_substrate_readiness"
QUEUE_ID = "V3-EXQ-648a"
SUPERSEDES = "v3_exq_648_mech314a_phase2_substrate_readiness"
...
C0_PAIRWISE_DIST_FLOOR = 0.05     # readiness leg 1: ARM_1 e2.world_forward spread (consumed rep)
C0_BIAS_RANGE_FLOOR = 1.0e-4      # readiness leg 2: ARM_1 per-candidate curiosity_bias_range
                                  # (the SAME statistic C2 routes on) -- the 648a same-statistic
                                  # gate on the CONSUMED representation. Below-floor here means
                                  # the consumed signal still collapsed -> substrate_not_ready,
                                  # NOT a wiring null (the 648 false-READY this run closes).
C0_MAGNITUDE_CEIL = 1.0e6         # readiness leg 3: rolled-out z_world magnitude bound (643a guard)
...
def _make_agent(env: CausalGridWorldV2, arm: Dict[str, Any]) -> REEAgent:
    ...
    cfg = REEConfig.from_dims(
        ...
        # --- V3-EXQ-648a CORRECTION: feed novelty + _candidate_spread from the
        # SD-056-trained e2.world_forward(z0,a_i) predictions instead of the
        # collapsed proposer trajectory.world_states[:,0,:] (the 648 root cause).
        # On every arm (no-op on ARM_0 residue/harm-free: residue empty -> zero
        # regardless of candidate source, so C1 still holds). The amend landed
        # 2026-06-07 (ree-v3 84c091c); default "proposer" reproduces 648 exactly.
...
def _evaluate(arm_results: List[Dict[str, Any]], c4: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # --- READINESS (substrate-not-ready guard) -- V3-EXQ-648a re-targeted ---
    # Positive control = ARM_1 (SD-056-trained, SP-CEM multi-class candidate pool).
    # Leg 1: e2.world_forward spread = the CONSUMED representation's spread
    # (post-amend, curiosity_candidate_source="e2_world_forward" feeds the novelty
    ...
    # DIFFERENT (proposer) representation. Here the consumed input IS e2, so a
    # below-floor bias range means the consumed signal still collapsed (e2
    # untrained / amend not wired) -> substrate_not_ready, never a wiring null.
    ...
    return {
        "readiness": {
            ...
            "c0_bias_range_floor": C0_BIAS_RANGE_FLOOR,
            "arm1_bias_range_mean": round(arm1_bias_range_mean, 8),
            "arm1_bias_range_seeds_above_floor": int(bias_range_seeds_ok),
        ...
        "preconditions": [
            {
                "name": "consumed_candidate_zworld_spread_supra_floor",
                "kind": "readiness",
                "description": (
                    "ARM_1 (SD-056-trained positive control, SP-CEM multi-class "
                    "candidate pool) per-candidate SPREAD of the CONSUMED novelty "
                    "signature (e2.world_forward(z0,a_i); post-648a-amend this IS "
                    "what curiosity consumes) clears the floor -- a cross-candidate "
            ...
            {
                "name": "consumed_curiosity_bias_range_supra_floor",
                "kind": "readiness",
                "description": (
                    "648a same-statistic gate: ARM_1 per-candidate "
                    "curiosity_bias_range -- the EXACT cross-candidate RANGE C2 "
                    "routes on -- clears the floor on the positive control. Closes "
                    "the 648 gap (precondition measured the e2 spread but C2 "
                    "consumed a DIFFERENT proposer representation); below-floor here "
                    "means the consumed signal still collapsed -> "
...
def run_experiment(dry_run: bool = False) -> Dict[str, Any]:
    ...
    manifest: Dict[str, Any] = {
        ...
        "evidence_direction_note": (
            ...
            "one-hot augmentation, AS AMENDED 2026-06-07 to feed novelty + the "
            "auto-augmentation _candidate_spread key from the SD-056-trained "
            "e2.world_forward(z0,a_i) predictions (curiosity_candidate_source="
            "'e2_world_forward'). SUPERSEDES V3-EXQ-648 (precondition_unmet FAIL: "
            "consumed the collapsed proposer summaries). claim_ids=[] (does not "
            "weight claim confidence). PASS (label=phase2_substrate_ready) gates "
            "/queue-experiment V3-EXQ-590b (novelty-bonus Goldilocks retest) AND "
            "the section-8 governance/claims updates for MECH-314a / MECH-314 / "
            "ARC-065. Readiness-below-floor (e2 spread OR consumed curiosity "
            "bias-range below floor) self-routes to substrate_not_ready_requeue, "
            "NOT a substrate verdict."
        ),
        "interpretation": {
            "label": summary["label"],
            ...
            "routing": {
                "substrate_not_ready_requeue": "re-queue as V3-EXQ-648b at higher P0 budget (or fix e2/amend wiring); do NOT weaken Phase-2",
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="V3-EXQ-648a MECH-314a Phase-2 substrate-readiness diagnostic (e2.world_forward novelty source; supersedes 648)"
    )
