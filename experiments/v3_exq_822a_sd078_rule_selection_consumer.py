"""V3-EXQ-822a -- SD-078 DOWNSTREAM CONSUMER, RE-RUN ON THE SD-082 PROPAGATION CONSUMER.

Same scientific question as V3-EXQ-822 (does centering the CandidateRuleField context
key let the executive RULE-SELECTION become context-conditioned and REACH the action
bias?), re-run now that the SD-082 read-out consumer is landed.

WHY THIS RE-RUN (supersedes 822). V3-EXQ-822 self-routed substrate_not_ready_requeue:
3/4 readiness gates passed (cone 0.963, ON pool differentiated 3.0, ON active 0.882) and
the executive state differentiated (on_rule_state_diff 0.644, c1_pass, max_live 16 vs 1),
but readiness gate (d) propagation_non_vacuity FAILED -- on/off_prop_delta_mean were both
exactly 0.0. Autopsy failure_autopsy_816c-822_2026-07-26 (REE_assembly afb2df901e) traced
this to a STRUCTURAL zero: the SD-033a lateral-PFC bias head read RAW z_world candidate
summaries and hard-clamped its output, so under the SD-008 ~0.98-cosine common-mode cone
every candidate saturated to the SAME clamp rail (rule_state's contribution erased) and
the clamp's flat region zeroed the REINFORCE gradient (head untrainable). SD-082
(ree-v3 d4f758082d, lateral_pfc_rule_readout_consumer) fixes both: (i) subtract the common
mode from the head's candidate-summary input (as SD-078 does upstream), (ii) scaled-tanh
soft bound instead of a hard clamp so the head stays gradient-trainable. This run enables
that consumer on BOTH arms so the SD-078 rule_state -> action-bias propagation is
MEASURABLE and non-vacuous, and re-asks the 822 question on the ON/OFF (crf_cue_centering)
contrast. The ONLY change from 822 is lateral_pfc_rule_readout_consumer=True on both arms;
crf_cue_centering remains the single swept variable.

SD-082 VALIDATION: readiness gate (d) propagation_non_vacuity passing here IS the SD-082
consumer validation (on_prop_delta_mean >= 1e-3 with an ON vs OFF contrast). The scored
claim remains SD-078 (does the differentiated pool yield a context-conditioned executive
bias); SD-082 is the enabling substrate the readiness gate exercises.

PROMOTION GATE (experiment_purpose = evidence). SD-078 is candidate_substrate_landed: its
INTERNAL cue geometry was validated in isolation by V3-EXQ-806 (raw z_world caps the
...
bias-preferred action, under a CENTERED context key with a TRAINED bias head + the SD-082
read-out consumer. The SD-082 read-out did not exist before 2026-07-26, so no manifest on
any substrate_hash can carry non-vacuous propagation for it. Not recoverable -> run.
...
bias. With the SD-082 consumer that context-varying rule_state now reaches the action bias.
This run tests whether that chain actually fires.
...
LateralPFCAnalog wired to consume the field's rule_state (crf_source), the bias head
UN-ZEROED + trained, AND the SD-082 read-out consumer ON (common-mode-invariant input +
scaled-tanh soft bound). Phases:
...
DEPENDENT VARIABLES (decision-level):
  ...
  * BEHAVIOURAL REACH (readiness / non-vacuity): ARM_ON prop_delta_mean > floor AND
    ...
    654f's C1d propagation non-vacuity. This is also the SD-082 consumer validation.
...
READINESS (self-route substrate_not_ready_requeue, never a false does_not_support):
  ...
  (d) propagation non-vacuity above -- the trained head + matured field + SD-082 read-out
      reach the action bias. If the head did not train (e.g. no reward signal) or the
      consumer did not propagate, this fails and the run self-routes rather than reporting
      a false null.
...
head is a nonlinear function of concat([rule_state, candidate_summary]), and under SD-082
the summary is common-mode-centered and the bound is a nonlinear tanh), so the accept/
...
See ree-v3/CLAUDE.md SD-078 + SD-082 + SD-033a + ARC-063 entries;
experiments/v3_exq_822_sd078_rule_selection_consumer.py (the superseded structural-zero
run this re-runs);
...
EXPERIMENT_TYPE = "v3_exq_822a_sd078_rule_selection_consumer"
EXPERIMENT_PURPOSE = "evidence"
CLAIM_IDS = ["SD-078"]
SUPERSEDES = "v3_exq_822_sd078_rule_selection_consumer"
...
def _make_agent(env: CausalGridWorldV2, centering: bool) -> REEAgent:
    ...
    SD-033a LateralPFCAnalog with the bias head UN-ZEROED + trainable, and enable the
    SD-082 rule-selection-action read-out consumer (common-mode-invariant summary input +
    scaled-tanh soft bound) so the rule_state reaches a non-zero per-candidate E3
    score-bias and the head can be trained in P1.
    """
    return REEAgent(REEConfig.from_dims(
        ...
        # SD-082: the rule_state -> action-bias read-out consumer (both arms). This is the
        # enabling substrate that makes the propagation non-vacuous; NOT the swept variable.
...
def run_experiment() -> Dict[str, Any]:
    ...
    # differs from OFF (paired by seed), the 654f C1d guard. This is the SD-082 validation.
            ...
            "preconditions": [
                ...
                {"name": "propagation_non_vacuity",
                 "description": ("ARM_ON rule_state reaches the per-candidate action bias "
                                 ...
                                 "seed -- the trained head + SD-082 read-out made the "
                                 "executive state behavioural. Also the SD-082 validation."),
    ...
    if not ready:
        ...
        if not prop_nonvac:
            reasons.append("ON rule_state did not reach the action bias (head untrained or consumer vacuous)")
