"""V3-EXQ-817 -- SD-080 consequence-grounding CAPABILITY falsifier.

Does GROUNDING the action-object head O in world-effects actually improve
planning? SD-080 established (spike 2026-07-22, run V3-EXQ-809) that
E2.action_object_head receives zero gradient from every REE training path, so
O is a frozen random projection: state-invariant, not consequence-structured
(99.5% of ao variance explained by the action label alone; pairwise-distance
matrix varies <1% across world states; head params bit-identical after warmup).
That is the CORRECTNESS finding. This experiment tests the ORTHOGONAL CAPABILITY
question the spike Sec 4.2 leaves open: if O were grounded, would the hippocampal
CEM (which searches and refits in O) plan better?

DESIGN (spike Sec 4.2). 3 arms x matched seeds, CausalGridWorldV2,
...
the P1 head treatment -- isolating O's content as the manipulation.
  ...
  ARM_0  baseline           : P0 only; action_object_head frozen (current substrate).
  ARM_1  consequence-grounded: P0 + P1 trains action_object_head (+ a linear readout)
                               so o_t predicts the realised world-effect
                               (z_world_{t+1} - z_world_t) from (z_world_t, a_t),
                               encoder frozen, on .detach()ed latents.
  ARM_2  parameter control   : P0 + P1 identical to ARM_1 but the world-effect
                               target is SHUFFLED across the batch. Same added
                               params and gradient traffic, no consequence
                               structure. Without it an ARM_1 win is confounded
                               with "more trained parameters".

PRIMARY GATE (substrate-side, must fire first). In ARM_1 ONLY:
  r2_explained_by_action_alone (M5) on ao falls well below 0.99 (threshold <= 0.95;
  baseline ~0.994) AND within-action-pair consequence Spearman (M6) rises
  materially above 0 (threshold >= 0.15; baseline ~0). If ARM_1 does not move
  these, the grounding objective FAILED and the behavioural read is
  uninterpretable -> self-route substrate_not_ready_requeue, non_contributory.
  Substrate DVs (M5/M6/M1/M2/M3 + world_forward reference) are the SPIKE PROBE's
  own measures, vendored verbatim below, so the ARM_1 gate needs no new instrument.
...
the CEM in O -- the same DVs SD-004's EXQ-003/EXQ-114 PASS rests on):
  ...
  (b) DEFECT REAL BUT NOT LOAD-BEARING -> outcome FAIL (informative, not a null):
      ...
      the EXQ-003 6x survival was produced by terrain/residue navigation, not O's
      content; SD-080 stays candidate as a correctness-not-capability finding.
...
(state, action), so it is neither a broadcast additive constant (would be
invisible to argmax/softmax selection), nor a monotone rescaling (would preserve
selection rank), nor a permutation of interchangeable units. harm_rate/survival
are therefore NOT invariant under the head-content manipulation for ANY of the
three arms (ARM_2 also moves o_t -- that is precisely what makes it the control
that separates "content changed randomly" from "content changed toward
consequences"). A null ARM_1-vs-ARM_0 behavioural result is thus a MEASURED null
(reading (b)), not an arithmetic identity.
...
EXPERIMENT_TYPE = "v3_exq_817_sd080_consequence_grounding_falsifier"
...
ALPHA_WORLD = 0.9
ENV_SIZE = 6
N_STATES = 120          # substrate-DV state sample (matches spike)
SEEDS = [0, 1, 2, 3, 4]
...
# Primary substrate-side readiness gate (ARM_1 only). Both must hold.
GATE_M5_R2_CEIL = 0.95          # r2_explained_by_action_alone must DROP to <= this (baseline ~0.994)
GATE_M6_SPEARMAN_FLOOR = 0.15   # mean_within_pair_spearman must RISE to >= this (baseline ~0)
...
# Collect (z_world_t, a_t, z_world_{t+1}) transitions under the P0 agent, then
# train action_object_head + a linear readout g: o_t -> predicted world-effect
# delta. ARM_2 shuffles the delta targets (same params/traffic, no structure).
# All latents are .detach()ed so no gradient reaches the encoder (phased P1).
# ============================================================================
...
def collect_grounding_transitions(agent, env, n_transitions, steps_per_episode, seed):
    ...
    while len(Zt) < n_transitions and steps < n_transitions * 4:
        ...
        for _ in range(steps_per_episode):
            ...
            if z_world_prev is not None and action_prev is not None:
                ...
                Delta.append((z_world - z_world_prev).detach().clone())
    ...
    return (torch.cat(Zt[:n_transitions], dim=0),
            ...
            torch.cat(Delta[:n_transitions], dim=0))


def train_head_grounding(agent, transitions, shuffle_target, n_steps, batch, lr, seed, label,
                         progress_denom):
    """Train action_object_head + readout so o_t predicts the world-effect delta.

    shuffle_target=True permutes the delta targets across the whole set -> the
    parameter/gradient control (ARM_2): same trained params, no consequence map.
    """
    Zt, Aoh, Delta = transitions
    device = agent.device
    n = Zt.shape[0]
    ao_dim = agent.e2.action_object_head[-1].out_features   # robust to config path
    world_dim = agent.config.latent.world_dim
    readout = nn.Linear(ao_dim, world_dim).to(device)

    if shuffle_target:
        g = torch.Generator().manual_seed(seed + 9000)
        perm = torch.randperm(n, generator=g)
        Delta = Delta[perm].clone()

    params = list(agent.e2.action_object_head.parameters()) + list(readout.parameters())
    ...
    for it in range(n_steps):
        ...
        if (it + 1) % max(1, n_steps // 4) == 0 or it == n_steps - 1:
            print(f"  [ground] {label} step {it + 1}/{progress_denom}  loss={final_loss:.5f}",
                  flush=True)
    agent.eval()
    return {
        "grounding_final_loss": final_loss,
        "grounding_first_loss": float(losses[0]) if losses else float("nan"),
...
def run_seed_arm(seed, arm, agent_p0, frozen_head_snapshot, env, args, config_slice):
    ...
    with arm_cell(
        ...
        print(f"  [{arm} seed={seed}] M5_r2={m5:.4f} M6_spearman={m6:.4f} "
              f"harm_rate={behav['harm_rate']:.4f} survival={behav['mean_survival_steps']:.1f} "
...
def adjudicate(rows_by_arm):
    """Gate first (ARM_1 substrate readiness), then behavioural verdict."""
    a0 = rows_by_arm["ARM_0"]
    a1 = rows_by_arm["ARM_1"]
    a2 = rows_by_arm["ARM_2"]

    arm1_m5 = _mean([r["ao_M5_r2_explained_by_action_alone"] for r in a1])
    arm1_m6 = _mean([r["ao_M6_mean_within_pair_spearman"] for r in a1])
    m5_met = arm1_m5 <= GATE_M5_R2_CEIL
    m6_met = arm1_m6 >= GATE_M6_SPEARMAN_FLOOR
    grounding_took = bool(m5_met and m6_met)

    preconditions = [
        {"name": "arm1_ao_state_dependence_acquired",
         "description": "ARM_1 r2_explained_by_action_alone (M5, ao) DROPS below the ceiling "
                        "(baseline ~0.994); grounding made O state-dependent.",
         "measured": arm1_m5, "threshold": GATE_M5_R2_CEIL, "direction": "upper",
         "control": "same M5 statistic the spike reported; ARM_0 stays ~0.994",
         "met": bool(m5_met)},
        {"name": "arm1_ao_consequence_structure_acquired",
         "description": "ARM_1 mean_within_pair_spearman (M6, ao) RISES above the floor "
                        "(baseline ~0); grounding made O consequence-structured.",
         "measured": arm1_m6, "threshold": GATE_M6_SPEARMAN_FLOOR, "direction": "lower",
         "control": "same M6 statistic the spike reported; ARM_0 stays ~0",
         "met": bool(m6_met)},
    ]
    ...
    if not grounding_took:
        label = "substrate_not_ready_requeue"
        outcome = "FAIL"
        summary = ("Grounding objective did NOT move the substrate DVs in ARM_1 "
                   "(M5 r2=%.4f vs ceil %.2f; M6=%.4f vs floor %.2f). Behavioural read "
                   "uninterpretable; re-queue with a stronger grounding objective."
                   % (arm1_m5, GATE_M5_R2_CEIL, arm1_m6, GATE_M6_SPEARMAN_FLOOR))
        edir = {"SD-080": "non_contributory", "SD-004": "non_contributory"}
        non_degenerate = False
        degeneracy_reason = ("ARM_1 grounding gate failed: substrate DVs did not move, so the "
                             "behavioural comparison cannot test whether grounding is load-bearing.")
    elif load_bearing:
        label = "grounding_load_bearing_defect_confirmed"
        outcome = "PASS"
        summary = ("Grounding O improves planning: ARM_1 harm_rate=%.4f beats ARM_0=%.4f and "
                   "ARM_2=%.4f by the pre-registered margin. SD-080's defect is behaviourally "
                   "load-bearing; SD-004's mechanism is repaired." % (a1_hr, a0_hr, a2_hr))
        ...
        outcome = "FAIL"
        summary = ("Grounding O moved the substrate DVs (gate fired: M5 r2=%.4f, M6=%.4f) but NOT "
                   "behaviour (ARM_1 harm_rate=%.4f vs ARM_0=%.4f, ARM_2=%.4f). SD-004's "
                   "semantic-grounding efficiency rationale is demoted; SD-080 stays candidate as "
                   "a correctness-not-capability finding." % (arm1_m5, arm1_m6, a1_hr, a0_hr, a2_hr))
...
def run_experiment(args):
    ...
    full_config = {
        ...
        "gate_m5_r2_ceil": GATE_M5_R2_CEIL, "gate_m6_spearman_floor": GATE_M6_SPEARMAN_FLOOR,
    ...
    manifest: Dict[str, Any] = {
        ...
        "evidence_direction": ("supports" if adj["outcome"] == "PASS"
                                     ...
                                     if adj["interpretation"]["label"] == "substrate_not_ready_requeue"
                                     else "mixed")),
...
def main(args):
    ...
    print(f"\nV3-EXQ-817 outcome: {manifest['outcome']}", flush=True)
