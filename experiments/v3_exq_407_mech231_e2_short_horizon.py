"""
EXQ-407: MECH-231 E2 short-horizon efference-copy discriminative pair

Claim: E2 is a short-horizon 1-step efference-copy forward model whose prediction
accuracy degrades FASTER than E1 across multi-step horizons.

This is the corrected reformulation of MECH-070. MECH-070 claimed E2's planning
horizon EXCEEDS E1's (longer horizon). MECH-231 claims the OPPOSITE: E2 is a
1-step efference-copy model that degrades faster with horizon than E1.
...
Design:
  ...
  - If MECH-231 holds: E2 (action-conditioned) should be better at h=1 (single-step
    grounding advantage) but degrade FASTER across horizons because chaining
    1-step errors compounds more rapidly than E1's context-based multi-step prediction.

Pre-registered acceptance thresholds:
  C1: E2 R2 at h=1 > E1 R2 at h=1   (E2 better at immediate step due to action conditioning -- same as EXQ-212)
  C2: E2 R2 at h=10 < E1 R2 at h=10 (E2 WORSE than E1 at medium horizon -- INVERTED from EXQ-212)
  C3: slope_e2 < slope_e1 (E2 slope more negative = degrades faster -- INVERTED from EXQ-212 abs comparison)
  C4: n_total_windows >= 50 (coverage sanity)

Note: EXQ-212 C1 PASS (r2_e2_h1=-19 > r2_e1_h1=-43) is proto-evidence for C1 direction.
EXQ-212 C2 FAIL was because E2 was much worse at h=10 -- exactly what MECH-231 predicts as PASS.
EXQ-212 C3: slope_e2 << slope_e1 (much more negative) -- now a PASS criterion under MECH-231.
...
def run_seed(seed: int, dry_run: bool) -> dict:
    ...
    # Phase 0: warmup                                                     #
    # ------------------------------------------------------------------ #
    print(f"  [EXQ-407] seed={seed} warmup={n_warmup} self_dim={self_dim}")
    agent.train()
    ...
    # ------------------------------------------------------------------ #
    agent.eval()
    print(f"  [EXQ-407] seed={seed} eval={n_eval}")
    ...
    # MECH-231 criteria (inverted from EXQ-212 MECH-070):
    # C1: E2 still better at h=1 (action-conditioning single-step advantage -- same direction)
    # C2: E2 WORSE than E1 at h=10 (short-horizon claim -- INVERTED)
    # C3: slope_e2 < slope_e1 (E2 degrades faster = more negative slope -- INVERTED from abs)
    # C4: coverage
    ...
    c3 = (not np.isnan(slope_e1)) and (not np.isnan(slope_e2)) and slope_e2 < slope_e1
    n_total = sum(n_windows_by_h.values())
    c4 = n_total >= THRESH_C4_MIN_WINDOWS

    print(f"  [EXQ-407] seed={seed} r2_e1_h1={r2_e1_h1:.4f} r2_e2_h1={r2_e2_h1:.4f} "
...
if __name__ == "__main__":
    ...
    print("[EXQ-407] MECH-231 E2 short-horizon efference-copy discriminative pair")
    ...
    result = {
        ...
        "acceptance_thresholds": {
            "c1_e2_r2_h1_gt_e1":      "e2 R2 at h=1 > e1 R2 at h=1 (action conditioning single-step advantage)",
            "c2_e2_r2_h10_lt_e1":     "e2 R2 at h=10 < e1 R2 at h=10 (E2 degrades more by h=10 -- short-horizon claim)",
            "c3_slope_e2_lt_slope_e1": "slope_e2 < slope_e1 (E2 slope more negative = faster degradation)",
            "c4_min_windows":         THRESH_C4_MIN_WINDOWS,
        },
    }

    result_str = "PASS" if overall_pass else "FAIL"
    print(f"[EXQ-407] RESULT: {result_str} "
    ...
    print(f"[EXQ-407] Written: {out_path}")
