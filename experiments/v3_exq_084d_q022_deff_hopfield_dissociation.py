"""
V3-EXQ-084d -- Q-022 D_eff / Hopfield Stability Dissociation Test

Claims: Q-022, MECH-118, MECH-119
Proposal: EVB-0069
Supersedes: V3-EXQ-084c (script deleted; all prior 084 iterations crashed due to
  compute_z_self_d_eff() and compute_self_maintenance_loss() not yet existing in
  REEAgent at time of run -- 2026-03-23/24. Both methods confirmed present as of
  2026-03-28.)
...
EXPERIMENT_TYPE = "v3_exq_084d_q022_deff_hopfield_dissociation"
...
def run(
    ...
    **kwargs,
    ...
    """
    print(
        f"\n[EXQ-084d] Q-022 D_eff/Hopfield dissociation  seed={seed}"
    ...
    # -----------------------------------------------------------------------
    print(f"[EXQ-084d] Phase 1: warmup ({warmup_episodes} eps)...", flush=True)
    agent.train()
    ...
    # -----------------------------------------------------------------------
    print(
        f"[EXQ-084d] Phase 2: memory collection ({memory_eps} eps)...",
        flush=True,
    )
    ...
    # -----------------------------------------------------------------------
    print(
        f"[EXQ-084d] Phase 3: analysis ({analysis_eps} eps)...",
        flush=True,
    )
    r1_d_eff: List[float] = []
    r1_stab:  List[float] = []
    ...
    print(
        f"\n[EXQ-084d] Results  (n={len(r1_d_eff)} steps):",
        flush=True,
    )
    print(
    ...
    if not c3_pass:
        failure_notes.append(
            ...
            f" -- noise_sigma={noise_sigma} may be too small to disperse z_self"
        )
    if not c4_pass:
        failure_notes.append(
    ...
    summary_markdown = (
        f"# V3-EXQ-084d -- Q-022 D_eff / Hopfield Stability Dissociation\n\n"
        f"**Status:** {status}\n"
