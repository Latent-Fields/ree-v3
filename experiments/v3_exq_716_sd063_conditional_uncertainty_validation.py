...
"""
V3-EXQ-716 -- SD-063 conditional predictive-uncertainty head validation.

EXPERIMENT_PURPOSE = diagnostic. PROMOTES NOTHING (claim_ids = []). This is the
substrate-readiness validation for the newly-implemented SD-063 head; it tests no
registered claim for governance weight. It self-routes a falsifiable verdict on
whether the head earns its keep.

WHAT SD-063 BUILT (validated here)
----------------------------------
...
PRE-REGISTERED READINESS + SELF-ROUTE (diagnostic, falsifiable)
---------------------------------------------------------------
...
If readiness MET:
  ...
  PASS (all three) -> label sd063_conditional_uncertainty_validated.
  else FAIL with the specific failing-criterion label.

Diagnostic; PROMOTES NOTHING. A PASS motivates clearing SD-063's v3_pending via
/governance (adjudicate this diagnostic first -- the self-route is a hypothesis).
...
See ree_core/predictors/e2_world_uncertainty.py (module under test),
    ...
    experiments/v3_exq_353_arc033_sd003_interventional_vs_observational.py
      (P0 encoder warmup + forward-model training loop template).
"""
...
def run_experiment(dry_run: bool = False) -> Dict[str, Any]:
    ...
    if not readiness_met:
        ...
        outcome, label, evidence_direction = "FAIL", "sd063_validation_degenerate", "non_contributory"
    elif c1_pass and c2_pass and c3_pass:
        outcome, label, evidence_direction = "PASS", "sd063_conditional_uncertainty_validated", "non_contributory"
    else:
        fails = []
        if not c1_pass:
        ...
        outcome, label, evidence_direction = "FAIL", "sd063_" + "+".join(fails), "non_contributory"
