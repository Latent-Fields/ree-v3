"""
V3-EXQ-626: goal-pipeline developmental-window diagnostic (4-arm dissociation).

Diagnostic follow-up to V3-EXQ-622 staged-curriculum FAIL (failure_autopsy
V3-EXQ-622 2026-06-01T07:55Z) and V3-EXQ-621a substrate-readiness PASS
(2026-06-01T05:55Z). Pre-registered design at
REE_assembly/evidence/planning/goal_pipeline_developmental_window_diagnostic_memo_2026-06-01.md
(REE_assembly master 12d2a48310). Substrate-of-record:
scaffolded_sd054_onboarding (REE_assembly/evidence/planning/sd_054_scaffolded_onboarding_substrate_design.md).

What this experiment does
-------------------------
4-arm dissociation on the scaffolded_sd054_onboarding substrate that isolates
which axis (drive-floor anneal, hazard introduction, or writer-freeze rescue)
is load-bearing for the V3-EXQ-622 S1+ z_goal collapse. use_dacc=True is
enabled on ALL arms for the first time in this lineage (621a + 622 both ran
with dACC unconfigured; dacc_bias_nonzero=0 in both manifests was a
measurement-OFF artefact, NOT a substrate failure).

Arms
----
ARM_A_FORMATION_ONLY        regression guard on 622 S0 PASS. drive_floor=0.9
                            constant across P0+P1+P2, hazard_food_attraction=0.0
                            constant, full goal pipeline ON throughout. baselines
                            dACC readout under non-trivial z_goal.

ARM_B_DRIVE_ANNEAL_ONLY     H1 isolation: drive_floor 0.9 -> 0.2 anneal during
                            P1, NO hazard introduction (HFA constant 0.0).
                            Tests whether drive-anneal alone collapses z_goal.

ARM_C_HAZARD_ONLY           hazard-axis isolation: drive_floor=0.9 constant,
                            hazard_food_attraction 0.0 -> 0.7 anneal during P1.
                            Tests whether hazard introduction alone collapses
                            z_goal.

ARM_D_WRITER_FROZEN_DURING_TRANSITION   Option-A test: freeze use_mech295 +
                            use_mech307 to False during P1 while annealing
                            BOTH drive_floor 0.9->0.2 AND HFA 0.0->0.7;
                            unfreeze at P2 start; measure z_goal at P2 start
                            and beyond. Tests whether writer-freeze across the
                            transition rescues z_goal state.

3 seeds [42, 43, 44]. P0(~30 ep) + P1(~30 ep) + P2(~30 ep). 200 steps/ep.

Pre-registered acceptance criteria (verbatim from memo Section 3)
-----------------------------------------------------------------
C1 (formation regression guard): ARM_A z_goal_median_last_window >= 0.05 on
   >= 2/3 seeds. FAIL = substrate regressed; routing /diagnose-errors.
...
C4 (writer-freeze rescue): ARM_D z_goal_median_last_window AT P2 START >=
   0.5 * ARM_A. PASS = writer-freeze across the transition preserves z_goal.
C5 (consumer readout under non-trivial z_goal): in the arm with highest
   z_goal during P2 (ARM_A or ARM_D), dacc_bias_nonzero_steps_per_episode_mean
   >= 1.0 on >= 2/3 seeds.

Pre-registered metrics (verbatim from memo Section 3)
------------------------------------------------------
...
- approach_commit_rate AND approach_commit_at_high_z_goal_rate
  (conditional on z_goal_norm > 0.05 at commit tick -- replaces 622's
  criterion-trivial S3 metric)
- mean_episode_length              (survival check)

Interpretation grid (verbatim from memo Section 3)
---------------------------------------------------
| C2 | C3 | C4  | C5   | Reading                                                       | Routing                                                                                                          |
|----|----|-----|------|---------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------|
|PASS|FAIL| any | PASS | drive-anneal alone collapses; consumer reads fine            | /implement-substrate AMEND on scaffolded_sd054_onboarding (sustained-drive EMA at low drive_floor) -- conservative |
|FAIL|PASS| any | PASS | hazard alone collapses; competing-representation implicated  | /implement-substrate AMEND with risk introduction decoupled in time from drive anneal                            |
|PASS|PASS|PASS | any  | both axes contribute AND writer-freeze rescues; Option-A confirmed | /implement-substrate AMEND adding writer-freeze P1 sub-phase; bounded, single-purpose                          |
|any |any | any | FAIL | non-trivial z_goal does not reach dACC                       | /failure-autopsy on dACC readout chain (E3 score_bias composition, dACC sub-weights)                             |

claim_ids = []  (substrate-readiness diagnostic; NOT load-bearing evidence
weighting per CLAIM_IDS Accuracy Rule. Same epistemic class as 621a + 622.)
experiment_purpose = "diagnostic".
supersedes = null  (new question, new EXQ number).

References:
- failure_autopsy_V3-EXQ-622_2026-06-01.md  (REE_assembly master d178efcbf3)
- goal_pipeline_developmental_window_diagnostic_memo_2026-06-01.md  (REE_assembly master 12d2a48310)
- z_goal_collapse_triage_2026-05-31.md       (substrate-ownership memo)
- sd_054_scaffolded_onboarding_substrate_design.md
- Closure-plan docs: goal_pipeline_plan.md GAP-4, behavioral_diversity_isolation_plan.md GAP-C
"""
...
# -- Header constants --------------------------------------------------------
EXPERIMENT_TYPE = "v3_exq_626_goal_pipeline_developmental_window_diagnostic"
QUEUE_ID = "V3-EXQ-626"
...
# -- Pre-registered thresholds (memo Section 3, acceptance criteria) ---------
...
# dACC bundle is computed when use_dacc=True regardless of dacc_weight; the
# weights here are non-zero so the bundle contributes to score_bias and the
# C5 dacc_bias_nonzero_steps metric is meaningful (bundle values > 1e-6).
DACC_WEIGHT = 0.5
...
def evaluate_acceptance(cells: List[Dict[str, Any]]) -> Dict[str, Any]:
    """Evaluate C1..C5 per memo Section 3. Returns flat dict for manifest."""
    ...
    return {
        ...
        "overall_pass": bool(c1_pass and (c2_pass or c3_pass or c4_pass) and c5_pass),
    }
...
def emit_manifest(
    ...
    run_id = f"v3_exq_626_goal_pipeline_developmental_window_diagnostic_{ts}_v3"
    ...
    manifest = {
        ...
        "timestamp_utc": ts,
        "dry_run": dry_run,
        "related_queue_ids": ["V3-EXQ-621a", "V3-EXQ-622"],
        "design_memo": (
...
def main(args: argparse.Namespace) -> Tuple[str, Optional[str]]:
    ...
    outcome = "PASS" if acceptance["overall_pass"] else "FAIL"
    print(
        f"Overall: {outcome} | C1={acceptance['C1_arm_a_formation_regression_guard']['pass']} "
