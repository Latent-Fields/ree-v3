...
"""
EXQ-187a: MECH-153 Validation -- Supervised Context-Labeling Training for E1 ContextMemory
...
BUG FIX (187a supersedes 187): EXQ-187 never called context_memory.write()
  during training. Memory slots stayed at randn*0.01 initialization, making
  attention over slots produce uniform/constant output regardless of z_world
  query. Supervised terrain_loss converged to mean(targets) but could not
  differentiate contexts. Fix: call context_memory.write(obs_state) each step
  during Phase 0 to populate memory with diverse environment states.
...
EXPERIMENT_TYPE = "v3_exq_187a_mech153_supervised_context_labeling"
...
def _run_seed(
    ...
    for ep in range(warmup_episodes):
        ...
        for _ in range(steps_per_episode):
            ...
            # FIX (187a): Populate ContextMemory with diverse environment states.
            # Without this, memory slots stay at initialization (randn*0.01),
            # making attention output constant regardless of z_world query.
...
def run(
    ...
    summary_markdown = (
        f"# V3-EXQ-187a -- MECH-153 Supervised Context-Labeling Validation\n\n"
        f"**Status:** {status}\n"
    ...
    print(f"\n[V3-EXQ-187a] Final results:", flush=True)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="EXQ-187a: MECH-153 supervised context-labeling validation (fix: populate ContextMemory)"
    )
