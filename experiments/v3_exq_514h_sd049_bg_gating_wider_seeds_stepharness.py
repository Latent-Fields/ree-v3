...
"""V3-EXQ-514h: SD-049 BG gating wider-seed sweep under StepHarness.

Next-letter re-run after V3-EXQ-514g was hard-killed by SIGTERM mid-seed-43 on
ree-cloud-1 at 2026-05-08T19:26Z (~93 min in). 514g phantom-PASS (stdout-derived
heuristic firing 4 seconds after SIGTERM) was retracted to INTERRUPTED in
REE_assembly commit 13c142debb (2026-05-10). Script logic and design are
unchanged from 514g; only the identifier and SUPERSEDES change.
...
def main_run(dry_run: bool = False) -> Dict:
    ...
    t0_total = time.time()
    for seed in seeds:
        print(f"[514h] seed={seed} episodes={n_eps}", flush=True)
        t0 = time.time()
