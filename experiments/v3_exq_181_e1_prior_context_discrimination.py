...
"""
EXQ-181: E1 Prior Context Discrimination Diagnostic

Context: SD-016 specifies a frontal cue-indexed integration circuit where E1's ContextMemory
is queried with z_world to produce cue-specific association context feeding E2 (action bias)
and E3 (terrain precision). Before implementing SD-016, this diagnostic verifies whether E1's
existing generate_prior() output already carries context-discriminative information sufficient
to distinguish hazard-proximate from hazard-distal sensory contexts.

Hypothesis: E1.generate_prior() output is contextually discriminative -- priors differ between
high-harm-gradient and low-harm-gradient z_world contexts, and predict harm gradient linearly.

Design: Train REEAgent for 100 warmup episodes. Then collect generate_prior() outputs across
100 collection episodes, classifying steps as Context A (hazard-proximate) or B (hazard-distal)
by hazard_field_view.max(). Measure cosine similarity between context mean priors and R^2 of
harm gradient prediction from prior vectors.

Pass criteria:
- C1 PASS: cosine_sim(mean_prior_A, mean_prior_B) < 0.85 (priors differ by context)
- C2 PASS: ridge regression R^2 > 0.3 (prior predicts harm gradient)
...
EXPERIMENT_TYPE = "v3_exq_181_e1_prior_context_discrimination"
...
def get_hazard_max(obs_dict: Dict, world_obs: Optional[torch.Tensor]) -> float:
    """
    Return the max value in the hazard proximity field for the current step.
    Tries harm_obs first (CausalGridWorldV2 emits this), then falls back
    to world_obs first-25-dims (hazard_field embedded in proxy-field mode).
    """
    # Try harm_obs first (hazard_field is first 25 dims of harm_obs[51])
    ...
    # Fallback: explicit key from env
    if "hazard_field_view" in obs_dict:
        hfv = obs_dict["hazard_field_view"]
        if hasattr(hfv, 'shape'):
            return float(hfv.max().item())
    # Fallback: first 25 dims of world_obs (proxy-field mode embeds hazard_field there
    # after local_view[175] + contamination_view[25], i.e. dims 200-224 of world_obs[250])
    # The hazard_field_view is at world_obs[200:225] in proxy mode (world_obs_dim=250).
...
def get_harm_scalar(obs_dict: Dict) -> float:
    """
    Return a scalar harm signal for the current step.
    Uses harm_obs[50] (harm_exposure is at index 50 in 51-dim harm_obs: hazard_field_view[25] + resource_field_view[25] + harm_exposure[1]) when available.
    """
    if "harm_obs" in obs_dict:
        harm_obs = obs_dict["harm_obs"]
...
def ridge_r2(
    ...
    """
    R^2 of ridge regression X -> y, computed on held-out test fraction.
    X: [N, D], y: [N]
...
def _run_seed(
    ...
    # Standard optimizer (no separate harm_eval / benefit_eval heads training here)
    ...
    # Lists per context class
    priors_A: List[torch.Tensor] = []   # hazard-proximate
    priors_B: List[torch.Tensor] = []   # hazard-distal
    harm_A:   List[float]        = []
    harm_B:   List[float]        = []
    ...
    for ep in range(collect_episodes):
        ...
        for _ in range(steps_per_episode):
            ...
            # Build the concatenated [z_self, z_world] for generate_prior
            z_self_det  = latent.z_self.detach()   # [1, self_dim]
            z_world_det = latent.z_world.detach()  # [1, world_dim]
            total_state = torch.cat([z_self_det, z_world_det], dim=-1)  # [1, self_dim+world_dim]

            with torch.no_grad():
                prior = agent.e1.generate_prior(total_state)  # [1, world_dim]

            # Classify context by hazard field max
    ...
    # C1: cosine similarity between mean priors
    if n_A >= 10 and n_B >= 10:
        mean_A = torch.stack(priors_A).mean(dim=0)  # [world_dim]
        mean_B = torch.stack(priors_B).mean(dim=0)  # [world_dim]
        cos_sim_val = float(
    ...
    # C2: ridge regression prior -> harm_scalar, combining both contexts
    all_priors = priors_A + priors_B
    all_harms  = harm_A + harm_B
    if len(all_priors) >= 8:
        X = torch.stack(all_priors).float()   # [N, world_dim]
        y = torch.tensor(all_harms, dtype=torch.float32)  # [N]
        r2_val = ridge_r2(X, y)
    ...
    return {
        ...
        "n_context_B": n_B,
        "n_skipped": n_skipped,
        "cos_sim_AB": cos_sim_val if not (cos_sim_val != cos_sim_val) else None,
...
def run(
    ...
    """
    Run E1 prior context discrimination diagnostic across seeds.

    Args:
        seeds:             list of random seeds (default [42, 7, 11])
        warmup_episodes:   number of training warmup episodes per seed
        collect_episodes:  number of collection episodes per seed
        dry_run:           if True, run minimal configuration (no file write)

    Returns:
        Standard result dict with status, metrics, summary_markdown, etc.
    """
    if seeds is None:
        seeds = [42, 7, 11]
    ...
    # Mean cosine_sim (only over seeds that had enough context steps)
    cos_vals = [
    ...
    # Global pass/fail: majority vote across seeds
    crit1_pass_global = seeds_c1_pass >= (n_seeds + 1) // 2   # majority (>= 2/3)
    ...
    # Criterion interpretation strings (ASCII only)
    if crit1_pass_global:
        ...
        c1_interp = (
            ...
            " SD-016 cue-indexed retrieval will require explicit training of ContextMemory."
        )

    if crit2_pass_global:
        c2_interp = (
            f"C2 PASS: harm_r2={avg_harm_r2:.4f} > 0.3 in {seeds_c2_pass}/{n_seeds} seeds."
            " E1 prior predicts harm gradient linearly -- cue context carries harm information."
        )
    else:
        c2_interp = (
            f"C2 FAIL: harm_r2={avg_harm_r2:.4f} <= 0.3 (passed {seeds_c2_pass}/{n_seeds} seeds)."
            " E1 prior does NOT reliably predict harm gradient."
            " SD-016 frontal integration will need supervised context-harm association training."
        )
    ...
    summary_markdown = (
        f"# V3-EXQ-181 -- E1 Prior Context Discrimination Diagnostic\n\n"
        f"**Status:** {status}\n"
        f"**Claims:** MECH-150, ARC-041\n"
        f"**Seeds:** {seeds}\n\n"
        f"## Context\n\n"
        f"SD-016 specifies a frontal cue-indexed integration circuit using E1's ContextMemory"
        f" to produce context-specific priors feeding E2 (action bias) and E3 (terrain precision)."
        f" This diagnostic tests whether the EXISTING generate_prior() interface already"
        f" discriminates hazard-proximate from hazard-distal z_world contexts, before"
        f" implementing the SD-016 extract_cue_context() extension.\n\n"
        f"## Design\n\n"
        f"**Phase 0 (warmup):** {warmup_episodes} episodes x 200 steps. Standard REEAgent"
        f" training (E1 + E2 losses, Adam lr={lr}, alpha_world={alpha_world}).\n\n"
        f"**Phase 1 (collection):** {collect_episodes} episodes x 200 steps. Random actions."
        f" Each step: call agent.e1.generate_prior([z_self, z_world]) to get prior [1, world_dim]."
        f" Classify step as Context A (hazard_field_view.max() > 0.3),"
        f" Context B (hazard_field_view.max() < 0.1), or ambiguous (skip).\n\n"
    ...
    print(f"\n[V3-EXQ-181] Final results:", flush=True)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="EXQ-181: E1 prior context discrimination diagnostic"
    )
    parser.add_argument(
        "--seeds", type=int, nargs="+", default=[42, 7, 11],
        help="Random seeds to run (default: 42 7 11)",
    )
    parser.add_argument(
        "--warmup", type=int, default=100,
        help="Warmup episodes per seed (default: 100)",
    )
    parser.add_argument(
        "--collect", type=int, default=100,
        help="Collection episodes per seed (default: 100)",
    )
    parser.add_argument(
        "--dry-run", action="store_true",
        help="Smoke test: 1 seed, 3 warmup eps, 5 collect eps. No output file.",
    )
    ...
    if args.dry_run:
        ...
        args.warmup  = 3
        args.collect = 5
        print("[DRY-RUN] 1 seed, 3 warmup, 5 collect", flush=True)
