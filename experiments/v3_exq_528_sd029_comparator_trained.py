...
"""V3-EXQ-528: SD-029 Comparator with Extended Training Budget

Supersedes V3-EXQ-523a (INCONCLUSIVE_UNDERTRAINED: max 8000 steps was insufficient
for HarmEncoder to reach r2>=0.85; ARM_1 p1_r2=0.12 after full budget).

Key changes vs EXQ-523a:
  1. GRAD_MAX_WINDOWS: 40->120 (24000 max steps per phase)
  2. GRAD_THRESHOLD: 0.85->0.75 (more achievable for initial validation)
  3. P0 episodes: 80->120 (larger replay buffer; ~24000 transitions)
  4. Same reef + external hazard injection design that confirmed balanced events
     (ARM_1 EXQ-523a: n_self=336, n_ext=187 -- substrate works)

Three phases per arm/seed:
  P0: epsilon=0.4 heuristic + aggressive external injection (interval=5, prob=1.0)
  P1: Train HarmEncoder (harm_obs->z_harm_s) graduation gate r2>=0.75 for 5 windows
  P2: Train E2HarmSForward on frozen z_harm_s, same graduation gate
  P3: Evaluate causal comparator cf_gap

Two arms:
  ARM_0: baseline (reef_enabled=False, no injection)
  ARM_1: reef + hazard_food_attraction=0.7 + aggressive injection

Acceptance criteria:
  C0: ARM_1 n_self >= 20 AND n_ext >= 20 per seed
  C1: ARM_1 harm_forward_r2 >= 0.9
...
EXPERIMENT_TYPE = "v3_exq_528_sd029_comparator_trained"
QUEUE_ID = "V3-EXQ-528"
CLAIM_IDS = ["SD-029", "MECH-256"]
...
if __name__ == "__main__":
    ...
    t0 = time.time()
    print("V3-EXQ-528 SD-029 Comparator with Extended Training Budget")
