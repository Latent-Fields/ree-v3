...
"""
V3-EXQ-171 -- MECH-033: E2 forward-prediction kernels seed hippocampal rollouts.

Claim:    MECH-033
Why now:  active_conflict, directional_conflict_alert
Prior:    EXQ-055 PASS (action-object chaining reduces harm 67x vs random on single agent)
          EXQ-124 FAIL (harm_rate_CHAIN=0.621 vs ABLATED=0.057; root cause: 400-episode
          warmup insufficient, E2 world_forward not well trained before eval begins)

This experiment fixes EXQ-124's root cause: use 600 warmup episodes (vs 400 in EXQ-124)
and add an explicit E2 convergence quality check (world_forward_r2 > THRESH_C3_E2_QUALITY)
before declaring a result.  The kernel chaining effect (MECH-033) requires that E2 be
minimally trained -- without that, the chain evaluation evaluates random noise and may
actively hurt policy quality.

Conditions (inline architecture, NOT full REEAgent):
  KERNEL_CHAIN: For each step, evaluate 5 candidate actions by rolling out k=3 steps
    with the E2 forward model and scoring cumulative predicted harm.  Select the action
    minimizing predicted harm total.
  ABLATED: Select action uniformly at random (no E2 chaining).
...
Training (600 warmup episodes, both conditions):
  ...
  Both with Adam lr=3e-4.  Separate optimizer.
...
E2 convergence check (after warmup):
  ...
  This is C3: if R^2 < THRESH_C3_E2_QUALITY, E2 is not trained enough to support
  valid kernel chaining, and the experiment is FAIL (inconclusive, not a true FAIL
  of MECH-033).
...
--------------------------
C1: harm_rate_CHAIN < harm_rate_ABLATED (both seeds).
    Chaining must reduce harm -- direction must replicate.

C2: harm_reduction_delta >= THRESH_C2_MIN_REDUCTION (both seeds averaged).
    Mean delta = mean(harm_rate_ABLATED - harm_rate_CHAIN) across seeds >= threshold.
    THRESH_C2_MIN_REDUCTION = 0.01 (1 pp absolute reduction).

C3: world_forward_r2 >= THRESH_C3_E2_QUALITY (both seeds).
    E2 must be minimally trained. If this fails, outcome is FAIL (inconclusive -- not
    a refutation of MECH-033, just insufficient training).
    THRESH_C3_E2_QUALITY = 0.20 (weak positive R^2).

C4: n_harm_contacts_ABLATED >= THRESH_C4_MIN_CONTACTS (both seeds).
    Sufficient harm contacts in ablated condition for reliable harm rate estimation.
    THRESH_C4_MIN_CONTACTS = 5

PASS: C1 + C2 + C3 + C4 (both seeds) -- E2 chaining reduces harm, replicated.
...
Protocol:
  2 conditions x 2 seeds x 650 total episodes x 100 steps/ep.
  Estimated runtime: 2 x 2 x 650 x 0.005 min/ep = ~13 min (lightweight CPU).
"""
...
THRESH_C4_MIN_CONTACTS   = 5      # C4: n_harm_contacts_ABLATED data quality
...
SEEDS                = [42, 123]
CONDITIONS           = ["KERNEL_CHAIN", "ABLATED"]
...
def _train_models(
    ...
    for ep in range(warmup_eps):
        ...
        if ep % 100 == 0 or ep == warmup_eps - 1:
            print(
                f"  [warmup] seed={seed} ep={ep}/{warmup_eps}"
                f" e2_loss={e2_loss.item():.5f} harm_loss={harm_loss.item():.5f}",
                flush=True,
            )
    ...
    print(f"  [warmup] seed={seed} world_forward_r2={r2:.4f}", flush=True)
...
def _evaluate_criteria(
    ...
    # C1: harm_rate_CHAIN < harm_rate_ABLATED (both seeds)
    c1 = all(chain[i]["harm_rate"] < ablated[i]["harm_rate"] for i in range(n_s))

    # C2: mean(harm_rate_ABLATED - harm_rate_CHAIN) >= THRESH_C2_MIN_REDUCTION
    deltas = [ablated[i]["harm_rate"] - chain[i]["harm_rate"] for i in range(n_s)]
    mean_delta = sum(deltas) / max(len(deltas), 1)
    c2 = mean_delta >= THRESH_C2_MIN_REDUCTION

    # C3: world_forward_r2 >= THRESH_C3_E2_QUALITY (both seeds, from CHAIN results)
    c3 = all(chain[i]["world_forward_r2"] >= THRESH_C3_E2_QUALITY for i in range(n_s))

    # C4: n_harm_contacts_ABLATED >= THRESH_C4_MIN_CONTACTS (both seeds)
    c4 = all(ablated[i]["n_harm_events"] >= THRESH_C4_MIN_CONTACTS for i in range(n_s))

    criteria = {
        "C1_chain_harm_rate_lower_than_ablated":  c1,
        "C2_mean_harm_reduction_above_threshold": c2,
        "C3_e2_world_forward_r2_quality":         c3,
        "C4_ablated_sufficient_harm_contacts":    c4,
    ...
    if c1 and c2 and c3 and c4:
        ...
        # E2 trained, sufficient data, but chaining hurts or ties -- direction wrong
        outcome           = "FAIL"
...
def run_experiment(dry_run: bool = False) -> Dict:
    print("=== V3-EXQ-171: MECH-033 Kernel Chain Pair ===", flush=True)
    print(f"Conditions: {CONDITIONS}  Seeds: {SEEDS}", flush=True)
    print("Pre-registered thresholds:", flush=True)
    print(f"  C2 THRESH_C2_MIN_REDUCTION = {THRESH_C2_MIN_REDUCTION}", flush=True)
    print(f"  C3 THRESH_C3_E2_QUALITY    = {THRESH_C3_E2_QUALITY}", flush=True)
    print(f"  C4 THRESH_C4_MIN_CONTACTS  = {THRESH_C4_MIN_CONTACTS}", flush=True)
    print(
    ...
    results: Dict[str, List[Dict]] = {"KERNEL_CHAIN": [], "ABLATED": []}

    for seed in SEEDS:
        print(f"\n=== Warmup training: seed={seed} ===", flush=True)
        ...
        for condition in CONDITIONS:
            print(f"\n=== Eval: condition={condition} seed={seed} ===", flush=True)
            r = _run_condition(
                seed=seed,
                condition=condition,
                world_enc=world_enc,
    ...
    summary_metrics = {
        ...
        "ablated_harm_rate":           _mean("ABLATED",      "harm_rate"),
        "chain_n_harm_events":         _mean("KERNEL_CHAIN", "n_harm_events"),
        "ablated_n_harm_events":       _mean("ABLATED",      "n_harm_events"),
        "mean_world_forward_r2":       _mean("KERNEL_CHAIN", "world_forward_r2"),
        "mean_harm_reduction_delta":   (
            _mean("ABLATED", "harm_rate") - _mean("KERNEL_CHAIN", "harm_rate")
        ),
    }

    run_id = (
        "v3_exq_171_mech033_kernel_chain_pair_"
    ...
    pack = {
        ...
        "seeds": SEEDS,
        "scenario": (
            "Two-condition E2 kernel chaining test for MECH-033."
            ...
            " ABLATED: select action uniformly at random."
            ...
            " 2 conditions x 2 seeds, shared warmup per seed = 4 eval cells."
            " Fixes EXQ-124 root cause: 600 vs 400 warmup episodes."
        ),
        "interpretation": (
            "PASS => MECH-033 supported at inline V3 scale: E2 forward model kernels"
            " provide a usable prediction substrate for action selection that reduces harm."
            " Effect size (harm_reduction_delta) exceeds 1 pp threshold."
            ...
            " Weakens MECH-033 at this architecture/scale. E2 predictions may be"
            " misleading the greedy policy (adversarial gradient toward harm-rich latents)."
            " FAIL (C1+C3+C4 pass, C2 fails) => Direction correct but effect too small"
            " to cross 1 pp threshold. Mixed evidence -- may need longer eval or larger"
            " harm density to see effect clearly."
        ),
