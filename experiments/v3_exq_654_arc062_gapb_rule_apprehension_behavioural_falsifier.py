...
"""
V3-EXQ-654 -- arc_062_rule_apprehension:GAP-B behavioural falsifier for
MECH-309 / ARC-062 (the non-Bayesian rule-CREATOR present-vs-absent test).

IGW-20260609-004. Plan-of-record:
REE_assembly/evidence/planning/arc_062_rule_apprehension_plan.md GAP-B node.

One-line framing
----------------
MECH-309: "trainers weight rules they do not invent." Without a non-Bayesian
rule-creator at the policy layer, the policy collapses to the smoothest single
regime good-enough across the whole state space. ARC-062 is the V3-tractable
architectural slot for the rule-creator; ARC-063 v1 (CandidateRuleField) is the
non-Bayesian creator that mints distinct subspace-partitioned rule slots so the
rule_state handed to SD-033a LateralPFCAnalog is DIFFERENTIATED BY CONSTRUCTION.
This experiment is the behavioural falsifier: does the rule-creator break the
monomodal collapse?

Single-variable arm contrast (user-confirmed 2026-06-09)
--------------------------------------------------------
...
The ONLY swept variable is use_candidate_rule_field:
  ...
  ARM_ON:  use_candidate_rule_field=True  -> the agent sets
           use_candidate_rule_source=True so LateralPFCAnalog.update() consumes
           crf_source -- the field's DIFFERENTIATED active-rule-stack rule_state
           (the literal 598b C3 trainable_not_monomodal fix).
...
the per-candidate cand_world_summaries the bias channels consume is keyed on the
candidate's FIRST action only (e2.world_forward(z0, a_first)), and compute_bias
broadcasts a single rule_state across all K candidates. So within a first-action
class (same first action) every candidate receives an IDENTICAL rule bias.
Therefore the rule-creator moves WHICH CLASS is committed (the across-class /
committed-class axis), and has NO path to within-class representative selection
(decided by primary scores + MECH-341 argmin, identical in both arms).

The matched readout is therefore committed-class diversity, NOT
within-class-representative diversity. The GAP-A fix (candidate_summary_source=
e2_world_forward, trained e2) is exactly what makes it non-vacuous: pre-GAP-A the
candidate pool was class-collapsed (cand_world_pairwise_dist ~0), so the class
bias was degenerate and committed-class diversity was non_contributory (the prior
604a/624a/614d/614e lineage). The 614e autopsy Learning #2 ("use
within-class-representative, not committed-class") applies to the MECH-341
within-class TEMPERATURE lever, which samples WITHIN a class; it does not transfer
to the class-keyed rule bias. Within-class-representative entropy is retained here
as a SECONDARY NEGATIVE CONTROL (expected ~null, confirming the bias is
class-keyed and the rule-creator's signal lives in the committed-class axis).

Per P1 tick (e2 FROZEN -- trained only in P0):
  committed_class = executed action argmax class (614e parity).
  pre_e3_classes  = distinct first-action classes among the candidate pool.
  consumed_summary_pairwise_dist = mean pairwise L2 over the consumed
                    cand_world_summaries (the 649 GAP-A statistic; readiness that
                    the class bias is non-degenerate).
  rep_signature   = selected trajectory's first H_SIGNATURE action-class argmaxes,
                    grouped by its first-action class (SECONDARY negative control).
  ARM_ON: CandidateRuleField get_state (crf_n_active, crf_max_pairwise_rule_dist).

Per seed (P1 window):
  committed_class_entropy_nats = entropy of the executed committed-class
                    distribution. <-- PRIMARY DV.
  frac_pre_ge2 = frac of P1 ticks with >= 2 candidate first-action classes.
  consumed_summary_pairwise_dist_mean = GAP-A divergence (readiness).
  mean_within_class_rep_entropy_nats = SECONDARY negative control.
...
----------------------------------
  C1 (READINESS / non-vacuity -- protects against a false weakens; C1 fail
     ...
     (a) committed-class axis exercisable: frac_pre_ge2 > FRAC_PRE_GE2_FLOOR on a
         majority (>= 2/3) of seeds in BOTH arms (committed-class entropy is
         structurally 0 otherwise). SAME-statistic family as the DV (class
         multiplicity bounds class entropy).
     (b) GAP-A divergence real: consumed_summary_pairwise_dist_mean >
         CONSUMED_SPREAD_FLOOR (and bounded < CONSUMED_MAGNITUDE_CEIL) on a
         majority of seeds in BOTH arms -- e2.world_forward gives action-conditional
         spread so the class bias is non-degenerate.
     (c) ARM_ON manipulation live: the CandidateRuleField minted distinct rules --
         per-tick crf_n_active >= CRF_N_ACTIVE_FLOOR on a majority of P1 ticks AND
         crf_max_pairwise_rule_dist > CRF_DIST_FLOOR, on a majority of ARM_ON seeds.
...
Overall outcome
---------------
  PASS  = C1 (non-vacuous) AND C2 (committed-class lift). The rule-creator's
          DIFFERENTIATED rule_state propagates to greater committed-class
          diversity -> supports MECH-309 + ARC-062.
  FAIL (C1 holds, C2 fails) = lever operative, manipulation live, GAP-A divergence
          real, but the differentiated rule_state adds no marginal committed-class
          diversity over the collapsed baseline -> weakens MECH-309 + ARC-062.
  FAIL (C1 fails) = substrate not exercisable (class axis collapsed / GAP-A
          divergence absent / field minted no distinct rules) -> non_contributory /
          substrate_not_ready_requeue; route to substrate enrichment / re-queue.

Claims: [MECH-309, ARC-062]. experiment_purpose = evidence. evidence_direction_per_claim
applies the same direction to both (the single DV tests the rule-creator-necessity
claim MECH-309 and its ARC-062 weak-reading instantiation jointly).

Phases / budget
---------------
P0 (60 ep, e2 TRAINED online via SD-056 contrastive; matches the V3-EXQ-649 /
   604a / 648a warmup budget): builds the action-conditional e2.world_forward
   divergence the GAP-A candidate_summary_source consumes. P1 (60 ep, e2 FROZEN,
   instrumentation ON): the behavioural measurement window.
Budget: 2 arms x 3 seeds x 120 ep x 200 steps = 144k steps total. ~3-4 h. The C2
paired lift is within-run (same seed, same machine) -> machine_affinity "any".

See REE_assembly/evidence/planning/arc_062_rule_apprehension_plan.md (GAP-B),
REE_assembly/evidence/planning/failure_autopsy_V3-EXQ-614e_2026-06-07.md
(Learning #2), V3-EXQ-649 (GAP-A shared-channel readiness; P0 e2-training harness
mirrored here), ree-v3/CLAUDE.md (ARC-063 v1 CandidateRuleField + ARC-062
GAP-A/B/C/D + modulatory-bias-selection-authority + SD-056 entries),
...
# C1(a) readiness: committed-class axis exercisable (>= 2 candidate first-action
# classes available); matches the 614e frac_pre_ge2 gate.
...
# C1(c) readiness: ARM_ON rule field minted distinct rules AND actively
# contributed a non-zero differentiated rule_state on a meaningful fraction of
# P1 ticks. Differentiation is "by construction" (pinned-distinct directions), so
# minting >= CRF_MIN_MINTED distinct rules is the differentiation; requiring >= 2
# SIMULTANEOUSLY active rules would be the wrong gate (the rule_state varies
# across contexts even with one rule active at a time). The frac-active floor
# guards that the field is not inert (rule_state mostly zeros == no manipulation).
CRF_MIN_MINTED = 2              # distinct rules created over the P1 window
CRF_N_ACTIVE_FLOOR = 1          # >= 1 active rule => non-zero rule_state this tick
CRF_FRAC_ACTIVE_FLOOR = 0.30    # fraction of P1 ticks the field fired a rule_state
CRF_DIST_FLOOR = 1e-3           # reported diagnostic (pinned-distinct separability)

# Only classes committed to at least this many P1 ticks feed the unweighted mean
...
SEEDS = [42, 43, 44]
P0_WARMUP_EPISODES = 60          # SD-056 e2 contrastive warmup (V3-EXQ-649 budget)
...
# IDENTICAL env to V3-EXQ-614e / 614d / 611 (SD-054 reef + hazard_food_attraction
...
ARMS: List[Dict[str, Any]] = [
    ...
    {
        "arm_id": "ARM_ON",
        "label": "rule_creator_present_differentiated_crf_source",
...
def _make_agent(env: CausalGridWorldV2, use_candidate_rule_field: bool) -> REEAgent:
    ...
    non-zero per-candidate E3 score-bias. ARM_OFF keeps the legacy collapsed
    delta/world EMA rule_state source; ARM_ON's field sets use_candidate_rule_source
    so LateralPFCAnalog consumes the differentiated crf_source. candidate_summary_source
    = e2_world_forward on BOTH arms (GAP-A; e2 trained online in P0).
    """
    cfg = REEConfig.from_dims(
        ...
        # SD-033a LateralPFCAnalog with the bias head UN-ZEROED so the rule_state
        # maps to a non-zero per-candidate bias (else compute_bias == 0 and the
        # whole test is vacuous). Fixed seeded random projection; not trained.
...
def _run_seed_arm(
    ...
    # PRIMARY DV + readiness accumulators (P1).
    ...
    # ARM_ON differentiation + bias diagnostics (P1).
    ...
    for ep in range(total_train_eps):
        ...
        for _step in range(steps_per_episode):
            ...
            # SD-056 e2 training -- P0 ONLY (e2 frozen in P1 for a stable measurement).
    ...
    # Manipulation-live: minted >= CRF_MIN_MINTED distinct rules (the by-construction
    # differentiation) AND fired a non-zero rule_state on >= CRF_FRAC_ACTIVE_FLOOR of
    # P1 ticks (not inert). crf_max_pairwise_rule_dist is a reported diagnostic.
    ...
    return {
        ...
        "mean_lateral_pfc_bias_abs": round(mean_lateral_pfc_bias_abs, 6),
    }
...
def run_experiment(
    ...
    for arm in ARMS:
        print(
            ...
            f"(P0={p0_episodes} ep e2-train, P1={p1_episodes} ep measure, "
    ...
    interpretation = {
        ...
        "preconditions": [
            {
                ...
                "kind": "readiness",
                "description": (
                    "frac of P1 ticks with >= 2 candidate first-action classes "
                    "exceeds floor on a majority of seeds in BOTH arms (committed-"
                    "class entropy is structurally 0 otherwise). SAME-statistic "
            ...
            },
            {
                ...
                "description": (
                    ...
                    "SPREAD clears the floor and stays bounded on a majority of seeds "
                    "in BOTH arms -- the class bias is non-degenerate. Same range "
                    "statistic the 649 GAP-A readiness asserts."
                ),
            ...
            {
                "name": "arm_on_rule_field_differentiated",
                "kind": "readiness",
                "description": (
                    "ARM_ON CandidateRuleField minted >= CRF_MIN_MINTED distinct "
                    "rules (the by-construction differentiation) AND fired a non-zero "
                    "rule_state on >= CRF_FRAC_ACTIVE_FLOOR of P1 ticks (not inert), "
                    "on a majority of ARM_ON seeds. Positive control that the "
                    "manipulation is live (else the rule_state is mostly zeros and "
                    "the rule-creator contributes nothing -> substrate_not_ready)."
                ),
                "control": "ARM_ON crf frac-active (non-zero rule_state) + crf_n_minted_total",
                "measured": float(
        ...
        "criteria_non_degenerate": {
            ...
            "C1c_arm_on_differentiated": bool(c1c_holds),
    ...
    return {
        ...
        "acceptance_criteria": {
            ...
            "C1c_arm_on_rule_field_differentiated": c1c_holds,
        ...
        "secondary_negative_control_not_load_bearing": {
            "note": (
                ...
                "bias is class-keyed (per-candidate summary is first-action-keyed; "
            ...
            "arm_off_lateral_pfc_bias_abs_mean": round(
                _mean([r["mean_lateral_pfc_bias_abs"] for r in off_rows]), 6
            ),
            "arm_on_lateral_pfc_bias_abs_mean": round(
                _mean([r["mean_lateral_pfc_bias_abs"] for r in on_rows]), 6
            ),
        },
        "interpretation_grid": {
            "PASS_C1_C2": (
                "The non-Bayesian rule-creator's DIFFERENTIATED rule_state "
                "propagates to greater committed-class diversity than the collapsed "
                "legacy baseline on the GAP-A-fixed pool. supports MECH-309 "
                "(rule-creator necessity) + ARC-062 (V3 weak-reading instantiation). "
                "Route to /governance: MECH-309 / ARC-062 supports evidence; "
                "consider GAP-B closure + the trained-bias-head behavioural successor."
            ),
            "FAIL_C1_holds_C2_fails": (
                "Class axis exercisable, GAP-A divergence real, ARM_ON minted "
                "distinct rules, but the differentiated rule_state adds no marginal "
                "committed-class diversity over the collapsed baseline. weakens "
                "MECH-309 / ARC-062 on this eval-frozen readout; route to "
                "/governance + /failure-autopsy (does propagation require a TRAINED "
                "bias head, i.e. a P1 frozen-encoder bias-head training arm?)."
            ),
            "FAIL_C1_substrate_not_ready_requeue": (
                "The committed-class axis was not exercisable (< 2 candidate classes) "
                "and/or GAP-A consumed-summary divergence was absent (under-trained "
                "e2) and/or ARM_ON minted no distinct rules. The falsifier could not "
                "run -- NOT an MECH-309 / ARC-062 falsification. Route to substrate "
                "enrichment / higher P0 budget and re-queue; do NOT weaken the claims."
            ),
        },
...
def _build_manifest(
    ...
    return {
        ...
        "evidence_direction_note": (
            f"V3-EXQ-654 arc_062 GAP-B behavioural falsifier (MECH-309 / ARC-062). "
            f"Single-variable: ARM_OFF (use_candidate_rule_field=False, legacy "
            f"collapsed rule_state) vs ARM_ON (use_candidate_rule_field=True, "
            f"differentiated crf_source), both on the matched 649/643a/SD-056/SP-CEM/"
            f"MECH-341 stack with the SD-033a bias head un-zeroed and e2 trained "
            f"online in P0. PRIMARY DV = COMMITTED-CLASS entropy (the rule bias is "
            f"class-keyed -- within-class-representative entropy is a SECONDARY "
            f"negative control, expected ~null). C1 (non-vacuity) = committed-class "
            f"axis exercisable AND GAP-A consumed-summary divergence real (both arms) "
            f"AND ARM_ON minted distinct rules; C2 (PRIMARY) = paired-by-seed ARM_ON > "
        ...
        "env_kwargs": dict(ENV_KWARGS),
        "config_summary": {
            "arms": "ARM_OFF (rule field off) vs ARM_ON (rule field on)",
            ...
            "matched_stack": (
                ...
                "(lateral_pfc_train_rule_bias_head=True) + SD-056 all levers"
            ),
            ...
            "e2_trained_in_p0_frozen_in_p1": True,
            "use_modulatory_selection_authority": USE_MODULATORY_SELECTION_AUTHORITY,
            "modulatory_authority_gain": MODULATORY_AUTHORITY_GAIN,
            "z_goal_enabled": True,
            "drive_weight": 2.0,
...
def main() -> Tuple[Optional[str], Optional[str]]:
    parser = argparse.ArgumentParser(description="V3-EXQ-654 GAP-B falsifier")
