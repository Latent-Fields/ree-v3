...
"""V3-EXQ-485j: SD-033b trained-OFC-head BEHAVIOURAL validation (MECH-263 a+b) ON
the MECH-448 (ARC-107) rank-preserving F->eligibility DEMOTION-ENABLED E3 selector,
with the demotion ENVELOPE FLOOR CALIBRATED to the behavioural candidate bank.

commitment_closure:GAP-8 evidence-grade follow-on. SUPERSEDES V3-EXQ-485i, which
RAN FAIL / self-routed substrate_not_ready_requeue 2026-06-21 (non_contributory,
non_degenerate:false -- NO governance weight): ARM_2 readiness gates 1+2 PASSED
(OFC bias cross-candidate range 0.368>=0.05; head weight-delta 5.39) but the
MECH-448 NON-DEGENERACY gate FAILED -- f_eligibility_excluded_count==0 /
envelope_size=8 on all 3 seeds. The 689d-validated demotion config admitted EVERY
candidate because the OFC-isolated SD-054 behavioural bank has a SPREAD
(non-divergent) F pool -- no candidate's merit-share cleared the absolute 0.30
floor -> all-admit fallback -> the F->eligibility demotion was a STRUCTURAL no-op
(ARM_2==ARM_1) -> the DVs never ran through a demoted selector
(failure_autopsy_V3-EXQ-485i_2026-06-21).

THE 485j FIX (envelope-engagement calibration, NOT a MECH-439 F-variance rebalance):
the demotion-on arms CALIBRATE the absolute f_eligibility_envelope_floor per
(arm, seed) to the behavioural bank's MEASURED merit-share distribution
(_calibrate_envelope_floor) so the MECH-448 envelope genuinely EXCLUDES a non-empty
F-worst tail (keep the F-best top-k, k in [2,4], at the largest share-gap). raw_scores
(the primary F the envelope thresholds) is bias-independent and constant across the
eval drives, so one calibration governs all drives. KEEP an ABSOLUTE share floor (a
fraction-of-max degenerates to the margin shortlist); KEEP dn_sigma=0.0. NOTE this
harness calls e3.select() DIRECTLY with a hand-built OFC score_bias (NOT
agent.select_action), so candidate_summary_source=e2_world_forward is INERT here AND
the envelope thresholds raw_scores which that knob does not peak -- option (b)
calibration is the applicable fix, not option (a) divergent-pool re-sourcing.
...
PRE-REGISTERED ACCEPTANCE (per seed, then >= MIN_PASS_SEEDS of 3):
  READINESS (non-vacuity precondition, on the ARM_2 test arm; SAME statistic the
    load-bearing DVs route on):
      ...
      (c) MECH-448 NON-DEGENERACY (the 485i fix, kept as a HARD gate): the calibrated
          envelope actually EXCLUDED a candidate (f_eligibility_excluded_count > 0 AND
          f_eligibility_demotion_active), not an all-admit no-op. Calibration fails on
          a flat/near-flat F (no clean share-gap) -> default floor -> all-admit ->
          excluded_count==0 -> this gate FAILS.
...
INTERPRETATION GRID (one row per plausible outcome -> next action):
  ...
  FAIL / substrate_not_ready_requeue (readiness UNMET): below the 0.05 bias-range
    floor, head untrained, OR the calibrated envelope still admitted all
    (excluded_count == 0 -- flat/near-flat F with no clean share-gap) so the demotion
    never engaged -> the behavioural test never ran. Re-queue with a stronger driver /
    budget / a divergent F pool. NOT a weakens.
...
EXPERIMENT_TYPE = "v3_exq_485j_sd033b_demotion_envelope_calibrated_behavioural"
EXPERIMENT_PURPOSE = "evidence"
QUEUE_ID = "V3-EXQ-485j"
    ...
    manifest = {
        ...
        "degeneracy_reason": (
            None if result["readiness_met"]
                 ...
                 "floor, head untrained, or the CALIBRATED MECH-448 envelope still "
                 "admitted all candidates (excluded_count == 0 -- flat/near-flat F, no "
                 "clean share-gap to calibrate a floor) -> the behavioural test never ran"
        ),
        "dry_run": dry_run,
        "elapsed_sec": elapsed,
        "acceptance": acc,
        ...
        "substrate_under_test": (
            ...
            "(default 0.30 fallback; keep F-best top-k in [2,4]) so the envelope "
            "genuinely excludes -- the 485i excluded_count==0 all-admit fix"
        ),
        "unblocks": (
        ...
        "supersedes": "V3-EXQ-485i",
        "predecessor": (
            "V3-EXQ-485i (RAN FAIL / self-routed substrate_not_ready_requeue 2026-06-21; "
            "non_contributory / non_degenerate:false -- the MECH-448 demotion envelope "
            "admitted ALL candidates (f_eligibility_excluded_count==0) on the spread "
            "behavioural-bank F, so the demotion was a structural no-op and the DVs "
            "never ran through a demoted selector)"
        ),
        "predecessors": [
        ...
        "notes": (
            ...
            "the MECH-448 demotion-enabled E3 selector. SUPERSEDES V3-EXQ-485i, which "
            "self-routed substrate_not_ready_requeue because the 689d-validated demotion "
            "config (floor=0.30) admitted EVERY candidate on the OFC-isolated behavioural "
            "bank's SPREAD F pool (f_eligibility_excluded_count==0 = all-admit no-op, "
            "ARM_2==ARM_1). THE 485j FIX is a behavioural-harness calibration (NOT a "
            "MECH-439 F-variance rebalance): the demotion-on arms calibrate the ABSOLUTE "
            "f_eligibility_envelope_floor per (arm, seed) to the bank's measured "
            "merit-share distribution (keep F-best top-k in [2,4] at the largest "
            "share-gap; raw_scores is bias-independent + constant across the eval drives) "
            "so the MECH-448 envelope genuinely EXCLUDES a non-empty tail and the OFC "
            "channel arbitrates the committed action within the F-eligible set with F "
            "removed from the final argmin. KEEP an ABSOLUTE share floor (a "
            "fraction-of-max degenerates to the margin shortlist); KEEP dn_sigma=0.0. "
            "The PRIMARY C1 devaluation_selection_shift + C2 between-context TV are "
            "measured on the COMMITTED selection through the real E3.select() "
            "(SelectionResult.selected_index). 3-arm dissociation attributes conversion "
            "to the CONJUNCTION trained-head AND demotion: ARM_2 (test) must convert "
            "where ARM_1 (trained, demotion off = F-dominance ceiling) and ARM_0 (frozen "
            "head + demotion = silence) do not. Readiness gates: OFC bias range > 0.05 "
            "(gate 1) + head trained (gate 2) PLUS the MECH-448 non-degeneracy gate "
            "(f_eligibility_excluded_count > 0 on >=2/3 seeds -- now reachable via "
            "calibration) self-route substrate_not_ready_requeue (non_contributory) "
            "below floor, NEVER a false SD-033b/MECH-263 weakens. PROMOTES NOTHING by "
            "itself; a PASS contributes provisional behavioural evidence and helps close "
...
def main():
    ...
    print("\n=== V3-EXQ-485j SUMMARY ===", flush=True)
