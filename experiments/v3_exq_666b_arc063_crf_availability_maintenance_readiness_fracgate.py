"""V3-EXQ-666b: ARC-063 CRF availability-maintenance readiness -- ISOLATING-STATISTIC gate.

Purpose: diagnostic (claim_ids=[]). Supersedes V3-EXQ-666a. Re-states the
load-bearing CRF-readiness gate on the ISOLATING statistic crf_frac_maintained,
routed by the confirmed failure_autopsy_V3-EXQ-666a_2026-06-11. Does NOT weight
governance and does NOT validate/weaken MECH-309/ARC-062/ARC-063 -- those stay
candidate / substrate_ceiling / v3_pending / pending_retest_after_substrate. The
ARC-062 GAP-B BEHAVIOURAL re-run (654c, committed-class entropy falsifier on the
field-populated substrate) is a SEPARATE later /queue-experiment session GATED on
this readiness PASS.

THE GAP THIS CLOSES (the 666a measurement_test_design_defect). V3-EXQ-666a
re-stated the gate on the MAINTAINED pool but gated on the COUNT floor
crf_n_maintained_reactivatable >= 2, which is GAP-DENSITY-SENSITIVE: at
behavioural-runtime context-absent gap density, differentiation-alone (ARM_1, no
maintenance) keeps 2-7 rules reactivatable and so CLEARS the count floor -- the
readiness PASS was unattributable to the maintenance mechanism (the 666a
discrimination FAIL; criteria_non_degenerate.maintained_pool_gate_discriminates_vs_arm1
_no_maintenance = false). The autopsy adjudicated this a defect, NOT a substrate
failure: the maintenance mechanism WORKS and strictly dominates when read on the
ISOLATING FRACTION statistic crf_frac_maintained (already in the 666a manifest):
ARM_2 (maintenance) 1.0/0.625/0.938 vs ARM_1 (no maintenance) 0.188/0.125/0.438 vs
ARM_0 0.0 -- clean monotone separation, 3/3 seeds. The lit-prescribed readout
redefinition (the maintained-pool fraction replaces crf_frac_active) is validated
(crf_frac_active ~0.016-0.031 in BOTH e2ctx arms -- the averaged-activity artefact).

WHAT 666b CHANGES vs 666a (two coupled levers; the autopsy's "and/or"):
  (1) LOAD-BEARING: the readiness gate is RE-STATED on crf_frac_maintained (the
      isolating fraction), NOT the gap-density-sensitive count floor:
        readiness    = ARM_2 crf_frac_maintained >= FRAC_MAINTAINED_FLOOR (0.625)
                       on >=2/3 seeds  (maintenance clears)
        discrimination = ARM_1 crf_frac_maintained <  ARM1_FRAC_CEILING (0.5)
                       on >=2/3 seeds  (differentiation-alone does NOT clear --
                       isolates the maintenance contribution; the 666a count floor
                       could not do this)
      On the 666a-collected data this gate discriminates 3/3 already.
  (2) ROBUSTNESS: after the SAME 666a behavioural-runtime training, each cell runs
      a CONTEXT-ABSENT GAP PHASE of GAP_TICKS silent ticks (the canonical
      activity-silent stress: iterate CandidateRuleField.credit() with no matching
      context, so the maintenance-vs-decay branch is exercised over pure silence).
      Differentiation-alone (ARM_1) erodes its maintained pool below the floor;
      activity-silent maintenance (ARM_2, maintenance_decay=0.0) HOLDS it. This
      makes the discrimination robust to gap density -- the exact property the
      666a count floor lacked. The fraction gate is load-bearing regardless of the
      gap (works at GAP_TICKS=0 on the 666a regime); the gap only sharpens ARM_1's
      erosion below the floor.

SAME-STATISTIC NON-VACUITY DISCIPLINE (V3-EXQ-643 lesson; the gate statistic itself
is asserted as a P0 readiness precondition so a starved test self-routes
substrate_not_ready_requeue, never a false verdict):
  - minted >= 2 on every cell (the field engaged at all).
  - e2ctx arms DIFFERENTIATE (crf_max_pairwise_rule_dist > DIFF_FLOOR on the full
    pool, >=2/3) -- the differentiated pool the maintenance is supposed to HOLD
    must first exist.
  - the MAINTAINED pool the FRACTION gate counts is itself non-degenerate on the
    ARM_2 positive control: n_maintained >= MAINT_N_FLOOR (count-readiness for a
    count-fraction gate) AND crf_maintained_pairwise_dist > MAINT_DIST_FLOOR (so a
    high crf_frac_maintained cannot be a vacuous count of undifferentiated clones).
  Any unmet precondition -> substrate_not_ready_requeue (NOT a fix verdict).

DESIGN (3 arms x 3 seeds). ALL arms run the SAME 654b/666/666a matched stack
(use_candidate_rule_field=True + use_lateral_pfc_analog=True UN-ZEROED trainable
head + crf_persist_rules_across_episode_reset=True + SD-056 e2 contrastive trained
ONLINE so e2.world_forward is action-divergent). The swept variables are the three
flags; ARM_2 (mature + e2ctx + activity-silent maintenance) is adopted as the
mature-regime default:
  ...
  ARM_1_MATURE_E2CTX              : mature + e2ctx, maintenance OFF
                                    -- DIFFERENTIATES but the maintained pool does NOT
                                       hold under silence (the 666 persistence collapse).
  ARM_2_MATURE_E2CTX_MAINTENANCE  : mature + e2ctx + maintenance (floor 0.45, decay 0.0)
                                    -- the differentiated pool is held activity-silently
                                       (the adopted mature default).

INTERPRETATION GRID (the self-route is a hypothesis -- adjudicate before it drives any
governance action):
  - non_vacuity unmet (a cell minted <2, OR e2ctx arms did not differentiate, OR the
    ARM_2 maintained pool is degenerate: n<2 / undifferentiated) ->
    substrate_not_ready_requeue (the fraction-gate test was starved; NOT a verdict).
  - ARM_2 clears the frac_maintained gate on >=2/3 AND ARM_1 does NOT clear the
    frac ceiling on >=2/3 -> PASS: activity-silent maintenance HOLDS a larger
    fraction of the differentiated pool than differentiation-alone (isolated on the
    fraction statistic).
  ...
  - ARM_1 ALSO clears the frac ceiling -> FAIL: even the isolating fraction statistic
    does not separate maintenance from differentiation-alone -> /failure-autopsy
    (would contradict the 666a manifest data; revisit the gap regime).

Run with:
  /opt/local/bin/python3 experiments/v3_exq_666b_arc063_crf_availability_maintenance_readiness_fracgate.py [--dry-run]
...
SUPERSEDES = "v3_exq_666a_arc063_crf_availability_maintenance_readiness"

SEEDS = [42, 43, 44]
N_EPISODES = 100
STEPS_PER_EPISODE = 40
# Context-absent silence gap (the "longer gap regime" lever): pure decay ticks AFTER
# behavioural-runtime training so differentiation-alone (ARM_1) erodes its maintained
# pool below the floor while activity-silent maintenance (ARM_2) HOLDS it.
GAP_TICKS = 2000
DRY_N_EPISODES = 3
DRY_STEPS_PER_EPISODE = 10
DRY_GAP_TICKS = 50

# CRF-readiness gate floors -- LOAD-BEARING, re-stated on the ISOLATING statistic
# crf_frac_maintained (the maintained-reactivatable fraction = n_maintained / n_slots),
# NOT the gap-density-sensitive count floor 666a used.
FRAC_MAINTAINED_FLOOR = 0.625   # ARM_2 crf_frac_maintained must clear this (maintenance
#                                 holds the pool); 666a ARM_2 = 1.0/0.625/0.938.
ARM1_FRAC_CEILING = 0.5         # ARM_1 crf_frac_maintained must stay BELOW this
#                                 (differentiation-alone does NOT clear); 666a ARM_1 =
#                                 0.188/0.125/0.438 -- the discrimination the count floor
#                                 could not deliver.

# Same-statistic non-vacuity floors (assert the maintained-pool substrate the FRACTION
# gate counts is genuine, on the ARM_2 positive control).
MAINT_N_FLOOR = 2          # crf_n_maintained_reactivatable on ARM_2 (count-readiness for
#                            a count-fraction gate).
MAINT_DIST_FLOOR = 0.1     # crf_maintained_pairwise_dist on ARM_2 -- the COUNTED pool is
#                            differentiated, not a vacuous clone-count.
DIFF_FLOOR = 0.1           # crf_max_pairwise_rule_dist on the FULL pool (e2ctx arms): the
#                            differentiated pool the maintenance is supposed to HOLD exists.
MIN_MINTED_FLOOR = 2       # every cell must mint >=2 rules (field engaged).
SEED_MAJORITY = 2          # >=2/3 seeds.

# SD-056 online e2 training (mirror V3-EXQ-666a / 666 / 654b / 649 harness).
SD056_WEIGHT = 0.05
...
# IDENTICAL env to V3-EXQ-666a / 666 / 654b / 614e so the readiness regime matches the
# GAP-B falsifier.
ENV_KWARGS = dict(
    size=12,
    num_hazards=4,
...
ARMS: List[Dict[str, Any]] = [
    ...
    },
    {
        ...
        "label": "mature_e2ctx_no_maintenance_differentiates_but_pool_erodes_under_silence",
...
def _make_agent(env: CausalGridWorldV2, arm: Dict[str, Any]) -> REEAgent:
    """654b/666/666a matched stack; the ONLY swept variables are the three CRF amend flags.
...
# ---------------------------------------------------------------------------
# SD-056 online e2 training (mirror V3-EXQ-666a)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# One (arm, seed) cell: drive the agent, run a context-absent silence gap,
# then read the maintained CRF pool.
# ---------------------------------------------------------------------------
def _run_cell(arm: Dict[str, Any], seed: int, n_episodes: int,
    ...
    # --- CONTEXT-ABSENT SILENCE GAP (the longer-gap-regime lever) ---
    ...
    # Differentiation-alone (ARM_1) erodes its maintained pool below the floor;
    # activity-silent maintenance (ARM_2, maintenance_decay=0.0) HOLDS it. Waking-
    # stream silence (NOT replay) -- no minting, no matching, no z_goal write.
...
def main(dry_run: bool = False):
    ...
    # --- Gates ---
    # NON-VACUITY (same-statistic discipline): (a) every cell mints >=2; (b) the e2ctx
    # arms DIFFERENTIATE on the FULL pool (crf_max_pairwise_rule_dist > DIFF_FLOOR,
    # >=2/3); (c) the ARM_2 MAINTAINED pool the FRACTION gate counts is itself a genuine
    # >=2-rule DIFFERENTIATED pool (count-readiness + differentiation on the positive
    # control). Any unmet -> substrate_not_ready_requeue.
    ...
    maintained_pool_nondeg_met = (
        arm_summary["ARM_2_MATURE_E2CTX_MAINTENANCE"][
            "n_maintained_pool_nondegenerate"
        ]
        >= SEED_MAJORITY
    )
    ...
    # DISCRIMINATION (same statistic, the 666a defect this closes): ARM_1 (no
    # maintenance) must stay BELOW the fraction ceiling on >=2/3 seeds -- the count
    # floor could not deliver this; the fraction can.
    discrimination_met = (
    ...
    if not non_vacuity_met:
        ...
        route = (
            ...
            "differentiate, OR the ARM_2 maintained pool the fraction gate counts is "
            "degenerate -- the isolating-fraction test was starved, NOT a fix verdict)"
        )
        ...
        route = (
            ...
            "not separate maintenance from differentiation-alone; contradicts the 666a "
            "manifest data, revisit the gap regime)"
        )
    if any_error:
        outcome = "FAIL"

    exp_type = "v3_exq_666b_arc063_crf_availability_maintenance_readiness_fracgate"
    ...
    manifest = {
        ...
        "interpretation": {
            ...
            "preconditions": [
                ...
                {
                    "name": "e2ctx_full_pool_differentiated_supra_floor",
                    "kind": "readiness",
                    "description": (
                        "the differentiated pool the maintenance is supposed to HOLD "
                        "must first exist: crf_max_pairwise_rule_dist > floor on the "
                        "FULL pool of the e2ctx arms. Below floor (e2ctx did not "
                        "differentiate) -> substrate_not_ready_requeue, NOT a "
                        "maintenance verdict (same-statistic V3-EXQ-643 lesson)."
                    ),
                    "control": (
                ...
                },
                {
                    ...
                    "description": (
                        ...
                        ">=2 maintained-reactivatable rules, else the fraction the gate "
                        "reads is degenerate -> substrate_not_ready_requeue."
                    ),
                    "control": (
            ...
            "criteria_non_degenerate": {
                ...
                "e2ctx_full_pool_differentiated": bool(differentiation_met),
        ...
        "notes": (
            ...
            "Supersedes V3-EXQ-666a. The readiness gate is RE-STATED on the ISOLATING "
            "statistic crf_frac_maintained (the maintained-reactivatable FRACTION = "
            "n_maintained / n_slots), NOT the gap-density-sensitive count floor "
            "crf_n_maintained_reactivatable >= 2 that 666a used (and that "
            "differentiation-alone cleared, the 666a measurement_test_design_defect). "
            "ARM_2 (maintenance) must clear crf_frac_maintained >= 0.625 where ARM_1 "
            "(mature+e2ctx, no maintenance) stays below 0.5 -- the discrimination the "
            "count floor could not deliver (666a ARM_2 1.0/0.625/0.938 vs ARM_1 "
            "0.188/0.125/0.438). A context-absent silence gap (CandidateRuleField.credit "
            "iterated GAP_TICKS times after behavioural-runtime training) sharpens "
            "ARM_1's erosion below the floor while activity-silent maintenance "
            "(maintenance_decay=0.0) HOLDS the ARM_2 pool -- robustifying the "
            "discrimination against gap density. Same-statistic non-vacuity discipline: "
            "minted>=2, full-pool differentiation, AND the ARM_2 maintained pool the "
            "fraction gate counts is itself a >=2-rule differentiated pool (count + "
            "differentiation readiness on the positive control); any unmet -> "
            "substrate_not_ready_requeue, never a false verdict. ARM_2 maintenance-ON is "
            "adopted as the mature-regime default. PASS clears the substrate_queue ready "
            "gate before the 654c GAP-B behavioural re-run (MECH-309 / ARC-062) is "
            "scored; that falsifier is the governance-weighting successor, queued "
            "separately. crf_frac_active ~0.016-0.031 in both e2ctx arms is the "
            "averaged-activity artefact the lit verdict identified -- kept as a "
            "secondary readout, NOT the persistence criterion. MECH-309/ARC-062/ARC-063 "
