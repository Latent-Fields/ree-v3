...
"""
V3-EXQ-708a -- MECH-440 NOISY-SELECTION-HEAD PROPAGATION FALSIFIER (diagnostic).

CORRECTED RE-RUN OF V3-EXQ-708 -- INSTRUMENT REPAIR, SAME SCIENTIFIC QUESTION.
supersedes: V3-EXQ-708.

WHY THIS EXISTS (read before editing the measurement path)
----------------------------------------------------------
The 2026-07-19 re-adjudication (REE_assembly/evidence/planning/
failure_autopsy_V3-EXQ-708_2026-07-19.{md,json}) found V3-EXQ-708's SECONDARY DV -- pre-commit
class entropy -- INVALID, and MECH-440 therefore never validly tested. The withdrawn finding
("supra-floor weight noise raised pre-commit entropy by 0.0, so it washes out at the
F-dominated single-arena argmax") is NOT evidence and must not be re-cited. 708's
epistemic_category was corrected substrate_ceiling -> measurement_test_design_defect.

THE DEFECT: ree_core/predictors/e3_selector.py sets last_score_diagnostics (rebuilt at :2452)
and last_precommit_probs (assigned at :2687) ONLY inside select(). E3 select() does NOT run
every env step -- ree_core/agent.py:5429 returns the held/stepped action when ticks["e3_tick"]
is False, BEFORE the e3.select() call at agent.py:7011. The cadence is NOT fixed:
heartbeat.e3_steps_per_tick defaults to 10 (ree_core/utils/config.py:2017) and MECH-093
modulates it 5-20 steps with z_beta arousal (ree_core/heartbeat/clock.py:52-70). 708 read both
attributes once per ENV STEP with no clear and no freshness guard, so each probability vector
was re-recorded once per held tick -- a mass-weighted mixture, not a tick sample. The
numel()==len(candidates) check in 708 was NOT a freshness guard: candidate count is constant,
so a stale vector passes it. Confirmed empirically: n_precommit_ticks == n_p2_ticks EXACTLY on
all 24 arm-seeds.

WHY IT WAS DIRECTIONAL, NOT MERELY AN INFLATED n: uniform replication cancels under
normalization, but exposure is ARM-DEPENDENT. A0_OFF and ARM_TEMP were tick-identical on every
seed while ARM_NOISE_SINGLE diverged by +1.7% / +57.5% / +32.1% on divergent seeds 44/45/46.
That imbalance IS the mechanism of the distortion, so this run RECORDS it per arm-seed.

THE REPAIR (six properties, from autopsy section 10):
  1. Clear agent.e3.last_score_diagnostics AND .last_precommit_probs to None IMMEDIATELY before
     every agent.select_action(...); record a row ONLY if repopulated. Pattern ported from
     experiments/v3_exq_785a_mech463_arousal_exogenous_urgency_decomp.py:525-543.
  2. Emit n_fresh_select / n_latched / fresh_select_yield as manifest telemetry, so the
     replication factor is VISIBLE rather than inferred.
  3. Record the per-arm-seed tick imbalance explicitly (exposure_imbalance_vs_off).
  4. Per-seed fresh-selection counts live in the MANIFEST ITSELF -- NOT a declared per_tick_sink.
     Phase 3 cloud workers POST only manifest_bytes, so a declared sink is not transported
     (twice-confirmed: V3-EXQ-785 and V3-EXQ-708).
  5. supersedes: V3-EXQ-708 on both the queue entry and this manifest.
  6. stamp_recording_core(...) per REE_assembly/evidence/planning/
     experimental_recording_standard_2026-07-12.md sections 3b/3c.

RE-DERIVE BRAKE: NOT FIRED, and this is not what the brake forbids. It is instrument repair of
a run that never validly measured its DV -- the same shape as V3-EXQ-785 -> 785a, whose
corrected instrument returned a clean, well-powered null. The autopsy JSON records
targets[0].re_derive_brake.fired = false: the recommended category is
measurement_test_design_defect, so the counting rule is not met. MECH-440's only standing
substrate_ceiling autopsy (failure_autopsy_700d-708-single-arena-ceiling_2026-06-29) was
SUPERSEDED for its 708 target by that same re-adjudication.
...
VENUE -- WHY THE LOOP-SEGREGATION ARM IS DROPPED (708 ran 4 arms; 708a runs 3)
------------------------------------------------------------------------------
708 carried ARM_NOISE_LOOPSEG to disambiguate the injection-SITE axis from the SINGLE-ARENA
collapse (-> ARC-110), and the prior cluster autopsy's recommended next step was that arm. That
question has since been ANSWERED independently: V3-EXQ-707b returned evidence_direction=weakens
on the single-arena-artefact sub-hypothesis with a valid non_degenerate null, concluding the
conversion ceiling is INTRINSIC to MECH-439 F-dominance rather than an artefact of the single
arena. 708a is therefore designed against that current reading and is explicitly NOT a bet that
loop segregation relieves the ceiling -- re-running that arm would re-pose a settled question,
and it is the arm least related to the DV whose instrument is under repair. Dropping it also
removes the ARC-110 loop non-degeneracy precondition (which cannot be met by the retained arms),
narrows the grid 24 -> 18 cells, and leaves exactly the three arms whose exposure imbalance the
autopsy quantified. claim_ids stays [MECH-440]; ARC-110 is neither tagged nor tested here.

THE 3 ARMS (all carry the SAME landed SOTA conversion stack -- 569i top-k shortlist +
...
6 seeds x 3 arms = 18 cells. PRIMARY DV = committed-action-class entropy (nats), P2.
SECONDARY DV = PRE-COMMIT sampling-class entropy (nats; from e3.last_precommit_probs, now read
ONLY on a verified-fresh select) -- the thrash-vs-carve discriminator.
...
------------------------------------------------
  ...
  WEAKENED (the noise thrashes at pre-commit and washes out at the committed argmax):
    ARM_NOISE_SINGLE does NOT lift committed-class entropy strict-above ARM_TEMP, despite
    genuinely raising PRE-COMMIT entropy (verified by precondition 2, now measured on a
    fresh-select-only DV). The weight noise washes at the argmax even on the SOTA stack --
    thrash, not carve -- so the injection SITE is not the binding constraint.
    Read this against the CURRENT venue: V3-EXQ-707b already returned weakens on the
    single-arena-artefact sub-hypothesis with a valid null, so a null here does NOT route to
    /implement-substrate ARC-110 (that branch is settled and is deliberately not offered).
    It weakens MECH-440 as a standalone fix and leaves the ceiling attributed to MECH-439
    F-dominance, which is the current reading.

NOTE ON WHAT THIS RUN CANNOT DO: 708's withdrawn finding is not re-testable by re-analysis --
the correction is NOT recoverable from 708's recorded emission (no per-row sink; Phase 3 cloud
workers transport only manifest_bytes; its metrics.json is 46 chars). Only per-arm-seed scalars
survived, which is exactly why a re-run is required rather than a reanalysis artifact
(GOV-REUSE-1 checked: decisive readout is pre-commit class entropy on a FRESH-SELECT-ONLY
denominator, which exists in NO recorded manifest -> not recoverable -> run).
...
NON-VACUITY READINESS GATES (self-route substrate_not_ready_requeue, NEVER a false weakens):
  ...
  (5) FRESH-SELECT SUFFICIENCY (NEW in 708a -- the repaired instrument's own readiness gate):
      every arm-seed must record at least MIN_FRESH_SELECTS genuinely fresh E3 selections in
      P2. Because the DV is now accumulated ONLY on verified-fresh ticks, its denominator is
      ~1/e3_steps_per_tick of 708's inflated one (cadence 5-20 steps, MECH-093-modulated), so
      a run that would silently have had n=600 replicated rows now honestly reports ~67. If
      that honest n is too small to estimate a class marginal, the correct route is
      substrate_not_ready_requeue -- NOT a weakens on an underpowered DV.
...
EXPERIMENT_TYPE = "v3_exq_708a_mech440_noisy_selection_head_propagation_falsifier"
QUEUE_ID = "V3-EXQ-708a"
SUPERSEDES = "V3-EXQ-708"
...
# v3_exq_708_..._20260628T220908Z_v3 recorded met=True on four of the five inherited gates
# retained by 708a -- enough_divergent_seeds 3.0/3.0, noise_bias_range_supra_floor_vs_raw
# 0.22164/1e-4, dacc_suppression_live 1.0/0.0, learning_engaged_finer_channels_dissociable
# 0.00113/1e-4 -- on 24 completed arm-seeds of this exact substrate and config.
# The two that recorded met=False (temperature_control_raises_precommit_entropy and
# weight_noise_raises_precommit_entropy, both 0.0/2.0) are precisely the two that READ THE
# CONTAMINATED PRE-COMMIT DV, which is the defect this run repairs; whether they are met on a
# fresh-select-only denominator is the open scientific question, not an instrument gap.
# The one NEW gate (fresh_selects_sufficient) is reachable with ~2 orders of magnitude of
# headroom: P2 is 100 episodes x 200 steps = 20000 ticks per cell at an E3 cadence of 5-20
# steps, so ~1000-4000 fresh selections against a floor of 30.
# CAUTION for the reader: enough_divergent_seeds cleared at EXACTLY its threshold (3.0/3.0) in
# 708 -- it has zero headroom, so a single lost divergent seed self-routes this run to
# substrate_not_ready_requeue. That is the correct conservative behaviour, not a defect.
ANCHOR_REACHABILITY_EXEMPT = (
    "reachability established empirically by the predecessor V3-EXQ-708's landed manifest "
    "(4/5 retained gates met=True over 24 completed arm-seeds on this exact substrate+config); "
    "the 2 unmet gates read the contaminated DV this run repairs; the new "
    "fresh_selects_sufficient floor of 30 sits ~1000-4000 expected fresh selects per cell."
)
...
def run_experiment(
    ...
    if not preconditions_met:
        ...
        degeneracy_reason = (
            ...
            "/ too few genuinely-fresh E3 selections to estimate the class marginal). "
            "NOT a falsification."
        )
        ...
        non_degenerate = True
        degeneracy_reason = ""
        label = "weight_noise_thrashes_at_precommit_and_washes_at_committed_argmax_weakens_mech440"
    ...
    manifest_core = {
        ...
        "interpretation_grid": {
            ...
            "FAIL_weight_noise_thrashes_at_precommit_and_washes_at_committed_argmax_weakens_mech440": (
                "DECISIVE. preconditions met (ARM_NOISE_SINGLE genuinely raises pre-commit "
                "entropy on a FRESH-SELECT-ONLY denominator) BUT it does not lift committed-class "
                "entropy strict-above ARM_TEMP. The weight noise washes at the argmax even on the "
                "SOTA stack -- thrash, not carve -- so the injection SITE is not the binding "
                "constraint -> weakens MECH-440 as a standalone fix. NOTE: this does NOT route to "
                "/implement-substrate ARC-110. V3-EXQ-707b already returned weakens on the single-"
                "arena-artefact sub-hypothesis with a valid non_degenerate null, concluding the "
                "conversion ceiling is intrinsic to MECH-439 F-dominance; the loop-segregation "
                "branch is settled and is deliberately not offered as an outcome here."
            ),
            "FAIL_substrate_not_ready_requeue": (
                ...
                "learning was not engaged, OR too FEW genuinely-fresh E3 selections were recorded "
                "to estimate the pre-commit class marginal. The propagation question could NOT be "
                "measured -- NOT a falsification."
            ),
        },
...
def _build_manifest(
    ...
    return {
        ...
        "evidence_direction_note": (
            f"V3-EXQ-708a MECH-440 NOISY-SELECTION-HEAD PROPAGATION FALSIFIER -- CORRECTED "
            f"RE-RUN OF V3-EXQ-708 (supersedes V3-EXQ-708; experiment_purpose=diagnostic; "
            f"claim_ids=[MECH-440]; PROMOTES NOTHING). WHY: the 2026-07-19 re-adjudication "
            f"(failure_autopsy_V3-EXQ-708_2026-07-19) found 708's pre-commit class entropy DV "
            f"INVALID -- the driver read agent.e3.last_precommit_probs and .last_score_diagnostics "
            f"once per ENV STEP with no clear and no freshness guard, while E3 select() runs only "
            f"on an E3 tick at a 5-20-step MECH-093-modulated cadence, so each probability vector "
            f"was replicated by its hold duration (n_precommit_ticks == n_p2_ticks on all 24 "
            f"arm-seeds). The distortion was DIRECTIONAL because exposure was ARM-DEPENDENT (OFF "
            f"and TEMP tick-identical while NOISE_SINGLE diverged +1.7%/+57.5%/+32.1% on divergent "
            f"seeds 44/45/46). 708's finding -- that supra-floor weight noise raised pre-commit "
            f"entropy by 0.0 and therefore washes out at the F-dominated single-arena argmax -- is "
            f"WITHDRAWN, and MECH-440 was never validly tested. THE REPAIR: both attributes are "
            f"cleared to None immediately before every select_action and a row is recorded ONLY if "
            f"repopulated (pattern from v3_exq_785a), with n_fresh_select / n_latched / "
            f"fresh_select_yield and the per-arm-seed exposure imbalance recorded in the MANIFEST "
            f"ITSELF (not a per_tick_sink -- Phase 3 cloud workers transport only manifest_bytes). "
            f"3 arms (708 ran 4) on the SAME GAP-A reef-bipartite foraging substrate + the SAME "
            f"landed SOTA conversion stack as a MATCHED CONSTANT; the ONLY swept factor is the "
            f"exploration-injection site: A0_OFF / ARM_TEMP (the 687 matched-pre-commit-variance "
            f"temperature control, use_noise_floor) / ARM_NOISE_SINGLE (MECH-440 "
            f"use_noisy_selection_head, single arena). ARM_NOISE_LOOPSEG was DROPPED: V3-EXQ-707b "
            f"returned weakens on the single-arena-artefact sub-hypothesis with a valid "
            f"non_degenerate null, so the conversion ceiling is read as intrinsic to MECH-439 "
            f"F-dominance and the ARC-110 route is settled -- this run neither tags nor tests "
            f"ARC-110. PRE-REGISTERED decisive: C1 ARM_NOISE_SINGLE committed strict-above "
            f"ARM_TEMP on a strict-majority of divergent seeds => the weight noise carves through "
            f"to commit (the 687 non-propagation fix) -> supports MECH-440; NOT C1 despite a "
            f"genuine pre-commit lift => thrash-not-carve -> weakens MECH-440 as a standalone fix. "
            f"Non-vacuity self-route substrate_not_ready_requeue (NEVER a false weakens): both "
            f"arms must genuinely raise pre-commit entropy, the noise bias range supra-floor vs "
            f"raw, dACC live, learning engaged, the pool divergent, AND every arm-seed must clear "
            f"MIN_FRESH_SELECTS genuinely-fresh E3 selections. RE-DERIVE BRAKE NOT FIRED: this is "
            f"instrument repair of a run that never validly measured its DV (same shape as "
            f"V3-EXQ-785 -> 785a), and the autopsy records re_derive_brake.fired=false because the "
            f"corrected category is measurement_test_design_defect, not substrate_ceiling. "
            f"COORDINATE (no duplicate): the empirical 687-successor (GAP-C leg) tests EMPIRICAL "
            f"conversion; the 705/706 lineage is the curiosity channel (MECH-441). PROMOTES "
            f"NOTHING (MECH-440 candidate/v3_pending; its substrate_ceiling epistemic_category was "
            f"CLEARED by the re-adjudication -- untested, not ceiling-limited). "
        ...
        "env_kwargs": dict(ENV_KWARGS),
        "config_summary": {
            "design": "3-arm MECH-440 noise-injection-site falsifier (A0_OFF / ARM_TEMP / ARM_NOISE_SINGLE) on the 569i top-k + MECH-448 demotion SOTA conversion stack + per-seed-divergent gating + matched-pre-commit-variance non-vacuity + thrash-vs-carve pre-commit-vs-committed entropy discriminator; CORRECTED RE-RUN of V3-EXQ-708 with a fresh-select freshness guard on the pre-commit DV",
