...
"""
V3-EXQ-164 -- MECH-142: Valence-arousal axis orthogonality requires active
...
# Experiment identity
# ---------------------------------------------------------------------------
EXPERIMENT_TYPE = "v3_exq_164_mech142_axis_decorrelation"
CLAIM_IDS       = ["MECH-142", "MECH-063"]
RUN_ID_PREFIX   = "v3_exq_164_mech142_axis_decorrelation"
...
def main(dry_run: bool = False) -> None:
    ...
    print(f"[EXQ-164] Starting {EXPERIMENT_TYPE}")
    print(f"[EXQ-164] run_id    = {run_id}")
    print(f"[EXQ-164] dry_run   = {dry_run}")
    print(f"[EXQ-164] conditions= {CONDITIONS}")
    print(f"[EXQ-164] seeds     = {SEEDS}")
    print(f"[EXQ-164] n_episodes= {N_EPISODES}  (dry_run overrides to 1)")
    ...
    for seed in SEEDS:
        for condition in CONDITIONS:
            print(
                f"[EXQ-164] --- condition={condition}  seed={seed} ---"
            )
            result = _run_cell(
                seed=seed,
                condition=condition,
            ...
            print(
                f"[EXQ-164]   DONE  initial_corr={result['initial_corr']:.4f}  "
    ...
    print(f"[EXQ-164] C1 (WITH corr < NO corr - gap):   {c1_pass}  gaps={[round(v,4) for v in c1_values]}")
    print(f"[EXQ-164] C2 (NO corr drifts by DRIFT_MIN): {c2_pass}  drifts={[round(v,4) for v in c2_values]}")
    print(f"[EXQ-164] C3 (WITH regime_sep >= NO + margin): {c3_pass}  diffs={[round(v,4) for v in c3_values]}")
    print(f"[EXQ-164] C4 (WITH harm <= NO + margin):    {c4_pass}  diffs={[round(v,4) for v in c4_values]}")
    print(f"[EXQ-164] Outcome: {outcome}")
    ...
    if not dry_run:
        ...
        print(f"[EXQ-164] Written to {output_path}")
    else:
        print("[EXQ-164] dry_run=True -- output not written")
        print(f"[EXQ-164] Would write to {output_path}")
