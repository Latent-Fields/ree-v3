"""
V3-EXQ-071 -- Rollout-Batched SD-003 Attribution (GPU-Optimized Variant)
...
EXPERIMENT_TYPE = "v3_exq_071_rollout_batched_attribution"
...
def run(
    ...
    **kwargs,
    ...
    print(
        f"[V3-EXQ-071] Rollout-Batched SD-003 Attribution\n"
    ...
    print(f"\n[V3-EXQ-071] Training ({warmup_episodes} episodes)...", flush=True)
    train_out = _train(
    ...
    for n_haz in DISTRACTOR_LEVELS:
        print(f"\n[V3-EXQ-071] Eval: n_hazards={n_haz} ({eval_episodes} eps)...", flush=True)
    ...
    print(f"\n[V3-EXQ-071] Verdict: {status}  ({n_criteria_met}/5 criteria)", flush=True)
    ...
    summary_markdown = f"""# V3-EXQ-071 -- Rollout-Batched SD-003 Attribution
