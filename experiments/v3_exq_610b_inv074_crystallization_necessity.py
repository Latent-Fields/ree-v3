...
"""V3-EXQ-610b: INV-074 crystallization necessity test (post-MECH-341-retune retest).

Supersedes V3-EXQ-610a (FAIL; D1=False D2=False; crystallization did not preserve
diversity AND control did not show predicted collapse -> non_contributory per
2026-05-30 governance).
...
SUBSTRATE DEPENDENCIES (2026-06-01 retest):
  - MECH-341 (E3 score diversity preservation) was landed 2026-05-27 commit 547faa3
    and RETUNED 2026-05-27 commit a45ca7f: entropy_lambda 0.05 -> 0.5, bias_scale
    0.1 -> 1.0. The retune significantly amplifies the diversity-preservation signal.
  - V3-EXQ-610a (2026-05-29) ran with the original MECH-341 parameters and showed
    FAIL (D1=False, D2=False): crystallization did NOT preserve diversity, and
    control did NOT show collapse -> non_contributory (substrate may already be
    doing enough diversity preservation to make crystallization test non-discriminative).
  - This experiment (V3-EXQ-610b) retests with the updated MECH-341 parameters to
    determine if the amplified diversity signal changes the outcome.
  - Additional substrate changes since 610a: SD-054 scaffolded onboarding (provides
    phased training regime for goal-stream work), SD-056 multi-step rollout stability.
...
--------------
  ...
  (c) D2 FAIL (ARM_0 does not show collapse):
      -> control does not exhibit the predicted failure mode; either (i) the
         diversity signals (MECH-341 retuned parameters) are sufficiently robust
         that crystallization is unnecessary, or (ii) insufficient training epochs.
         Non_contributory for INV-074/MECH-334 (test is non-discriminative), but
         potentially strengthens MECH-341 (diversity preservation working well enough
         to prevent collapse even without crystallization).
...
Estimated runtime: ~800 min (2 arms x 3 seeds x ~2500 episodes @ 200 steps/ep)
  -- V3-EXQ-610 actual_secs=47729 (~13.25h) on DLAPTOP-4.local.
  -- V3-EXQ-610a completed normally on ree-cloud-3 in similar time.
"""
...
def _run_arm(arm_config: Dict, seed: int, dry_run: bool) -> Dict:
    ...
    print(
        f"[V3-EXQ-610b] Starting {arm_label} seed={seed} crystallize={crystallize}",
        flush=True,
    )
...
def main(dry_run: bool = False) -> None:
    """Run V3-EXQ-610b: INV-074 crystallization necessity test (post-MECH-341-retune)."""
    print(
        f"[V3-EXQ-610b] INV-074 crystallization necessity test (EVB-0270, post-MECH-341-retune retest)",
        flush=True,
    )
