...
"""
V3-EXQ-322a: SD-015 ResourceEncoder vs z_world Seeding of z_goal
Supersedes V3-EXQ-322.

experiment_purpose: evidence

BUG FIX from EXQ-322 (n_cosine_samples=0 in all seeds, both conditions):
  Root cause: make_config() passed latent=lat as a keyword argument to
  REEConfig.from_dims(). from_dims() does not declare a 'latent' parameter;
  the argument was silently absorbed by **kwargs and discarded. Therefore
  use_resource_encoder=True was never applied to the actual LatentStack
  config, ResourceEncoder was never instantiated, latent.z_resource was
  always None, and the cosine measurement gate never fired.

  Fix: use_resource_encoder and z_resource_dim are now set directly on the
  config object returned by REEConfig.from_dims() (post-call attribute set).
  This is the minimal experiment-level fix and does not require substrate
  changes to from_dims().
...
EXPERIMENT_TYPE = "v3_exq_322a_sd015_resource_encoder_seeding"
...
def make_config(use_resource_encoder: bool) -> REEConfig:
    # FIX (EXQ-322a): do NOT pass latent=<LatentStackConfig object> to from_dims()
    # -- from_dims() has no 'latent' parameter and silently ignores it via **kwargs.
    # Instead, build via from_dims() then set use_resource_encoder on the returned config.
    ...
    # Set ResourceEncoder flags directly on the LatentStackConfig
    cfg.latent.use_resource_encoder = use_resource_encoder
    cfg.latent.z_resource_dim = 32  # must match goal_dim (=world_dim=32)
    # Keep goal_dim in sync (already set to world_dim=32 by from_dims, but be explicit)
...
def run_training(agent: REEAgent, env: CausalGridWorldV2, device, n_eps: int):
    ...
    for ep in range(n_eps):
        ...
        for step in range(STEPS_PER_EPISODE):
            ...
            # FIX: use raw harm_signal on resource contact (not pre-step EMA).
            # harm_signal at resource contact = resource_benefit + proximity_benefit (~0.18+).
            # Pre-step obs_body[11] is the EMA BEFORE the step; at contact from zero it is
            # ~0.018 (0.1 * 0.18), which is below benefit_threshold=0.1 before drive mult.
            # Using harm_signal directly guarantees effective_benefit > threshold.
            if ttype == "resource":
...
def eval_goal_quality(agent: REEAgent, env: CausalGridWorldV2, device, n_eps: int) -> Dict:
    ...
    cosine_sims_resource_cond = []  # only valid for RESOURCE_SEED (z_resource != None)
    cosine_sims_world_cond = []     # for WORLD_SEED (use z_world as reference)
    benefit_episodes = 0
    use_resource_enc = (agent.latent_stack.resource_encoder is not None)
    ...
    for ep in range(n_eps):
        ...
        for step in range(STEPS_PER_EPISODE):
            ...
            with torch.no_grad():
                action = agent.select_action(candidates, ticks)
            action_idx = int(action.argmax(dim=-1).item())
            _, harm_signal_eval, done, info, obs_dict = env.step(action_idx)

            benefit_exp = float(info.get("benefit_exposure", 0.0))
            ep_benefit += benefit_exp
            ttype_eval = info.get("transition_type", "none")

            # FIX: use raw harm_signal on resource contact for z_goal seeding.
            if ttype_eval == "resource":
                drive_lvl = float(REEAgent.compute_drive_level(obs_body))
                agent.update_z_goal(float(harm_signal_eval), drive_level=drive_lvl)

            # z_goal vs reference latent cosine similarity
            # Measure after seeding so the first-contact t=0 norm is captured.
            if agent.goal_state is not None and agent.goal_state.is_active():
                with torch.no_grad():
                    ...
                    if use_resource_enc and latent.z_resource is not None:
                        # RESOURCE_SEED: compare z_goal to z_resource
                        z_ref = latent.z_resource
                        if z_ref.ndim == 1:
                            z_ref = z_ref.unsqueeze(0)
                        if z_g.shape[-1] == z_ref.shape[-1]:
                            cos = float(F.cosine_similarity(z_g, z_ref, dim=-1).mean().item())
                            cosine_sims_resource_cond.append(cos)
                    else:
                        # WORLD_SEED: compare z_goal to z_world
                        z_ref = latent.z_world
                        if z_ref.ndim == 1:
                            z_ref = z_ref.unsqueeze(0)
                        if z_g.shape[-1] == z_ref.shape[-1]:
                            cos = float(F.cosine_similarity(z_g, z_ref, dim=-1).mean().item())
                            cosine_sims_world_cond.append(cos)
    ...
    # Return the appropriate cosine list depending on condition
    cosine_sims = cosine_sims_resource_cond if use_resource_enc else cosine_sims_world_cond
    ...
    return {
        ...
        "use_resource_encoder": use_resource_enc,
    }
...
def run_seed(seed: int, dry_run: bool = False) -> Dict:
    ...
    for condition in ["RESOURCE_SEED", "WORLD_SEED"]:
        ...
        print(
            ...
            f"n_cosine_samples={metrics['n_cosine_samples']}"
        )
...
def main():
    ...
    run_id = (
        "v3_exq_322a_sd015_resource_encoder_seeding_dry" if args.dry_run
        else f"v3_exq_322a_sd015_resource_encoder_seeding_{timestamp}_v3"
    )
    print(f"EXQ-322a start: {run_id}")
    ...
    output = {
        ...
        "bug_fix_note": (
            "EXQ-322 had latent=lat silently ignored by from_dims(**kwargs). "
            "use_resource_encoder was never True. z_resource was always None. "
            "Fix: set cfg.latent.use_resource_encoder directly after from_dims()."
        ),
    }
