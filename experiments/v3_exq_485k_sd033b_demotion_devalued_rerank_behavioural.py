...
def run(seeds: Optional[List[int]] = None, dry_run: bool = False) -> Dict:
    ...
    return {
        ...
        "n_bias_range_devalued_seeds": sum(
            1 for r in test if r["bias_range_devalued"] > BIAS_RANGE_FLOOR),
        "kth_test_bias_range_devalued": _kth_largest(
            [r["bias_range_devalued"] for r in test], MIN_PASS_SEEDS),
...
def _interpretation(result: Dict) -> Dict:
    ...
    preconditions = [
        ...
        },
        {
            ...
            # k-OF-n, DECLARED AS SUCH. The shipped readiness leg is the per-seed
            # `bias_range_devalued > BIAS_RANGE_FLOOR`, counted and required on
            # >= MIN_PASS_SEEDS seeds; `measured` is therefore the MIN_PASS_SEEDS-th
            # LARGEST per-seed range, for which `measured > threshold` is exactly
            # "at least MIN_PASS_SEEDS seeds cleared the floor". It previously
            # reported max-over-seeds, which the indexer's AUTHORITATIVE recompute
            # reads as met on a SINGLE clearing seed. Strict floor (`>`), declared
            # rather than left to the inclusive default.
            "measured": float(result["kth_test_bias_range_devalued"]),
            "threshold": BIAS_RANGE_FLOOR,
            "comparator": ">",
            "direction": "lower",
            "seeds_required": MIN_PASS_SEEDS,
            "seeds_clearing_floor": int(result["n_bias_range_devalued_seeds"]),
            "max_over_seeds_diagnostic": float(result["max_test_bias_range_devalued"]),
            "control": "ARM_2 trained-arm bias over a real candidate bank at the DEVALUED state",
            "met": result["n_bias_range_devalued_seeds"] >= MIN_PASS_SEEDS,
        },
        {
