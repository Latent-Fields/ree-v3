"""
V3-EXQ-887b: SD-014 hippocampal node-valence vector -- STRONGER-GAIN RETEST of
REPRESENTATIONAL separability and FUNCTIONAL drive-gated replay prioritisation, with the
incentive-sensitization write-path decouple ENABLED at an above-default gain magnitude.

SUPERSEDES v3_exq_887a (2026-08-07), which ran the incentive-sensitization decouple at the
ree_core substrate DEFAULT gain (rate 0.05, max 4.0, coupling 1.0) and FAILED/weakened --
but as a genuine PARTIAL improvement over the pre-fix V3-EXQ-887, not a repeat collapse:
seed 2026 cleared BOTH load-bearing criteria (C1 tau=0.835<=0.85, C2 |Spearman(w,l)|=0.896
<=0.90) while seeds 42/137 missed narrowly (tau 0.852-0.863, |Spearman| 0.919-0.925), a
clear move in from 887's wider uniform miss (tau 0.867-0.889, Spearman 0.93-0.97). The
887a autopsy (failure_autopsy_V3-EXQ-887a_2026-08-08.json, status=confirmed) found the
CLEARING seed carried the LOWEST raw resource exposure of the three -- evidence the
mechanism is genuinely DRIVE-LEVEL-gated rather than exposure-count-gated -- and routed
here (887b, /queue-experiment) with a STRONGER gain magnitude, not more episodes.

CHANGE FROM 887a (the ONLY substantive difference; instrument byte-identical otherwise):
  sensitization_rate     0.05 -> 0.10   (2x)
  sensitization_max      4.0  -> 8.0    (2x)
  sensitization_coupling 1.0  -> 2.5    (2.5x, the saturation-safe primary lever)
The strengthening is weighted toward coupling, a post-accumulation multiplier that amplifies
the drive-driven per-node g-spread WITHOUT risking the uniform-saturation regime (in which
g -> gmax everywhere and wanting collapses back to being proportional to salience,
reintroducing 887's collinearity by a new route). rate/max are only doubled -- enough to
sharpen drive-gating and lift the ceiling that likely CLIPPED 887a's productive spread,
not so much that all g saturates. Full reasoning at the SENSITIZATION_* constants below.

The instrument is byte-for-byte the V3-EXQ-887/887a instrument: its P0 synthetic-control
gate, R1-R4 readiness floors, and C1/C2/C3 acceptance thresholds (TAU_MAX 0.85, RHO_MAX
0.90, JACCARD_MAX 0.80) are UNCHANGED and remain the pre-registered criteria. The two
substrate hooks are identical to 887a:
  (1) make_config() sets incentive_sensitization_enabled=True (+ the STRONGER
      sensitization_rate/max/coupling above), routing the WANTING write through
...
Everything about the DVs, the wall-independence, and the adjudication is identical to 887a,
so a PASS here is a clean gain-magnitude sweep on the same validated instrument: it isolates
whether SD-014's dissociation is achievable at all on this substrate given a large-enough
drive-coupled gain, or whether the node vector is collapsible to a composite scalar even
then. 887a already established the readiness gate is green on this exact config (all four
channels populate, the tau instrument moves on the positive control, and the composite
null-reference is the exact arithmetic identity), so this run tests the science, not the
setup.
...
EXPERIMENT_TYPE = "v3_exq_887b_sd014_node_valence_repfunc_sensitized"
SUPERSEDES_RUN_ID = "v3_exq_887a_sd014_node_valence_repfunc_sensitized_20260807T214013Z_v3"
...
# V3-EXQ-887b uses a STRONGER-THAN-DEFAULT gain magnitude. 887a ran the ree_core
# substrate DEFAULTS (rate 0.05, max 4.0, coupling 1.0) and produced a genuine PARTIAL
# improvement -- seed 2026 cleared both load-bearing criteria (tau 0.835, |Spearman(w,l)|
# 0.896) while seeds 42/137 missed narrowly (tau 0.852-0.863, |Spearman| 0.919-0.925 vs
# the 0.85/0.90 ceilings). The 887a autopsy (failure_autopsy_V3-EXQ-887a_2026-08-08.json)
# found the CLEARING seed had the LOWEST raw resource exposure of the three -- consistent
# with the mechanism being genuinely DRIVE-LEVEL-gated, not exposure-count-gated -- and
# routed here (887b, /queue-experiment) with a stronger gain rather than more episodes.
#
# WHY THESE VALUES (saturation-aware). The write is
#     wanting = benefit_salience * (1 + coupling * g),  g <- min(gmax, g + rate*drive_level)
# with drive_level = 1.0 - energy in [0, 1] (SD-012). Decoupling from LIKING (which reads
# raw benefit_salience) comes from g VARYING across nodes in a drive-driven way orthogonal
# to salience. The failure to AVOID: if g saturates UNIFORMLY at gmax, wanting collapses
# back to salience*(1+coupling*gmax) proportional to salience -- reintroducing the exact
# collinearity 887 measured, by a new route. So the strengthening is weighted toward the
# saturation-SAFE lever:
#   * coupling 1.0 -> 2.5 : a POST-accumulation multiplier; amplifies the existing per-node
#       g-spread directly and CANNOT itself drive uniform saturation. Primary lever.
#   * rate 0.05 -> 0.10   : 2x only. Sharper drive-gating within the fixed 40-ep budget
#       without collapsing all g toward the ceiling (a 10x rate would).
#   * max 4.0 -> 8.0      : 2x headroom. At rate 0.05 a frequently-visited high-drive node
#       plausibly already hit the 4.0 ceiling in 887a (~160 wanting writes/node over 40x120
#       steps, per-visit increment up to 0.05), CLIPPING the productive inter-node spread --
#       the likely cause of the marginal miss. The higher ceiling lets that spread survive.
# The run's own C2 instrument (|Spearman(w,l)| <= 0.90) catches an over-shoot into the
# uniform-saturation regime as a fresh C2 failure, so the design is self-checking.
...
def _run(dry_run: bool):
    ...
    manifest = {
        ...
        "custom_information": {
            ...
            "gov_reuse_1_check": (
                "Decisive readout = C1/C2/C3 valence-discrimination stats (mean pairwise "
                "Kendall tau, |Spearman(w,l)|, top-K Jaccard) under a STRONGER-THAN-DEFAULT "
                "incentive-sensitization gain (rate 0.10 / max 8.0 / coupling 2.5). This is "
                "a NEW manipulation: no recorded run carries these gain values. The nearest "
                "predecessors 887 (default write-path, no sensitization) and 887a (default "
                "gain 0.05/4.0/1.0) both recorded the readout but at DIFFERENT operating "
                "points, so their numbers answer a different question and cannot be reused. "
                "The stronger-gain readout is neither recorded nor derivable post-hoc "
                "(the gain changes the WANTING write at accumulation time, not a "
                "reprocessing of stored valence). Not recoverable -> run."
            ),
        },
    }
