...
"""V3-EXQ-535a: SD-029 P3 Eval Fix -- Dedicated Post-Training Evaluation Phase

Root-cause fix for V3-EXQ-535 C1 FAIL (n_ext < 20).

Diagnosis (2026-05-07):
  EXQ-535 reuses P0 data collection episodes for the P3 counterfactual
  evaluation (events_self / events_ext are collected during the same 120
  episodes used to build the transition buffer for P1/P2 training).

  Fix 3 in EXQ-535 switched injection to interval=20, prob=0.5 to grow
  the buffer from ~950 to ~2000+ transitions. But at this rate + the
  30%-hazard-food-attraction environment, the agent survives only ~3 steps
  per episode on average before the externally-injected hazard kills it.
  With interval=20, the injection fires at most once per 3-step episode:
  if prob=0.5, expected ext events = 120 eps * 1 event/ep * 0.5 = ~60.
  But with a 3-step episode and interval=20 the injection fires on step 0
  of *every 20th step*, so only ~1/7 of episodes even reach the injection
  step. Actual: ~10 ext events/seed (EXQ-535 manifest).

  C1 requires n_ext >= 20 AND n_self >= 20 per seed.
  C1 failed in ALL 3 seeds of EXQ-535 (n_ext=8/11/9).

Fix: Dedicated post-training P3 evaluation phase (completely separate from
     the P0 data-collection episodes).

  P3 design:
  - Runs AFTER P1 and P2 graduation (on the trained agent).
  - Uses more aggressive injection: interval=5, prob=1.0 (every 5 steps,
    guaranteed). Roughly 4-6x more ext events per episode than interval=20.
  - Auto-reset on done=True: if the agent dies, reset the episode
    immediately and keep collecting. Episode death does not end P3.
  - Target: collect 50+ ext events per seed (far above the C1 threshold
    of 20).
  - Budget: up to 10,000 steps total (safety cap).
  - P3 uses the same trained HarmEncoder + E2HarmSForward weights from
    P2 graduation -- the forward model is already trained, P3 just
    evaluates it on a richer event sample.

P0/P1/P2 phases: IDENTICAL to EXQ-535.
Acceptance criteria (C0-C4): IDENTICAL to EXQ-535.

claim_ids: ["SD-029", "MECH-256"]
experiment_purpose: evidence
supersedes: V3-EXQ-535
"""
...
EXPERIMENT_TYPE = "v3_exq_535a_sd029_p3_eval_fix"
QUEUE_ID = "V3-EXQ-535a"
CLAIM_IDS = ["SD-029", "MECH-256"]
...
# P1 graduation gate (same as EXQ-535)
GRAD_THRESHOLD = 0.65
...
# P3 eval parameters (NEW vs EXQ-535)
P3_EXT_INTERVAL = 5         # inject every 5 steps (vs 20 in P0)
P3_EXT_PROB = 1.0           # guaranteed injection (vs 0.5 in P0)
P3_TARGET_EXT = 50          # collect at least this many ext events
P3_TARGET_SELF = 50         # collect at least this many self events
P3_MAX_STEPS = 10000        # hard cap
P3_STEPS_PER_EP = 200       # episode length in P3 (same as P0)
...
def _p1_graduated_train(agent, harm_head, harm_opt, transitions, seed,
    ...
    Target: max(h_obs[:HAZARD_FIELD_DIMS]) -- same as EXQ-535.
    Linear R2 ceiling: 0.798. Graduation gate: 0.65.
    """
...
def _p3_eval_phase(agent, env_kwargs_base, seed, action_dim, rng, dry_run=False):
    """Dedicated post-training P3 evaluation phase.

    Runs fresh episodes on the trained agent with aggressive external hazard
    injection (interval=5, prob=1.0) and auto-reset on agent death.
    Collects events until P3_TARGET_EXT and P3_TARGET_SELF are met,
    or P3_MAX_STEPS total steps are reached.

    Returns: events_self, events_ext, n_steps_total
    """
    # Build a fresh env with P3 injection parameters
    p3_kwargs = dict(**env_kwargs_base)
    # Enable aggressive injection for event collection
    ...
    p3_kwargs["seed"] = seed + 9000  # fresh seed for eval phase
    ...
    max_steps = 200 if dry_run else P3_MAX_STEPS
    target_ext = 5 if dry_run else P3_TARGET_EXT
    target_self = 5 if dry_run else P3_TARGET_SELF
    ...
    while total_steps < max_steps:
        ...
        # Mixed policy: 40% epsilon-random, 60% heuristic (same as P0)
        if p3_rng.random() < P0_EPSILON:
            action = int(p3_rng.integers(0, action_dim))
        else:
            action = _heuristic_action(p3_env, reef_cells, p3_kwargs.get("reef_enabled", False), rng)
        ...
        # Auto-reset on done (don't stop collection)
        if done:
            _, obs_dict = p3_env.reset()
            reef_cells = getattr(p3_env, "_reef_cells", set())

        # Stop when both targets met
        if n_self >= target_self and n_ext >= target_ext:
            break

    return events_self, events_ext, total_steps


def _eval_cf_gaps(agent, events_self, events_ext, action_dim):
    """Evaluate counterfactual gaps on the given events."""
    cf_gaps_self = []
    cf_gaps_ext = []
    ...
    with torch.no_grad():
        for ev_list, cf_list in [(events_self, cf_gaps_self),
            ...
            for ev in ev_list:
                ...
                a_actual_oh = _action_onehot(act, action_dim)
                a_cf_oh = _action_onehot(act_cf, action_dim)
    ...
    return mean_cf_gap_self, mean_cf_gap_ext
...
def _run_arm(arm_id, env_kwargs, n_episodes, steps_per_ep, n_seeds, p0_epsilon,
    ...
    for seed_idx in range(n_seeds):
        ...
        # --- P0: Data collection (training buffer + baseline event counts) ---
        transitions = []
        p0_events_self = []
        p0_events_ext = []
        n_self_p0 = 0
        ...
        for ep in range(n_episodes):
            ...
            for _step in range(steps_per_ep):
                ...
                if is_self:
                    p0_events_self.append({"harm_obs_t": h_obs.clone(), "action": action})
                    n_self_p0 += 1
                if is_ext:
                    p0_events_ext.append({"harm_obs_t": h_obs.clone(), "action": action})
                    n_ext_p0 += 1
        ...
        if dry_run:
            print(
                ...
                f"n_self={n_self_p0} n_ext={n_ext_p0} transitions={len(transitions)}"
            )

        # --- P1: Train HarmEncoder ---
        ...
        if not p1_grad:
            seed_results.append({
                ...
                "n_self": n_self_p0, "n_ext": n_ext_p0,
                "p0_n_self": n_self_p0, "p0_n_ext": n_ext_p0,
                "p3_n_self": 0, "p3_n_ext": 0, "p3_steps": 0,
        ...
        # Re-encode transitions with updated encoder
        new_transitions = []
        with torch.no_grad():
        ...
        # Held-out forward model R2 (same as EXQ-535)
        n_tr = len(transitions)
        ...
        # --- P3: Dedicated evaluation phase (NEW vs EXQ-535) ---
        # For ARM_0 (baseline, no injection), skip the injected P3 eval
        # and use a simple event collection without injection.
        # For ARM_1 (reef+food+ext), run the dedicated P3 eval.
        if env_kwargs.get("scheduled_external_hazard_enabled", False):
            # Strip the training-injection params and add P3-specific ones
            # (P3 uses its own env instance with aggressive injection)
            base_kw = {k: v for k, v in env_kwargs.items()
                       if not k.startswith("scheduled_external_hazard")}
            p3_events_self, p3_events_ext, p3_steps = _p3_eval_phase(
                agent, base_kw, seed, env.action_dim, rng, dry_run=dry_run
            )
        else:
            # ARM_0: just re-run a short eval without injection
            # (confirms n_ext == 0 for C4)
            p3_events_self, p3_events_ext, p3_steps = [], [], 0

        n_self_p3 = len(p3_events_self)
        n_ext_p3 = len(p3_events_ext)

        if dry_run:
            print(
                f"  arm={arm_id} seed={seed_idx} P3: "
                f"n_self={n_self_p3} n_ext={n_ext_p3} steps={p3_steps}"
            )

        # C1 counts come from P3 (not P0) for ARM_1
        # C4 check (n_ext == 0 for ARM_0) uses P0 counts
        n_self_for_c1 = n_self_p3 if p3_events_self or p3_steps > 0 else n_self_p0
        n_ext_for_c1 = n_ext_p3 if p3_events_ext or p3_steps > 0 else n_ext_p0

        # Evaluate counterfactual gaps on P3 events
        events_self_eval = p3_events_self if p3_events_self else p0_events_self
        events_ext_eval = p3_events_ext if p3_events_ext else p0_events_ext

        mean_cf_gap_self, mean_cf_gap_ext = _eval_cf_gaps(
            agent, events_self_eval, events_ext_eval, env.action_dim
        )

        if dry_run:
            print(
                f"  arm={arm_id} seed={seed_idx} cf_gaps: "
        ...
        seed_results.append({
            "arm_id": arm_id, "seed_idx": seed_idx,
            # Merged n_self / n_ext (P3 counts for ARM_1, P0 for ARM_0)
            "n_self": n_self_for_c1, "n_ext": n_ext_for_c1,
            "p0_n_self": n_self_p0, "p0_n_ext": n_ext_p0,
            "p3_n_self": n_self_p3, "p3_n_ext": n_ext_p3,
            "p3_steps": p3_steps,
...
def run_experiment(dry_run=False):
    ...
    arms_def = [
        ...
        ("ARM_1_reef_food_ext",
         dict(**common,
              ...
              # P0 training injection (same as EXQ-535 Fix 3)
    ...
    if not graduated:
        ...
        # C0: both arms graduated (P1+P2)
        c0 = all(
            r["p1_graduated"] and r["p2_graduated"] for r in arm1_results
        )
        # C1: sufficient event counts (using P3 eval counts for ARM_1)
        c1_events = all(
            r["n_self"] >= 20 and r["n_ext"] >= 20 for r in arm1_results
        )
        # C1 also requires forward model quality
        c1_r2 = _min_val(arm1, "harm_forward_r2") >= 0.9
        c1 = c1_events and c1_r2
        # C2: self-caused events have positive cf_gap
        c2 = _agg(arm1, "mean_cf_gap_self") > 0.0
        # C3: self-caused gap > externally-caused gap
        c3 = _agg(arm1, "mean_cf_gap_self") > _agg(arm1, "mean_cf_gap_ext")
        # C4: ARM_0 has no external events (P0 counts)
        c4 = _agg(arm0, "p0_n_ext") == 0

        overall_pass = c0 and c1 and c2 and c3
        outcome = "PASS" if overall_pass else "FAIL"

        sd029_dir = "supports" if (c2 and c3) else "weakens"
        mech256_dir = "supports" if c1_r2 else "weakens"
    ...
    return {
        ...
        "criteria": {
            ...
            "c1_events": c1_events if graduated else False,
            "c1_r2": c1_r2 if graduated else False,
        },
        ...
        "p3_ext_interval": P3_EXT_INTERVAL,
        "p3_ext_prob": P3_EXT_PROB,
        "p3_target_ext": P3_TARGET_EXT,
        "p3_target_self": P3_TARGET_SELF,
        "p3_max_steps": P3_MAX_STEPS,
    }
...
def write_result(result, run_id):
    ...
    manifest = {
        ...
        "experiment_purpose": "evidence",
        "supersedes": "v3_exq_535_sd029_p1_target_fix",
...
if __name__ == "__main__":
    ...
    t0 = time.time()
    print("V3-EXQ-535a SD-029 P3 Eval Fix")
    print(f"dry_run={args.dry_run}")
    print("P1 target: max(hazard_field_view)  ceiling_r2=0.798  gate=0.65")
    print(f"P3 eval: interval={P3_EXT_INTERVAL} prob={P3_EXT_PROB} "
          f"target_ext={P3_TARGET_EXT} max_steps={P3_MAX_STEPS}")
    ...
    for arm_id, stats in result["per_arm"].items():
        print(
            f"  {arm_id}: "
            f"p0_n_self={stats['p0_n_self']:.0f} p0_n_ext={stats['p0_n_ext']:.0f} "
            f"p3_n_self={stats['p3_n_self']:.0f} p3_n_ext={stats['p3_n_ext']:.0f} "
