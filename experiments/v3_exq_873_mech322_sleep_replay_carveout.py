"""V3-EXQ-873 -- MECH-322 sleep-replay carve-out: does the AND-gate actually fire?
...
Zero genuine experimental evidence exists for MECH-322 (backlog EVB-0489,
...
never executed into the atomic-commit pool (MECH-094). This run is the FIRST
to flip that flag and exercise the entry point at all. Nine prior manifests
carry the `chunk_acc_n_replay_formed` diagnostic key (v3_exq_810a/810b/834/815/
816/820/... -- confirmed via `reanalysis_query.py query --readout
chunk_acc_n_replay_formed`), but every one reads the audit-null value: the flag
was False in all of them (the MECH-094-strict default), so none of them
answers "does the carve-out actually mint, refuse, and expire correctly" --
GOV-REUSE-1: not recoverable, run.
...
GOV-REUSE-1 (Step 2.4). Decisive readout: whether `note_chunk_replay_sequence`
mints under a genuine real-value-tagged sleep-phase attempt
(chunk_acc_n_replay_formed / chunk_lib_n_replay_origin), and whether it
correctly refuses under each of the three failure variants. Checked via
`REE_assembly/scripts/reanalysis_query.py query --readout
chunk_acc_n_replay_formed --require-readout`: 9 manifests carry the key
(v3_exq_810a, 810b, 834, 815, 816, 820, plus others in the ARC-070/ARC-071
lineage), spanning several distinct substrate_hash groups. All 9 ran with
`use_chunk_replay_origin_path=False` (the module's own audit convention: the
counter "must stay 0 unless the carve-out is deliberately on"), so every one
reads 0 by construction -- none exercises the flag ON. Re-derive brake (Step
2.5b): 0 prior failure_autopsy targets tag MECH-322 with a substrate_ceiling
disposition (checked `failure_autopsy_*.json` for `claim_ids` containing
MECH-322; none exist at all -- zero autopsies reference this claim). Not
recoverable -> run.

Substrate readiness (Step 2.5). `use_chunk_replay_origin_path` -- confirmed
read at `ree_core/agent.py:1357-1359` into `PolicyChunkingConfig`, and
`chunk_replay_value_quantile` / `chunk_replay_corroboration_episodes` at
1360-1365. Entry point confirmed wired end-to-end: `REEAgent.
note_chunk_replay_sequence` (agent.py:8649) -> `PolicyChunking.
note_replay_sequence` (policy_chunking.py:2068) -> `ChunkAccumulator.
record_replay_sequence` (policy_chunking.py:1305), which checks the flag,
then `in_sleep_phase`, then size, then the value bar, in that order, each
branch incrementing `_n_replay_refusals` and returning None. The default
`in_sleep_phase=None` path reads the REAL substrate authority
(`self.serotonin.phase`, agent.py:8674-8676), not a caller override -- this
run deliberately uses that default path (never passing `in_sleep_phase=`
explicitly) so it tests the actual SD-017 wiring, not a bypass. MECH-323/324
substrate (chunk formation + crystallisation + dissolution) is independently
confirmed live by V3-EXQ-810a (PASS, chunk_accumulator_fires, 2026-07-30) at
this exact env/schedule -- reused verbatim here (ENV_KWARGS, N_EPISODES,
STEPS_PER_EPISODE, chunk_min_repetitions/window/crystallisation_min all from
`experiments/_lib/baselines/arc071_chunking.py`) so this run inherits that
confirmed reliability rather than re-establishing it.
...
corroboration-deadline readout (`minted_chunk_state_end_of_run`,
`minted_episodes_since_corroboration_end_of_run`) is a categorical/count state
of the ONE tracked chunk object created by PROBE 2, likewise not an
aggregate. No symmetry group renders any of these manipulations invisible to
their own DV.
...
EXPERIMENT_TYPE = "v3_exq_873_mech322_sleep_replay_carveout"
...
# 3 seeds, not 810a's 8. Rationale: MECH-323/324 real-execution formation
# reliability is ALREADY confirmed at 8/8 seeds (V3-EXQ-810a), so this run's
# open question -- does record_replay_sequence's own AND-gate logic fire and
# refuse correctly -- is a near-deterministic plumbing check given a real
# candidate exists, not a stochastic "does it happen" measure. Seeds vary only
# whether a qualifying real candidate is available by the checkpoint episode.
SEEDS = [101, 202, 303]
...
SEED_PASS_FRACTION = 1.0  # deterministic plumbing check, no stochastic slack
...
PRECONDITIONS = [
    ...
    PreconditionSpec(
        name="replay_high_value_candidate_available",
        description=("MECH-322 condition (a)'s own precondition: at the "
                     "sleep checkpoint, at least one tracked real sub-sequence "
                     "must have an outcome mean clearing "
                     "ChunkAccumulator.replay_value_threshold() (the SAME "
                     "statistic condition (a) itself compares against) or "
                     "PROBE 2 (the should-succeed cell) is structurally "
                     "unattemptable this seed."),
        control=("worst seed in this arm; measured as the tracked "
                 "candidate's margin above the threshold (mu - threshold), "
                 "so a seed with no candidate at all reads a large negative "
                 "sentinel rather than a missing value"),
        threshold=0.0,
        direction="lower",
        kind="readiness",
        applies_to=lambda ctx: bool(ctx["chunking_on"]),
...
def run_experiment() -> Dict[str, Any]:
    ...
    for arm in ARMS:
        ...
        cand_w, cand_cell = _worst(cells, "replay_candidate_margin")
        arm_gates.append(evaluate_arm_gate(
            ...
            measured={
                ...
                "replay_high_value_candidate_available": cand_w,
            },
        ))
        arm_gates[-1]["offending_cells"] = {
            ...
            "replay_high_value_candidate_available": cand_cell,
        }
    ...
    # --- C1 (LOAD-BEARING): the carve-out mints under all three valid conditions.
    c1_pass = frac(
        ARM_REPLAY_ON,
        lambda r: bool(r["success_minted"] and r["success_minted_replay_origin"]),
    ) >= SEED_PASS_FRACTION
    ...
    criteria_non_degenerate = arm_criteria_non_degenerate(
        criteria_by_arm, agg,
        extra={
            "C1_carveout_fires_under_valid_conditions": c1_arm_green,
