"""V3-EXQ-542a: ARC-062 Phase 1 substrate-readiness refresh (one-hot head input).

Supersedes V3-EXQ-542 (2026-05-09 PASS). Re-runs UC1-UC5 unchanged and adds UC6
for the GAP-B option-2 first-action one-hot head-input path landed 2026-05-17
(EXQ-543e autopsy: z_world-only heads were starved of first-action signal).
...
Five sub-tests (deterministic arithmetic + API integration):
  ...
  UC6 first-action one-hot head input (GAP-B option-2): with
      use_first_action_onehot=True the scoring-head input dim expands to
      world_dim + action_dim; different per-candidate one-hots change head
      outputs on a fixed candidate_features tensor; REEAgent wires the flag.

Run with:
  /opt/local/bin/python3 experiments/v3_exq_542a_arc062_gated_policy_substrate_readiness_onehot.py
...
# ----------------------------------------------------------------------
# UC6 first-action one-hot head input (GAP-B option-2)
# ----------------------------------------------------------------------
def run_uc6_first_action_onehot_head_input() -> dict:
    """UC6: one-hot augments head input; agent flag wires first_action_dim."""
    torch.manual_seed(0)
    action_dim = 4
    world_dim, self_dim, harm_a_dim = 32, 16, 16
    K = 4
    ...
    onehot_a = torch.zeros(K, action_dim)
    for i in range(K):
        onehot_a[i, i % action_dim] = 1.0
    onehot_b = torch.zeros(K, action_dim)
    for i in range(K):
        onehot_b[i, (i + 2) % action_dim] = 1.0
    ...
    with torch.no_grad():
        ...
        out_b = gp(
            ...
            z_harm_a=za,
            candidate_features=cand,
            first_action_onehots=onehot_b,
            simulation_mode=False,
        )
    head_delta = float(
        (out_a.head_0_bias - out_b.head_0_bias).abs().mean().item()
    )

    agent_cfg = REEConfig.from_dims(
        body_obs_dim=12,
    ...
    agent = REEAgent(agent_cfg)
    agent_onehot_on = (
        agent.gated_policy is not None
        and agent.gated_policy.config.use_first_action_onehot
        and agent.gated_policy.config.first_action_dim == action_dim
        and agent.gated_policy._head_in_dim == world_dim + action_dim
    )

    result = {
        "head_in_dim": int(gp._head_in_dim),
        "expected_head_in_dim": world_dim + action_dim,
        "onehot_was_none_flag": bool(gp._last_onehot_was_none),
        "head_output_delta_across_onehots": head_delta,
        "agent_onehot_wired": agent_onehot_on,
    }
    result["pass"] = (
        result["head_in_dim"] == result["expected_head_in_dim"]
        and not result["onehot_was_none_flag"]
        and result["head_output_delta_across_onehots"] > 1e-6
        and result["agent_onehot_wired"]
    )
    return result
...
def main() -> tuple[str, str]:
    ...
    uc6 = run_uc6_first_action_onehot_head_input()

    all_pass = all(r["pass"] for r in [uc1, uc2, uc3, uc4, uc5, uc6])
    elapsed = time.time() - t0

    run_id = "v3_exq_542a_arc062_gated_policy_substrate_readiness_onehot_v3"
    ts = datetime.utcnow().strftime("%Y%m%dT%H%M%SZ")
    manifest = {
        "schema_version": "v1",
        "run_id": f"{run_id}_{ts}",
        "queue_id": "V3-EXQ-542a",
        "supersedes": "V3-EXQ-542",
        "experiment_type": "v3_exq_542a_arc062_gated_policy_substrate_readiness_onehot",
        ...
        "metrics": {
            ...
            "UC6_first_action_onehot_head_input": uc6,
        },
        "elapsed_seconds": elapsed,
        "notes": (
            "ARC-062 Phase 1 substrate-readiness refresh (supersedes "
            "V3-EXQ-542). UC1-UC5 identical to 542; UC6 validates "
            "gated_policy_use_first_action_onehot + agent wiring for "
            "GAP-B option-2 (post EXQ-543e autopsy). Does NOT test "
            "MECH-309/ARC-062 behavioural claims (Phase 2 falsifier "
            "chain on SD-054)."
        ),
    }
    ...
    return str(out_path), manifest["outcome"]


if __name__ == "__main__":
    from experiment_protocol import emit_outcome

    print("Seed 0 Condition substrate_readiness", flush=True)
    print("  [train] label seed=0 ep 1/1 UC battery", flush=True)
