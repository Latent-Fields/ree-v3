...
EXQ-095 FAIL: harm_fwd_r2 = 0.0 with 300 training episodes.
Root cause undiagnosed -- no loss curves or gradient diagnostics were captured.

This experiment is a diagnostic rerun with full instrumentation to answer:
  (a) Zero-gradient problem? HarmForwardModel gets no gradient signal.
  (b) Insufficient training? Needs more episodes to converge.
  (c) Signal learnability problem? Full harm_obs_s has un-learnable components.

Diagnostic tests run in parallel:
  FULL: HarmForwardModel trained on full z_harm_s (from all 51 harm_obs_s dims)
  HAZARD: HarmForwardModel_hazard trained on z_harm_s_hazard (first 25 dims only --
    hazard proximity, which is purely action-predictable via movement geometry)

Rationale for HAZARD-only test:
  harm_obs_s [51 dims] = hazard_proximity[25] + resource_proximity[25] + exposure[1].
  Resource proximity changes stochastically when resources respawn -- not action-predictable.
  Hazard proximity changes only with agent movement -- fully action-predictable.
  If resource noise swamps the forward model, HAZARD-only will succeed while FULL fails.
  This distinguishes signal-learnability failure from gradient or capacity failure.

Instrumentation added vs EXQ-095:
  - hf_loss_log: HarmForwardModel MSE loss every LOG_INTERVAL training steps
  - grad_norm_log: HarmForwardModel gradient norm every LOG_INTERVAL steps
  - hf_hazard_loss_log: HAZARD model loss (parallel, same interval)
  - Phase 1 extended: 900 episodes (3x EXQ-095)

Diagnostic criteria (ALL required for PASS, but partial results are informative):
  C1: max_grad_norm_hf > 1e-5        -- gradients flowing (rules out detach bug)
  C2: hf_loss_final < hf_loss_init * 0.5  -- FULL loss halved
  C3: harm_fwd_r2_hazard > 0.10     -- HAZARD model learns structure
  C4: harm_fwd_r2 > 0.20            -- FULL model learns (original EXQ-095 C1)
  C5: causal_approach > 0.001       -- counterfactual signal non-trivial

Supersedes: V3-EXQ-095
"""
...
EXPERIMENT_TYPE = "v3_exq_095a_harm_fwd_diagnosis"
CLAIM_IDS = ["ARC-033", "SD-011"]

HARM_OBS_DIM     = 51   # 25 hazard + 25 resource + 1 exposure
HAZARD_DIM       = 25   # first 25 dims: hazard proximity fields only
HARM_OBS_A_DIM   = 50
Z_HARM_DIM       = 32
...
MAX_HF_DATA      = 4000
LOG_INTERVAL     = 100   # log loss + grad every N training steps
...
def run(
    ...
    **kwargs,
    ...
    # FULL stream: harm encoder for all 51 dims
    ...
    # HAZARD stream: encoder for first 25 dims only (hazard proximity)
    ...
    # Affective encoder (diagnostic only, not in criteria)
    ...
    print(
        f"[V3-EXQ-095a] HarmForwardModel training diagnosis\n"
        ...
        f"  Phase1: {phase1_episodes} eps (3x EXQ-095)"
    ...
    wf_data: List         = []   # (z_world_t, action_t, z_world_t1)
    hf_data: List         = []   # (z_harm_s_t, action_t, z_harm_s_t1)
    hf_hazard_data: List  = []   # (z_harm_hz_t, action_t, z_harm_hz_t1)
    ...
    # -- Phase 1: terrain + HarmForwardModel training (3x longer than EXQ-095) --
    print(
    ...
    for ep in range(phase1_episodes):
        ...
        for _ in range(steps_per_episode):
            ...
            # Encode harm streams (detached -- forward models train on stored reps)
            ...
            # Train HarmEncoder (FULL) + E3 harm head jointly on proximity label
            ...
            # Collect FULL harm-forward transitions
            ...
            # Collect HAZARD harm-forward transitions
            ...
            # Train HAZARD HarmForwardModel (separate backward pass)
            hf_hz_loss_val = 0.0
            ...
            # World-forward data (z_world_prev -> z_world_curr)
            ...
            # Diagnostic logging every LOG_INTERVAL steps
            step_counter += 1
    ...
    # Evaluate FULL HarmForwardModel R2
    ...
    print(
        f"  FULL  HarmForwardModel R2: {harm_fwd_r2:.4f}  (baseline bridge: 0.0)",
        flush=True,
    )
    print(
    ...
    # Use whichever model is better for counterfactuals
    ...
    for ep in range(eval_episodes):
        ...
        for _ in range(steps_per_episode):
            with torch.no_grad():
                ...
                # Counterfactual via best available forward model
                cf_vals: List[float] = []
    ...
    print(f"\n  --- EXQ-095a diagnostic results ---", flush=True)
    ...
    if not c1:
        failure_notes.append(
            f"C1 FAIL: max_grad_norm={max_grad_norm:.6f} -- no gradient"
            " flow (detach bug or disconnected graph)")
    if not c2:
        failure_notes.append(
            f"C2 FAIL: loss not halved"
            f" (init={hf_loss_init:.5f} final={hf_loss_final:.5f})"
            " -- needs more episodes or lr increase")
    if not c3:
        failure_notes.append(
            f"C3 FAIL: hazard_r2={harm_fwd_r2_hazard:.4f} -- hazard proximity"
            " not action-learnable")
    if not c4:
        failure_notes.append(
    ...
    print(f"V3-EXQ-095a verdict: {status}  ({n_met}/5)", flush=True)
    for note in failure_notes:
        print(f"  {note}", flush=True)

    # Loss curve thirds (early / mid / late)
    ...
    summary_markdown = f"""# V3-EXQ-095a -- ARC-033 / SD-011 HarmForwardModel Training Diagnosis

**Status:** {status}
**Claims:** ARC-033, SD-011
**Supersedes:** EXQ-095 (R2=0.0, undiagnosed failure)
**Phase 1:** {phase1_episodes} eps (3x EXQ-095)
...
- HAZARD_ONLY_WORKS: Use hazard-only (first {HAZARD_DIM}d) for ARC-033. Resource
...
if __name__ == "__main__":
    ...
    parser.add_argument("--phase1", type=int, default=900)
