"""
V3-EXQ-468e (supersedes V3-EXQ-468d): SD-034 / MECH-268 / MECH-090 commitment-vs-
contradiction de-commit-hold retest, on the foraging-competent scaffolded_sd054_onboarding
603n agent WITH the BETA-ENGAGEMENT amend live (use_closure_commit_beta_coupling,
ree-v3 main f4ceea4).

WHY THIS SUPERSEDES 468d (failure_autopsy_V3-EXQ-460e_2026-06-17, confirmed): 468d's
commitment non-vacuity readiness gate (total_beta_elevated > 0 on both arms) is the SAME
commit-with-beta precondition that 460e showed fails on 2/3 seeds on the 603n substrate --
with both MECH-090 R-c gates OFF the bistable BetaGate elevates only on the fragile
natural running_variance crossing (result.committed), so the agent commits the
closure-plane trajectory WITHOUT engaging beta and the contradiction-release / de-commit-
hold DV cannot be exercised. The beta-engagement amend (mechanism (a),
use_closure_commit_beta_coupling, ree-v3 main f4ceea4) couples the closure-plane
commitment to bistable beta elevation, so the agent reliably commits-WITH-beta and
MECH-090 commit-entry is achieved under the trained head -- making the MECH-268
contradiction-release + Leg-B de-commit-hold DV exercisable. The mechanism under test is
unchanged from 468d; only the substrate is the beta-engagement-amended one (hence a NEW
letter, not a silent re-run). The new per-arm diagnostic sd034_n_closure_coupled_elevations
records that the coupling -- not a fluke natural commit-entry -- engaged the latch
(non-vacuity). The commitment non-vacuity gate still self-routes substrate_not_ready_requeue
if beta engagement is somehow still unmet -- NEVER a false weakens. (Lineage note: this
file's original commitment-vs-contradiction mechanism, arms, and gates below are unchanged
from 468d, which was itself the 603n-curriculum successor to V3-EXQ-468b/468c.)
...
PRE-REGISTERED ACCEPTANCE (thresholds are constants, NOT post-hoc):
  ...
  C2  (NON-CAP-PINNED de-commitment DV -- the autopsy fix for the 468c cap-pin)
      (OFF committed_frac_post_absolute - ON committed_frac_post_absolute)
       > C2_ON_LT_OFF_MARGIN, where committed_frac_post_absolute =
       committed_post / post_window_steps in [0, 1]. The Leg-B de-commit hold keeps
       ON UNcommitted after a contradiction-coupled release; OFF perseverates.
       Replaces the 468c post/pre ratio that pinned >= 1 and never dropped.
  Per-seed PASS = C1 AND C2. EXPERIMENT PASS = contact non-vacuity met AND
...
experiment_purpose: diagnostic (substrate-readiness retest of the de-commit hold).
supersedes: V3-EXQ-468c. (lineage: V3-EXQ-468b is non_contributory/substrate_ceiling,
...
EXPERIMENT_TYPE = "v3_exq_468e_sd034_mech268_decommit_hold_behavioural"
QUEUE_ID = "V3-EXQ-468e"
CLAIM_IDS: List[str] = ["SD-034", "MECH-268", "MECH-090"]
EXPERIMENT_PURPOSE = "diagnostic"
SUPERSEDES = "V3-EXQ-468d"

SEEDS = [42, 43, 44]
CONDITION_LABEL = "CURRICULUM_BUILT_DECOMMIT_HOLD_VS_CONTRADICTION"
...
# --- SD-034 commitment-closure-control-plane amend (Leg B de-commit hold) ---
...
# 468d NON-CAP-PINNED de-commitment DV (the autopsy fix for the 468c C2 cap-pin):
# the 468c C2 read committed_frac_post_vs_pre = mean_committed_post / mean_committed_pre,
# which is structurally >= 1 whenever the agent commits at all in the long post-window
# (post pins at the ~195-step window, pre ~5) -> it could never drop below 0.85 and
# the de-commit hold's effect was invisible. 468d instead reads the ABSOLUTE
# post-contradiction committed fraction (committed_post / post_window_steps, bounded
# [0, 1]) and tests ON < OFF by a margin: the Leg-B de-commit hold keeps the ON arm
# UNcommitted for the refractory window after a contradiction-coupled release, while
# the substrate-OFF arm perseveres. C2_ON_LT_OFF_MARGIN is the minimum ON<OFF gap.
C2_ON_LT_OFF_MARGIN = 0.05  # C2: (OFF committed_frac_post_abs - ON ...) must exceed this
...
def _make_config(env) -> REEConfig:
    ...
    cfg = REEConfig.from_dims(
        ...
        # SD-034 commitment-closure-control-plane amend (2026-06-12), the 468d legs:
        use_closure_env_completion_hook=True,            # Leg A (env completion -> emit_closure)
        closure_decommit_hold_ticks=CLOSURE_DECOMMIT_HOLD_TICKS,  # Leg B (de-commit hold)
        lateral_pfc_train_rule_bias_head=True,           # Leg C (GAP-D rule_state)
        ...
        # commits-with-beta (the 468d/460e commit-without-beta dissociation) and the
        # contradiction-release / de-commit-hold DV is exercisable on every seed.
...
def _eval_contradiction_behaviour(
    ...
    total_pre_steps = 0     # 468d: absolute pre-contradiction window step count
    total_post_steps = 0    # 468d: absolute post-contradiction window step count
    total_hook_fires = 0    # 468d: env-completion-hook closures
    total_closure_coupled = 0  # BETA-ENGAGEMENT amend: closure-coupled beta elevations
    ...
    with torch.no_grad():
        for _ep in range(n_eps):
            ...
            for step in range(steps_per_episode):
                ...
                # 468d NON-CAP-PINNED accounting: count ALL post-window ticks
                # (committed or not) as the absolute-fraction denominator, and the
                # committed subset as the numerator. cur_committed is the pre-step
                # committed state.
                if first_contradiction_step >= 0:
                    ep_post_steps += 1
                    if cur_committed:
    ...
    # 468d NON-CAP-PINNED DV: absolute post-contradiction committed fraction in
    # [0, 1] (committed post-window ticks / ALL post-window ticks). The de-commit
    # hold drives this DOWN on the ON arm (released + held uncommitted) relative
    # to the perseverating OFF arm.
...
def _run_seed(seed: int, dry_run: bool, total_eps: int) -> Dict[str, Any]:
    ...
    # C2 (468d NON-CAP-PINNED): the ON arm's ABSOLUTE post-contradiction committed
    # fraction is LOWER than the substrate-OFF arm's by a margin -- the de-commit
    # hold keeps ON uncommitted after a contradiction-coupled release while OFF
    # perseverates. This replaces the 468c cap-pinned post/pre ratio (mean_post pins
    # at the ~195-step window so the ratio was structurally >= 1).
    decommit_gap = arm_off["committed_frac_post_absolute"] - arm_on["committed_frac_post_absolute"]
    c2 = bool(decommit_gap > C2_ON_LT_OFF_MARGIN)

    # Commitment non-vacuity: the ON arm actually committed AND a contradiction
    # fired during the eval AND both arms have a non-empty post-contradiction window
    # (so the absolute-fraction DV denominator is meaningful, never a vacuous read).
    commitment_non_vacuity = bool(
        arm_on["total_beta_elevated"] > 0
        and arm_on["episodes_with_contradiction"] > 0
        and arm_on["total_post_steps"] > 0
        and arm_off["total_post_steps"] > 0
    )

    seed_pass = bool(c1 and c2)

    print(f"  [train] decommit_eval seed={seed} ep {done}/{total_eps}"
          f" c1={c1} c2={c2} decommit_gap={decommit_gap:.4f}"
          f" on_cfp_abs={arm_on['committed_frac_post_absolute']:.4f}"
          f" off_cfp_abs={arm_off['committed_frac_post_absolute']:.4f}"
          f" hook_fires={arm_on['n_hook_fires']}"
        ...
        "interpretation": {
            ...
            "criteria": [
                {"name": "C1_beta_release_near_contradiction", "load_bearing": True,
                 "passed": c1_all},
                {"name": "C2_committed_frac_post_absolute_on_lt_off", "load_bearing": True,
                 "passed": c2_all},
            ],
            ...
            "commitment_non_vacuity_gate": {
                ...
                "c2_on_lt_off_margin": C2_ON_LT_OFF_MARGIN,
            },
            "non_cap_pinned_dv": {
                "definition": "468d C2 reads the ABSOLUTE post-contradiction committed "
                              "fraction committed_post / post_window_steps in [0, 1] and tests "
                              "ON < OFF by C2_ON_LT_OFF_MARGIN (the Leg-B de-commit hold keeps "
                              "ON uncommitted after a contradiction-coupled release while OFF "
                              "perseverates). Replaces the 468c cap-pinned post/pre ratio that "
                              "could never drop below 0.85.",
                "c2_on_lt_off_margin": C2_ON_LT_OFF_MARGIN,
            },
