...
"""V3-EXQ-541a -- MECH-204 precision recalibration consumer (F1 validation;
SLEEP DRIVER: K=2 multi-fire (SleepLoopManager, sleep_loop_episodes_K=2, fires every 2 episodes)
supersedes V3-EXQ-541).

Sleep-substrate plan Phase 1 (REE_assembly/evidence/planning/
sleep_substrate_plan.md, GAP-1) under the F1 substrate fix landed
2026-05-09. SerotoninModule now maintains a cross-cycle persistent
zero-point reference (EMA-tracked across REM captures with configurable
alpha), and compute_recalibration_target() returns that persistent value
rather than the just-captured precision_at_rem_entry. This breaks the
within-cycle no-op pattern V3-EXQ-541 surfaced (target == rv at writeback
by construction -> Option A linear interpolation collapses; mean_abs_delta
9e-8, six orders of magnitude under the 1e-3 threshold).

Same 2-arm ablation as V3-EXQ-541: ARM_0 (use_rem_precision_recalibration
=False, baseline) and ARM_1 (ON, step=0.1). The substrate change makes
the consumer read a stable long-horizon reference, so cycles 2..N have
target != rv whenever waking drift moves rv between captures, and Option
A produces measurable rv movement with sign consistent with the target.

Acceptance (unchanged from V3-EXQ-541):
...
EXPERIMENT_TYPE = "v3_exq_541a_mech204_precision_recalibration_f1"
CLAIM_IDS = ["MECH-204"]
...
def _make_agent(env: CausalGridWorldV2, seed: int, *, recalibration_on: bool) -> REEAgent:
    ...
    cfg = REEConfig.from_dims(
        ...
        # MECH-204 consumer (under test) -- F1 cross-cycle persistent reference
...
def main(dry_run: bool = False):
    ...
    print(
        f"V3-EXQ-541a MECH-204 recalibration consumer F1 -- {outcome} in {elapsed:.1f}s",
        flush=True,
    )
    if dry_run:
    ...
    manifest = {
        ...
        "notes": (
            "F1 validation of MECH-204 precision recalibration consumer "
            "under the cross-cycle persistent zero-point reference (EMA "
            "alpha=0.1). Supersedes V3-EXQ-541, which surfaced the within-"
            "cycle no-op pattern (target == rv at writeback by construction "
            "in legacy capture-only semantics). ARM_0 disables the consumer "
            "(use_rem_precision_recalibration=False); ARM_1 enables it with "
            "step=0.1 on top of the F1 substrate fix. Hazard-heavy / "
            "resource-thin env drives sustained E3 _running_variance drift "
            "between sleep cycles; the cross-cycle EMA reference now lags "
            "the moment-snapshot, so cycles 2..N have target != rv whenever "
            "drift moves rv between captures. Acceptance: C1 substrate-"
            "readiness (recalibration fires every cycle in >=2/3 ON-arm "
            "seeds), C2 movement-direction consistency with target and "
            "non-zero mean_abs_delta on cycles 2..N (>= 1e-3), C3 cross-arm "
            "divergence on post-cycle rv (>= 5%). First cycle's no-op is "
            "still expected (cold-start: persistent = first capture = rv); "
            "meaningful movement measured on cycles 2..N."
        ),
    }
