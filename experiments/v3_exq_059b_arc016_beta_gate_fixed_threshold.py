"""
V3-EXQ-059b -- ARC-016 / MECH-057b / MECH-090: Beta Gate with Fixed Commit Threshold
...
NOTE (2026-03-21): This experiment was queued as V3-EXQ-059b but the script had
  EXPERIMENT_TYPE="v3_exq_060_arc016_beta_gate_fixed_threshold" hardcoded, causing
  output to be written to the EXQ-060 directory. V3-EXQ-060 subsequently ran correctly
  and PASSED (4/5 criteria). This corrected version has the proper EXPERIMENT_TYPE.
  Do not queue -- EXQ-060 covers the science.
...
def _make_agent_and_env(seed: int):
    ...
    agent = REEAgent(cfg)
    print(
        f"[EXQ-059b] commit_threshold={cfg.e3.commitment_threshold}  "
...
if __name__ == "__main__":
    ...
    print(f"[EXQ-059b] Training {WARMUP_EPISODES} eps...")
    _train(agent, env, WARMUP_EPISODES)

    print("\n[EXQ-059b] Evaluating beta gate / ARC-016 commitment...")
    m = _eval(agent, env)
    ...
    print(f"\n[EXQ-059b] {status} ({n_pass}/5 criteria)")
    ...
    print(f"[EXQ-059b] Result written to {out_path.name}")
