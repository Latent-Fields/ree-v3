...
"""
V3-EXQ-905a -- MECH-075 DORSAL LEG: LC-Arousal Gain on Hippocampal Attractor
Dynamics (Kempadoo et al. 2016 PNAS) -- LC_GAIN RECALIBRATION of V3-EXQ-905

EXPERIMENT_PURPOSE = "evidence"

V3-EXQ-905a CHANGE (LC_GAIN re-derived against the observed signal distribution)
--------------------------------------------------------------------------------
V3-EXQ-905 (run_id v3_exq_905_mech075_dorsal_lc_arousal_probe_20260808T232406Z_v3,
FAIL) was the FIRST non-degenerate manipulation test of this leg: P0 (signal
magnitude) and C1 (manipulation check) both cleanly PASSED for the first time,
correcting the signal-magnitude defect of all three prior attempts (EXQ-192a/
209/230). But its load-bearing C2 (basin-width widening >= 15%) failed 0/3 with
near-zero, sign-inconsistent gaps (relative gap ~1e-6). The failure_autopsy_
mech075-vta-lc-cluster_2026-08-09 traced this to the substrate code
(ree_core/hippocampal/module.py _compute_mode_noise_scale): the CEM-noise
manipulation gain LC_GAIN=2.0 was INHERITED UNCHANGED from the already-FAILED
EXQ-192a's novelty_gain=2.0 and was never re-derived against this substrate's
actual arousal_ema magnitude. At the observed eval-phase arousal_ema
(0.0022-0.0029), mode_scale = 1 + LC_GAIN * arousal_ema = 1.0044-1.0057 with
LC_GAIN=2.0 -- a 0.4-0.6% increase in CEM proposal std, an order of magnitude
too small to plausibly produce a 15%-scale basin-width change. So C2's FAIL was
an UNDERPOWERED manipulation (measurement/test-design defect), NOT a
falsification of the mechanism -- the autopsy reclassified 905 from the filed
`weakens` to `measurement_test_design_defect` / `non_contributory`.

FIX (this iteration): re-derive LC_GAIN from the observed 905 eval-phase
arousal_ema distribution to land mode_scale in a DECISIVE ~1.1-1.3 range (by
direct analogy with the sibling ventral script V3-EXQ-903, which derived
RPE_GAIN=50 from its own observed |delta_t| magnitude to hit a decisive
mode_scale range rather than inheriting a prior constant). Observed 905
eval-phase gated arousal_ema was [0.002196, 0.002726, 0.002851] (mean ~0.00259);
target mode_scale ~1.2 at the mean => LC_GAIN = 0.2 / 0.00259 ~= 77 -> rounded to
LC_GAIN = 80.0, giving mode_scale = 1 + 80*arousal_ema = 1.176-1.228 across the
observed range. NOTE arousal_ema is computed from novelty and is downstream-
INDEPENDENT of LC_GAIN (LC_GAIN is only applied in mode_scale = 1 + LC_GAIN*
arousal_ema, which does not feed back into arousal_ema), so this derivation
against the prior run's arousal_ema is valid. This iteration additionally
RECORDS the realized per-seed eval cem_noise_scale (= 1 + LC_GAIN*arousal_ema)
so the actual applied mode_scale is auditable, in case the stronger manipulation
shifts trajectories (and hence novelty/arousal_ema) via second-order feedback.
Nothing else changes: same LC channel, same terrain metric, same P0/C1/C2
criteria and thresholds; only the manipulation GAIN constant is re-derived.
`supersedes` = the V3-EXQ-905 run_id (its C2 test was underpowered, so its
result must not continue to weight governance).
...
# Metadata
# --------------------------------------------------------------------------
EXPERIMENT_TYPE = "v3_exq_905a_mech075_dorsal_lc_arousal_probe"
CLAIM_IDS = ["MECH-075"]
EXPERIMENT_PURPOSE = "evidence"
# Superseded predecessor (its C2 manipulation was underpowered at LC_GAIN=2.0 --
# see the module docstring "V3-EXQ-905a CHANGE" section and the 2026-08-09 autopsy).
SUPERSEDES_RUN_ID = "v3_exq_905_mech075_dorsal_lc_arousal_probe_20260808T232406Z_v3"
...
LC_GAIN       = 80.0  # CEM noise scaling: ao_std *= (1 + LC_GAIN * arousal_ema).
                      # RE-DERIVED for 905a from the observed V3-EXQ-905 eval-phase
                      # arousal_ema [0.002196, 0.002726, 0.002851] (mean ~0.00259)
                      # to land mode_scale = 1 + LC_GAIN*arousal_ema in a DECISIVE
                      # ~1.176-1.228 range (target ~1.1-1.3), vs the ~1.004-1.006
                      # (0.4-0.6% lift) LC_GAIN=2.0 gave -- the underpowered-
                      # manipulation defect the 2026-08-09 autopsy identified.
                      # 905 inherited 2.0 unchanged from the already-FAILED
                      # EXQ-192a; this is derived from the signal, like the sibling
                      # ventral script's RPE_GAIN=50. See docstring "V3-EXQ-905a
                      # CHANGE".
...
def _run_single(seed: int, condition: str, dry_run: bool) -> Dict[str, Any]:
    ...
    cem_scale_history: List[float] = []  # 905a: realized mode_scale = 1 + LC_GAIN*arousal_ema
    ...
    for ep in range(n_eval):
        ...
        for _ in range(steps):
            ...
            with torch.no_grad():
                ...
                _scale = lc_state.get_cem_noise_scale()
                hook_state["scale"] = _scale
                cem_scale_history.append(_scale)  # 905a: audit the realized mode_scale
    ...
    mean_cem_scale = sum(cem_scale_history) / max(1, len(cem_scale_history))
    ...
    print(
        ...
        f" cem_scale_mean={mean_cem_scale:.4f}"
    ...
    return {
        ...
        # 905a: realized mode_scale = 1 + LC_GAIN*arousal_ema, applied in the GATED
        # arm and (computed-but-not-applied) reported for the ABLATED arm too. This
        # is the audit trail that the re-derived LC_GAIN lands in the target
        # decisive ~1.1-1.3 band, and catches any second-order arousal_ema drift.
        "mean_cem_noise_scale": mean_cem_scale,
...
def run(dry_run: bool = False) -> Dict[str, Any]:
    ...
    for i, (g, a) in enumerate(zip(results_gated, results_ablated)):
        ...
        # 905a: realized applied mode_scale in the GATED arm -- confirms the
        # re-derived LC_GAIN lands in the decisive ~1.1-1.3 band (was ~1.005 at
        # LC_GAIN=2.0, the underpowered-manipulation defect this iteration fixes).
        metrics[f"seed{i}_gated_cem_noise_scale"] = float(g["mean_cem_noise_scale"])
    ...
    per_seed_rows = "\n".join(
        f"| {g['seed']} | {g['mean_arousal_ema']:.6f} | {g['mean_cem_noise_scale']:.4f}"
        f" | {g['mean_basin_width']:.6f}"
    ...
    summary_markdown = (
        f"# V3-EXQ-905a -- MECH-075 DORSAL LEG: LC-Arousal Attractor-Basin Probe\n\n"
        f"**Supersedes:** {SUPERSEDES_RUN_ID} (LC_GAIN recalibration -- 905's C2 was"
        f" underpowered at LC_GAIN=2.0; see 2026-08-09 autopsy)\n"
        ...
        f"**LC gain (905a re-derived):** {LC_GAIN}  **LC EMA alpha:** {LC_EMA_ALPHA}\n\n"
        f"## P0 Readiness Gate\n\n"
        ...
        f"## Per-Seed Results\n\n"
        f"| Seed | GATED arousal_ema | GATED cem_scale | GATED basin_width"
        f" | ABLATED basin_width | gap | C1 | C2 |\n"
        f"|---|---|---|---|---|---|---|---|\n{per_seed_rows}\n\n"
        f"## PASS Criteria\n\n"
