...
EXQ-100 FAIL (2026-03-27):
  C3 PASS: step-std z_harm_a=0.026 vs z_harm_s=0.369 -- EMA structural smoothing present.
  C1 FAIL: z_harm_a LOWER in HIGH_HAZARD (0.392) than LOW_HAZARD (0.425) -- inverted.
  C2 FAIL: autocorr_lag10 z_harm_a=-0.017 -- NEGATIVE autocorrelation from an EMA output.

The negative autocorrelation is anomalous. EMA outputs should be highly positively
autocorrelated by definition (each step = alpha*prev + (1-alpha)*current). Negative
autocorrelation implies the encoder output is oscillating or near-noise, NOT that the
EMA itself is failing. The inverted density response also doesn't match physics.

This experiment diagnoses the root cause by separating two components:

Phase 0 (no encoder, no training): Collect RAW harm_obs_a directly from environment
  across HIGH_HAZARD and LOW_HAZARD conditions. If harm_obs_a itself fails to respond
  to hazard density or has poor autocorrelation, the bug is in the environment EMA.
  If harm_obs_a is healthy, the bug is in AffectiveHarmEncoder.

Phase 1 (encoder training): Train AffectiveHarmEncoder with explicit supervision.
  Collect z_harm_a statistics and compare to raw harm_obs_a properties.

Diagnosis codes:
  RAW_EMA_FAIL:   harm_obs_a from env does not respond to hazard density or has
                  poor autocorrelation. Root cause is CausalGridWorldV2 EMA setup.
  ENCODER_FAIL:   harm_obs_a is healthy but z_harm_a encoder degenerates (poor
                  density response or low autocorrelation at encoder output).
  PASS:           Both raw EMA and encoder output meet all criteria.

PASS criteria:
  C1: raw_harm_obs_a_mean_high > raw_harm_obs_a_mean_low * 1.1  (raw EMA responds)
  C2: raw_autocorr_lag10 > 0.3                                   (raw EMA autocorrelated)
...
EXPERIMENT_TYPE = "v3_exq_100b_affective_harm_diagnostic"
...
def _collect_raw_stats(
    ...
    harm_obs_a_means: List[float] = []   # per-step mean of harm_obs_a
    harm_obs_s_means: List[float] = []   # per-step mean of harm_obs_s
    steps_collected = 0
    ...
    while steps_collected < collect_steps:
        ...
        # Record raw signal means as a scalar proxy
...
def _collect_encoder_stats(
    ...
    # Explicit supervision on harm_obs_a mean -> target = harm_obs_a.mean()
    opt_harm = optim.Adam(
    ...
    for ep in range(warmup_episodes):
        ...
        for _ in range(steps_per_episode):
            ...
            # Supervised: z_harm_s -> proximity label
            ...
            # Supervised: z_harm_a -> harm_obs_a mean (scalar proxy for accumulation level)
    ...
    # Eval: collect encoder output stats
    print(
...
def run(
    ...
    **kwargs,
    ...
    print("\n[V3-EXQ-100b] Phase 0: Raw harm_obs_a diagnostic (no encoder)...",
          flush=True)
    ...
    if not c1_raw or not c2_raw:
        print(
            "\n  DIAGNOSIS: RAW_EMA_FAIL -- root cause is in CausalGridWorldV2 EMA."
            " harm_obs_a does not have expected properties before any encoder.",
            flush=True,
        )

    # ---- Phase 1: Encoder training and eval ----
    print("\n[V3-EXQ-100b] Phase 1: Encoder training and eval...", flush=True)
    ...
    print(f"\n[V3-EXQ-100b] {status} ({n_met}/5)  Diagnosis: {diagnosis}", flush=True)

    failure_notes = []
    if not c1_raw:
        failure_notes.append(
            f"C1 FAIL: raw harm_obs_a NOT responding to density"
            f" (high={raw_high['mean_harm_a']:.4f} vs low={raw_low['mean_harm_a']:.4f})"
            " -- root cause: CausalGridWorldV2 EMA not generating density-sensitive signal")
    if not c2_raw:
        failure_notes.append(
            f"C2 FAIL: raw harm_obs_a autocorr={raw_high['autocorr_a']:.3f}"
            " (< 0.3) -- EMA not providing temporal persistence")
    if not c3_enc:
        failure_notes.append(
            f"C3 FAIL: z_harm_a encoder output not density-sensitive"
            f" (high={enc_high['mean_z_harm_a']:.4f} vs low={enc_low['mean_z_harm_a']:.4f})"
            " -- encoder degenerates despite healthy input")
    if not c4_enc:
        failure_notes.append(
            f"C4 FAIL: z_harm_a autocorr={enc_high['ac_z_harm_a']:.3f}"
            f" not > z_harm_s autocorr={enc_high['ac_z_harm_s']:.3f}"
            " -- encoder not preserving temporal stability")
    ...
    summary_markdown = f"""# V3-EXQ-100b -- SD-011 / ARC-016 Affective Harm Stream Diagnostic
...
"""
    ...
    return {
        ...
        "diagnosis":          diagnosis,
        "supersedes":         "v3_exq_100_z_harm_a_integration",
    }
