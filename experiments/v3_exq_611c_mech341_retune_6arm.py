...
"""
V3-EXQ-611c -- MECH-341 retune 6-arm validation (supersedes V3-EXQ-611b).

V3-EXQ-611b ran on DLAPTOP-4.local 2026-05-28T17:26:42Z -> 18:14:45Z and
the runner's local completion cache reports outcome=PASS completed=18/18
(C1=True, C2=False, C3=True, R2c=True). The result manifest is missing
from REE_assembly/evidence/experiments/, absent from the coordinator DB
results table, and never landed on origin/master. Sentinel
_runner_signals/V3-EXQ-611b.json is also missing. The signature matches
the heartbeat-autostash-silent-revert pattern documented in
REE_assembly/CLAUDE.md / MEMORY (DLAPTOP-4 in coordination_mode=shadow,
no active TASK_CLAIM covering evidence/ at the time of write).

V3-EXQ-611c is a bit-identical re-run on the same substrate to recover
the manifest so behavioral_diversity_isolation_plan.md GAP-B can advance
to the B_only / ablate_B / ALL_ON behavioural falsifier per its NEXT
step (2). 611c script body is identical to 611b except for run-id /
queue-id / supersedes string updates.
...
Supersedes: V3-EXQ-611b (manifest lost 2026-05-28; runner cache reports PASS).
...
EXPERIMENT_TYPE = "v3_exq_611c_mech341_retune_6arm"
QUEUE_ID = "V3-EXQ-611c"
