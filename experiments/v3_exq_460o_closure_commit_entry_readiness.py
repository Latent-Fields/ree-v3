"""
V3-EXQ-460m: CLAIM-FREE substrate-readiness diagnostic for the F-INDEPENDENT
closure-plane commit-ENTRY primitive (use_closure_commit_entry, ree-v3 main 84c1e7c).

WHY: the 2026-06-22 closure-exclusive de-commit eval arms the natural-commit latch-hold
ONLY via _closure_commit_active, which gated on e3._committed_trajectory is not None --
whose only non-None writer is the F-driven commit path (running_variance < commit
threshold). On the closure-exclusive eval substrate the F-commit never sustains, so the
trajectory is rarely non-None and the eval rarely armed: V3-EXQ-460k/460l showed
ncl_hold_closure_armed_total = 0 on every arm (the lever_did_not_arm signature). The new
use_closure_commit_entry flag adds a sticky F-INDEPENDENT latch e3._closure_committed_active
SET on a goal-active, rule-directed commitment, so the natural-commit latch-hold arms +
sustains a beta occupancy WITHOUT any F-driven commit (e3._committed_trajectory stays None).

THIS DIAGNOSTIC asks ONLY: on the closure-exclusive de-commit eval substrate, does
use_closure_commit_entry make the closure-coupled latch-hold ARM and SUSTAIN a multi-tick
beta occupancy that the OFF baseline (the 460k/460l ncl_hold_closure_armed_total=0
signature) does NOT? It is claim-free (claim_ids=[]) -- it certifies the substrate is
ready before any de-commit falsifier (a 460-lineage successor) is scored.

DESIGN doc: REE_assembly/docs/architecture/natural_commit_occupancy_release.md
  (closure-plane commit-ENTRY primitive amend section).
UNIT contract: ree-v3/tests/contracts/test_closure_commit_entry.py (C3 C-KEY: entry ON +
  committed never True -> latch arms + multi-tick committed_run_length; entry OFF -> arms
  exactly 0).

ARMS (one curriculum build per seed; the entry flag toggled at eval via clone -- the latch
+ hold carry no trainable parameters so the clone is exact, mirroring 460k/460l). Both arms
keep closure_exclusive_decommit_eval + use_closure_commit_beta_coupling +
use_natural_commit_latch_hold ON; the variable is the F-independent commit-ENTRY primitive:
  ARM_ENTRY_OFF -- use_closure_commit_entry=False. The 460k/460l baseline; the latch-hold
                   arms ONLY via the fragile F-commit, which does not sustain on this
                   substrate -> ncl_hold_closure_armed_total ~ 0 (must NOT arm).
  ARM_ENTRY_ON  -- use_closure_commit_entry=True. The fix: a goal-active, rule-directed
                   commitment SETs the F-independent latch -> the hold arms + sustains a
                   beta occupancy WITHOUT any F-driven commit (must arm + sustain).

The eval loop ticks the agent and reads the closure-armed counter (sum of per-episode
agent._ncl_hold_closure_armed_count), the longest consecutive beta-elevated run
(max_consecutive_beta_run), F-driven natural commits (n_f_commits, expected ~0 on this
substrate -- the occupancy is F-INDEPENDENT), the SD-034 closure fire count, and a
NON-VACUITY readout n_rule_directed_commit_ticks (goal active AND rule_state norm >= floor)
-- the SET predicate's precondition. If the predicate never has a rule-directed commitment
to set the latch on, the test is starved, not falsified -> substrate_not_ready_requeue.

PRE-REGISTERED GATE (a) PRIMARY (load-bearing): on ARM_ENTRY_ON, "armed_and_sustained" =
(ncl_hold_closure_armed_total > 0) AND (max_consecutive_beta_run >= SUSTAIN_MIN_TICKS) on
>= ARM_PASS_FRACTION (2/3) seeds. Secondary contrast: ARM_ENTRY_OFF reproduces the baseline
(ncl_hold_closure_armed_total == 0). Context: n_f_commits per arm/seed (expected ~0 --
confirms F-independence; if F happens to commit it does not invalidate the F-independent
arming, but the headline is arming works WITHOUT sustained F).

HARNESS-FIX RE-ISSUE (supersedes V3-EXQ-460m; failure_autopsy_V3-EXQ-460m-460n_2026-06-23,
...
EXPERIMENT_TYPE = "v3_exq_460o_closure_commit_entry_readiness"
QUEUE_ID = "V3-EXQ-460o"
CLAIM_IDS: List[str] = []  # claim-free substrate-readiness diagnostic
EXPERIMENT_PURPOSE = "diagnostic"

SEEDS = [42, 43, 44]
CONDITION_LABEL = "CURRICULUM_BUILT_CLOSURE_COMMIT_ENTRY_READINESS_ENTRY_OFF_ON"
...
def _run_seed(seed: int, dry_run: bool, total_eps: int,
    ...
    print(f"  [eval] arm_eval seed={seed} ep {done}/{total_eps}"
          ...
          f" rdir={arm_off['n_rule_directed_commit_ticks']} closures={arm_off['n_closures']}"
          f" | ON armed={arm_on['ncl_hold_closure_armed_total']}"
          f" run={arm_on['max_consecutive_beta_run']} f_commits={arm_on['n_f_commits']}"
          f" rdir={arm_on['n_rule_directed_commit_ticks']} rs_peak={arm_on['rule_state_norm_peak']:.3f}"
          f" closures={arm_on['n_closures']}"
          f" | on_armed_sustained={on_armed_and_sustained} off_no_arm={off_did_not_arm}"
          f" on_rdir_met={on_rule_directed_met}", flush=True)
    seed_pass = bool(guard_pass and on_rule_directed_met and on_armed_and_sustained)
    print(f"verdict: {'PASS' if seed_pass else 'FAIL'} seed={seed}"
          f" guard_pass={guard_pass} on_rule_directed_met={on_rule_directed_met}"
          f" on_armed_and_sustained={on_armed_and_sustained} off_did_not_arm={off_did_not_arm}"
...
def run_experiment(dry_run: bool = False,
    ...
    # Non-vacuity precondition: ARM_ENTRY_ON formed a goal-active rule-directed commitment
    # the latch could SET on (>= RULE_DIRECTED_MIN_TICKS) on >= 2/3 ON seeds.
    ...
    rdir_measured = (
        min(int(r["arms"][ARM_ON].get("n_rule_directed_commit_ticks", 0))
            for r in guard_passing)
        if guard_passing else 0
    )

    # PRIMARY gate (a): armed_and_sustained on ARM_ENTRY_ON on >= 2/3 seeds.
    armed_flags = [bool(r.get("on_armed_and_sustained", False)) for r in guard_passing]
    armed_frac = _frac(armed_flags)
    ...
    # Non-degeneracy of gate (a): the ON-vs-OFF contrast genuinely differs on the passing
    # seeds (OFF armed==0 while ON armed>0) -- the two arms are not the same random variable.
    contrast_seeds = [
    ...
    # Routing. Claim-free diagnostic: PASS = substrate ready (the ENTRY primitive arms +
    # sustains). A non-vacuity failure self-routes substrate_not_ready_requeue (NEVER a
    # *_ceiling / does_not_support label -- below-floor is starved, not falsified).
    ...
    elif armed_and_sustained_met:
        outcome = "PASS"
        label = "closure_commit_entry_arms_and_sustains"
        route_reason = "on_armed_and_sustained_2of3_with_off_baseline_contrast"
    else:
        # Precondition met (a goal-active rule-directed commitment formed) but the latch did
        # not arm/sustain on >= 2/3 -- a genuine readiness FAIL, NOT a starved test.
        outcome = "FAIL"
        label = "closure_commit_entry_did_not_arm"
        route_reason = "rule_directed_commit_formed_but_latch_did_not_arm_or_sustain"
    ...
    print(f"[{EXPERIMENT_TYPE}] contact={contact_non_vacuity_met}"
          ...
          f" | gate_a armed_and_sustained={armed_and_sustained_met} (frac={armed_frac:.3f})"
          f" off_did_not_arm_frac={off_frac:.3f} non_degenerate={gate_a_non_degenerate}"
    ...
    acceptance = {
        ...
        "rule_directed_min_across_on_seeds": rdir_measured,
        ...
        "per_seed_on_armed_and_sustained": [
            bool(r.get("on_armed_and_sustained", False)) for r in per_seed
        ],
    ...
    return {
        ...
        "interpretation": {
            ...
            "preconditions": [
                {
                    ...
                    "description": "ARM_ENTRY_ON forms a goal-active rule-directed "
                                   "commitment the latch can set on (goal_state.is_active() "
                                   "AND lateral_pfc.rule_state norm >= "
                                   "closure_commit_entry_rule_norm_floor on >= 2/3 ON seeds). "
                                   "Below floor -> the SET predicate is starved, not "
            ...
            "criteria_non_degenerate": {
                # The gate (a) ON-vs-OFF contrast is non-degenerate iff ARM_ENTRY_OFF
                # armed == 0 while ARM_ENTRY_ON armed > 0 on at least the passing seeds --
                # i.e. the two arms genuinely differ (not the same random variable).
                "gate_a_armed_sustained": gate_a_non_degenerate,
            },
            "criteria": [
                {
                    "name": "gate_a_armed_and_sustained_2of3",
                    "load_bearing": True,
                    "passed": armed_and_sustained_met,
                },
            ],
            "secondary_contrast": {
                "off_did_not_arm_fraction": off_frac,
                "note": "ARM_ENTRY_OFF should reproduce the 460k/460l baseline "
                        "(ncl_hold_closure_armed_total == 0). Reported as context, not the "
                        "gate -- the load-bearing PASS is gate (a) on ARM_ENTRY_ON.",
            },
            "f_independence_note": "n_f_commits per arm/seed expected ~0 (the occupancy is "
                                   ...
                                   "the closure-plane commit-ENTRY latch arms the hold). If F "
                                   "happens to commit occasionally it does NOT invalidate the "
                                   "F-independent arming; the headline is arming works WITHOUT "
                                   "sustained F. Reported as context, not a hard gate.",
            "primitive_under_test": {
                "flag": "use_closure_commit_entry (ree-v3 main 84c1e7c)",
                "latch": "e3._closure_committed_active (sticky, F-INDEPENDENT)",
                "diagnostic_counter": "agent._ncl_hold_closure_armed_count",
                ...
                "note": "closure_exclusive_decommit_eval + use_closure_commit_beta_coupling "
                        "+ use_natural_commit_latch_hold ON in BOTH arms; the only variable "
                        "is use_closure_commit_entry. The JOB-2 rho ramp / habenula + the "
                        "rung-6 NaturalCommitUrgencyRelease are OFF in both arms.",
            },
        },
        "per_seed": per_seed,
    }
...
def main(dry_run: bool = False,
    ...
    manifest = {
        ...
        "queue_id": QUEUE_ID,
        "supersedes": "V3-EXQ-460m",  # harness-fix re-issue (z_goal seeded in eval loop)
        "claim_ids": CLAIM_IDS,
        ...
        "substrate": "scaffolded_sd054_onboarding (full curriculum: Stage-0 -> Stage-0b -> "
                     ...
                     "mode (closure_exclusive_decommit_eval, ree-v3 e52158d) ARMED in BOTH "
                     "arms. The F-independent closure-plane commit-ENTRY primitive "
                     "(use_closure_commit_entry, ree-v3 84c1e7c) toggled per arm "
                     "(ENTRY_OFF/ENTRY_ON). rung-6 NaturalCommitUrgencyRelease + JOB-2 DRIVER "
                     "pair OFF in both arms.",
        "condition": CONDITION_LABEL,
        "method_note": "CLAIM-FREE substrate-readiness diagnostic for the F-independent "
                       "closure-plane commit-ENTRY primitive (use_closure_commit_entry). On "
                       "the closure-exclusive de-commit eval substrate, does the ENTRY "
                       "primitive make the closure-coupled latch-hold ARM + SUSTAIN a "
                       "multi-tick beta occupancy that the OFF baseline (the 460k/460l "
                       "ncl_hold_closure_armed_total=0 signature) does NOT? Two eval arms on "
                       "one trained substrate per seed (the ENTRY flag toggled at eval via "
                       "clone): ARM_ENTRY_OFF (the 460k/460l baseline; must NOT arm) / "
                       "ARM_ENTRY_ON (the fix; a goal-active rule-directed commitment SETs "
                       "the F-independent latch -> arms + sustains WITHOUT any F-driven "
                       "commit). PRIMARY load-bearing gate (a): armed_and_sustained "
                       "(ncl_hold_closure_armed_total > 0 AND max_consecutive_beta_run >= "
                       "SUSTAIN_MIN_TICKS) on >= 2/3 ON seeds. Non-vacuity precondition: a "
                       "goal-active rule-directed commitment formed (else "
                       "substrate_not_ready_requeue, NEVER a falsification). claim_ids=[] -- "
                       "certifies the substrate is ready before any de-commit falsifier (a "
                       "460-lineage successor) is scored.",
        "arm_note": "ARMS = " + ", ".join(a["key"] for a in ARMS) + ". closure-exclusive eval "
                    "+ beta-engagement coupling + natural-commit latch-hold ON in both arms; "
                    "the only variable is use_closure_commit_entry. ARM_ENTRY_OFF is the "
                    "460k/460l no-arm baseline; ARM_ENTRY_ON adds the F-independent "
                    "commit-ENTRY primitive (must arm + sustain).",
        ...
        "scaffold_curriculum": {
            ...
            "config_basis": "closure-exclusive de-commit eval (ree-v3 main e52158d, "
                            "2026-06-22) + F-independent closure-plane commit-ENTRY primitive "
                            "(use_closure_commit_entry, ree-v3 main 84c1e7c, 2026-06-23)",
        },
