...
"""V3-EXQ-821b -- MECH-457 GOV-FANOUT-1 RETENTION leg 4, H-consummation-binding: CALIBRATED-INSTALL
re-run of V3-EXQ-821 (supersedes V3-EXQ-821a, an ERROR/crash-before-manifest).

WHY THIS SUPERSEDES 821a (diagnosed here, 2026-08-08, via /diagnose-errors). 821a crashed at setup
with `experiments._lib.readiness_anchor.AnchorUnreachable`, exit code 1, no manifest written
(ree-cloud-3, 2026-08-08T11:17:11Z). The traceback (confirmed via the worker's systemd journal --
no manifest/heartbeat carries it) shows `assert_anchor_reachable` refusing to run even though its
OWN reported reference score was 2/2 = 1.0000 ("reachable" by inspection) against a stated gate of
">= 10.0000". That gate value is not a legal fraction (assert_anchor_reachable's `threshold` param
is the FRACTION of reference cells the SHIPPED predicate must score True, 0.0-1.0 -- see
experiments/_lib/readiness_anchor.py's docstring and the sibling MECH-457 scripts 788/789/792/792a,
which all declare a dedicated `POST_BC_ANCHOR_REACHABILITY_THRESHOLD = 1.0`). 821a's call instead
passed `threshold=float(POST_BC_INSTALL_FLOOR)` (= 10.0, the RAW competence floor `score_fn`
already applies internally per-cell), reusing that constant for two different semantic roles. In
821 (the original), `POST_BC_INSTALL_FLOOR` was `baselines.POST_BC_INSTALL_FLOOR` = 1.0, which
happens to double as a legal "require every reference cell" fraction, so the conflation was inert
there. 821a legitimately RAISED that floor to USABLE_INSTALL_FLOOR=10.0 for the install-took gate
(the whole point of 821a's fix) but did not split off a separate reachability-fraction constant, so
the guard became a required score of 10.0 against a maximum-possible reference_score of 1.0 --
structurally unmeetable regardless of how well the anchors perform, and it crashed on the very
first call, before any boot-arm compute. THE FIX (this script, 821b): a dedicated
`POST_BC_ANCHOR_REACHABILITY_THRESHOLD = 1.0` constant, matching the sibling scripts' convention,
passed to `assert_anchor_reachable` in place of `POST_BC_INSTALL_FLOOR`. Nothing else changes --
same install dose (1200), same USABLE_INSTALL_FLOOR (10.0) for the install-took gate itself, same
arms, same seeds, same routing logic. The live anchors captured in 821a's own traceback
(local_view_greedy=34.167, greedy_oracle=42.800, both >> the 10.0 floor) confirm the corrected
guard passes trivially once the units match.

WHY 821a SUPERSEDED 821. V3-EXQ-821 (2026-07-25) came back FAIL / consummation_binding_eroded_
...
repose_2026-08-08.md re-posed it as a CALIBRATED single re-run (821a, then this crash-fix 821b).
...
EXPERIMENT_TYPE = "v3_exq_821b_mech457_consummation_binding"
QUEUE_ID = "V3-EXQ-821b"
SUPERSEDES = "V3-EXQ-821a"
...
# THE 821b FIX. assert_anchor_reachable's `threshold` is a FRACTION of reference cells the shipped
# predicate must score True (0.0-1.0) -- NOT the raw competence floor `score_fn` already applies
# per cell. 821a passed POST_BC_INSTALL_FLOOR (10.0) here, an illegal fraction that no reference
# score (max 1.0) can ever clear, so the guard crashed unconditionally at setup regardless of anchor
# quality. Matches the sibling MECH-457 scripts' convention (788/789/792/792a):
# "every reference cell must clear the install floor".
POST_BC_ANCHOR_REACHABILITY_THRESHOLD = 1.0
...
def run_experiment(seeds: List[int], on_budget: int, eval_eps: int, steps: int,
    ...
    print(
        f"MECH-457 GOV-FANOUT-1 H-consummation-binding (821b CALIBRATED re-run, anchor-reachability "
        f"fraction-threshold fix) "
...
def _build_manifest(result: Dict[str, Any], timestamp_utc: str, dry_run: bool,
    ...
    return {
        ...
        "notes": (
            ...
            "superseding V3-EXQ-821a (crash-before-manifest, /diagnose-errors) which itself "
            "superseded V3-EXQ-821. Pre-registered under question 'competence_floor' in "
            ...
            "/failure-autopsy. 821 FIX (carried unchanged from 821a): (1) BC install dose "
            "300->1200 (calibrated 2026-08-08: 300->post_bc 3.1, 1200->25.05, 2400->21.05 "
            "saturated; the rare CONSUME action is under-cloned at low dose in the 6-action "
            "consummatory env, an install class-imbalance defect NOT a new mechanism); (2) USABLE "
            "install readiness floor 10.0 not the trivial 1.0 (821 accepted a 4.3 install and "
            "produced a degenerate two-arms-at-zero comparison); (3) foraging RATE (resources/tick) "
            "reported alongside the composite resources/episode, survival_horizon as a covariate "
            "(instrument audit 2026-08-07 sections 2/7; probe/install full-8-metric fix ree-v3 "
            "0550a2f). 821b FIX (this script): (4) assert_anchor_reachable's `threshold` is a "
            "FRACTION of reference cells (0.0-1.0), not the raw 10.0 install floor -- 821a passed "
            "the raw floor and crashed unconditionally at setup (AnchorUnreachable, exit 1, no "
            "manifest) despite both live anchors clearing the floor with margin; fixed via a "
            "dedicated POST_BC_ANCHOR_REACHABILITY_THRESHOLD=1.0 constant, matching sibling "
            "MECH-457 scripts 788/789/792/792a. MANIPULATION = the "
...
def main() -> Tuple[Optional[str], Optional[str], bool]:
    parser = argparse.ArgumentParser(
        description="V3-EXQ-821b MECH-457 GOV-FANOUT-1 H-consummation-binding (calibrated re-run)"
    )
