...
"""
V3-EXQ-733a-b -- Leg P-B of the MECH-456 functional-rebinding redesign portfolio.
ENVIRONMENT / TEST-BED AXIS: generate ground-truth region-boundary crossings
INDEPENDENT of foraging survival, so the pre-registered overtake quorum is met by
CONSTRUCTION and the DV1/DV2 readouts are isolated from the navigation-competence
confound that starved V3-EXQ-733.

supersedes: V3-EXQ-733 (same scientific question -> letter suffix; GOV-FANOUT-1
2-leg portfolio with the sibling P-A survival-onboarded leg
v3_exq_733a_rebinding_pA_survival_onboarded.py).

WHY 733 FAILED (not a rebinding verdict). Only seed 42 reached the 20-overtake
floor (min 2 vs 20): a COLD agent dies in ~10-40 steps in the lethal SD-054 env, so
within-episode region overtakes (g(t) != g(t-1)) are starved. The binder itself is
fine (DV1 6/6, DV2 5/6). The overtake generator was entangled with foraging survival.

THE P-B FIX (this leg). Keep the SAME cold V3-EXQ-733 agent (no onboarding -- this
isolates the test-bed axis), but change the TEST-BED so overtakes are guaranteed:
  (1) SOFTENED SURVIVAL PRESSURE -- hazard_harm 0.05 -> 0.0, proximity_harm_scale
      ...
      z_world structure) but non-lethal, so episodes are not truncated by death.
  ...
  (3) DIRECTED TRAVERSAL -- a scripted region tour: the agent is directed-respawned
      ...
      region visit, so the readiness quorum (overtakes + coverage) is met REGARDLESS
      of the policy's foraging competence. Between teleports the agent still senses/
      acts naturally, so z_self/z_world are the agent's genuine latents; the teleport
      only sets WHICH region is currently true (the competing-configuration overtake).

NULL (declared): with overtakes GUARANTEED, if DV1/DV2 still fail to clear on
>= MIN_SEEDS_FOR_PASS of 6 seeds -> the binder does NOT generally track the true
competitor / confers no graded advantage; the seed-42 signal of 733 was not general
(WEAKENS MECH-456). If the binder does not converge / coverage is somehow short ->
...
RUN PASS = DV1 AND DV2 on >= MIN_SEEDS_FOR_PASS seeds with readiness met. MECH-456
stays candidate / v3_pending regardless (V3-pending gate).
...
with 733 via experiments/_lib/rebinding_functional_harness.py.
...
EXPERIMENT_TYPE = "v3_exq_733a_rebinding_pB_directed_traversal"
QUEUE_ID = "V3-EXQ-733a-b"
CLAIM_IDS: List[str] = ["MECH-456"]
EXPERIMENT_PURPOSE = "evidence"
SUPERSEDES = "V3-EXQ-733"

# Converged learned cross_stream_binder activation (identical to V3-EXQ-733).
    ...
    for s in seeds:
        ...
        if first:
            print(
                ...
                f"latency_margin={H.LATENCY_MARGIN}, dry_run={dry_run})",
                flush=True,
            )
            first = False
...
def _build_manifest(result: Dict[str, Any], timestamp_utc: str, dry_run: bool) -> Dict[str, Any]:
    ...
    review_caveats = [
        "Leg P-B (environment / test-bed axis) of the V3-EXQ-733a portfolio. The "
        "agent is the COLD V3-EXQ-733 agent VERBATIM (no onboarding); ONLY the "
        "test-bed changes: softened survival (hazard_harm/proximity_harm/hazard_food_"
        ...
        "steps -- so overtakes + region coverage are met BY CONSTRUCTION, decoupled "
        "from foraging survival. Between teleports the agent senses/acts naturally, "
        "so z_self/z_world are its genuine latents; the teleport only sets which "
        "region is currently the true competitor.",
        ...
        "state and does NOT trigger contact harm/consumption (movement-driven in the "
        "env), so it generates a clean ground-truth overtake without foraging.",
        ...
        "NULL declared: with overtakes GUARANTEED, DV1/DV2 failing on >= "
        f"{H.MIN_SEEDS_FOR_PASS} of 6 seeds -> the binder does not GENERALLY track "
        "the true competitor / confers no graded advantage (the 733 seed-42 signal "
        "was not general) => WEAKENS MECH-456. rebinding_inert_off_equals_on (DV1 "
        "holds, DV2 fails) and rebinding_not_tracking_truth (DV1 fails) both WEAKEN; "
    ...
    if not interp.get("all_ready", False):
        review_caveats.insert(
            ...
            f"{H.MIN_REGION_VISITS_P0}). For P-B this should NOT happen (traversal "
            "guarantees both) -- investigate the binder-convergence leg. Self-routes "
            "substrate_not_ready_requeue (non_contributory); NOT a rebinding verdict.",
        )
