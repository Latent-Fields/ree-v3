...
"""
V3-EXQ-328a: MECH-112 z_goal Structured Latent Discriminative Pair
Supersedes V3-EXQ-328.

experiment_purpose: evidence

BUG FIX from EXQ-328 (z_goal_norm=0.0 despite resource contacts):
  Root cause: The goal seeding trigger used benefit_raw=obs_body[11] (the
  benefit_exposure EMA from the observation BEFORE the env step). At the
  moment of a resource contact, obs_body[11] reflects the EMA state BEFORE
  the contact occurs -- the env updates benefit_exposure only AFTER env.step().
  With nociception_ema_alpha=0.1 and resource_benefit=0.5, the post-step EMA
  update gives benefit_exposure_new = 0.9*old + 0.1*0.5 = 0.05 (from zero).
  effective_benefit = 0.05 * (1 + drive_weight * drive_level) barely reaches
  the benefit_threshold=0.1 requirement only when drive_level is near 1.0.
  In practice, obs_body[11] before the contact is near 0, so seeding never
  fires in most seeds.

  Secondary issue: seed 13 showed constant z_goal_norm across all time offsets
  (t=0, 10, 25, 50) and in both DRIVE_ACTIVE and DRIVE_ABLATED conditions.
  This occurred because (a) the pre-step benefit_exposure occasionally exceeded
  threshold from proximity approach accumulation, and (b) multiple closely-spaced
  contacts reset post_contact_buffer before the t=50 window completes, resulting
  in only late-contact trajectories being recorded (which have nearly identical
  values since z_goal decays slowly at decay_goal=0.005).

  Fix: Use the raw harm_signal from env.step() as the benefit input to
  update_z_goal() on resource contact steps. harm_signal at resource contact
  is resource_benefit + proximity_benefit = ~0.5+, which reliably exceeds the
  threshold. The obs_body-based EMA (old approach) is reserved for the
  exploratory proximity gradient update only.

  Additionally: the post_contact_buffer reset on new contact is preserved, but
  we now only attempt to record a trajectory when there are at least MIN_WINDOW
  steps left in the episode (so t=50 can be measured).
...
# Experiment metadata
# ---------------------------------------------------------------------------
EXPERIMENT_TYPE    = "v3_exq_328a_mech112_zgoal_structured_latent"
...
def main():
    ...
    run_id = (
        f"v3_exq_328a_mech112_zgoal_structured_latent_dry_{ts}_v3"
        if args.dry_run
        else f"v3_exq_328a_mech112_zgoal_structured_latent_{ts}_v3"
    )
    print(f"EXQ-328a start: {run_id}")
