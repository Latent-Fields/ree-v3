...
"""
V3-EXQ-397c -- ARC-007 Path Memory Ablation: Harder Environment + Terrain Fix

Claims: ARC-007, SD-004

Supersedes: V3-EXQ-397 (FAIL: inverted quality_gap -- terrain_prior trained on
E3-selected trajectories, which were high-residue because hazard-proximate resources
attracted E3 selection, teaching hippocampus to navigate TOWARD harm).

Two fixes vs EXQ-397:

1. TERRAIN TRAINING FIX (critical): terrain_prior is now trained on the MINIMUM-residue
   candidate from the candidate set, NOT on E3's selected trajectory.
   Root cause of EXQ-397 inversion: E3 selects trajectories through hazard-proximate
   resources (high reward but high residue). Training terrain_prior on this trajectory
   taught the hippocampus to navigate TOWARD residue, inverting the quality_gap sign.
   Fix: at each E3 tick, score all candidates by mean residue over the trajectory,
   pick the minimum, and train terrain_prior on that candidate's action objects.

2. HARDER ENVIRONMENT: size=16, num_hazards=6, num_resources=3 (sparse).
   EXQ-397 environment was too simple -- only 12x12 grid with plentiful resources.
   Sparse resources force longer navigation paths, making hippocampal terrain guidance
   essential. More hazards increase residue contrast between safe and unsafe routes.

Additional: 800 training episodes (was 600) to allow more terrain learning in the
harder environment.

Same 5-criterion PASS criteria as EXQ-397 (C1-C5).
Same ablation protocol: train -> eval intact -> ablate residue field -> eval ablated ->
restore -> eval restored.
...
PASS criteria (ALL must hold):
  ...
  C3: hippo_quality_gap_restored >= hippo_quality_gap_ablated
                                             (restoration does not make things worse)
...
EXPERIMENT_TYPE = "v3_exq_397c_arc007_harder_env"
CLAIM_IDS = ["ARC-007", "SD-004"]
EXPERIMENT_PURPOSE = "evidence"
    ...
    for ep in range(num_episodes):
        ...
        for _ in range(steps_per_episode):
            ...
            if ticks.get("e3_tick", False) and candidates:
                ...
                # KEY FIX vs EXQ-397: train terrain_prior on MINIMUM-residue candidate,
                # not on E3's selected trajectory. E3 selects toward high-reward areas
                # that are often hazard-proximate (high residue), which would teach the
                # hippocampus to navigate toward harm rather than away from it.
                best_ao = None
                if candidates:
                    residue_scores = []
                    for c in candidates:
...
def run(
    ...
    **kwargs,
    ...
    print(
        f"[V3-EXQ-397c] ARC-007 Path Memory Ablation -- Harder Env + Terrain Fix\n"
        ...
        f"  terrain fix: minimum-residue candidate (not E3-selected)",
        flush=True,
    )
    ...
    print("\n[V3-EXQ-397c] Eval A (intact residue field)...", flush=True)
    eval_a = _eval_hippo_quality(agent, env, eval_episodes, steps_per_episode, world_dim, "intact")

    print("\n[V3-EXQ-397c] Ablating residue field...", flush=True)
    orig_eval, orig_eval_traj = _ablate_residue_field(agent)

    print("[V3-EXQ-397c] Eval B (ablated residue field)...", flush=True)
    ...
    print("[V3-EXQ-397c] Residue field restored.", flush=True)

    print("[V3-EXQ-397c] Eval C (restored residue field)...", flush=True)
    ...
    print(f"\nV3-EXQ-397c verdict: {status}  ({criteria_met}/5)", flush=True)
    ...
    summary_markdown = f"""# V3-EXQ-397c -- ARC-007 Path Memory Ablation: Harder Env + Terrain Fix
...
if __name__ == "__main__":
    ...
    if args.dry_run:
        ...
        global_args_warmup = args.warmup
        global_args_eval = args.eval_eps
        global_args_restore = args.eval_restore
        global_args_steps = args.steps
        args.warmup = 3
        args.eval_eps = 2
        args.eval_restore = 2
        args.steps = 10
