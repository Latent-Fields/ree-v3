...
"""
V3-EXQ-258a -- MECH-205: Surprise-Gated Replay Validation (Write Path Fix)

Supersedes: V3-EXQ-258
...
=== CHANGES FROM EXQ-258 ===

EXQ-258 FAIL was caused by two bugs:
  1. P1 check used _rbf_layer (nonexistent) instead of rbf_field
  2. pe_ema_alpha=0.1 tracked PE so fast surprise stayed near zero

Fixes:
  - rbf_field attribute access corrected
  - pe_ema_alpha=0.02 (configurable, ~50-step EMA window)
  - pe_surprise_threshold=0.001 gate before residue write
  - _surprise_write_count diagnostic counter added
...
# Experiment metadata
# ---------------------------------------------------------------------------
EXPERIMENT_TYPE    = "v3_exq_258a_mech205_surprise_gated_replay"
CLAIM_IDS          = ["MECH-205"]
EXPERIMENT_PURPOSE = "evidence"
...
def _run_seed(
    ...
    for label, surprise_gated in CONDITIONS:
        print(
            f"\n[V3-EXQ-258a] Seed {seed} Condition {label}"
...
def main() -> None:
    ...
    print(
        f"[V3-EXQ-258a] MECH-205: Surprise-Gated Replay Validation"
    ...
    all_results = []
    for seed in seeds:
        print(f"\n[V3-EXQ-258a] === Seed {seed} ===", flush=True)
    ...
    print(f"\n[V3-EXQ-258a] === Results ===", flush=True)
    ...
    manifest = {
        ...
        "supersedes":         "v3_exq_258_mech205_surprise_gated_replay",
        "outcome":            outcome,
    ...
    print(f"\n[V3-EXQ-258a] Output written: {out_path}", flush=True)
