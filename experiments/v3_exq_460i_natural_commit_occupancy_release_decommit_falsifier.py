"""
V3-EXQ-460i (460h-successor; NOT a supersede): SD-034 closure de-commit falsifier on
the rung-6 COMMIT/RELEASE-DURATION lever -- the graded natural-commit-occupancy release
(ree_core/policy/natural_commit_urgency.py, NaturalCommitUrgencyRelease, landed ree-v3
main ab2c1a9 2026-06-20).
...
self-routes substrate_not_ready_requeue -- NEVER a false weakens):
  ...
  (3) LEVER ACTUALLY SHORTENED OCCUPANCY (the rung-6 non-vacuity gate): on ARM_GAP_SCALED
      ...
      guard seeds. If the lever did not shorten occupancy there is nothing to test ->
      substrate_not_ready_requeue.
  (4) closure-coupling non-vacuity (MECH-445, refractory-INDEPENDENT): on ARM_GAP_SCALED
  ...
  (5) closure-trigger available: ARM_GAP_SCALED n_closures > 0 on >= 2/3 guard seeds.
  (6) within-arm window non-vacuity: ARM_GAP_SCALED produced >= C2_MIN_WINDOW_EVENTS
      scored around-closure windows with mean_pre_occ > WITHIN_PRE_OCC_FLOOR on >= 2/3
      guard seeds (something committed to de-commit).

PRE-REGISTERED ACCEPTANCE (constants; scored only once all six readiness gates clear):
...
EXPERIMENT_TYPE = "v3_exq_460i_natural_commit_occupancy_release_decommit_falsifier"
QUEUE_ID = "V3-EXQ-460i"
...
def _run_seed(seed: int, dry_run: bool, total_eps: int,
    ...
    print(f"verdict: {'PASS' if (guard_pass and seed_pass) else 'FAIL'} seed={seed}"
          f" guard_pass={guard_pass} lever_shortened={lever_shortened}"
...
def run_experiment(dry_run: bool = False,
    ...
    # Readiness gate (3): the rung-6 lever actually shortened occupancy.
    ...
    readiness_all_met = bool(
        contact_non_vacuity_met and rule_bias_trained_met and lever_shortened_met
        and coupling_nonvacuity_met and closure_trigger_available_met and within_window_met
    )
...
def main(dry_run: bool = False,
    ...
    manifest = {
        ...
        "substrate": "scaffolded_sd054_onboarding (full curriculum: Stage-0 -> Stage-0b -> "
                     ...
                     "(use_natural_commit_urgency_release, ree-v3 main ab2c1a9) toggled per arm.",
