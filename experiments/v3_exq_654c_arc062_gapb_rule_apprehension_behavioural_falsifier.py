...
"""
V3-EXQ-654c -- arc_062_rule_apprehension:GAP-B behavioural falsifier for
MECH-309 / ARC-062, on the NOW-VALIDATED crf-availability-maintenance substrate.

SUPERSEDES V3-EXQ-654b (FAIL non_contributory, substrate_not_ready_requeue,
2026-06-10T20:05Z): even with crf_persist_rules_across_episode_reset=True and a
longer (P0+P1 = 150+90 = 240 ep) maturation window, the persisted
CandidateRuleField did NOT clear the C1c floor -- crf_frac_active = 0.130 < 0.30,
0/3 ON seeds matured -- so the C2 committed-class falsifier DV never ran
(correctly non_contributory; NOT a falsification). The 654/654a/654b lineage all
failed for the SAME reason: the field minted distinct rules (cumulative mints
131-1014) but the pool never HELD >=2 differentiated rules active simultaneously
-- mint -> brief life -> retire -> re-mint churn. See
REE_assembly/evidence/planning/failure_autopsy_V3-EXQ-654b_2026-06-11.{md,json}.

WHY THIS RE-QUEUE CAN NOW SCORE: the crf-availability-maintenance mechanism
(ARC-063 amend, activity-silent maintenance trace + maintained-pool readout,
landed 2026-06-11) was VALIDATED by V3-EXQ-666c (PASS 2026-06-15;
crf-availability-maintenance substrate_queue ready=True). 666c showed ARM_2
(mature + e2ctx + maintenance) HOLDS a differentiated reactivatable pool through
context-absent ticks: mean_frac_maintained 0.854 > 0.625 floor, all 4 readiness
preconditions met (field mints >=2 rules/cell; e2ctx full-pool differentiation
1.598 > 0.1 PRE-gap on all 3 seeds; ARM_2 maintained pool count 9 +
differentiation 1.701 supra-floor). So the matured, maintained, differentiated
pool the C1c floor needs is now reproducibly populated.

DESIGN CHANGES vs V3-EXQ-654b (port onto the validated substrate; everything
else matched):
  1. The 666c-validated maintenance levers are ARMED on ARM_ON:
     crf_mature_pool_dynamics=True (deadlock-free conflict gate + slow decay +
     mint-youth protection), crf_context_from_e2_world_forward=True (the
     differentiation source -- action-regime-separated CRF context), and
     crf_availability_maintenance=True (crf_maintenance_floor=0.45 / decay=0.0 --
     the activity-silent trace that HOLDS a differentiated rule's availability
     across sparse-matching ticks). These are exactly the 666c ARM_2 flags. The
     654b run had ONLY crf_persist (cross-episode) -- no within-run maintenance --
     which is why its pool churned below the C1c floor.
  2. ENLARGED P0 maturation window 150 -> 200 ep (matching the 666c maturation
     budget that formed full-pool differentiation PRE-gap on all 3 seeds), so the
     maintained pool reaches a live differentiated >= 0.30 active fraction BEFORE
     the frozen P2 behavioural measurement.

INHERITED from 654b (unchanged):
...
V3-EXQ-571; the authority is gap-relative ADDITIVE at gain 0.5, subdominant to
the F-dominated primary). So if 654c's MATURED + DIFFERENTIATED + MAINTAINED rule
pool clears the C1c floor AND its bias reaches committed action (C1d) but STILL
fails to lift committed-class entropy (C2), that is the SAME SHARED CONVERSION
CEILING -- NOT a MECH-309 / ARC-062 falsification. The 654b "weakens" branch is
therefore REPLACED here by a conversion-ceiling off-ramp that self-routes
...
supersedes V3-EXQ-654b.

See REE_assembly/evidence/planning/arc_062_rule_apprehension_plan.md (GAP-B),
REE_assembly/evidence/planning/failure_autopsy_V3-EXQ-654b_2026-06-11.json,
...
EXPERIMENT_TYPE = "v3_exq_654c_arc062_gapb_rule_apprehension_behavioural_falsifier"
QUEUE_ID = "V3-EXQ-654c"
SUPERSEDES = "V3-EXQ-654b"
...
def _build_manifest(
    ...
    return {
        ...
        "evidence_direction_note": (
            f"V3-EXQ-654c arc_062 GAP-B behavioural falsifier (MECH-309 / ARC-062), "
            f"supersedes V3-EXQ-654b, on the NOW-VALIDATED crf-availability-maintenance "
            f"substrate (V3-EXQ-666c PASS 2026-06-15; substrate_queue ready=True). "
            f"Single-variable: ARM_OFF (use_candidate_rule_field=False, legacy collapsed "
            f"rule_state) vs ARM_ON (use_candidate_rule_field=True + crf_persist + the "
            f"666c ARM_2 levers crf_mature_pool_dynamics + crf_context_from_e2_world_forward "
...
def main() -> Tuple[Optional[str], Optional[str]]:
    parser = argparse.ArgumentParser(description="V3-EXQ-654c GAP-B falsifier (on the 666c-validated crf-availability-maintenance substrate)")
