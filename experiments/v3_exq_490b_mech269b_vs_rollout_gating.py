...
"""
V3-EXQ-490b -- MECH-269b VsRolloutGate substrate-readiness probe (Q-040a).

supersedes: V3-EXQ-490a

Claims: Q-040 (precondition arm: Q-040a -- does the gate fire at all under
the V3-EXQ-490 substrate?). Does NOT test MECH-269b's behavioural-sufficiency
hypothesis (Q-040b) -- at smoke thresholds, even healthy streams with mild
velocity get held. MECH-269b was deliberately removed from claim_ids in
this iteration per /diagnose-errors 2026-04-27 claim_ids accuracy review.

Purpose (diagnostic / substrate-readiness)
------------------------------------------
V3-EXQ-490 + V3-EXQ-490a both FAILed c1 (vs_gate_total_held_e1 +
vs_gate_total_held_e2 = 0) across both arms in all 3 seeds. Sub-diagnostic
outcome: c2 / c3 cannot be interpreted because the gate never fired.

Root cause (/diagnose-errors 2026-04-27): the gate's hold trigger
(V_s[stream] < per-side threshold, default 0.4) is unreachable under
typical Phase 1 V_s dynamics. Phase 1 V_s is computed as an identity-
prediction proxy:

    err = ||z_curr - z_prev|| / (||z_curr|| + 1e-6)
    score = clip_[0,1](1 - err)
    V_s[s] = (1-tau)*V_s_prev[s] + tau*score   # tau=0.1, seeded at 1.0

This is a measure of step-to-step latent CHANGE, not stream-to-reality
alignment. For typical agent dynamics post-warmup (latents drifting
slowly), err is small (~0.1), score ~0.9, EMA stays near 0.9. To drop
V_s below 0.4 requires ~9 consecutive ticks of fully misaligned latent
(score=0). That doesn't happen in smoke-scale runs. The gate has nothing
to fire on.

V3-EXQ-490b smoke fix: raise vs_gate_e1_threshold + vs_gate_e2_threshold
to 0.85 and vs_gate_snapshot_refresh_threshold to 0.95 so the gate fires
under typical Phase 1 V_s dynamics. This makes the substrate observable:
we confirm the gate is wired, snapshots refresh, hold substitution works.
What this CANNOT confirm is whether MECH-269b's hypothesis -- that
holding stale streams produces behavioural revision -- is correct. At
threshold 0.85 even healthy streams with mild velocity get held; the
discrimination between "stale" and "healthy" is lost.

Q-040 narrowing (governance 2026-04-27):
    Q-040a (this experiment): does the gate fire at all under the
        V3-EXQ-490 substrate? PASS = c1_on_total_held > 0 in >= 2/3
        seeds in the ON_ON arm.
    Q-040b (deferred): conditional on Q-040a PASS, does MECH-269b
        produce non-zero approach_commit AND non-zero dACC behavioural-
        adjustment magnitude? Gated on Phase 2 forward-predictor V_s
        (z_world via ReafferencePredictor SD-007; z_harm_s via
        HarmForwardModel SD-011) OR a substrate change that wires
        staleness_accumulator into VsRolloutGate.gate() before the
        threshold comparison.

Architectural followup (NOT addressed by this experiment): the rollout
gate reads `per_stream_vs` (the flat dict) but the architectural intent
of MECH-269b is to gate on streams that have become STALE relative to
reality. Either Phase 2 T2 forward-predictor V_s OR staleness-corrected
V_s is required for the gate's threshold-crossing condition to track
the real "stale stream" signal.
...
EXPERIMENT_TYPE = "v3_exq_490b_mech269b_vs_rollout_gating"
CLAIM_IDS = ["Q-040"]
...
def _make_agent_and_env(seed: int, arm: Dict) -> Tuple[REEAgent, CausalGridWorldV2]:
    ...
    config = REEConfig.from_dims(
        ...
        # V3-EXQ-490b smoke threshold override: Phase 1 V_s identity proxy
        # stays in [0.85, 1.0] under typical agent dynamics, so default
        # thresholds (0.4) are unreachable in smoke-scale runs and the gate
        # never fires (V3-EXQ-490 / 490a c1_total_held = 0). Raising the
        # snapshot refresh + per-side hold thresholds into the typical V_s
        # operating range exercises the gate-firing precondition (Q-040a)
        # so substrate wiring is observable. NOTE: this is a smoke-only
        # acceptance bound; it does NOT validate MECH-269b's hypothesis
        # that streams should be held when stale -- at thresh=0.85 even
        # healthy streams with mild velocity get held. Behavioural
        # validation of MECH-269b (Q-040b) remains gated on Phase 2
        # forward-predictor-error V_s OR staleness-corrected V_s.
