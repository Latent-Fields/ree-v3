...
"""
V3-EXQ-436d -- SD-017/ARC-045/MECH-166 slot-differentiation retest on the
REPAIRED ContextMemory write path (contextmemory_gated_content_write=True)
SLEEP DRIVER: manual-multi (run_sleep_cycle() called directly every SLEEP_INTERVAL episodes in training loop)

experiment_purpose: evidence

Discharges IGW-20260803-212 (claim ARC-045, "Retest after substrate:
ARC-045"; pending_retest_after_substrate=true). Same experiment as 436c in
every respect EXCEPT one substrate flag -- see WHAT CHANGES vs 436c below.

WHY THIS RUN EXISTS (the substrate build that gates the retest has LANDED).
436c was a fair, non-degenerate test whose PRIMARY DV (slot_cosine_sim
differentiation, C1) FAILED 0/5 seeds and, worse, ran in the OPPOSITE
direction: SWS_THEN_REM drove whole-bank slot cosine similarity to
~0.9999-1.0 (homogenization) in 4/5 seeds -- see
failure_autopsy_V3-EXQ-436c_2026-08-03 (confirmed). That autopsy root-caused
the homogenization to ContextMemory.write()'s write_gate acting as CONTENT
rather than a MODULATOR: a plain nn.Linear+Sigmoid whose output sits within
+-0.02 of 0.5 on every channel, so every write blends the addressed slot
toward the SAME vector 0.5*ones(memory_dim). The fix
(ree-v3 5f331fe, ContextMemory.gated_content_write, default-OFF and
bit-identical to the legacy path until opted in) restores write_gate to its
namesake role: write_signal = sigmoid(write_gate_pre(x)) * write_content(x),
zero-centred and content-bearing. The autopsy's own §8 and the fix's code
comment both name THIS run explicitly: "re-run the 436-family under a new
letter (e.g. V3-EXQ-436d) against the repaired write path ... once the
substrate build lands -- not before."

Readiness confirmed empirically at authoring time (a throwaway probe replaying
200 writes at 436c's operating point, state rms ~0.08): legacy path drives
whole-bank off-diagonal cosine to 0.99993 (seed 2) while
gated_content_write=True holds it at 0.02487 -- differentiation, the direction
SD-017/ARC-045/MECH-166 predict. The flag is reachable via
REEConfig.from_dims(contextmemory_gated_content_write=True) -> cfg.e1.

WHAT CHANGES vs 436c (the ONLY substrate change):
  contextmemory_gated_content_write=True (added to _make_agent's from_dims and
  to both cells' config_slice). It is a SUBSTRATE property, held CONSTANT
  across BOTH conditions (WAKING_ONLY and SWS_THEN_REM) -- exactly like the
  MECH-313 noise floor. It is not the manipulation under test; the
  manipulation is still sleep (SWS_THEN_REM vs WAKING_ONLY). All conditions,
  DV definitions, action-selection wiring, thresholds (C1 >= 3/5, P0 gate),
  and seeds are UNCHANGED from 436c. This is a one-variable retest: hold the
  entire validated 436c design, flip only the write-path repair.

INTERPRETATION UPDATE vs 436c. Because the homogenization confound is removed,
a C1 FAIL here (with P0 met) is now a GENUINE weakens for SD-017/ARC-045/
MECH-166 rather than a write-path artifact, and a C1 PASS is genuine support
for the slot-differentiation prediction. Either outcome is a real, adjudicable
PASS/FAIL signal on the repaired substrate. (The prior 436c note that "this
letter fixes a RECORDING gap ... not the experimental design" refers to
436b->436c; 436c->436d fixes the write-path SUBSTRATE, holding the design.)
...
EXPERIMENT_TYPE = "v3_exq_436d_sd017_mech166_writepath_retest"
QUEUE_ID = "V3-EXQ-436d"
SUPERSEDES = "V3-EXQ-436c"
CLAIM_IDS = ["SD-017", "ARC-045", "MECH-166"]
EXPERIMENT_PURPOSE = "evidence"

# 436d substrate repair (the ONLY change vs 436c). See module docstring.
# Held CONSTANT across both conditions -- a substrate property, not the
...
def _make_agent(env: CausalGridWorldV2, sws_enabled: bool, rem_enabled: bool,
    ...
    cfg = REEConfig.from_dims(
        ...
        # 436d write-path repair (ree-v3 5f331fe) -- restores ContextMemory
        # write_gate to a modulator so writes are content-bearing. Constant
        # across both conditions. See module docstring.
...
def run(dry_run: bool = False) -> Tuple[dict, ZGoalStreamAccumulator]:
    ...
    result = {
        ...
        "notes": (
            "SD-017/ARC-045/MECH-166 slot-differentiation retest on the "
            "REPAIRED ContextMemory write path. Discharges IGW-20260803-212 "
            "(ARC-045 pending_retest_after_substrate=true). Successor to "
            "V3-EXQ-436c, which was a fair, non-degenerate test whose primary "
            "DV (slot_cosine_sim differentiation, C1) FAILED 0/5 seeds in the "
            "OPPOSITE direction (SWS_THEN_REM homogenized whole-bank cosine "
            "to ~0.9999-1.0 in 4/5 seeds) -- root-caused by "
            "failure_autopsy_V3-EXQ-436c_2026-08-03 (confirmed) to "
            "ContextMemory.write()'s write_gate behaving as constant content "
            "(sigmoid-midpoint 0.5*ones) rather than a modulator. The ONLY "
            "change here is the substrate flag "
            "contextmemory_gated_content_write=True (ree-v3 5f331fe, "
            "default-OFF and bit-identical until opted in), which restores "
            "write_gate to a modulator so writes are content-bearing: "
            "write_signal = sigmoid(write_gate_pre(x)) * write_content(x). "
            "The flag is held CONSTANT across both conditions (a substrate "
            "property, like the noise floor); the manipulation under test is "
            "still sleep (SWS_THEN_REM vs WAKING_ONLY). Conditions, DVs, "
            "thresholds (C1 >= 3/5, P0 write-counter gate), seeds, and "
            "action-selection wiring are otherwise UNCHANGED from 436c. With "
            "the homogenization confound removed, a C1 FAIL here (P0 met) is "
            "now a genuine weakens rather than a write-path artifact, and a "
            "C1 PASS is genuine support. The P0 gate (introduced in 436c) "
            "still guards against an unrecorded/unfired write path. Authoring "
            "probe: legacy path drives 200-write off-diagonal cosine to "
            "0.99993 (seed 2) vs 0.02487 with the flag on -- differentiation, "
