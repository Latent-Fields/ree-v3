...
"""V3-EXQ-485l: CORRECTED SD-033b trained-OFC-head BEHAVIOURAL DEVALUATION arm
(MECH-263 sig-a) ON the MECH-448 (ARC-107) rank-preserving F->eligibility
DEMOTION-ENABLED E3 selector, NOW WITH THE MECH-449 Go/No-Go ELIGIBILITY
CONSTITUTION ENGAGED so the active No-Go WITHDRAWAL that behavioural
outcome-devaluation requires can express.
...
PRE-REGISTERED ACCEPTANCE (per seed, then >= MIN_PASS_SEEDS of 3):
  READINESS (non-vacuity precondition, on the ARM_2 test arm; reported as PER-SEED PASS
    ...
    on):
      ...
      (c) head trained (state_bias_head weight-delta > HEAD_DELTA_MIN) on >= MIN_PASS_SEEDS.
...
# -- 485f Fix 1 (kept): defeat OFC clamp-saturation at the eval read --
OFC_BIAS_SCALE = 0.5
...
def _demotion_committed_eval(
    ...
    try:
        with torch.no_grad():
            ...
            bias_range_devalued = _bias_range(agent, bank)  # the C1/C1b DVs route on this
...
def run(seeds: Optional[List[int]] = None, dry_run: bool = False) -> Dict:
    ...
    head_trained_seeds = sum(1 for r in test if r["head_weight_delta_norm"] > HEAD_DELTA_MIN)
...
def _interpretation(result: Dict) -> Dict:
    ...
    preconditions = [
        ...
        },
        {
            ...
            "description": (
                "Number of ARM_2 seeds whose state_bias_head weight-delta-from-init L2 "
                "norm clears HEAD_DELTA_MIN -- the head genuinely trained under the paired "
                "threat-conditioned driver. PER-SEED PASS COUNT (>= MIN_PASS_SEEDS)."
            ),
...
def write_manifest(result: Dict, dry_run: bool, elapsed: float) -> Tuple[Path, str]:
    ...
    manifest = {
        ...
        "supersedes": "V3-EXQ-485k",
        "predecessor": (
            "V3-EXQ-485k (run_id v3_exq_485k_sd033b_demotion_devalued_rerank_behavioural_"
            "20260621T192541Z_v3; confirmed FAIL/non_contributory, "
            "failure_autopsy_V3-EXQ-485k_2026-06-21). Both DVs vacuous: FIX-1's re-ranking "
            "devaluation driver (gain 4.0) OVERSHOT the +/-0.5 OFC clamp -> uniform "
            "saturation -> readable devalued range exactly 0.0 on 2/3 seeds (the bias "
            "vector still inverted, cosine -0.71/-0.57), and it REGRESSED 485j's C2 "
            "discrimination conversion (1.0/0.0/1.0 -> 0/0/0). Root cause: demotion-alone "
            "(MECH-448 eligibility access) cannot express the active No-Go WITHDRAWAL "
            "behavioural outcome-devaluation requires -- blocked on the MECH-449 Go/No-Go "
            "constitution (now built)"
        ),
        "predecessors": [
        ...
        "notes": (
            "CORRECTED OFC outcome-devaluation arm, MECH-449-ENGAGING. SUPERSEDES "
            "V3-EXQ-485k (FAIL/non_contributory, failure_autopsy_V3-EXQ-485k_2026-06-21). "
            "The autopsy localised the residual to a STRUCTURAL gap: demotion-alone "
...
def main():
    ...
    print("\n=== V3-EXQ-485l SUMMARY ===", flush=True)
