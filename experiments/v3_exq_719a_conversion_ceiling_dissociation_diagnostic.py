...
"""
V3-EXQ-719a -- CONVERSION-CEILING DISSOCIATION DIAGNOSTIC for MECH-309 / ARC-062.

SUPERSEDES V3-EXQ-719 (a minimal, surgical re-queue). 719 RAN and self-routed
`substrate_not_ready_requeue` -- but this was a pure SAMPLING starvation, not a science
failure. 719's P2 eval phase was a FIXED 60 ep x 200 steps; the agent DIES EARLY on 2 of 3
seeds, so P2-tick counts were 951 / 10782 / 1369 -- only 1/3 seeds cleared the
`mi_estimable_sufficient_ticks` readiness floor of 3000 ticks (the readiness gate needs
>= 2/3), and the MI dissociation was therefore never scored. This is exactly the case the
pre-registered `substrate_not_ready_requeue` route exists for. 719a's ONE substantive change
makes the P2 eval loop TICK-BUDGET-DRIVEN (target P2_TICKS_TARGET=6000 committed P2 ticks OR
a cap of P2_MAX_EPISODES=400 episodes, whichever first). On a surviving seed ~6000 ticks
arrives in ~35 episodes; on an early-dying seed the 400-episode cap yields ~6000+ ticks.
Either way every seed clears the 3000 floor and per-seed tick counts equalize, improving MI
comparability. Everything else -- P0=200 warmup, P1=90 bias-head training, the all-ON config,
the state-binning, committed_class extraction, the three DV families, the shuffle-null MI, the
self-route grid, all thresholds (incl. the UNCHANGED 3000-tick readiness floor), and all
contract fields -- is IDENTICAL to 719.
...
demonstrator), run in CausalGridWorldV2 over SEEDS = [42, 43, 44]. Phases:
  ...
  P2 (TICK-BUDGETED eval / logging phase; all frozen; OFC devaluation viability injected into
     the Go/No-Go gate exactly as 714) -- runs P2 episodes until P2_TICKS_TARGET=6000 committed
     P2 ticks are collected OR a cap of P2_MAX_EPISODES=400 episodes is reached (whichever
     first), so every seed clears the 3000-tick readiness floor and MI over up to ~25
     state-bins is estimable. (719 used a FIXED 60-ep P2 and undersampled: only 1/3 seeds
     cleared the 3000 floor because the agent died early on 2/3 seeds.)
...
EXPERIMENT_TYPE = "v3_exq_719a_conversion_ceiling_dissociation_diagnostic"
QUEUE_ID = "V3-EXQ-719a"
SUPERSEDES = "v3_exq_719_conversion_ceiling_dissociation_diagnostic"
...
# P2 is TICK-BUDGET-DRIVEN (719a change; 719 used a fixed 60-ep P2 that undersampled).
# 719's fixed P2 died early on 2/3 seeds -> per-seed P2 ticks were 951 / 10782 / 1369, so
# only 1/3 seeds cleared the TOTAL_TICKS_FLOOR (3000) readiness gate and the MI dissociation
# was never scored (self-routed substrate_not_ready_requeue -- a pure SAMPLING starvation).
# 719a runs P2 episodes until P2_TICKS_TARGET committed P2 ticks are collected OR a cap of
# P2_MAX_EPISODES episodes is reached, whichever comes first. On a surviving seed ~6000 ticks
# arrives in ~35 episodes; on an early-dying seed the 400-episode cap yields ~6000+ ticks.
# Either way every seed clears the 3000 floor and per-seed tick counts equalize.
P2_TICKS_TARGET = 6000           # collect at least this many committed P2 ticks per seed
P2_MAX_EPISODES = 400            # hard cap on P2 episodes (denominator for the [train] print)

DRY_RUN_SEEDS = [42, 43]
DRY_RUN_P0 = 2
DRY_RUN_P1 = 2
DRY_RUN_P2_TICKS_TARGET = 30     # tiny tick target so the smoke stays fast
DRY_RUN_P2_MAX_EPISODES = 3      # tiny episode cap so the smoke stays fast
DRY_RUN_STEPS = 40
...
def _run_seed(
    ...
    # P2 is tick-budgeted: the outer loop is bounded by P2_MAX_EPISODES worth of P2 episodes,
    # but breaks out of P2 early once p2_ticks_target committed P2 ticks are collected.
    ...
    for ep in range(total_train_eps):
        ...
        # Phase-relative progress print (P0/P1 denominators are their own episode
        # counts; the P2 denominator is P2_MAX_EPISODES). Keeps the '[train] ... ep N/M'
        # format the runner's progress/time estimator parses.
        if phase_label == "P2":
        ...
        # P2 tick-budget termination: stop the eval phase once enough committed P2 ticks
        # are collected (equalizes per-seed tick counts vs 719's early-death undersampling).
        # The P2_MAX_EPISODES cap is enforced by the outer loop bound.
...
def run_experiment(
    ...
    print(
        ...
        f"P1={p1_episodes} ep two-head REINFORCE, P2 tick-budgeted "
        f"(target={p2_ticks_target} ticks / cap={p2_max_episodes} ep) eval/logging, "
    ...
    for s in seeds:
        ...
        row = _run_seed(
            s, p0_episodes, p1_episodes, p2_ticks_target, p2_max_episodes,
            steps_per_episode,
        )
...
def _build_manifest(
    ...
    return {
        ...
        "evidence_direction_note": (
            f"V3-EXQ-719a CONVERSION-CEILING DISSOCIATION DIAGNOSTIC (MECH-309 / ARC-062; "
            f"experiment_purpose=diagnostic, EXCLUDED from governance scoring; SUPERSEDES "
            f"v3_exq_719 -- 719 undersampled P2, 1/3 seeds cleared the 3000-tick floor due to "
            f"early episode death, so P2 is now TICK-BUDGETED: target "
            f"{result['p2_ticks_target']} ticks / cap {result['p2_max_episodes']} eps per "
            f"seed). Dissociates two "
            ...
            f"warmup -> P1 two-head REINFORCE -> P2 tick-budgeted eval/logging. Per P2 tick logs a "
        ...
        "config_summary": {
            ...
            "phases": "P0 e2-warmup (field matures) -> P1 frozen-encoder TWO-head REINFORCE (lateral_pfc + OFC devaluation) -> P2 frozen TICK-BUDGETED eval/logging (run until P2_TICKS_TARGET committed ticks OR P2_MAX_EPISODES eps, whichever first) w/ OFC viability injection",
            "matched_stack": (
                "SP-CEM + candidate_summary_source=e2_world_forward (SD-056/GAP-A) + "
...
def main() -> Tuple[Optional[str], Optional[str], bool]:
    parser = argparse.ArgumentParser(
        description=(
            "V3-EXQ-719a conversion-ceiling dissociation DIAGNOSTIC "
