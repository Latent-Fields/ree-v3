"""
V3-EXQ-467e (EXP-0163 behavioural): MECH-266 mode stickiness / Hold-decay
...
V3-EXQ-467d (NOT a supersede).

ROUTING: 467d re-enabled use_external_task_drive but STILL FAILed
non_contributory (substrate_ceiling): fraction_in_external_task = 0.0 on EVERY
seed/ratio, identical to 467c's zero-occupancy signature
...
of 464d's own substrate (shared foraging/salience substrate with 467d)
isolated TWO compounding root causes, both fixed here:
  ...
  (1) DRIVER BUG -- `_clone_for_arm()` cloned the trained agent via
      ...
      clone -- each ratio arm's clone `goal_state.is_active()` read False for
      the ENTIRE eval, hard-gating `external_task_drive`'s engagement to 0.0
  ...
  (2) SUBSTRATE MAGNITUDE MISMATCH -- `dacc_pe` is uncapped by default and
      ...
      applied. 467e opts IN (`salience_affinity_input_cap=AFFINITY_INPUT_CAP`,
      the same calibrated starting point as 464e, from a scratch diagnostic
      combining both fixes -- see AFFINITY_INPUT_CAP below; NOT yet a fully
      re-derived, pre-registered production value for the full curriculum).

THREE substrate/driver changes vs 467d (467d's own change vs 467c --
use_external_task_drive=True -- is retained unchanged):
  ...
  (2) `salience_affinity_input_cap=AFFINITY_INPUT_CAP` added to the agent
      config (fix 2 above).
  (3) the non-vacuity readiness gate is UNCHANGED from 467d -- the same
      OCCUPANCY_FLOOR / min-over-ratio-arms(fraction_in_external_task)
      structure, since it is the gate that correctly self-routed both 467c
      and 467d to non_contributory rather than a false weakens; only the
      substrate underneath it could not previously clear it.

WHAT THIS MEASURES (unchanged from 467c/467d): the MECH-266 hysteresis
dose-response. Sweep the uniform exit-rail ratio across r in
{0.10, 0.50, 1.00, 1.50, 2.00}. A lower exit rail (r=0.10) makes modes harder
to leave -> longer dwell + fewer switches; a higher rail (r=2.00) makes them
easy to leave -> shorter dwell. Mean dwell should be monotone non-increasing
in r, with the lowest-r dwell at least C2_MIN_DWELL_RATIO x the highest-r
dwell.
...
EXTERNAL_TASK OCCUPANCY NON-VACUITY GATE (unchanged from 467d -- the SAME
  occupancy statistic the dose-response operates over, per the V3-EXQ-643
  same-statistic rule): before scoring C1/C2, assert the contested external_task
  mode is genuinely occupied across the sweep -- per-seed occupancy_min = min
  over ratio arms of fraction_in_external_task > OCCUPANCY_FLOOR -- on >= 2/3
  guard-passing seeds. Below floor -> substrate_not_ready_requeue
  (non_contributory), NEVER a false weakens. A zero-occupancy read at this
  point would mean the two fixes above are STILL insufficient (e.g.
  AFFINITY_INPUT_CAP mis-calibrated for the full curriculum), not "MECH-266
  falsified".
...
predecessor: V3-EXQ-467d (NOT a supersede).
...
QUEUE_ID = "V3-EXQ-467e"
CLAIM_IDS: List[str] = ["MECH-266", "SD-032a"]
EXPERIMENT_PURPOSE = "evidence"
PREDECESSOR = "V3-EXQ-467d (successor, NOT supersede)"
...
# occupied across the sweep before the dose-response is scored. Unchanged from
# 467d/467c's gate structure.
...
# `9bcde4cb63`). Same calibrated starting point as V3-EXQ-464e, from a scratch
# diagnostic combining this cap with the goal_state clone fix below. NOT yet
# re-derived/justified against the full 3-seed curriculum -- this run is that
# calibration check, not a confirmed final value.
...
def _make_config(env) -> REEConfig:
    cfg = REEConfig.from_dims(
        ...
        # SOURCE the SalienceCoordinator lacked. Unchanged from 467d.
        ...
        # occupancy-gap fix (2), NEW vs 467d: bound the dacc_pe/foraging affinity-logit
        ...
        # substrate knob (ree-v3 9bcde4cb63); this experiment opts in.
...
def _build_dual_cue_env(scaffold_cfg: ScaffoldedSD054OnboardingConfig,
    ...
    dual_cue requires SD-049, which the scaffolded _sd049_kwargs enables.    `seed` is the DRIVER-OWNED counterpart of the scheduler's `scaffold_env_seed`
...
def _clone_for_arm(trained_agent: REEAgent, device: torch.device) -> REEAgent:
    """MECH-266 occupancy-gap fix (1), NEW vs 467d: also clone `goal_state`.
    ...
    weights-only clone below -- each ratio arm's clone `goal_state.is_active()`
    then read False for the whole eval, hard-gating external_task_drive's
    engagement to 0.0 regardless of beta_elevated / goal_proximity
...
def _eval_mode_dwell(
    ...
    from 467b/d, using _sense_with_optional_harm). Applies set_hysteresis_ratio(ratio)
...
def _run_seed(seed: int, dry_run: bool, total_eps: int,
    ...
    # mode-governance-engagement occupancy non-vacuity gate (unchanged from 467d):
    # the contested mode must be occupied across the sweep.
            ...
            "preconditions": [
                ...
                },
                {
                    ...
                    "description": "the contested external_task mode is genuinely occupied across "
                                   ...
                                   "Unchanged from 467d's gate; 467e adds the goal_state clone fix + "
                                   "salience_affinity_input_cap so the substrate underneath the gate "
                                   "can actually clear it. A zero-occupancy read here means the two "
                                   "fixes are still insufficient (e.g. AFFINITY_INPUT_CAP "
                                   "mis-calibrated for the full curriculum), NOT a MECH-266 "
                                   "falsification.",
...
def main(dry_run: bool = False,
    ...
    manifest = {
        ...
        "substrate": "scaffolded_sd054_onboarding (full curriculum; 603n config; ready=true "
                     ...
                     "primitive + goal_state clone fix + salience_affinity_input_cap (ree-v3 "
                     "9bcde4cb63, landed 2026-08-12). use_closure_operator OFF (closure would "
                     "inject a confounding mode-switch signal).",
        "condition": CONDITION_LABEL,
        "predecessor": PREDECESSOR,
        "method_note": "467d re-enabled use_external_task_drive but STILL FAILed non_contributory "
                       "(fraction_in_external_task=0.0 on every seed/ratio) because of two "
                       "compounding bugs: (1) _clone_for_arm() dropped the GoalState z_goal "
                       "attractor on clone (GoalState is not an nn.Module, so "
                       "state_dict()/load_state_dict() never touched it), pinning "
                       "goal_state.is_active()=False for the whole eval and hard-gating "
                       "external_task_drive's engagement to 0; (2) dacc_pe was uncapped and "
                       "measured ~15-17 throughout eval, two orders of magnitude above "
                       "external_task_drive's [0,1]-bounded contribution, so internal_planning's "
                       "argmax never yielded regardless of engagement. 467e fixes both: an explicit "
        ...
        "readiness_note": "TWO non-vacuity gates self-route a substrate-not-engaged read to "
                          ...
                          "statistic the dose-response operates over). Gate structure unchanged "
                          "from 467d; the fixes are in the substrate the gate measures, not the "
                          "gate itself.",
