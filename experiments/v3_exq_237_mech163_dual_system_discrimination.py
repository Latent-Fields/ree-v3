...
"""
V3-EXQ-237 -- MECH-163: Dual Goal-Directed Systems Discrimination
...
# Experiment metadata
# ---------------------------------------------------------------------------
EXPERIMENT_TYPE    = "v3_exq_237_mech163_dual_system_discrimination"
CLAIM_IDS          = ["MECH-163"]
EXPERIMENT_PURPOSE = "evidence"
...
def _run_seed(
    ...
    for context in ["SIMPLE", "LONG_HORIZON"]:
        ...
        for condition in ["HABIT", "PLANNED"]:
            print(
                f"\n[V3-EXQ-237] seed={seed} context={context} cond={condition}"
                f" steps={steps}",
                flush=True,
            )
...
def main() -> None:
    ...
    print(
        f"[V3-EXQ-237] MECH-163 Dual Goal-Directed Systems Discrimination"
    ...
    all_results = []
    for seed in seeds:
        print(f"\n[V3-EXQ-237] === seed={seed} ===", flush=True)
    ...
    print(f"\n[V3-EXQ-237] Written: {out_path}", flush=True)
