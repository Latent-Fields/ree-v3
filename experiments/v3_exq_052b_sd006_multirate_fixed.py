...
Root cause of EXQ-052 C3 FAIL:
  EXQ-052 tracked theta buffer fill level with:
    tsize = len(agent.theta_buffer._buffer)
  The ThetaBuffer class does not have a `_buffer` attribute. The correct attribute
  is `_z_world_buffer`. ThetaBuffer.__len__ returns len(self._z_world_buffer).
  The AttributeError was silently caught, keeping tsize=0 always and
  max_theta_buffer_size=0 throughout the run. This caused C3 to FAIL despite
  the buffer actually filling correctly.

  All other criteria were already passing:
    C1 PASS: cal_gap_approach=0.846 > 0
    C2 PASS: e3_tick_ratio=0.109 < 0.3
    C4 PASS: e3_tick_count >= 10
    C5 PASS: no fatal errors
  Only C3 failed due to the diagnostic bug.

Fix:
  Replace:
    tsize = len(agent.theta_buffer._buffer)   # BUG: _buffer does not exist
  With:
    tsize = len(agent.theta_buffer)   # FIX: ThetaBuffer.__len__ returns len(_z_world_buffer)
...
EXPERIMENT_TYPE = "v3_exq_052b_sd006_multirate_fixed"
...
def _run_condition(
    ...
    for ep in range(num_episodes):
        ...
        for _ in range(steps_per_episode):
            ...
            # FIX: use len(agent.theta_buffer) which calls ThetaBuffer.__len__
            # returning len(self._z_world_buffer). The _buffer attribute does not exist.
            try:
                tsize = len(agent.theta_buffer)
...
def run(
    ...
    **kwargs,
    ...
    print(
        f"[V3-EXQ-052b] SD-006: Multi-Rate Validation (Fixed)\n"
        f"  warmup={warmup_episodes}  alpha_world={alpha_world}\n"
        f"  Default clock rates: e2_steps_per_tick=3, e3_steps_per_tick=10\n"
        f"  Fix: len(agent.theta_buffer) instead of len(agent.theta_buffer._buffer)",
        flush=True,
    )
    ...
    print("[V3-EXQ-052b] Running separated-rate condition (N_e2=3, N_e3=9)...", flush=True)
    ...
    print(f"\nV3-EXQ-052b verdict: {status}  ({criteria_met}/5)", flush=True)
    ...
    summary_markdown = f"""# V3-EXQ-052b -- SD-006: Multi-Rate Execution Validation (Fixed)
