...
"""V3-EXQ-610a: INV-074 crystallization necessity test (discriminative pair).

Supersedes V3-EXQ-610 (SIGTERM after 13.25h actual vs 180min estimate;
no code change -- just realistic estimated_minutes=800).
...
Estimated runtime: ~800 min (2 arms x 3 seeds x ~2500 episodes @ 200 steps/ep)
  -- V3-EXQ-610 (predecessor) actual_secs=47729 (~13.25h) on DLAPTOP-4.local;
     180min predecessor estimate was off by ~4.5x and caused SIGTERM during
     ARM_1_test seed=42 around ep 2100/2500.
"""
...
def _run_arm(arm_config: Dict, seed: int, dry_run: bool) -> Dict:
    ...
    print(
        f"[V3-EXQ-610a] Starting {arm_label} seed={seed} crystallize={crystallize}",
        flush=True,
    )
...
def main(dry_run: bool = False) -> None:
    """Run V3-EXQ-610a: INV-074 crystallization necessity test."""
    print(
        f"[V3-EXQ-610a] INV-074 crystallization necessity test (EVB-0270)",
        flush=True,
    )
    ...
    if not dry_run:
        ...
        print(f"[V3-EXQ-610a] Manifest written: {manifest_path}", flush=True)
