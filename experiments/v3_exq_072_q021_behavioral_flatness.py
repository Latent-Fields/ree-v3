"""
V3-EXQ-072 -- Q-021 Behavioral Flatness Diagnostic
...
EXPERIMENT_TYPE = "v3_exq_072_q021_behavioral_flatness"
CLAIM_IDS = ["Q-021"]

# Proxy-field environment dims
BODY_OBS_DIM = 12   # use_proxy_fields=True adds harm_exposure[10], benefit_exposure[11]
WORLD_OBS_DIM = 250  # base 200 + hazard_field_view(25) + resource_field_view(25)
ACTION_DIM = 4
...
def _run_nogo_or_competitive(
    ...
    print(f"\n[EXQ-072] TRAIN {cond_label} seed={seed}", flush=True)
    agent.train()
    ...
    for ep in range(warmup_episodes):
        ...
        for _ in range(steps_per_episode):
            ...
            if harm_signal < 0 and agent._current_latent is not None:
                ...
                harm_target = torch.tensor([[1.0]], device=agent.device)
                ...
                z_world = agent._current_latent.z_world.detach()
                harm_pred = agent.e3.harm_eval(z_world)
                harm_target = torch.tensor([[0.0]], device=agent.device)
                harm_loss = F.mse_loss(harm_pred, harm_target)
                e3_opt.zero_grad()
                harm_loss.backward()
                e3_opt.step()
            ...
            if benefit_eval_enabled and agent._current_latent is not None:
                ...
                benefit_target = torch.tensor([[float(benefit_exp)]], device=agent.device)
...
def run(
    ...
    **kwargs,
    ...
    """
    print(f"\n[EXQ-072] Q-021 Behavioral Flatness Diagnostic", flush=True)
    ...
    summary_markdown = (
        f"# V3-EXQ-072 -- Q-021 Behavioral Flatness Diagnostic\n\n"
        f"**Status:** {status}\n"
        f"**Claims:** Q-021\n"
