...
"""
V3-EXQ-654d -- arc_062_rule_apprehension:GAP-B behavioural falsifier for
MECH-309 / ARC-062, ported onto the GAP-A DE-COLLAPSED substrate (ARM_STD_G2
conversion lever armed).

SUPERSEDES V3-EXQ-654c (FAIL non_contributory, substrate_ceiling, 2026-06-15T12:38Z;
confirmed failure_autopsy_V3-EXQ-654c_2026-06-15). 654c established the INVERTED
signature that motivates THIS re-queue: the 666c availability-maintenance amend
SOLVED mint/maintain (crf_max_pairwise_rule_dist 0.0 -> 1.711, minting stabilised
12-16, the pool now HOLDS >=2 differentiated rules) -- but activation collapsed to
crf_frac_active = EXACTLY 0.0: every matched rule was gated OUT. Code-grounded
two-coupled-fault cause:
  (fault 1) the SHARED GAP-A monostrategy collapse reached the CRF context key.
    crf_context_from_e2_world_forward keys rule mint/match off e2.world_forward,
    but that summary was itself collapsed (consumed_summary_pairwise_dist 0.0089,
    below the 0.05 GAP-A floor). Under a collapsed context the differentiated
    rules' context_tags all co-match the SAME context -> n_matched inflates to >=3.
  (fault 2) gate_and_select theta = 0.15 + 0.25*(n_matched-1) climbs to >= 0.65
    once >=3 rules co-match, ABOVE the maintenance_floor 0.45 -> no maintained rule
    clears the gate -> frac_active = 0.0. MAINTAINED != ACTIVE.
The autopsy routed: implement-substrate amend crf-availability-maintenance for BOTH
faults, with the 654d re-queue GATED on GAP-A context DE-COLLAPSE -- because the
CRF amend is necessary-but-not-sufficient while the shared collapse persists.

WHY THIS RE-QUEUE CAN NOW SCORE: the GAP-A gate has CLEARED. V3-EXQ-684a PASSED
2026-06-15 (conversion_mechanism_identified; confirmed coordinator DB + manifest):
the modulatory-bias-selection-authority CONVERSION amend's ARM_STD_G2 config
(modulatory_authority_normalize_basis=std + authority_gain=2.0 + the routed
e2_world_forward channel) drives committed selected-action entropy strict-above
the legacy additive-range bar (0.989 > 0.775) on 3/3 seeds. ARMING that config
here de-collapses the e2 context the CRF mint/match keys off (fault 1), which
drops n_matched and lets the existing maintenance_floor clear the gate (so fault 2
becomes moot when the context de-collapses), AND gives the authority the gain to
move the F-dominated committed argmax (the C2 lever).

DESIGN CHANGES vs V3-EXQ-654c (port onto the de-collapsed substrate; everything
else matched -- the CRF mature+maintenance+persist levers are UNCHANGED):
  1. ARM the 684a-validated ARM_STD_G2 GAP-A CONVERSION lever as a MATCHED-STACK
     constant on BOTH arms (the de-collapsed substrate both arms run on):
       modulatory_authority_normalize_basis = "std"  (was the default "range")
       modulatory_authority_gain            = 2.0     (was 0.5)
       use_modulatory_channel_routing       = True    (was off)
       modulatory_channel_route_source      = "cand_world_summary"
     candidate_summary_source=e2_world_forward is already present (654c). Only
     use_candidate_rule_field stays the swept variable, so the contrast is still a
     clean single-variable rule-field on/off on the de-collapsed substrate.
  2. RECORD crf_n_matched_last (the 654c-autopsy discriminator, learning #4): the
     654c script read crf_n_active but NOT crf_n_matched, so it could not directly
     prove "matched but gated out" vs "never matched". 654d records it so that if
     C1c fails AGAIN despite de-collapse, the manifest disambiguates whether the
     residual is still gate-lockout (n_matched high) -- routing to the CRF
     maintenance-theta coupling amend (autopsy part ii) -- or a different fault.

NOTE -- the residual CRF maintenance-theta coupling (autopsy part ii) is NOT
implemented in code (substrate_queue crf-availability-maintenance ready=False,
amend owed). 654d tests the autopsy's primary hypothesis -- that GAP-A context
de-collapse alone drops n_matched enough for the EXISTING maintenance_floor 0.45
to clear the gate. The C1c precondition is the GUARD: if de-collapse alone is
insufficient, C1c (crf_frac_active < 0.30) self-routes substrate_not_ready_requeue
-> route to the part-(ii) maintenance-theta amend, NOT a falsification.
...
EXPERIMENT_TYPE = "v3_exq_654d_arc062_gapb_rule_apprehension_behavioural_falsifier"
QUEUE_ID = "V3-EXQ-654d"
SUPERSEDES = "V3-EXQ-654c"
...
def _build_manifest(
    ...
    return {
        ...
        "evidence_direction_note": (
            f"V3-EXQ-654d arc_062 GAP-B behavioural falsifier (MECH-309 / ARC-062), "
            f"supersedes V3-EXQ-654c, ported onto the GAP-A DE-COLLAPSED substrate: the "
            f"684a-validated ARM_STD_G2 conversion lever (modulatory_authority_normalize_"
            f"basis={MODULATORY_AUTHORITY_NORMALIZE_BASIS} + authority_gain="
            f"{MODULATORY_AUTHORITY_GAIN} + use_modulatory_channel_routing="
            f"{USE_MODULATORY_CHANNEL_ROUTING} source={MODULATORY_CHANNEL_ROUTE_SOURCE}) "
            f"is ARMED as a matched-stack constant on BOTH arms. Single-variable: ARM_OFF "
            f"(use_candidate_rule_field=False, legacy collapsed rule_state) vs ARM_ON "
            f"(use_candidate_rule_field=True + crf_persist + the 666c ARM_2 levers "
            f"crf_mature_pool_dynamics + crf_context_from_e2_world_forward "
            f"+ crf_availability_maintenance[floor {CRF_MAINTENANCE_FLOOR}/decay "
            f"{CRF_MAINTENANCE_DECAY}], MATURED + MAINTAINED differentiated crf_source), "
...
def main() -> Tuple[Optional[str], Optional[str]]:
    parser = argparse.ArgumentParser(description="V3-EXQ-654d GAP-B falsifier (on the GAP-A de-collapsed substrate; ARM_STD_G2 conversion lever armed)")
