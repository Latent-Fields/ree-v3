...
"""
V3-EXQ-325d: SD-032c AIC-Analog -- Operating-Mode + Drive-Dependent Descending Modulation
             (reduced-compute retest of EXQ-325c)

experiment_purpose: evidence

Supersedes: V3-EXQ-325c (ERROR exit -15 on ree-cloud-1 at 33 min -- external
termination during governance window; compute budget 8.6x under estimated_minutes,
script itself showed no bug). This iteration:
  - 3 seeds (was 5), PASS threshold >=2/3 (was >=3/5)
  - 150 training episodes per arm (was 300); 30 eval unchanged
  - machine_affinity pinned to DLAPTOP-4.local to avoid shared-vCPU preemption
  - estimated_minutes raised to 360 (matches reduced-compute projection on Mac)
  - progress instrumentation added (ep N/M + verdict: per run)

Mechanism under test is UNCHANGED from 325c: SD-032c AIC-analog replaces the
legacy SD-021 raw-beta-gate check with a harm_s_gain function that depends on
operating_mode AND drive_level. EXQ-325a FAIL (DESCENDING==CONTROL bit-identical)
remains the observation this arms-race resolves.
...
EXPERIMENT_TYPE = "v3_exq_325d_sd032c_aic_descending_modulation"
...
def main():
    ...
    output = {
        ...
        "supersedes": "v3_exq_325c_sd032c_aic_descending_modulation",
