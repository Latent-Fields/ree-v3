"""V3-EXQ-794a: MECH-204 Phase 7 (broadcast precision anchor) x SD-076 (waking
confidence inflation) -- 2x2 substrate-readiness validation. Same-question re-run of
V3-EXQ-794 on the repaired drift source. Successor to V3-EXQ-774 via 794.

WHAT CHANGED FROM V3-EXQ-794 -- THE DESIGN IS UNCHANGED, THE SUBSTRATE IS REPAIRED
----------------------------------------------------------------------------------
794 tested nothing. `E3Config.waking_confidence_rv_floor` defaults to 0.01, while this
substrate's un-inflated operating point is rv = 0.005420 (ARM_OFF_OFF) and its true
error reference is ~0.0037 -- so the floor sat 1.8x ABOVE the operating point and 2.7x
above true error. `max(0.01, rv)` therefore clamped on the first tick inflation bit and
never released: `rv_final` finished at EXACTLY 0.010000 on ALL FOUR inflation arms, and
`overconfidence_score` was bit-identical to 15 significant figures
(-1.004111904519277) at asymmetry 0.6 AND 0.8, with `calibration_ratio` likewise
identical (2.7564936387545953).

Two nominally different doses producing the same value beyond float noise is a
SATURATION signature, not a null: the variable was clamped before the dose could
express itself. All four inflation arms went RED on `inflation_lowers_rv` -- not
because the sign was wrong, but because rv was pinned at a floor sitting ABOVE the
OFF arm, so the gate saw rv RISE. `failure_autopsy_V3-EXQ-794_2026-07-22` (confirmed)
WITHDREW SD-076's recorded `does_not_support` and revised it to `non_contributory`, and
MECH-204's `unknown` stands: C2's `d_broadcast_under_drift_at_operative.per_seed` was
EMPTY because C1 never established an operative drift level, so the correction had
nothing to correct.

The repair landed in ree-v3 452f99e367 (`sd_waking_confidence_inflation_headroom`) and
is what this run consumes:
  waking_confidence_rv_floor_relative_frac = 0.2   -- the bound is now a FRACTION of the
      counterfactual un-inflated rv (E3TrajectorySelector._wci_symmetric_rv_ref), so it
      scales with the substrate's own error scale instead of sitting at a constant that
      was only ever a floor at the scale it was chosen for. Caps overconfidence at
      1 - frac = 0.8, leaving ample room for C1's +0.10 margin.
  waking_confidence_rv_floor_mode = "soft" (softness 0.25) -- the bound is approached
      through a STRICTLY MONOTONIC softplus, so two distinct doses can never map to one
      value. A residual saturation can only SHRINK the LO/HI separation, never collapse
      it to the exact tie that made 794 unadjudicable without an autopsy.
`waking_confidence_rv_floor` (0.01) is retained in the config record for comparability
with 794 but is INERT here: a non-zero relative_frac replaces the absolute bound.

WHY A SMALLER ABSOLUTE FLOOR WOULD NOT HAVE BEEN ENOUGH. C1 counts seeds whose rv sits
BELOW true error (~0.0037). Any bound anchored at 0.01 makes that unreachable no matter
how softly it is approached -- so soft-clipping alone would have reproduced the null.
And note the 2026-07-20 SD-076 unit smoke passed 6/6 against this very defect, because
it ran on a synthetic error sequence with true mean 0.05, ~13x this substrate's real
scale, where 0.01 IS a floor with headroom. That is precisely why the repair is
relative and why this script gates on a MEASURED dose separation below.

NEW PRE-SCORING GATE (the autopsy's acceptance target, made machine-checkable). A new
regime-conditioned precondition `dose_levels_separated` asserts, on every inflation
arm, that its `rv_final` differs from its SIBLING arm at the other asymmetry level (same
broadcast level) by more than DOSE_SEPARATION_FLOOR -- the same statistic the DV routes
on, measured against a matched positive control differing only in the dose. If the
clamp is still binding, the arms are pinned together, this precondition fails, those
arms are gated RED and marked non-degenerate:false, and the run self-routes
`substrate_not_ready_requeue` -- it CANNOT record a refutation off a clamped lever the
way 794 did. `per_level` additionally now carries `rv_final` / `calibration_ratio` /
`true_error_ref`, and the manifest declares `dose_key: "asymmetry"`, so the standing
`dose_saturation` lint (experiments/_lib/dose_saturation.py, landed in the same commit)
independently stamps `dose_levels_separable` at manifest-write time.
...
GOV-REUSE-1: the decisive readouts (LO-vs-HI separation in rv_final, and
n_seeds_overconfident) ARE recorded in V3-EXQ-794 -- but as the clamped values 0.010000
/ 0.010000 and 0 / 0, which is exactly the saturation this run exists to escape. 774 is
on a substrate with neither Phase 7 nor SD-076, and 794 predates the floor repair, so
BOTH carry a different substrate_hash from this run by construction (the repair edits
ree_core/predictors/e3_selector.py + ree_core/utils/config.py, both inside the substrate
glob). Not recoverable -> run. Checked run_ids:
v3_exq_794_mech204_phase7_sd076_calibration_loop_2x2_20260721T113848Z_v3 and
v3_exq_774_mech173_rem_suppression_precision_calibration_20260717T152554Z_v3.
"""
...
EXPERIMENT_TYPE = "v3_exq_794a_mech204_phase7_sd076_calibration_loop_2x2"
...
# SD-076 HEADROOM REPAIR knobs (ree-v3 452f99e367). The absolute floor above is INERT
# while relative_frac > 0; it is retained only so this run's config record stays
# directly comparable with 794's. See the module docstring for why an absolute bound was
# the wrong KIND of quantity rather than merely the wrong value.
INFLATION_RV_FLOOR_RELATIVE_FRAC = 0.2   # bound = 0.2 x the un-inflated counterfactual rv
INFLATION_RV_FLOOR_MODE = "soft"         # softplus approach; strictly monotonic
INFLATION_RV_FLOOR_SOFTNESS = 0.25       # knee width as a fraction of the effective floor
...
# Pre-registered MINIMUM separation in rv_final between the two dose levels. Units are
# rv (variance), NOT log-ratio. Set at 1e-4 -- three orders of magnitude below the
# ~4.3e-4 LO/HI gap the repaired substrate produced in the unit smoke at this error
# scale, and far above float noise. Its job is to catch a STILL-CLAMPED lever (794's gap
# was exactly 0.0), not to adjudicate effect size.
...
# Each inflation arm's SIBLING at the OTHER asymmetry level, same broadcast level. This
# is the matched positive control for the dose-separation precondition: the pair differs
# ONLY in the dose, so a pinned-together pair is a clamped lever and nothing else.
DOSE_SIBLING = {
    "ARM_INFL_LO": "ARM_INFL_HI",
    "ARM_INFL_HI": "ARM_INFL_LO",
    "ARM_BOTH_LO": "ARM_BOTH_HI",
    "ARM_BOTH_HI": "ARM_BOTH_LO",
}
...
PRECONDITION_SPECS: Tuple[PreconditionSpec, ...] = (
    ...
    PreconditionSpec(
        ...
        description="|rv_final(this arm) - rv_final(sibling arm at the OTHER asymmetry, "
                    "same broadcast level)|. THE 794 GATE: two nominally different doses "
                    "producing the same rv is a SATURATION signature, not a null -- 794 "
                    "measured EXACTLY 0.010000 on all four inflation arms and a "
                    "bit-identical overconfidence_score at both doses. Same statistic "
                    "the DV routes on, against a control differing ONLY in the dose.",
        control="sibling inflation arm at the other asymmetry level, same broadcast "
                "level, same seeds -- differs only in the dose",
        threshold=DOSE_SEPARATION_FLOOR,
        direction="lower",
        applies_to=lambda ctx: bool(ctx["inflation"]),
        applies_note="an inflation-OFF arm has no dose, so it has no sibling level to "
                     "separate from; asserting this would be structurally unsatisfiable "
                     "and would vacate the arm.",
    ),
)
...
def _make_agent(env: CausalGridWorldV2, bcast: bool, asym: Optional[float]) -> REEAgent:
    ...
    # SD-076 headroom repair: a non-zero relative_frac supersedes the absolute floor.
...
def _analyse(cells: List[Dict], seeds: List[int]) -> Dict:
    ...
    for (arm_id, bcast, asym) in ARMS:
        ...
        if infl:
            ...
            # THE 794 GATE. Compared on the arm-level means (the same reduction the DV
            # and per_level use), NOT per-seed: a clamp pins the arm MEAN, which is the
            # quantity `per_level` reports and the dose_saturation lint reads.
            sibling = DOSE_SIBLING[arm_id]
            measured["dose_levels_separated"] = abs(
    ...
    for (lvl, asym, infl_arm, both_arm) in INFLATION_LEVELS:
        ...
        per_level[lvl] = {
            ...
            # Carried so the LO/HI contrast is visible per-level in the manifest AND so
            # the standing dose_saturation lint has non-zero float readouts to compare.
            # 794 recorded these only under `aggregates.arm_*`, which is why its
            # saturation needed an autopsy to see.
            # DELIBERATELY EXCLUDES true_error_ref. It is the DENOMINATOR of the DV, a
            # CONTROL measured independently of the inflation lever from the same seeds,
            # so it is invariant across dose BY DESIGN -- 794 recorded exactly
            # 0.0036995331068548566 for all four inflation arms, and the autopsy notes it
            # is "essentially constant across every arm". Carrying it here would tie on
            # EVERY run and make dose_saturation fire a guaranteed false positive, which
            # is how a lint loses its credibility. It stays in aggregates.arm_true_error_ref
            # where a reader can still check the denominator is not moving.
            "rv_final": arm_rv[infl_arm],
            "calibration_ratio": arm_ratio[infl_arm],
...
def main(dry_run: bool = False) -> Dict:
    ...
    manifest = {
        ...
        # Explicit dose declaration for the standing dose_saturation lint
        # (experiments/_lib/dose_saturation.py). It would infer "asymmetry" here anyway
        # -- it is the only fully-varying numeric per_level key -- but declaring it makes
        # the lint's verdict independent of what other numeric readouts per_level grows
        # later, and records the intent for a reader.
        "dose_key": "asymmetry",
        "interpretation": {
            "label": adj["label"],
