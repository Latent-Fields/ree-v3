...
Dependency note:
  Requires EXQ-018b to establish that relative threshold produces both committed
  and uncommitted steps. Queue after EXQ-018b.

Key change from EXQ-050:
  EXQ-050 used the default commitment_threshold from config, which may be set at
  an absolute value that causes the agent to either commit on every step or never
  commit. This experiment calibrates the threshold from the training baseline:
    calibrated_threshold = 2.0 * mean_training_variance
  This approach (from EXQ-018b) ensures the agent enters both committed and
  uncommitted modes during eval, making the doing_mode_delta observable.

Changes:
  1. After training, compute mean_training_variance from the last 100 training
     episodes by tracking agent.e3._running_variance values during training.
  2. Set agent.e3.config.commitment_threshold = mean_training_variance * 2.0
     (patches the config in-place for the eval phase).
  3. In eval, track committed steps and uncommitted steps separately.
  4. PASS: doing_mode_delta > 0.002 AND uncommitted_step_count >= 10.
...
PASS criteria (ALL must hold):
  ...
  C2: uncommitted_step_count >= 10  (uncommitted mode is entered -- threshold not too high)
...
EXPERIMENT_TYPE = "v3_exq_050b_mech025_doing_mode_relative_threshold"
CLAIM_IDS = ["MECH-025"]
...
def _train(
    ...
    Also collects running_variance samples from the last 100 episodes for threshold calibration."""
    agent.train()
    ...
    # Collect running_variance samples from the last 100 episodes
    running_variance_samples: List[float] = []
    collect_from_ep = max(0, num_episodes - 100)
    ...
    for ep in range(num_episodes):
        ...
        for _ in range(steps_per_episode):
            ...
            if ticks.get("e3_tick", False) and candidates:
                ...
                # Collect running variance for threshold calibration
                if ep >= collect_from_ep:
                    rv = getattr(agent.e3, "_running_variance", None)
                    if rv is not None:
                        try:
                            running_variance_samples.append(float(rv))
                        except Exception:
                            pass
            else:
    ...
    return {
        ...
        "running_variance_samples": running_variance_samples,
    }
...
def run(
    ...
    **kwargs,
    ...
    print(
        f"[V3-EXQ-050b] MECH-025: Action-Doing Mode Probe (Relative Threshold)\n"
    ...
    # Calibrate commitment threshold from training baseline
    rv_samples = train_out["running_variance_samples"]
    if rv_samples:
        mean_training_variance = _mean_safe(rv_samples)
    else:
        # Fallback: read current running_variance directly
        rv = getattr(agent.e3, "_running_variance", None)
        mean_training_variance = float(rv) if rv is not None else 0.1

    calibrated_threshold = mean_training_variance * 2.0
    print(
        f"  mean_training_variance={mean_training_variance:.6f}"
        f"  calibrated_threshold={calibrated_threshold:.6f}",
        flush=True,
    )

    # Patch config in-place for eval phase
    if hasattr(agent.e3, "config") and hasattr(agent.e3.config, "commitment_threshold"):
        agent.e3.config.commitment_threshold = calibrated_threshold
        print(f"  Patched agent.e3.config.commitment_threshold = {calibrated_threshold:.6f}", flush=True)
    elif hasattr(agent.e3, "_commit_threshold_override"):
        agent.e3._commit_threshold_override = calibrated_threshold
        print(f"  Patched agent.e3._commit_threshold_override = {calibrated_threshold:.6f}", flush=True)
    else:
        print(f"  WARNING: Could not patch commitment_threshold -- no config or override attribute found", flush=True)

    print(f"\n[V3-EXQ-050b] Eval -- probing action-doing mode...", flush=True)
    eval_out = _eval_doing_mode(agent, env, eval_episodes, steps_per_episode, world_dim)

    # PASS / FAIL
    c1_pass = eval_out["doing_mode_delta"] > 0.002
    c2_pass = eval_out["uncommitted_step_count"] >= 10
    ...
    if not c2_pass:
        failure_notes.append(
            f"C2 FAIL: uncommitted_step_count={eval_out['uncommitted_step_count']} < 10"
        )
    if not c3_pass:
    ...
    print(f"\nV3-EXQ-050b verdict: {status}  ({criteria_met}/5)", flush=True)
    ...
    metrics = {
        ...
        "mean_training_variance":      float(mean_training_variance),
    ...
    summary_markdown = f"""# V3-EXQ-050b -- MECH-025: Action-Doing Mode Probe (Relative Threshold)
...
| Parameter | Value |
|-----------|-------|
| mean_training_variance | {mean_training_variance:.6f} |
| calibrated_threshold (2x) | {calibrated_threshold:.6f} |
...
| C2: uncommitted_step_count >= 10 (uncommitted mode entered) | {"PASS" if c2_pass else "FAIL"} | {eval_out['uncommitted_step_count']} |
