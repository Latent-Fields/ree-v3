...
"""
V3-EXQ-192 -- MECH-075 Hippocampal-VTA Novelty Loop Probe
...
EXPERIMENT_TYPE = "v3_exq_192_mech075_novelty_loop_probe"
CLAIM_IDS = ["MECH-075"]
...
def _compute_world_novelty(
    ...
    with torch.no_grad():
        ...
        # Extract z_world portion of prediction (last world_dim dims)
        pred_world = pred[:, -world_dim:]
...
def _run_single(
    ...
    print(
        f"\n[V3-EXQ-192] TRAIN {cond_label} seed={seed}"
...
def run(
    ...
    summary_markdown = (
        f"# V3-EXQ-192 -- MECH-075 Hippocampal-VTA Novelty Loop Probe\n\n"
        f"**Status:** {status}\n"
        f"**Claims:** MECH-075\n"
        f"**Decision:** {decision}\n"
        f"**Seeds:** {list(seeds)}\n"
