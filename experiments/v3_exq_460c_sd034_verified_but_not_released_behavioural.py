...
def _make_scaffold_cfg(dry_run: bool,
    ...
    legacy OS-entropy path exactly as every landed 460c run had it."""
    if dry_run:
...
def _build_closure_env(scaffold_cfg: ScaffoldedSD054OnboardingConfig,
    ...
    bit-identical to every landed 460c run, and `_run_seed`'s np/torch seeding
    still does not reach this env unless the knob is set. Do NOT flip the default:
    pinning it unconditionally would change the eval-arm env layout, and therefore
    the results, breaking comparability with the landed run.
    """
    p2_hfa = (
...
def _run_seed(seed: int, dry_run: bool, total_eps: int,
    ...
    # three seeds still see three different worlds rather than collapsing onto
    # one shared layout -- `_derive_env_seed` multiplies the base by 1e6, so
    # adjacent bases cannot collide.
...
def main(dry_run: bool = False, env_seed_base: Optional[int] = None) -> Dict[str, Any]:
