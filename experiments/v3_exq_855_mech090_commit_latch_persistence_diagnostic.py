#!/usr/bin/env python3
"""V3-EXQ-855 -- MECH-090 commit-latch cross-tick persistence: H1(wiring) vs
...
EXPERIMENT_TYPE = "v3_exq_855_mech090_commit_latch_persistence_diagnostic"
...
# The gate is UNREACHABLE only at toy --dry-run scale (n_train=2), which is
# the expected, self-routing substrate_not_ready_requeue smoke-test outcome,
# not a defect -- confirmed directly by this script's own --dry-run run.
ANCHOR_REACHABILITY_EXEMPT = (
    ...
    "base.STEPS_PER_EPISODE=72, same ENV_KWARGS/CHUNK_* constants); this "
    "script changes nothing about formation, only what happens to a chunk "
    "after it is committed, so re-deriving reachability here would just "
    "re-run 810a/841's own already-recorded result at extra compute cost."
)
...
def _arm_flags() -> Dict[str, Any]:
    ...
    construction as V3-EXQ-841's `_arm_flags(A_HIER_S2)`, PLUS a script-local
    override of `use_persistent_committed_program_handle`.

    See module docstring "STATUS UPDATE (2026-08-02)": ree-v3 main commit
    278599a landed a fallback in agent.py's between-E3-tick stepping branch to
    `agent.e3._persistent_committed_trajectory`, gated behind this flag
    (default False). The shared lineage baseline (`experiments/_lib/baselines/
    arc071_chunking.py off_arm_flags()`) deliberately leaves the flag at its
    global default pending a governance-aware review of the arm-reuse
    fingerprint blast radius (fix gap (b) in the diagnosis doc) -- so this
    script overrides it locally, exactly mirroring the existing precedent at
    `experiments/_lib/baselines/sd084_midexec_reachability.py:271`, which does
    the same without touching the shared baseline. Without this override the
    run would just reproduce the already-known (and already-confirmed) pre-fix
    behaviour under organic formation instead of testing whether the landed
    fix actually restores persistence there.
    """
