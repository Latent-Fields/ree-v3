...
"""
V3-EXQ-418 -- SD-017 Context-Conditioned Action: Sleep + SD-016 Integration

EXPERIMENT_PURPOSE: evidence

MECHANISM UNDER TEST:
  SD-017 (sleep_phase.minimal_sleep_infrastructure_v3) combined with SD-016
  (frontal cue-indexed context retrieval). EXQ-265 proved that the SD-017 SWS/REM
  passes activate and differentiate E1 ContextMemory slots (slot_diversity > 0.05).
  The next question is whether those differentiated slots actually guide context-
  appropriate action selection via SD-016's action_bias pathway.

SCIENTIFIC QUESTION:
  Does periodic sleep consolidation (SD-017) produce ContextMemory slot differentiation
  that, when routed through SD-016's cue_action_proj, yields measurably distinct action
  biases across SAFE vs DANGEROUS contexts after training?

  Specifically: does WITH_SLEEP show higher cosine distance between action_bias vectors
  sampled in SAFE vs DANGEROUS contexts compared to WITHOUT_SLEEP?

  EXP-0106 proposal: "EXQ-242 showed slot differentiation works (4/5 seeds) but agent
  doesn't use context knowledge. Add context->action pathway." This experiment adds
  sd016_enabled=True (the pathway) to the sleep ablation, testing whether sleep-
  differentiated slots actually drive differentiated behavior.

DESIGN:
  Two conditions, 3 seeds each:
    WITH_SLEEP_SD016:    sws_enabled=True, rem_enabled=True, sd016_enabled=True
                         (sleep differentiates slots; SD-016 routes them to action)
    WITHOUT_SLEEP_SD016: sws_enabled=False, rem_enabled=False, sd016_enabled=True
                         (no sleep; undifferentiated slots; SD-016 has nothing to route)

  Both conditions: sd016_enabled=True. This controls for SD-016 pathway presence;
  the only difference is whether sleep differentiates the upstream ContextMemory slots.

  Training:
    P0 (P0_EPISODES): Encoder warmup. Both conditions. Alternating SAFE/DANGEROUS contexts.
    P1 (P1_EPISODES): WITH_SLEEP_SD016 gets a sleep cycle every SLEEP_INTERVAL episodes.
                      WITHOUT_SLEEP_SD016 continues waking-only.
  Evaluation:
    After P0+P1 training, run EVAL_EPISODES evaluation steps.
    In each eval episode, sample action_bias vectors in both SAFE and DANGEROUS contexts.
    Compute context action_bias_divergence = mean cosine distance between
      action_bias(SAFE steps) and action_bias(DANGEROUS steps).

ACCEPTANCE CRITERIA:
  C1: action_bias_divergence in WITH_SLEEP_SD016 >= 0.05 in >= 2/3 seeds
      (Sleep-consolidated slots produce context-differentiating action biases)
  C2: action_bias_divergence(WITH_SLEEP_SD016) > action_bias_divergence(WITHOUT_SLEEP_SD016)
      in >= 2/3 seeds
      (Sleep improves context-conditioned action bias, not just slot metrics)
  C3: slot_diversity(WITH_SLEEP_SD016) > slot_diversity(WITHOUT_SLEEP_SD016)
      in >= 2/3 seeds
      (Sanity check: sleep still differentiates slots, as confirmed by EXQ-265)

PASS: C1 AND C2 (primary chain: sleep -> differentiated slots -> context-differentiated action)
  C3 secondary (replication check for EXQ-265 slot differentiation result).

EVIDENCE DIRECTION:
  PASS -> supports SD-017 (sleep-differentiated slots produce context-appropriate action biases)
  FAIL -> does_not_support SD-017 at this level (slots differentiate but don't guide behavior,
          or sleep doesn't differentiate enough to drive action_bias divergence)
...
EXPERIMENT_TYPE = "v3_exq_418_sd017_context_conditioned_action"
CLAIM_IDS = ["SD-017"]
EXPERIMENT_PURPOSE = "evidence"

P0_EPISODES = 50          # encoder warmup (no sleep in either condition)
P1_EPISODES = 150         # P1: WITH_SLEEP gets sleep cycles; WITHOUT does not
...
CONTEXT_SWITCH_EVERY = 5  # alternate SAFE/DANGEROUS every N episodes during training

LR = 1e-4

SEEDS = [42, 49, 56]

# Minimum number of context-tagged action_bias samples for a valid eval
...
def _make_agent(env: CausalGridWorldV2, with_sleep: bool) -> REEAgent:
    ...
    WITH_SLEEP: sws_enabled + rem_enabled + sd016_enabled.
    WITHOUT_SLEEP: sd016_enabled only (no sleep consolidation).
    Both conditions share sd016_enabled=True to isolate the sleep contribution
    to ContextMemory slot differentiation.
    """
...
def _extract_action_bias(agent: REEAgent, z_world: torch.Tensor) -> torch.Tensor:
    """Extract action_bias from E1 ContextMemory via SD-016 cue projection.

    Returns action_bias [action_object_dim] tensor or None if SD-016 not enabled.
    """
...
def _action_bias_divergence(
    ...
    Computes all pairwise cosine distances between the two sets and returns mean.
    Value in [0, 1]: 0 = identical action biases (no context differentiation),
    1 = fully orthogonal biases (maximum context differentiation).
    """
    if len(safe_biases) < MIN_CONTEXT_SAMPLES or len(dang_biases) < MIN_CONTEXT_SAMPLES:
        return 0.0
    with torch.no_grad():
        safe_mat = torch.stack(safe_biases[:50])   # cap at 50 for memory
        dang_mat = torch.stack(dang_biases[:50])
        safe_norm = F.normalize(safe_mat, dim=-1)
        dang_norm = F.normalize(dang_mat, dim=-1)
        # Mean of all pairwise cosine similarities -> convert to distance
        sim_mat = torch.mm(safe_norm, dang_norm.t())  # [n_safe, n_dang]
...
# ---------------------------------------------------------------------------
# Training episode (random actions, world-model update only)
# ---------------------------------------------------------------------------
...
def _run_eval_episode(
    ...
    env_label: 'SAFE' or 'DANGEROUS' -- determines which bias list to append to.
    Returns harm_rate for this episode.
    """
    device = agent.device
    _, obs_dict = env.reset()
    agent.reset()
    ...
    for _step in range(STEPS_PER_EPISODE):
        ...
        # Collect action_bias from SD-016 cue retrieval
...
def run_condition(
    ...
    eval_eps = EVAL_EPISODES if not dry_run else 4  # even number for SAFE/DANGEROUS balance
    ...
    # Final slot diversity after training
    slot_diversity = _compute_slot_diversity(agent)

    # ---- Evaluation: collect action_bias vectors per context ----
    safe_biases: List[torch.Tensor] = []
    dang_biases: List[torch.Tensor] = []
    eval_harm_rates: List[float] = []

    # Alternate SAFE/DANGEROUS eval episodes
    ...
    # Action bias divergence: how different are biases in SAFE vs DANGEROUS?
...
def main(dry_run: bool = False) -> Dict:
    ...
    c1_wins = 0  # action_bias_div WITH >= 0.05
    c2_wins = 0  # action_bias_div WITH > WITHOUT
    c3_wins = 0  # slot_diversity WITH > WITHOUT
    ...
    for with_r, wo_r in zip(
        ...
        bias_improvement_win = (
            with_r["action_bias_divergence"] > wo_r["action_bias_divergence"]
        )
