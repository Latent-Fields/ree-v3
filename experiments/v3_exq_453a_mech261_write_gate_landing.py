...
"""
V3-EXQ-453a -- MECH-261 per-subdivision write-gate landing validation
               (env.step unpacking bug fix, supersedes V3-EXQ-453).

Supersedes V3-EXQ-453. EXQ-453 ran to completion but every episode
terminated after step 0 because the env.step 5-tuple was unpacked as
...
Result: n_steps_total=6 per seed, argmax_modes_seen={"external_task"}
only, C6 FAIL in both COORD_ON seeds. Unit checks (UC1-UC5) remain
unaffected -- they are synthetic arithmetic tests independent of the
env loop. C7 (COORD_OFF no-op) also unaffected.

EXQ-453a corrects the unpacking and uses `done` as the termination
flag. Same scientific question, same thresholds, same seeds. C6
re-measurement is the sole scientific deliverable.
...
EXPERIMENT_TYPE = "v3_exq_453a_mech261_write_gate_landing"
