"""V3-EXQ-669b -- MECH-329 wanting-before-liking goal-seeding ordering test.

Supersedes V3-EXQ-669a (v3_exq_669a_mech329_wanting_first_goal_seeding), which was
adjudicated non_contributory + substrate_ceiling
(failure_autopsy_V3-EXQ-669a_2026-06-13): anchor_count = total_writes = p01_writes
= 0 in ALL THREE arms INCLUDING the both_delayed positive/sanity control
(c4_delayed_sanity=false), with byte-identical benefit_contacts/harm_events across
arms. Root cause = goal_pipeline:GAP-2: 669a ran the ecological
InfantCurriculumScheduler, which never reaches self-sustaining benefit contact
(z_goal_norm < 0.1), so the MECH-189 SuperOrdinalGoalMemory salience write gate
never fired and the wanting/liking ordering was measured over an EMPTY set. (This
is also why the freeze-at-phase-2 logic made 669a's c4 control unsatisfiable by
construction -- there was no writable window in which z_goal was ever seeded.)

THE HARNESS FIX (same scientific question; identical claim):  re-issue the
three-arm ordering test on the scaffolded_sd054_onboarding Stage-0 FORCED-FEED
nursery pattern (dense, hazard-free safe context; supra-threshold benefit forced
each step), decoupled from the GAP-2 ecological-contact ceiling, exactly as the
MECH-189 substrate's own validation V3-EXQ-588c did. z_goal_enabled=True +
drive_weight=2.0 so the GoalState attractor forms and the super-ordinal write gate
(salience = benefit * (1 + drive_weight*drive) = 0.5*(1+2*0.9) = 1.4 >> 0.5
threshold) actually fires.

MECH-329 (the ordering claim): the wanting system (z_goal seeding / approach
drive) seeds super-ordinal goal anchors via accidental benefit contacts BEFORE the
liking system (benefit_eval / hedonic calibration) is calibrated. This is an
ORDERING claim -- it predicts a DIFFERENCE in the timing/quantity/complexity of
super-ordinal writes between a wanting-first and a liking-first developmental
schedule.
...
non_degenerate=False) -- NEVER a weakens FAIL:
  ...
  R3 (anchor-hierarchy non-degeneracy -- the SAME-STATISTIC guard the load-bearing
     criteria route on): max anchor_count across all arms/seeds >= 2. If every arm
     saturates to a single anchor (merge_similarity collapses the nursery z_world
     contexts), anchor_count and per-context complexity have ZERO cross-arm range
     and C1/C3 cannot differentiate -- that is a substrate-readiness limit (needs a
     more context-diverse nursery / lower merge_similarity), NOT a MECH-329 weakens.
...
+ write timing, not encoder fidelity (mirrors V3-EXQ-588c).
"""
...
EXPERIMENT_TYPE = "v3_exq_669b_mech329_wanting_first_goal_seeding"
...
def run_experiment(n_child: int, n_wean: int, steps: int, seeds: List[int],
    ...
    if not readiness_met:
        ...
        if not r1_writes_fire:
            ...
            degeneracy_reason = (
                ...
                "and C1/C3 cannot differentiate. Substrate not ready -- re-queue with a "
                "more context-diverse nursery or a lower super_ordinal_merge_similarity."
            )
    elif ordering_pass:
        outcome = "PASS"
    ...
    result: Dict[str, Any] = {
        ...
        "supersedes": "v3_exq_669a_mech329_wanting_first_goal_seeding",
