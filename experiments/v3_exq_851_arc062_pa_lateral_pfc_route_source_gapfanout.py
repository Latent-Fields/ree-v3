...
"""
V3-EXQ-851 -- ARC-062 conversion-fanout GOV-FANOUT-1 Leg P-A (H1: selection-authority
coupling gap), TESTING THE CORRECTED ROUTE SOURCE.

DESIGN OF RECORD: REE_assembly/evidence/planning/arc_062_conversion_fanout_2026-07-29.md
Section 2 "Leg P-A". Template: v3_exq_654j (this file's matched-stack base: SP-CEM,
GAP-A e2_world_forward candidate summaries, MECH-448 rank-preserving F->eligibility
demotion, MECH-449 active Go/No-Go opponency, the CRF/LateralPFCAnalog rule-apprehension
stack) MERGED WITH the routing wiring + latch-clearing pattern of v3_exq_791a
(channel_route_bias / modulatory_channel_route_source / per-tick diagnostics-latch
discipline).

ERRATUM THIS SCRIPT FIXES (2026-07-31, /queue-experiment build session for
chip-20260729-arc062-pa-route-gatedpolicy; full write-up: the "Erratum" section of the
design doc above). The design doc as ORIGINALLY WRITTEN specified
modulatory_channel_route_source="gated_policy" as the matched-constant routing config for
this leg, on the premise that "gated_policy" routes "the rule-apprehension channel's OWN
per-candidate range." THAT PREMISE WAS FACTUALLY WRONG:
  - modulatory_channel_route_source=="gated_policy" (ree_core/agent.py) routes _bdc_gp,
    the output of self.gated_policy(...) -- the ARC-062 PHASE-1 GatedPolicy MODULE
    (ree_core/policy/gated_policy.py). That module's own docstring is explicit: "There is
    no connection to SD-033a LateralPFCAnalog in Phase 1 -- that wiring is Phase 3." Its
    forward-pass inputs are z_world / z_self / z_harm_a / per-candidate feature summaries
    / first-action one-hots -- none of which is rule_state or any CandidateRuleField
    output.
  - The bias this leg's swept variable (use_candidate_rule_field) ACTUALLY differentiates
    is LateralPFCAnalog.compute_bias's output, stashed as _bdc_lpfc
    (ree_core/agent.py:~6594). Routing "gated_policy" would have added an unrelated,
    equally-present-on-both-arms bias term and silently retested 654j's own
    (cand_world_summary-routed) mechanism with cosmetic config noise -- NOT what H1 asks.
  - FIX (2026-07-31, ARC-062/MECH-309 P-A substrate chip
    chip-20260731-arc062-pa-lpfc-route-source): added a
    modulatory_channel_route_source=="lateral_pfc" branch to
    REEAgent.select_action's elif chain (ree_core/agent.py), identity-routing _bdc_lpfc,
    mirroring the existing "gated_policy"/"curiosity"/"mech295"/"coherence" branches
    exactly. Contract: tests/contracts/test_e3_score_bias_candidate_support.py::
    test_channel_routing_lateral_pfc_source_agent_level. Documented in ree-v3/CLAUDE.md
    "modulatory-bias-selection-authority AMEND: route upstream-channel range..." and
    ree_core/utils/config.py's REEConfig.use_modulatory_channel_routing docstring.

WHAT THIS LEG TESTS (H1 -- selection-authority coupling gap). 654j's own lineage (654g/
654h/654i/654j, all FAIL non_contributory / substrate_not_ready_requeue) always routed
"cand_world_summary" -- a DIFFERENT channel from the one use_candidate_rule_field
differentiates. So every prior 654-lineage run left open the possibility that the
rule-apprehension channel's OWN cross-candidate range never reached the modulatory bias
the selection authority rescales (it was flattened by the SD-033a-consuming additive
head before reaching E3's route-range authority), and THAT -- not F-dominance (H2) or a
competence gap (H3) -- is why differentiated rule-apprehension bias reaches committed
action (C1d, propagation non-vacuity) but does not convert to committed-class diversity
(C2). This leg closes that gap: with modulatory_channel_route_source="lateral_pfc" now
CORRECTLY routing _bdc_lpfc's own range into _modulatory_accum (identity-routed, per
CLAUDE.md route-range AMEND), a C1-holds/C2-fails outcome cleanly REFUTES H1 as the
ceiling (not merely re-testing a mis-wired proxy) and shifts portfolio weight to H2
(Leg P-B, F-dominance) / H3 (Leg P-C, competence).

CHANGES vs v3_exq_654j (matched-stack base; everything else BYTE-IDENTICAL to 654j --
same P0/P1/P2 phase structure, SEEDS, MIN_SEEDS_FOR_PASS, C1a-f, C2 committed-class
entropy DV, three-branch NO-weakens outcome map):
  1. modulatory_channel_route_source changed from "cand_world_summary" (654j's value --
     routing was ALREADY ON as a matched constant on both arms in 654j, via
     USE_MODULATORY_SELECTION_AUTHORITY=True + USE_MODULATORY_CHANNEL_ROUTING=True; only
     the ROUTED CHANNEL changes here) to "lateral_pfc" -- now routing the SAME channel
     use_candidate_rule_field differentiates, rather than the unrelated
     cand_world_summary channel every prior 654-lineage run used. Swept variable
     UNCHANGED: use_candidate_rule_field (ARM_OFF collapsed legacy rule_state vs ARM_ON
     matured differentiated CRF rule_state).
  2. NEW C1g READINESS GATE (routing readiness; any fail -> substrate_not_ready_requeue,
     NEVER a weakens, exactly like C1a-f): on a majority of seeds in BOTH arms (both
     route as a matched constant, so both must show the gate cleanly, mirroring C1a/b/
     e/f's both-arms pattern) --
       (a) route_range_mean > ROUTE_RANGE_SEED_FLOOR (0.01, the 791a-precedented
           magnitude floor, NOT the raw 1e-6 numerical-noise substrate floor) --
           modulatory_channel_route_range is the RAW cross-candidate range of the
           routed _bdc_lpfc bias (the P0 gate signal per CLAUDE.md route-range AMEND);
       (b) n_p2_fresh_select >= FRESH_SELECT_FLOOR, the cadence-derived (NOT a nominal-
           cadence proxy) worst-case floor = nominal_p2_window_ticks /
           beta_rate_max_steps (the MECH-093-modulated cadence's slowest E3-reselection
           rate; 791a's corrected derivation, replacing the mis-derived e3_steps_per_tick
           -keyed floor 790 used);
       (c) n_p2_fresh_select / n_p2_ticks >= FRESH_SELECT_YIELD_FLOOR (1 /
           beta_rate_max_steps), the SAME cadence bound expressed as a
           truncation-invariant FRACTION of the cell's OBSERVED window (episodes can
           terminate early on `done`, so an absolute count alone conflates "the
           selection loop under-fired" with "this cell had a shorter window").
  3. LATCH-CLEARING (791a pattern; the ~9x pseudo-replication defect V3-EXQ-785/790
     named and 791a's corrected denominator is the reference for): agent.e3.
     last_score_diagnostics is explicitly cleared to None immediately before EVERY
     agent.select_action call. The route-range/route-active readout (item 2 above) is
     collected ONLY on ticks where agent.e3.last_score_diagnostics is populated AFTER
     the select_action call (a GENUINE fresh E3.select() this tick, not a held/latched
     replay of the previous selection) -- n_p2_fresh_select / n_p2_latched_ticks make
     the true denominator auditable, exactly as 791a's n_fresh_select / n_latched_ticks
     do. IMPORTANT: this latch-clearing is scoped ONLY to the new route diagnostic. The
     PRE-EXISTING C1e (MECH-448 demotion) / C1f (MECH-449 No-Go) diagnostic reads are
     deliberately left BIT-IDENTICAL to 654j (they still see the held/latched diagnostics
     snapshot on non-select ticks, exactly as 654j's own already-validated
     DEMOTION_ACTIVE_FRAC_FLOOR / NOGO_ACTIVE_FRAC_FLOOR thresholds were calibrated
     against) -- retrofitting THEIR denominators to a fresh-select basis is out of scope
     for this leg's erratum fix and risks an uncontrolled regression to gates this script
     did not set out to touch (see the P-A build session's own diagnosis before writing
     this file).
  4. C1_holds now ANDs in c1g_holds alongside c1a..c1f (unchanged).
  5. Docstring / EXPERIMENT_TYPE / QUEUE_ID / manifest fields updated for this leg; no
     `supersedes` (this is a DIFFERENT design axis / mechanism-under-test from 654j's
     lineage -- H1 coupling-gap specifically -- not a lettered bug-fix continuation, per
     the fanout portfolio's own framing of Leg P-A as a distinct probe).

DV-SYMMETRY INVARIANCE DECLARATION (MANDATORY, per the 604c net; carried from the design
doc's Leg P-A spec): the routed "lateral_pfc" bias is a per-candidate [K] vector
(identity-routed), NOT a broadcast scalar, so it is NOT invariant under the argmax/softmax
symmetry of the committed-class DV -- consistent with 791a's C2 already showing the
committed-class distribution MOVES on routing-on vs routing-off for a per-candidate
channel. Declared per-arm in the manifest interpretation block.

DECLARED NULL (pre-registered, per the design doc): C1 (including the new C1g) confirms
the lateral_pfc-routed channel's OWN range REACHES and the routing readiness gate clears
on both arms, but C2 (committed-class entropy lift) fails => H1 (selection-authority
coupling gap) is REFUTED as the ceiling; portfolio weight shifts to H2 (Leg P-B,
F-dominance) / H3 (Leg P-C, competence). This is a non_contributory result per the
existing three-branch NO-weakens map (C1 fail -> substrate_not_ready_requeue; C1 holds +
C2 fails -> routed to the next open ARC-107 constitution component, exactly as 654j's own
map already routes a C1-holds/C2-fails outcome -- unchanged here), NOT a falsification.

Claims: [MECH-309, ARC-062]. experiment_purpose = "evidence". Only the PASS branch
(C1 incl. C1g holds AND C2 holds) weights them (as supports); both FAIL branches are
pre-registered non_contributory, matching 654j's own outcome map exactly (C1g failing is
just one more C1 sub-gate feeding the SAME "substrate_not_ready_requeue" branch).

See REE_assembly/evidence/planning/arc_062_conversion_fanout_2026-07-29.md (Section 2,
Leg P-A design + the erratum this script implements),
REE_assembly/evidence/planning/arc_062_rule_apprehension_plan.md (GAP-B),
experiments/v3_exq_654j_arc062_gapb_rule_apprehension_nogo_behavioural_falsifier.py (the
matched-stack template this merges from -- see that file's own docstring for the full
MECH-448/449 + CRF lineage this leg inherits unchanged),
experiments/v3_exq_791a_channel_routing_cross_class_magnitude_replication.py (the routing
wiring + latch-clearing pattern this merges from),
ree-v3/CLAUDE.md "modulatory-bias-selection-authority AMEND: route upstream-channel range
into the bias the authority rescales (569f/661/654a, 2026-06-10)" (route source list, now
including "lateral_pfc"),
ree_core/policy/gated_policy.py (the module docstring that is the erratum's primary
evidence: "no connection to SD-033a LateralPFCAnalog in Phase 1").
"""
...
QUEUE_ID = "V3-EXQ-851"
SUPERSEDES = None  # different design axis (H1 coupling-gap) from 654j's lineage, not a lettered fix
...
# V3-EXQ-851 (Leg P-A erratum fix): routes the SAME channel use_candidate_rule_field
# differentiates (_bdc_lpfc, the SD-033a LateralPFCAnalog rule-apprehension bias), not
# 654j's cand_world_summary. See the module docstring ERRATUM section.
...
# C1(g) ROUTING READINESS (V3-EXQ-851 new gate; any fail -> substrate_not_ready_requeue,
# NEVER a weakens, mirroring C1a-f): matched-constant routing is live on BOTH arms, so
# both must clear this gate on a majority of seeds. ROUTE_RANGE_SEED_FLOOR is the
# 791a-precedented magnitude floor (NOT the raw numerical-noise substrate floor
# MODULATORY_ROUTE_MIN_RANGE_FLOOR above). FRESH_SELECT_FLOOR / FRESH_SELECT_YIELD_FLOOR
# are the 791a-corrected cadence-derived sample-adequacy floors (nominal P2 window ticks
# / beta_rate_max_steps, and the same bound as a truncation-invariant fraction of the
# cell's observed window -- NOT keyed to the nominal e3_steps_per_tick=10, which sits
# above the MECH-093-modulated cadence's worst case).
ROUTE_RANGE_SEED_FLOOR = 0.01
BETA_RATE_MAX_STEPS = 20  # ree_core/heartbeat/clock.py MECH-093 slowest E3-reselection cadence
# nominal P2 window ticks = P2_MEASUREMENT_EPISODES * STEPS_PER_EPISODE = 60 * 200 = 12000
...
def _run_seed_arm(
    ...
    # V3-EXQ-851 C1(g) routing readiness (latch-aware; see the select_action call).
    n_p2_fresh_select = 0
    ...
    for ep in range(total_train_eps):
        ...
        for _step in range(steps_per_episode):
            ...
            # V3-EXQ-851 latch-clearing (791a pattern; the ~9x pseudo-replication defect).
            # Snapshot the PRE-clear value first so the pre-existing C1e/C1f demotion/
            # No-Go reads below stay BIT-IDENTICAL to 654j (they fall back to the held/
            # latched snapshot on non-select ticks, exactly as 654j's own already-
            # validated DEMOTION_ACTIVE_FRAC_FLOOR / NOGO_ACTIVE_FRAC_FLOOR thresholds
            # were calibrated against). Only the NEW route-range readout below is
            # latch-aware (it uses the post-select value directly, so a still-None
            # value there proves this exact tick did NOT run a genuine E3.select()).
            ...
            if is_p2:
                ...
                # BIT-IDENTICAL to 654j: falls back to the held/latched snapshot on a
                # non-select tick, exactly as 654j's own C1e/C1f floors were calibrated
                # against (see the latch-clearing note at the select_action call above).
                diag = _fresh_diag if _fresh_diag is not None else (_prev_diag_snapshot or {})

                # V3-EXQ-851 C1g route-range readout: latch-aware, GENUINE fresh
                # selections only (791a pattern -- avoids the ~9x pseudo-replication
                # defect). n_p2_fresh_select / n_p2_latched_ticks make the true
    ...
    # V3-EXQ-851 C1(g) routing readiness aggregates. Denominator is FRESH selections
    # (n_p2_fresh_select), not P2 env ticks -- both numerator and denominator counted
    # on the same (genuine-selection) event, mirroring 791a's corrected pattern.
    route_range_mean = (
    ...
    return {
        ...
        # ----- C1g routing readiness (V3-EXQ-851 new gate) -----
...
def run_experiment(
    ...
    # C1(g) V3-EXQ-851 ROUTING READINESS -- the lateral_pfc-routed channel's own range
    # is supra-floor AND the fresh-selection sample is adequate (both cadence-derived
    # sub-checks folded into seed_route_ready), on a majority of seeds in BOTH arms
    # (routing is a matched constant on both arms, unlike 791a's single swept arm).
    n_off_route_ready = sum(1 for r in off_rows if r["route_ready"])
    n_on_route_ready = sum(1 for r in on_rows if r["route_ready"])
    c1g_holds = bool(
        n_off_route_ready >= MIN_SEEDS_FOR_PASS and n_on_route_ready >= MIN_SEEDS_FOR_PASS
    )
    ...
    interpretation = {
        ...
        "preconditions": [
            ...
            {
                "name": "lateral_pfc_route_range_supra_floor_and_sample_adequate_both_arms",
                "kind": "readiness",
                "description": (
                    "V3-EXQ-851 (ARC-062 P-A erratum fix): modulatory_channel_route_source="
                    "'lateral_pfc' identity-routes the SD-033a rule-apprehension bias "
                    "(_bdc_lpfc, the SAME channel use_candidate_rule_field differentiates) "
                    "into _modulatory_accum. On a majority of seeds in BOTH arms (routing is "
                    "a matched constant, not swept -- unlike 791a's single ARM_1): (a) the "
                    "routed range's mean clears ROUTE_RANGE_SEED_FLOOR; (b) the GENUINE "
                    "fresh-selection sample (latch-cleared, 791a pattern) clears "
                    "FRESH_SELECT_FLOOR (cadence-derived worst-case: nominal P2 window ticks "
                    "/ beta_rate_max_steps); (c) the fresh-select YIELD clears "
                    "FRESH_SELECT_YIELD_FLOOR (the same cadence bound as a "
                    "truncation-invariant fraction of the observed window). An unrouted or "
                    "under-sampled channel means the C2 committed-class DV cannot be "
                    "attributed to routing => substrate_not_ready_requeue, NEVER a false "
                    "weakens. Mirrors the V3-EXQ-791a route-range + sample-adequacy P0 gates."
                ),
                "control": "modulatory_channel_route_range + n_p2_fresh_select on both arms (matched lever)",
                # COUNT-shaped, INCLUSIVE floor: `met` is c1g_holds, i.e.
                # `n_off_route_ready >= MIN_SEEDS_FOR_PASS and n_on_route_ready >=
                # MIN_SEEDS_FOR_PASS`, and min(counts) >= k iff every count >= k, so this
                # reproduces `met` EXACTLY (same pattern as C1a/b/e/f above -- route_ready
                # is itself a per-seed CONJUNCTION of the three sub-floors; the underlying
                # statistics are preserved below as NON-BOUND diagnostics).
                "measured": float(min(n_off_route_ready, n_on_route_ready)),
                "threshold": float(MIN_SEEDS_FOR_PASS),
                "comparator": ">=",
                "direction": "lower",
                "observed_min_route_range_mean": float(
                    min(
                        [r["modulatory_channel_route_range_mean"] for r in (off_rows + on_rows)]
                        or [0.0]
                    )
                ),
                "observed_route_range_seed_floor": float(ROUTE_RANGE_SEED_FLOOR),
                "observed_min_n_p2_fresh_select": float(
                    min([r["n_p2_fresh_select"] for r in (off_rows + on_rows)] or [0.0])
                ),
                "observed_fresh_select_floor": float(FRESH_SELECT_FLOOR),
                "observed_min_route_sample_yield": float(
                    min([r["route_sample_yield"] for r in (off_rows + on_rows)] or [0.0])
                ),
                "observed_fresh_select_yield_floor": float(FRESH_SELECT_YIELD_FLOOR),
                "met": bool(c1g_holds),
            },
        ],
        "criteria": [
    ...
    return {
        ...
        "decision_rule_thresholds": {
            ...
            "modulatory_channel_route_source": str(MODULATORY_CHANNEL_ROUTE_SOURCE),
            "modulatory_channel_route_weight": float(MODULATORY_CHANNEL_ROUTE_WEIGHT),
            "modulatory_route_min_range_floor": float(MODULATORY_ROUTE_MIN_RANGE_FLOOR),
        ...
        "interpretation_grid": {
            "PASS_C1_C2": (
                "GOV-FANOUT-1 Leg P-A, H1 CONFIRMED: with the rule-apprehension channel's "
                "OWN cross-candidate range (lateral_pfc, _bdc_lpfc) now CORRECTLY routed "
                "into the modulatory-bias-selection-authority accumulator (C1g clears -- "
                "route_range supra-floor, adequate fresh-selection sample, both arms), the "
                "non-Bayesian rule-creator's DIFFERENTIATED + MATURED rule_state propagates "
                "to greater committed-class diversity than the collapsed legacy baseline, "
                "under the same MECH-448/449 matched stack 654j's cand_world_summary-routed "
                "predecessor could not convert with -> closes "
                "behavioral_diversity_isolation:GAP-I via the coupling-gap fix, not the "
                "constitution alone. supports MECH-309 + ARC-062. Route to /governance: "
                "MECH-309 / ARC-062 supports evidence; consider GAP-B + GAP-I closure; the "
                "H2 (Leg P-B) / H3 (Leg P-C) fanout legs become confirmatory rather than "
                "load-bearing."
            ),
            "FAIL_C1_holds_C2_fails": (
                "GOV-FANOUT-1 Leg P-A, H1 REFUTED as the ceiling: C1 fully holds -- class "
                "axis exercisable, GAP-A divergence real, ARM_ON minted distinct rules + "
                "matured + MAINTAINED, propagation non-vacuous (the bias REACHES committed "
                "action), the MECH-448 demotion lever LIVE, the MECH-449 active No-Go LIVE "
                "and SUPPRESSING, AND (NEW, the erratum fix) the lateral_pfc-routed "
                "channel's OWN range clears the route-range + fresh-selection-sample "
                "readiness gate (C1g) on both arms -- so the channel's own range "
                "GENUINELY reached the selection-authority accumulator this time, not an "
                "unrelated proxy channel -- but the differentiated rule_state STILL adds "
                "no marginal committed-class diversity. This cleanly REFUTES H1 "
                "(selection-authority coupling gap) as the ceiling: the coupling gap is "
                "NOT the cause, since routing the correct channel's own range did not "
                "convert. NOT a MECH-309 / ARC-062 falsification (both FAIL branches are "
                "pre-registered non_contributory in the fanout design). non_contributory; "
                "portfolio weight shifts to H2 (Leg P-B, F-dominance) / H3 (Leg P-C, "
                "competence). Do NOT weaken the claims."
            ),
            "FAIL_C1_substrate_not_ready_requeue": (
                ...
                "gng_perseveration_floor), and/or (NEW C1g) the lateral_pfc-routed range "
                "or the fresh-selection sample was inadequate on either arm -- the H1 test "
                "could not be run cleanly this cycle. The falsifier could not run -- NOT a "
...
def _build_manifest(
    ...
    return {
        ...
        "evidence_direction_note": (
            ...
            f"Section 2. Matched-stack base identical to V3-EXQ-654j (SP-CEM, GAP-A "
            f"e2_world_forward candidate summaries, MECH-448 rank-preserving F->eligibility "
            f"demotion, MECH-449 active Go/No-Go opponency, the CRF/LateralPFCAnalog "
            f"rule-apprehension stack; SD-033a bias head un-zeroed AND TRAINED in a "
            f"frozen-encoder P1 REINFORCE window). THE ONE CHANGE vs 654j: "
            f"modulatory_channel_route_source changed from 654j's 'cand_world_summary' to "
            f"'lateral_pfc' -- now routing the SAME channel use_candidate_rule_field "
            f"differentiates (_bdc_lpfc), not an unrelated channel. This corrects an "
            f"ERRATUM in the design doc's original Leg P-A spec, which had specified "
            f"'gated_policy' on the false premise that it routes the rule-apprehension "
            f"channel's own range; 'gated_policy' actually routes the unrelated ARC-062 "
            f"Phase-1 GatedPolicy module's bias (no connection to LateralPFCAnalog/"
            f"CandidateRuleField -- see that module's own docstring). The substrate fix "
            f"(a new modulatory_channel_route_source=='lateral_pfc' branch in "
            f"REEAgent.select_action, identity-routing _bdc_lpfc) landed ree-v3 as chip "
            f"chip-20260731-arc062-pa-lpfc-route-source before this script was authored. "
            f"Swept variable UNCHANGED: use_candidate_rule_field (ARM_OFF legacy collapsed "
            f"rule_state vs ARM_ON matured+active differentiated crf_source). PRIMARY DV = "
            f"COMMITTED-CLASS entropy. C1 (non-vacuity, mirrors 654j C1a-f exactly) = class "
            f"axis exercisable AND GAP-A divergence (both arms) AND ARM_ON matured "
            f"(crf_frac_active >= {CRF_FRAC_ACTIVE_FLOOR}) AND propagation non-vacuous AND "
            f"the MECH-448 demotion lever LIVE (active >= {DEMOTION_ACTIVE_FRAC_FLOOR} of P2 "
            f"ticks AND excluded_count > {EXCLUDED_COUNT_FLOOR}, both arms) AND the MECH-449 "
            f"active No-Go LIVE and SUPPRESSING (active >= {NOGO_ACTIVE_FRAC_FLOOR} of P2 "
            f"ticks AND mean per-tick safety+soft suppressions > {NOGO_SUPPRESSED_FLOOR}, "
            f"both arms) AND (NEW C1g) the lateral_pfc-routed range clears "
            f"{ROUTE_RANGE_SEED_FLOOR} AND the latch-cleared fresh-selection sample clears "
            f"{FRESH_SELECT_FLOOR} (yield >= {FRESH_SELECT_YIELD_FLOOR}), both arms. C2 "
            f"(PRIMARY) = paired-by-seed ARM_ON > ARM_OFF committed-class entropy "
            ...
            f"branch weights MECH-309/ARC-062 (as supports); C1-fail (including the new C1g) "
            f"self-routes substrate_not_ready_requeue AND C1-holds-C2-fail self-routes "
            f"conversion_ceiling_persists_despite_active_nogo -- H1 (selection-authority "
            f"coupling gap) is REFUTED as the ceiling, portfolio weight shifts to H2 (Leg "
            f"P-B, F-dominance) / H3 (Leg P-C, competence) -- BOTH non_contributory, NEITHER "
            f"a falsification (NO weakens branch, matching 654j's own outcome map exactly). "
        ...
        "config_summary": {
            ...
            "active_nogo_engagement": "inherited unchanged from 654j: engages the MECH-449 active No-Go (the demotion-alone 654i conversion ceiling's named off-ramp); the perseveration No-Go is fed ecologically by the MECH-260 recency-share vector (use_dacc=True), NOT synthetic injection",
            "lateral_pfc_route_source_fix": "ARC-062/MECH-309 P-A erratum fix (2026-07-31): modulatory_channel_route_source='lateral_pfc' identity-routes _bdc_lpfc (the SD-033a rule-apprehension bias use_candidate_rule_field differentiates), replacing 654j's cand_world_summary route -- the ONLY substantive change vs 654j",
            "z_goal_enabled": True,
            "drive_weight": 2.0,
            "alpha_world": 0.9,
            "reef_enabled": True,
...
def main() -> Tuple[Optional[str], Optional[str], bool]:
    parser = argparse.ArgumentParser(description="V3-EXQ-851 ARC-062 conversion-fanout Leg P-A (H1 coupling-gap; lateral_pfc route source, MECH-449 active Go/No-Go constitution + MECH-448 demotion w/ channel-adaptive floor)")
