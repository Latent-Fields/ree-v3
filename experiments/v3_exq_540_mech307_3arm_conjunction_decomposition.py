...
"""V3-EXQ-540 -- MECH-307 anticipatory affect: 3-arm conjunction decomposition.
...
ARM_0_off (replicates EXQ-539 ARM_OFF substrate):
  - All four MECH-307 substrate flags False.
...
ARM_1_signed_pe (Gap 1 only):
  - use_mech307_signed_pe = True
  - Gaps 2/3/4 unfixed.
  - Bridge consumer-conjunction-read flag ON.
  - Predicted: signed-surprise counter > 0 (negative VALENCE_SURPRISE
    centers accumulate on harm contacts). VALENCE_LIKING / z_beta-pulse
...
ARM_2_full_conjunction (all four gaps):
  - use_mech307_signed_pe = True
...
C1 (substrate-readiness counter dissociation, per-gap):
   ARM_0: liking_writes == 0, predicted_writes == 0, neg_surprise_centers == 0,
          z_beta_excursion ~ 0.
   ARM_1: neg_surprise_centers > 0; liking_writes == 0; predicted_writes == 0;
          z_beta_excursion ~ 0. (Gap 1 isolated.)
   ARM_2: all four signatures fire (liking_writes > 0, predicted_writes > 0,
          neg_surprise_centers > 0, z_beta_excursion >= 0.05).
...
Smoke
-----
  /opt/local/bin/python3 experiments/v3_exq_540_mech307_3arm_conjunction_decomposition.py --dry-run
...
EXPERIMENT_TYPE = "v3_exq_540_mech307_3arm_conjunction_decomposition"
...
def _evaluate_acceptance(agg: Dict[str, Dict]) -> Dict:
    arm0 = agg["ARM_0_off"]
    arm1 = agg["ARM_1_signed_pe"]
    arm2 = agg["ARM_2_full"]

    # C1 substrate-readiness counter dissociation.
    # Gap 1 marker: n_negative_surprise_centers (signed VALENCE_SURPRISE writes).
    ...
    c1_arm0 = (
        ...
        and arm0["n_negative_surprise_centers_total"] == 0
        and arm0["z_beta_excursion_mean"] < C1_Z_BETA_FLOOR_ARM2
    )
    c1_arm1 = (
        arm1["n_negative_surprise_centers_total"] >= C1_NEG_SURPRISE_FLOOR_ARM12
    ...
    c1_arm2 = (
        ...
        and arm2["n_negative_surprise_centers_total"] >= C1_NEG_SURPRISE_FLOOR_ARM12
    ...
    return {
        ...
        "C1_arm1_signed_pe_only": bool(c1_arm1),
...
def main(dry_run: bool = False):
    ...
    for seed in seeds:
        for arm_cfg in ARMS:
            ...
            print(
                ...
                f"neg_surp_centers={r['n_negative_surprise_centers']} "
    ...
    for arm in ("ARM_0_off", "ARM_1_signed_pe", "ARM_2_full"):
        a = agg[arm]
        print(
            ...
            f"neg_surp={a['n_negative_surprise_centers_total']} "
    ...
    manifest = {
        ...
        "note": (
            "3-arm gap-decomposition successor to V3-EXQ-539 (which FAILed C5 "
            "approach_commit lift despite C1-C4 substrate counter PASS). This "
            "experiment introduces the consumer-side fix (Path B): the MECH-295 "
            "liking bridge's compute_conjunction_score_bias() reads the four-way "
            "conjunction at predicted-imminent locations and adds an additional "
            "approach bias when it holds. ARM_3 (SD-014 6-channel fallback) "
            "remains deferred -- if this experiment PARTIAL-PASSes (C1+C2 only), "
            "ARM_3 becomes the natural follow-on."
        ),
    }
