...
"""
V3-EXQ-166c -- SD-003 Harm Latent Forward Model: Shuffled-Action Ablation

Claims: SD-003, ARC-033, SD-011
Supersedes: EXQ-166b (random-weights ablation -- criterion design flaw)

Root cause of EXQ-166b C2 failure (criterion flaw, not architecture):
  EXQ-166b TRAINED: latent_fwd_r2=0.62, C3/C4 PASS -- forward model IS learning.
  EXQ-166b ABLATED: random-weights model maps z_harm_t -> random z_harm space.
  harm_eval was trained on in-distribution z_harm. Random z_harm is out-of-
  distribution -> harm_eval outputs near-zero -> causal_sig_ablated = actual - ~0
  = large positive at ALL events. TRAINED causal_sig is small positive because
  mean(cf) is meaningful (in-distribution). Result: delta = TRAINED - ABLATED < 0.

  C3/C4 already showed the trained model discriminates correctly by event type.
  The architecture is working. The comparison baseline was wrong.

This experiment -- shuffled-action ablation:
  Replace random-weights ABLATED with SHUFFLED: a second forward model trained on
  the same buffer, same number of gradient steps, but with ACTIONS RANDOMLY
  PERMUTED within each training batch. The shuffled model learns z_harm dynamics
  (so it is in-distribution for harm_eval) but has NO action-conditional information.

  New primary test (C2): Does the TRAINED model predict higher harm for the
  APPROACH action than for RETREAT actions (direct forward model discrimination)?
    fwd_discrimination = harm_eval(fwd_trained(z_harm_t, a_approach))
                       - mean(harm_eval(fwd_trained(z_harm_t, a_cf != a_approach)))
  At approach events, a_approach leads toward the hazard -> higher z_harm_next.
  a_retreat leads away -> lower z_harm_next. If trained, this gap > 0.

  New ablation control (C3): TRAINED discrimination > SHUFFLED discrimination.
  SHUFFLED model has no action-conditional info -> its predictions are independent
  of which action is approach vs retreat -> discrimination ~ 0.

  C4 (causal ordering) retained from EXQ-166b: original causal_sig approach > none.

Architecture (identical to EXQ-166b):
  P0: HarmEncoder as autoencoder + scalar harm regression
  P1: HarmForwardModel (TRAINED) + HarmForwardModel (SHUFFLED) + HarmDecoder
      TRAINED: normal (z_harm_t, action, z_harm_next)
      SHUFFLED: same loss, same batches, but actions randomly permuted in-batch
  P2: E3 harm head calibration (stratified, frozen encoder)
  P3: Direct action-discrimination eval (new criterion design)

PRE-REGISTERED ACCEPTANCE CRITERIA:
  C1 (latent fwd R2, gate): latent_fwd_r2_mean > 0.30
    Forward model learned harm dynamics.
  C2 (approach discrimination): fwd_discrimination_trained_mean > 0.01
    TRAINED model predicts higher harm for approach actions than retreat actions,
    averaged across approach events and seeds. Direct test of action-conditional
    causal knowledge.
  C3 (ablation control): fwd_discrimination_trained_mean > fwd_discrimination_shuffled_mean + 0.005
    TRAINED has more action-conditional knowledge than the shuffled baseline.
    Confirms the learned signal is specifically action-conditional, not a
    trivial function of z_harm_t.
  C4 (causal ordering): causal_approach_trained_mean > causal_none_trained_mean
    Retain from EXQ-166b: trained causal signal correctly orders approach > neutral.
    PASS in EXQ-166b; should remain PASS.
  C5 (data quality): n_approach_eval_mean > 20
  C6 (seed consistency): fwd_discrimination_trained > 0 for >=3/4 seeds
...
EXPERIMENT_TYPE = "v3_exq_166c_sd003_harm_latent_shuffled_ablation"
CLAIM_IDS = ["SD-003", "ARC-033", "SD-011"]

# Pre-registered thresholds
THRESH_C1_LATENT_FWD_R2         = 0.30
THRESH_C2_FWD_DISCRIMINATION    = 0.01   # approach - retreat harm prediction gap (trained)
THRESH_C3_ABLATION_MARGIN       = 0.005  # trained discrimination > shuffled discrimination
THRESH_C4_CAUSAL_ORDERING       = 0.0    # causal_approach > causal_none
...
def _run_single(
    ...
    # TRAINED forward model + decoder (shared decoder: reconstruction loss uses trained model)
    opt_fwd_trained  = optim.Adam(
        list(harm_fwd_trained.parameters()) + list(harm_dec_p1.parameters()), lr=1e-3)
    # SHUFFLED forward model (no decoder -- not supervised on reconstruction, only latent pred)
    ...
    print(f"[EXQ-166c seed={seed}] P0: HarmEncoder autoencoder ({phase0_episodes} eps)...", flush=True)
    ...
    print(f"[EXQ-166c seed={seed}] P1: TRAINED + SHUFFLED forward models ({phase1_episodes} eps)...", flush=True)
    ...
    for ep in range(phase1_episodes):
        ...
        for _ in range(steps_per_episode):
            ...
            if len(valid) >= 32:
                ...
                # SHUFFLED: same latent prediction loss, but actions randomly permuted in batch
    ...
    print(f"[EXQ-166c seed={seed}] P2: E3 harm head calibration ({phase2_episodes} eps)...", flush=True)
    ...
    # ---- Phase 3: Action-discrimination eval (new design) --------------------------
    print(f"[EXQ-166c seed={seed}] P3: action-discrimination eval ({eval_episodes} eps)...", flush=True)
    agent.eval(); harm_enc.eval(); harm_fwd_trained.eval(); harm_fwd_shuffled.eval()

    # Per-event: approach-vs-retreat harm prediction gap (trained and shuffled)
    fwd_disc_trained_approach:  List[float] = []   # approach events
    fwd_disc_shuffled_approach: List[float] = []
    # Original causal_sig for C4 ordering check
    ...
    for ep in range(eval_episodes):
        ...
        for _ in range(steps_per_episode):
            ...
            with torch.no_grad():
                ...
                # C4 causal_sig (for ordering test -- same as EXQ-166b)
                cf_vals = []
                ...
                # C2/C3: approach-vs-retreat harm prediction gap
                # Approach action: the one toward the hazard (a_approach_idx)
                # Retreat: mean over all OTHER actions
                ...
                harm_approach_t     = float(agent.e3.harm_eval_z_harm(z_approach_trained).item())
                retreat_vals_t = [float(agent.e3.harm_eval_z_harm(
                    harm_fwd_trained(z_harm_t, _action_to_onehot(i, n_actions, agent.device))
                ).item()) for i in retreat_indices]
                disc_trained = harm_approach_t - float(np.mean(retreat_vals_t))

                # SHUFFLED discrimination
                z_approach_shuffled = harm_fwd_shuffled(z_harm_t, a_approach_oh)
                harm_approach_s     = float(agent.e3.harm_eval_z_harm(z_approach_shuffled).item())
                retreat_vals_s = [float(agent.e3.harm_eval_z_harm(
                    harm_fwd_shuffled(z_harm_t, _action_to_onehot(i, n_actions, agent.device))
                ).item()) for i in retreat_indices]
                disc_shuffled = harm_approach_s - float(np.mean(retreat_vals_s))

                # Collect at ALL timesteps (not just approach events):
                # discrimination at approach events should be highest
                if bucket == "approach":
                    fwd_disc_trained_approach.append(disc_trained)
                    fwd_disc_shuffled_approach.append(disc_shuffled)

            if done: break

    def _m(lst): return float(np.mean(lst)) if lst else 0.0

    fwd_discrimination_trained  = _m(fwd_disc_trained_approach)
    fwd_discrimination_shuffled = _m(fwd_disc_shuffled_approach)
    ...
    n_approach_eval             = len(fwd_disc_trained_approach)

    print(
        f"  [seed={seed}] latent_fwd_r2={latent_fwd_r2:.4f}"
        f"  fwd_disc_trained={fwd_discrimination_trained:.4f}"
        f"  fwd_disc_shuffled={fwd_discrimination_shuffled:.4f}"
    ...
    return {
        ...
        "fwd_discrimination_trained":  fwd_discrimination_trained,
        "fwd_discrimination_shuffled": fwd_discrimination_shuffled,
...
def main(dry_run: bool = False):
    ...
    fwd_discrimination_trained   = _mean("fwd_discrimination_trained")
    fwd_discrimination_shuffled  = _mean("fwd_discrimination_shuffled")
    causal_approach_trained_mean = _mean("causal_approach_trained")
    causal_none_trained_mean     = _mean("causal_none_trained")
    causal_contact_trained_mean  = _mean("causal_contact_trained")
    n_approach_eval_mean         = _mean("n_approach_eval")

    disc_per_seed = {s: results_per_seed[s]["fwd_discrimination_trained"] for s in SEEDS}

    c1 = latent_fwd_r2_mean          > THRESH_C1_LATENT_FWD_R2
    c2 = fwd_discrimination_trained  > THRESH_C2_FWD_DISCRIMINATION
    c3 = (fwd_discrimination_trained - fwd_discrimination_shuffled) > THRESH_C3_ABLATION_MARGIN
    c4 = causal_approach_trained_mean > causal_none_trained_mean + THRESH_C4_CAUSAL_ORDERING
    c5 = n_approach_eval_mean         > THRESH_C5_MIN_APPROACH_EVT
    n_positive = sum(1 for s in SEEDS if disc_per_seed[s] > 0.0)
    ...
    print(f"\n=== EXQ-166c RESULTS ===", flush=True)
    print(f"  latent_fwd_r2_mean            = {latent_fwd_r2_mean:.4f}  (C1 >{THRESH_C1_LATENT_FWD_R2}: {'PASS' if c1 else 'FAIL'})", flush=True)
    print(f"  fwd_discrimination_trained    = {fwd_discrimination_trained:.4f}  (C2 >{THRESH_C2_FWD_DISCRIMINATION}: {'PASS' if c2 else 'FAIL'})", flush=True)
    print(f"  disc_trained - disc_shuffled  = {fwd_discrimination_trained - fwd_discrimination_shuffled:.4f}  (C3 >{THRESH_C3_ABLATION_MARGIN}: {'PASS' if c3 else 'FAIL'})", flush=True)
    print(f"  causal_approach > causal_none = {causal_approach_trained_mean:.4f} > {causal_none_trained_mean:.4f}  (C4: {'PASS' if c4 else 'FAIL'})", flush=True)
    ...
    flat = {
        ...
        "total_criteria": 6,
        "dry_run": dry_run,
        "supersedes": "v3_exq_166b_sd003_harm_latent_reconstruction",
        "notes": (
            "EXQ-166c: shuffled-action ablation. Fixes EXQ-166b C2 criterion flaw "
            "(random-weights ABLATED produced out-of-distribution z_harm -> harm_eval "
            "near-zero -> inflated causal_sig_ablated). SHUFFLED model trains on same "
            "data with actions permuted per batch -- learns z_harm dynamics but no "
            "action-conditional info. New C2: trained model predicts higher harm for "
            "approach action than retreat actions (direct forward model discrimination). "
            "Architecture identical to EXQ-166b (HarmForwardModel + HarmDecoder reconstruction branch)."
        ),
        "metrics": {
            "latent_fwd_r2_mean":            latent_fwd_r2_mean,
            "fwd_discrimination_trained":    fwd_discrimination_trained,
            "fwd_discrimination_shuffled":   fwd_discrimination_shuffled,
            "ablation_margin":               fwd_discrimination_trained - fwd_discrimination_shuffled,
