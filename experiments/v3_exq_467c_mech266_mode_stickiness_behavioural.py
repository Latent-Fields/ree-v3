"""
V3-EXQ-467c (EXP-0163 behavioural): MECH-266 mode stickiness / Hold-decay
...
loop the agent never switched modes (n_switches=0). 467b was built on the
committed_mode_curriculum, which trains commitment but NOT foraging competence, so the
SalienceCoordinator never received the varying signals that drive mode switches. This
rewires the TRAINING harness to the scaffolded_sd054_onboarding curriculum at the 603n
config (the 514n pattern; ready=true 2026-06-11), the substrate that DELIVERS the
foraging competence + active goal/drive dynamics that drive mode switching.

WHAT THIS MEASURES (unchanged from 467b): the MECH-266 hysteresis dose-response. Sweep
...
MODE-SWITCHING NON-VACUITY GATE (the exact n_switches=0 gap the *b cohort scored):
  before scoring C1/C2, assert the highest-r (loosest-rail) arm actually switched modes
...
PRE-REGISTERED ACCEPTANCE (constants; PASS = majority 2/3 guard seeds):
  ...
  C3  n_switches(r=2.00) >= C3_MIN_SWITCHES (the mode-switching non-vacuity)
...
predecessor: V3-EXQ-467b (NOT a supersede).
...
EXPERIMENT_TYPE = "v3_exq_467c_mech266_mode_stickiness_behavioural"
QUEUE_ID = "V3-EXQ-467c"
CLAIM_IDS: List[str] = ["MECH-266", "SD-032a"]
EXPERIMENT_PURPOSE = "evidence"
PREDECESSOR = "V3-EXQ-467b (successor, NOT supersede)"
...
def _aborted_seed_record(seed: int, stage: str, reason: str) -> Dict[str, Any]:
    empty_conds = [
        {"ratio": r, "mean_dwell": 0.0, "n_switches": 0, "n_runs": 0,
...
def _run_seed(seed: int, dry_run: bool, total_eps: int,
    ...
    print(f"  [train] dwell_sweep seed={seed} ep {done}/{total_eps}"
          f" c1={c1} c2={c2} c3={c3} dwells={dwells}"
          f" high_r_switches={condition_results[-1]['n_switches']}", flush=True)
        ...
        "interpretation": {
            ...
            "preconditions": [
                ...
                },
                {
                    ...
                    "description": "the highest-r (loosest-rail) arm switched modes (n_switches >= "
                                   "C3_MIN_SWITCHES) on >= 2/3 guard-passing seeds -- the exact "
                                   "n_switches=0 gap the *b cohort scored.",
                    "control": "fraction of guard-passing seeds with high-r-arm switches.",
                    "measured": sw_frac,
                    "threshold": MIN_FRACTION,
            ...
            "criteria": [
                ...
                {"name": "C3_high_r_switches_nonvacuity", "load_bearing": True,
            ...
            "mode_switching_non_vacuity_gate": {
                "definition": "high-r (r=2.00) arm n_switches >= C3_MIN_SWITCHES on >= 2/3 "
...
def main(dry_run: bool = False,
    ...
    manifest = {
        ...
        "substrate": "scaffolded_sd054_onboarding (full curriculum; 603n config; ready=true "
                     ...
                     "ratio sweep + GAP-3 dual_cue env primitive. use_closure_operator OFF "
                     "(closure would inject a confounding mode-switch signal).",
        "condition": CONDITION_LABEL,
        "predecessor": PREDECESSOR,
        "method_note": "467b's MECH-266 mode-stickiness dose-response (a lower uniform exit rail "
                       "-> longer dwell + fewer switches; mean dwell monotone non-increasing in r) "
                       "re-run on a foraging-competent agent BUILT THROUGH the "
                       "scaffolded_sd054_onboarding curriculum. 467b's committed_mode_curriculum "
                       "trained commitment but NOT foraging competence, so the coordinator never "
                       "received the varying signals that drive mode switches (n_switches=0).",
        "readiness_note": "TWO non-vacuity gates self-route a substrate-not-engaged read to "
                          ...
                          "mode-switching non-vacuity gate (the loosest-rail arm actually switched "
                          "modes).",
