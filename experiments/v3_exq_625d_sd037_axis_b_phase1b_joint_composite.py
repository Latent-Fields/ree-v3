"""
V3-EXQ-625d -- SD-037 axis (b) Phase 1b JOINT-COMPOSITE substrate-readiness
falsifier (successor to V3-EXQ-625c; supersedes it).
...
ON 625d PASS: the P1b gate clears and the chain P2 (deterministic p70 recalibration)
...
supersedes: V3-EXQ-625c (the warmup_train-base-policy dynamic-crossings measurement that
  locked monostrategy; 625d replaces the base policy with the scaffold-trained joint
  composite + 569i-validated conversion config and pre-registers the non-vacuity gates).
"""
...
EXPERIMENT_TYPE = "v3_exq_625d_sd037_axis_b_phase1b_joint_composite"
QUEUE_ID = "V3-EXQ-625d"
CLAIM_IDS: List[str] = []  # substrate readiness; tags no claim
EXPERIMENT_PURPOSE = "diagnostic"
SUPERSEDES = "v3_exq_625c_sd037_axis_b_phase1b_dynamic_crossings_mech341"
...
# --- Axis-(b) sustained-threat env overlay (SD-029; plan section 3.1). ---
AXIS_B_INJECT_INTERVAL = 20
AXIS_B_INJECT_PROB = 0.7
AXIS_B_HAZARD_HARM = 0.2
AXIS_B_PROXIMITY_HARM_SCALE = 0.2
...
def main(dry_run: bool = False) -> Dict[str, Any]:
    ...
    manifest = {
        ...
        "design_note": "JOINT-COMPOSITE Phase-1b: scaffold-trained competent policy (603n full "
                       "curriculum) + 569i-validated conversion config (top_k shortlist k=3 + "
                       "authority gain=2.0 std-basis + e2_world_forward source) + MECH-341 + "
                       "SD-056 (online on axis-(b)) measured for z_harm_a dynamic crossings under "
                       "the SD-029 sustained-threat overlay. Supersedes 625c (warmup_train base "
                       "policy that monostrategy-locked). PASS = C3 dynamic crossings >=2/3 with "
                       "all four non-vacuity preconditions met. A non-vacuity miss self-routes "
                       "substrate_not_ready_requeue (NEVER a weakens; claim_ids=[]).",
        "axis_b_env_overlay": {
