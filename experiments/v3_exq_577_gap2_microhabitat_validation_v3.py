"""
V3-EXQ-577: GAP-2 microhabitat zones env feature substrate validation.

infant_substrate:GAP-2 -- Voronoi-seeded per-episode zone map with per-zone
resource/hazard spawn weighting + zone-C ambient presence bonus in
CausalGridWorldV2.

ARM_0_disabled: microhabitat_enabled=False -- legacy behaviour, bit-identical
                to a plain CausalGridWorldV2 over a fixed action stream.
...
evidence):
  C1  ARM_0 bit-identical OFF: (reward, agent_x, agent_y, done) sequence over a
      ...
      seed.
  C2  ARM_1 zone map covers every interior cell; base zones {0,1,2} all present
      and border zone 3 present -- every seed.
  C3  ARM_1 aggregated over resets: zone-C hazard count == 0 (zone_C_hazard_factor
      =0.0 hard guarantee) AND zone-B hazard density > zone-A hazard density --
      every seed.
  C4  ARM_1 zone-C ambient bonus fires and decays: on a forced all-zone-C probe
      env (use_proxy_fields=False) the first zone-C entry yields
      bonus * decay^0 and the second yields bonus * decay^1 -- every seed.
...
claim_ids: [] (env feature validation only; mirrors V3-EXQ-576 GAP-1 precedent.
...
def _check_bit_identical_off(seed):
    ...
    a legacy CausalGridWorldV2 over a fixed action stream."""
    ...
    for seed in SEEDS:
        ...
        for cond_label, mh_enabled in CONDITIONS:
            ...
            if not mh_enabled:
                ...
                # ARM_1: C2 coverage, C3 density, C4 ambient decay.
                env = CausalGridWorldV2(
                    size=SIZE, seed=seed, num_hazards=N_HAZARDS,
                ...
                for ep in range(n_episodes):
                    ...
                    if (interior == -1).any() or not {0, 1, 2}.issubset(codes) \
                            or not codes.issubset({0, 1, 2, 3}) or 3 not in codes:
                        c2_ok = False
                    for z in range(4):
...
if __name__ == "__main__":
    ...
    print("V3-EXQ-577 GAP-2 microhabitat zones validation", flush=True)
    ...
    run_id = f"v3_exq_577_gap2_microhabitat_validation_{timestamp}_v3"
    ...
    manifest = {
        ...
        "acceptance_checks": {
            ...
            "C2_arm1_zone_map_coverage": result["c2_pass"],
    ...
    print(f"  C2 (ARM_1 zone map coverage): {result['c2_pass']}", flush=True)
    print(f"  C3 (ARM_1 zone-C zero hazard + B>A): {result['c3_pass']}", flush=True)
    print(f"  C4 (ARM_1 zone-C ambient decay): {result['c4_pass']}", flush=True)
