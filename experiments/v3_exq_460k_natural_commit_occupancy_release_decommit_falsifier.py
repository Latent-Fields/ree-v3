"""
V3-EXQ-460k (SUPERSEDES V3-EXQ-460j): SD-034 closure de-commit falsifier on the rung-6
COMMIT/RELEASE-DURATION lever -- the graded natural-commit-occupancy release
(ree_core/policy/natural_commit_urgency.py, NaturalCommitUrgencyRelease) -- now on the
now-BUILT CLOSURE-EXCLUSIVE DE-COMMIT EVAL substrate (closure_exclusive_decommit_eval,
landed ree-v3 main e52158d 2026-06-22) so the OFF baseline sustains a natural-commit beta
latch via the CLOSURE PLANE, independently of the fragile F-driven natural commit.

WHY 460k SUPERSEDES 460j (failure_autopsy_V3-EXQ-460j_2026-06-21, user-adjudicated
"Park + amend, name the substrate" -> /implement-substrate commitment_closure:GAP-4 rung-6
BUILD):
  460j armed the natural-commit LATCH-HOLD in all arms, but the hold arms ONLY on a
  DECISIVE NATURAL commit (result.committed), which does NOT form on the full
  closure-coupling-amend substrate. 460j ARM_LEVER_OFF, 3/3 seeds: ncl_hold_reassert_total=0
  (the latch-hold NEVER armed), max_consecutive_beta_run=1 + mean_per_commit_hold ~1.0 (no
  occupancy to shorten), sd034_n_closure_commit_intent=0 (MECH-445 certifier silent) ->
  off_baseline_sustained_natural_commit_hold 0/3 -> substrate_not_ready_requeue. natural-commit
  and closure-de-commit were NON-DISSOCIABLE (the 460h disjoint-certifier problem persisted).
  THE BUILT SUBSTRATE (closure_exclusive_decommit_eval, no-op-default flag; bit-identical OFF):
    (a) closure-EXCLUSIVE beta elevation: _commit_for_beta = _closure_commit_active ONLY
        (the fragile F-driven result.committed path is SUPPRESSED from beta elevation), so the
        occupancy is provably closure-formed.
    (b) closure-coupled latch-hold ARM: the natural-commit latch-hold also arms on
        _closure_commit_active (guarded by beta_gate.refractory_remaining==0 so it yields to
        the SD-034 closure de-commit). The closure commitment persists across ticks (unlike
        the fragile result.committed), so the hold's existing re-assertion sustains the
        occupancy RELIABLY on all seeds.
  So a sustained natural-commit occupancy forms via the closure plane independently of the
  fragile natural commit, and the SD-034 closure de-commit acts on it -- natural-commit and
  closure-de-commit are now DISSOCIABLE (the 460h disjoint-certifier problem dissolves).
  This run sets closure_exclusive_decommit_eval=True on EVERY arm (its preconditions
  use_closure_commit_beta_coupling=True + use_natural_commit_latch_hold=True are both already
  set on this substrate) and re-states the gates: a NEW substrate-readiness gate certifies the
  eval mode ACTUALLY armed the closure-coupled hold in ARM_LEVER_OFF
  (ncl_hold_closure_armed_total / ncl_hold_reassert_total > 0 -- the counters 460j had at 0),
  THEN gate 3 certifies the OFF baseline SUSTAINS the occupancy (mean per-commit hold above
  floor), THEN gate 4 certifies the rung-6 release SHORTENS it. Any unmet readiness gate
  self-routes substrate_not_ready_requeue -- NEVER a false MECH-445/446 weakening.

(Original rung-6 lever, landed ree-v3 main ab2c1a9 2026-06-20; latch-hold amend 2026-06-21;
closure-exclusive de-commit eval mode, ree-v3 main e52158d 2026-06-22.)
...
self-routes substrate_not_ready_requeue -- NEVER a false weakens):
  ...
  (2.5) CLOSURE-EXCLUSIVE EVAL ARMED THE HOLD (the V3-EXQ-460k NEW gate): on ARM_LEVER_OFF
      the closure-exclusive de-commit eval mode actually armed the closure-coupled
      natural-commit latch-hold -- ncl_hold_closure_armed_total > 0 (the closure plane
      armed the hold) AND ncl_hold_reassert_total > 0 (the hold re-asserted the latch) on
      >= 2/3 guard seeds. This is the direct "the BUILT substrate engaged" check: 460j had
      BOTH counters at 0 (the hold armed only on result.committed, which never formed). If
      they stay ~0 the eval mode did not produce the dissociable occupancy -> substrate_not_
      ready_requeue, NEVER a false MECH-445/446 weakening.
  (3) OFF-BASELINE SUSTAINED NATURAL-COMMIT HOLD (the V3-EXQ-460j gate-3 REDESIGN,
      ...
      SUSTAINED_HOLD_MEAN_FLOOR on >= 2/3 guard seeds (the closure-exclusive eval mode now
      establishes the 460h monolithic-hold regime via the closure plane; 460j OFF was ~1.0
      and FAILED this -> substrate_not_ready_requeue, as intended). max_consecutive_beta_run
      + ncl_hold_reassert_total reported. SAME statistic the 460i proxy was blind to.
...
EXPERIMENT_TYPE = "v3_exq_460k_natural_commit_occupancy_release_decommit_falsifier"
QUEUE_ID = "V3-EXQ-460k"
...
EXPERIMENT_PURPOSE = "evidence"
SUPERSEDES = "v3_exq_460j_natural_commit_occupancy_release_decommit_falsifier"
...
def _make_config(env) -> REEConfig:
    ...
    cfg = REEConfig.from_dims(
        ...
        # the hold re-asserts the beta latch against the de-commit churn so the OFF
        # baseline sustains (the sustained reference the rung-6 release shortens + the
        # gate-3 sustained-hold proxy certifies). The hold yields to the closure
        # de-commit / MECH-091 / the rung-6 release (no trainable params).
        use_natural_commit_latch_hold=True,
        # V3-EXQ-460k: the CLOSURE-EXCLUSIVE DE-COMMIT EVAL mode (BUILT ree-v3 e52158d
        # 2026-06-22; the named dissociable substrate from the 460j park). It (a) makes
        # beta elevation closure-EXCLUSIVE (_commit_for_beta = _closure_commit_active
        # only; the fragile F-driven result.committed path is SUPPRESSED from beta) AND
        # (b) arms the natural-commit latch-hold on the closure-coupled commit
        # (_closure_commit_active, guarded by beta_gate.refractory_remaining==0). The
        # closure commitment persists across ticks (unlike result.committed), so the OFF
        # baseline now sustains a natural-commit occupancy via the closure plane -- the
        # 460j ARM_LEVER_OFF reassert=0/armed=0 failure dissolves. Carried into EVERY arm
        # via _clone_arm's deepcopy. Preconditions (loud ValueError at REEAgent.__init__):
        # closure_exclusive_decommit_eval=True REQUIRES use_closure_commit_beta_coupling
        # AND use_natural_commit_latch_hold -- both set above.
...
def _build_closure_env(scaffold_cfg: ScaffoldedSD054OnboardingConfig,
    ...
    (see `_run_seed`). Default None passes `seed=None` straight through to
    CausalGridWorldV2, whose `np.random.default_rng(None)` takes OS entropy at
    CONSTRUCTION and does not consume the numpy global RNG -- so the default is
    bit-identical to every landed run, and `_run_seed`'s np/torch seeding still
    does not reach this env unless the knob is set. Do NOT flip the default:
    pinning it unconditionally would change the eval-arm env layout, and
    therefore the results, breaking comparability with the landed runs.
    """
    p2_hfa = (
...
def _eval_arm_behaviour(
    ...
    # V3-EXQ-460k: closure-exclusive eval-mode arm count (closure-coupled hold arms)
    ...
    with torch.no_grad():
        for _ in range(n_eps):
            ...
            # V3-EXQ-460k closure-exclusive eval-mode arm count (read BEFORE reset wipes
            # it, like the re-assert count). >0 certifies the eval mode armed the
            # closure-coupled latch-hold (the 460j gap: this was 0 because the hold armed
            # only on result.committed).
...
def _closure_exclusive_eval_armed(arm_off: Dict[str, Any]) -> bool:
    """V3-EXQ-460k readiness gate 2.5: the closure-exclusive de-commit eval mode actually
    ARMED the closure-coupled natural-commit latch-hold on ARM_LEVER_OFF --
    ncl_hold_closure_armed_total > 0 (the closure plane armed the hold) AND
    ncl_hold_reassert_total > 0 (the hold re-asserted the beta latch). 460j had BOTH at 0
    (the hold armed only on result.committed, which never formed on the closure-coupling
    substrate). Below this -> substrate_not_ready_requeue (the BUILT eval mode did not
    engage), NEVER a false MECH-445/446 weakening."""
    return bool(
        int(arm_off.get("ncl_hold_closure_armed_total", 0)) > 0
        and int(arm_off.get("ncl_hold_reassert_total", 0)) > 0
    )
...
def _run_seed(seed: int, dry_run: bool, total_eps: int,
    ...
    # world_obs_dim parity probe env, 1 + arm_i = the per-arm eval env.
    probe_env = _build_closure_env(
    ...
    for arm_i, arm in enumerate(ARMS):
        closure_env = _build_closure_env(
            scaffold_cfg,
            seed=_derive_env_seed(seed_env_base, stream=2, idx=1 + arm_i),
        )
        closure_env.reset()
    ...
    # V3-EXQ-460k gate 2.5 (NEW): the closure-exclusive eval mode armed the hold.
    closure_eval_armed = _closure_exclusive_eval_armed(arm_off)
    # V3-EXQ-460j gate 3: the OFF baseline must sustain a natural-commit hold.
    ...
    print(f"  [train] arm_eval seed={seed} ep {done}/{total_eps}"
          ...
          f" reassert={arm_off['ncl_hold_reassert_total']}"
          f" closure_armed={arm_off['ncl_hold_closure_armed_total']})"
          f" eval_armed={closure_eval_armed} sustained={sustained_hold_certified}"
    ...
    print(f"verdict: {'PASS' if (guard_pass and seed_pass) else 'FAIL'} seed={seed}"
          f" guard_pass={guard_pass} eval_armed={closure_eval_armed}"
          f" sustained_hold={sustained_hold_certified}"
...
def run_experiment(dry_run: bool = False,
    ...
    # Readiness gate (2.5, NEW for 460k): the closure-exclusive de-commit eval mode ACTUALLY
    # armed the closure-coupled latch-hold on ARM_LEVER_OFF (ncl_hold_closure_armed_total > 0
    # AND ncl_hold_reassert_total > 0). 460j had both at 0 (the hold armed only on
    # result.committed, which never formed). Below this -> substrate_not_ready_requeue (the
    # BUILT eval mode did not engage), NEVER a false MECH-445/446 weakening.
    ...
    readiness_all_met = bool(
        ...
        and sustained_hold_met and lever_shortened_met and coupling_nonvacuity_met
    ...
    if not contact_non_vacuity_met:
        ...
        # The closure-exclusive de-commit eval mode did NOT arm the closure-coupled
        # natural-commit latch-hold on ARM_LEVER_OFF (ncl_hold_closure_armed_total /
        # ncl_hold_reassert_total stay ~0, the 460j counters) -> the BUILT substrate did
        # not produce the dissociable occupancy, so there is nothing for the rung-6 lever
        # or the SD-034 de-commit to act on = substrate not ready (NEVER a false MECH-445/446
        # weakening). The new V3-EXQ-460k gate.
        ...
        # All seven readiness gates clear -> the co-occurrence DV is interpretable.
        ...
        "interpretation": {
            ...
            "preconditions": [
                ...
                },
                {
                    ...
                    "description": "V3-EXQ-460k gate 2.5 (NEW): the closure-exclusive "
                                   "de-commit eval mode (closure_exclusive_decommit_eval, "
                                   "BUILT ree-v3 e52158d) ACTUALLY armed the closure-coupled "
                                   "natural-commit latch-hold on ARM_LEVER_OFF -- "
                                   "ncl_hold_closure_armed_total > 0 (closure plane armed the "
                                   "hold) AND ncl_hold_reassert_total > 0 (hold re-asserted the "
                                   "latch) on >= 2/3 guard seeds. 460j had BOTH at 0 (the hold "
                                   "armed only on result.committed, which never formed). Below "
                                   "this -> substrate_not_ready_requeue (the BUILT eval mode did "
                                   "not engage), NEVER a false MECH-445/446 weakening.",
                    "control": "ARM_LEVER_OFF ncl_hold_closure_armed_total + "
                               "ncl_hold_reassert_total under closure_exclusive_decommit_eval.",
                    "measured": ce_frac,
                    "threshold": MIN_FRACTION,
            ...
            "criteria_non_degenerate": {
                # The co-occurrence DV is non-degenerate iff all seven readiness gates
                # cleared (contact, trained head, closure-exclusive eval armed the hold,
                # OFF baseline sustained, lever shortened occupancy, coupling engaged,
            ...
            "amend_legs_under_test": {
                ...
                "natural_commit_latch_hold": "use_natural_commit_latch_hold (sustains the OFF baseline).",
                "closure_exclusive_decommit_eval": "closure_exclusive_decommit_eval (BUILT ree-v3 "
                    "e52158d 2026-06-22): beta elevation closure-EXCLUSIVE + latch-hold arms on "
                    "_closure_commit_active so the OFF baseline sustains via the closure plane "
                    "(the named dissociable substrate from the 460j park). ON in ALL arms.",
...
def main(dry_run: bool = False,
    ...
    manifest = {
        ...
        "substrate": "scaffolded_sd054_onboarding (full curriculum: Stage-0 -> Stage-0b -> "
                     ...
                     "(use_natural_commit_urgency_release) toggled per arm + the natural-commit "
                     "LATCH-HOLD (use_natural_commit_latch_hold) + the V3-EXQ-460k "
                     "CLOSURE-EXCLUSIVE DE-COMMIT EVAL mode (closure_exclusive_decommit_eval, "
                     "BUILT ree-v3 e52158d 2026-06-22) ARMED in ALL arms: beta elevation is "
                     "closure-EXCLUSIVE + the latch-hold arms on _closure_commit_active so the "
                     "OFF baseline sustains a natural-commit occupancy via the closure plane "
                     "(the named dissociable substrate from the 460j park; 460j ARM_LEVER_OFF "
                     "had reassert=0/armed=0).",
        "condition": CONDITION_LABEL,
        "method_note": "460j-SUPERSEDING successor on the now-BUILT closure-exclusive de-commit "
                       "eval substrate. 460j ARM_LEVER_OFF failed to sustain (ncl_hold "
                       "reassert=0/armed=0; the latch-hold armed only on result.committed, which "
                       "does not form on the closure-coupling substrate) -> "
                       "off_baseline_not_sustained -> substrate_not_ready_requeue. The closure_"
                       "exclusive_decommit_eval mode (BUILT ree-v3 e52158d 2026-06-22) makes "
                       "beta elevation closure-EXCLUSIVE and arms the latch-hold on "
                       "_closure_commit_active, so the OFF baseline sustains a natural-commit "
                       "occupancy via the closure plane -- natural-commit and closure-de-commit "
                       "are now DISSOCIABLE. This run sets the eval mode ON in EVERY arm and "
                       "tests whether the rung-6 graded natural-commit-occupancy release (BG-3 "
                       "D1: graded BG/pallidal urgency, NOT a fixed refractory) shortens that "
                       "occupancy so MECH-445 commit-intent + MECH-446 occupancy-drop CO-OCCUR "
                       "on >= 2/3 seeds. Four eval arms (one trained substrate per seed, lever "
                       "toggled at eval via clone): ARM_LEVER_OFF (occupancy reference) / "
                       "ARM_GAP_SCALED (graded urgency, PRIMARY) / ARM_FLAT_RATE "
                       "(gap_entry_sensitivity=0, the D1 fixed-refractory control) / "
                       "ARM_ACTION_EXTENT (Jin rendering). SEVEN readiness gates self-route "
                       "substrate_not_ready_requeue when unmet -- never a false weakens (incl. "
                       "the NEW closure-exclusive-eval-armed gate: ncl_hold_closure_armed_total "
                       "/ ncl_hold_reassert_total > 0 on ARM_LEVER_OFF, the counters 460j had "
                       "at 0). claim_ids: MECH-446 scored (within-arm occupancy-drop component), "
                       "MECH-445 the commit-intent precondition; MECH-260/MECH-261 NOT re-tagged.",
        "arm_note": "ARMS = " + ", ".join(a["key"] for a in ARMS) + ". Closure ON + "
                    "closure_exclusive_decommit_eval ON in every arm; the variable is the "
                    "natural-commit RELEASE lever config. ARM_GAP_SCALED is the load-bearing "
                    "primary; ARM_LEVER_OFF is the occupancy reference (the closure-plane "
                    "sustained baseline the release shortens); ARM_FLAT_RATE / ARM_ACTION_EXTENT "
                    "are the D1 contrasts.",
        ...
        "scaffold_curriculum": {
            ...
            "scaffold_train_harm_pathway": True,
            "scaffold_train_rule_bias_head": True,
            "config_basis": "V3-EXQ-460j substrate + rung-6 natural-commit-occupancy-release "
                            "lever (ree-v3 main ab2c1a9, 2026-06-20) + natural-commit latch-hold "
                            "(2026-06-21) + closure-exclusive de-commit eval mode "
                            "(closure_exclusive_decommit_eval, ree-v3 main e52158d, 2026-06-22)",
        },
