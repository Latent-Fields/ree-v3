...
"""V3-EXQ-819 -- MECH-457 / INV-088 competence_floor R4 INSTRUMENT-VALIDITY leg: does a
PREDICTION-TRAINED z_world confer any competence advantage over a FROZEN RANDOM-PROJECTION
z_world of the same dimensionality?

DIAGNOSTIC instrument-validity probe (experiment_purpose=diagnostic; claim_ids tag relevance
only -> excluded from governance confidence/conflict scoring). PROMOTES / DEMOTES NOTHING.
Routes to /failure-autopsy for adjudication. MECH-457 stays candidate / v3_pending; INV-088
unchanged.
...
representation), not about another candidate mechanism. NOT braked: the re-derive brake
(>=2 substrate_ceiling autopsies on MECH-457) is RELEASED because the upstream substrate the
most-recent counted autopsy owed -- sd_zworld_warmup_optimizer_group -- is now BUILT (ree-v3
b523b9c70a) and VALIDATED (REE_assembly 25a69fcd4c, 2026-07-22: world_encoder tensors move on
every seed at zworld_p0_episodes=60), and this tests a NEW, never-tested question with a new
EXQ NUMBER, not a lettered same-mechanism iteration.
...
EXPERIMENT_TYPE = "v3_exq_819_mech457_inv088_zworld_trained_vs_random"
QUEUE_ID = "V3-EXQ-819"
...
ADVANTAGE_ABS_FLOOR = 1.0             # a competence-scale advantage of >= one resource/ep floor
...
PRECONDITION_SPECS: Tuple[PreconditionSpec, ...] = (
    ...
    PreconditionSpec(
        ...
        description=(
            "The BC install took on this arm before RL: post_bc_foraging_competence (measured "
            "pre-RL, post warm-start) clears the 1.0 floor on the WORST seed. An install that did "
            "not take carries no installed prior; the run self-routes substrate_not_ready_requeue "
            "when NEITHER arm installs, or records an asymmetric-install ADVANTAGE finding when "
            "only one does. Per-arm and NON-vacating: a non-installing control never vacates the "
            "trained arm's valid result."
        ),
