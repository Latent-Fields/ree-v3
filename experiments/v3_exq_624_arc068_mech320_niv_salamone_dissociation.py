"""V3-EXQ-624: ARC-068 / MECH-320 opportunity-cost vs effort-cost dissociation.
...
Salamone axis; no broad profile sweep):
  ARM_0_baseline:
    use_tonic_vigor=False, default env (no elevated movement cost).
    Reference action_density under the legacy substrate.
  ARM_1_vigor_niv:
    ...
    gate-drive collapse), default env.
  ...
  ARM_2_vigor_salamone:
    ...
    movement cost (limb_damage_enabled=True, damage_increment=0.30, failure_prob_scale=0.6).
...
Protocol:
  P0 warmup (100 ep, vigor OFF + default env in all arms): identical baseline
    policy checkpoint per seed. Vigor is target-free and its EWMA starts at
    zero; warmup with vigor OFF + same env in all arms ensures the policy
    learns on identical data so any P1 difference is attributable to the arm
    intervention not training divergence.
  P1 measurement (30 ep x 200 steps): arm flag and env config toggled. ARM_0
    and ARM_3 keep vigor=False; ARM_1 and ARM_2 instantiate TonicVigor with
    v_t_floor=0.05. ARM_0 and ARM_1 use default env; ARM_2 and ARM_3 use
    Salamone-style elevated-movement-cost env.
...
Environment (well-fed-safe-familiar regime, matched across arms by seed):
  ...
  ARM_0 / ARM_1: limb_damage_enabled=False (default).
  ARM_2 / ARM_3: limb_damage_enabled=True, damage_increment=0.30,
    failure_prob_scale=0.6 (movement fails with probability damage * 0.6
    per limb; cumulative damage accrues on hazard contact). Heal_rate
...
Run with:
  /opt/local/bin/python3 experiments/v3_exq_624_arc068_mech320_niv_salamone_dissociation.py
or:
  /opt/local/bin/python3 experiments/v3_exq_624_arc068_mech320_niv_salamone_dissociation.py --dry-run
"""
...
# ----------------------------------------------------------------------
EXPERIMENT_TYPE = "v3_exq_624_arc068_mech320_niv_salamone_dissociation"
...
# Salamone-style elevated movement cost env knobs.
...
# Helpers
# ----------------------------------------------------------------------
def make_env(seed: int, salamone_arm: bool) -> CausalGridWorldV2:
    """Build env. Salamone arms get elevated movement cost via SD-022 limb damage.

    Default arms (ARM_0 / ARM_1): well-fed-safe-familiar regime. No limb damage
    enabled; movement is effectively free.

    Salamone arms (ARM_2 / ARM_3): SD-022 limb damage enabled with elevated
    damage_increment and failure_prob_scale. Movement still attempts but fails
    with probability damage * 0.6 per limb; movement is structurally more
    expensive.
    """
    kwargs = dict(
        size=GRID_SIZE,
    ...
    if salamone_arm:
        kwargs.update(
            ...
            damage_increment=SALAMONE_DAMAGE_INCREMENT,
            failure_prob_scale=SALAMONE_FAILURE_PROB_SCALE,
        )
...
def run_arm(
    ...
    P0 warmup uses the DEFAULT env (no Salamone movement cost) and vigor OFF in
    ALL arms so the policy learns on identical data. P1 measurement toggles the
    vigor flag and env config per arm.
    """
    print(f"Seed {seed} Condition {arm_label}", flush=True)
    torch.manual_seed(seed)

    # P0 warmup env (default; no salamone) for identical baseline policy.
    env_init_p0 = make_env(seed=seed, salamone_arm=False)
    ...
    for ep in range(TOTAL_EPISODES_PER_ARM):
        ...
        # P0 always default env; P1 uses the arm's env config.
        env = make_env(seed=ep_seed, salamone_arm=salamone_env and in_p1)
...
def main():
    ...
    print(
        f"V3-EXQ-624 ARC-068 / MECH-320 Niv-vs-Salamone dissociation",
        flush=True,
    )
    print(
    ...
    manifest = {
        ...
        "summary": {
            "scenario": (
                ...
                "seeds. P0 warmup 100 ep (vigor OFF + default env in all arms), P1 "
                "eval 30 ep x 200 steps (arm flag + env config toggled). 4-arm "
                "Niv-vs-Salamone dissociation: ARM_0 baseline; ARM_1 vigor + default "
                "env (Niv kernel); ARM_2 vigor + Salamone-style elevated movement "
                "cost; ARM_3 baseline + Salamone env (movement-cost-only control). "
                "Tests the R4 verdict: ARC-068 / MECH-320 w_passive must remain "
                "insensitive to parametric movement cost (Niv 2007 opportunity-cost-"
                "on-time), distinct from MECH-258 / SD-032b dacc_effort_cost "
                "(Salamone & Correa 2003). Forced-vigor probe v_t_floor=0.05 "
                "implements V3-EXQ-549 prescribed fix bypassing gate-drive collapse."
            ),
            "interpretation": (
