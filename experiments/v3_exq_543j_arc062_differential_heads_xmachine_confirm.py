...
"""V3-EXQ-543j: ARC-062 differential-heads CROSS-MACHINE CONFIRMATION of 543i.

NOT a supersession. V3-EXQ-543i stands and keeps its 543h/543g supersedes
chain; this is a byte-identical replication on a second host (ree-cloud-4,
set via the queue entry's machine_affinity) to clear the single-machine
cross-machine caveat before ARC-062/MECH-309 governance acts on 543i.
...
WHY (cross-machine confirmation of V3-EXQ-543i_20260518T191052Z):
  the real 543i run (3.4h, dry_run=False) FAILed into interpretation branch
  e_collapse_survives_structure_MECH309_strong_ARC063_required -- ALL 4
  diff-ON gated arms (ARM_8..11) 3/3 inert (TV~0) despite delta_hat L2
  pinned to differential_bias_scale=0.1 making head_0==head_1 collapse a
  structural non-equilibrium; matched diff-OFF gated arms also 3/3 inert
  (clean reproduction). Per-claim: MECH-309 supports, ARC-062 weakens,
  INV-074/MECH-334 non_contributory. BUT that rests on a SINGLE full run on
  one machine (DLAPTOP-4.local / Mac). The entire 543g->543h->543i chain
  exists BECAUSE of a cross-machine bistability: the same 543g config landed
  gating-ACTIVE on host-A (Mac) but INERT (n_inert=3, TV<0.05) on cloud-3
  AND cloud-4 -- head_0==head_1 collapse was the cross-machine COMMON
  attractor and 543g's "weakens" was a 1/3 minority-basin artifact. 543i is
  the mirror image (uniform collapse on Mac); before MECH-309-strong /
  ARC-062-weakens is locked into governance the result needs n_inert
  confirmation on a machine that participated in the original divergence.
  ree-cloud-4 is exactly one of the two machines (cloud-3/cloud-4) that
  produced the divergent INERT basin for 543g -- reproducing the uniform
  3/3 collapse on the diff-ON gated arms there is the strongest
  single-variable cross-machine confirmation. The differentiated-heads
  state is a flat / saddle equilibrium under outcome-coupled REINFORCE:
  head_init_bias_offset is softmax-invisible and the removed head_div term
  was softmax-cancelable.
...
-------------------------
  ...
  CROSS-MACHINE: this run IS the second-machine confirmation. 543i FAILed
  on the Mac (uniform 3/3 collapse, branch e). machine_affinity is pinned
  to ree-cloud-4 (set in the queue entry) -- a participant in the original
  543g cross-machine bistability (ACTIVE host-A / INERT cloud-3+cloud-4),
  precisely the failure mode under test.
  CONTEXT (lineage continuity, reported, NOT pass-gating): the full 543h
  legacy grid (D2_xtal, repro_543g, D1/D3/D4, C2/C3/C4, F1/F2) on
  ARM_0..ARM_7.

Interpretation grid (acceptance LOGIC byte-identical to 543i; branch labels
re-read here as CONFIRMS / CONTRADICTS / NON-CONTRIBUTORY vs the 543i
20260518T191052Z branch-e result):
  (a) diff_off_reproduced_collapse AND diff_on_escape AND c2c3_on_pass
      -> CONTRADICTS-543i: the diff-ON gated arms ESCAPE collapse on
         ree-cloud-4. 543i's uniform Mac collapse was itself a
         single-machine basin artifact (symmetric to the 543g failure).
         ...
         INV-074 supports, MECH-334 supports}. PASS. Action: do NOT demote
         ARC-062 on 543i; escalate to /failure-autopsy + a >=2-machine
         matrix design.
  (e) diff_off_reproduced_collapse AND NOT (diff_on_escape AND c2c3_on_pass)
      -> CONFIRMS-543i: collapse survives the structural non-equilibrium on
         ree-cloud-4 too -> branch e holds CROSS-MACHINE; the weak reading
         is insufficient; a genuine rule-apprehender (ARC-063, distributed
         CandidateRule field, V4) is required. {MECH-309 supports (STRONG
         confirmation); ARC-062 weakens; INV-074/MECH-334 non_contributory}.
         FAIL. Action: clears the single-machine caveat; ARC-062 demotion +
         ARC-063/V4 governance can proceed.
  (c) NOT diff_off_reproduced_collapse
      -> NON-CONTRIBUTORY: diff-OFF gated arms did not reproduce the
         collapse on ree-cloud-4: host substrate / seed drift; nothing
         cleanly attributable. {all non_contributory}. FAIL. Action: treat
         as host substrate-drift, do not weight governance.
...
QUEUE_ID = "V3-EXQ-543j"
# NOT a supersession. This is a byte-identical cross-machine CONFIRMATION of
# the real V3-EXQ-543i_20260518T191052Z run (branch e, MECH-309 supports /
# ARC-062 weakens). 543i stands and retains its own 543h/543g supersedes
# chain; 543j neither supersedes nor is superseded by anything.
SUPERSEDES = None
SUPERSEDES_CHAIN = []
CONFIRMS = "V3-EXQ-543i"
...
def write_manifest(result: Dict, dry_run: bool, elapsed: float) -> Tuple[Path, str]:
    ...
    manifest = {
        ...
        "dry_run": dry_run,
        "notes": (
            "V3-EXQ-543j: ARC-062 differential-heads CROSS-MACHINE "
            "CONFIRMATION of V3-EXQ-543i_20260518T191052Z. NOT a "
            "supersession -- 543i stands and keeps its 543h/543g supersedes "
            "chain; 543j is a byte-identical replication pinned (queue "
            "machine_affinity) to ree-cloud-4. The real 543i run FAILed into "
            "branch e_collapse_survives_structure_MECH309_strong_ARC063_"
            "required: ALL 4 diff-ON gated arms 3/3 inert (TV~0) despite "
            "delta_hat L2 pinned to differential_bias_scale=0.1 making "
            "delta==0 a structural non-equilibrium; matched diff-OFF gated "
            "arms also 3/3 inert (clean reproduction). Per-claim there: "
            "MECH-309 supports, ARC-062 weakens, INV-074/MECH-334 "
            "non_contributory. That rests on a SINGLE full run on one "
            "machine (DLAPTOP-4.local/Mac). The 543g->543h->543i chain "
            "exists because of a cross-machine bistability: the same 543g "
            "config landed gating-ACTIVE on host-A but INERT (n_inert=3, "
            "TV<0.05) on cloud-3 AND cloud-4; 543g's 'weakens' was a 1/3 "
            "minority-basin artifact. ree-cloud-4 is one of the two machines "
            "that produced the divergent INERT basin for 543g; reproducing "
            "the uniform 3/3 collapse on the diff-ON gated arms there is the "
            "strongest single-variable cross-machine confirmation. 12-arm "
            "pruned design: ARM_0..ARM_7 = the exact 543h 2x2x2 with "
            "use_differential_heads OFF (diff-OFF repro/sanity baseline); "
            ...
            "differential_bias_scale=0.1). The P1 outcome-coupled REINFORCE "
            "loss is BYTE-IDENTICAL across every arm and to 543i -- the ONLY "
            "varied factor vs 543i is the host machine. The "
            "crystallize_at_phase3 third factor is retained "
            "(CRYSTALLIZE_P1_OPEN_FRACTION={}, residue EWC lambda={}). "
            "Acceptance LOGIC byte-identical to 543i; read here as "
            "confirmation: branch e CONFIRMS-543i (collapse holds "
            "cross-machine -> clears the single-machine caveat; ARC-062 "
            "demotion + ARC-063/V4 governance can proceed); branch a "
            "CONTRADICTS-543i (escape on ree-cloud-4 -> 543i was itself a "
            "single-machine basin artifact; do NOT demote ARC-062 on 543i, "
            "escalate to /failure-autopsy + a >=2-machine matrix); branch c "
            "NON-CONTRIBUTORY (diff-OFF did not reproduce collapse on this "
            "host -> host substrate/seed drift, do not weight governance). "
            "Interpretation branch in "
            "metrics.acceptance.interpretation_branch. SD-054 bipartite "
            "layout; thresholds/metrics otherwise inherited from 543c-i."
...
if __name__ == "__main__":
    ...
    print("\n=== V3-EXQ-543j SUMMARY (cross-machine confirm of 543i) ===", flush=True)
