...
"""V3-EXQ-537d: SD-029/MECH-256 single-pass comparator -- DROP INTERVENTIONAL margin loss.

Supersedes V3-EXQ-537c (FAIL non_contributory 2026-05-08T20:59Z, hidden_dim
128->256 capacity lift did not recover R^2 ceiling: p2_grad=False all 3 seeds at
p2_steps=48000, p2_r2 0.68-0.74, fwd_r2 0.66-0.68 -- same plateau as 537/537a/537b).

DIAGNOSIS OF V3-EXQ-537c (2026-05-08T20:59Z):
    Hidden_dim 128 -> 256 produced essentially the same R^2 ceiling as 128.
    Width is not the bottleneck. The remaining suspect in the model+training
    stack is the interventional contrastive loss (SD-013): a margin loss
    applied to 30% of batches forcing E2_harm_s outputs to diverge for
    different actions from the same state. While the interventional loss is
    architecturally correct for SD-013, it competes with the raw MSE fit on
    the residual delta architecture and may be capping training-time R^2
    before the model converges.

    Reference points:
      EXQ-195 (no interventional, baseline substrate):       R^2 = 0.914
      EXQ-320 (interventional, different substrate):         R^2 ~ 0.98
      EXQ-537 family (interventional, reef+limb-damage):     R^2 ~ 0.65-0.74

    The R^2 gap between EXQ-195 and EXQ-537 (~25 points) is consistent with
    interventional + substrate complexity together capping fit. Width lift
    alone closed none of the gap. Drop interventional and see whether R^2
    recovers EXQ-195's 0.91 or stays at 0.7.

THE 537d FIX
------------
USE_INTERVENTIONAL = False at the script-level constant. The corresponding
margin loss in _p2_train_forward_model is then skipped for 100% of batches
(use_interventional=False path). E2HarmSConfig.use_interventional / fraction /
margin fields are still passed (for substrate consistency) but have no effect
when use_interventional=False.

KEEP from 537c:
  - hidden_dim=256 (no evidence width is harmful; one less variable to flip back)
  - decoupled training/eval curricula (architecturally correct, separately motivated)
  - emit_outcome runner-conformance hook
  - C1-C4 thresholds unchanged

ALSO ADD for 537d (belt-and-braces for runner-side bug):
  - `[experiment_type] outcome=FAIL elapsed=...` stdout fallback line so the
    runner's stdout-regex fallback classifies correctly even if the
    REE_QUEUE_ID env var doesn't propagate to the subprocess (separate
    runner-side bug noted but not fixed here). 539a worked correctly only
    because it had this print pattern; 537/537a/537b/537c did not.

EXPECTED RESULT:
  PASS: arms cleanly distinguished, MECH-256 supports.
  C1 PASS + C2 FAIL: forward model now fit (R^2 >= 0.85), but still no
                     efference-driven dissociation -- clean weakens on SD-029.
  C1 still FAIL: R^2 plateau is intrinsic to substrate complexity (not
                 interventional loss alone). Route to /diagnose-errors for 537e
                 -- candidates: longer training, bigger model, simpler env, or
                 reframe the test.

PER-CLAIM DIRECTION GRID (unchanged from 537/537c):
    C1 PASS + C2 PASS + C3 PASS + C4 PASS: SD-029 + MECH-256 = supports
    C1 FAIL: blanket non_contributory
    C1 PASS + C2 FAIL + C4 PASS: SD-029 + MECH-256 = weakens
    C1 PASS + C2 PASS + C3 FAIL: MECH-256 = weakens, SD-029 = mixed
    C4 FAIL: blanket non_contributory

claim_ids: ["SD-029", "MECH-256"]
experiment_purpose: evidence
supersedes: V3-EXQ-537c
"""

# (Original 537c docstring follows below for context.)
...
EXPERIMENT_TYPE = "v3_exq_537d_sd029_no_interventional"
QUEUE_ID = "V3-EXQ-537d"
CLAIM_IDS = ["SD-029", "MECH-256"]
EXPERIMENT_PURPOSE = "evidence"
...
USE_INTERVENTIONAL = False  # 537d: dropped to test capacity-vs-margin-loss hypothesis
...
def main(dry_run=False):
    ...
    manifest = {
        ...
        "experiment_purpose": "evidence",
        "supersedes": "v3_exq_537c_sd029_capacity_lift",
    ...
    # 537d: stdout-regex-fallback pattern for the runner. Belt-and-braces
    # in case REE_QUEUE_ID env var doesn't propagate to subprocess (537c
    # silent-UNKNOWN root cause; runner-side bug). Match form used by 539a.
    print(
        f"[{EXPERIMENT_TYPE}] outcome={outcome} elapsed={time.time() - _t_start:.1f}s",
        flush=True,
    )
