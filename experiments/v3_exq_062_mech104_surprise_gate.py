...
Context:
  EXQ-061 (PASS) confirmed the jitter approximation: adding a fixed epsilon to
  running_variance on any harm contact enables de-commitment. However EXQ-061
  tests a *structural* property (the mechanism is wired) rather than the *functional*
  property the claim requires: that the interrupt is **selectively** triggered by
  **unexpected** harm (high prediction error), not routine/predicted harm.

  MECH-104 full claim: running_variance receives an upward impulse proportional to
  surprise magnitude — |actual_harm - predicted_harm| — not to harm magnitude per se.
  This selectivity is what makes the mechanism neurobiologically plausible (LC-NE fires
  on prediction errors, not on all aversive events).

Design — single trained agent, three interrupt policies (eval only):
  All three conditions share one trained agent (400 eps), ensuring identical starting
  state (variance collapsed to ~0, agent committed). Only the interrupt policy differs
  during eval.

  Condition A — NO INTERRUPT (baseline)
    running_variance unchanged on harm contact.
    Expected: 0 uncommitted steps (confirmed collapse).

  Condition B — ALWAYS-SPIKE (jitter, EXQ-061 style)
    running_variance += spike_magnitude on any step with actual_harm < -0.01.
    Expected: frequent de-commitment (fires on ALL harm, predicted or not).
...
EXPERIMENT_TYPE = "v3_exq_062_mech104_surprise_gate"
...
def _eval_interrupt_policy(
    ...
    The agent's running_variance is reset to initial_variance at the start
    of each eval episode (committed if initial_variance < commit_threshold,
    ...
    for _ in range(num_episodes):
        flat_obs, obs_dict = env.reset()
        agent.reset()
        # Reset variance to initial committed state
...
def run(
    ...
    **kwargs,
    ...
    print(
        f"[V3-EXQ-062] MECH-104: Surprise-Gated Volatility Interrupt\n"
    ...
    # ── Train single agent ───────────────────────────────────────────────────
    print(f"\n{'='*60}", flush=True)
    print(f"[V3-EXQ-062] Training ({warmup_episodes} eps)...", flush=True)
    print('='*60, flush=True)
    ...
    for policy, label in [
        ...
        ("always",   "Condition B — always-spike"),
        ("surprise", "Condition C — surprise-gated"),
    ]:
        print(f"\n{'='*60}", flush=True)
        print(f"[V3-EXQ-062] {label}", flush=True)
        print('='*60, flush=True)
    ...
    print(f"\nV3-EXQ-062 verdict: {status}  ({criteria_met}/5)", flush=True)
    ...
    summary_markdown = f"""# V3-EXQ-062 — MECH-104: Surprise-Gated Volatility Interrupt
...
|---|---|---|---|
| A | None (baseline) | {r_none['n_uncommitted']} | — |
| B | Always-spike | {r_always['n_uncommitted']} | {r_always['n_spikes']} |
| C | Surprise-gated | {r_surprise['n_uncommitted']} | {r_surprise['n_spikes']} |

Selectivity ratio (C/B): {metrics['selectivity_ratio']:.3f}
