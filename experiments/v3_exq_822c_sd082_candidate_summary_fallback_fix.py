"""V3-EXQ-822c -- SD-082 HEAD-INTERNALS DIAGNOSTIC, BUG-FIX RE-RUN OF V3-EXQ-822b.

WHY THIS IS A BUG FIX, NOT A NEW EXPERIMENT (read this before the inherited
822b docstring below, which is otherwise unchanged and still describes the
overall design correctly).
---------------------------------------------------------------------------
V3-EXQ-822b (2026-07-27) tried to root-cause why SD-082's rule_bias_head never
receives a training signal (readiness gate (d) propagation_non_vacuity stuck
at exactly 0.0 across V3-EXQ-822 and V3-EXQ-822a). It self-routed
substrate_not_ready_requeue because readiness (f) -- P2 head-diagnostic sample
sufficiency -- failed: ALL SIX cells (both arms, all three seeds) reported
n_head_diag_samples=0 AND n_prop_samples=0, not merely the reported worst
cell. That uniformity across every cell is the signature of a driver bug, not
seed noise, and tracing it found one:

`_candidate_summaries()` (below) called only `agent._candidate_world_summaries
(candidates)`, whose OWN docstring states it "[r]eturns None when
candidate_summary_source is the default 'proposer'" -- and `_make_agent()` in
this script (and in 822/822a before it) never sets
`candidate_summary_source="e2_world_forward"`. So `_candidate_summaries()`
returned None on EVERY tick of EVERY cell, in every one of 822/822a/822b.

Consequence, traced through the script: P1's `p1_snap` (gated on `cs is not
None`) was always None, so `ep_buf` never accumulated, so `outcome_buf`
stayed permanently empty, so `_lpfc_reinforce_loss` always hit its
`len(outcome_buf) < 2` early return and produced a zero tensor with no
`grad_fn`, so `l_loss.requires_grad` was always False and the REINFORCE
optimiser step was SKIPPED ON EVERY EPISODE. `rule_bias_head`'s last layer
never moved not because of a dead ReLU, not because of gradient starvation
inside the head, but because the training loop that was supposed to feed it
never produced a single sample. Separately, the exact same None short-circuit
also starved the P2 propagation/head-diagnostic measurement this run's own
gate (f) exists to guard.

THE FIX IS NOT "enable candidate_summary_source=e2_world_forward"
--------------------------------------------------------------------
That flag exists for a DIFFERENT problem (ARC-065 GAP-A: the collapsed-
proposer first-step z_world has near-zero cross-candidate spread under
monostrategy). Turning it on here would introduce a confound 822/822a/822b
never tested under, changing the substrate under test rather than fixing the
measurement. The PRODUCTION `select_action` path (ree_core/agent.py, the
`cand_world_summaries` block feeding `lateral_pfc.compute_bias`) already
handles a None return correctly: it falls back to building per-candidate
summaries from each candidate's own `world_states` (the legacy
collapsed-proposer path) -- see agent.py's `try: cand_world_summaries; except
NameError:` block, `if cand_world_summaries is None: ... K = len(candidates);
... cand_world_list.append(ws[0, 0, :])`. `_candidate_summaries()` here is
fixed to replicate that EXACT fallback (same source, same shape), so this
script's own measurement now reads what `select_action` actually computes on
the DEFAULT config -- the identical substrate 822/822a/822b already claimed
to be testing, minus the measurement gap.

SUPERSEDES: v3_exq_822b_sd082_head_internals_diagnostic_20260727T180919Z_v3
(self-routed substrate_not_ready_requeue; corrected here.) Everything else
below -- design, arms, seeds, schedule, readiness gates (a)-(c),(e),(f),
thresholds -- is UNCHANGED from 822b; only `_candidate_summaries()` differs.

--- 822b's original docstring follows, describing the shared design/rationale ---
...
EXPERIMENT_TYPE = "v3_exq_822c_sd082_candidate_summary_fallback_fix"
...
SUPERSEDES = "v3_exq_822b_sd082_head_internals_diagnostic_20260727T180919Z_v3"
# Bug-fix re-run: 822b self-routed substrate_not_ready_requeue because
# _candidate_summaries() always returned None (see module docstring); this
# run fixes that measurement gap and re-executes the identical design.
...
def _candidate_summaries(agent: REEAgent, candidates) -> Optional[torch.Tensor]:
    """BUG FIX (V3-EXQ-822c): replicate the PRODUCTION select_action fallback
    (ree_core/agent.py, the cand_world_summaries block feeding
    lateral_pfc.compute_bias) instead of returning None whenever
    candidate_summary_source is the default "proposer". 822/822a/822b called
    only agent._candidate_world_summaries(candidates), whose own docstring
    states it returns None in exactly that (default, unset-flag) case --
    which starved BOTH the P1 REINFORCE training loop and the P2 propagation
    measurement in every cell of every prior run in this lineage. See module
...
def main():
    ...
    full_config = {
        ...
        "supersedes_note": ("bug-fix re-run of v3_exq_822b_sd082_head_internals_diagnostic "
                             "-- fixes _candidate_summaries() always returning None "
                             "(candidate_summary_source fallback gap), see module docstring; "
                             "not a supersession of v3_exq_822a_sd078_rule_selection_consumer, "
                             "which this remains an instrumentation-only diagnostic companion to"),
    }
    manifest = {
