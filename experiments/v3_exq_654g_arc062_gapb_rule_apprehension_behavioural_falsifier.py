...
"""
V3-EXQ-654g -- arc_062_rule_apprehension:GAP-B behavioural falsifier for
MECH-309 / ARC-062, ported onto the 569i-VALIDATED TOP-K SHORTLIST conversion.

SUPERSEDES V3-EXQ-654f (FAIL non_contributory, 2026-06-18; run_id
v3_exq_654f_arc062_gapb_rule_apprehension_behavioural_falsifier_20260618T005228Z_v3;
confirmed failure_autopsy_V3-EXQ-654f_2026-06-18). The 654f autopsy proved the
CRF-gate calibration amend (ree-v3 main 42895f6) WORKED -- C1 was FULLY MET:
crf_frac_active 0.83-0.97 cleared the 0.30 floor; the conflict-gate lockout is gone
(crf_mean_n_matched 2.18-3.81, NOT the 654d 7-8); propagation non-vacuous. CRF is
DONE -- this run adds NO further CRF amend. The 654f RESIDUAL was that it armed the
SUPERSEDED ADDITIVE conversion lever ARM_STD_G2 (modulatory_authority_normalize_basis
=std + authority_gain=2.0) as the STANDALONE conversion, which
behavioral_diversity_isolation:GAP-A proved INSUFFICIENT (V3-EXQ-569h FAIL, the
std-basis additive lift cleared C_R1B on only 1/3 seeds). GAP-A has SINCE SOLVED the
conversion ceiling via the TOP-K SHORTLIST: V3-EXQ-569i PASS/supports 2026-06-17
("diversity reaches committed action"; ARC-065 promoted stable). 654g wires that
validated fix into the GAP-B falsifier -- the fix EXISTS and is VALIDATED; only the
experiment wiring it is owed.

THE 569i-VALIDATED TOP-K SHORTLIST CONVERSION (the matched-stack CONSTANT armed on
BOTH arms here): F (the raw primary harm/goal scores) filters the candidate pool to a
SMALL fixed top-k F-best near-tie set (use_modulatory_shortlist_then_modulate=True +
modulatory_shortlist_mode="top_k" + modulatory_shortlist_k=3); the modulatory channel
(_modulatory_accum: the lateral_pfc CRF rule-bias + the routed cand_world_summary
range) then ARBITRATES the winner WITHIN that k-set (argmin committed). A SMALL fixed
k gives an eligible set whose MEMBERSHIP rotates with state, so argmin-of-the-routed-
channel within the rotating small set produces committed-action diversity that
reflects genuine per-candidate structure WITHOUT the modulatory channel having to
...
are eligible, so a clearly-harmful candidate is never selectable. The std-basis
authority (gain 2.0) is RETAINED on both arms exactly as 569i kept it ("shortlist
OVERRIDES selection"); it is NO LONGER the load-bearing conversion -- the TOP-K
SHORTLIST is. So ARM_STD_G2 is NOT armed as the standalone additive lever 654f
relied on; this run does not test it.

DESIGN CHANGES vs V3-EXQ-654f (everything else matched -- the now-working CRF stack,
the GAP-A e2_world_forward channel source, the C1a-d preconditions, the committed-
class entropy PRIMARY DV, and the three-branch NO-weakens routing are UNCHANGED):
  1. ARM the TOP-K SHORTLIST conversion (use_modulatory_shortlist_then_modulate +
     modulatory_shortlist_mode="top_k" + modulatory_shortlist_k=3) as a matched-stack
     CONSTANT on BOTH arms -- the actual 569i-validated conversion fix.
  2. The CRF-gate calibration levers (crf_mature_context_match_threshold=0.7 +
     crf_tolerance_conflict_cap=3 + crf_maintenance_couple_to_theta=True) stay armed
     on the ARM_ON CRF stack EXACTLY as 654f had them -- CRF is done (654f C1 fully
     met), NO further CRF amend is added.

KNOWN OPEN RISK (does NOT block this run; pre-registered as branch (b) below): per the
conversion-ceiling Phase-0 synthesis (MECH-439, the F-dominance live root), the top-k
shortlist's 2/3-seed margin is THIN (569i committed entropy 0.711 vs proposer 0.650 --
a 2/3-seed PASS that thinly cleared) and may not survive the GAP-B composite. If C2
fails on this VALIDATED top-k lever, that is branch (b) -- a deeper shared selection-
authority conversion ceiling (route to /implement-substrate, the F-dominance rebalance
MECH-439 owns), NOT a MECH-309 / ARC-062 falsification, NOT a weakens.

C1c GUARD (EXPECTED to clear -- 654f already cleared it on the now-working CRF stack):
if crf_frac_active < 0.30 STILL, the substrate regressed -> C1c self-routes
substrate_not_ready_requeue (NEVER a false weakens). CONVERSION-CEILING off-ramp: C1
holds (gate fires, CRF matured, propagation non-vacuous) but the C2 committed-class
entropy lift is absent -> conversion_ceiling_persists, non_contributory +
/implement-substrate (the F-dominance / MECH-439 deeper rebalance), NOT a
falsification of MECH-309 / ARC-062.
...
V3-EXQ-571; the LIVE root per the MECH-439 Phase-0 synthesis). 654g ARMS the
569i-VALIDATED TOP-K SHORTLIST conversion (F filters to a small rotating top-k
near-tie set; the modulatory channel arbitrates within it) precisely to overcome
this -- the lever that V3-EXQ-569i PASSed where the SUPERSEDED additive ARM_STD_G2
lever FAILED (569h, 1/3 seeds). So if THIS run's MATURED + DIFFERENTIATED +
MAINTAINED rule pool clears the C1c floor AND its bias reaches committed action
(C1d) but STILL fails to lift committed-class entropy (C2) EVEN WITH the validated
top-k shortlist armed, that is the SAME SHARED CONVERSION CEILING persisting under
the validated lever (the thin 569i margin not surviving the composite; see KNOWN
OPEN RISK) -- NOT a MECH-309 / ARC-062 falsification. The legacy "weakens" branch
is therefore REPLACED here by a conversion-ceiling off-ramp that self-routes
non_contributory and routes to /implement-substrate (the deeper F-dominance
rebalance MECH-439 owns). The only claim-weighting outcome is the PASS branch
(C2 lift = supports). A C2 fail cannot discriminate "rule-creator adds no
diversity" from "the shared F-dominance ceiling masks the lift" while the
conversion ceiling is open, so it does NOT weaken the claims.
...
(V3-EXQ-643a float32 fix) + channel routing (cand_world_summary) + the
569i-validated TOP-K SHORTLIST conversion (use_modulatory_shortlist_then_modulate +
mode="top_k" + k=3), MECH-341 stratified-select, MECH-313 noise floor, the
...
------------------------------------------------------------------------------
  ...
  FAIL (C1 holds, C2 fails) = the matured + maintained + differentiated rule pool's
          ...
          DEEPER SHARED selection-authority CONVERSION CEILING / F-dominance
          (behavioral_diversity_isolation:GAP-A; 569g/682; MECH-439), NOT a
          falsification. non_contributory; route to /implement-substrate (the
          F-dominance rebalance). Do NOT weaken MECH-309 / ARC-062.
...
supersedes V3-EXQ-654f (FAIL non_contributory on the superseded additive ARM_STD_G2
lever; CRF stack was fully working, only the conversion lever was wrong; 654g wires
the 569i-validated top-k shortlist).

See REE_assembly/evidence/planning/arc_062_rule_apprehension_plan.md (GAP-B),
REE_assembly/evidence/planning/failure_autopsy_V3-EXQ-654f_2026-06-18 (proved CRF C1
fully met; residual = superseded additive conversion lever),
...
experiments/v3_exq_569i_gapa_conversion_topk_shortlist_falsifier.py (the validated
TOP-K SHORTLIST conversion config ported here),
ree-v3/CLAUDE.md (modulatory-bias-selection-authority TOP-K shortlist amend +
ARC-063 AMEND crf-availability-maintenance + ARC-062 GAP-A/B/C/D + SD-056 entries),
...
EXPERIMENT_TYPE = "v3_exq_654g_arc062_gapb_rule_apprehension_behavioural_falsifier"
QUEUE_ID = "V3-EXQ-654g"
SUPERSEDES = "V3-EXQ-654f"
...
#
# 654g: arm the 569i-VALIDATED TOP-K SHORTLIST conversion (V3-EXQ-569i PASS/supports
# 2026-06-17; ARC-065 promoted stable). The retained std-basis authority + channel
# routing build the modulatory accumulator (_modulatory_accum); the TOP-K SHORTLIST
# (below) then arbitrates within F's small rotating near-tie set -- the shortlist
# OVERRIDES the additive-authority selection, so the std-basis authority is NO LONGER
# the load-bearing conversion (that is the SHORTLIST). 654f's residual was arming the
# SUPERSEDED ARM_STD_G2 additive lever STANDALONE (no shortlist), which 569h proved
# insufficient (1/3 seeds). This config = the literal 569i conversion stack.
...
MODULATORY_CHANNEL_ROUTE_WEIGHT = 1.0          # 569i routed-vs-legacy accumulator proportion
MODULATORY_ROUTE_MIN_RANGE_FLOOR = 1e-6        # substrate numerical active/inactive floor
# TOP-K SHORTLIST conversion (the 569i fix; the load-bearing conversion this run wires):
# F filters to the k F-best near-tie set; the modulatory channel arbitrates within it.
# A SMALL fixed k gives a state-ROTATING eligible set -> committed-action diversity
# from genuine per-candidate structure without out-magnitude-ing the F-dominated primary.
USE_MODULATORY_SHORTLIST_THEN_MODULATE = True
MODULATORY_SHORTLIST_MODE = "top_k"            # the 569i lever (vs the failed 684 margin)
...
def _make_agent(env: CausalGridWorldV2, use_candidate_rule_field: bool) -> REEAgent:
    ...
    cfg = REEConfig.from_dims(
        ...
        # TOP-K SHORTLIST conversion (V3-EXQ-569i PASS; the load-bearing conversion).
...
def _build_manifest(
    ...
    return {
        ...
        "evidence_direction_note": (
            f"V3-EXQ-654g arc_062 GAP-B behavioural falsifier (MECH-309 / ARC-062), "
            f"supersedes V3-EXQ-654f (FAIL non_contributory 2026-06-18; confirmed "
            f"failure_autopsy_V3-EXQ-654f_2026-06-18). The 654f autopsy proved the CRF-gate "
            f"calibration amend WORKED -- C1 FULLY MET (crf_frac_active 0.83-0.97 > 0.30; "
            f"conflict-gate lockout gone, crf_mean_n_matched 2.18-3.81; propagation "
            f"non-vacuous). CRF is DONE; 654g adds NO further CRF amend. The 654f RESIDUAL "
            f"was arming the SUPERSEDED ADDITIVE conversion lever ARM_STD_G2 standalone, "
            f"which GAP-A proved insufficient (V3-EXQ-569h FAIL 1/3 seeds). 654g WIRES THE "
            f"569i-VALIDATED TOP-K SHORTLIST conversion (V3-EXQ-569i PASS/supports; ARC-065 "
            f"promoted stable): use_modulatory_shortlist_then_modulate=True + "
            f"modulatory_shortlist_mode={MODULATORY_SHORTLIST_MODE} + "
            f"modulatory_shortlist_k={MODULATORY_SHORTLIST_K} as a matched-stack CONSTANT on "
            f"BOTH arms -- F filters to the small rotating top-k near-tie set, the modulatory "
            f"channel (lateral_pfc CRF rule-bias + routed cand_world_summary) arbitrates "
            f"within it; the shortlist OVERRIDES the additive selection so the retained "
            f"std-basis authority (basis={MODULATORY_AUTHORITY_NORMALIZE_BASIS}, gain="
            f"{MODULATORY_AUTHORITY_GAIN}) is NOT the load-bearing conversion. The "
            f"now-working CRF stack (crf_persist + 666c maintenance + the CRF-gate amend "
            f"levers crf_mature_context_match_threshold={CRF_MATURE_CONTEXT_MATCH_THRESHOLD}/"
            f"crf_tolerance_conflict_cap={CRF_TOLERANCE_CONFLICT_CAP}/"
            f"crf_maintenance_couple_to_theta={CRF_MAINTENANCE_COUPLE_TO_THETA}) + "
            f"candidate_summary_source=e2_world_forward are CONSTANT on both arms. "
            f"Single-variable: ARM_OFF (use_candidate_rule_field=False, legacy collapsed "
            f"rule_state) vs ARM_ON (use_candidate_rule_field=True -> MATURED + MAINTAINED + "
            f"ACTIVE differentiated crf_source), both on the matched "
            f"649/643a/SD-056/SP-CEM/MECH-341 stack with the SD-033a bias head un-zeroed AND "
            f"TRAINED in a frozen-encoder P1 REINFORCE window (GAP-D); P0 200 ep. PRIMARY DV "
            f"= COMMITTED-CLASS entropy. C1 (non-vacuity) = committed-class axis exercisable "
            f"AND GAP-A divergence real (both arms) AND ARM_ON matured (crf_frac_active >= "
            f"{CRF_FRAC_ACTIVE_FLOOR}) AND propagation non-vacuous; C2 (PRIMARY) = "
            f"paired-by-seed ARM_ON > ARM_OFF committed-class entropy lift (>= "
            f"{C2_MIN_LIFT_SEEDS}/3 seeds). KNOWN OPEN RISK: the 569i top-k 2/3-seed margin "
            f"is thin (MECH-439 F-dominance live root) and may not survive the composite; if "
            f"C2 fails, that is branch (b) (deeper F-dominance rebalance), NOT a "
            f"falsification. interpretation_label={result['interpretation_label']}. "
            ...
            f"substrate_not_ready_requeue AND C1-holds-C2-fail self-routes the deeper "
            f"selection-authority CONVERSION ceiling (GAP-A 569g/682; MECH-439) -> "
        ...
        "config_summary": {
            ...
            "matched_stack": (
                ...
                "channel routing (cand_world_summary) + 569i TOP-K SHORTLIST conversion "
                "(use_modulatory_shortlist_then_modulate + mode=top_k + k=3) + "
            ...
            "modulatory_shortlist_mode": MODULATORY_SHORTLIST_MODE,
            "modulatory_shortlist_k": MODULATORY_SHORTLIST_K,
            "conversion_lever": "569i TOP-K SHORTLIST (V3-EXQ-569i PASS/supports; ARC-065 stable); shortlist OVERRIDES additive selection",
            "superseded_lever_not_armed": "ARM_STD_G2 standalone additive (569h FAIL 1/3); std-authority retained but not load-bearing",
...
def main() -> Tuple[Optional[str], Optional[str]]:
    parser = argparse.ArgumentParser(description="V3-EXQ-654g GAP-B falsifier (on the 569i-validated TOP-K shortlist conversion; supersedes 654f)")
