...
**Count as of 2026-09-07: 29 files.** (Recounted on this date; the previous
...
| `test_precondition_index_coupled_read_lint.py` | `precondition_index_read` | A precondition list is read BY POSITION (`preconditions[0]["measured"]`) instead of looked up by name. The list's ORDER is not part of its contract, so inserting a check -- a routine red-team repair -- silently re-points every positional read, with no error and no failing test. | Warn | V3-EXQ-993a: a red-team fix inserted `control_arm_coverage_complete` at index 0 before the run, so the landed manifest records `metrics.worst_harm_action_sensitivity` = 8.0 (the coverage COUNT) while the true value 0.11382 sat one slot along. Found by the fable red-team of `failure_autopsy_V3-EXQ-993a_2026-09-05.md` (hygiene H1). Corpus scan 2026-09-07 over 1465 drivers: 2 carriers before the forward fix, 1 after (`v3_exq_865_q081_zgoal_reach_preflight_scan.py:256`, `preconditions[-1]["met"]`, not retro-edited). |
